...
// \brief functions to help with SATA specific things. Printing out FIS,
// creating FIS, etc.
...
eReturnValues build_H2D_FIS_From_ATA_PT_Command(ptrSataH2DFis h2dFis, ataTFRBlock* ataPTCmd, uint8_t pmPort)
{
    ...
    h2dFis->byte1 =
        H2D_COMMAND_BIT_MASK | M_Nibble0(pmPort); // set the command bit so that the drive performs the
                                                  // command, rather than a soft reset. Also set port
                                                  // multiplier port so the command goes to the correct device
...
// NOTE: This prints some bit fields, so some architectures or compilers may not
// pack those in the same order. Check the actual byte data for real
// interpretation, then ifdef the structure definitions to correct it as errors
// are found - TJE
void print_FIS(void* fis, uint32_t fisLengthBytes)
{
    if (fis)
    {
        uint8_t* fisPtr = fis; // this is so that if using a fis structure, there
                               // aren't casting warnings calling the function
        // uint8_t pmPort = 0;//TODO: extract the pmPort value from byte 1 using the
        // nibble macro
        switch (fisPtr[0])
        {
        ...
        default: // unknown fis type/reserved/vendor unique
            ...
            // start loop at 1 since we've already printed the first byte describing
            // the type of FIS
