...
// All code in this file is from a JMicron USB to NVMe product specification for
                                            ...
                                            nvmeCmdCtx*             nvmCmd)
{
    ...
    // These bytes will be set depending on the size of the transfer for the
    // protocol
    ...
    case JM_PROTOCOL_SET_PAYLOAD:
        *cdbDataDirection = XFER_DATA_OUT;
        // setup the data payload, which may include a command to issue depending on
        ...
        else
        {
            ...
            // based on vendor ctrl value, we may setup a cmd, or leave those fields
            // blank to setup some other action
            ...
            if (jmCtrl == JM_VENDOR_CTRL_SERVICE_PROTOCOL_FIELD)
            {
                ...
                if (nvmCmd)
                {
                    if (nvmCmd->commandType == NVM_ADMIN_CMD)
                    {
                        ...
                        // NOTE: bytes 9, 10, 11 hold fused bits, prp vs sgl, and CID. None
                        // of these are filled in for now...-TJE NSID is 15:12
...
eReturnValues send_JM_NVMe_Cmd(nvmeCmdCtx* nvmCmd)
{
    ...
    // There may be some sense data outputs where the return response info won't
    // work or isn't necessary, but they don't seem documented today. Most likely
    // only for illegal requests.
    ...
    if (sendRet != OS_COMMAND_TIMEOUT)
    {
        ...
        {
            // first, check for the NVMe signature to make sure the correct response
            // is here.
            if (0 == memcmp(jmPayload, JMICRON_NVME_NAMESTRING, safe_strlen(JMICRON_NVME_NAMESTRING)))
            {
                ...
                nvmCmd->commandCompletionData.statusAndCID =
                    ...
                    << 17; // only the status field is returned so shift it into the
                           // place it's expected to be.
    ...
    if (senseDataIsAllWeGot)
    {
        ...
        // Didn't get NVMe response, so no judgements beyond sense data translation
        // can be made.
        ret = sendRet;
    }
    return ret;
