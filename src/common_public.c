...
bool is_Seagate_Model_Number_Vendor_F(tDevice *device, bool USBchildDrive)
{
    ...
    if (USBchildDrive)
    {
        if (
            ...
            ||
			((strstr(device->drive_info.bridge_info.childDriveMN, "ZA") != NULL)
				&& (find_last_occurrence_in_string(device->drive_info.bridge_info.childDriveMN, "GM") == 7))		// FireCuda 120
			||
    ...
    else
    {
        if (
            ...
            ||
			((strstr(device->drive_info.product_identification, "ZA") != NULL)
				&& (find_last_occurrence_in_string(device->drive_info.product_identification, "GM") == 7))					// BC125
			||
