...
void print_Low_Level_Info(tDevice* device)
{
    if (device)
    {
        ...
        // Anything operating system specific would also be useful to output in
        // here. Do this last and try to do common stuff up front.
        ...
        {
            // TODO: print ataoptions struct? This is things setup based on detection
            // from identify and helps with how some commands are built/issued
            printf("\t\t---ata flags---\n");
        }
        // TODO: Software SAT flags? These are currently always setup even when
        // software translator is not active.
        ...
        {
            printf("\t\t\t\t\tSECPROTI512 (security protocol command with inc512 bit "
                   "supported)\n");
        }
        ...
        {
            printf("\t\t\t\t\tWBND (HBA blocks SCSI write buffer - deferred download "
                   "modes)\n");
        }
        ...
        {
            printf("\t\t\t\t\tLIMPT (Limited passthrough capabilities. Only specific "
        ...
        {
            printf("\t\t\t\t\tSCTSM (SCT commands only compatible with smart "
                   "commands)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.a1NeverSupported)
        {
            printf("\t\t\t\t\tNA1 (A1h opcode, 12B SAT, is NOT supported by this "
                   "device at all)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.a1ExtCommandWhenPossible)
        {
            printf("\t\t\t\t\tA1EXT (Some 48 bit commands must be issued with A1h "
                   "opcode. Many limitations to this device)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.returnResponseInfoSupported)
        {
            printf("\t\t\t\t\tRS (SAT return response info protocol is supported for "
                   "determining command completion)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.returnResponseInfoNeedsTDIR)
        {
            printf("\t\t\t\t\tRSTD (SAT return response info TDIR bit can be set to "
                   "ensure proper interpretation of data direction)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.returnResponseIgnoreExtendBit)
        {
            printf("\t\t\t\t\tRSIE (SAT return response info data requires ignoring "
                   "the extend bit as it isn't handled properly)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.alwaysUseTPSIUForSATPassthrough)
        {
            printf("\t\t\t\t\tTSPIU (SAT commands must use the TSPIU transfer type "
                   "for all commands to work properly)\n");
        }
        ...
        {
            printf("\t\t\t\t\tFDMA (SAT dma commands MUST use protocol set to DMA "
                   "instead of UDMA-in/out)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.dmaNotSupported)
        {
            printf("\t\t\t\t\tNDMA (No DMA commands are supported on this adapter. "
                   "Must use PIO only)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.partialRTFRs)
        {
            printf("\t\t\t\t\tPARTRTFR (Only 28bit responses are available. All "
                   "extended registers are missing)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.noRTFRsPossible)
        {
            printf("\t\t\t\t\tNORTFR (It is impossible to get the drive's response. "
                   "Can only rely on SAT translation of errors if that is even "
                   "available)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.multiSectorPIOWithMultipleMode)
        {
            printf("\t\t\t\t\tMMPIO (Multi-sector PIO commands are only possible if "
                   "multiple mode configuration is done first)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.singleSectorPIOOnly)
        {
            printf("\t\t\t\t\tSPIO (Only single sector PIO commands are possible. "
                   "Any attempts at multiple-sectors will cause massive problems)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.ata28BitOnly)
        {
            printf("\t\t\t\t\tATA28 (Only 28bit ATA commands are possible on this "
                   "adapter)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.noMultipleModeCommands)
        {
            printf("\t\t\t\t\tNOMMPIO (Do not use multiple mode read/write commands "
                   "on this device. They are not handled correctly)\n");
        }
        ...
        {
            printf("\t\t\t\t\tTPID (TSPIU can be used on identify commands and possibly "
                   "a few others, but it cannot be used on every command)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.disableCheckCondition)
        {
            printf("\t\t\t\t\tNCHK (Do not use the check condition bit. It causes "
                   "problems on this system)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.checkConditionEmpty)
        {
            printf("\t\t\t\t\tCHKE (Check condition bit is accepted but sense data "
                   "is empty, so this bit is unusable)\n");
        }
        if (device->drive_info.passThroughHacks.ataPTHacks.possilbyEmulatedNVMe)
        {
            printf("\t\t\t\t\tPEMUNV (Adapter is possibly a USB to NVMe adapter that "
                   "responds to SAT ATA identify CDBs with only MN, SN, FW)\n");
        }
        // print out the os_info unique things. This has a lot of ifdefs for the
        // different OSs/configurations so need to watch out for the differences in
        // here
        printf("\t---OS Info---\n");
...
bool scan_Drive_Type_Filter(tDevice* device, uint32_t scanFlags)
{
    ...
    // if no filter flags are being used, then we need to just return true to show
    // the device
...
// this is the "generic" scan. It uses the OS defined "get device count" and
// "get device list" calls to do the scan.
void scan_And_Print_Devs(unsigned int flags, eVerbosityLevels scanVerbosity)
{
    ...
    // set the verbose flags to send onwards to the getDeviceList and
    // getDeviceCount functions.
            ...
            if (ret == SUCCESS || ret == WARN_NOT_ALL_DEVICES_ENUMERATED)
            {
                ...
                for (uint32_t devIter = 0; devIter < deviceCount; ++devIter)
                {
                    ...
                    if (csmiDeviceCountValid &&
                        devIter >= (deviceCount - csmiDeviceCount)) // if the csmi device count is valid then
                                                                    // we found some for the scan and need to
                                                                    // see if we need to check for duplicates.
                    {
                        ...
                        if (!(flags & ALLOW_DUPLICATE_DEVICE))
                        {
                            ...
                            {
                                // check if the WWN is valid (non-zero value) and then check if
                                // it matches anything else already in the list...this should be
                                // faster than the SN comparison below. - TJE
                                ...
                                // check if the SN is valid (non-zero length) and then check if
                                // it matches anything already seen in the list... - TJE
                                else if (safe_strlen(deviceList[devIter].drive_info.serialNumber) &&
...
#define SCAN_DISPLAY_HANDLE_STRING_LENGTH 256
...
#if defined(_WIN32)
                        snprintf(displayHandle, SCAN_DISPLAY_HANDLE_STRING_LENGTH, "%s",
                        ...
                        if ((flags & SG_TO_SD) > 0)
                        {
                            ...
                            if (SUCCESS == map_Block_To_Generic_Handle(displayHandle, &genName, &blockName))
                            {
                                ...
                                snprintf(displayHandle, SCAN_DISPLAY_HANDLE_STRING_LENGTH, "%s<->%s", genName,
                                         blockName);
                            }
                            safe_free(&genName);
                            safe_free(&blockName);
            ...
            {
                printf("Permission to access all devices was denied. Please check your "
...
static void set_IEEE_OUI(uint32_t* ieeeOUI, tDevice* device, bool USBchildDrive)
{
    ...
    case 2: // bytes 2,3,4
        *ieeeOUI = C_CAST(uint32_t, (wwn & UINT64_C(0x0000FFFFFF000000)) >> 24);
        break;
    case 5: // most common - ATA requires this and I think SCSI almost always
            // matches    bytes 0 - 3 (half of 0, half of 3) see SPC4 for details
    case 6: // same as NAA format 5
        *ieeeOUI = C_CAST(uint32_t, (wwn & UINT64_C(0x0FFFFFF000000000)) >> 36);
        break;
    default:
        // don't do anything since we don't have a way to parse it out of here or it
        // is a new format that wasn't defined when writing this
        break;
    }
}
...
bool is_Maxtor(tDevice* device, bool USBchildDrive)
{
    ...
    default:
        ...
        {
            // on a USB drive, check the child information as well as the bridge
            // information
    ...
    if (!isMaxtor)
    {
        ...
        if (!isMaxtor)
        {
            ...
            // if after a model number check, the result is still false and it's USB,
            // we need to check the child drive information just to be certain
...
bool is_Seagate_MN(char* string)
{
    ...
    if (stringLen > 0)
    {
        ...
        snprintf(localString, stringLen + 1, "%s", string);
        localString[stringLen] = '\0';
        // convert_String_To_Upper_Case(localString);//Removing uppercase
        // converstion, thus making this a case sensitive comparison to fix issues
        // with other non-Seagate products being detected as Seagate.
...
bool is_Seagate(tDevice* device, bool USBchildDrive)
{
    ...
    // This check should work well enough, but we do support checking the IEEE OUI
    // as well now. It must be set in the WWN correctly with the NAA field set to
    // 5h or 6h - TJE
...
{
    bool isLaCie = false;
    // LaCie drives do not have a IEEE OUI, so the only way we know it's LaCie is
    // to check the VendorID field reported by the device
...
bool is_Samsung_HDD(tDevice* device, bool USBchildDrive)
{
    ...
    if (!isSamsung && !isSSD)
    {
        // this fall back method should only be called on samsung HDD's and these
        // should only be really old ones without a WWN which should be a minority.
        // All drives with IEEE_SEAGATE_SAMSUNG_HDD should be caught long before
        // this we need to check the Vendor ID if SCSI or USB interface
        if (device->drive_info.interface_type == USB_INTERFACE ||
            (device->drive_info.interface_type == SCSI_INTERFACE && device->drive_info.drive_type != ATA_DRIVE))
        {
...
bool is_Vendor_A(tDevice* device, bool USBchildDrive)
{
    ...
    if (!isVendorA)
    {
        ...
        // should only be really old ones without a WWN which should be a minority.
        // All drives with IEEE_SEAGATE_SAMSUNG_HDD should be caught long before
        // this we need to check the Vendor ID if SCSI or USB interface if
        ...
        //{
        //    isVendorA =
        //    is_Vendor_A_String(device->drive_info.product_identification);
        //     //if after a model number check, the result is still false and it's
        //     USB, we need to check the child drive information just to be certain
        //     if (!isVendorA && device->drive_info.interface_type == USB_INTERFACE)
        //     {
        //         isVendorA =
        //         is_Vendor_A_String(device->drive_info.bridge_info.childDriveMN);
        //     }
        // }
    }
    return isVendorA;
...
eIronwolf_NAS_Drive is_Ironwolf_NAS_Drive(tDevice* device, bool USBchildDrive)
{
    ...
    if (safe_strlen(modelNumber))
    {
        ...
        else if (wildcard_Match("ST*NE*", modelNumber) || wildcard_Match("ST*NT*", modelNumber) ||
                 wildcard_Match("ST*ND*", modelNumber)) // check if Ironwolf Pro HDD
            isIronWolfNASDrive = IRONWOLF_PRO_NAS_DRIVE;
        else if (wildcard_Match("*ZA*NM*",
                                modelNumber)) // check if SATA Ironwolf SSD
            isIronWolfNASDrive = IRONWOLF_NAS_DRIVE;
        else if (wildcard_Match("*ZA*NX*",
                                modelNumber)) // check if SATA Ironwolf Pro SSD
            isIronWolfNASDrive = IRONWOLF_PRO_NAS_DRIVE;
        else if (wildcard_Match("*ZP*NM*",
                                modelNumber)) // check if PCIe Ironwolf SSD
            isIronWolfNASDrive = IRONWOLF_NAS_DRIVE;
    }
...
bool is_Firecuda_Drive(tDevice* device, bool USBchildDrive)
{
    ...
    if (safe_strlen(modelNumber))
    {
        ...
        else if (wildcard_Match("*ZA*GM*",
                                modelNumber)) // check if SATA Firecuda SSD
            isFirecudaDrive = true;
        else if (wildcard_Match("*ZP*GM*", modelNumber) || wildcard_Match("*ZP*GV*",
                                                                          modelNumber)) // check if PCIe Firecuda SSD
            isFirecudaDrive = true;
    }
...
eSkyhawk_Drive is_Skyhawk_Drive(tDevice* device, bool USBchildDrive)
{
    ...
    if (safe_strlen(modelNumber))
    {
        if (wildcard_Match("ST*VX*", modelNumber) || wildcard_Match("ST*HKVS*", modelNumber) ||
...
bool is_Nytro_Drive(tDevice* device, bool USBchildDrive)
{
    ...
    if (safe_strlen(modelNumber))
    {
        if (wildcard_Match("XS*SE*",
                           modelNumber)) // Nytro 3332, Nytro 3331, Nytro 2332
            isNytroDrive = true;
        else if (wildcard_Match("*XS*LE*",
                                modelNumber)) // Nytro 3532, Nytro 3531, Nytro 2532
            isNytroDrive = true;
...
bool is_Exos_Drive(tDevice* device, bool USBchildDrive)
{
    ...
    if (safe_strlen(modelNumber))
    {
        if (wildcard_Match("ST*NM*", modelNumber)) // Exos X-series
            isExosDrive = true;
        else if (wildcard_Match("*ST*MP*", modelNumber) || wildcard_Match("*ST*MM*", modelNumber) ||
        ...
        if (((strstr(device->drive_info.bridge_info.childDriveMN, "ST") != M_NULLPTR) &&
            ...
            ((strstr(device->drive_info.bridge_info.childDriveMN, "ZA") != M_NULLPTR) &&
             (find_last_occurrence_in_string(device->drive_info.bridge_info.childDriveMN, "NM10002") ==
              7)) // Vendor_F and Vendor_G has same model# except for last part, so
                  // need more chars for comparison
            || ((strstr(device->drive_info.bridge_info.childDriveMN, "ZA") != M_NULLPTR) &&
            ...
            /*The following are unique to some USB SSD products. These seem to all
             * report UHFS00.1 for firmware rev, but trying to only match UHFS for
             * now These drives also seem to set "Seagate SSD" for the MN in the
             * child drive info. Since they are USB, these should only be checked
             * like this as they are not being manufactured any other way for now. -
             * TJE
             */
...
bool is_Seagate_Vendor_K(tDevice* device)
{
    ...
    if (is_LaCie(device))
    {
        ...
        {
            // Already checked vendor ID, so check SCSI MN, then check ATA reported
            // info
...
eSeagateFamily is_Seagate_Family(tDevice* device)
{
    ...
    uint8_t        iter            = 0;
    uint8_t        numChecks       = 11; // maxtor, seagate, samsung, lacie, seagate-Vendor. As
                                         // the family of seagate drives expands, we will need
                                         // to increase this and add new checks
    for (iter = 0; iter < numChecks && isSeagateFamily == NON_SEAGATE; iter++)
    {
        ...
        case 0: // is_Samsung_HDD
            ...
            {
                // If this is an NVMe drive, we need to check if it's Seagate since both
                // Samsung HDD's and Seagate NVMe drives use the same IEEE OUI
                if (device->drive_info.drive_type == NVME_DRIVE)
                {
        ...
        case 3: // is_Vendor_A
            ...
            {
                // we aren't done yet! Need to check the model number to make sure it's
        ...
        case 7: // is_Miniscribe
            // TODO: figure out what model numbers would be reported for ATA/IDE so we
            // can detect them
        ...
        case 10: // is_Vendor_H - NVMe SSDs
            ...
            break;
            // TODO: Add in CDC, DEC, & PrarieTek detection. Currently not in since
            // these drives are even more rare than the Conner and Miniscribe
            // drives...
        default:
            break;
        }
...
bool is_SATA(tDevice* device)
{
    if (device->drive_info.drive_type == ATA_DRIVE)
    {
        // Word 76 will be greater than zero, and never 0xFFFF on a SATA drive (bit
        // 0 must be cleared to zero)
...
uint32_t get_Sector_Count_For_Read_Write(tDevice* device)
{
    ...
    case RAID_INTERFACE:
    case NVME_INTERFACE:
        // set the sector count for a 64k transfer. This is most compatible
        // (typically 128 sectors at a time-512B sector size) - TJE
        return DATA_64K / device->drive_info.deviceBlockSize;
    case USB_INTERFACE:
    case MMC_INTERFACE:
    case SD_INTERFACE:
    case IEEE_1394_INTERFACE:
        // set the sector count for a 32k transfer. This is most compatible on these
        // external interface drives since they typically have RAM limitations on
        // the bridge chip - TJE
        return DATA_32K / device->drive_info.deviceBlockSize;
    default:
        return 64; // just set something in case they try to use this value but
                   // didn't check the return code from this function - TJE
    }
}
...
uint32_t get_Sector_Count_For_4096B_Based_XFers(tDevice* device)
{
    ...
    default:
        return 8; // just set something in case they try to use this value but
                  // didn't check the return code from this function - TJE
    }
}
...
uint64_t align_LBA(tDevice* device, uint64_t LBA)
{
    ...
    {
        // make sure the incoming LBA is aligned to the start of the physical sector
        // it is in
...
static bool set_USB_Passthrough_Hacks_By_PID_and_VID(tDevice* device)
{
    ...
    if (device->drive_info.interface_type == USB_INTERFACE)
    {
        // Most USB bridges are SAT so they'll probably fall into the default cases
        // and issue an identify command for SAT
        ...
        case USB_Vendor_Seagate_RSS: // 0BC2
            ...
            case 0x0501: //
                ...
                device->drive_info.passThroughHacks.ataPTHacks.dmaNotSupported =
                    true; // TODO: Cypress passthrough has a bit for UDMA mode, but
                          // didn't appear to work in testing.
            ...
            case 0x2322: // Expansion (Xbox drive uses this chip)
                // NOTE: Need to rerun a full test someday as we don't have full
                // confirmation on supported read commands...just assumed same as above
                // chip for now.
                passthroughHacksSet                                                       = true;
                device->drive_info.passThroughHacks.passthroughType                       = ATA_PASSTHROUGH_SAT;
                device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure   = true;
            ...
            case 0xAB2A: // Fast HDD
                ...
                // = true;//Not sure if this actually worked when the test tool pumped
                // this result out. Off for now.
                device->drive_info.passThroughHacks.ataPTHacks.limitedUseTPSIU               = true;
                device->drive_info.passThroughHacks.ataPTHacks.alwaysCheckConditionAvailable = true;
                device->drive_info.passThroughHacks.ataPTHacks.maxTransferLength             = 130560;
                // This device has 2 HDDs in a RAID inside it.
                // There is no known way to address each one separately.
                // Also, for whatever reason, it is not possible to read any logs from
                // the drive you can get identify data from. This may be a
                // pre-production unit that was tested and other shipping products may
                // work differently. - TJE
                break;
            ...
            case 0xAA17: // FireCuda Gaming SSD
                // NOTE: Recommend a retest for this device to double check the hacks.
                // Most are setup based on other ASMedia bridge chip tests.
                passthroughHacksSet                                                     = true;
            ...
            default: // unknown
                ...
                device->drive_info.passThroughHacks.ataPTHacks.alwaysCheckConditionAvailable =
                    false; // turning this off because this causes some products to
                           // report "Invalid operation code" instead of "invalid field
                           // in CDB"
                device->drive_info.passThroughHacks.ataPTHacks.maxTransferLength = 130560;
                break;
            }
        ...
        case USB_Vendor_LaCie: // 059F
            ...
            case 0x1064: // RuggedKey
                // One other note: For some reason, the MaxLBA that is reported is
                // DIFFERENT between read capacty 10 and read capacity 16
                passthroughHacksSet                                                     = true;
                device->drive_info.passThroughHacks.passthroughType                     = PASSTHROUGH_NONE;
                device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure = true;
                device->drive_info.passThroughHacks.turfValue                           = 13;
            ...
            case 0x1107: // Mobile Secure
                // oddly I cannot get a security protocol in command to work on this
                // device (1107) despite how it is marketted. May need to recheck this
                // in the future in case this was a mislabbelled drive
                passthroughHacksSet                                                       = true;
                device->drive_info.passThroughHacks.passthroughType                       = ATA_PASSTHROUGH_SAT;
                device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure   = true;
                device->drive_info.passThroughHacks.turfValue                             = 34;
            ...
            case 0x1132: // LaCie Rugged Mini SSD
                ...
                device->drive_info.passThroughHacks.ataPTHacks.alwaysCheckConditionAvailable = true;
                device->drive_info.passThroughHacks.ataPTHacks.dmaNotSupported =
                    true; // This probably has more to do with only supporting Identify
                          // and SMART commands
                device->drive_info.passThroughHacks.ataPTHacks.maxTransferLength =
        ...
        case USB_Vendor_Maxtor: // 0D49
            ...
            case 0x5020: // 5000DV
                ...
                device->drive_info.passThroughHacks.scsiHacks.noReportSupportedOperations = true;
                // device->drive_info.passThroughHacks.scsiHacks.maxTransferLength =
                // 65536; //Unable to get a good test result on this, so this is
                // currently commented out. - TJE
                break;
        ...
        case USB_Vendor_Oxford: // 0928
            switch (device->drive_info.adapter_info.productID)
            {
            case 0x0008: // NOTE: This should be retested for full hacks to improve
                         // performance.
                device->drive_info.passThroughHacks.passthroughType = ATA_PASSTHROUGH_SAT;
                passthroughHacksSet                                 = true;
                break;
        ...
        case USB_Vendor_Initio: // 13FD
            ...
            case 0x1340: // Seen in ThermalTake BlackX
                ...
                // NOTE: This device doesn't allow for large LBAs, so it is limited on
                // the SCSI read/write commands to a 32bit LBA.
                //       Using a 12TB drive, this reports 483.72GB....so may need an
                //       additional hack at some point to force passthrough read/write
                //       commands on a device like this.
                device->drive_info.passThroughHacks.passthroughType                       = ATA_PASSTHROUGH_SAT;
                passthroughHacksSet                                                       = true;
                device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure   = true;
                device->drive_info.passThroughHacks.turfValue                             = 12;
        ...
        case USB_Vendor_ASMedia: // 174C
            ...
            case 0x2362: // USB to NVMe adapter
                // Tested 3 adapters. All report the exact same USB level information,
                // but do in fact work differently (slightly) The  difference seems to
                // be only noticable in Inquiry Data
                // 1. PID: "ASM236X NVME" could use mode sense 6 and read 6 commands
                // 2. PID: "USB3.1 TO NVME" could NOT do these commands. So setting the
                // lowest common set of rules. If there are other ASMedia chips that
                // vary in capabilities, then may need to adjust what is done in here,
                // or add a hack to check INQ data to finish setting up remaining hacks
                passthroughHacksSet                                 = true;
            ...
            case 0x5106: // Seen in ThermalTake BlackX 5G
                ...
                // 7/29/2021 - retested manually. TPSIU seems to work better only for
                // identify. UDMA mode definitely doesn't work.
                //             but DMA mode works fine. Remaining hacks seem appropriate
                //             overall
                device->drive_info.passThroughHacks.passthroughType                     = ATA_PASSTHROUGH_SAT;
                passthroughHacksSet                                                     = true;
                device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure = true;
                ...
                device->drive_info.passThroughHacks.ataPTHacks.smartCommandTransportWithSMARTLogCommandsOnly =
                    true; // NOTE: Can use PIO read log ext just fine, but DMA is the
                          // problem. THis still works with SMART though, but a future
                          // hack may be needed.-TJE
                device->drive_info.passThroughHacks.ataPTHacks.alwaysUseDMAInsteadOfUDMA = true;
                device->drive_info.passThroughHacks.ataPTHacks.maxTransferLength         = 524288;
                break;
        ...
        case USB_Vendor_JMicron: // 152D
            ...
            case 0x0567: // Github user reported USB to SATA adapter
                ...
                device->drive_info.passThroughHacks.scsiHacks.noLogSubPages             = true;
                device->drive_info.passThroughHacks.scsiHacks.noModeSubPages            = true;
                device->drive_info.passThroughHacks.scsiHacks.reportAllOpCodes =
                    true; // this may need better validation
                ...
                device->drive_info.passThroughHacks.ataPTHacks.disableCheckCondition =
                    true; // this does not crash the bridge, just useless as it's
                          // empty...just setting this as well for consistency since
                          // return reponse info works.
            ...
            case 0x2338: // Sabrent USB 2.0 to SATA/PATA. Only tested SATA.
                ...
                device->drive_info.passThroughHacks.ataPTHacks.maxTransferLength               = 122880;
                break;
            case 0x2339: // MiniD2 - NOTE: This has custom firmware. If other things
                         // use this chip, additional product verification will be
                         // necessary.
                device->drive_info.passThroughHacks.passthroughType                       = ATA_PASSTHROUGH_SAT;
                passthroughHacksSet                                                       = true;
                device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure   = true;
        ...
        case USB_Vendor_Realtek: // 0BDAh
            ...
            case 0x9210: // USB to SATA OR USB to NVMe
                ...
                // When using with an NVMe device, the same SCSI rules apply, however it
                // will also respond to SAT ATA identify with NVMe MN, SN, FW being
                // returned, but nothing else is valid. So somehow this needs to set a
                // "possibly NVMe" flag somewhere. this is being setup as through it is
                // an ATA drive for now since there is not other good way to figure out
                ...
                device->drive_info.passThroughHacks.scsiHacks.noModeSubPages =
                    true; // this supports some mode pages, but unable to test for
                          // subpages, so considering them not supported at this time
                          // -TJE
                device->drive_info.passThroughHacks.scsiHacks.noReportSupportedOperations = true;
                // NOTE: Security protocol is supported according to online web page. I
                // do not have a device supporting security to test against at this time
                // to see if INC512 is needed/required -TJE
                device->drive_info.passThroughHacks.scsiHacks.securityProtocolSupported = true;
                ...
                // to try an SCT command (write log) to see how check condition works,
                // but at least with reads, this is a no-go
                device->drive_info.passThroughHacks.scsiHacks.noSATVPDPage = true;
                device->drive_info.passThroughHacks.ataPTHacks.alwaysUseTPSIUForSATPassthrough =
                    true; // seems to make no difference whether this is used or not.
                          // Can switch this to "limited use" if we need to
                device->drive_info.passThroughHacks.ataPTHacks.singleSectorPIOOnly = true;
        ...
        case USB_Vendor_Samsung: // 04E8
            ...
            case 0x1F05: // S2 Portable
                ...
                // Serial number is not reported in inquiry data or any other known
                // location
                break;
            case 0x5F12: // Story Station
                passthroughHacksSet = true;
                // hacks based on revision 1302h. Not sure if revision level filter is
                // needed right now Set ATA passthrough hacks
                device->drive_info.passThroughHacks.passthroughType = ATA_PASSTHROUGH_SAT;
                // device->drive_info.passThroughHacks.ataPTHacks.useA1SATPassthroughWheneverPossible
                // = true;
                device->drive_info.passThroughHacks.ataPTHacks.returnResponseInfoSupported   = true;
        ...
        case USB_Vendor_Silicon_Motion: // 090C
            ...
            case 0x1000: // USB DISK - Rev1100
                // Don't set a passthrough type! This is a USB flash memory, that
                // responds to one of the legacy command requests and it will break it!
                device->drive_info.media_type                       = MEDIA_SSM_FLASH;
                device->drive_info.passThroughHacks.passthroughType = PASSTHROUGH_NONE;
                passthroughHacksSet                                 = true;
                // this device also supports the device identification VPD page even
                // though the list of pages doesn't work.
                device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure = true;
                device->drive_info.passThroughHacks.turfValue                           = 15;
                device->drive_info.passThroughHacks.scsiHacks.unitSNAvailable           = true;
                device->drive_info.passThroughHacks.scsiHacks.noVPDPages                = true;
        ...
        case USB_Vendor_SanDisk_Corp: // 0781
            ...
            case 0x5580: // Extreme
                passthroughHacksSet           = true;
                device->drive_info.media_type = MEDIA_SSM_FLASH; // Leaving this as flash since it is a flash
                                                                 // drive/thumb drive, but this is an odd one that
                                                                 // seems to do SAT commands.
                device->drive_info.passThroughHacks.passthroughType                          = ATA_PASSTHROUGH_SAT;
                device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure      = true;
                device->drive_info.passThroughHacks.turfValue                                = 15;
                device->drive_info.passThroughHacks.scsiHacks.readWrite.available            = true;
        ...
        case USB_Vendor_Symwave: // 1CA1
            ...
            case 0x18AE: // rev 0852
                // This is a USB to SAS adapter. Not everything works as it is targetted
                // at read/write and other basic informatio only. If sent commands it
                // doesn't understand, this struggles. Very basic information is set
                // here in order to keep this simple. The main reason is because most
                // capabilties will depend on the Attached SAS device more than the USB
                // adapter itself. The exception to that rule is transfer size.
                // Disabling ATA passthrough test also helps
                passthroughHacksSet = true;
                device->drive_info.passThroughHacks.passthroughType =
                    PASSTHROUGH_NONE; // This disabled ATA passthrough which this
                                      // devices doesn't support anyways
                device->drive_info.passThroughHacks.scsiHacks.maxTransferLength = 1048576;
                device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure =
                    true; // This may not actually help this device. This hack is
                          // usually used to clear error conditions by sending a well
                          // known, easy command to clear previous SAT translation
                          // issues, but in this case, this may not be helpful
                device->drive_info.passThroughHacks.turfValue = 17;
                break;
            default:
                break;
        ...
        case USB_Vendor_Kingston_Generic:
            switch (device->drive_info.adapter_info.productID)
            {
            case 0x7777: // USB to M.2 SATA adapter...crashes without any recovery
                         // very easily.
                // This does NOT like TPSIU for ANYTHING but identify and crashes if
                // using UDMA mode.
                passthroughHacksSet                                                     = true;
                device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure = true;
                device->drive_info.passThroughHacks.turfValue                           = 34;
                ...
                device->drive_info.passThroughHacks.ataPTHacks.alwaysCheckConditionAvailable = true;
                device->drive_info.passThroughHacks.ataPTHacks.dmaNotSupported =
                    true; // Attempting any DMA passthrough fails for invalid field in
                          // CDB. UDMA mode crashes the firmware entirely.
                ...
                device->drive_info.passThroughHacks.ataPTHacks.checkConditionEmpty =
                    true; // returns empty sense data in return for check condition,
                          // when a result is expected to come back
                break;
            default:
                break;
            }
...
static bool set_IEEE1394_Passthrough_Hacks_By_PID_and_VID(tDevice* device)
{
    ...
    if (device->drive_info.interface_type == IEEE_1394_INTERFACE)
    {
        ...
        // Some devices had both USB and IEEE1394 interfaces and one may have
        // passthrough while the other does not. They may even be different.
        switch (device->drive_info.adapter_info.vendorID)
        {
        ...
        case IEEE1394_Vendor_Quantum: // 00E09E
            ...
            break;
        case 0x000BC2: // This vendor ID doesn't make sense for the product that was
                       // tested!!!
            switch (device->drive_info.adapter_info.productID)
            {
            case 0x000000: // Seagate external drive with USB and 1394. USB mode has
                           // passthrough, but that doesn't work in 1394. Likely 2
                           // different chips
                passthroughHacksSet                                 = true;
                device->drive_info.passThroughHacks.passthroughType = PASSTHROUGH_NONE;
                // device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure
                ...
                device->drive_info.passThroughHacks.scsiHacks.noReportSupportedOperations = true;
                device->drive_info.passThroughHacks.scsiHacks.maxTransferLength =
                    524288; // Unable to get a good test result on this, so this is
                            // currently commented out. - TJE
                break;
            default:
                break;
            ...
            break;
        case 0x000500: // This vendor ID doesn't make sense for the product that was
                       // tested!!!
            switch (device->drive_info.adapter_info.productID)
            {
            case 0x000001: // Seagate external drive with USB and 1394. USB mode has
                           // passthrough, but that doesn't work in 1394. Likely 2
                           // different chips
                passthroughHacksSet                                 = true;
                device->drive_info.passThroughHacks.passthroughType = PASSTHROUGH_NONE;
...
static bool set_PCI_Passthrough_Hacks_By_PID_and_VID(tDevice* device)
{
    ...
    //       This may be ok in general, but may get confusing since this isn't
    //       necessarily the target drive having an issue, but the hardware
    //       controller that a drive is attached to having some other
    //       functionality. The hacks list started as specific to USB, but needs
    //       to handle some things for PCIe controllers too.
    if (device->drive_info.adapter_info.vendorIDValid)
    {
        switch (device->drive_info.adapter_info.vendorID)
        {
        case PCI_VENDOR_RED_HAT:
            // note: these all appear to be virtual devices from what I can find
            // online. Only handling those that this software is likely to encounter
            // for now.
            switch (device->drive_info.adapter_info.productID)
            {
            case 0x0008: // reported in openSeaChest#47
                // TODO: This seemed to only allow identify and smart through. May need
                // a more thorough test for this in the future, but for now listing the
                // ata28bit only hack
                //       it is possible that even withing the A1h CDB other ATA commands
                //       will also be filtered and additional hacks will be required.
                passthroughHacksSet                                         = true;
                device->drive_info.passThroughHacks.ataPTHacks.ata28BitOnly = true;
                break;
            default:
        ...
        case PCI_VENDOR_MICROCHIP: // PMC?
            ...
            case 0x8070:
                ...
                //     minor ver : 1
                //     revision : 37
                // Unknown if these hacks are specific to the controller or the driver,
                // but for now applying everywhere when it is detected
                passthroughHacksSet = true;
                device->drive_info.passThroughHacks.scsiHacks.noReportSupportedOperations =
                    true; // This command seems to abort or cause an internal error for
                          // some reason, so turning it off.
                // none of the report supported operation codes modes work on this
                // controller. I do not have one of these to do a full test so this
                // workaround list is definitely incomplete.
                device->drive_info.passThroughHacks.scsiHacks.writeBufferNoDeferredDownload = true;
                // This controller/driver absolutely monitors the mode field in write
                // buffer and blocks the deferred download modes, but regular old
                // segmented works fine.-TJE
                break;
            default:
                break;
            }
        ...
        //         //Adaptec controllers in 8 series definitely require ATA
        //         passthrough commands to use DMA mode instead of UDMA mode. Have
        //         seen this on the ASR8405
        //         //Right now this is handled with retries in other parts of the
        //         code
        //         //We should run a more thorough test before turning on hacks here
        //         to make sure it covers all capabilities.-TJE
        //         //Another known SAT thing is that soft-reset works, but hard
        //         reset does not. break;
        //     }
        //     break;
        default:
...
bool setup_Passthrough_Hacks_By_ID(tDevice* device)
{
    ...
    {
        // The OS didn't report the vendor and product identifiers needed to set
        // hacks this way. Anything else will be done based on known Product matches
        // or trial and error
    }
    return success;
}
