...
{
    return false; // Documentation does not state if an infinite timeout is
                  // supported. If it actually is, need to define the infinite
                  // timeout value properly, and set it to the correct value
}
...
//       We use the rdsk for issuing passthrough commands, but the other handle
//       is probably what will really be needed to do this correctly I do not
//       have a solaris machine or VM available to test this right now, so it's
//       written as best I could to prevent needing changes. - TJE
static int get_Partition_Count(const char* blockDeviceName)
{
    int   result = 0;
    FILE* mount  = fopen("/etc/mnttab",
                         "r"); // we only need to know about mounted partitions.
...
static eReturnValues get_Partition_List(const char* blockDeviceName, ptrsPartitionInfo partitionInfoList, int listCount)
{
    ...
    if (listCount > 0)
    {
        FILE* mount = fopen("/etc/mnttab",
                            "r"); // we only need to know about mounted partitions.
                                  // Mounted partitions need to be known so that
                                  // they can be unmounted when necessary. - TJE
        if (mount)
        {
            struct mnttab entry;
            memset(&entry, 0, sizeof(struct mnttab));
...
static eReturnValues set_Device_Partition_Info(tDevice* device)
{
    ...
    // note: this mess above is to get rid of /rdsk/ in the file handle as that
    // raw disk handle won't be part of the information in the mount tab file.
...
eReturnValues get_Device(const char* filename, tDevice* device)
{
    ...
    device->os_info.minimumAlignment = sizeof(void*); // setting to be compatible with certain aligned memory
...
eReturnValues send_IO(ScsiIoCtx* scsiIoCtx)
{
    ...
    case RAID_INTERFACE:
        ...
        break;
    case NVME_INTERFACE:
        // haven't defined a way to send NVME commands yet. Need to add this in
        // later...-TJE
        ret = send_uscsi_io(scsiIoCtx);
        break;
    default:
...
static int uscsi_filter(const struct dirent* entry)
{
    ...
    // now, we need to filter out the device names that have "p"s for the
    // partitions and "s"s for the slices
