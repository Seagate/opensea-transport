...
// pointer to NVMe controller identify data should always be 4096B since all
// NVMe identify data is this long. All parameters should be the length of what
// is required in NVMe spec + 1 for a M_NULLPTR terminating character.
...
eReturnValues fill_In_NVMe_Device_Info(tDevice* device)
{
    ...
    // this so that on USB, we can store some info about the child drive, without
    // disrupting the standard drive_info that has already been filled in by the
    ...
    // If not an NVMe interface, such as USB, then we need to store things
    // differently RAID Interface should be treated as "Native" or
    // "NVME_INTERFACE" since there is likely an underlying API providing direct
    // access of some kind.
    ...
    if (ret == SUCCESS)
    {
        ...
        device->drive_info.media_type = MEDIA_NVM; // This will bite us someday when someone decided to put
                                                   // non-ssds on NVMe interface.
        // set scsi version to 6 if it is not already set
        if (device->drive_info.scsiVersion == 0)
        {
            device->drive_info.scsiVersion = 6; // most likely this is what will be set by a translator and keep
                                                // other parts of code working correctly
        }

        // Do not overwrite this with non-NVMe interfaces. This is used by USB to
        // figure out and track bridge chip specific things that are stored in this
        // location
        ...
        // set the IEEE OUI into the WWN since we use the WWN for detecting if the
        // drive is a Seagate drive. This is a shortcut to verify this is a Seagate
        // drive later. There is a SCSI translation whitepaper we can follow, but
        // that will be more complicated of a change due to the different formatting
        // used.
        *fillWWN = M_BytesTo8ByteValue(0x05, ctrlData->ieee[2], ctrlData->ieee[1], ctrlData->ieee[0], 0, 0, 0, 0) << 4;
        ...
        if (ret == SUCCESS)
        {
            ...
            // get the LBAF number. THis field varies depending on other things
            // reported by the drive in NVMe 2.0
            if (NVME_0_BASED(nsData->nlbaf) > 16)
            {
                // need to append 2 more bits to interpret this correctly since number
                // of formats > 16
            ...
            *fillMaxLba = nsData->nsze - 1; // spec says this is from 0 to (n-1)!
            ...
            if (ctrlData->lpa & BIT5 && ctrlData->ctratt & BIT4 && enduranceGroup > 0)
            {
                ...
                // First read the supported logs log page, then if the rotating media
                // log is there, read it.
                ...
                if (supportedLogs)
                {
                    ...
                    if (SUCCESS == nvme_Get_Log_Page(device, &supLogs))
                    {
                        ...
                        if (rotMediaSup & BIT0)
                        {
                            ...
                            // Set the media type because this is supported, at least for now.
                            // We can read the log and the actual rotation rate if needed.
