 ...
 *
 *                VMK error code name                        Description Unix
 * name
 *                ===================                        ===========
 * =========
 */
...
#define VMK_ERROR_CODES                                                                                                \
    ...
    DEFINE_VMK_ERR(VMK_MEM_MINLIMIT_LT_RESERVED, "Memory minLimit less than memory already reserved by children",      \
                   ENOSPC)                                                                                             \
    ...
    DEFINE_VMK_ERR(VMK_NFS_NFS_NOT_SUPPORTED, "The NFS server does not support requested NFS version over TCP",        \
                   EINVAL)                                                                                             \
    ...
    DEFINE_VMK_ERR(VMK_MIGRATE_PREEMPTIVE_FAIL, "Migration failed to start due to lack of CPU or memory resources",    \
                   ENOMEM)                                                                                             \
    ...
    DEFINE_VMK_ERR(VMK_TOO_MANY_LOCK_HOLDERS,                                                                          \
                   "Maximum number of concurrent hosts are already accessing "                                         \
                   "this resource",                                                                                    \
                   EUSERS)                                                                                             \
    ...
    DEFINE_VMK_ERR(VMK_MEM_MIN_LT_CONSUMED, "Memory min less than memory required to support current consumption",     \
                   ENOSPC)                                                                                             \
    DEFINE_VMK_ERR(VMK_MEM_MAX_LT_CONSUMED, "Memory max less than memory required to support current consumption",     \
                   ENOSPC)                                                                                             \
    ...
    DEFINE_VMK_ERR(VMK_MIGRATE_PRECOPY_NO_FORWARD_PROGRESS,                                                            \
                   "Migration failed due to lack of pre-copy forward progress", EINVAL)                                \
    ...
    DEFINE_VMK_ERR(VMK_FS_LOCKSTATE_IN_TRANSITION_DEPRECATED, "Lock state is in transition (ok to retry) DEPRECATED",  \
                   EBUSY)                                                                                              \
    ...
    DEFINE_VMK_ERR(VMK_HBR_WIRE_REQUEST_CHECKSUM_MISMATCH, "The checksum for the replication request was invalid",     \
                   EINVAL)                                                                                             \
    DEFINE_VMK_ERR(VMK_HBR_WIRE_RESPONSE_CHECKSUM_MISMATCH, "The checksum for the replication response was invalid",   \
                   EINVAL)                                                                                             \
    DEFINE_VMK_ERR(VMK_HBR_WIRE_GROUP_REMOVED, "The replication group was removed on the server side", ENOENT)         \
    DEFINE_VMK_ERR(VMK_HBR_WIRE_GROUP_SESSION_REVOKED,                                                                 \
                   "A newer client for this group is connected to the replication server", EEXIST)                     \
    ...
    DEFINE_VMK_ERR(VMK_MODULE_CONSUMED_RESOURCE_COUNT_NOT_ZERO, "Consumed resource count of module is not zero",       \
                   EBUSY)                                                                                              \
    DEFINE_VMK_ERR(VMK_HBR_SERVER_DOES_NOT_SUPPORT_REQUEST, "vSphere Replication Server does not support request",     \
                   EOPNOTSUPP)                                                                                         \
...
// #define DEFINE_VMK_ERR_AT(_err, _str, _val, _uerr) /** \brief _str */ _err =
// _val,
// /** \endcond */
// typedef enum {
//    VMK_ERROR_CODES
