...
// generic function to handle taking in the various RAID FW Requests to keep
// code from being dumplicated
                                           ...
                                           uint32_t* returnCode)
{
    ...
    if (device)
    {
        ...
        if (raidFirmwareRequest)
        {
            ...
            // Next fill in IOCTL_RAID_FIRMWARE_BUFFER, then work down from there and
            // memcpy the input data to this function since it will have the specific
            // download function data in it
            ...
            else
            {
                ...
                // may need to add in remaining scsi address in the future, but for now
                // these other fields are reserved
            }
            ...
            if (ptrDataRequest)
            {
                // NOTE: The offset should be a multiple of sizeof(void), which reading
                // the structure types indicated that this should be the case for 64bit
                // and 32bit builds. Anything else will need additional byte padding.
            ...
            if (ERROR_IO_PENDING == device->os_info.last_error) // This will only happen for overlapped commands. If
                                                                // the drive is opened without the overlapped flag,
                                                                // everything will work like old synchronous
                                                                // code.-TJE
            {
            ...
            else
            {
                ...
                ret = SUCCESS; // IO sent successfully in the system...BUT we need to
                               // check the SRB return code to determine if the command
                               // went through to the device
    ...
    PINTEL_STORAGE_FIRMWARE_INFO_V2 firmwareInfo =
        C_CAST(PINTEL_STORAGE_FIRMWARE_INFO_V2,
               safe_calloc(allocationSize,
                           sizeof(uint8_t))); // alignment not needed since this is passed to
                                              // another function where it will be copied as needed
    if (firmwareInfo)
    {
        uint32_t flags      = 0;
...
// The idea with this function is that it can handle NVMe or SCSI with generic
// inputs that will work to reduce code
                                                           ...
                                                           uint32_t  timeoutSeconds)
{
    ...
    if (device && imagePtr)
    {
        ...
        PINTEL_STORAGE_FIRMWARE_DOWNLOAD_V2 download = C_CAST(
            PINTEL_STORAGE_FIRMWARE_DOWNLOAD_V2, safe_calloc(allocationSize,
                                                             sizeof(uint8_t))); // alignment not needed since this will
                                                                                // get copied to an aligned location
        if (download)
        {
            ...
            download->ImageSize  = imageDataLength; // TODO: Not sure if this is supposed to be the same
                                                    // or different from the buffersize listed above
                                                           ...
                                                           uint32_t  timeoutSeconds)
{
    ...
    if (device)
    {
        ...
        PINTEL_STORAGE_FIRMWARE_ACTIVATE activate       = C_CAST(
            PINTEL_STORAGE_FIRMWARE_ACTIVATE, safe_calloc(allocationSize,
                                                                sizeof(uint8_t))); // alignment not needed since this will
                                                                                   // get copied to an aligned location
        if (activate)
        {
...
// This must also check to ensure that the alignment requirements and transfer
// length requirements are also met This is really only needed on SCSI or ATA
// since NVMe will only take this API to update firmware
static bool is_Compatible_SCSI_FWDL_IO(ScsiIoCtx* scsiIoCtx, bool* isActivate)
{
    bool     compatible          = false;
    uint32_t transferLengthBytes = 0;
    // check if this is a SCSI Write buffer command or ATA download microcode. Can
    // only support deferred and activate subcommands.
    ...
    if (compatible)
    {
        // before we call it a supported command, we need to validate if this meets
        // the alignment requirements, etc
...
eReturnValues send_Intel_Firmware_Download(ScsiIoCtx* scsiIoCtx)
{
    ...
    {
        uint32_t returnCode = 0;   // will most likely help with debugging, but we should check this to
                                   // dummy up sense data or rtfrs as necessary.
        uint32_t flags        = 0; // flags aren't required
        uint32_t timeout      = scsiIoCtx->timeout;
        uint8_t  firmwareSlot = 0; // NOTE: Some intel documentation suggests that this should be
                                   // INTEL_STORAGE_FIRMWARE_INFO_INVALID_SLOT, but we should test
                                   // before using that
        // special case, if running in SCSI translation mode for NVMe, we should set
        // the controller flag
        ...
        if (isActivate)
        {
            ...
            // set up the things we need to input to the activate from SCSI or ATA
            // info as appropriate
            ...
            {
                // assume SCSI write buffer if we made it this far. The
                // is_Compatible_SCSI_FWDL_IO will filter out other commands since the
                // opcode won't match
...
// NOTE: This function will handle calling appropriate NVMe firmware update
// function as well NOTE2: This will not issue whatever command you want. Only
// certain commands are supported by the driver. This function will attempt any
// command given in case driver updates allow other commands in the future.
static eReturnValues send_Intel_NVM_Passthrough_Command(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    if (nvmeIoCtx)
    {
        ...
        if (nvmPassthroughCommand)
        {
            ...
            // srb_io_control has been setup, now to main struct fields (minus data
            // which is set when configuring the NVMe command)
            nvmPassthroughCommand->Version = NVME_PASS_THROUGH_VERSION;
            // setup the SCSI address, pathID is only part needed at time of writing,
            // the rest is currently reserved, so leave as zeros pathId must be from
            // the SCSI address in non-raid mode, or CSMI port number, so this is
            ...
            case XFER_DATA_IN:
                ...
                nvmPassthroughCommand->Parameters.DataBufferOffset =
                    offsetof(NVME_IOCTL_PASS_THROUGH,
                             data); // NOTE: This must be DWORD aligned. The structure
                                    // being used SHOULD be, but if things aren't
                                    // working, this could be the problem
                break;
            case XFER_NO_DATA:
            ...
            if (ERROR_IO_PENDING == nvmeIoCtx->device->os_info.last_error) // This will only happen for overlapped
                                                                           // commands. If the drive is opened without
                                                                           // the overlapped flag, everything will work
                                                                           // like old synchronous code.-TJE
            {
                success = GetOverlappedResult(handleToUse, &overlappedStruct, &bytesReturned, TRUE);
            }
            ...
            else
            {
                ...
                {
                case INTEL_SRB_STATUS_SUCCESS:
                    // TODO: Handle unsupported commands error codes and others that don't
                    // fill in the completion data
                    //       This may not come into this SUCCESS case and instead may need
                    //       handling elsewhere.
...
static void dummy_Up_NVM_Status_FWDL(nvmeCmdCtx* nvmeIoCtx, uint32_t returnCode)
{
    ...
    case INTEL_FIRMWARE_STATUS_UNCORRECTABLE_DATA_ERROR:
        nvmeIoCtx->commandCompletionData.dw3Valid = true;
        nvmeIoCtx->commandCompletionData.dw3      = DUMMY_NVME_STATUS(NVME_SCT_MEDIA_AND_DATA_INTEGRITY_ERRORS,
                                                                      0x81); // unrecovered read error...not sure what else
                                                                             // to set if this happens-TJE
        break;
    case INTEL_FIRMWARE_STATUS_POWER_CYCLE_REQUIRED:
    case INTEL_FIRMWARE_STATUS_MEDIA_CHANGE:
    case INTEL_FIRMWARE_STATUS_MEDIA_CHANGE_REQUEST:
...
eReturnValues send_Intel_NVM_Firmware_Download(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    if (nvmeIoCtx)
    {
        if (nvmeIoCtx->commandType == NVM_ADMIN_CMD)
        {
            ...
            uint32_t returnCode   = 0;
            uint8_t  firmwareSlot = INTEL_STORAGE_FIRMWARE_INFO_INVALID_SLOT; // Starting with this...we
            ...
            if (nvmeIoCtx->cmd.adminCmd.opcode == NVME_ADMIN_CMD_DOWNLOAD_FW)
            {
                ...
                // NOTE: CDW10 and 11 are numbers of dwords! So shift by 2 (mul by 4) is
                // required as this function is expecting a byte offset
            ...
            else if (nvmeIoCtx->cmd.adminCmd.opcode == NVME_ADMIN_CMD_ACTIVATE_FW)
            {
                ...
                {
                    // Activate actions 2, & 3 sound like the closest match to this flag.
                    // Each of these requests switching to the a firmware already on the
                    // drive. Activate action 0 & 1 say to replace a firmware image in a
