...
static eReturnValues get_Win_Device(const char* M_NONNULL filename, tDevice* M_NONNULL device)
{
    ...
    else
    {
        ...
        char* tempFriendlyName = &tempFriendlyNameBuf[0];
...
static eReturnValues open_Force_Unit_Access_Handle_For_OS_Read_OS_Write(tDevice* device)
{
    ...
    if (device->os_info.forceUnitAccessRWfd == M_NULLPTR || device->os_info.forceUnitAccessRWfd == INVALID_HANDLE_VALUE)
    {
        ...
        if (0 < _stprintf_s(fuaDevice, WIN_MAX_DEVICE_NAME_LENGTH, TEXT("%hs"), device->os_info.name))
        {
            return MEMORY_FAILURE;
        }
...
eReturnValues os_Read(const tDevice* device, uint64_t lba, bool forceUnitAccess, uint8_t* ptrData, uint32_t dataSize)
{
    ...
    set_Last_Command_Time_To_tDevice(M_CONST_CAST(tDevice*, device), get_Nano_Seconds(commandTimer));
