...
#if defined(ENABLE_CSMI) // when this is enabled, the "get_Device",
                         // "get_Device_Count", and "get_Device_List" will also
                         // check for CSMI devices unless a flag is given to
                         // ignore them. For apps only doing CSMI, call the csmi
...
// If this returns true, a timeout can be sent with INFINITE_TIMEOUT_VALUE
// definition and it will be issued, otherwise you must try
// MAX_CMD_TIMEOUT_SECONDS instead
bool os_Is_Infinite_Timeout_Supported(void)
{
    return false;
}

// MinGW may or may not have some of these, so there is a need to define these
// here to build properly when they are otherwise not available. as mingw
// changes versions, some of these below may be available. Need to have a way to
// check mingw preprocessor defines for versions to work around these. NOTE: The
// device property keys are incomplete in mingw. Need to add similar code using
// setupapi and some sort of ifdef to switch between for VS and mingw to resolve
// this better.
...
#        if WINVER >= SEA_WIN32_WINNT_WINBLUE // 8.1 introduced NVMe
    case BusTypeNvme:
        printf("NVMe");
        break;
#            if WINVER >= SEA_WIN32_WINNT_WIN10 // Win10 API kits may have more or less of these bus
                                                // types so need to also check which version of the
                                                // Win10 API is being targetted
...
// This function is only used in get_Adapter_IDs which is why it's here. If this
// is useful for something else in the future, move it to opensea-common. static
...
static bool get_IDs_From_TCHAR_String(DEVINST instance, TCHAR* buffer, size_t bufferLength, tDevice* device)
{
    ...
    // here is where we need to parse the USB VID/PID or TODO: PCI Vendor,
    ...
    if (_tcsncmp(TEXT("USB"), buffer, _tcsclen(TEXT("USB"))) == 0)
    {
        ...
        // This is a hack around how VS2013 handles string concatenation with how
        // the printf format macros were defined for it versus newer versions.
        int scannedVals =
        ...
        // We can do this other property request to read it, but it's wide
        // characters only. No TCHARs allowed.
        ...
        if (CR_SUCCESS == cmRet || CR_INVALID_POINTER == cmRet ||
            CR_BUFFER_SMALL == cmRet) // We'll probably get an invalid pointer or small buffer, but
                                      // this will return the size of the buffer we need, so allow
                                      // it through - TJE
        {
            PBYTE propertyBuf = C_CAST(PBYTE, safe_calloc(propertyBufLen + 1, sizeof(BYTE)));
            if (propertyBuf)
            {
                propertyBufLen += 1;
                // NOTE: This key contains all 3 parts, VID, PID, and REV from the
                // "parentInst": Example: USB\VID_174C&PID_2362&REV_0100
    ...
    else if (_tcsncmp(TEXT("PCI"), buffer, _tcsclen(TEXT("PCI"))) == 0)
    {
        ...
        // can also read DEVPKEY_Device_HardwareIds for parentInst to get all this
        // data
    }
    else if (_tcsncmp(TEXT("1394"), buffer, _tcsclen(TEXT("1394"))) == 0)
    {
        // Parent buffer already contains the vendor ID as part of the buffer where
        // a full WWN is reported...we just need first 6 bytes. example, brackets
        // added for clarity: 1394\Maxtor&5000DV__v1.00.00\[0010B9]20003D9D6E
        // DEVPKEY_Device_CompatibleIds gets use 1394 specifier ID for the device
        // instance DEVPKEY_Device_CompatibleIds gets revision and specifier ID for
        // the parent instance: 1394\<specifier>&<revision>
        // DEVPKEY_Device_ConfigurationId gets revision and specifier ID for parent
        // instance: sbp2.inf:1394\609E&10483,sbp2_install NOTE: There is no
        // currently known way to get the product ID for this interface
        ULONG             propertyBufLen = 0;
        DEVPROPTYPE       propertyType   = 0;
                                      ...
                                      // it through - TJE
        {
            ...
            if (propertyBuf)
            {
                ...
                {
                    // multiple strings can be returned for some properties. This one will
                    // most likely only return one.
                    for (LPWSTR property = C_CAST(LPWSTR, propertyBuf); *property; property += wcslen(property) + 1)
                    {
                        if (property &&
                            (C_CAST(uintptr_t, property) - C_CAST(uintptr_t, propertyBuf)) < propertyBufLen &&
...
// It is possible to do this with setupapi as well. cfgmgr32 is supposedly
// available in some form for universal apps, whereas setupapi is not.
                                     ...
                                     ULONG                      deviceDescriptorLength)
{
    ...
    if (deviceDescriptor &&
        deviceDescriptorLength > sizeof(STORAGE_DEVICE_DESCRIPTOR)) // make sure we have a device
                                                                    // descriptor bigger than the header
                                                                    // so we can access the raw data
    {
        // First, get the list of disk device IDs, then locate a matching one...then
        // find the parent and parse the IDs out.
        ...
        ULONG        deviceIdListFlags =
            CM_GETIDLIST_FILTER_PRESENT | CM_GETIDLIST_FILTER_CLASS; // Both of these flags require Windows 7 and
                                                                     // later. The else case below will handle
                                                                     // older OSs
        ...
        if (cmRet == CR_SUCCESS)
        {
            ...
            {
                listBuffer = C_CAST(TCHAR*, safe_calloc(deviceIdListLen, sizeof(TCHAR)));
                if (listBuffer)
                {
...
#endif // WINVER > SEA_WIN32_WINNT_VISTA
        {
            ...
            // TODO: We may need to add other things for firewire or other attachment
            // types that existed back in Vista or XP if they aren't handled under USB
            // or SCSI
            const TCHAR* scsiFilter = TEXT("SCSI");
            const TCHAR* usbFilter  = TEXT("USBSTOR"); // Need to use USBSTOR in order to find a match.
                                                       // Using USB returns a list of VID/PID but we don't
                                                       // have a way to match that.
            ...
            // First get the SCSI list, then the USB list/ TODO: add more things to
            // the list as we need them.
            scsicmRet = CM_Get_Device_ID_List_Size(&scsiIdListLen, scsiFilter, filterFlags);
            if (scsicmRet == CR_SUCCESS)
            {
                if (scsiIdListLen > 0)
                {
                    scsiListBuff = C_CAST(TCHAR*, safe_calloc(scsiIdListLen, sizeof(TCHAR)));
                    if (scsiListBuff)
                    {
            ...
            if (usbcmRet == CR_SUCCESS)
            {
                if (usbIdListLen > 0)
                {
                    usbListBuff = C_CAST(TCHAR*, safe_calloc(usbIdListLen, sizeof(TCHAR)));
                    if (usbListBuff)
                    {
            ...
            // now that we got USB and SCSI, we need to merge them together into a
            // common list
            ...
            if (listBuffer)
            {
                ...
                {
                    cmRet = CR_SUCCESS; // set the status as SUCCESS, as we have list of
                                        // SCSI drives to use later
        ...
        // First, reduce the list to something that seems like it is the drive we
        // are looking for...we could have more than 1 match, but we're filtering
        // out all the extras that definitely aren't the correct device.
        if (ret != MEMORY_FAILURE && cmRet == CR_SUCCESS && deviceIdListLen > 0)
        {
            bool foundMatch = false;
            // Now we have a list of device IDs.
            // Each device ID is structured with a EmumeratorName\Disk&Ven_<vendor
            // ID>&Prod_<Product ID>&Rev_<Revision number>\<some random numbers and
            // characters> Since we have a device descriptor, we have the vendor ID,
            // product ID, and revision number, so we can match those. NOTE: Some
            // string matching is possible, but not reliable. All string matching was
            // removed due to it not quite working as expected when it was needed the
            ...
            for (LPTSTR deviceID = listBuffer; *deviceID && !foundMatch; deviceID += _tcslen(deviceID) + 1)
            {
                ...
                // if a match is found, call locate devnode. If this is not present, it
                // will fail and we need to continue through the loop convert the
                // deviceID to uppercase to make matching easier
                _tcsupr(deviceID);
                cmRet = CM_Locate_DevNode(&deviceInstance, deviceID, CM_LOCATE_DEVNODE_NORMAL);
                if (CR_SUCCESS == cmRet)
                {
                    // with the device node, get the interface list for this device (disk
                    // class GUID is used). This SHOULD only return one idem which is the
                    // full device path. TODO: save this device path
                    ULONG interfaceListSize = 0;
                    GUID  classGUID         = GUID_DEVINTERFACE_DISK; // TODO: If the tDevice handle that was
                                                                      // opened was a tape, changer or something
                                                                      // else, change this GUID.
                    ...
                    {
                        TCHAR* interfaceList = C_CAST(TCHAR*, safe_calloc(interfaceListSize, sizeof(TCHAR)));
                        if (interfaceList)
                        {
                            ...
                            if (CR_SUCCESS == cmRet)
                            {
                                // Loop through this list, just in case more than one thing
                                // comes through
                                for (LPTSTR currentDeviceID = interfaceList; *currentDeviceID && !foundMatch;
                                     currentDeviceID += _tcslen(currentDeviceID) + 1)
                                {
                                    // With this device path, open a handle and get the storage
                                    // device number. This is a match for the PhysicalDriveX
                                    // number and we can check that for a match
                                    ...
                                    {
                                        // If the storage device number matches, get the parent
                                        // device instance, then the parent device ID. This will
                                        // contain the USB VID/PID and PCI Vendor, product, and
                                        // revision numbers.
                                                            ...
                                                            &returnedDataSize, M_NULLPTR))
                                        {
                                            if (deviceNumber.DeviceNumber == device->os_info.os_drive_number)
                                            {
                                                ...
                                                foundMatch         = true;
                                                // Now that we have a matching handle, get the parent
                                                // information, then parse the values from that string
                                                ...
                                                if (CR_SUCCESS == cmRet)
                                                {
                                                    ...
                                                    {
                                                        TCHAR* parentBuffer =
                                                            C_CAST(TCHAR*, safe_calloc(parentLen, sizeof(TCHAR)));
                                                        if (parentBuffer)
                                                        {
                                                            ...
                                                            if (CR_SUCCESS == cmRet)
                                                            {
                                                                // uncomment this else case to view all the
                                                                // possible device or parent properties when
                                                                // figuring out what else is necessary to store
                                                                // for a new device.
                                                                /*  //This is a comment switch. two slashes
                                                                means uncomment the below, 1 means comment it
                                                                out
                                                                {
                                                                    ULONG propertyBufLen = 0;
                                                                    DEVPROPTYPE propertyType = 0;
                                                                ...
                                                                &DEVPKEY_NAME; uint16_t counter = 0; uint16_t
                                                                instanceCounter = 0; char *propertyName =
                                                                M_NULLPTR; size_t propertyNameLength = 0;
                                                                ...
                                                                properties\n");
                                                                        ...
                                                                        while (devproperty)
                                                                        {
                                                                            //get name of property being
                                                                checked. switch (counter)
                                                                            {
                                                                            case 0:
                                                                                devproperty =
                                                                ...
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DeviceDesc"); break; case 1:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_HardwareIds"); break; case 2:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_CompatibleIds"); break; case 3:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Service; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Service") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Service");
                                                                                break;
                                                                            case 4:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Class; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Class") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Class");
                                                                                break;
                                                                            case 5:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_ClassGuid");
                                                                                break;
                                                                            case 6:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Driver; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Driver") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Driver");
                                                                                break;
                                                                            case 7:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_ConfigFlags"); break; case 8:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Manufacturer; propertyNameLength
                                                                = safe_strlen("DEVPKEY_Device_Manufacturer") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_Manufacturer"); break; case 9:
                                                                                devproperty =
                                                                &DEVPKEY_Device_FriendlyName; propertyNameLength
                                                                = safe_strlen("DEVPKEY_Device_FriendlyName") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_FriendlyName"); break; case 10:
                                                                                devproperty =
                                                                &DEVPKEY_Device_LocationInfo; propertyNameLength
                                                                = safe_strlen("DEVPKEY_Device_LocationInfo") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_LocationInfo"); break; case 11:
                                                                                devproperty =
                                                                &DEVPKEY_Device_PDOName; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_PDOName") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_PDOName");
                                                                                break;
                                                                            case 12:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Capabilities; propertyNameLength
                                                                = safe_strlen("DEVPKEY_Device_Capabilities") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_Capabilities"); break; case 13:
                                                                                devproperty =
                                                                &DEVPKEY_Device_UINumber; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_UINumber") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_UINumber");
                                                                                break;
                                                                            case 14:
                                                                                devproperty =
                                                                &DEVPKEY_Device_UpperFilters; propertyNameLength
                                                                = safe_strlen("DEVPKEY_Device_UpperFilters") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_UpperFilters"); break; case 15:
                                                                                devproperty =
                                                                &DEVPKEY_Device_LowerFilters; propertyNameLength
                                                                = safe_strlen("DEVPKEY_Device_LowerFilters") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_LowerFilters"); break; case 16:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_BusTypeGuid"); break; case 17:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_LegacyBusType"); break; case 18:
                                                                                devproperty =
                                                                &DEVPKEY_Device_BusNumber; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_BusNumber") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_BusNumber");
                                                                                break;
                                                                            case 19:
                                                                                devproperty =
                                                                &DEVPKEY_Device_EnumeratorName;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_EnumeratorName") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_EnumeratorName"); break; case
                                                                20: devproperty = &DEVPKEY_Device_Security;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Security") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Security");
                                                                                break;
                                                                            case 21:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_SecuritySDS"); break; case 22:
                                                                                devproperty =
                                                                &DEVPKEY_Device_DevType; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_DevType") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_DevType");
                                                                                break;
                                                                            case 23:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Exclusive");
                                                                                break;
                                                                            case 24:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Characteristics;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Characteristics") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_Characteristics"); break; case
                                                                25: devproperty = &DEVPKEY_Device_Address;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Address") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Address");
                                                                                break;
                                                                            case 26:
                                                                                devproperty =
                                                                &DEVPKEY_Device_UINumberDescFormat;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_UINumberDescFormat")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_UINumberDescFormat"); break;
                                                                            case 27:
                                                                                devproperty =
                                                                &DEVPKEY_Device_PowerData; propertyNameLength =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_RemovalPolicy"); break; case 29:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_RemovalPolicyDefault"); break;
                                                                            case 30:
                                                                                devproperty =
                                                                &DEVPKEY_Device_RemovalPolicyOverride;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_RemovalPolicyOverride")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_RemovalPolicyOverride"); break;
                                                                            case 31:
                                                                                devproperty =
                                                                &DEVPKEY_Device_InstallState; propertyNameLength
                                                                = safe_strlen("DEVPKEY_Device_InstallState") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_InstallState"); break; case 32:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_LocationPaths"); break; case 33:
                                                                                devproperty =
                                                                &DEVPKEY_Device_BaseContainerId;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_BaseContainerId") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_BaseContainerId"); break; case
                                                                34: devproperty = &DEVPKEY_Device_InstanceId;
                                                                                propertyNameLength =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_InstanceId"); break; case 35:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DevNodeStatus"); break; case 36:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_ProblemCode"); break; case 37:
                                                                                devproperty =
                                                                &DEVPKEY_Device_EjectionRelations;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_EjectionRelations")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_EjectionRelations"); break; case
                                                                38: devproperty =
                                                                &DEVPKEY_Device_RemovalRelations;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_RemovalRelations") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_RemovalRelations"); break; case
                                                                39: devproperty =
                                                                &DEVPKEY_Device_PowerRelations;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_PowerRelations") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_PowerRelations"); break; case
                                                                40: devproperty = &DEVPKEY_Device_BusRelations;
                                                                                propertyNameLength =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_BusRelations"); break; case 41:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Parent; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Parent") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Parent");
                                                                                break;
                                                                            case 42:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Children; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Children") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Children");
                                                                                break;
                                                                            case 43:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Siblings; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Siblings") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Siblings");
                                                                                break;
                                                                            case 44:
                                                                                devproperty =
                                                                &DEVPKEY_Device_TransportRelations;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_TransportRelations")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_TransportRelations"); break;
                                                                            case 45:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_ProblemStatus"); break; case 46:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Reported; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Reported") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Reported");
                                                                                break;
                                                                            case 47:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Legacy; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Legacy") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Legacy");
                                                                                break;
                                                                            case 48:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_ContainerId"); break; case 49:
                                                                                devproperty =
                                                                ...
                                                                "DEVPKEY_Device_InLocalMachineContainer");
                                                                            ...
                                                                            case 50:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Model; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Model") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Model");
                                                                                break;
                                                                            case 51:
                                                                                devproperty =
                                                                &DEVPKEY_Device_ModelId; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_ModelId") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_ModelId");
                                                                                break;
                                                                            case 52:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_FriendlyNameAttributes"); break;
                                                                            case 53:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_ManufacturerAttributes"); break;
                                                                            case 54:
                                                                                devproperty =
                                                                &DEVPKEY_Device_PresenceNotForDevice;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_PresenceNotForDevice")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_PresenceNotForDevice"); break;
                                                                            case 55:
                                                                                devproperty =
                                                                &DEVPKEY_Device_SignalStrength;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_SignalStrength") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_SignalStrength"); break; case
                                                                56: devproperty =
                                                                ...
                                                                &DEVPKEY_Device_ShowInUninstallUI;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_ShowInUninstallUI")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_ShowInUninstallUI"); break; case
                                                                58: devproperty =
                                                                &DEVPKEY_Device_Numa_Proximity_Domain;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Numa_Proximity_Domain")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_Numa_Proximity_Domain"); break;
                                                                            case 59:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DHP_Rebalance_Policy"); break;
                                                                            case 60:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Numa_Node; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Numa_Node") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Numa_Node");
                                                                                break;
                                                                            case 61:
                                                                                devproperty =
                                                                &DEVPKEY_Device_BusReportedDeviceDesc;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_BusReportedDeviceDesc")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_BusReportedDeviceDesc"); break;
                                                                            case 62:
                                                                                devproperty =
                                                                &DEVPKEY_Device_IsPresent; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_IsPresent") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_IsPresent");
                                                                                break;
                                                                            case 63:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_HasProblem"); break; case 64:
                                                                                devproperty =
                                                                &DEVPKEY_Device_ConfigurationId;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_ConfigurationId") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_ConfigurationId"); break; case
                                                                65: devproperty =
                                                                &DEVPKEY_Device_ReportedDeviceIdsHash;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_ReportedDeviceIdsHash")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_ReportedDeviceIdsHash"); break;
                                                                            case 66:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_PhysicalDeviceLocation"); break;
                                                                            case 67:
                                                                                devproperty =
                                                                &DEVPKEY_Device_BiosDeviceName;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_BiosDeviceName") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_BiosDeviceName"); break; case
                                                                68: devproperty =
                                                                &DEVPKEY_Device_DriverProblemDesc;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_DriverProblemDesc")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverProblemDesc"); break; case
                                                                69: devproperty = &DEVPKEY_Device_DebuggerSafe;
                                                                                propertyNameLength =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DebuggerSafe"); break; case 70:
                                                                                devproperty =
                                                                &DEVPKEY_Device_PostInstallInProgress;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_PostInstallInProgress")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_PostInstallInProgress"); break;
                                                                            case 71:
                                                                                devproperty =
                                                                &DEVPKEY_Device_Stack; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_Stack") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_Stack");
                                                                                break;
                                                                            case 72:
                                                                                devproperty =
                                                                ...
                                                                &DEVPKEY_Device_IsRebootRequired;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_IsRebootRequired") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_IsRebootRequired"); break; case
                                                                74: devproperty = &DEVPKEY_Device_FirmwareDate;
                                                                                propertyNameLength =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_FirmwareDate"); break; case 75:
                                                                                devproperty =
                                                                &DEVPKEY_Device_FirmwareVersion;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_FirmwareVersion") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_FirmwareVersion"); break; case
                                                                76: devproperty =
                                                                &DEVPKEY_Device_FirmwareRevision;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_FirmwareRevision") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_FirmwareRevision"); break; case
                                                                77: devproperty =
                                                                &DEVPKEY_Device_DependencyProviders;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_DependencyProviders")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DependencyProviders"); break;
                                                                            case 78:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DependencyDependents"); break;
                                                                            case 79:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_SoftRestartSupported"); break;
                                                                            case 80:
                                                                                devproperty =
                                                                &DEVPKEY_Device_ExtendedAddress;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_ExtendedAddress") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_ExtendedAddress"); break; case
                                                                81: devproperty = &DEVPKEY_Device_SessionId;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_SessionId") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_Device_SessionId");
                                                                                break;
                                                                            case 82:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_InstallDate"); break; case 83:
                                                                                devproperty =
                                                                &DEVPKEY_Device_FirstInstallDate;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_FirstInstallDate") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_FirstInstallDate"); break; case
                                                                84: devproperty =
                                                                &DEVPKEY_Device_LastArrivalDate;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_LastArrivalDate") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_LastArrivalDate"); break; case
                                                                85: devproperty =
                                                                &DEVPKEY_Device_LastRemovalDate;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_LastRemovalDate") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_LastRemovalDate"); break; case
                                                                86: devproperty = &DEVPKEY_Device_DriverDate;
                                                                                propertyNameLength =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverDate"); break; case 87:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverVersion"); break; case 88:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverDesc"); break; case 89:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverInfPath"); break; case 90:
                                                                                devproperty =
                                                                &DEVPKEY_Device_DriverInfSection;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_DriverInfSection") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverInfSection"); break; case
                                                                91: devproperty =
                                                                &DEVPKEY_Device_DriverInfSectionExt;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_DriverInfSectionExt")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverInfSectionExt"); break;
                                                                            case 92:
                                                                                devproperty =
                                                                &DEVPKEY_Device_MatchingDeviceId;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_MatchingDeviceId") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_MatchingDeviceId"); break; case
                                                                93: devproperty =
                                                                &DEVPKEY_Device_DriverProvider;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_DriverProvider") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverProvider"); break; case
                                                                94: devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverPropPageProvider"); break;
                                                                            case 95:
                                                                                devproperty =
                                                                &DEVPKEY_Device_DriverCoInstallers;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_DriverCoInstallers")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverCoInstallers"); break;
                                                                            case 96:
                                                                                devproperty =
                                                                &DEVPKEY_Device_ResourcePickerTags;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_ResourcePickerTags")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_ResourcePickerTags"); break;
                                                                            case 97:
                                                                                devproperty =
                                                                ...
                                                                "DEVPKEY_Device_ResourcePickerExceptions");
                                                                            ...
                                                                            case 98:
                                                                                devproperty =
                                                                &DEVPKEY_Device_DriverRank; propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_DriverRank") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverRank"); break; case 99:
                                                                                devproperty =
                                                                &DEVPKEY_Device_DriverLogoLevel;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_DriverLogoLevel") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_DriverLogoLevel"); break; case
                                                                100: devproperty =
                                                                &DEVPKEY_Device_NoConnectSound;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_NoConnectSound") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_NoConnectSound"); break; case
                                                                101: devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_GenericDriverInstalled"); break;
                                                                            case 102:
                                                                                devproperty =
                                                                ...
                                                                &DEVPKEY_Device_SafeRemovalRequired;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_Device_SafeRemovalRequired")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_Device_SafeRemovalRequired"); break;
                                                                            case 104:
                                                                                devproperty =
                                                                ...
                                                                "DEVPKEY_Device_SafeRemovalRequiredOverride");
                                                                            ...
                                                                            case 105:
                                                                                devproperty =
                                                                &DEVPKEY_DrvPkg_Model; propertyNameLength =
                                                                safe_strlen("DEVPKEY_DrvPkg_Model") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_DrvPkg_Model");
                                                                                break;
                                                                            case 106:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DrvPkg_VendorWebSite"); break; case
                                                                107: devproperty =
                                                                &DEVPKEY_DrvPkg_DetailedDescription;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DrvPkg_DetailedDescription")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DrvPkg_DetailedDescription"); break;
                                                                            case 108:
                                                                                devproperty =
                                                                &DEVPKEY_DrvPkg_DocumentationLink;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DrvPkg_DocumentationLink")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DrvPkg_DocumentationLink"); break; case
                                                                109: devproperty = &DEVPKEY_DrvPkg_Icon;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DrvPkg_Icon") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_DrvPkg_Icon");
                                                                                break;
                                                                            case 110:
                                                                                devproperty =
                                                                &DEVPKEY_DrvPkg_BrandingIcon; propertyNameLength
                                                                = safe_strlen("DEVPKEY_DrvPkg_BrandingIcon") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DrvPkg_BrandingIcon"); break; case 111:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceClass_UpperFilters;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_UpperFilters")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_UpperFilters"); break; case
                                                                112: devproperty =
                                                                &DEVPKEY_DeviceClass_LowerFilters;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_LowerFilters")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_LowerFilters"); break; case
                                                                113: devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_Security"); break; case
                                                                114: devproperty =
                                                                &DEVPKEY_DeviceClass_SecuritySDS;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_SecuritySDS") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_SecuritySDS"); break; case
                                                                115: devproperty = &DEVPKEY_DeviceClass_DevType;
                                                                                propertyNameLength =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_DevType"); break; case 116:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceClass_Exclusive;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_Exclusive") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_Exclusive"); break; case
                                                                117: devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_Characteristics"); break;
                                                                            case 118:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceClass_Name; propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_Name") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_DeviceClass_Name");
                                                                                break;
                                                                            case 119:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceClass_ClassName;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_ClassName") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_ClassName"); break; case
                                                                120: devproperty = &DEVPKEY_DeviceClass_Icon;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_Icon") + 1;
                                                                                propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength, "DEVPKEY_DeviceClass_Icon");
                                                                                break;
                                                                            case 121:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceClass_ClassInstaller;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_ClassInstaller")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_ClassInstaller"); break;
                                                                            case 122:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceClass_PropPageProvider;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_PropPageProvider")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_PropPageProvider"); break;
                                                                            case 123:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceClass_NoInstallClass;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_NoInstallClass")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_NoInstallClass"); break;
                                                                            case 124:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceClass_NoDisplayClass;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_NoDisplayClass")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_NoDisplayClass"); break;
                                                                            case 125:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceClass_SilentInstall;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_SilentInstall")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_SilentInstall"); break;
                                                                            case 126:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceClass_NoUseClass;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_NoUseClass") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_NoUseClass"); break; case
                                                                127: devproperty =
                                                                &DEVPKEY_DeviceClass_DefaultService;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceClass_DefaultService")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_DefaultService"); break;
                                                                            case 128:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_IconPath"); break; case
                                                                129: devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceClass_ClassCoInstallers"); break;
                                                                            case 131:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceInterface_FriendlyName;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceInterface_FriendlyName")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceInterface_FriendlyName"); break;
                                                                            case 132:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceInterface_Enabled;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceInterface_Enabled") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceInterface_Enabled"); break; case
                                                                133: devproperty =
                                                                &DEVPKEY_DeviceInterface_ClassGuid;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceInterface_ClassGuid")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceInterface_ClassGuid"); break;
                                                                            case 134:
                                                                                devproperty =
                                                                ...
                                                                &DEVPKEY_DeviceInterface_Restricted;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceInterface_Restricted")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceInterface_Restricted"); break;
                                                                            case 136:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceInterface_SchematicName"); break;
                                                                            case 138:
                                                                                devproperty =
                                                                ...
                                                                &DEVPKEY_DeviceInterfaceClass_Name;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceInterfaceClass_Name")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceInterfaceClass_Name"); break;
                                                                            case 140:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceContainer_Address;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_Address") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_Address"); break; case
                                                                141: devproperty =
                                                                ...
                                                                &DEVPKEY_DeviceContainer_IsEncrypted;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_IsEncrypted")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_IsEncrypted"); break;
                                                                            case 143:
                                                                                devproperty =
                                                                ...
                                                                &DEVPKEY_DeviceContainer_IsConnected;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_IsConnected")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_IsConnected"); break;
                                                                            case 145:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceContainer_IsPaired;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_IsPaired")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_IsPaired"); break; case
                                                                146: devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_Icon"); break; case
                                                                147: devproperty =
                                                                &DEVPKEY_DeviceContainer_Version;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_Version") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_Version"); break; case
                                                                148: devproperty =
                                                                &DEVPKEY_DeviceContainer_Last_Seen;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_Last_Seen")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_Last_Seen"); break;
                                                                            case 149:
                                                                                devproperty =
                                                                ...
                                                                &DEVPKEY_DeviceContainer_MetadataPath;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_MetadataPath")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_MetadataPath"); break;
                                                                            case 153:
                                                                                devproperty =
                                                                ...
                                                                &DEVPKEY_DeviceContainer_HasProblem;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_HasProblem")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_HasProblem"); break;
                                                                            case 164:
                                                                                devproperty =
                                                                ...
                                                                &DEVPKEY_DeviceContainer_ExperienceId;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_ExperienceId")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_ExperienceId"); break;
                                                                            case 170:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_Category"); break; case
                                                                171: devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_Category_Icon"); break;
                                                                            case 174:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_ConfigFlags"); break;
                                                                            case 183:
                                                                                devproperty =
                                                                ...
                                                                &DEVPKEY_DeviceContainer_FriendlyName;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_FriendlyName")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_FriendlyName"); break;
                                                                            case 187:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceContainer_Manufacturer;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_Manufacturer")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_Manufacturer"); break;
                                                                            case 188:
                                                                                devproperty =
                                                                &DEVPKEY_DeviceContainer_ModelName;
                                                                                propertyNameLength =
                                                                safe_strlen("DEVPKEY_DeviceContainer_ModelName")
                                                                + 1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_ModelName"); break;
                                                                            case 189:
                                                                                devproperty =
                                                                ...
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DeviceContainer_ModelNumber"); break;
                                                                            case 190:
                                                                                devproperty =
                                                                ...
                                                                = safe_strlen("DEVPKEY_DevQuery_ObjectType") +
                                                                1; propertyName = C_CAST(char*,
                                                                safe_calloc(propertyNameLength, sizeof(char)));
                                                                                snprintf(propertyName,
                                                                propertyNameLength,
                                                                "DEVPKEY_DevQuery_ObjectType"); break; default:
                                                                ...
                                                                &propertyType, M_NULLPTR, &propertyBufLen, 0);
                                                                            if (CR_SUCCESS == cmRet ||
                                                                CR_INVALID_POINTER == cmRet || CR_BUFFER_SMALL
                                                                == cmRet)//We'll probably get an invalid pointer
                                                                or small buffer, but this will return the size
                                                                of the buffer we need, so allow it through - TJE
                                                                            {
                                                                                PBYTE propertyBuf =
                                                                C_CAST(PBYTE, safe_calloc(propertyBufLen + 1,
                                                                sizeof(BYTE))); if (propertyBuf)
                                                                                {
                                                                                    propertyBufLen += 1;
                                                                                    cmRet =
                                                                CM_Get_DevNode_PropertyW(propInst, devproperty,
                                                                &propertyType, propertyBuf, &propertyBufLen, 0);
                                                                                    if (CR_SUCCESS == cmRet)
                                                                                    {
                                                                                        DEVPROPTYPE
                                                                propertyModifier = propertyType &
                                                                DEVPROP_MASK_TYPEMOD;
                                                                                        //print the property
                                                                name here in case anything fails above so we
                                                                don't print a bunch of empty properties - TJE
                                                                                        printf("=========================================================================\n");
                                                                                        printf(" %s: \n",
                                                                propertyName); switch (propertyType &
                                                                DEVPROP_MASK_TYPE)//need to mask as there may
                                                                also be modifiers to notate lists, etc
                                                                                        {
                                                                                        case
                                                                DEVPROP_TYPE_STRING:
                                                                                            // Fall-through //
                                                                ...
                                                                DEVPROP_TYPEMOD_LIST ? 1 : 0;//this adjusts the
                                                                loop because if this ISN'T set, then we don't
                                                                need any more length than the string length for
                                                                (LPWSTR property = C_CAST(LPWSTR, propertyBuf);
                                                                *property; property += wcslen(property) +
                                                                propListAdditionalLen)
                                                                                            {
                                                                                                if (property &&
                                                                (C_CAST(uintptr_t, property) - C_CAST(uintptr_t,
                                                                propertyBuf)) < propertyBufLen &&
                                                                wcslen(property))
                                                                                                {
                                                                                                    wprintf(L"\t%s\n",
                                                                property);
                                                                                            ...
                                                                                            break;
                                                                                        case
                                                                DEVPROP_TYPE_SBYTE://8bit signed byte
                                                                                        {
                                                                                            char *signedByte =
                                                                ...
                                                                "\n", *signedByte);
                                                                                        ...
                                                                                        {
                                                                                            //TODO: Handle
                                                                arrays which could show as this type. Check
                                                                propertyModifier.
                                                                                            //      Currently
                                                                only popping up for power data which can be
                                                                converted to a structure and output. BYTE
                                                                *unsignedByte = C_CAST(BYTE*, propertyBuf);
                                                                                            printf("\t%" PRIu8
                                                                "\n", *unsignedByte);
                                                                                        }
                                                                                        break;
                                                                ...
                                                                DEVPROP_TYPE_UINT16:
                                                                                        {
                                                                                            UINT16 *unsigned16 =
                                                                C_CAST(UINT16*, propertyBuf); printf("\t%"
                                                                PRIu16 "\n", *unsigned16);
                                                                                        }
                                                                                        break;
                                                                ...
                                                                DEVPROP_TYPE_UINT32:
                                                                                        {
                                                                                            UINT32 *unsigned32 =
                                                                C_CAST(UINT32*, propertyBuf); printf("\t%"
                                                                PRIu32 "\n", *unsigned32);
                                                                                        }
                                                                                        break;
                                                                ...
                                                                DEVPROP_TYPE_UINT64:
                                                                                        {
                                                                                            UINT64 *unsigned64 =
                                                                C_CAST(UINT64*, propertyBuf); printf("\t%"
                                                                PRIu64 "\n", *unsigned64);
                                                                                        }
                                                                                        break;
                                                                ...
                                                                *theFloat);
                                                                ...
                                                                == DEVPROP_TRUE)
                                                                                        ...
                                                                                        break;
                                                                                        case
                                                                DEVPROP_TYPE_ERROR://win32 error code
                                                                                        {
                                                                                            DWORD *win32Error =
                                                                C_CAST(DWORD*, propertyBuf);
                                                                                        ...
                                                                                        break;
                                                                                        case
                                                                DEVPROP_TYPE_GUID://128bit guid
                                                                                            //format ( hex
                                                                digits ):
                                                                                            //{8-4-4-4-12}
                                                                                        {
                                                                                            GUID *theGuid =
                                                                C_CAST(GUID*, propertyBuf); printf("\t{%08" "lu"
                                                                "-%04" PRIX16 "-%04" PRIX16 "-", theGuid->Data1,
                                                                theGuid->Data2, theGuid->Data3);
                                                                                            //now print data 4
                                                                which is an 8 byte array
                                                                                            //first 2 bytes
                                                                first: printf("%02" PRIX8 "%02" PRIX8,
                                                                theGuid->Data4[0], theGuid->Data4[1]);
                                                                                            //now remaining 6
                                                                bytes printf("-%02" PRIX8 "%02" PRIX8 "%02"
                                                                PRIX8 "%02" PRIX8 "%02" PRIX8 "%02" PRIX8 "}\n",
                                                                theGuid->Data4[2], theGuid->Data4[3],
                                                                theGuid->Data4[4], theGuid->Data4[5],
                                                                ...
                                                                dumb.
                                                                                            //The conversions
                                                                are all MFC/ATL code, which we don't want.
                                                                                            //The following
                                                                attempts to convert it to something in C DATE
                                                                *date = C_CAST(DATE*, propertyBuf); uint64_t
                                                                wholePart = C_CAST(uint64_t, floor(*date));
                                                                                            double fractionPart
                                                                = *date - wholePart; printf("date = %f, whole =
                                                                %llu, fraction = %0.02f\n", *date, wholePart,
                                                                fractionPart);
                                                                                            //If we need this
                                                                converted, we can do it but it will take more
                                                                work.
                                                                                            //Currently only
                                                                using this when debugging and raw data above is
                                                                enough
                                                                                        }
                                                                                        break;
                                                                                        case
                                                                DEVPROP_TYPE_FILETIME:
                                                                                        {
                                                                                            FILETIME *fileTime =
                                                                C_CAST(FILETIME*, propertyBuf); SYSTEMTIME
                                                                systemTime; TIME_ZONE_INFORMATION
                                                                currentTimeZone;
                                                                                            //DWORD tzret =
                                                                                            GetTimeZoneInformation(&currentTimeZone);//need
                                                                this to adjust the converted time below. note,
                                                                return value specifies std vs dst time (1 vs 2).
                                                                0 is unknown. if (FileTimeToSystemTime(fileTime,
                                                                &systemTime))
                                                                                            {
                                                                                                SYSTEMTIME
                                                                localTime;
                                                                                                //convert the
                                                                system time structure to the current time zone
                                                                                                if
                                                                ...
                                                                localTime.wYear, localTime.wHour,
                                                                ...
                                                                systemTime.wSecond);
                                                                                            ...
                                                                                            {
                                                                                                printf("\tUnable
                                                                to convert filetime to system time\n");
                                                                                            }
                                                                                        }
                                                                                        break;
                                                                                        case
                                                                ...
                                                                DEVPROP_TYPE_DEVPROPKEY: case
                                                                DEVPROP_TYPE_DEVPROPTYPE: case
                                                                ...
                                                                if (!get_IDs_From_TCHAR_String(parentInst, parentBuffer,
                                                                                               parentLen, device))
                                                                {
                                                                    // try the parent's parent. There are some
                                                                    // cases where this seems to be necessary to
                                                                    // get this data. one known case is AMD's RAID
                                                                ...
                                                                // following with the parent's instance. (don't
                                                                // need parent's parent)
                                                                //  1. DEVPKEY_Device_Service <- gives the
                                                                //  driver's name
                                                                //  2. DEVPKEY_Device_DriverVersion
                                                                // Both return strings that we can parse as
                                                                // needed. DEVPKEY_Device_DriverDesc gives the
                                                                // driver's "displayable" name, which is not
                                                                // always helpful since it's usually marketting
                                                                // crap
                                                                ULONG       propertyBufLen = 0;
                                                                DEVPROPTYPE propertyType   = 0;
                                                                ...
                                                                if (CR_SUCCESS == cmRet ||
                                                                    ...
                                                                    CR_BUFFER_SMALL ==
                                                                        cmRet) // We'll probably get an invalid
                                                                               // pointer or small buffer, but
                                                                               // this will return the size of
                                                                               // the buffer we need, so allow
                                                                               // it through - TJE
                                                                {
                                                                    PBYTE propertyBuf =
                                                                        C_CAST(PBYTE, safe_calloc(propertyBufLen + 1,
                                                                            ...
                                                                            uint8_t propListAdditionalLen =
                                                                                propertyModifier == DEVPROP_TYPEMOD_LIST
                                                                                    ...
                                                                                    : 0; // this adjusts the loop
                                                                                         // because if this ISN'T set,
                                                                                         // then we don't need any
                                                                                         // more length than the
                                                                                         // string length
                                                                            for (LPWSTR property =
                                                                ...
                                                                propertyBufLen =
                                                                    0; // reset to zero before going into this
                                                                       // function to read the string again.-TJE
                                                                cmRet = CM_Get_DevNode_PropertyW(
                                                ...
                                                {
                                                    // for some reason, we matched, but didn't find a
                                                    // matching drive number. Keep going through the list
                ...
                } // else node is not available most likely...possibly not attached to
                  // the system.
            }
            if (foundMatch)
            {
...
#define WIN_DUMMY_NVME_STATUS(sct, sc) C_CAST(uint32_t, (sct << 25) | (sc << 17))
...
// this in an internal function so that it can be reused for reading firmware
// slot info, sending a download command, or sending an activate command. The
// inputs
                                                        ...
                                                        uint64_t*        timeNanoseconds)
{
    ...
    ULONG                   bufferSize =
        sizeof(SRB_IO_CONTROL) + sizeof(FIRMWARE_REQUEST_BLOCK); // ((sizeof(SRB_IO_CONTROL) +
                                                                 // sizeof(FIRMWARE_REQUEST_BLOCK) - 1) /
                                                                 // sizeof(PVOID) + 1) * sizeof(PVOID);//Right
                                                                 // from MSFT example toi align the data
                                                                 // buffer on pointer alignment - TJE
    ULONG firmwareRequestDataOffset = bufferSize;                // This must be before we add any additional length to
                                                  // buffersize since this is where data will be copied to for
                                                  // the request - TJE
    ...
    bufferSize += dataRequestLength; // Add the length of the passed in request
                                     // that will be issued.
    buffer = C_CAST(PUCHAR, safe_calloc_aligned(bufferSize, sizeof(UCHAR),
    ...
    // First fill out the srb header and firmware request block since these are
    ...
    firmwareRequest = C_CAST(PFIRMWARE_REQUEST_BLOCK,
                             srbControl + 1); // this structure starts right after the
                                              // SRB_IO_CONRTOL so simply add 1.
    ...
    if (ERROR_IO_PENDING == getLastError) // This will only happen for overlapped commands. If the
                                          // drive is opened without the overlapped flag, everything
                                          // will work like old synchronous code.-TJE
    {
    ...
    CloseHandle(overlappedStruct.hEvent); // close the overlapped handle since it
                                          // isn't needed any more...-TJE
    ...
    if (result)
    {
        ...
        {
            // request was to read the firmware info, so copy this out to the buffer
            // for the calling function to deal with - TJE
            memcpy(ptrDataRequest, buffer + firmwareRequestDataOffset, dataRequestLength);
        }
        ret = SUCCESS;
        if (returnCode)
        {
            *returnCode = srbControl->ReturnCode; // this is so the caller can do what it wants
                                                  // to with this information - TJE
        }
    ...
    {
        // something else went wrong. Check Windows last error code. Likely an
...
#    if WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_THRESHOLD
    if (is_Windows_10_Or_Higher())
    {
        ...
        ULONG firmwareInfoLength =
            ...
            (sizeof(STORAGE_FIRMWARE_SLOT_INFO_V2) * 7); // max of 7 slots in NVMe. This should be plenty of space
                                                         // regardless of device type -TJE
        PSTORAGE_FIRMWARE_INFO_V2 firmwareInfo =
            C_CAST(PSTORAGE_FIRMWARE_INFO_V2, safe_calloc(firmwareInfoLength, sizeof(UCHAR)));
        if (firmwareInfo)
        {
...
#    endif // WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_THRESHOLD
        if (is_Windows_8_One_Or_Higher())
        {
            ...
            ULONG firmwareInfoLength =
                ...
                (sizeof(STORAGE_FIRMWARE_SLOT_INFO) * 7); // max of 7 slots in NVMe. This should be plenty of space
                                                          // regardless of device type -TJE
            PSTORAGE_FIRMWARE_INFO firmwareInfo =
                C_CAST(PSTORAGE_FIRMWARE_INFO, safe_calloc(firmwareInfoLength, sizeof(UCHAR)));
            if (firmwareInfo)
            {
                uint32_t returnCode = 0;
                // Setup input values
                ...
                if (ret == SUCCESS)
                {
                    ...
                    device->os_info.fwdlIOsupport.maxXferSize =
                        device->os_info.adapterMaxTransferSize > 0
                            ...
                            : 65536; // Set 64K in case this is not otherwise set...not
                                     // great but will likely work since this is the
                                     // common transfer size limit in Windows - TJE
#    if defined(_DEBUG)
                    printf("Got Miniport V1 FWDL Info\n");
                    printf("\tSupported: %d\n", firmwareInfo->UpgradeSupport);
                    // printf("\tPayload Alignment: %ld\n",
                    // firmwareInfo->ImagePayloadAlignment); printf("\tmaxXferSize:
                    // %ld\n", firmwareInfo->ImagePayloadMaxSize);
                    printf("\tPendingActivate: %d\n", firmwareInfo->PendingActivateSlot);
                    printf("\tActiveSlot: %d\n", firmwareInfo->ActiveSlot);
                    printf("\tSlot Count: %d\n", firmwareInfo->SlotCount);
                    ...
                    for (uint8_t iter = 0; iter < firmwareInfo->SlotCount && iter < 7; ++iter)
                    {
                        ...
                        printf("\t\tRead Only: %d\n", firmwareInfo->Slot[iter].ReadOnly);
                        printf("\t\tRevision: %s\n",
                               v1Revision); // temp storage since there was not enough room
                                            // for null terminator in API so it gets messy.
                    }
#    endif
                }
                safe_free(&firmwareInfo);
...
/*
This API only supported deferred download and activate commands as defined in
ACS3+ and SPC4+

This table defines when this API is supported based on the drive and interface
of the drive. IDE | SCSI ATA    Y  |   N SCSI   N  |   Y

This table defines when this API is supported based on the Interface and the
Command being sent ATA DL | ATA DL DMA | SCSI WB IDE       Y   |      Y     | N
SCSI      N   |      N     |    Y

The reason the API is used only in the instances shown above is because the
library is trying to honor issuing the expected command on a specific interface.

If the drive is an ATA drive, behind a SAS controller, then a Write buffer
command is issued to the controller to be translated according to the SAT spec.
Sometimes, this may not be what a caller is wanting to do so we assume that we
will only issue the command the caller is expecting to issue.

There is an option to allow using this API call with any supported FWDL command
regardless of drive type and interface that can be set.
Device->os_info.fwdlIOsupport.allowFlexibleUseOfAPI set to true will check for a
supported SCSI or ATA command and all other payload requirements and allow it to
be issued for any case. This is good if your only goal is to get firmware to a
drive and don't care about testing a specific command sequence. NOTE: Some SAS
HBAs will issue a readlogext command before each download command when
performing deferred download, which may not be expected if taking a bus trace of
the sequence.
...
#    endif
    ...
    // SCSI and RAID interfaces depend on the SATL to translate it correctly, but
    // that is not checked by windows and is not possible since nothing responds
    // to the report supported operation codes command A future TODO will be to
    // have either a lookup table or additional check somewhere to send the report
    // supported operation codes command, but this is good enough for now, since
    // it's unlikely a SATL will implement that...
#    if defined(_DEBUG_FWDL_API_COMPATABILITY)
...
#    endif
        // We're sending an ATA passthrough command, and the OS says the io is
        // supported, so it SHOULD work. - TJE
...
#    endif
                // We know it's a download command, now we need to make sure it's a
                // multiple of the Windows alignment requirement and that it isn't
...
#    endif
        // Should we check that this is a SCSI Drive? Right now we'll just attempt
        // the download and let the drive/SATL handle translation check that it's a
        // write buffer command for a firmware download & it's a deferred download
...
{
    eReturnValues ret = SUCCESS; // assume this for anything where we could
                                 // generate a usable status and a few other cases
    uint8_t senseKey = SENSE_KEY_NO_ERROR, asc = 0,
            ascq = 0; // no fru since that is vendor unique info that we have no
                      // way of dummying up - TJE
    DECLARE_ZERO_INIT_ARRAY(uint8_t, localSense, SPC3_SENSE_LEN);
    if (scsiIoCtx->pAtaCmdOpts)
    {
        // sense code should be "ATA Passthrough information available" since this
        // is how a passed ATA command will show up versus a SCSI command - TJE
        ...
        case FIRMWARE_STATUS_SUCCESS:
            ...
            else
            {
                ...
                {
                    scsiIoCtx->pAtaCmdOpts->rtfr.secCnt = 0x01; // expecting more
                                                                // microcode
                }
            }
            break;
...
#    endif
    ...
    // now set the sense code into the data buffer output - using fixed format for
    // simplicity - TJE
    ...
    if (scsiIoCtx->pAtaCmdOpts)
    {
        localSense[0] |= BIT7; // set valid bit since we are filling in information
                               // field according to a standard - TJE
        // set information and command specific info fields for passthrough register
        // data
...
#    if WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_THRESHOLD
    if (is_Windows_10_Or_Higher())
    {
        ...
        PSTORAGE_FIRMWARE_DOWNLOAD_V2 firmwareDownload =
            C_CAST(PSTORAGE_FIRMWARE_DOWNLOAD_V2, safe_calloc(firmwareDLLength, sizeof(UCHAR)));
        if (firmwareDownload)
        {
            uint32_t returnCode = 0;
            uint32_t fwdlFlags  = FIRMWARE_REQUEST_FLAG_CONTROLLER; // start with this, but may need
                                                                    // other flags
            // Setup input values
            ...
            // we need to set the offset since MS uses this in the command sent to the
            // device.
...
#        if defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_15063
            if (scsiIoCtx->fwdlLastSegment)
            {
                ...
                // One website says that this flag is new in Win10 1704 - creators
                // update (10.0.15021)
...
#    endif // WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_THRESHOLD
        if (is_Windows_8_One_Or_Higher())
        {
            ...
            PSTORAGE_FIRMWARE_DOWNLOAD firmwareDownload =
                C_CAST(PSTORAGE_FIRMWARE_DOWNLOAD, safe_calloc(firmwareDLLength, sizeof(UCHAR)));
            if (firmwareDownload)
            {
                uint32_t returnCode = 0;
                uint32_t fwdlFlags  = FIRMWARE_REQUEST_FLAG_CONTROLLER; // start with this, but may need
                ...
                // we need to set the offset since MS uses this in the command sent to
                // the device.
                firmwareDownload->Offset = 0;
                if (scsiIoCtx && scsiIoCtx->pAtaCmdOpts)
                {
                ...
                // copy the image to ImageBuffer
                memcpy(firmwareDownload->ImageBuffer, scsiIoCtx->pdata, scsiIoCtx->dataLength);
                // no other flags to setup since 8.1 only had "controller" flag and
                // existing slot flag (which only affects activate) Issue the minport
                // IOCTL
                ret = send_Win_Firmware_Miniport_Command(
...
static eReturnValues win_FW_Activate_IO_SCSI_Miniport(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (is_Windows_8_One_Or_Higher())
    {
        ...
        PSTORAGE_FIRMWARE_ACTIVATE firmwareActivate =
            C_CAST(PSTORAGE_FIRMWARE_ACTIVATE, safe_calloc(firmwareActivateLength, sizeof(UCHAR)));
        if (firmwareActivate)
        {
            uint32_t returnCode = 0;
            ...
            {
                firmwareActivate->SlotToActivate = scsiIoCtx->cdb[2]; // Set the slot number to the buffer ID
                                                                      // number...This is the closest this translates.
            }
            if (scsiIoCtx->device->drive_info.interface_type ==
                NVME_INTERFACE) // SCSI interface, but NVMe in 8.1 will likely only be
                                // identified by earlier bustype or vendor id
            {
                // if we are on NVMe, but the command comes to here, then someone forced
                // SCSI mode, so let's set this flag correctly
...
// This may need adjusting with bus trace help in the future, but for now this
// is a best guess from what info we have.-TJE
...
#    if WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_15063
    ...
    case FIRMWARE_STATUS_UNCORRECTABLE_DATA_ERROR:
        nvmeIoCtx->commandCompletionData.dw3Valid = true;
        nvmeIoCtx->commandCompletionData.dw3      = WIN_DUMMY_NVME_STATUS(NVME_SCT_MEDIA_AND_DATA_INTEGRITY_ERRORS,
                                                                          0x81); // unrecovered read error...not sure what
                                                                                 // else to set if this happens-TJE
        break;
    case FIRMWARE_STATUS_MEDIA_CHANGE:
        ret = OS_PASSTHROUGH_FAILURE;
        break;
...
eReturnValues set_NVMe_Firmware_Activate_Flags(nvmeCmdCtx* nvmeIoCtx, uint32_t* currentFlags)
{
    ...
    case NVME_CA_REPLACE_ACTIVITE_ON_RST:
        ...
        // NOTE: No else here because this is the default action in Win10, so we do
        // not want to return an error.
        break;
...
#    if WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_THRESHOLD
    if (is_Windows_10_Or_Higher())
    {
        ...
        if (firmwareDownload)
        {
            ...
            firmwareDownload->BufferSize = C_CAST(ULONGLONG, nvmeIoCtx->cmd.adminCmd.cdw10 + 1)
                                           << 2; // add one since this is zeroes based then multiply by 4 to
                                                 // convert words to bytes
            firmwareDownload->ImageSize = C_CAST(ULONG, nvmeIoCtx->cmd.adminCmd.cdw10 + 1)
                                          << 2; // add one since this is zeroes based then multiply by 4 to
                                                // convert words to bytes
            firmwareDownload->Slot = STORAGE_FIRMWARE_INFO_INVALID_SLOT; // SHould this be zero? It
                                                                         // technically shouldn't be used
                                                                         // in this command in the NVMe
                                                                         // spec - TJE
...
#    endif // WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_THRESHOLD
        if (is_Windows_8_One_Or_Higher())
        {
            ...
            if (firmwareDownload)
            {
                ...
                firmwareDownload->Version = STORAGE_FIRMWARE_DOWNLOAD_STRUCTURE_VERSION;
                firmwareDownload->Size    = sizeof(STORAGE_FIRMWARE_DOWNLOAD);
                firmwareDownload->Offset  = nvmeIoCtx->cmd.adminCmd.cdw11
                                           << 2; // multiply by 4 to convert words to bytes
                firmwareDownload->BufferSize = (nvmeIoCtx->cmd.adminCmd.cdw10 + 1)
                                               << 2; // add one since this is zeroes based then multiply by 4 to
                                                     // convert words to bytes
...
//         errno is interpretted correctly after this call drive_num =
//         strtol(pdev + 1, M_NULLPTR, 0);
//     }
//     return drive_num;
// }
...
#endif
        {
            close_SCSI_SRB_Handle(dev);                            //\\.\SCSIx: could be opened for different
                                                                   // reasons...so we need to close it here.
            safe_free_csmi_dev_info(&dev->os_info.csmiDeviceData); // CSMI may have been used, so free this memory
                                                                   // if it was before we close out.
            CloseHandle(dev->os_info.forceUnitAccessRWfd);         // if FUA handle was opened, this
                                                                   // will close it out.
...
// Best I can find right now is that this IOCTL is available in XP and up.
// Change the definition below as needed. Many of the IOCTLs are wrapped for the
// version of Win API that they appeared in but may not be perfect. Using MSFT's
// documentation on them as best we can find. - TJE
#if defined(WINVER) && WINVER >= SEA_WIN32_WINNT_WINXP

// This is a basic way of getting storage properties and cannot account for some
// which require additional input parameters Others with additional parameters
// should be in their own function since the additional parameters will vary!
...
// Determines if a property exists (true if it does) and optionally returns the
// length of the property NOTE: This does works by requesting only the header to
// get the size.
//       Trying to setup a query with PropertyExistsQuery results in strange
//       results that aren't always true, so this was the best way I found to
//       handle this - TJE
...
// This is used internally by many of the pieces of code to read properties
// since it's very similar in how it gets requested for each of them. the
// "sizeOfExpectedPropertyStructure" is meant to be a
// sizeof(MSFT_STORAGE_STRUCT) output. This exists because some devices or
// drivers don't necessarily report accurate sizes for certain calls which can
// result in really weird data if not at least a minimum expected size.
...
// SCSI VPD device IDs
// static eReturnValues win_Get_Device_ID_Property(HANDLE deviceHandle,
// PSTORAGE_DEVICE_ID_DESCRIPTOR *deviceIdData)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceIdProperty, C_CAST(void**, deviceIdData),
//     sizeof(STORAGE_DEVICE_ID_DESCRIPTOR));
// }

#    if defined(WINVER) && WINVER >= SEA_WIN32_WINNT_VISTA
// static eReturnValues win_Get_Write_Cache_Info(HANDLE *deviceHandle,
// PSTORAGE_WRITE_CACHE_PROPERTY *writeCacheInfo)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceWriteCacheProperty, C_CAST(void**, writeCacheInfo),
...
// static eReturnValues win_Get_Seek_Time_Penalty(HANDLE *deviceHandle,
// PDEVICE_SEEK_PENALTY_DESCRIPTOR *seekTimePenaltyDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceSeekPenaltyProperty, C_CAST(void**,
//     seekTimePenaltyDescriptor), sizeof(DEVICE_SEEK_PENALTY_DESCRIPTOR));
// }

// static eReturnValues win_Get_Trim(HANDLE *deviceHandle,
// PDEVICE_TRIM_DESCRIPTOR *trimDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceTrimProperty, C_CAST(void**, trimDescriptor),
//     sizeof(DEVICE_TRIM_DESCRIPTOR));
// }
#    endif // SEA_WIN32_WINNT_WIN7

#    if defined(WINVER) && WINVER >= SEA_WIN32_WINNT_WIN8
// static eReturnValues win_Get_Logical_Block_Provisioning(HANDLE *deviceHandle,
// PDEVICE_LB_PROVISIONING_DESCRIPTOR *lbProvisioningDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceLBProvisioningProperty, C_CAST(void**,
//     lbProvisioningDescriptor), sizeof(DEVICE_LB_PROVISIONING_DESCRIPTOR));
// }

// static eReturnValues win_Get_Power_Property(HANDLE *deviceHandle,
// PDEVICE_POWER_DESCRIPTOR *powerDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDevicePowerProperty, C_CAST(void**, powerDescriptor),
//     sizeof(DEVICE_POWER_DESCRIPTOR));
// }

// static eReturnValues win_Get_Copy_Offload(HANDLE *deviceHandle,
// PDEVICE_COPY_OFFLOAD_DESCRIPTOR *copyOffloadDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceCopyOffloadProperty, C_CAST(void**, copyOffloadDescriptor),
//     sizeof(DEVICE_COPY_OFFLOAD_DESCRIPTOR));
// }

// static eReturnValues win_Get_Resiliency_Descriptor(HANDLE *deviceHandle,
// PSTORAGE_DEVICE_RESILIENCY_DESCRIPTOR *resiliencyDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceResiliencyProperty, C_CAST(void**, resiliencyDescriptor),
...
// static eReturnValues win_Get_Medium_Product_Type(HANDLE *deviceHandle,
// PSTORAGE_MEDIUM_PRODUCT_TYPE_DESCRIPTOR  *mediumType)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceMediumProductType, C_CAST(void**, mediumType),
...
// static eReturnValues win_Get_IO_Capability(HANDLE *deviceHandle,
// PSTORAGE_DEVICE_IO_CAPABILITY_DESCRIPTOR *capabilityDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceIoCapabilityProperty, C_CAST(void**, capabilityDescriptor),
//     sizeof(STORAGE_DEVICE_IO_CAPABILITY_DESCRIPTOR));
// }

// static eReturnValues win_Get_Adapter_Temperature(HANDLE *deviceHandle,
// PSTORAGE_TEMPERATURE_DATA_DESCRIPTOR *temperatureDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageAdapterTemperatureProperty, C_CAST(void**, temperatureDescriptor),
//     sizeof(STORAGE_TEMPERATURE_DATA_DESCRIPTOR));
// }

// static eReturnValues win_Get_Device_Temperature(HANDLE *deviceHandle,
// PSTORAGE_TEMPERATURE_DATA_DESCRIPTOR *temperatureDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceTemperatureProperty, C_CAST(void**, temperatureDescriptor),
//     sizeof(STORAGE_TEMPERATURE_DATA_DESCRIPTOR));
// }

// static eReturnValues win_Get_Adapter_Physical_Topology(HANDLE *deviceHandle,
// PSTORAGE_PHYSICAL_TOPOLOGY_DESCRIPTOR  *topologyDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageAdapterPhysicalTopologyProperty, C_CAST(void**,
//     topologyDescriptor), sizeof(STORAGE_PHYSICAL_TOPOLOGY_DESCRIPTOR));
// }

// static eReturnValues win_Get_Device_Physical_Topology(HANDLE *deviceHandle,
// PSTORAGE_PHYSICAL_TOPOLOGY_DESCRIPTOR  *topologyDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDevicePhysicalTopologyProperty, C_CAST(void**,
//     topologyDescriptor), sizeof(STORAGE_PHYSICAL_TOPOLOGY_DESCRIPTOR));
// }
...
*/
// static eReturnValues win_Get_Device_Attributes(HANDLE *deviceHandle,
// PSTORAGE_DEVICE_ATTRIBUTES_DESCRIPTOR *deviceAttributes)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceAttributesProperty, C_CAST(void**, deviceAttributes),
...
// static eReturnValues win_Get_Rpmb(HANDLE *deviceHandle,
// PSTORAGE_RPMB_DESCRIPTOR *rpmbDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageAdapterRpmbProperty, C_CAST(void**, rpmbDescriptor),
//     sizeof(STORAGE_RPMB_DESCRIPTOR));
// }

// static eReturnValues win_Get_Device_Management_Status(HANDLE *deviceHandle,
// PSTORAGE_DEVICE_MANAGEMENT_STATUS *managementStatus)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceManagementStatus, C_CAST(void**, managementStatus),
//     sizeof(STORAGE_DEVICE_MANAGEMENT_STATUS));
// }

// static eReturnValues win_Get_Adapter_Serial_Number(HANDLE *deviceHandle,
// PSTORAGE_ADAPTER_SERIAL_NUMBER *adapterSerialNumber)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageAdapterSerialNumberProperty, C_CAST(void**, adapterSerialNumber),
//     sizeof(STORAGE_ADAPTER_SERIAL_NUMBER));
// }

// static eReturnValues win_Get_Device_Location(HANDLE *deviceHandle,
// PSTORAGE_DEVICE_LOCATION_DESCRIPTOR *deviceLocation)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceLocationProperty, C_CAST(void**, deviceLocation),
//     sizeof(STORAGE_DEVICE_LOCATION_DESCRIPTOR));
// }
#        endif // WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_14393

#        if defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_15063
// static eReturnValues win_Get_Crypto_Property(HANDLE *deviceHandle,
// PSTORAGE_CRYPTO_DESCRIPTOR *cryptoDescriptor)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageAdapterCryptoProperty, C_CAST(void**, cryptoDescriptor),
//     sizeof(STORAGE_CRYPTO_DESCRIPTOR));
// }

// STORAGE_DEVICE_NUMA_NODE_UNKNOWN
// static eReturnValues win_Get_Device_Numa_Property(HANDLE *deviceHandle,
// PSTORAGE_DEVICE_NUMA_PROPERTY *numaProperty)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceNumaProperty, C_CAST(void**, numaProperty),
//     sizeof(STORAGE_DEVICE_NUMA_PROPERTY));
// }
#        endif // WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_15063

#        if defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_16299
// static eReturnValues win_Get_Device_Zoned_Property(HANDLE *deviceHandle,
// PSTORAGE_ZONED_DEVICE_DESCRIPTOR *zonedProperty)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceZonedDeviceProperty, C_CAST(void**, zonedProperty),
//     sizeof(STORAGE_ZONED_DEVICE_DESCRIPTOR));
// }

// static eReturnValues win_Get_Device_Unsafe_Shutdown_Count(HANDLE
// *deviceHandle, PSTORAGE_DEVICE_UNSAFE_SHUTDOWN_COUNT *unsafeShutdownCount)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceUnsafeShutdownCount, C_CAST(void**, unsafeShutdownCount),
//     sizeof(STORAGE_DEVICE_UNSAFE_SHUTDOWN_COUNT));
// }
#        endif // #if defined (WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >=
               // WIN_API_TARGET_WIN10_16299

#        if defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_18362
// static eReturnValues win_Get_Device_Endurance(HANDLE *deviceHandle,
// PSTORAGE_HW_ENDURANCE_DATA_DESCRIPTOR *enduranceProperty)
//{
//     return check_And_Get_Storage_Property(deviceHandle,
//     StorageDeviceEnduranceProperty, C_CAST(void**, enduranceProperty),
//     sizeof(STORAGE_HW_ENDURANCE_DATA_DESCRIPTOR));
// }
...
// This function is supposed to update partition table information after it has
// been changed...this should work after an erase has been done to a drive - TJE
...
{
#if defined(WINVER) && WINVER >= SEA_WIN32_WINNT_WINXP && defined(IOCTL_DISK_UPDATE_PROPERTIES)
    // TODO: Need to find a way to support other things like RAID or CSMI, etc in
    // the future - TJE
...
eReturnValues os_Erase_Boot_Sectors(tDevice* device)
{
    ...
    {
        // do not do anything here for RAID devices. This is really only needed for
        // JBOD.
        return SUCCESS;
    }
    else
...
// WinVer not wrapping this IOCTL...so it's probably old enough not to need it -
// TJE
static eReturnValues win_Get_Drive_Geometry(HANDLE devHandle, PDISK_GEOMETRY* geom)
{
    ...
    if (geom)
    {
        *geom = C_CAST(PDISK_GEOMETRY, safe_malloc(diskGeomSize));
        if (*geom)
        {
...
//         return BAD_PARAMETER;
//     }
//     if (DeviceIoControl(devHandle, IOCTL_DISK_IS_WRITABLE, M_NULLPTR, 0,
//     M_NULLPTR, 0, &bytesReturned, M_NULLPTR))
//     {
//         *writable = true;
//     }
//     else
...
// static M_INLINE void safe_free_disk_controller_number(DISK_CONTROLLER_NUMBER
// **ctrlnum)
// {
//     safe_Free(M_REINTERPRET_CAST(void**, ctrlnum));
// }
//
// static eReturnValues
// win_Get_IDE_Disk_Controller_Number_And_Disk_Number(HANDLE *devHandle,
// PDISK_CONTROLLER_NUMBER *numbers)
//{
//    eReturnValues ret = FAILURE;
//    DWORD bytesReturned = 0;
...
//    if (numbers)
//    {
//        *numbers = C_CAST(PDISK_CONTROLLER_NUMBER,
//        safe_malloc(controllerNumberSize)); if (*numbers)
//        {
//            memset(*numbers, 0, controllerNumberSize);
//            if (DeviceIoControl(devHandle, IOCTL_DISK_CONTROLLER_NUMBER,
//            M_NULLPTR, 0, *numbers, controllerNumberSize, &bytesReturned,
//            M_NULLPTR))
//            {
//                ret = SUCCESS;
//            }
...
// partInfo and detectInfo are optional and are meant to be helpful to set these
// pointers correctly for you from returned data rather than needing to figure
// it out yourself. geom should be free'd when done with it since it is
// allocated on the heap. If partinfo or detectinfo are null on completion, then
// these we not returned in this IOCTL
/* Example Use:
    PDISK_GEOMETRY_EX diskGeom = M_NULLPTR;
    PDISK_PARTITION_INFO partInfo = M_NULLPTR;
    PDISK_DETECTION_INFO diskDetect = M_NULLPTR;

    if (SUCCESS == win_Get_Drive_Geometry_Ex(device->os_info.fd, &diskGeom,
   &partInfo, &diskDetect))
    {
                                               ...
                                               PDISK_DETECTION_INFO* detectInfo)
{
    ...
    if (geom)
    {
        *geom = C_CAST(PDISK_GEOMETRY_EX, safe_malloc(diskGeomSize));
        if (*geom)
        {
            memset(*geom, 0, diskGeomSize);
...
// }
// static eReturnValues win_Get_Length_Information(HANDLE *devHandle,
...
//    if (length)
//    {
//        *length = C_CAST(PGET_LENGTH_INFORMATION,
//        safe_malloc(lengthInfoSize)); if (*length)
//        {
//            memset(*length, 0, lengthInfoSize);
//            if (DeviceIoControl(devHandle, IOCTL_DISK_GET_LENGTH_INFO,
//            M_NULLPTR, 0, *length, lengthInfoSize, &bytesReturned, M_NULLPTR))
//            {
//                ret = SUCCESS;
//            }
//            else
...
#if defined(WINVER) && WINVER >= SEA_WIN32_WINNT_WIN8
// static eReturnValues win_SCSI_Get_Inquiry_Data(HANDLE deviceHandle,
// PSCSI_ADAPTER_BUS_INFO *scsiBusInfo)
//{
//     eReturnValues ret = SUCCESS;
//     UCHAR busCount = 1, luCount = 1;
//     DWORD inquiryDataSize = sizeof(SCSI_INQUIRY_DATA) +
//     INQUIRYDATABUFFERSIZE;//this is supposed to be rounded up to an alignment
//     boundary??? but that is not well described... DWORD busDataLength =
//     sizeof(SCSI_ADAPTER_BUS_INFO) + busCount * sizeof(SCSI_BUS_DATA) +
//     luCount * inquiryDataSize;//Start with this, but more memory may be
//     necessary. *scsiBusInfo = C_CAST(PSCSI_ADAPTER_BUS_INFO,
//     safe_calloc_aligned(busDataLength, sizeof(uint8_t), 8)); if (scsiBusInfo)
//     {
//         BOOL success = FALSE;
//         DWORD returnedBytes = 0;
//         while (!success)
//         {
//             //try this ioctl and reallocate memory if not enough space error
//             is returned until it can be read! success =
//             DeviceIoControl(deviceHandle, IOCTL_SCSI_GET_INQUIRY_DATA,
//             M_NULLPTR, 0, scsiBusInfo, busDataLength, &returnedBytes,
//             M_NULLPTR); if (!success)
//             {
//                 DWORD error = GetLastError();
//                 //figure out what the error was to see if we need to allocate
//                 more memory, or exit with errors. if (error ==
//                 ERROR_INSUFFICIENT_BUFFER)
//                 {
//                     //MSFT recommends doubling the buffer size until this
//                     passes. void *tempBuf = M_NULLPTR; busDataLength *= 2;
//                     tempBuf = safe_realloc_aligned(scsiBusInfo, busDataLength
//                     / 2, busDataLength, 8); if (tempBuf)
//                     {
//                         scsiBusInfo = tempBuf;
//                     }
//                 }
...
static eReturnValues get_Win_Device(const char* filename, tDevice* device)
{
    ...
    device->os_info.fd = CreateFile(ptrDeviceName,
                                    /* We are reverting to the GENERIC_WRITE | GENERIC_READ because
                                       in the use case of a dll where multiple applications are
                                       using our library, this needs to not request full access. If
                                       you suspect some commands might fail (e.g. ISE/SED because
                                       of that please write to developers  -MA */
                                    GENERIC_WRITE | GENERIC_READ, // FILE_ALL_ACCESS,
                                    FILE_SHARE_READ | FILE_SHARE_WRITE, M_NULLPTR, OPEN_EXISTING,
#if !defined(WINDOWS_DISABLE_OVERLAPPED)
                                    FILE_FLAG_OVERLAPPED,
    ...
    else
    {
...
#endif                                                        // WIN_DEBUG
        device->os_info.scsiSRBHandle = INVALID_HANDLE_VALUE; // set this to invalid ahead of anywhere that it
                                                              // might get opened below for discovering
        ...
        // NOTE: No final else returning failure as we want to support some other
        // handles that don't map to these easy names.
        //       There are very long names for a drive handle we can also support if
        //       a caller knows how to pass them in.
#if defined(WIN_DEBUG)
       ...
       // map the drive to a volume letter
        ...
        driveLetters           = GetLogicalDrives();               // TODO: This will remount everything. If we can
                                                                   // figure out a better way to do this, we should so
                                                                   // that not everything is remounted. - TJE
        device->os_info.fileSystemInfo.fileSystemInfoValid = true; // Setting this since we have code here to detect the
                                                                   // volumes in the OS
        uint8_t volIter = 0;
            ...
            if (M_BitN(volIter) & driveLetters)
            {
                ...
                if (letterHandle != INVALID_HANDLE_VALUE)
                {
                    ...
                    DWORD                diskExtentsSizeBytes =
                        C_CAST(DWORD, sizeof(VOLUME_DISK_EXTENTS) + (sizeof(DISK_EXTENT) * maxExtents));
                    diskExtents = C_CAST(PVOLUME_DISK_EXTENTS, safe_malloc(diskExtentsSizeBytes));
                    if (diskExtents)
                    {
                            ...
                            for (DWORD counter = 0; counter < diskExtents->NumberOfDiskExtents; ++counter)
                            {
                                if (diskExtents->Extents[counter].DiskNumber == device->os_info.os_drive_number)
                                {
                                    device->os_info.fileSystemInfo.hasActiveFileSystem =
                                        true; // We found a filesystem for this drive, so set this
                                              // to true.

                                    // Set a bit to note that this particular volume (letter) is
                                    // on this device
                                    device->os_info.volumeBitField |= (UINT32_C(1) << volumeCounter);

                                    // now we need to determine if this volume has the system
                                    // directory on it.
                                    ...
                                    if (GetSystemDirectory(systemDirectoryPath, MAX_PATH) > 0)
                                    {
                                        ...
                                        {
                                            // we need to check only the first letter of the returned
                                            // string since this is the volume letter
                                        ...
                                        {
                                            printf("\nWARNING! Asked for system directory, but got a "
                                                   "zero length string! Unable to detect if this is "
                                                   "a drive with a system folder!\n");
                                        }
#endif // WIN_DEBUG
                                    }
                                }
       ...
       // saving max transfer size (in bytes)
            ...
            // saving the SRB type so that we know when an adapter supports the new
            // SCSI Passthrough EX IOCTLS - TJE
#if WINVER >= SEA_WIN32_WINNT_WIN8        // If this check is wrong, make sure minGW is properly
                                          // defining WINVER in the makefile.
            if (is_Windows_8_Or_Higher()) // from opensea-common now to remove
                                          // versionhelpes.h include
            {
            ...
            if (win_ret == SUCCESS)
            {
                ...
                // save the bus types to the tDevice struct since they may be helpful in
                // certain debug scenarios
                device->os_info.adapterDescBusType = adapter_desc->BusType;
                device->os_info.deviceDescBusType =
                    C_CAST(uint8_t,
                           device_desc->BusType); // NOTE: This enum seems to be a byte in definition
                                                  // today, but if we run into future issues, we may
                                                  // need to change it. - TJE
#if defined(WIN_DEBUG)
...
#    endif // WIN_DEBUG
                ...
                if (device_desc->BusType == BusTypeUnknown) // Add other device types that can't be handled with
                                                            // other methods of SCSI or ATA passthrough among
                                                            // other options below.
                {
                    ...
                    device->os_info.osReadWriteRecommended = true; // recommend calling this to reduce any translation
                                                                   // of calls that are received. - TJE
                }
                ...
                else if ((adapter_desc->BusType == BusTypeAtapi) || (device_desc->BusType == BusTypeAtapi))
                {
                    ...
                    device->drive_info.passThroughHacks.ataPTHacks.a1NeverSupported =
                        true; // This is GENERALLY true because almost all times the CDB
                              // is passed through instead of translated for a passthrough
                              // command, so just block it no matter what.
                    // These devices use the SCSI MMC command set in packet commands over
                    // ATA...other than for a few other commands. If we care to properly
                    // support this, we should investigate either how to send a packet
                    // command, or we should try issuing only SCSI commands
                ...
                else if (device_desc->BusType == BusTypeSata)
                {
                    ...
                    // we are assuming, for now, that SAT translation is being done below,
                    // and so far through testing on a few chipsets this appears to be
                    // correct.
                    device->drive_info.interface_type                               = IDE_INTERFACE;
                    device->os_info.ioType                                          = WIN_IOCTL_SCSI_PASSTHROUGH;
                    device->drive_info.passThroughHacks.someHacksSetByOSDiscovery   = true;
                ...
                {
                    // set this to SCSI_DRIVE. The fill_Drive_Info_Data call will change
                    // this if it supports SAT
                    device->drive_info.drive_type     = SCSI_DRIVE;
                ...
                // NVMe bustype can be defined for Win7 with openfabrics nvme driver, so
                // make sure we can handle it...it shows as a SCSI device on this
                // interface unless you use a SCSI?: handle with the IOCTL directly to
                // the driver.
                else if (device_desc->BusType == BusTypeNvme)
                {
                    device->drive_info.namespaceID = device->os_info.scsi_addr.Lun + 1;
                    if (device_desc->VendorIdOffset) // Open fabrics will set a vendorIDoffset,
                                                     // MSFT driver will not.
                    {
#if defined(WIN_DEBUG)
...
#        endif // WIN_DEBUG
                            if (!foundNVMePassthrough && supports_OFNVME_IO(device->os_info.scsiSRBHandle))
                            {
                                ...
                                device->os_info.osReadWriteRecommended =
                                    true; // setting this so that read/write LBA functions will
                                          // call Windows functions when possible for this.
                                foundNVMePassthrough = true;
                                // This is set because asking for list of pages + subpages
                                // returns a list wihtout them, so it is interpretted wrong. It
                                // returns the data for list of pages WITHOUT subpages, so CDB
                                // is not validated correctly. It is possible that other drivers
                                // based on ofnvme work different, but I don't have any others
                                // to test. Tested using the latest version before it stopped
                                // receiving support-TJE Code in ofnvme 1.5 does not properly
                                // validate if log subpages or mode subpages are requested and
                                // returns the incorrect data. setup some flags for everything
                                // else though based on what is in the translation code.
                                ...
                                device->drive_info.passThroughHacks.scsiHacks.securityProtocolSupported =
                                    true; // depending on if this is uncommented or not in the
                                          // compiled driver, it may or may not work, but code
                                          // seems to have some knowledge of this translation in
                                          // some cases.-TJE
...
#        endif // WIN_DEBUG
                            if (!foundNVMePassthrough && device_Supports_CSMI_With_RST(device))
                            {
                                ...
                                device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported.setFeatures =
                                    true;
                                device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                                    .firmwareCommit = true; // NOTE: Always requires a reboot for activation even
                                                            // when error code does not indicate this
                                device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                                    .firmwareDownload = true;
                                // supported features: power management, temperature threshold,
                                // APST, HCTM (17.2+), vendor specific.
                                ...
                                device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                                    .deviceSelfTest = true; // not documented, but found with trial and error.
                                                            // NOTE: some of this comes from an intel document and some
                                                            // comes from trial and error.
                                //       It is extremely likely that the supported commands vary
                                //       by driver version, but we do not have that level of
                                //       detail to populate this list at this time-TJE
...
#endif // WIN_DEBUG
       // unable to do passthrough, and isn't in normal Win10 mode, this means
       // it's some other driver that we don't know how to use. Treat as SCSI
...
#endif // WIN_DEBUG
       // close the handle that was opened. TODO: May need to remove this in the
       // future.
...
#    endif // WIN_DEBUG
                            ...
                            device->os_info.osReadWriteRecommended =
                                true; // setting this so that read/write LBA functions will
                                      // call Windows functions when possible for this,
                                      // althrough SCSI Read/write 16 will work too!
                            device->drive_info.passThroughHacks.someHacksSetByOSDiscovery                  = true;
                            device->drive_info.passThroughHacks.nvmePTHacks.limitedPassthroughCapabilities = true;
                            ...
                            device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported.getLogPage =
                                true; // This has seen some changes as Win10 API has advanced.
                                      // May need more specific flags!
                            device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                                .identifyController = true;
...
#    if defined(WIN_DEBUG)
                                printf("WIN: 1903+\n");
#    endif // WIN_DEBUG
           // this is definitely blocked in 1809, so this seems to have started
           // being available in 1903 NOTE: probably specific to a certain Win10
           // update. Not clearly documented when this became available, so need to
           // do some testing before this is perfect
                                device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                                    .deviceSelfTest = true;
#    if defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_14393
                                if ((device->os_info.fileSystemInfo.fileSystemInfoValid &&
                                     !device->os_info.fileSystemInfo.isSystemDisk))
                                {
                                    ...
                                    // IOCTL_STORAGE_REINITIALIZE_MEDIA changes behanvrio based on
                                    // OS version and drive capabilities.
#        if defined(ENABLE_TRANSLATE_FORMAT)
                                    device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                                        .formatCryptoSecureErase = true;
#        endif // ENABLE_TRANSLATE_FORMAT
               // NOTE: Cannot do format with user erase because that is not what MSFT
               // translates for the Sanitize block erase...it just does sanitize block
               // erase.
               //       No idea when this was implemented because the documentation on
               //       this translation is non-existent right now.
               // NOTE: It is not clear if sanitize crypto was switched to starting in
               // 1903 or if it was an earlier version. The documentation only goes back
               // to 1903 online.-TJE
                                    device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                                        .sanitizeCrypto = true;
                                }
#    endif // WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_14393
                            ...
                            device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported.securitySend =
                                true;
                            device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported.setFeatures =
                                true; // Only 1 feature today. <--this is old. There is now a
                                      // set features API, but I don't see what it does or
                                      // does not allow. Still needs implementing. - TJE
#    if defined(WIN_DEBUG)
...
#    endif // WIN_DEBUG
           // If in Windows PE, then these other commands become available
                                ...
                                device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported.sanitize =
                                    true;
                                // adding more detail since the previous bool was a little too
                                // generic and this will improve overall accuracy of supported
                                // commands-TJE
                                device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                                    .sanitizeCrypto = true;
...
#    if defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_20348
                                if ((device->os_info.fileSystemInfo.fileSystemInfoValid &&
                                     !device->os_info.fileSystemInfo.isSystemDisk))
                                {
                                    // Microsoft's documentation was wrong or just worded weirdly
                                    // that it seemeed like it should work in Win11 just like PE,
                                    // but that is not the case! The only way that seems to
                                    // actually work is PE as always or the reinitialize media
                                    // IOCTL which is slightly more limited in capabilities.
                                    device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                                        .sanitizeCrypto = true;
                                    device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported
                                        .sanitizeBlock = true;
...
#    endif // #if defined (WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >=
           // WIN_API_TARGET_WIN10_20348
                ...
                {
                    device->drive_info.interface_type = SCSI_INTERFACE;
                    // This does NOT mean that drive_type is SCSI, but set SCSI drive for
                    // now
                    device->drive_info.drive_type = SCSI_DRIVE;
                    device->os_info.ioType        = WIN_IOCTL_SCSI_PASSTHROUGH;
...
// RAID must also be considered due to how drivers, especially Intel's shows
// devices, both RAIDs and individual devices behind controllers in RAID mode.
#if defined(WIN_CSMI_PASSTHROUGH_SUPPORT_CHECK)
                        checkForCSMI = true;
#endif // WIN_CSMI_PASSTHROUGH_SUPPORT_CHECK
                    }
                    ...
                    {
#if defined(WIN_DEBUG)
                        printf("WIN: RAID bus type. Need to check for additional NVMe/CSMI "
                               "support\n");
#endif // WIN_DEBUG
       // TODO: Need to figure out a better way to decide this.
       //       Unfortunately, the Intel RST driver will show NVMe drives as
       //       RAID, but no vendor ID, so we need to check them all for this
       //       until we can find something else to use. This means issuing an
       //       admin identify which should fail gracefully on drivers that
       //       don't actually support this since it's vendor unique IOCTL code
       //       and signature.
                        checkForNVMe = true;
#if defined(WIN_CSMI_PASSTHROUGH_SUPPORT_CHECK)
                        checkForCSMI = true;
                ...
                // Doing this here because the NSID may be needed for NVMe over USB
                // interfaces too
                device->drive_info.namespaceID = device->os_info.scsi_addr.Lun + 1;
                ...
                // Note: On a couple systems here, when using double buffered IO with
                // ATA Pass-through, invalid checksums are returned for identify
                // commands, but direct is fine... Also observed that certain command
                // data is cached in double buffered mode, but not direct mode, which is
                // an issue for our utilities This is the other reason to force direct,
                // even though the code otherwise supports double buffered IO
                device->os_info.ioMethod = WIN_IOCTL_FORCE_ALWAYS_DIRECT;

                if (device->dFlags & OPEN_HANDLE_ONLY) // This is this far down because there is a lot of
                                                       // other things that need to be saved in order for
                                                       // windows pass-through to work correctly.
                {
                    return SUCCESS;
                }
                ...
                if (checkForNVMe)
                {
                        ...
                        SUCCESS == open_SCSI_SRB_Handle(device))
                    {
                        ...
                        {
                            // use OS read/write calls since this driver may not allow these
                            // to work since it is limited in capabilities for passthrough.
...
#endif // WIN_DEBUG
       // TODO: This passthrough may be limited in commands allowed to be sent.
       // If this is limited, need to fill in the nvme hacks to show what is or
       // is not supported.
                            device->drive_info.passThroughHacks.someHacksSetByOSDiscovery                  = true;
                            device->drive_info.passThroughHacks.nvmePTHacks.limitedPassthroughCapabilities = true;
                            ...
                            device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported.firmwareCommit =
                                true; // NOTE: Always requires a reboot for activation even
                                      // when error code does not indicate this
                            device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported.firmwareDownload =
                                true;
                            // supported features: power management, temperature threshold,
                            ...
                            device->drive_info.passThroughHacks.nvmePTHacks.limitedCommandsSupported.deviceSelfTest =
                                true; // not documented, but found with trial and error.
                                      // NOTE: some of this comes from an intel document and some comes
                ...
                /*
                While in most newer systems we found out that _force_ SCSI PassThrough
                will work, using older version of WinPE will cause the SCSI IOCTL to
                fail - MA
                */
...
#endif // WIN_DEBUG
       // we weren't successful getting device information...so now try
       // switching to the other IOCTLs
                    if (device->os_info.ioType == WIN_IOCTL_SCSI_PASSTHROUGH ||
                        device->os_info.ioType == WIN_IOCTL_SCSI_PASSTHROUGH_EX)
                    {
                        device->os_info.ioType = WIN_IOCTL_ATA_PASSTHROUGH;
                    }
                    else if (device->os_info.ioType == WIN_IOCTL_ATA_PASSTHROUGH) // ATA pass-through didn't
                                                                                  // work...so try SCSI
                    ...
                    if (ret != SUCCESS)
                    {
                        // if we are here, then we are likely dealing with an old legacy
                        // driver that doesn't support these other IOs we've been
                        // trying...so fall back to some good old legacy stuff that may
                        // still not work. - TJE
                        ...
                        // Try an identify command. Should we use check power mode command
                        // instead???
                        ...
                        ret                                    = fill_Drive_Info_Data(device);
#if defined(WIN_CSMI_PASSTHROUGH_SUPPORT_CHECK)
                        checkForCSMI = false; // if we are using any of these limited, old
                                              // IOCTLs, it is extremely
...
#    endif // WIN_DEBUG
           // open up the CSMI handle and populate the pointer to the csmidata
           // structure. This may allow us to work around other commands.
                ...
                // Fill in IDE for ATA interface so we can know based on scan output
                // which passthrough may need debugging
                if (device->drive_info.interface_type == IDE_INTERFACE)
                {
                    ...
                    // Setting the vendor ID for ATA controllers like this so we can have
                    // an idea when we detect what we think is IDE and what we think is
                    // SATA. This may be helpful for debugging later. - TJE
                ...
                // now windows api gives us some extra details that we should check to
                // make sure that our fill_Drive_Info_Data call did correctly...just for
                // some things the generic code may miss
                ...
                case BusTypeMmc:
                    ...
                    break;
                default:
                    // do nothing since we assume everything else was set correctly
                    // earlier
                    break;
                }
...
{
#if defined(ENABLE_CSMI)
    // check is the handle is in the format of a CSMI device handle so we can open
    // the csmi device properly.
...
//  get_Device_Count()
//
//! \brief   Description:  Get the count of devices in the system that this
//! library
//!                        can talk to. This function is used in conjunction
//!                        with get_Device_List, so that enough memory is
//!                        allocated.
//
//  Entry:
...
eReturnValues get_Device_Count(uint32_t* numberOfDevices, uint64_t flags)
{
    ...
    // Configuration manager library is not available on ARM for Windows. Library
    // didn't exist when I went looking for it - TJE ARM requires 10.0.16299.0 API
    // to get cfgmgr32 library! try forcing a system rescan before opening the
    ...
    for (driveNumber = 0; driveNumber < MAX_DEVICES_TO_SCAN; ++driveNumber)
    {
        ...
        {
            ++found;
            // Check if the interface is reported as RAID from adapter data because an
            // additional scan for RAID devices will be needed.
            ...
            if (SUCCESS == win_Get_Adapter_Descriptor(fd, &adapterData))
            {
                if (adapterData->BusType == BusTypeRAID)
                {
                    ...
                    // get the SCSI address for this device and save it to the RAID handle
                    // list so it can be scanned for additional types of RAID interfaces.
                    ...
                    if (SUCCESS == win_Get_SCSI_Address(fd, &scsiAddress))
                    {
                        ...
                        raidHint.unknownRAID = true; // TODO: Look up driver name to set hint instead of
                                                     // unknown to prevent excess IOCTLs being sent from
                                                     // retries.
                        snprintf(raidHandle, RAID_HANDLE_STRING_MAX_LEN, "\\\\.\\SCSI%" PRIu8 ":",
                                 scsiAddress.PortNumber);
                        if (VERBOSITY_COMMAND_NAMES <= winCountVerbosity)
                        {
                            printf("Adding SCSI port handle to RAID list to check for "
                                   "compatible devices: %s\n",
                                   raidHandle);
                        }
                        raidHandleList =
...
#if defined(ENABLE_CSMI)
    if (!(flags & GET_DEVICE_FUNCS_IGNORE_CSMI)) // check whether they want CSMI
                                                 // devices or not
    {
...
//!                        needed to be allocated for the device list. The
//!                        memory allocated must be the multiple of device
//!                        structure. The application can pass in less memory
//!                        than needed for all devices in the system, in which
//!                        case the library will fill the provided memory with
//!                        how ever many device structures it can hold.
//  Entry:
//!   \param[out] ptrToDeviceList = pointer to the allocated memory for the
//!   device list \param[in]  sizeInBytes = size of the entire list in bytes.
//!   \param[in]  versionBlock = versionBlock structure filled in by application
//!   for
...
//!
//  Exit:
//!   \return SUCCESS - pass, WARN_NOT_ALL_DEVICES_ENUMERATED - some deviec had
//!   trouble being enumerated.
//!                     Validate that it's drive_type is not UNKNOWN_DRIVE,
//!                     !SUCCESS fail or something went wrong
//
//-----------------------------------------------------------------------------
    ...
    else
    {
        ...
        for (driveNumber = 0; ((driveNumber < MAX_DEVICES_TO_SCAN) && (found < numberOfDevices)); driveNumber++)
        {
            ...
            if (fd != INVALID_HANDLE_VALUE)
            {
                ...
                else
                {
                    ...
                    if (SUCCESS == win_Get_Adapter_Descriptor(d->os_info.fd, &adapterData))
                    {
                        if (adapterData->BusType == BusTypeRAID)
                        {
                            // get the SCSI address for this device and save it to the RAID
                            // handle list so it can be scanned for additional types of RAID
                            // interfaces.
                            SCSI_ADDRESS scsiAddress;
                            memset(&scsiAddress, 0, sizeof(SCSI_ADDRESS));
                            ...
                            if (SUCCESS == win_Get_SCSI_Address(d->os_info.fd, &scsiAddress))
                            {
                                ...
                                if (VERBOSITY_COMMAND_NAMES <= winListVerbosity)
                                {
                                    printf("Adding %s to RAID handle list to scan for compatible "
                                           "devices\n",
                                           raidHandle);
                                }
            ...
            else
            {
                ...
                // NOTE: No generic else like other OS's due to the way devices are
                // scanned in Windows today. Since we are just trying to open handles,
                // they can fail for various reasons, like the handle not even being
                // valid, but that should not cause a failure. If the code is updated to
                // use something like setupapi or cfgmgr32 to figure out devices in the
                // system, then it would make sense to add additional error checks here
                // like we have for 'nix OSs. - TJE If a handle does not exist
        ...
        if (!(flags & GET_DEVICE_FUNCS_IGNORE_CSMI))
        {
            ...
            if (csmiDeviceCount > 0)
            {
                ...
                {
                    // this will override the normal ret if it is already set to success
                    // with the CSMI return value
                    returnValue = csmiRet;
                }
            }
        }
...
typedef struct _scsiPassThroughEXIOStruct
{
    ...
    UCHAR cdbPad[CDB_LEN_32 - 1]; // Padding for max sized CDB that the above struct can
                                  // make...this needs to be here because of the ANYSIZE_ARRAY
                                  // trick they use in the structure. If CDBs larger than 32B
                                  // are ever made, this will need adjusting - TJE
    ULONG          padding;       // trying to help buffer alignment like the MS example shows.
    STOR_ADDR_BTL8 storeAddr;
    UCHAR          senseBuffer[SPC3_SENSE_LEN]; // If we do auto-sense, we need to allocate
                                                // 252 bytes, according to SPC-3.
    UCHAR
    dataInBuffer[DOUBLE_BUFFERED_MAX_TRANSFER_SIZE];        // Setting to this
                                                            // defined value to help
                                                            // prevent problems...TJE
    UCHAR dataOutBuffer[DOUBLE_BUFFERED_MAX_TRANSFER_SIZE]; // Setting to this defined value to
                                                            // help prevent problems...TJE
...
static eReturnValues convert_SCSI_CTX_To_SCSI_Pass_Through_EX(ScsiIoCtx* scsiIoCtx, ptrSCSIPassThroughEXIOStruct psptd)
{
    ...
    case XFER_NO_DATA:
        ...
        break;
        // add in the next case later when we have better defined a difference
        // between in and out data buffers...-TJE
...
static eReturnValues send_SCSI_Pass_Through_EX(ScsiIoCtx* scsiIoCtx)
{
    ...
    ptrSCSIPassThroughEXIOStruct sptdioEx =
        C_CAST(ptrSCSIPassThroughEXIOStruct, safe_malloc(sizeof(scsiPassThroughEXIOStruct)));
    if (!sptdioEx)
    {
        return MEMORY_FAILURE;
    }
    ...
    {
        SetLastError(ERROR_SUCCESS); // clear any cached errors before we try to
                                     // send the command
        ...
        if (ERROR_IO_PENDING ==
            scsiIoCtx->device->os_info.last_error) // This will only happen for overlapped commands. If
                                                   // the drive is opened without the overlapped flag,
                                                   // everything will work like old synchronous code.-TJE
        {
        ...
        stop_Timer(&commandTimer);
        CloseHandle(overlappedStruct.hEvent); // close the overlapped handle since
                                              // it isn't needed any more...-TJE
        overlappedStruct.hEvent          = M_NULLPTR;
        ...
        else
        {
            ...
            case ERROR_ACCESS_DENIED:
                ...
                break;
            case ERROR_NOT_SUPPORTED: // this is what is returned when we try to send
                                      // a sanitize command in Win10
        ...
        {
            memcpy(scsiIoCtx->psense, &sptdioEx->senseBuffer[0],
                   M_Min(sptdioEx->scsiPassThroughEX.SenseInfoLength, scsiIoCtx->senseDataSize));
        }

        if (scsiIoCtx->psense != M_NULLPTR)
        {
            // use the format, sensekey, acq, acsq from the sense data buffer we
            // passed in rather than what windows reports...because windows doesn't
            // always match what is in your sense buffer
...
static eReturnValues send_SCSI_Pass_Through_EX_Direct(ScsiIoCtx* scsiIoCtx)
{
    ...
    ptrSCSIPassThroughEXIOStruct sptdio =
        C_CAST(ptrSCSIPassThroughEXIOStruct,
               safe_malloc(sizeof(scsiPassThroughEXIOStruct))); // add cdb and data length so that the
    ...
    // Check the alignment...if we need to use a local buffer, we'll use one, then
    // copy the data back
    if (scsiIoCtx->pdata && scsiIoCtx->device->os_info.alignmentMask != 0)
    {
        // This means the driver requires some sort of aligned pointer for the data
        // buffer...so let's check and make sure that the user's pointer is aligned
        // If the user's pointer isn't aligned properly, align something local that
        // is aligned to meet the driver's requirements, then copy data back for
        // them.
        ...
        if (alignedPointer != scsiIoCtx->pdata)
        {
            ...
            localBuffer              = C_CAST(uint8_t*, safe_calloc(totalBufferSize, sizeof(uint8_t)));
            if (!localBuffer)
            {
                perror("error allocating aligned buffer for ATA Passthrough "
                       "Direct...attempting to use user's pointer.");
    ...
    if (SUCCESS == convert_SCSI_CTX_To_SCSI_Pass_Through_EX_Direct(scsiIoCtx, sptdio, alignedPointer))
    {
        ...
        if (scsiIoCtx->psense != M_NULLPTR && scsiIoCtx->senseDataSize > 0)
        {
            memcpy(scsiIoCtx->psense, &sptdio->senseBuffer[0],
...
// This structure MUST be dynamically allocated for double buffered transfers so
// that there is enough room for the return data! - TJE
                                                                  ...
                                                                  uint8_t*                   alignedPointer)
{
    ...
    // Use offsetof macro to set where to place the sense data. Old code, for
    // whatever reason, didn't always work right...see comments below-TJE
    ...
    // psptd->scsiPassthroughDirect.SenseInfoOffset = C_CAST(ULONG,
    // (&psptd->senseBuffer[0] - C_CAST(uint8_t*,
                                                                           ...
                                                                           ptrSCSIPassThroughIOStruct psptd)
{
    ...
    case XFER_NO_DATA:
        ...
        psptd->scsiPassthrough.DataBufferOffset =
            offsetof(scsiPassThroughIOStruct,
                     dataBuffer); // this may also be better off as M_NULLPTR...IDK - TJE
        break;
    default:
        if (VERBOSITY_QUIET < scsiIoCtx->device->deviceVerbosity)
        {
    ...
    // sets the offset to the beginning of the sense buffer-TJE
    // psptd->scsiPassthrough.SenseInfoOffset = C_CAST(ULONG,
    // (&psptd->senseBuffer[0] - C_CAST(uint8_t*, &psptd->scsiPassthrough)));
...
static eReturnValues send_SCSI_Pass_Through(ScsiIoCtx* scsiIoCtx)
{
    ...
    BOOL                       success       = FALSE;
    ULONG                      returned_data = 0;
    ptrSCSIPassThroughIOStruct sptdioDB =
        C_CAST(ptrSCSIPassThroughIOStruct, safe_malloc(sizeof(scsiPassThroughIOStruct) + scsiIoCtx->dataLength));
    if (!sptdioDB)
    {
        return MEMORY_FAILURE;
    }
    ...
    if (SUCCESS == convert_SCSI_CTX_To_SCSI_Pass_Through_Double_Buffered(scsiIoCtx, sptdioDB))
    {
        ...
        if (scsiIoCtx->psense != M_NULLPTR && scsiIoCtx->senseDataSize > 0)
        {
            memcpy(scsiIoCtx->psense, sptdioDB->senseBuffer,
...
static eReturnValues send_SCSI_Pass_Through_Direct(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (SUCCESS == convert_SCSI_CTX_To_SCSI_Pass_Through_Direct(scsiIoCtx, &sptdio, alignedPointer))
    {
        ...
        if (scsiIoCtx->psense != M_NULLPTR && scsiIoCtx->senseDataSize > 0)
        {
            memcpy(scsiIoCtx->psense, sptdio.senseBuffer,
...
static eReturnValues send_SCSI_Pass_Through_IO(ScsiIoCtx* scsiIoCtx)
{
    ...
    {
        /*The following commend out of check on srbtype is a fix for the Microsoft
           Hyper-V issue where it was not able to support _EX commands even though
           the device / adapter descriptor said that it could based on the SRB
           Type.*/
        if (/*scsiIoCtx->device->os_info.srbtype == SRB_TYPE_SCSI_REQUEST_BLOCK &&
             */
            scsiIoCtx->cdbLength <= CDB_LEN_16 && scsiIoCtx->device->os_info.ioType == WIN_IOCTL_SCSI_PASSTHROUGH)
        {
                                                       ...
                                                       uint8_t*                 alignedDataPointer)
{
    ...
    case ATA_PROTOCOL_RET_INFO:
        // this doesn't do anything in ATA PassThrough and is only useful for SCSI
        // PassThrough since this is an HBA request, not a drive request, but we
        // don't want to print out an error message
        return NOT_SUPPORTED;
    default:
        if (VERBOSITY_QUIET < p_scsiIoCtx->device->deviceVerbosity)
        {
    ...
    if (p_scsiIoCtx->pAtaCmdOpts->commandType == ATA_CMD_TYPE_EXTENDED_TASKFILE)
    {
        ...
        ptrATAPassThroughDirect->PreviousTaskFile[5] = 0; // p_scsiIoCtx->pAtaCmdOpts->tfr.DeviceHead;//this is either 0
                                                          // or device...
...
static eReturnValues send_ATA_Passthrough_Direct(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (SUCCESS == ret)
    {
        ...
        if (success)
        {
            ...
            if (localAlignedBuffer && scsiIoCtx->direction == XFER_DATA_IN)
            {
                // memcpy the data back to the user's pointer since we had to allocate
                // one locally.
                memcpy(scsiIoCtx->pdata, alignedPointer, scsiIoCtx->dataLength);
            }
        }
        ...
        // get the rtfrs and put them into a "sense buffer". In other words, fill in
        // the sense buffer with the rtfrs in descriptor format current = 28bit,
        // previous = 48bit
        if (scsiIoCtx->psense != M_NULLPTR) // check that the pointer is valid
        {
            if (scsiIoCtx->senseDataSize >= 22) // check that the sense data buffer is big enough to fill in our
                                                // rtfrs using descriptor format
            {
                scsiIoCtx->returnStatus.format   = SCSI_SENSE_CUR_INFO_DESC;
                ...
                if (scsiIoCtx->pAtaCmdOpts->commandType == ATA_CMD_TYPE_EXTENDED_TASKFILE)
                {
                    ...
                    // fill in the ext registers while we're in this if...no need for
                    // another one
    ...
    scsiIoCtx->device->drive_info.lastCommandTimeNanoSeconds = get_Nano_Seconds(commandTimer);
    safe_free(&localBuffer); // This will check if M_NULLPTR before freeing it, so
...
typedef struct _ATADoubleBufferedIO
{
    ...
    ULONG               padding; // trying to help buffer alignment like the MS spti example
                                 // shows....may or may not be needed here
    UCHAR dataBuffer[1];
...
static eReturnValues convert_SCSI_CTX_To_ATA_PT_Ex(ScsiIoCtx* p_scsiIoCtx, ptrATADoubleBufferedIO p_t_ata_pt)
{
    ...
    case XFER_NO_DATA:
        ...
        p_t_ata_pt->ataPTCommand.DataBufferOffset   = offsetof(ATADoubleBufferedIO,
                                                               dataBuffer); // we always allocate at least 1 byte
                                                                            // here...so give it something? Or do we set
                                                                            // M_NULLPTR? Seems to work as is... - TJE
        break;
    default:
        if (VERBOSITY_QUIET < p_scsiIoCtx->device->deviceVerbosity)
        {
    ...
    if (p_scsiIoCtx->pAtaCmdOpts->commandType == ATA_CMD_TYPE_EXTENDED_TASKFILE)
    {
        ...
        p_t_ata_pt->ataPTCommand.PreviousTaskFile[5] = 0; // p_scsiIoCtx->pAtaCmdOpts->tfr.DeviceHead;//this is either 0
                                                          // or device...
...
static eReturnValues send_ATA_Passthrough_Ex(ScsiIoCtx* scsiIoCtx)
{
    ...
    ptrATADoubleBufferedIO doubleBufferedIO =
        C_CAST(ptrATADoubleBufferedIO, safe_malloc(sizeof(ATA_PASS_THROUGH_EX) + dataLength));
    if (!doubleBufferedIO)
    {
...
static eReturnValues convert_SCSI_CTX_To_IDE_PT(ScsiIoCtx* p_scsiIoCtx, ptrIDEDoubleBufferedIO p_t_ide_pt)
{
    ...
    case ATA_PROTOCOL_UDMA:
    case ATA_PROTOCOL_PACKET_DMA:
        // Some comments in old old legacy code say DMA is not supported with this
        // IOCTL, so they don't issue the command. We are allowing this through for
        // now until we can debug it ourselves to see if it was an implementation
        // issue or not using aligned memory, etc.
        break;
    case ATA_PROTOCOL_DEV_DIAG:
    case ATA_PROTOCOL_NO_DATA:
    case ATA_PROTOCOL_PACKET:
...
static eReturnValues send_IDE_Pass_Through_IO(ScsiIoCtx* scsiIoCtx)
{
    ...
    ptrIDEDoubleBufferedIO doubleBufferedIO =
        C_CAST(ptrIDEDoubleBufferedIO, safe_malloc(sizeof(IDEDoubleBufferedIO) - 1 + dataLength));
    if (!doubleBufferedIO)
    {
        // something went really wrong...
        return MEMORY_FAILURE;
    ...
    else
    {
        if (VERBOSITY_QUIET < scsiIoCtx->device->deviceVerbosity)
        {
            printf("Couldn't convert SCSI-To-IDE interface (legacy IDE double "
                   "buffered)\n");
        }
    }
    scsiIoCtx->device->drive_info.lastCommandTimeNanoSeconds = get_Nano_Seconds(commandTimer);
...
//     if (scsiIoCtx->pAtaCmdOpts->commandType ==
...
//     ideCtx.cdb[2] = scsiIoCtx->pAtaCmdOpts->tfr.ErrorFeature; // Features
//     Register ideCtx.cdb[3] = scsiIoCtx->pAtaCmdOpts->tfr.SectorCount; //
//     Sector Count Reg ideCtx.cdb[4] = scsiIoCtx->pAtaCmdOpts->tfr.LbaLow; //
//     Sector Number ( or LBA Lo ) ideCtx.cdb[5] =
//     scsiIoCtx->pAtaCmdOpts->tfr.LbaMid; // Cylinder Low ( or LBA Mid )
//     ideCtx.cdb[6] = scsiIoCtx->pAtaCmdOpts->tfr.LbaHi; // Cylinder High (or
//     LBA Hi) ideCtx.cdb[7] = scsiIoCtx->pAtaCmdOpts->tfr.DeviceHead; //
//     Device/Head Register ideCtx.cdb[8] =
//     scsiIoCtx->pAtaCmdOpts->tfr.CommandStatus; // Command Register
...
eReturnValues get_Windows_FWDL_IO_Support(tDevice* device, STORAGE_BUS_TYPE busType)
{
    ...
    uint8_t slotCount = 7; // 7 is maximum number of firmware slots...always reading with this for
                           // now since it doesn't hurt sas/sata drives. - TJE
    uint32_t outputDataSize = sizeof(STORAGE_HW_FIRMWARE_INFO) + (sizeof(STORAGE_HW_FIRMWARE_SLOT_INFO) * slotCount);
    uint8_t* outputData     = C_CAST(uint8_t*, safe_malloc(outputDataSize));
    if (!outputData)
    {
        return MEMORY_FAILURE;
    }
    ...
    // STORAGE_HW_FIRMWARE_REQUEST_FLAG_CONTROLLER is needed for NVMe to report
    // relavant data. Without it, we only see 1 slot available.
    ...
    // Got the version info, but that doesn't mean we'll be successful with
    // commands...
    if (fwdlRet)
    {
...
static eReturnValues win10_FW_Activate_IO_SCSI(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && !scsiIoCtx->pAtaCmdOpts)
    {
        downloadActivate.Slot = scsiIoCtx->cdb[2]; // Set the slot number to the buffer ID number...This
                                                   // is the closest this translates.
    }
    ...
    DWORD returned_data = 0;
    SetLastError(ERROR_SUCCESS); // clear any cached errors before we try to send
                                 // the command
    seatimer_t commandTimer;
    memset(&commandTimer, 0, sizeof(seatimer_t));
    OVERLAPPED overlappedStruct;
    ...
    scsiIoCtx->device->os_info.last_error = GetLastError();
    if (ERROR_IO_PENDING ==
        scsiIoCtx->device->os_info.last_error) // This will only happen for overlapped commands. If the
                                               // drive is opened without the overlapped flag,
                                               // everything will work like old synchronous code.-TJE
    {
    ...
    if (fwdlIO)
    {
        ...
        if (scsiIoCtx->pAtaCmdOpts)
        {
            ...
            scsiIoCtx->pAtaCmdOpts->rtfr.secCnt = 0x02; // This is supposed to be set when the drive has applied the new
                                                        // code.
    ...
    else
    {
        ...
        case ERROR_IO_DEVICE: // aborted command is the best we can do
            ...
            else
            {
                ...
                scsiIoCtx->psense[7] = 7; // set so that ASC, ASCQ, & FRU are
                                          // available...even though they are zeros
            }
            break;
        case ERROR_INVALID_FUNCTION:
            // disable the support bits for Win10 FWDL API.
            // The driver said it's supported, but when we try to issue the commands
            // it fails with this status, so try pass-through as we would otherwise
            // use.
            if (scsiIoCtx->device->deviceVerbosity >= VERBOSITY_COMMAND_VERBOSE)
            {
                printf("Win 10 FWDL API returned invalid function, retrying with "
                       "passthrough\n");
            }
...
// DO NOT Attempt to use the STORAGE_HW_FIRMWARE_DOWNLOAD_V2 structure! This is
// not compatible as the low-level driver has a hard-coded alignment for the
// image buffer and will not transmit the correct data!!!
...
static eReturnValues win10_FW_Download_IO_SCSI(ScsiIoCtx* scsiIoCtx)
{
    ...
    PSTORAGE_HW_FIRMWARE_DOWNLOAD downloadIO =
        C_CAST(PSTORAGE_HW_FIRMWARE_DOWNLOAD, safe_malloc(downloadStructureSize));
    if (!downloadIO)
    {
        return MEMORY_FAILURE;
    }
...
#    if defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_15063
    ...
    {
        // This IS documented on MSDN but VS2015 can't seem to find it...
        // One website says that this flag is new in Win10 1704 - creators update
        // (10.0.15021)
...
#    endif
    ...
    if (scsiIoCtx && !scsiIoCtx->pAtaCmdOpts)
    {
        downloadIO->Slot = scsiIoCtx->cdb[2]; // Set the slot number to the buffer ID number...This
                                              // is the closest this translates.
    }
    // we need to set the offset since MS uses this in the command sent to the
    // device.
...
// Checks if SMART IO is supported and sets some bit flags up for later when
// issuing those IOs
eReturnValues get_Windows_SMART_IO_Support(tDevice* device)
{
    ULONG              returned_data = 0;
    ...
    if (smartRet)
    {
        ...
        if (!smartError) // if there was an error, then assume the driver does not
                         // support this request. - TJE
        {
            if (smartVersionInfo.fCapabilities > 0)
            {
                ...
                // TODO: Save driver version info? skipping for now since it doesn't
                // appear useful.-TJE
...
static DWORD io_For_SMART_Cmd(ScsiIoCtx* scsiIoCtx)
{
    ...
    // this checks to make sure we are issuing a STD spec Identify or SMART
    // command to the drive. Non-standard things won't be supported by the lower
    // level drivers or OS - TJE
    switch (scsiIoCtx->pAtaCmdOpts->tfr.CommandStatus)
    {
    case ATA_IDENTIFY:
        // make sure it's standard spec identify! (no lba fields set or feature
        // field set)
        if (scsiIoCtx->pAtaCmdOpts->tfr.ErrorFeature != 0 || scsiIoCtx->pAtaCmdOpts->tfr.LbaLow != 0 ||
            scsiIoCtx->pAtaCmdOpts->tfr.LbaMid != 0 || scsiIoCtx->pAtaCmdOpts->tfr.LbaHi != 0)
        {
            return INVALID_IOCTL;
        }
    ...
    case ATA_SMART_CMD:
        ...
        {
            // check that the feature field matches something Microsoft documents
            // support for...using MS defines - TJE
...
static eReturnValues convert_SCSI_CTX_To_ATA_SMART_Cmd(ScsiIoCtx* scsiIoCtx, PSENDCMDINPARAMS smartCmd)
{
    ...
    smartCmd->bDriveNumber = 0; // MSDN says not to set this to anything!
...
static eReturnValues send_ATA_SMART_Cmd_IO(ScsiIoCtx* scsiIoCtx)
{
    ...
    uint32_t magicPadding  = 16; // This is found through trial and error to get
                                 // nForce 680i driver working.
    ...
    case XFER_DATA_OUT:
        ...
        break;
    case XFER_NO_DATA:
        dataOutLength += sizeof(IDEREGS); // need to add this much to the buffer to
                                          // get the RTFRs back as the data buffer!
        nonDataRTFRs = true;
        break;
    default:
        break;
    }
    PSENDCMDINPARAMS smartIOin = C_CAST(PSENDCMDINPARAMS, safe_calloc(1, sizeof(SENDCMDINPARAMS) - 1 + dataInLength));
    if (!smartIOin)
    {
        // something went really wrong...
        return MEMORY_FAILURE;
    }
    PSENDCMDOUTPARAMS smartIOout =
        C_CAST(PSENDCMDOUTPARAMS, safe_calloc(1, sizeof(SENDCMDOUTPARAMS) - 1 + dataOutLength + magicPadding));
    if (!smartIOout)
    {
        ...
        if (scsiIoCtx->psense != M_NULLPTR) // check that the pointer is valid
        {
                                                ...
                                                // rtfrs using descriptor format
            {
                ...
                case SMART_NO_ERROR: // command issued without error
                    ...
                    break;
                case SMART_IDE_ERROR: // command error...status register should get set
                                      // to 51h
                    smartTFRs.status = 0x51;
                    smartTFRs.error  = smartIOout->DriverStatus.bIDEError;
                    // This should be sufficient since the drive returned some other type
                    // of command abort...BUT we may want to attempt looking for RTFRs to
                    // know more.
                    break;
                ...
                case SMART_INVALID_REGISTER: // should we return NOT_SUPPORTED here? -
                                             // TJE
...
{
    eReturnValues ret = FAILURE;
    // this IOCTL is only supported for non-scsi devices, which includes anything
    // (ata or scsi) attached to a USB or SCSI or SAS interface This does not seem
    // to work since it is obsolete and likely not implemented in modern drivers
    // use the Windows API call -
    // http://msdn.microsoft.com/en-us/library/windows/hardware/ff560603%28v=vs.85%29.aspx
    // ULONG returned_data = 0;
    BOOL success = 0;
...
{
    eReturnValues ret = FAILURE;
    // This does not seem to work since it is obsolete and likely not implemented
    // in modern drivers use the Windows API call -
    // http://msdn.microsoft.com/en-us/library/windows/hardware/ff560600%28v=vs.85%29.aspx
    ULONG                     returned_data = 0;
    BOOL                      success       = 0;
...
// TODO: We may need to switch between locking fd and scsiSrbHandle in some
// way...for now just locking fd value.
...
static eReturnValues wbst_Inquiry(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && scsiIoCtx->cdbLength == 6)
    {
        ...
        else
        {
            ...
            {
                DECLARE_ZERO_INIT_ARRAY(uint8_t, vpdPage,
                                        96); // this should be more than big enough for the pages supported
                                             // on this basic device - TJE
                ...
                if (SUCCESS == win_Get_Device_Descriptor(scsiIoCtx->device->os_info.fd, &deviceDesc))
                {
                    ...
                    //       the vpd pagecodes below are probably the only ones that could
                    //       be implemented with the limited capabilities we have at this
                    //       point
                    ...
                    case UNIT_SERIAL_NUMBER:
                        ...
                        if (deviceDesc->RawPropertiesLength > 0)
                        {
                            ...
                            {
                                memcpy(&vpdPage[4], devSerial,
                                       M_Min(safe_strlen(devSerial),
                                             92)); // 92 for maximum size of current remaining
                ...
                {
                    // aborted command. TODO: Use windows errors to set more accurate
                    // sense data
                    senseKey     = SENSE_KEY_ABORTED_COMMAND;
                    asc          = 0x00;
                    ascq         = 0x00;
                ...
                if (scsiIoCtx->cdb[2] == 0)
                {
                    ...
                    if (SUCCESS == win_Get_Device_Descriptor(scsiIoCtx->device->os_info.fd, &deviceDesc))
                    {
                        ...
                        if (deviceDesc->RawPropertiesLength > 0)
                        {
                            ...
                            {
                                memset(&inquiryData[8], ' ',
                                       8); // space pad first as spec says ASCII data should be
                                           // space padded
                                memcpy(&inquiryData[8], devVendor,
                                       M_Min(safe_strlen(devVendor),
                                             8)); // maximum of 8 characters in length
                            }
                            else
                            {
                                // unknown vendor ID
                            ...
                            {
                                memset(&inquiryData[16], ' ',
                                       16); // space pad first as spec says ASCII data should be
                                            // space padded
                                memcpy(&inquiryData[16], devModel, M_Min(safe_strlen(devModel), 16));
                            }
                            else
                            {
                                inquiryData[16] = 'N';
                            ...
                            {
                                memset(&inquiryData[32], ' ',
                                       4); // space pad first as spec says ASCII data should be
                                           // space padded
                                memcpy(&inquiryData[32], devRev, M_Min(safe_strlen(devRev), 4));
                            }
                            else
                            {
                                inquiryData[32] = 'F';
                            ...
                            // SN is not described in spec to be here, but it is listed as
                            // "Vendor Specific", so we're putting it here since it is a
                            // fairly common practice overall. - TJE
                            if (deviceDesc->SerialNumberOffset && deviceDesc->SerialNumberOffset != UINT32_MAX)
                            {
                                memcpy(&inquiryData[36], devSerial, M_Min(safe_strlen(devSerial), 20));
                            }
                        }
                        else
                        {
                            // did not return the properties. Set "Unknown" for vendor and
                            // nothing else. vendorID
                            inquiryData[8]  = 'U';
                            inquiryData[9]  = 'N';
                            inquiryData[10] = 'K';
                            inquiryData[11] = 'N';
                    ...
                    {
                        // Nothing came back, so fill in with emptyness...just generate a
                        // response of any kind. vendorID
                        inquiryData[8]  = 'U';
                        inquiryData[9]  = 'N';
                        inquiryData[10] = 'K';
                        inquiryData[11] = 'N';
...
static eReturnValues wbst_Read_Capacity_16(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && scsiIoCtx->cdbLength >= 16)
    {
        ...
        if (M_GETBITRANGE(scsiIoCtx->cdb[1], 4, 0) == 0x10)
        {
            ...
            {
                // device geometry IOCTLs for remaining data
                // If Win version great enough, use the access alignment descriptor for
                // block sizes and alignment requirements
#if defined(WINVER) && WINVER >= SEA_WIN32_WINNT_VISTA
                    ...
                    win_Get_Drive_Geometry_Ex(scsiIoCtx->device->os_info.fd, &geometryEx, &partition, &detection))
                {
                    ...
                    // don't set any other fields unless we can get the access alignment
                    // descriptor
                    safe_free_disk_geometry_ex(&geometryEx);
                }
                else
                ...
                {
                    if (accessAlignment->BytesPerLogicalSector > 0) // making sure we don't divide by zero on bad
                                                                    // reports from the system
                    {
                        ...
                        readCapacityData[14] =
                            M_Byte1(lowestAlignedLBA) & 0x3F; // shouldn't cause a problem as alignment shouldn't be
                                                              // higher than this
...
static eReturnValues wbst_Read(ScsiIoCtx* scsiIoCtx, uint64_t lba, bool fua, uint32_t transferLength)
{
        ...
        (transferLength * scsiIoCtx->device->drive_info.deviceBlockSize) == scsiIoCtx->dataLength)
    {
        ...
        if (ret != SUCCESS)
        {
            // aborted command. TODO: Use windows errors to set more accurate sense
            // data
            senseKey     = SENSE_KEY_ABORTED_COMMAND;
            asc          = 0x00;
            ascq         = 0x00;
...
static eReturnValues wbst_Read_10(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && scsiIoCtx->cdbLength == 10)
    {
        ...
        if ((scsiIoCtx->cdb[1] & BIT0)    // reladr bit. Obsolete.
                                          //|| (scsiIoCtx->cdb[1] & BIT1)//FUA_NV bit.
                                          // Can be ignored by SATLs or implemented
            || (scsiIoCtx->cdb[1] & BIT2) // cannot support RACR bit in this
                                          // translation since we cannot do fpdma
...
static eReturnValues wbst_Write_6(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && scsiIoCtx->cdbLength == 6)
    {
        ...
        else
        {
            uint64_t lba = M_BytesTo4ByteValue(0, (scsiIoCtx->cdb[1] & 0x1F), scsiIoCtx->cdb[2], scsiIoCtx->cdb[3]);
            uint32_t transferLength = scsiIoCtx->cdb[4];
            if (transferLength == 0) // write 6, zero means a maximum possible
                                     // transfer size, which is 256
            {
                transferLength = 256;
            }
            if (transferLength > 65536)
...
static eReturnValues wbst_Write_16(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && scsiIoCtx->cdbLength == 16)
    {
        ...
        if ((scsiIoCtx->cdb[1] & BIT0)    // reladr bit. Obsolete. also now the DLD2 bit
                                          //|| (scsiIoCtx->cdb[1] & BIT1)//FUA_NV bit. Can be ignored by
                                          // SATLs or implemented
            || (scsiIoCtx->cdb[1] & BIT2) // reserved bit
            || (M_GETBITRANGE(scsiIoCtx->cdb[14], 7, 6) != 0))
        {
            // invalid field in CDB
...
static eReturnValues wbst_Verify_10(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && scsiIoCtx->cdbLength == 10)
    {
        ...
        else
        {
            ...
            if (verificationLength != 0) // this is allowed and it means to validate
                                         // inputs and return success
            {
...
static eReturnValues wbst_Test_Unit_Ready(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && scsiIoCtx->cdbLength == 6)
    {
        ...
        {
            // Do nothing. Just return ready unless we find some IO we can send that
            // does a TUR type of check - TJE
        }
        wbst_Set_Sense_Data(scsiIoCtx, setSenseData, senseKey, asc, ascq);
    }
    else
...
static eReturnValues wbst_Request_Sense(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && scsiIoCtx->cdbLength == 6)
    {
        ...
        {
            // Do nothing. Return good sense (all zeros) to the call since there is
...
static eReturnValues wbst_Send_Diagnostic(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && scsiIoCtx->cdbLength == 6)
    {
        ...
        if (scsiIoCtx->cdb[1] & 0xFB  // only allow self-test bit to be set to 1. All
        ...
        {
            // regardless of the self test bit being 1 or 0, return good status since
            // there is nothing that can be done right here
        }
        wbst_Set_Sense_Data(scsiIoCtx, setSenseData, senseKey, asc, ascq);
    }
    else
...
// TODO: This can be broken down into other functions to make this shorter and
// easier to follow.
//       Since this is extremely unlikely to be used as "basic" devices are not
//       very common, it is left like this for now-TJE
static eReturnValues wbst_Format_Unit(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx && scsiIoCtx->cdbLength == 6)
    {
        ...
        uint8_t ascq         = 0;
        bool    setSenseData = false;
        // NOTE: There is a format IOCTL not implemented, but it is likely only for
        // floppy drives.
        //       It may be worth implementing if they ever return from beyond the
        //       grave...or if we can test and prove it works on HDDs
        // Ideally this is a nop and it returns that it's ready without actually
        // doing anything
        ...
        else
        {
            ...
            if (formatData && scsiIoCtx->pdata && scsiIoCtx->dataLength > 4)
            {
                ...
                // uint8_t protectionFieldUsage = M_GETBITRANGE(scsiIoCtx->pdata[0], 2,
                // 0);
                bool formatOptionsValid = scsiIoCtx->pdata[1] & BIT7;
                // bool disablePrimary = scsiIoCtx->pdata[1] & BIT6; //ignore this bit.
                // Commented out so we can use it if we ever need to.
                ...
                bool    disableSaveParameters       = scsiIoCtx->pdata[1] & BIT2; // long
                                                                                  // obsolete
                ...
                if (                                      /*check all combinations that we don't support*/
                    (scsiIoCtx->pdata[0] != 0)            // we don't support protection information and reserved bytes
                                                          // should also be zeroed out
                    || (formatOptionsValid && stopFormat) // Doesn't make sense to support since we cannot
                                                          // accept a defect list. We could also ignore this,
                                                          // but this should be fine
                    || (disableSaveParameters)            // check that this obsolete bit isn't set
                    || (immediate)                        // we cannot support the immediate bit in this
                                                          // implementation right now. We would need
                                                          // multi-threading to do this...
                    || (vendorSpecific)                   // We don't have a vendor specific functionality
                                                          // in here
                ...
                {
                    // need to check a few more combinations. This code will NOT support
                    // certify unlike the SATL software translation code to simplify this
                    // a bit more. - TJE Check FOV bit combinations
                    if (formatOptionsValid)
                    {
                        bool performWriteOperation   = false;
                        bool performCertifyOperation = false;
                        // Initialization pattern information. Check that
                        // initializationPattern bool is set to true!
                        ...
                        if (initializationPattern)
                        {
                            // Set up the initialization pattern information since we were
                            // given one
                        ...
                        // Check bit combinations to make sure we can do things the right
                        // way!
                        ...
                        {
                            // SATL may or may not support media certification. If not
                            // supported, then return invalid field in parameter
                            // list...otherwise we have stuff to do
                        ...
                        else if (!disableCertification && initializationPattern)
                        {
                            ...
                            performCertifyOperation = true;
                            // SATL may or may not support write operation described by the
                            // initialization pattern descriptor to perform this translation
                        ...
                        // Before we begin writing or certification, we need to check some
                        // flags to make sure nothing invalid is set.
                        if ((initializationPattern &&       // this must be set for us to care
                                                            // about any of these other fields...
                             (initializationPatternModifier // check if obsolete bits are
                                                            // set
                              || securityInitialize         /*not supporting this since we cannot
                                                               write to the reallocated sectors on
                                                               the drive like this asks*/
                              || (initializationPatternByte0ReservedBits != 0) ||
                              ((initializationPatternType != 0x00 && initializationPatternType != 0x01)))) ||
                            performCertifyOperation // certify operation is not supported in
                                                    // this translation, only writes. -TJE
                        )
                        {
                            // invalid field in parameter list
                            senseKey     = SENSE_KEY_ILLEGAL_REQUEST;
                        ...
                        else
                        {
                            ...
                            {
                                // make sure the initialization pattern is less than or equal to
                                ...
                                else
                                {
                                    ...
                                    uint8_t* writePattern = C_CAST(
                                        uint8_t*, safe_calloc_aligned(writeSectors64K, sizeof(uint8_t),
                                    ...
                                    if (writePattern)
                                    {
                                        ...
                                        if (initializationPatternLength > 0)
                                        {
                                            ...
                                            for (uint32_t copyIter = 0; copyIter < writeDataLength;
                                                 copyIter += scsiIoCtx->device->drive_info.deviceBlockSize)
                                            {
                                                memcpy(&writePattern[copyIter], initializationPatternPtr,
                                                       initializationPatternLength);
                                            }
                                        }
                                        for (uint64_t lba = 0; lba < scsiIoCtx->device->drive_info.deviceMaxLba;
                                             lba += numberOfLBAs)
                                        {
                    ...
                    {
                        // Not checking disablePrimary below because the SATL ignores this
                        // bit
            ...
            {
                // just return success. This is similar to SAT translation, which
...
// This basic translation is for odd or old devices that aren't supporting
// passthrough. It is set to be more basic, while still fulfilling some required
// SCSI commands to make sure that upper layer code is happy with what this is
// reporting. This may be able to be expanded, but don't count on it too much.
// Expansion could come in VPD pages, maybe mode pages (read only most likely).
// But that is more than needed for now.
...
#    endif // WINVER >= SEA_WIN32_WINNT_WIN10
    ...
    else
    {
        ...
        {
        case IDE_INTERFACE:
            // Note: This code is problematic for ATAPI devices. Windows just passes
            // any CDB through to the device, which may not be what we want when we
            // want identify packet device data or other things like it.
            if (scsiIoCtx->device->os_info.ioType == WIN_IOCTL_ATA_PASSTHROUGH)
            {
                ...
                {
                    // ATAPI drives talk with SCSI commands...hopefully this works! If
                    // this doesn't work, then we will need to write some other function
                    // to check the request and call the Win API. - TJE
                ...
                else
                {
                    ret = translate_SCSI_Command(scsiIoCtx->device, scsiIoCtx);
                     ...
                     scsiIoCtx->device->os_info.ioType == WIN_IOCTL_SCSI_PASSTHROUGH_EX) // we need this else because
                                                                                         // we CAN issue scsi pass
                                                                                         // through commands to an
                                                                                         // ata interface drive;
                                                                                         // Windows returns dummied
                                                                                         // up data for things like
                                                                                         // inquiry and read capacity
                                                                                         // which we do send
                                                                                         // occasionally, so do not
                                                                                         // remove this.
            {
                if (scsiIoCtx->device->drive_info.drive_type == ATAPI_DRIVE) // ATAPI drives should just receive the CDB
                                                                             // to do what they will with it
                {
                    ret = send_SCSI_Pass_Through_IO(scsiIoCtx);
                }
                // else if (scsiIoCtx->pAtaCmdOpts->tfr.CommandStatus ==
                // ATA_IDENTIFY)//TODO: make sure all other LBA registers are zero
                //{
                //   ret = send_Win_ATA_Identify_Cmd(scsiIoCtx);
                // }
                // else if (scsiIoCtx->pAtaCmdOpts->tfr.CommandStatus ==
                // ATA_READ_LOG_EXT || scsiIoCtx->pAtaCmdOpts->tfr.CommandStatus ==
                // ATA_READ_LOG_EXT_DMA)
                //{
                //   ret = send_Win_ATA_Get_Log_Page_Cmd(scsiIoCtx);
                // }
                else if (scsiIoCtx->cdb[OPERATION_CODE] == ATA_PASS_THROUGH_12 ||
                         scsiIoCtx->cdb[OPERATION_CODE] == ATA_PASS_THROUGH_16)
                {
                    ret = send_SCSI_Pass_Through_IO(scsiIoCtx);
                }
                else // not letting the low level translate other things since many of
                     // the translations are wrong or fail to report errors - TJE
                {
                    ret = translate_SCSI_Command(scsiIoCtx->device, scsiIoCtx);
                }
            }
            ...
            {
                // In this case, we want to use the SMART IO over the IDE pass-through
                // whenever we can since it has better support. - TJE
                if (scsiIoCtx->pAtaCmdOpts)
                {
                    if (is_ATA_Cmd_Supported_By_SMART_IO(scsiIoCtx))
                    {
        ...
        case NVME_INTERFACE: // If on this path for NVMe, then let the driver do
                             // SCSI translation
        case SCSI_INTERFACE:
...
#if WINVER >= SEA_WIN32_WINNT_WIN10
/*
    MS Windows treats specification commands different from Vendor Unique
   Commands.
*/
...
static eReturnValues send_NVMe_Vendor_Unique_IO(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    {
        // assuming bidirectional commands have the same amount of data transferring
        // in each direction
    ...
    if (ERROR_IO_PENDING ==
        nvmeIoCtx->device->os_info.last_error) // This will only happen for overlapped commands. If the
                                               // drive is opened without the overlapped flag,
                                               // everything will work like old synchronous code.-TJE
    {
    ...
    if (ret == SUCCESS)
    {
        if (nvmeIoCtx->commandDirection != XFER_DATA_OUT && protocolCommand->DataFromDeviceBufferOffset != 0 &&
            nvmeIoCtx->ptrData)
        {
            memcpy(nvmeIoCtx->ptrData, &commandBuffer[protocolCommand->DataFromDeviceBufferOffset],
                   nvmeIoCtx->dataSize);
        }
    }
...
#    endif
    ...
    // not checking this today since we have the raw drive error response to use
    // instead.
...
static eReturnValues win10_Translate_Identify_Active_Namespace_ID_List(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    {
        // invalid field in cmd...assuming that this should be filtered since it is
        // not translatable!
        nvmeIoCtx->commandCompletionData.dw3Valid = true;
        nvmeIoCtx->commandCompletionData.dw3      = WIN_DUMMY_NVME_STATUS(NVME_SCT_GENERIC_COMMAND_STATUS, 2);
        nvmeIoCtx->device->drive_info.lastCommandTimeNanoSeconds = 0;
    ...
    {
        // NOTE: Need to only return NSID's that are greater than the requested
        // NSID, so will need to filter report LUNS output - TJE report LUNs uses
        // 64bits for each lun. NVMe uses 32bits for each NSID. This command
        // describes 1024 NSIDs, so we can request the whole thing, then
        // translate/filter as needed - TJE
        uint32_t reportLunsDataSize = 16 + (1024 * 8); // 131072B
        uint8_t* reportLunsData     = C_CAST(uint8_t*, safe_calloc_aligned(reportLunsDataSize, sizeof(uint8_t),
        ...
        if (reportLunsData)
        {
            ...
            {
                // Win10 follows SCSI translation and reports LUNs starting at zero, so
                // for each LUN in the list, add 1 to get a NSID. - TJE
                uint32_t lunListLength =
                    M_BytesTo4ByteValue(reportLunsData[0], reportLunsData[1], reportLunsData[2], reportLunsData[3]);
                // now go through the list and for each NSID greater than the provided
                // NSID in the command, add it to the return data (reported as LUNs so
                // add 1!)
                for (uint32_t lunOffset = 8, nsidOffset = 0;
                     lunOffset < (8 + lunListLength) && nsidOffset < nvmeIoCtx->dataSize &&
                     lunOffset < reportLunsDataSize;
                     lunOffset += 8)
                {
                    uint64_t lun = M_BytesTo8ByteValue(reportLunsData[lunOffset + 0], reportLunsData[lunOffset + 1],
                                                       reportLunsData[lunOffset + 2], reportLunsData[lunOffset + 3],
                                                       reportLunsData[lunOffset + 4], reportLunsData[lunOffset + 5],
                                                       reportLunsData[lunOffset + 6], reportLunsData[lunOffset + 7]);
...
static eReturnValues send_Win_NVMe_Identify_Cmd(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    if (cnsValue == 2)
    {
        // need to issue SCSI translation report LUNs and dummy the result back up
        // to the caller
        return win10_Translate_Identify_Active_Namespace_ID_List(nvmeIoCtx);
    }
    else if (cnsValue >= 3)
    {
        // NOTE: MSFT has changed their APIs a few times. This could change in the
        // future!
        return OS_COMMAND_NOT_AVAILABLE;
    }
    else
...
#    endif
        ...
        */
        ...
        {
            // these bytes are reserved in NVMe 1.2 which is the highest MS supports
            // right now. - TJE
            safe_free(&buffer);
            return OS_COMMAND_NOT_AVAILABLE;
        }
        ...
        switch (cnsValue)
        {
        case 0: // for the specified namespace. If nsid = UINT32_MAX it's for all
                // namespaces
        ...
        case 2: // list of 1024 active namespace IDs
            ...
            // NOTE: This command is documented in MSDN, but it doesn't actually work,
            // so we are returning an error instead! - TJE
            safe_free(&buffer);
            return OS_COMMAND_NOT_AVAILABLE;
            // All values below here are added in NVMe 1.3, which MS doesn't support
            // yet! - TJE
        case 3:    // list of namespace identification descriptor structures
                   // namespace management cns values
        case 0x10: // list of up to 1024 namespace IDs with namespace identifier
                   // greater than one specified in NSID
        case 0x11: // identify namespace data for NSID specified
        case 0x12: // list of up to 2047 controller identifiers greater than or
                   // equal to the value specified in CDW10.CNTID. List contains
                   // controller identifiers that are attached to the namespace
                   // specified
        case 0x13: // list of up to 2047 controller identifiers greater than or
                   // equal to the value specified in CDW10.CNTID. List contains
                   // controller identifiers that that may or may not be attached to
                   // namespaces
...
// So it is defined differently (name only) to prevent colisions if WinAPI
// version updates and has it at some point - TJE
...
// WIN_API_TARGET_WIN10_17763 gave offset fields to the get log page through a
// subvalue input. The documentation online states
// 1903(WIN_API_TARGET_WIN10_18362) and up support read buffer 16...it is
// unclear what version first supported the read buffer 16 method to read the
// telemetry log This function has been defined to read telemetry with the read
// buffer 16 command. NOTE: Read buffer 10 will NOT work, only 16. this function
// is only necessary for versions older than 1809...if readbuffer16 is even
// supported in those old versions. static eReturnValues
// get_NVMe_Telemetry_Data_With_RB16(nvmeCmdCtx* nvmeIoCtx)
//{
//     eReturnValues ret = BAD_PARAMETER;
//     uint8_t logID = M_Byte0(nvmeIoCtx->cmd.adminCmd.cdw10);
//     if (nvmeIoCtx->commandType == NVM_ADMIN_CMD &&
//     nvmeIoCtx->cmd.adminCmd.opcode == NVME_ADMIN_CMD_GET_LOG_PAGE && (logID
//     == NVME_LOG_TELEMETRY_CTRL_ID || logID == NVME_LOG_TELEMETRY_HOST_ID))
//     {
//         ret = SUCCESS;
//         uint8_t bufferID = 0x10;//Assume host as it is most common to pull
//         uint64_t numberOfDWords =
//         M_WordsTo4ByteValue(M_Word0(nvmeIoCtx->cmd.adminCmd.cdw11),
//         M_Word1(nvmeIoCtx->cmd.adminCmd.cdw10)); uint64_t logPageOffset =
//         M_DWordsTo8ByteValue(nvmeIoCtx->cmd.adminCmd.cdw13,
//         nvmeIoCtx->cmd.adminCmd.cdw12);
//         //bool createNewSnapshot = M_ToBool(nvmeIoCtx->cmd.adminCmd.cdw10 &
//         BIT8); if (logID == NVME_LOG_TELEMETRY_CTRL_ID)
//         {
//             bufferID = 0x11;
//         }
...
//         else
//         {
//             //now call the SCSI read-buffer 16 command with the correct
//             parameters to pull the data
//             //TODO: figure out how the "trigger" to create a new snapshot
//             will work. use the commented out createNewSnapshot above -TJE ret
//             = scsi_Read_Buffer_16(nvmeIoCtx->device, 0x1C, 0 /*TODO take new
//             snapshot???*/, bufferID, logPageOffset, C_CAST(uint32_t,
//             numberOfDWords << 4), nvmeIoCtx->ptrData);
...
#    if WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_20348
    ...
    PSTORAGE_PROTOCOL_DATA_SUBVALUE_GET_LOG_PAGE nvmeSubValue4 =
        C_CAST(PSTORAGE_PROTOCOL_DATA_SUBVALUE_GET_LOG_PAGE,
               &protocolData->ProtocolDataRequestSubValue4); // protocol data request subvalue 4
                                                             // is what this is in newer
                                                             // documentation...needs a different
                                                             // structure to really see if this
                                                             // way, but this should be
                                                             // compatible.
...
#    elif WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_18362
    ...
    protocolData->ProtocolDataRequestSubValue2 = nvmeIoCtx->cmd.adminCmd.cdw13;          // offset higher 32 bits
    protocolData->ProtocolDataRequestSubValue3 = M_Word1(nvmeIoCtx->cmd.adminCmd.cdw11); // log specific ID
    PMSFT_NVME_STORAGE_PROTOCOL_DATA_GET_LOG_PAGE_SUB_VALUE_4 nvmeSubValue4 =
        C_CAST(PMSFT_NVME_STORAGE_PROTOCOL_DATA_GET_LOG_PAGE_SUB_VALUE_4,
               &protocolData->Reserved); // protocol data request subvalue 4 is what
                                         // this is in newer documentation...needs a
                                         // different structure to really see if this
                                         // way, but this should be compatible.
    nvmeSubValue4->LogSpecificField = M_Nibble2(nvmeIoCtx->cmd.adminCmd.cdw10);
...
static eReturnValues send_Win_NVMe_Get_Features_Cmd(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    // Allocate buffer for use.
    //
    uint32_t nvmeGetFtMinSize = M_Max(4, nvmeIoCtx->dataSize); // This works around get features commands that
                                                               // are not actually transferring any data. If you
                                                               // size the buffer based on a zero data transfer,
                                                               // windows gives an error -TJE
    bufferLength = FIELD_OFFSET(STORAGE_PROPERTY_QUERY, AdditionalParameters) + sizeof(STORAGE_PROTOCOL_SPECIFIC_DATA) +
                   nvmeGetFtMinSize;
    buffer = safe_malloc(bufferLength);
...
#    endif
    ...
    protocolData->ProtocolDataRequestSubValue =
        nvmeIoCtx->cmd.adminCmd.cdw11; // latest Win API shows using CDW11 in a comment, which is a
...
#    endif
        ...
        nvmeIoCtx->commandCompletionData.commandSpecific =
            protocolData->FixedProtocolReturnData; // This should only be DWORD 0 on a get
                                                   // features command anyways...
        nvmeIoCtx->commandCompletionData.dw0Valid = true;
    }
...
#    endif
    ...
    downloadActivate.Version = sizeof(STORAGE_HW_FIRMWARE_ACTIVATE);
    downloadActivate.Size    = sizeof(STORAGE_HW_FIRMWARE_ACTIVATE);
    downloadActivate.Flags |= STORAGE_HW_FIRMWARE_REQUEST_FLAG_CONTROLLER; // this command must go to
                                                                           // the controller, not the
                                                                           // namespace
...
#    if defined(_DEBUG)
    printf("%s: "
           "downloadActivate->Version=%ld\n\t->Size=%ld\n\t->Flags=0x%lX\n\t->"
           "Slot=%d\n",
           __FUNCTION__, downloadActivate.Version, downloadActivate.Size, downloadActivate.Flags,
...
// uncomment this flag to switch to force using the older structure if we need
// to.
...
#    if defined(WIN_API_TARGET_VERSION) && !defined(DISABLE_FWDL_V2) &&                                                \
    ...
    PSTORAGE_HW_FIRMWARE_DOWNLOAD_V2 downloadIO =
        C_CAST(PSTORAGE_HW_FIRMWARE_DOWNLOAD_V2, safe_malloc(downloadStructureSize));
#        if defined(_DEBUG)
...
#    if defined(_DEBUG)
    ...
    // print_Data_Buffer(downloadIO->ImageBuffer, (downloadStructureSize -
    // FIELD_OFFSET(STORAGE_HW_FIRMWARE_DOWNLOAD, ImageBuffer)), false);
#    endif
...
static eReturnValues win10_Translate_Security_Send(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    // Windows API call does not exist...need to issue a SCSI IO and let the
    // driver translate it for us...how silly
    if (M_GETBITRANGE(nvmeIoCtx->cmd.adminCmd.cdw10, 7, 0) == 0) // check that the nvme specific field isn't set since
                                                                 // we can't issue that
    {
        // turn verbosity to silent since we don't need to see everything from
        // issueing the scsi io...purpose right now is to make it look like an NVM
        // io and be transparent to the caller.
...
static eReturnValues win10_Translate_Set_Error_Recovery_Time_Limit(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    if (!dulbe && !(nvmeIoCtx->cmd.adminCmd.cdw11 >> 16)) // make sure unsupported fields aren't set!!!
    {
        ...
        uint8_t* errorRecoveryMP =
            C_CAST(uint8_t*, safe_calloc_aligned(MODE_HEADER_LENGTH10 + MP_READ_WRITE_ERROR_RECOVERY_LEN,
                                                 sizeof(uint8_t), nvmeIoCtx->device->os_info.minimumAlignment));
        if (errorRecoveryMP)
        {
...
static eReturnValues win10_Translate_Set_Volatile_Write_Cache(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    if (!(nvmeIoCtx->cmd.adminCmd.cdw11 >> 31)) // make sure unsupported fields aren't set!!!
    {
        ...
        uint8_t* cachingMP =
            C_CAST(uint8_t*, safe_calloc_aligned(MODE_HEADER_LENGTH10 + MP_CACHING_LEN, sizeof(uint8_t),
                                                 nvmeIoCtx->device->os_info.minimumAlignment));
        if (cachingMP)
        {
            // first, read the page into memory
...
static eReturnValues win10_Translate_Set_Power_Management(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    if (workloadHint == 0 &&
        M_GETBITRANGE(nvmeIoCtx->cmd.adminCmd.cdw11, 31, 8) == 0) // cannot send workload hints in the API calls
                                                                  // available, also filtering out reserved bits
    {
        ...
        // Need to make sure this power value can actually work in Windows! MS only
        // supports values in milliwatts, and it's possible with the code above to
        // specify something less than 1 milliwatt...-TJE
...
static eReturnValues send_NVMe_Set_Temperature_Threshold(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    // TODO: check reserved fields are zero to return an error since they may
    // communicate a different behavior we are not currently
    ...
    tempThresh.Threshold = C_CAST(SHORT, temperatureThreshold - INT32_C(273)); // NVMe does every temp in kelvin, but
                                                                               // the API expects Celsius - TJE
    if (thsel == 0)
    {
...
static eReturnValues send_NVMe_Set_Features_Win10_Storage_Protocol(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    uint32_t      featTransferSize = (nvmeIoCtx->dataSize >= 4096)
                                         ? nvmeIoCtx->dataSize
                                         : 4096; // get features API requires a 4096 buffer, so allocating
                                                 // space for that in here too, in case this API is also
                                                 // expecting this data buffer size-TJE
    ...
    uint8_t* bufferData = C_CAST(uint8_t*, safe_calloc(bufferLength, sizeof(uint8_t)));
    if (bufferData)
    {
        ...
        if (nvmeIoCtx->ptrData && nvmeIoCtx->dataSize)
        {
            ...
            memcpy_s(C_CAST(uint8_t*, protocolSpecificData) + sizeof(STORAGE_PROTOCOL_SPECIFIC_DATA_EXT),
                     nvmeIoCtx->dataSize, nvmeIoCtx->ptrData, nvmeIoCtx->dataSize);
        }
        else
        {
            // no additional command data is being shared for this feature.
            // Due to get features requiring a 4096 sized buffer, we are passing one
            // in here too just to make Windows happy even if the feature does no t
            // use it.
...
#        endif
            ...
            nvmeIoCtx->commandCompletionData.commandSpecific =
                protocolSpecificData->FixedProtocolReturnData; // This should only be DWORD 0 on a get
                                                               // features command anyways...
            nvmeIoCtx->commandCompletionData.dw0Valid = true;
        }
...
#    if defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_18362
    // 1903 added storage_set_property IOCTL support, so try it first before
    // falling back on these other methods - TJE
    if (is_Windows_10_Version_1903_Or_Higher())
    {
...
#    endif
    ...
    {
        // Depending on the feature, we may need a SCSI translation, a Windows API
        // call, or we won't be able to perform any translation at all.
        ...
        default:
            ...
            {
                // call the vendor specific pass-through function to try and issue this
                // command
...
#        if defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_14393
// This IOCTL started in Windows 10, 1607. AKA 14393 API. It only supported NVM
// format with crypto erase though. When Windows 11 was released, this API added
// a structure to specify a sanitize command to run, but is limited to block and
// crypto erase only.
//  This Windows 11/10 API change was 20348 and supposedly also supports Windows
//  10 21H1
// Win11:
//   Issuing this IOCTL with no parameters on a drive that supports crypto erase
//   will issue sanitize crypto erase with the ause bit set to 1 Issuing this
//   IOCTL with no parameters on a drive that supports block erase will return
//   an error (1) for "incorrect function" Assumed that old drive without
//   sanitize crypto will run the format with crypto erase as this IOCTL
//   originally would do.
// Win10:
//   This will change depending on the version of Windows 10 that is running.
//   1607 - 1809(?) will do format NVM. Microsoft docs state 1903 and later
//   support sanitize, but it is not clear exactly where the ending cutoff is.
//   Starting with 21H1, it is assumed that the behavior matches Windows 11
//   behavior. NOTE: Need to figure out if we have a way to test specific
//   versions to figure out the exact cut off. -TJE Assumed that old drive
//   without sanitize crypto will run the format with crypto erase as this IOCTL
//   originally would do.
// Because of all the various changes in behavior, the function below has to
// assess the incoming command and the drive's santize capabilities to decide
// what it will do.
typedef enum _eNVM_ReInit_Compatible
{
    NVM_REINIT_INCOMPATIBLE_CMD,
    NVM_REINIT_INCOMPATIBLE_FORMAT,
    NVM_REINIT_INCOMPATIBLE_SANITIZE_OLD_API_OR_OS, // compiled with API before
                                                    // 20348 or old version of
                                                    // Windows 10 that doesn't
                                                    // support sanitize.
    NVM_REINIT_INCOMPATIBLE_SANITIZE_OPTIONS,       // a bit or option specified in
                                                    // sanitize is not compatible/able
                                                    // to be specified in this IOCTL
    NVM_REINIT_INCOMPATIBLE_SANITIZE_OVERWRITE,     // overwrite is not allowed in
                                                    // this API
    ...
    if (is_Windows_10_Version_1607_Or_Higher())
    {
        ...
        if (nvmeIoCtx && nvmeIoCtx->commandType == NVM_ADMIN_CMD)
        {
            ...
            if (nvmeIoCtx->cmd.adminCmd.opcode == NVME_ADMIN_CMD_FORMAT_NVM)
            {
                ...
                {
                    // TODO: Figure out exactly when this IOCTL switches to sanitize from
                    // format with crypto erase. MSDN documentatino does not specify
                    // earlier than 1903.-TJE Starting with at least 1903, possibly
                    // earlier, this IOCTL will issue sanitize crypto erase instead.
                    // because this is not necessarily a malformed command, but a
                    // completeley different than expected behavior, returning this
                    // instead.
                    return NVM_REINIT_INCOMPATIBLE_CMD;
                }
                else
                {
                    // next we need to screen other fields to make sure there aren't other
                    // things being asked while running the format.
            ...
            else if (nvmeIoCtx->cmd.adminCmd.opcode == NVME_ADMIN_CMD_SANITIZE)
            {
                ...
                if (is_Windows_10_Version_21H1_Or_Higher())
                {
                    ...
                    // cannot currently specify "no deallocate after sanitize", so if this
                    // is set, return incompatible options
                    ...
                    {
                        // microsoft is PROBABLY setting the AUSE bit since that is default
                        // behavior in newer versions, so screen for that since this version
                        // is too old to take parameters
                        return compat;
                    }
                    // ignore the following since they only apply to overwrite which isn't
                    // supported anyways ignore "overwrite invert pattern between passes"
                    // ignore "overwrite pass count"
                    // ignore "overwrite pattern"

                    // If we are here and this is a supported OS and API, check for a
                    // valid Sanitize operation.
                    ...
                    case SANITIZE_NVM_BLOCK_ERASE:
                        if (is_Windows_10_Version_21H1_Or_Higher())
                        {
                            // can only specify sanitize block erase starting Windows 10 21H1
                            // or higher with this IOCTL
                            compat = NVM_REINIT_COMPATIBLE_SANITIZE_BLOCK;
                        }
                        break;
                    case SANITIZE_NVM_CRYPTO:
                        // 1909 and higher will likely allow sanitize crypto if all the bits
                        // are set as expected, whether the ioctl is given parameters or
                        // not. 21h1 definitely will, but between 1909 and 21h1 no
                        // parameters are accepted, so it is not clear what the behavior is.
...
static eReturnValues nvme_Ioctl_Storage_Reinitialize_Media(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    if (is_Windows_10_Version_1607_Or_Higher())
    {
        // Now make sure we have either format with crypto erase or sanitize block
        // erase or sanitize crypto erase.
...
#            endif //(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >=
                   // WIN_API_TARGET_WIN10_20348
            if (compatIO == NVM_REINIT_COMPATIBLE_FORMAT_CRYPTO || compatIO == NVM_REINIT_COMPATIBLE_SANITIZE_CRYPTO)
            {
                // Issue without the parameters for Format with crypto erase only! (in
                // 1607) At some point this switched to sanitize crypto when the drive
                // supports it, but it is not clear when. The compatibility checking
                // code assumes this happened in 1903, but it is not clear exactly when
                // that happened. So if we get to this case and one of the two crypto
                // copatible erases is specified, this will issue the command. If we
                // ever get more info to further refine the compatibility checks, we
                // should do that!
                seatimer_t commandTimer;
                memset(&commandTimer, 0, sizeof(seatimer_t));
                start_Timer(&commandTimer);
                ...
                if (nvmeIoCtx->device->deviceVerbosity >= VERBOSITY_COMMAND_VERBOSE)
                {
                    printf("Sending IOCTL_STORAGE_REINITIALIZE_MEDIA for Format/Sanitize "
                           "Crypto\n");
                }
...
#    if defined(ENABLE_TRANSLATE_FORMAT)
// This code is currently disabled due to the format unit translation not
// actually working. Also, microsoft updated some online documentation to show
// this and it clarified that the sanitize CDB issues the sanitize command. It
// is possible that some earlier version of Windows 10 supported format
// translation or handled sanitize differently, but that information is not
// available.
static eReturnValues win10_Translate_Format(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    if (reservedBitsDWord10 == 0 && !pil && !mset && pi == 0)
    {
        // bool issueFormatCMD = false; //While this says this command is
        // translated, I have yet to figure out which combo makes it work.
        ...
        if (lbaSize != nvmeIoCtx->device->drive_info.deviceBlockSize)
        {
            ...
            // The only mode page supported by Windows is the caching mode page. with
            // size set to 0x0A for a total of 12B of data
            //  block descriptors are never returned.
            //  only mode sense 10 works. LongLBA does NOT work.
            //  No matter which combo of bits/fields used for mode select, it is
            //  always rejected with invalid field in CDB.
            return OS_COMMAND_NOT_AVAILABLE;
        }
        else
        {
            // NO LBA size change.
            // In this case we MIGHT be able to run a secure erase using the
            // non-standard sanitize block erase or crypto erase translation MSFT
            // mentions this briefly in their documentation here:
            ...
            if (secureEraseSettings != 0)
            {
                ...
                //  NOTE: If you use sanitize block erase CDB, this translates to
                //  sanitize block erase NVM
                //        If you use sanitize crypto erase CDB, it translates to format
                //        + crypto erase from what I can tell. This is not documented,
                //        but found through trial and error testing and debugging. I've
                //        commented out the sanitize block erase since this does not
                //        issue the expected command.
                // if (secureEraseSettings == 1)
                //{
                //     //block
                //     ret = scsi_Sanitize_Block_Erase(nvmeIoCtx->device, false, false,
                //     false);
                // }
                // else
...
#        if defined(WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_14393
                    // crypto
                    // NOTE: the IOCTL_STORAGE_REINITIALIZE_MEDIA can be used for this too
                    // IF no buffer length is passed in win 10 1607.
                    //       By 1903, this IOCTL instead issues sanitize crypto erase
                    //       instead from what I can tell from trial and error testing and
                    //       the limited documentation available. It is unknown when this
                    //       CDB translation became available, but will choose which
                    //       command to issue depending on the fields and OS version as
                    //       best we can.
...
#        endif // #if defined (WIN_API_TARGET_VERSION) && WIN_API_TARGET_VERSION >=
            ...
            {
                // I've tried the format unit command without parameters and no luck. I
                // have tried with fmtdata and no luck.
                ret = OS_COMMAND_NOT_AVAILABLE;
            }
        }
        // if (issueFormatCMD)
        //{
        //     //format unit command
        //     //set up parameter data if necessary and send the scsi format unit
        //     command to be translated and sent to the drive. The mode sense/select
        //     should have been cached to be used by the format command.
        //     //if (pi == 0)
        //     //{
        //     //    //send without parameter data
        //     //    ret = scsi_Format_Unit(nvmeIoCtx->device, 0, false, false,
        //     false, 0, 0, M_NULLPTR, 0, 0, 60);
        //     //}
        //     //else
        //     //{
        //     //send with parameter data
        //     //DECLARE_ZERO_INIT_ARRAY(uint8_t, formatParameterData, 4);//short
        //     header
        //     //uint8_t fmtpInfo = 0;
        //     //uint8_t piUsage = 0;
        //     //switch (pi)
        ...
        //     ret = scsi_Format_Unit(nvmeIoCtx->device, 0, false, false, false, 0,
        //     0, M_NULLPTR, 0, 0, 60);
        //     //}
        // }
    }
...
static eReturnValues win10_Translate_Write_Uncorrectable(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    for (uint16_t iter = 0; iter < (M_Word0(nvmeIoCtx->cmd.nvmCmd.cdw12) + 1);
         ++iter) //+1 because nvme uses a zero based range value
    {
        ...
        {
            // This is making sure we don't have a bad command followed by a good
            // command, then missing a bad status code
...
static eReturnValues win10_Translate_Flush(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    eVerbosityLevels inVerbosity       = nvmeIoCtx->device->deviceVerbosity;
    nvmeIoCtx->device->deviceVerbosity = VERBOSITY_QUIET;
    // NOTE: Switched to synchronize cache 10 due to documentation from MSFT only
    // specifying the 10byte CDB opcode - TJE
...
static eReturnValues win10_Translate_Read(nvmeCmdCtx* nvmeIoCtx)
{
    eReturnValues    ret         = OS_COMMAND_NOT_AVAILABLE;
    eVerbosityLevels inVerbosity = nvmeIoCtx->device->deviceVerbosity;
    // extract fields from NVMe context, then see if we can put them into a
    // compatible SCSI command
    ...
    if (rdProtect != 0xFF && dsm == 0 && !limitedRetry) // rdprotect must be a valid value AND these other fields
                                                        // must not be set...-TJE
    {
        // NOTE: Spec only mentions translations for read 10, 12, 16...but we may
        // also need 32! Even though it isn't in the spec, we'll attempt it anyways
        // when we have certain fields set... - TJE
...
static eReturnValues win10_Translate_Write(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    if (wrProtect != 0xFF && dsm == 0 && !limitedRetry && dtype == 0 &&
        dspec == 0) // rdprotect must be a valid value AND these other fields must
                    // not be set...-TJE
    {
        // NOTE: Spec only mentions translations for write 10, 12, 16...but we may
        // also need 32! Even though it isn't in the spec, we'll attempt it anyways
        // when we have certain fields set... - TJE
...
// MSFT documentation does not show this translation as available. Code left
// here in case someone wants to test it in the future. static eReturnValues
// win10_Translate_Compare(nvmeCmdCtx *nvmeIoCtx)
//{
//     eReturnValues ret = OS_COMMAND_NOT_AVAILABLE;
//     eVerbosityLevels inVerbosity = nvmeIoCtx->device->deviceVerbosity;
//     //extract fields from NVMe context, then see if we can put them into a
//     compatible SCSI command uint64_t startingLBA =
//     M_BytesTo8ByteValue(M_Byte3(nvmeIoCtx->cmd.nvmCmd.cdw11),
//     M_Byte2(nvmeIoCtx->cmd.nvmCmd.cdw11),
//     M_Byte1(nvmeIoCtx->cmd.nvmCmd.cdw11),
//     M_Byte0(nvmeIoCtx->cmd.nvmCmd.cdw11),
//     M_Byte3(nvmeIoCtx->cmd.nvmCmd.cdw10),
//     M_Byte2(nvmeIoCtx->cmd.nvmCmd.cdw10),
//     M_Byte1(nvmeIoCtx->cmd.nvmCmd.cdw10),
//     M_Byte0(nvmeIoCtx->cmd.nvmCmd.cdw10)); bool limitedRetry =
//     nvmeIoCtx->cmd.nvmCmd.cdw12 & BIT31; bool fua =
//     nvmeIoCtx->cmd.nvmCmd.cdw12 & BIT30; uint8_t prInfo =
//     M_GETBITRANGE(nvmeIoCtx->cmd.nvmCmd.cdw12, 29, 26); bool pract = prInfo &
//     BIT3; uint8_t prchk = M_GETBITRANGE(prInfo, 2, 0); uint16_t
//     numberOfLogicalBlocks = M_Word0(nvmeIoCtx->cmd.nvmCmd.cdw12) + 1;
//     uint32_t expectedLogicalBlockAccessTag = nvmeIoCtx->cmd.nvmCmd.cdw14;
//     uint16_t expectedLogicalBlockTagMask =
//     M_Word1(nvmeIoCtx->cmd.nvmCmd.cdw12); uint16_t
//     expectedLogicalBlockApplicationTag =
//     M_Word0(nvmeIoCtx->cmd.nvmCmd.cdw12);
//     //now validate all the fields to see if we can send this command...
...
//         }
//     }
//     if (vrProtect != 0xFF && byteCheck != 0xFF && !limitedRetry &&
//     !fua)//vrProtect must be a valid value AND these other fields must not be
//     set...-TJE
//     {
//         //NOTE: Spec only mentions translations for verify 10, 12, 16...but
//         we may also need 32!
//         //Even though it isn't in the spec, we'll attempt it anyways when we
//         have certain fields set... - TJE if (expectedLogicalBlockAccessTag !=
//         0 || expectedLogicalBlockApplicationTag != 0 ||
//         expectedLogicalBlockTagMask != 0)
//         {
//             //verify 32 command
//             ret = scsi_Verify_32(nvmeIoCtx->device, vrProtect, false,
//             byteCheck, startingLBA, 0, numberOfLogicalBlocks,
//             nvmeIoCtx->ptrData, nvmeIoCtx->dataSize,
//             expectedLogicalBlockAccessTag,
//             expectedLogicalBlockApplicationTag, expectedLogicalBlockTagMask);
//         }
//         else
//         {
//             //verify 16 should work
//             ret = scsi_Verify_16(nvmeIoCtx->device, vrProtect, false,
//             byteCheck, startingLBA, 0, numberOfLogicalBlocks,
//             nvmeIoCtx->ptrData, nvmeIoCtx->dataSize);
//         }
//     }
...
// uncomment this to enable returning a not supported value when a context
// attribute is set #define WIN_NVME_DEALLOCATE_CONTEXT_FAILURE
...
static eReturnValues win10_Translate_Data_Set_Management(nvmeCmdCtx* nvmeIoCtx)
{
          ...
          (nvmeIoCtx->cmd.nvmCmd.cdw11 >> 3))) // checking for supported/unsupported flags and reserved bits
    {
        ...
        // NOTE: All context attributes will be ignored since it cannot be
        // translated. It is also optional and the controller may not do anything
        // with it. We can optionally check for this, but I do not think it's really
        // worth it right now. I will have code in place for this, just commented
        // out - TJE
...
#    endif // WIN_NVME_DEALLOCATE_CONTEXT_FAILURE
        ...
        uint8_t* unmapParameterData       = C_CAST(
            uint8_t*, safe_calloc_aligned(unmapParameterDataLength, sizeof(uint8_t),
                                                nvmeIoCtx->device->os_info.minimumAlignment)); // each range is 16 bytes
                                                                                               // plus an 8 byte header
        if (unmapParameterData)
        {
            // in a loop, set the unmap descriptors
            uint32_t scsiOffset = 8;
            uint32_t nvmOffset  = 0;
            for (uint16_t rangeIter = 0;
...
// In the Win 10, 1607 update, the IOCTL for Storage Reinitialize Media was
// added, which supported crypto erase. While this WAS working, it does not
// appear to work properly in the latest Windows 10 build and returns "Incorrect
// Function" meaning it is not a supported API. I did ask Microsoft a question
// about SCSI translation and they updated the docs to show the SCSI Sanitize
// CDB as supported for Block and Crypto erase. Since this IOCTL is not working
// in Win 10 22H2 for some unknown reason, this code will issue the SCSI CDB
// instead. If we need to we can update this code to calling that IOCTL again,
// but the SSCI CDB's work the same without random errors. -TJE
static eReturnValues win10_Translate_Sanitize(nvmeCmdCtx* nvmeIoCtx)
{
    eReturnValues ret = OS_COMMAND_NOT_AVAILABLE;
    if (is_Windows_10_Version_1607_Or_Higher()) // this is for IOCTL reinitialize
                                                // media. Not 100% sure when SCSI
                                                // Sanitize CDB translation was
                                                // supported. Update this if we
                                                // ever find out.-TJE
    {
        // This was the original code for the reinitialize IOCTL.
        // Since we ran into a weird compatibility issue, it is commented out.
        // Will need to add some conditions for when to call it, but it will build
        // and issue the IOCTL correctly-TJE #if defined (WIN_API_TARGET_VERSION) &&
        // WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_14393
        //     ret = nvme_Ioctl_Storage_Reinitialize_Media(nvmeIoCtx);
        // #endif //WIN_API_TARGET_VERSION >= WIN_API_TARGET_WIN10_14393
        ...
        else
        {
            ...
            nvmeIoCtx->device->drive_info.defaultTimeoutSeconds = 600; // change to 10 minutes since this does not
                                                                       // return immediately in Windows-TJE
            nvmeIoCtx->device->deviceVerbosity = VERBOSITY_QUIET;
            if (action == 4) // crypto
            {
...
// These commands are not supported VIA SCSI translation. There are however
// other Windows IOCTLs that may work static eReturnValues
// win10_Translate_Reservation_Register(nvmeCmdCtx *nvmeIoCtx)
//{
//     eReturnValues ret = OS_COMMAND_NOT_AVAILABLE;
//     eVerbosityLevels inVerbosity = nvmeIoCtx->device->deviceVerbosity;
//     //Command inputs
...
//     //uint64_t crkey = M_BytesTo8ByteValue(nvmeIoCtx->ptrData[0],
//     nvmeIoCtx->ptrData[1], nvmeIoCtx->ptrData[2], nvmeIoCtx->ptrData[3],
//     nvmeIoCtx->ptrData[4], nvmeIoCtx->ptrData[5], nvmeIoCtx->ptrData[6],
//     nvmeIoCtx->ptrData[7]); uint64_t nrkey =
//     M_BytesTo8ByteValue(nvmeIoCtx->ptrData[8], nvmeIoCtx->ptrData[9],
//     nvmeIoCtx->ptrData[10], nvmeIoCtx->ptrData[11], nvmeIoCtx->ptrData[12],
//     nvmeIoCtx->ptrData[13], nvmeIoCtx->ptrData[14], nvmeIoCtx->ptrData[15]);
//     //scsi command stuff
//     DECLARE_ZERO_INIT_ARRAY(uint8_t, scsiCommandData, 24);
//     uint8_t scsiServiceAction = 0;
//     bool issueSCSICommand = false;
//     //now check that those can convert to SCSI...if they can, then convert
//     it! if (!iekey && (rrega == 0 || rrega == 1) && (cptpl == 2 || cptpl ==
//     3))
//     {
...
//     }
//     else if (iekey && (rrega == 0 || rrega == 1) && (cptpl == 2 || cptpl ==
//     3))
//     {
//         //can translate. Service action is 6 (Register and ignore existing
//         key) scsiServiceAction = 6;
//         //set up the data buffer
//         if (nrkey == 0)
//         {
//             //reservation key
...
//     if (issueSCSICommand)
//     {
//         //if none of the above checks caught the command, then it cannot be
//         translated nvmeIoCtx->device->deviceVerbosity = VERBOSITY_QUIET; ret
//         = scsi_Persistent_Reserve_Out(nvmeIoCtx->device, scsiServiceAction,
//         0, 0, 24, scsiCommandData); nvmeIoCtx->device->deviceVerbosity =
//         inVerbosity;
//     }
//     return ret;
// }
//
// static eReturnValues win10_Translate_Reservation_Report(nvmeCmdCtx
// *nvmeIoCtx)
//{
//     eReturnValues ret = OS_COMMAND_NOT_AVAILABLE;
//     eVerbosityLevels inVerbosity = nvmeIoCtx->device->deviceVerbosity;
...
//     {
//         //if none of the above checks caught the command, then it cannot be
//         translated nvmeIoCtx->device->deviceVerbosity = VERBOSITY_QUIET;
//         //ret = scsi_Persistent_Reserve_Out(nvmeIoCtx->device,
//         scsiServiceAction, 0, 0, 24, scsiCommandData);
//         nvmeIoCtx->device->deviceVerbosity = inVerbosity;
//     }
//     return ret;
// }
//
// static eReturnValues win10_Translate_Reservation_Acquire(nvmeCmdCtx
// *nvmeIoCtx)
//{
//     eReturnValues ret = OS_COMMAND_NOT_AVAILABLE;
//     eVerbosityLevels inVerbosity = nvmeIoCtx->device->deviceVerbosity;
...
//     nvmeIoCtx->ptrData[4], nvmeIoCtx->ptrData[5], nvmeIoCtx->ptrData[6],
//     nvmeIoCtx->ptrData[7]);
//     //uint64_t prkey = M_BytesTo8ByteValue(nvmeIoCtx->ptrData[8],
//     nvmeIoCtx->ptrData[9], nvmeIoCtx->ptrData[10], nvmeIoCtx->ptrData[11],
//     nvmeIoCtx->ptrData[12], nvmeIoCtx->ptrData[13], nvmeIoCtx->ptrData[14],
...
//         translated nvmeIoCtx->device->deviceVerbosity = VERBOSITY_QUIET; ret
//         = scsi_Persistent_Reserve_Out(nvmeIoCtx->device, scsiServiceAction,
//         0, scsiType, 24, scsiCommandData); nvmeIoCtx->device->deviceVerbosity
//         = inVerbosity;
//     }
//     return ret;
// }
//
// static eReturnValues win10_Translate_Reservation_Release(nvmeCmdCtx
// *nvmeIoCtx)
//{
//     eReturnValues ret = OS_COMMAND_NOT_AVAILABLE;
//     eVerbosityLevels inVerbosity = nvmeIoCtx->device->deviceVerbosity;
...
// Note: Any odd parameters like a change in TFRs from the spec will not work
// here.
eReturnValues send_Win_ATA_Identify_Cmd(ScsiIoCtx* scsiIoCtx)
{
    eReturnValues returnValue = SUCCESS;
...
#    endif
    ...
    protocolData->ProtocolDataRequestSubValue = 0; // M_BytesTo2ByteValue(scsiIoCtx->pAtaCmdOpts->tfr.LbaMid48,
                                                   // scsiIoCtx->pAtaCmdOpts->tfr.LbaMid);
    protocolData->ProtocolDataOffset = sizeof(STORAGE_PROTOCOL_SPECIFIC_DATA);
    protocolData->ProtocolDataLength =
        /*M_BytesTo2ByteValue(scsiIoCtx->pAtaCmdOpts->tfr.SectorCount48,
           scsiIoCtx->pAtaCmdOpts->tfr.SectorCount) * */
...
#    endif
    ...
    protocolData->ProtocolDataRequestSubValue = M_BytesTo2ByteValue(scsiIoCtx->pAtaCmdOpts->tfr.LbaMid48,
                                                                    scsiIoCtx->pAtaCmdOpts->tfr.LbaMid); // Page number
    protocolData->ProtocolDataOffset          = sizeof(STORAGE_PROTOCOL_SPECIFIC_DATA);
    protocolData->ProtocolDataLength          = M_BytesTo2ByteValue(scsiIoCtx->pAtaCmdOpts->tfr.SectorCount48,
                                                                    scsiIoCtx->pAtaCmdOpts->tfr.SectorCount) *
                                       512U; // sector count * 512 = number of bytes
...
// MS NVMe requirements are listed here:
// https://msdn.microsoft.com/en-us/library/jj134356(v=vs.85).aspx Also: Here is
// a list of all the supported commands and features. This should help with
// implementing support for various commands. Before these links were found,
// everything that was implemented was based on sparse documentation of basic
// features and trial and error. SCSI Translations:
// https://docs.microsoft.com/en-us/windows-hardware/drivers/storage/stornvme-scsi-translation-support
// command set support:
...
// Most of the code below has been updated according to these docs, however some
// things may be missing and those enhancements should be made to better improve
// support.
static eReturnValues send_Win_NVMe_IO(nvmeCmdCtx* nvmeIoCtx)
{
#if !defined(DISABLE_NVME_PASSTHROUGH)
    eReturnValues ret = OS_COMMAND_NOT_AVAILABLE;
    // TODO: Should we be checking the nsid in each command before issuing it?
    // This should happen at some point, at least to filter out "all namespaces"
    // for certain commands since MS won't let us issue some of them through their
    // API - TJE
    if (nvmeIoCtx->commandType == NVM_ADMIN_CMD)
    {
#    if WINVER >= SEA_WIN32_WINNT_WIN10 // This should wrap around anything going through the
                                        // Windows API...Win 10 is required for NVMe IOs
        bool useNVMPassthrough = false; // this is only true when attempting the command with the generic
                                        // storage protocol command IOCTL which is supposed to be used
                                        // for VU commands only. - TJE
        ...
        case NVME_ADMIN_CMD_GET_LOG_PAGE:
            ...
            /*
            Supported through IOCTL_SCSI_PASS_THROUGH using command
            SCSIOP_READ_DATA_BUFF16 with buffer mode as
            ...
            from IOCTL_STORAGE_QUERY_PROPERTY. For host telemetry, this is also
            available through IOCTL_STORAGE_GET_DEVICE_INTERNAL_LOG. Older versions of
            Win10 do not allow pulling in segments with storage query property. May
            need to check Windows 10 version to change between APIs to get this
            data.-TJE
            */
            ret = send_Win_NVMe_Get_Log_Page_Cmd(nvmeIoCtx);
            break;
        case NVME_ADMIN_CMD_GET_FEATURES:
            // NOTE: I don't know if this will work for different select field
            // values...will need to debug this to find out. If different select
            // fields are not supported with this call, then we either need to do a
            // SCSI translation or we have to return "not-supported" - TJE
...
#        endif // ENABLE_TRANSLATE_FORMAT
        ...
        default:
            ...
            if (nvmeIoCtx->cmd.adminCmd.opcode >= 0xC0 /*&& nvmeIoCtx->cmd.adminCmd.opcode <= 0xFF*/) // admin commands
                                                                                                      // in this range
                                                                                                      // are vendor
                                                                                                      // unique
            {
        ...
        if (useNVMPassthrough)
        {
            // Call the function to send a VU command using STORAGE_PROTOCOL_COMMAND -
            // TJE
...
#    if WINVER >= SEA_WIN32_WINNT_WIN10 // This should wrap around anything going through the
        ...
        case NVME_CMD_FLUSH:
            ...
            break;
        case NVME_CMD_READ: // NOTE: This translation likely won't be hit since most
                            // code calls into the read function in cmds.h which
                            // will call os_Read instead. This is here for those
                            // requesting something specific...-TJE
            ret = win10_Translate_Read(nvmeIoCtx);
            break;
        case NVME_CMD_WRITE: // NOTE: This translation likely won't be hit since
                             // most code calls into the read function in cmds.h
                             // which will call os_Read instead. This is here for
                             // those requesting something specific...-TJE
            ret = win10_Translate_Write(nvmeIoCtx);
            break;
        case NVME_CMD_COMPARE: // according to MSFT documentation, this is only
                               // available in WinPE
        ...
        case NVME_CMD_DATA_SET_MANAGEMENT: // SCSI Unmap or Win API call?
            ...
            break;
        // NOTE: No reservation commands are supported according to MSFT docs. Code
        // for attempting SCSI commands is available, and MSFT does have IOCTLs
        // specific for reservations that might work, but it is not documented
        default:
            // Check if it's a vendor unique op code.
            if (nvmeIoCtx->cmd.adminCmd.opcode >= 0x80 /* && nvmeIoCtx->cmd.adminCmd.opcode <= 0xFF */) // admin
                                                                                                        // commands in
                                                                                                        // this range
                                                                                                        // are vendor
                                                                                                        // unique
...
{
    eReturnValues ret = OS_COMMAND_NOT_AVAILABLE;
    // This is a stub. We may not be able to do this in Windows, but want this
    // here in case we can and to make code otherwise compile without ifdefs
    if (device->deviceVerbosity > VERBOSITY_COMMAND_NAMES)
    {
...
static eReturnValues open_Force_Unit_Access_Handle_For_OS_Read_OS_Write(tDevice* device)
{
    ...
    if (device->os_info.forceUnitAccessRWfd == M_NULLPTR || device->os_info.forceUnitAccessRWfd == INVALID_HANDLE_VALUE)
    {
        ...
        // FILE_FLAG_NO_BUFFERING - this is for the operating system/file system
        // caching. NOT THE DISK! FILE_FLAG_WRITE_THROUGH - this seems to mean FUA
        // based on the description of the flag
        //  The following link describes the flags. It sounds like we need both of
        //  them for FUA otherwise it may go to the system cache and then be
        //  flushed, which may not be the same- TJE
...
{
    eReturnValues ret = SUCCESS;
    // clear the last command sense data and rtfrs. We'll dummy them up in a
    // minute
    ...
    else
    {
        ...
        uint8_t ascq     = 0;
        ret              = FAILURE;
        // For nvme, set the NVMe status as best we can, then fall through and set
        // SCSI style sense data as well. This switch case will handle many, if not
        // all the same cases as SCSI below, but this seemed like the easier way to
        // solve this problem for now. - TJE if the DISABLE_NVME_PASSTHROUGH flag is
        // refactored, this can probably be cleaned up a lot - TJE
        ...
        case ERROR_WRITE_FAULT:
            ...
            break;
        case ERROR_READ_FAULT: // should this be "Deallocated or unwritten logical
                               // block on NVME?
        ...
        case ERROR_SEEK:             // cannot find area or track on disk?
            M_FALLTHROUGH;           // Fallthrough for now unless we can figure out a better,
                                     // more specific error when this happens - TJE
        ...
        case ERROR_OFFSET_ALIGNMENT_VIOLATION: // alignment error for the device
            // namespace not ready??? THere doesn't seem to be anything similar in the
            // spec like SAS/SATA have...probably because LBAs don't report differing
            // logical and physical size
            device->drive_info.lastNVMeResult.lastNVMeStatus =
                WIN_DUMMY_NVME_STATUS(NVME_SCT_GENERIC_COMMAND_STATUS, NVME_GEN_SC_NS_NOT_READY_);
            break;
        case ERROR_TIMEOUT:
            // command abort requested. Assume this system asked to abort this when it
            // took too long
            device->drive_info.lastNVMeResult.lastNVMeStatus =
        ...
        case ERROR_BAD_COMMAND:
            ...
            break;
        case ERROR_INVALID_DATA:        // Not sure if this is the same as CRC or something
                                        // else, so this may need changing if we see it in
                                        // the future.
        case ERROR_DATA_CHECKSUM_ERROR: // Not sure if this will show up for RAW IO
                                        // like this is doing or not, but we may
                                        // need a case for this in the future.
            // data transfer error?
            device->drive_info.lastNVMeResult.lastNVMeStatus =
                WIN_DUMMY_NVME_STATUS(NVME_SCT_GENERIC_COMMAND_STATUS, NVME_GEN_SC_DATA_XFER_ERROR_);
            break;
        default:
            // setting to abort requested since we don't know what else to
            // set...generic enough
            device->drive_info.lastNVMeResult.lastNVMeStatus =
                WIN_DUMMY_NVME_STATUS(NVME_SCT_GENERIC_COMMAND_STATUS, NVME_GEN_SC_ABORT_REQ_);
            break;
        ...
        // failure for one reason or another. The last error may or may not tell us
        ...
        {
            // Some of these are in here "just in case", but this is not a
            // comprehensive list of what could be returned. Some may never be
            // returned, others may not be in this list and are falling to the default
            // case - TJE The only one I haven't been able to find a good answer for
            // is an interface CRC error, which are hard to create and test for - TJE
        case ERROR_NOT_READY: // sense key not ready...not sure this matches
                              // anything in ATA if this even were to happen there.
            senseKey = SENSE_KEY_NOT_READY;
        ...
        case ERROR_OFFSET_ALIGNMENT_VIOLATION: // alignment error for the device
            senseKey = SENSE_KEY_ILLEGAL_REQUEST;
            asc      = 0x21;
            ascq     = 0x04; // technically this is "unaligned write command" which would
                             // not be accurate with a read, but this is the best I can do
                             // right now....maybe 07 for read boundary error???
            if (device->drive_info.drive_type == ATA_DRIVE)
            {
        ...
        case ERROR_DATA_CHECKSUM_ERROR: // Not sure if this will show up for RAW IO
                                        ...
                                        // need a case for this in the future.
        default:
            // set the sense key to aborted command...don't set the asc or ascq since
            // we don't know what to set those to right now
            senseKey = SENSE_KEY_ABORTED_COMMAND;
            break;
        }
        ...
        if (asc || ascq)
        {
            device->drive_info.lastCommandSenseData[7] = 6; // get to bytes 12 & 13 for asc info...or should this change
                                                            // to a value of 7 to include fru, even though that is
                                                            // impossible for us to figure out??? - TJE
...
// The overlapped structure used here changes it to asynchronous IO, but the
// synchronous portions of code are left here in case the device responds as a
// synchronous device and ignores the overlapped strucutre...it SHOULD work on
// any device like this. See here:
...
eReturnValues os_Read(tDevice* device, uint64_t lba, bool forceUnitAccess, uint8_t* ptrData, uint32_t dataSize)
{
    ...
    /*BOOL timeoutGot = */
    GetCommTimeouts(handleToUse,
                    &comTimeout); // get timeouts if possible before trying to change them...
    ...
    {
        // could not get a FUA handle...so emulate with a verify command before the
        // read - TJE
    ...
    device->os_info.last_error = GetLastError();
    if (ERROR_IO_PENDING == device->os_info.last_error) // This will only happen for overlapped commands. If the
                                                        // drive is opened without the overlapped flag,
                                                        // everything will work like old synchronous code.-TJE
    {
...
eReturnValues os_Write(tDevice* device, uint64_t lba, bool forceUnitAccess, uint8_t* ptrData, uint32_t dataSize)
{
    ...
    if (forceUnitAccess && openFUA != SUCCESS)
    {
        // could not get a FUA handle...so emulate with a verify command after the
        // write - TJE
        os_Verify(device, lba, dataSize / device->drive_info.deviceBlockSize);
    }
    stop_Timer(&commandTimer);
...
// Seems to work. Needs some enhancements with timers and checking return codes
// more closely to dummy up better sense data.
eReturnValues os_Verify(tDevice* device, uint64_t lba, uint32_t range)
{
    ...
    verifyCmd.StartingOffset.QuadPart =
        C_CAST(LONGLONG,
               lba * device->drive_info.deviceBlockSize); // LBA needs to be converted to a byte offset
...
eReturnValues os_Verify(tDevice* device, uint64_t lba, uint32_t range)
{
    // flush the cache first to make sure we aren't reading something that is in
    // cache than disk (as close as we can get right here)
    os_Flush(device);
    // now do a read and throw away the data
    uint8_t* readData = C_CAST(uint8_t*, safe_malloc(device->drive_info.deviceBlockSize * range));
    if (readData)
    {
...
// This is for Windows XP and higher. This should issue a flush cache or
// synchronize cache command for us
eReturnValues os_Flush(tDevice* device)
{
    ...
    memset(&comTimeout, 0, sizeof(COMMTIMEOUTS));
    /*BOOL timeoutGot = */
    GetCommTimeouts(device->os_info.fd,
                    &comTimeout); // get timeouts if possible before trying to change them...
    uint64_t timeoutInSeconds = 0;
    if (device->drive_info.defaultTimeoutSeconds == 0)
    {
