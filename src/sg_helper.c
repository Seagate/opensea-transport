...
// NOTE: This will be undefined at the end of this file if
// _GNU_SOURCE_DEFINED_IN_SG_HELPER is set to prevent unexpected
//       behavior in any other parts of the code.
// NOTE: Adding this definition like this so that it can also easily be removed
// as necessary in the future in case of errors.
...
#if defined(__has_include) // GCC5 and higher support this, BUT only if a C
                           // standard is specified. The -std=gnuXX does not
                           // support this properly for some odd reason.
...
#        endif
#    else //__has_include could not locate the header, check if it was specified by
          // the user through a define.
#        if defined(SEA_NVME_IOCTL_H)
#            include <linux/nvme_ioctl.h>
...
// get sd devices, but ignore any partition number information since that isn't
// something we can actually send commands to
...
// This function is not currently used or tested...if we need to make more
// changes for pre-2.6 kernels, we may need this. bool
// does_Kernel_Support_SysFS_Link_Mapping()
//{
//     bool linkMappingSupported = false;
//     //kernel version 2.6 and higher is required to map the handles between sg
//     and sd/sr/st/scd OSVersionNumber linuxVersion; memset(&linuxVersion, 0,
//     sizeof(OSVersionNumber)); if(SUCCESS ==
//     get_Operating_System_Version_And_Name(&linuxVersion, M_NULLPTR))
//     {
...
{
    int   result = 0;
    FILE* mount  = setmntent("/etc/mtab", "r"); // we only need to know about mounted partitions.
                                                // Mounted partitions need to be known so that they can
                                                // be unmounted when necessary. - TJE
    if (mount)
    {
        struct mntent* entry = M_NULLPTR;
#if defined(_BSD_SOURCE) || defined(_SVID_SOURCE) // getmntent_r lists these feature test macros to look
                                                  // for - TJE
...
static eReturnValues get_Partition_List(const char* blockDeviceName, ptrsPartitionInfo partitionInfoList, int listCount)
{
    ...
    {
        FILE* mount = setmntent("/etc/mtab", "r"); // we only need to know about mounted partitions.
                                                   // Mounted partitions need to be known so that they
                                                   // can be unmounted when necessary. - TJE
        if (mount)
        {
            struct mntent* entry = M_NULLPTR;
#if defined(_BSD_SOURCE) || defined(_SVID_SOURCE) ||                                                                   \
    !defined(NO_GETMNTENT_R) // feature test macros we're defining _BSD_SOURCE or
                             // _SVID_SOURCE in my testing, but we want the reentrant
                             // version whenever possible. This can be defined if
                             // this function is not identified. - TJE
            struct mntent entBuf;
            DECLARE_ZERO_INIT_ARRAY(char, lineBuf, GETMNTENT_R_LINE_BUF_SIZE);
            while (M_NULLPTR != (entry = getmntent_r(mount, &entBuf, lineBuf, GETMNTENT_R_LINE_BUF_SIZE)))
#else // use the not thread safe version since that is all that is available
#    pragma message "Not using getmntent_r. Partition detection is not thread safe"
...
#endif
            {
                if (strstr(entry->mnt_fsname, blockDeviceName))
                {
                    ...
                    {
                        result = MEMORY_FAILURE; // out of memory to copy all results to the
                                                 // list.
                    }
                }
            }
...
{
    eSCSIPeripheralDeviceType scsiDevType; // in Linux this will be reading the "type" file to get this.
                                           // If it is not available, will retry with "inquiry" data
                                           // file's first byte
    ...
    char primaryHandleStr[OS_HANDLE_NAME_MAX_LENGTH];      // dev/sg or /dev/nvmexny
                                                           // (namespace handle)
    char secondaryHandleStr[OS_SECOND_HANDLE_NAME_LENGTH]; // dev/sd or /dev/nvmex
                                                           // (controller handle)
    char tertiaryHandleStr[OS_SECOND_HANDLE_NAME_LENGTH];  // dev/bsg or /dev/ngXnY
                                                           // (nvme generic handle)
                                                            ...
                                                            uint8_t*    versionCount)
{
    ...
    // There may be more.
    if (driververstr && versionlist && versionlistlen == 4 && versionCount) // require 4 spaces for the current parsing
                                                                            // based off of what is commented above
    {
        char* end           = M_NULLPTR;
...
static void get_Driver_Version_Info_From_Path(char* driverPath, sysFSLowLevelDeviceInfo* sysFsInfo)
{
    ...
    // the version number can be found in driverPath/module/version if this file
    // exists. Read this file and save the version information
    char* driverVersionFilePath = C_CAST(char*, safe_calloc(OPENSEA_PATH_MAX, sizeof(char)));
    if (driverVersionFilePath)
    {
        ...
        // convert relative path to a full path. Basically replace ../'s with /sys/
        // since this will always be ../../bus and we need /sys/buf
        char*  busPtr    = strstr(driverVersionFilePath, "/bus");
        size_t busPtrLen = safe_strlen(busPtr);
        // magic number 4 is for the length of the string "/sys" which is what is
        // being set in the beginning of the path. This is a bit of a mess, but a
        // simple call to realpath was not working, likely due to the current
        // directory not being exactly what we want to start with to allow that
        // function to correctly figure out the path.
            ...
            if (versionFileSize > 0)
            {
                ...
                if (versionFile)
                {
                    char* versionFileData =
                        C_CAST(char*, safe_calloc(C_CAST(size_t, versionFileSize) + 1, sizeof(char)));
                    if (versionFileData)
                    {
                            ...
                            !ferror(versionFile))
                        {
                            ...
                            if (get_Driver_Version_Info_From_String(versionFileData, versionList, 4, &versionCount))
                            {
                                ...
                                case 2:
                                    ...
                                    break;
                                default:
                                    // error reading the string! consider the whole scanf a
                                    // failure! Will need to add other format parsing here if
                                    // there is something else to read instead.-TJE
...
static void get_SYS_FS_ATA_Info(const char* inHandleLink, sysFSLowLevelDeviceInfo* sysFsInfo)
{
    ...
    if (newStrLen > 0)
    {
        char* pciPath = C_CAST(char*, safe_calloc(PATH_MAX, sizeof(char)));
        if (pciPath)
        {
            ...
            char*   driverPath = C_CAST(char*, safe_calloc(OPENSEA_PATH_MAX, sizeof(char)));
...
static void get_SYS_FS_USB_Info(const char* inHandleLink, sysFSLowLevelDeviceInfo* sysFsInfo)
{
    ...
    sysFsInfo->drive_type     = SCSI_DRIVE; // changed later depending on what
                                            // passthrough the USB adapter supports
    // set the USB VID and PID. NOTE: There may be a better way to do this, but
    ...
    if (newStrLen > 0)
    {
        char* usbPath = C_CAST(char*, safe_calloc(PATH_MAX, sizeof(char)));
        if (usbPath)
        {
            snprintf(usbPath, PATH_MAX, "%.*s", C_CAST(int, newStrLen), fullPciPath);
            usbPath = dirname(usbPath);
            // printf("full USB Path = %s\n", usbPath);
            // now that the path is correct, we need to read the files idVendor and
            // idProduct
...
static void get_SYS_FS_1394_Info(const char* inHandleLink, sysFSLowLevelDeviceInfo* sysFsInfo)
{
    ...
    if (newStrLen > 0)
    {
        char* fwPath = C_CAST(char*, safe_calloc(PATH_MAX, sizeof(char)));
        if (fwPath)
        {
            ...
            if (temp)
            {
                // This file contains everything in one place. Otherwise we would need
                // to parse multiple files at slightly different paths to get everything
                // - TJE
                if (4 == fscanf(temp, "ieee1394:ven%8" SCNx32 "mo%8" SCNx32 "sp%8" SCNx32 "ver%8" SCNx32,
                                &sysFsInfo->adapter_info.vendorID, &sysFsInfo->adapter_info.productID,
                                &sysFsInfo->adapter_info.specifierID, &sysFsInfo->adapter_info.revision))
                {
                    ...
                    sysFsInfo->adapter_info.revisionValid    = true;
                    // printf("Got vendor ID as %" PRIX16 "h\n",
                    // sysFsInfo->adapter_info.vendorID); printf("Got product ID as %"
                    // PRIX16 "h\n", sysFsInfo->adapter_info.productID); printf("Got
                    // specifier ID as %" PRIX16 "h\n",
                    // sysFsInfo->adapter_info.specifierID); printf("Got revision ID as %"
                    // PRIX16 "h\n", sysFsInfo->adapter_info.revision);
                }
                fclose(temp);
                temp = M_NULLPTR;
...
static void get_SYS_FS_SCSI_Info(const char* inHandleLink, sysFSLowLevelDeviceInfo* sysFsInfo)
{
    ...
    // adaptec:
    // /sys/devices/pci0000:00/0000:00:02.0/0000:02:00.0/host0/target0:1:0/0:1:0:0/scsi_generic/sg2
    // lsi:
    ...
    // fullPciPath), C_CAST(uintptr_t, strstr(fullPciPath, "/host")));
    intptr_t newStrLen = strstr(fullPciPath, "/host") - fullPciPath;
    if (newStrLen > 0)
    {
        char* pciPath = C_CAST(char*, safe_calloc(PATH_MAX, sizeof(char)));
...
// TODO: read other files in this structure that could be useful??? mn, vendor,
// revision, sas_address NOTE: not all files will exist for all devices/types
// NOTE: SAS, SCSI, SATA, USB will all use this since they all are treated as
// SCSI devices by the OS.
//       NVMe will need a different set of instructions/things to do.
// NOTE: This counts on "full device path" being set in sysFsInfo already (which
// it should be)
static void get_Linux_SYS_FS_SCSI_Device_File_Info(sysFSLowLevelDeviceInfo* sysFsInfo)
{
    ...
    {
        // could not open the type file, so try the inquiry file and read the first
        // byte as raw binary since this is how this file is stored
...
// while similar to the function below, this is used only by get_Device to set
// up some fields in the device structure for the above layers this function
// gets the following info:
//  pcie/usb product ID, vendor ID, revision ID, sets the interface type,
//  ieee1394 specifier ID, and sets the handle mapping for SD/BSG
// this also calls the function to get the driver version info as well as the
// name of the driver as a string.
// TODO: Also output the full device path from the read link???
//       get the SCSI peripheral device type to help decide when to scan for
//       RAIDs on a given handle
// handle nvme-generic handles???
// handle looking up nvme controller handle from a namespace handle???
// handle /dev/disk/by-<> lookups. These are links to /dev/sd or /dev/nvme, etc.
// We can convert these first, then convert again to sd/sg/nvme as needed
static void get_Linux_SYS_FS_Info(const char* handle, sysFSLowLevelDeviceInfo* sysFsInfo)
{
    // check if it's a block handle, bsg, or scsi_generic handle, then setup the
    // path we need to read.
    if (handle && sysFsInfo)
    {
        if (strstr(handle, "nvme") != M_NULLPTR)
        {
            size_t nvmHandleLen = safe_strlen(handle) + 1;
            char*  nvmHandle    = C_CAST(char*, safe_calloc(nvmHandleLen, sizeof(char)));
        ...
        else // not NVMe, so we need to do some investigation of the handle. NOTE:
             // this requires 2.6 and later kernel since it reads a link in the
             // /sys/class/ filesystem
        {
            ...
            if (stat(incomingHandleClassPath, &inHandleStat) == 0 && S_ISDIR(inHandleStat.st_mode))
            {
                ...
                if (lstat(incomingHandleClassPath, &link) == 0 && S_ISLNK(link.st_mode))
                {
                    ...
                    if (readlink(incomingHandleClassPath, inHandleLink, PATH_MAX) > 0)
                    {
                        ...
                        // ../../devices/pci0000:00/0000:00:1f.2/ata8/host8/target8:0:0/8:0:0:0/scsi_generic/sg2
                        // example usb device link:
                        ...
                        // if the link doesn't conatin ata or usb in it, then we are
                        // assuming it's scsi since scsi doesn't have a nice simple string
                        // to check
                        else
                        {
                        ...
                        // Lastly, call the mapping function to get the matching block
                        // handle and check what we got to set ATAPI, TAPE or leave as-is.
                        // Setting these is necessary to prevent talking to ATAPI as HDD due
                        // to overlapping A1h opcode
                        ...
                        if (SUCCESS == map_Block_To_Generic_Handle(handle, &gen, &block))
                        {
                            ...
                            // gen, block); Our incoming handle SHOULD always be sg/bsg, but
                            // just in case, we need to check before we setup the second
                            // handle (mapped handle) information
                            if (incomingBlock)
                            {
                                // block device handle was sent into here (and we made it this
                                // far...unlikely) Secondary handle will be a generic handle
                                if (is_Block_SCSI_Generic_Handle(gen))
                                {
                                    snprintf(sysFsInfo->secondaryHandleStr, OS_SECOND_HANDLE_NAME_LENGTH, "/dev/bsg/%s",
                                             gen);
                                }
                                else
                                {
                                    snprintf(sysFsInfo->secondaryHandleStr, OS_SECOND_HANDLE_NAME_LENGTH, "/dev/%s",
                                             gen);
                                }
                            }
                            else
...
eReturnValues map_Block_To_Generic_Handle(const char* handle, char** genericHandle, char** blockHandle)
{
    ...
    // if the handle passed in contains "nvme" then we know it's a device on the
    // nvme interface
    if (strstr(handle, "nvme") != M_NULLPTR)
    {
    ...
    else
    {
        ...
        if (stat(incomingHandleClassPath, &inHandleStat) == 0 && S_ISDIR(inHandleStat.st_mode))
        {
            ...
            if (readlink(incomingHandleClassPath, inHandleLink, PATH_MAX) > 0)
            {
                ...
                // now we need to map it to a generic handle (sg...if sg not available,
                // bsg)
                ...
                // now we need to loop through each think in the class folder, read the
                // link, and check if we match.
                ...
                {
                    // now we need to read the link for classPath/d_name into a
                    // buffer...then compare it to the one we read earlier.
                    ...
                    if (lstat(temp, &tempStat) == 0 && S_ISLNK(tempStat.st_mode)) /*check if this is a link*/
                    {
                        ...
                        if (readlink(temp, mapLink, PATH_MAX) > 0)
                        {
                            ...
                            // NOTE: If we are in the block class, we will see sda, sda1,
                            // sda 2. These are all matches (technically)
                            //       We SHOULD match on the first disk without partition
                            //       numbers since we did alphasort
                            // We need to match up until the class name (ex: block, bsg,
                            // scsi_generic)
                            ...
                            if (className)
                            {
                                ...
                                if (M_NULLPTR != classPtr &&
                                    strncmp(mapLink, inHandleLink, C_CAST(size_t, (classPtr - mapLink))) == 0)
                                {
                                    if (incomingBlock)
                                    {
                                    ...
                                    // this is causing a mem leak... when we bail the loop, there
                                    // are a string of classList[] items still allocated.
...
static eReturnValues set_Device_Partition_Info(tDevice* device)
{
    ...
    if (partitionCount > 0)
    {
        ...
        ptrsPartitionInfo parts =
            C_CAST(ptrsPartitionInfo, safe_calloc(C_CAST(size_t, partitionCount), sizeof(spartitionInfo)));
        if (parts)
        {
...
#endif
                    // check if one of the partitions is /boot and mark the system disk
                    // when this is found
...
static eReturnValues get_Lin_Device(const char* filename, tDevice* device)
{
    ...
    if (is_Block_Device_Handle(filename))
    {
        ...
        {
            deviceHandle = C_CAST(char*, safe_calloc(LIN_MAX_HANDLE_LENGTH, sizeof(char)));
    ...
    if ((device->os_info.fd >= 0) && (ret == SUCCESS))
    {
        if (is_NVMe_Handle(deviceHandle))
        {
            ...
            // Now we will set up the device name, etc fields in the os_info
            // structure.
...
#    if defined(_DEBUG)
            printf("\nsg helper-nvmedev --  NVME Passthrough disabled, device not "
                   "supported\n");
#    endif // DEBUG
            return NOT_SUPPORTED; // return not supported since NVMe-passthrough is
                                  // disabled
        ...
        else // not an NVMe handle
        {
            ...
            if (getHctl == 0 && errno == 0) // when this succeeds, both of these will be zeros
            {
                ...
                device->drive_info.namespaceID =
                    device->os_info.scsiAddress.lun + UINT32_C(1); // Doing this to help with USB to NVMe adapters. Luns
                                                                   // start at zero, whereas namespaces start with 1,
                                                                   // hence the plus 1.
                // also reported are per lun and per device Q-depth which might be nice
                // to store. printf("H:C:T:L = %" PRIu8 ":%" PRIu8 ":%" PRIu8 ":%" PRIu8
            ...
            else
            {
                ...
                // this code to set up passthrough commands for USB and IEEE1394 has
                // been removed for now to match Windows functionality. Need better
                // intelligence than this. Some of these old pass-through types issue
                // vendor specific op codes that could be misinterpretted on some
                // devices.
...
// sgResetType should be one of the values from the link above...so bus or
// device...controller will work but that shouldn't be done ever.
...
eReturnValues send_sg_io(ScsiIoCtx* scsiIoCtx)
{
    ...
    {
        localSenseBuffer = C_CAST(uint8_t*, safe_calloc_aligned(SPC3_SENSE_LEN, sizeof(uint8_t),
    ...
    case XFER_DATA_OUT_IN:
        ...
        io_hdr.dxfer_direction = SG_DXFER_UNKNOWN; // using unknown because SG_DXFER_TO_FROM_DEV is
                                                   // described as something different to use with
                                                   // indirect IO as it copied into kernel buffers before
                                                   // transfer.
#else
        ...
        scsiIoCtx->device->drive_info.defaultTimeoutSeconds > scsiIoCtx->timeout)
    {
        ...
        // this check is to make sure on commands that set a very VERY large timeout
        // (*cough* *cough* ata security) that we DON'T do a conversion and leave
        // the time as the max...
    ...
    else
    {
        if (scsiIoCtx->timeout != 0)
        {
            ...
            // this check is to make sure on commands that set a very VERY large
            // timeout (*cough* *cough* ata security) that we DON'T do a conversion
            // and leave the time as the max...
    ...
    if ((io_hdr.info & SG_INFO_OK_MASK) != SG_INFO_OK)
    {
        ...
        // Check the masked status, host status and driver status to see what
        // happened.
        if (io_hdr.masked_status != 0) // SAM_STAT_GOOD???
        {
            ...
            if (io_hdr.sb_len_wr == 0)
            {
                if (VERBOSITY_COMMAND_VERBOSE <= scsiIoCtx->device->deviceVerbosity)
                {
                    printf("\t(Masked Status) Sense data not available, assuming "
                           "OS_PASSTHROUGH_FAILURE\n");
                }
                // No sense data back. We need to set an error since the layers above
                // are going to look for sense data and we don't have any.
                ret = OS_PASSTHROUGH_FAILURE;
            }
        }
        if (io_hdr.host_status != 0)
        {
            ...
            if (io_hdr.sb_len_wr == 0) // Doing this because some drivers may set an error even if the
                                       // command otherwise went through and sense data was available.
            {
                // Special case for MegaRAID and controllers based on MegaRAID.
                // These controllers block the command and set "Internal Adapter Error"
                // with no other information available.
                // TODO: Need to test and see if SAT passthrough trusted send/receive
                // are also blocked to add them to this case. -TJE
                ...
                else
                {
                    if (VERBOSITY_COMMAND_VERBOSE <= scsiIoCtx->device->deviceVerbosity)
                    {
                        printf("\t(Host Status) Sense data not available, assuming "
                               "OS_PASSTHROUGH_FAILURE\n");
                    }
                    ret = OS_PASSTHROUGH_FAILURE;
                }
        ...
        if (io_hdr.driver_status != 0)
        {
            ...
            if (io_hdr.sb_len_wr == 0)
            {
                if (VERBOSITY_COMMAND_VERBOSE <= scsiIoCtx->device->deviceVerbosity)
                {
                    printf("\t(Driver Status) Sense data not available, assuming "
                           "OS_PASSTHROUGH_FAILURE\n");
                }
                // No sense data back. We need to set an error since the layers above
                // are going to look for sense data and we don't have any.
...
eReturnValues get_Device_Count(uint32_t* numberOfDevices, uint64_t flags)
{
    ...
    // need to check if existing sg/sd handles are attached to hpsa or smartpqi
    // drives in addition to /dev/cciss devices
    ...
    for (uint32_t iter = 0; iter < num_devs; ++iter)
    {
        ...
        memset(&raidHint, 0,
               sizeof(raidTypeHint)); // clear out before checking driver name since
                                      // this will be expanded to check other
...
#    if defined(ENABLE_CISS)
        if (sysFsInfo.scsiDevType == PERIPHERAL_STORAGE_ARRAY_CONTROLLER_DEVICE)
        {
            if (strcmp(sysFsInfo.driver_info.driverName, "hpsa") == 0)
            {
                ...
                // this handle is a /dev/sg handle with the hpsa driver, so we can scan
                // for cciss devices
                raidHandleList = add_RAID_Handle_If_Not_In_List(beginRaidHandleList, raidHandleList,
            ...
            {
                raidHint.cissRAID = true;
                // this handle is a /dev/sg handle with the smartpqi driver, so we can
                // scan for cciss devices
                raidHandleList = add_RAID_Handle_If_Not_In_List(beginRaidHandleList, raidHandleList,
                                                                namelist[iter]->d_name, raidHint);
                if (!beginRaidHandleList)
                {
             ...
             (driveNumber < MAX_DEVICES_TO_SCAN && driveNumber < totalDevs) && (found < numberOfDevices); ++driveNumber)
        {
            ...
            if (fd >= 0)
            {
                ...
                else
                {
                    memset(&raidHint, 0, sizeof(raidTypeHint));
#if defined(ENABLE_CISS)
                    // check that we are only scanning a SCSI controller for RAID to avoid
                    // duplicates NOTE: If num_sg_devs == 0, then the sg driver is missing
                    // and SCSI controllers do not get /dev/sd handles, so we will skip
                    // this check in this special case.
                    //       This special case exists because sometimes a kernel is built
                    //       and deployed without the SG driver enabled, but we still want
                    //       to detect RAID devices, so we don't want to skip enumerating
                    //       a RAID when all we see are the logical RAID volumes -TJE
                        ...
                        num_sg_devs == 0)
                    {
                        ...
                        {
                            raidHint.cissRAID = true;
                            // this handle is a /dev/sg handle with the hpsa driver, so we can
                            // scan for cciss devices
                            raidHandleList =
                        ...
                        {
                            raidHint.cissRAID = true;
                            // this handle is a /dev/sg handle with the smartpqi driver, so we
                            // can scan for cciss devices
                            raidHandleList =
                                add_RAID_Handle_If_Not_In_List(beginRaidHandleList, raidHandleList, name, raidHint);
                            if (!beginRaidHandleList)
                            {
...
{
#if !defined(DISABLE_NVME_PASSTHROUGH)
    eReturnValues ret = SUCCESS; // NVME_SC_SUCCESS;//This defined value used to exist in some
                                 // version of nvme.h but is missing in nvme_ioctl.h...it was a
                                 // value of zero, so this should be ok.
    seatimer_t commandTimer;
    memset(&commandTimer, 0, sizeof(commandTimer));
    struct nvme_admin_cmd adminCmd;
    struct nvme_user_io   nvmCmd; // it's possible that this is not defined in some funky early nvme
                                  // kernel, but we don't see that today. This seems to be defined
                                  // everywhere. -TJE
#    if defined(NVME_IOCTL_IO_CMD)
    struct nvme_passthru_cmd* passThroughCmd =
        (struct nvme_passthru_cmd*)&adminCmd; // setting a pointer since these are defined to be the
                                              // same. No point in allocating yet another structure. -
                                              // TJE
...
#    endif //__clang__, __GNUC__
            ...
            else
            {
                ...
                nvmeIoCtx->commandCompletionData.statusAndCID =
                    C_CAST(uint32_t, ioctlResult) << 17; // shift into place since we don't get the phase tag or
                                                         // command ID bits and these are the status field
            }
            break;
        default:
#    if defined(NVME_IOCTL_IO_CMD)
            ...
            passThroughCmd->data_len = nvmeIoCtx->dataSize; // Or do I use the other PRP2 data? Not sure - TJE
                                                            // //M_DWord1(nvmeIoCtx->cmd.nvmCmd.prp2);//guessing
                                                            // here since I don't really know - TJE
...
static eReturnValues linux_NVMe_Reset(tDevice* device, bool subsystemReset)
{
    ...
    // Can only do a reset on a controller handle. Need to get the controller
    // handle if this is a namespace handle!!!
    ...
    // Need to make sure the handle we use to issue the reset is a controller
    // handle and not a namespace handle.
...
#if defined(_DEBUG)
// making this a debug flagged call since it is currently an unused function. We
// should look into how to appropriately support this.-TJE
static eReturnValues nvme_Namespace_Rescan(int fd)
{
#    if defined(NVME_IOCTL_RESCAN) // This IOCTL is not available on older kernels,
                                   // which is why this is checked like this - TJE
    eReturnValues ret   = OS_PASSTHROUGH_FAILURE;
...
// Case to remove this from sg_helper.h/c and have a platform/lin/pci-herlper.h
// vs platform/win/pci-helper.c
...
{
    // TODO: if BLKZEROOUT available, use this to write zeroes to begining and end
    // of the drive???
    return NOT_SUPPORTED;
}
...
// This should be at the end of this file to undefine _GNU_SOURCE if this file
// manually enabled it
