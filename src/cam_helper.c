...
// This is being redefined here in case it is missing for backwards compatibiity
// with old FreeBSD versions
...
static int get_Partition_Count(const char* blockDeviceName)
{
    ...
    int            totalMounts = getmntinfo(&mountedFS, MNT_WAIT); // Can switch to MNT_NOWAIT and will probably be
                                                                   // fine, but using wait for best results-TJE
...
static eReturnValues get_Partition_List(const char* blockDeviceName, ptrsPartitionInfo partitionInfoList, int listCount)
{
    ...
    if (listCount > 0)
    {
        ...
        // This was chosen because it provides the info we want, and also claims to
        // only show mounted devices. I also tried using getfsent, but that didn't
        // return what we needed. If for any reason, getmntinfo is not available, I
        // recommend switching to getfsstat. The code is similar but slightly
        // different. I only had a VM to test with so my results showed the same
        // between the APIs, but the description of getmntinfo was more along the
        // lines of what has been implemented for other OS's we support. - TJE
        struct statfs* mountedFS   = M_NULLPTR;
        int            totalMounts = getmntinfo(&mountedFS, MNT_WAIT); // Can switch to MNT_NOWAIT and will probably be
                                                                       // fine, but using wait for best results-TJE
        if (totalMounts > 0 && mountedFS)
...
static eReturnValues set_Device_Partition_Info(tDevice* device)
{
    ...
    if (partitionCount > 0)
    {
        ...
        ptrsPartitionInfo parts =
            C_CAST(ptrsPartitionInfo, safe_calloc(int_to_sizet(partitionCount), sizeof(spartitionInfo)));
        if (parts)
        {
            if (SUCCESS == get_Partition_List(device->os_info.name, parts, partitionCount))
            {
                ...
                for (; iter < partitionCount; ++iter)
                {
                    ...
                    // when this is found Should / be treated as a system disk too?
...
eReturnValues get_Device(const char* filename, tDevice* device)
{
    ...
    device->os_info.cam_dev = M_NULLPTR; // initialize this to M_NULLPTR (which it already should be)
                                         // just to make sure everything else functions as expected
#if !defined(DISABLE_NVME_PASSTHROUGH)
    ...
    {
        // printf("%s fd %d name %s\n",__FUNCTION__, device->os_info.fd,
        // device->os_info.name);
        device->os_info.cam_dev =
            cam_open_spec_device(devName, devUnit, O_RDWR, M_NULLPTR); // O_NONBLOCK is not allowed
                ...
                if (cam_send_ccb(device->os_info.cam_dev, ccb) >= 0)
                {
                    if ((ccb->ccb_h.status & CAM_STATUS_MASK) == CAM_REQ_CMP)
                    {
                        ...
                        if (cgd.protocol == PROTO_SCSI)
                        {
                            ...
                            // this_drive_type = memcmp(device->drive_info.T10_vendor_ident,
                            // "ATA", 3); if (this_drive_type == 0)
                            // {
                        ...
                        else if (cgd.protocol == PROTO_ATA || cgd.protocol == PROTO_ATAPI)
                        {
                            ...
                            memcpy(&device->drive_info.product_identification, cgd.ident_data.model,
                                   M_Min(MODEL_NUM_LEN,
                                         40)); // 40 comes from ata_param stuct in the ata.h
                            memcpy(&device->drive_info.product_revision, cgd.ident_data.revision,
                                   M_Min(FW_REV_LEN,
                                         8)); // 8 comes from ata_param stuct in the ata.h
                            memcpy(&device->drive_info.serialNumber, cgd.ident_data.serial,
                                   M_Min(SERIAL_NUM_LEN,
                                         20)); // 20 comes from ata_param stuct in the ata.h
                        }
                        else
                        {
                            ...
                            if ((ccb->ccb_h.status & CAM_STATUS_MASK) == CAM_REQ_CMP)
                            {
                                ...
                                case XPORT_ATA:
                                    device->drive_info.interface_type =
                                        IDE_INTERFACE; // Seeing IDE may look strange, but that is
                                                       // how old code was written to identify an
                                                       // ATA interface regardless of parallel or
                                                       // serial.
                                    break;
                                case XPORT_USB:
                                ...
                                case XPORT_SPI:
                                    ...
                                    // Check hba_vid for "SBP" to tell the difference and set the
                                    // proper interface
                                    if (strncmp(cpi.hba_vid, "SBP", 3) == 0 &&
                                        cpi.hba_misc &
                                            (PIM_NOBUSRESET | PIM_NO_6_BYTE)) // or check initiator_id? That's
                                                                              // defined but not sure if that could
                                                                              // be confused with other SPI devices
                                                                              // - TJE
                                    {
                                        device->drive_info.interface_type = IEEE_1394_INTERFACE;
                                        // TODO: Figure out where to get device unique firewire IDs
                                        // for specific device compatibility lookups
                                    }
                                    break;
                                case XPORT_SAS:
                                case XPORT_ISCSI:
                                ...
                                // TODO: Parse other flags to set hacks and capabilities to help
                                // with adapter or interface specific limitations
                                //        - target flags which may help identify device
                                //        capabilities (no 6-byte commands, group 6 & 7 command
                                //        support, ata_ext request support.
                                //        - target flag that says no 6-byte commands can help
                                //        uniquly identify IEEE1394 devices
                                //       These should be saved later when we run into
                                //       compatibility issues or need to make other
                                //       improvements. For now, getting the interface is a huge
                                //       help
                                ...
                                if (device->drive_info.interface_type != USB_INTERFACE &&
                                    device->drive_info.interface_type != IEEE_1394_INTERFACE)
                                {
                                    // NOTE: Not entirely sure EXACTLY when this was introduced,
                                    // but this is a best guess from looking through cam_ccb.h
                                    // history
                                    if (cpi.hba_vendor != 0 && cpi.hba_device != 0) // try to filter out when
                                                                                    // information is not available
                                    {
                                        ...
                                        // NOT: Subvendor and subdevice seem to specify something
                                        // further up the tree from the adapter itself.
                                    }
                                }
#endif
                            }
                        ...
                        // let the library now go out and set up the device struct after
                        // sending some commands.
                        if (device->drive_info.interface_type == USB_INTERFACE ||
                            device->drive_info.interface_type == IEEE_1394_INTERFACE)
                        {
                            // TODO: Actually get the VID and PID set before calling this.
                            //       This will require some more research, but we should be
                            //       able to do this now that we know the interface
...
#    endif // ATA_FLAG_ICC
            ...
            if (ret == SUCCESS)
            {
                ...
                {
                    // cam_error_print(scsiIoCtx->device->os_info.cam_dev, ccb,
                    // CAM_ESF_ALL /*error string flags*/, CAM_EPF_ALL, stdout);
                    if ((ccb->ccb_h.status & CAM_STATUS_MASK) != CAM_REQ_CMP)
                    {
                        if ((ccb->ccb_h.status & CAM_STATUS_MASK) == CAM_ATA_STATUS_ERROR)
                        {
                            ...
                            if (VERBOSITY_QUIET < scsiIoCtx->device->deviceVerbosity)
                            {
                                printf("WARN: I/O went through but drive returned "
                                       "status=0x%02" PRIX8 " error=0x%02" PRIX8 "\n",
                    ...
                    // get the rtfrs and put them into a "sense buffer". In other words,
                    // fill in the sense buffer with the rtfrs in descriptor format
                    if (scsiIoCtx->psense != M_NULLPTR) // check that the pointer is valid
                    {
                                                            ...
                                                            // our rtfrs using descriptor format
                        {
                            ...
                            scsiIoCtx->returnStatus.senseKey = 0x01; // Not setting check condition since the IO was in
                                                                     // fact successful
                            ...
                            if (scsiIoCtx->pAtaCmdOpts->commandType == ATA_CMD_TYPE_EXTENDED_TASKFILE)
                            {
                                ...
                                // fill in the ext registers while we're in this if...no need
                                // for another one
                                if (ataio->res.flags & CAM_ATAIO_48BIT) /* check this flag to make sure we read
                                                                           valid data */
                                {
        ...
        else
        {
            ...
            {
                printf("WARN: Sending non-ATA commnad to ATA Drive [FreeBSD CAM driver "
                       "does not support SAT Specification]\n");
            }
            ret = BAD_PARAMETER;
        }
    }
...
eReturnValues send_Scsi_Cam_IO(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (ccb != M_NULLPTR)
    {
        ...
        csio->sense_len     = scsiIoCtx->senseDataSize; // So it seems the csio has it's own buffer
                                                        // for Sense...so revist.
        ...
        else
        {
            ...
            {
                // memcpy(scsiIoCtx->psense, &csio->sense_data.sense_buf[0],
                              ...
                              M_ATTR_UNUSED uint64_t flags)
{
    ...
    else
    {
             ...
             ++driveNumber)
        {
            ...
            if (fd >= 0)
            {
                // Not sure this is necessary, but add back in if we find any issues -
                // TJE
...
eReturnValues send_NVMe_IO(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    else
    {
        ...
        // NOTE: This ifdef may require more finite tuning using these version
        // values:
        // https://docs.freebsd.org/en_US.ISO8859-1/books/porters-handbook/versions-11.html
#    if defined(__FreeBSD_version) && (__FreeBSD_version >= 1200000)
        // FreeBSD 11.4 and later didn't use a structure for the status completion
        // data, but a uint16 type which made this easy
        nvmeIoCtx->commandCompletionData.dw3 = M_WordsTo4ByteValue(pt.cpl.status, pt.cpl.cid);
#    else
        // FreeBSD 11.3 or earlier with NVMe support used a structure, so we need to
        // copy to a temp variable to get around this compiler error. This is not
        // portable, but according to the FreeBSD source tree, the change away from
        // a bitfield struct was done to support big endian FreeBSD, so this SHOULD
        // be ok to keep like this.
        uint16_t temp = 0;
...
{
    // There is nothing to unlock since you cannot open a CAM device with
    // O_NONBLOCK
    return SUCCESS;
}

// For the file syste cache update and unmount, these two functions may be
// useful: getfsstat ???
// https://www.freebsd.org/cgi/man.cgi?query=getfsstat&sektion=2&apropos=0&manpath=FreeBSD+13.0-RELEASE+and+Ports
// statfs ???
// https://www.freebsd.org/cgi/man.cgi?query=statfs&sektion=2&apropos=0&manpath=FreeBSD+13.0-RELEASE+and+Ports
// fstab ???
// https://www.freebsd.org/cgi/man.cgi?query=fstab&sektion=5&apropos=0&manpath=FreeBSD+13.0-RELEASE+and+Ports
...
{
    // TODO: I have not found an analog to Linux which is usually the most helpful
    // for figuring out what to do.
    //       I haven't found any other API or IOCTL that reloads the partition
    //       table on the disk (which is pretty close) Only thing that might work
    //       is a reload of an already mounted FS? At least that's a best guess.
    //       I will need a better test setup to validate this than I currently
    //       have access to - TJE
    // MNT_RELOAD ???
    // sys/disk.h provides lots of info about the device. One of the IOCTLs may
    // allow for a reread of the partition table.
    return NOT_SUPPORTED;
}
