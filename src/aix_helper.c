...
#if defined(DF_NVME) // this will be defined in devinfo.h if NVMe devices are
                     // supported(should be in AIX7 and up)
...
// File systems are /dev/hd?, but block disks are /dev/hdisk? and raw disks are
// /dev/rhdisk? There is some way these are related, but I'm not sure how to get
// the mapping between them. The code below will compile and run, but you need
// to know what input to give it to do anything useful. If we can figure out how
// /dev/hd is on /dev/hdisk or the other way around, this should be usable, or
// can be modified to work better-TJE

// static int get_Partition_Count(const char * blockDeviceName)
// {
//     int result = 0;
//     FILE *mount = setmntent(MOUNTED, "r");//MNTTAB or MOUNTED. MOUNTED is
//     virtual file struct mntent *entry = M_NULLPTR; while(M_NULLPTR != (entry
//     = getmntent(mount)))
//     {
...
// //listCount is the number of these structures, which should be returned by
// get_Partition_Count static eReturnValues get_Partition_List(const char *
// blockDeviceName, ptrsPartitionInfo partitionInfoList, int listCount)
// {
//     int result = SUCCESS;
//     int matchesFound = 0;
//     if(listCount > 0)
//     {
//         FILE *mount = setmntent(MOUNTED, "r");//MNTTAB or MOUNTED. MOUNTED is
//         virtual file struct mntent *entry = M_NULLPTR; while(M_NULLPTR !=
//         (entry = getmntent(mount)))
//         {
//             if(strstr(entry->mnt_fsname, blockDeviceName))
//             {
...
//                 {
//                     snprintf((partitionInfoList + matchesFound)->fsName,
//                     PART_INFO_NAME_LENGTH, "%s", entry->mnt_fsname);
//                     snprintf((partitionInfoList + matchesFound)->mntPath,
//                     PART_INFO_PATH_LENGTH, "%s", entry->mnt_dir);
//                     ++matchesFound;
//                 }
//                 else
//                 {
//                     result = MEMORY_FAILURE;//out of memory to copy all
//                     results to the list.
//                 }
//             }
//         }
...
// //in future, when code is written, use internal API used by the other
// FS/unmount options to figure this out.-TJE static int
// set_Device_Partition_Info(M_ATTR_UNUSED tDevice* device)
// {
...
static void print_devinfo_struct(struct devinfo* devInfoData)
{
    ...
    if (devInfoData)
    {
        ...
        // based on type/subtype and flags, check proper structure in union called
        // un
        switch (devInfoData->devtype)
        {
        case DD_TMSCSI:
        ...
        case DD_SCDISK:
            ...
            if (devInfoData->flags & DF_LGDSK)
            {
                ...
                if (devInfoData->un.scdk64.flags & DF_LBP)
                {
                    ...
                    {
                        printf("\t\t\t\tLogical Block Provisioning support is enabled on "
                               "this AIX node\n");
                    }
                    if (devInfoData->un.scdk64.lbp_flags & SCDK_LBPF_UNSUPPORTED_DEVICE)
                    {
                        printf("\t\t\t\tAIX does not support Logical Block Provisioning on "
                               "this device\n");
                    }
        ...
        case DD_CIO:
            // do we care?
            break;
        case DD_DISK_C: // disk controller - fallthrough??? not sure if this goes
        ...
        case DD_BUS:
            ...
            case DS_NVME: /* non-volatile Memory controller   */
                ...
                {
                    printf("\t\t\t\tAdapter is using PRP & 4B data address alignment for "
                           "r/w\n")
                }
...
#endif // DS_VRTSCSI
            default:
                // Do nothing for now. We can parse more things out when we need to in
                // the future -TJE
                break;
            }
            break;
...
static void print_CuDv_Struct(struct CuDv* cudv)
{
    ...
    if (cudv->PdDvLn_info)
    {
        ...
        // TODO: Print out this structure...this seems to get deeper and deeper that
        // I'm stopping here for now - TJE
...
static int get_Adapter_IDs(tDevice* device, char* name)
{
    ...
    if (name && safe_strlen(name) > 0)
    {
        ...
        if (ptrcudv)
        {
            ...
            // first check if PdDvLn = "adapter/pci... in it. If it does, then this is
            // the result we are looking for
            if (strstr(ptrcudv->PdDvLn_Lvalue, "adapter/pci"))
            {
                // Through trial and error testing and searching, this is a typical
                // output we want to find and parse: adapter/pci/pcividpid or
                // adapter/pciex/pcividpid
                // Note that the vid and pid are not in the correct order. These are
                // reported as little endian, but the big endian AIX system does not
                // seem to swap them to the correct order for it...looks like it just
                // copies them which leaves them in the wrong order.

                // Set a char pointer to the last / + 1
                const char* ids = strrchr(ptrcudv->PdDvLn_Lvalue, '/') + 1;
                // now convert this out to a uint32, then byte swap it, then separate
                // into VID and PID
                ...
                else
                {
                    ...
                    // wrapping this as if there was little endian AIX, I doubt it would
                    // exhibit the same issue
                    byte_Swap_32(&idCombo);
                    word_Swap_32(&idCombo); // This is done after the byte swap since it
                                            // will change the order of VID and PID back
                                            // to expected place
            ...
            {
                // Check the parent. from hdisk, this is usually 2 parents up from what
                // I've seen -TJE
...
{
    // use openx. Do not set the SC_DIAGNOSTIC. That can be redone in the
    // lock/unlock routines. open can be used but always performed a SCSI2
    // reserve. Not necessary for this software first open the provided handle
    // Possible Extension flags:
    // SC_DIAGNOSTIC - more or less exclusive. Required for certain dianostic mode
    // IOCTLs SC_FORCED_OPEN_LUN - lun reset or target reset will occur regardless
    // of any reservations SC_FORCED_OPEN - forces a bus reset before opening
    // SC_RETAIN_RESERVATION - retains reservation of device after close by not
    // issuing the release (Might be useful if using the IOCTLs for reservations -
    // TJE) SC_NO_RESERVE - prevents reservation during openx (this might be good
    // for us since we are not usually reading or writing -TJE) SC_SINGLE - places
    // device in exclusive access mode. (Reservation exclusive access mode???)
    // SC_PR_SHARED_REGISTER - persistent reserve, register and ignore key is used
    // when opening
    ...
    if (handleOpened)
    {
        // able to open the device. Read the devinfo, then open controller and read
        // its devinfo -TJE
        ...
        // Now get the parent handle, open it and request the IOCINFO for the parent
        // since that fill provide more details -TJE set name and friendly name
            ...
            if (ptrcudv)
            {
                ...
                if (safe_strlen(ptrcudv->parent) > 0)
                {
                    ...
                    if (strstr(ptrcudv->parent, "sata"))
                    {
                        ...
                        device->os_info.ptType      = AIX_PASSTHROUGH_SATA; // If we ever get a handle other than
                                                                            // rhdisk, switch to atapi or SCSI for
                                                                            // those handles-TJE
                    ...
                    else if (strstr(ptrcudv->parent, "ide"))
                    {
                        ...
                        device->os_info.ptType      = AIX_PASSTHROUGH_IDE_ATA; // If we ever get a handle other than
                                                                               // rhdisk, switch to atapi for those
                                                                               // handles-TJE
                        device->drive_info.drive_type     = ATA_DRIVE;
                        device->drive_info.interface_type = IDE_INTERFACE;
                    }
                    ...
                    else if (strstr(ptrcudv->parent,
                                    "scsi")) // note this is parallel scsi
                    {
                        // SCSI passthrough.
                    ...
                    //                         {
                    //                             if (driveInfo.flags & DF_LGDSK &&
                    //                             driveInfo.flags & DF_IVAL)
                    //                             {
                    //                                 //check if NVMe
                    // #if defined (DF_NVME) && !defined (DISABLE_NVME_PASSTHROUGH)
                    //                                 if (driveInfo.un.scdk64.flags &
                    //                                 DF_NVME)
                    //                                 {
                    //                                     //NVMe device & interface.
                    //                                     NOTE: need to make sure we were
                    //                                     able to successfully
                    //                                     //able to open the controller
                    //                                     handle before we enable the
                    //                                     following code or
                    //                                     //unexpected behavior will
                    //                                     likely occur. -TJE if
                    //                                     (device->os_info.ctrlfdValid)
                    //                                     {
                    ...
                    //                             }
                    //                         }
                    // call the recursive function to get the adapter id. It uses odm to
                    // get this, which may need to work through multiple layers-TJE
...
{
    // if unable to find another way to do this, can close and reopen with
    // SC_FORCED_OPEN
    return NOT_SUPPORTED;
}
...
/*
NOTES:
https://www.ibm.com/docs/en/aix/7.1?topic=subsystem-scdisk-scsi-device-driver
...
Most of this code will focus on HDD, although Tape, CD, optical can all be
supported.
...
IOCINFO provides lots of information about the device, including max transfer
size, etc.
...
Other ioctls for specific commands:
    ...
    DK_RWBUFFER - issue one more more write buffer commands in single IOCTL. All
outstanding IO is quiesced before this is issued

Multi-path
(https://www.ibm.com/docs/en/aix/7.1?topic=management-multiple-path-io):
    DKPATHIOCMD - for multi-path capable devices. Works like DKIOCMD except that
the input path is used instead of normal path selection DKPATHIOCMD - force
specific path for all subsequent IO DKPATHRWBUFFER - same as DKRWBUFFER except
using input path instead DKPATHPASSTHRU - same as DK_PASSTHRU, except input path
is used DKPCMPASSTHRU - PCM specific structure passed to PCM (Path control
module) directly


DK_PASSTHRU - The DK_PASSTHRU operation differs from the DKIOCMD operation in
that it does not require an openx command with the ext argument of
SC_DIAGNOSTIC. Because of this, a DK_PASSTHRU operation can be issued to devices
that are in use by other operations.
            - SC_MIX_IO requests that write data to devices are prohibited and
will fail.
            - SC_QUIESCE_IO, all other I/O requests will be quiesced before the
DK_PASSTHRU request is issued to the device (no zero timeout allowed)
            - If an SC_QUIESCE_IO request has a nonzero timeout value that is
too large for the device, the DK_PASSTHRU request will be failed with a return
code of -1, the errno global variable will be set to EINVAL, the einval_arg
field will be set to a value of SC_PASSTHRU_INV_TO (defined in the
/usr/include/sys/scsi.h file), and the timeout_value will be set to the largest
allowed value.
            - The version field of the sc_passthru structure can be set to the
value of SC_VERSION_2, and the user can provide the following fields:
                variable_cdb_ptr is a pointer to a buffer that contains the
Variable SCSI cdb. variable_cdb_length determines the length of the cdb variable
to which the variable_cdb_ptr field points.
            - The devinfo structure defines the maximum transfer size for the
command

IDEPASSTHRU - only mentioned on the USB mass storage driver page for ATAPI
devices -
https://www.ibm.com/docs/en/aix/7.1?topic=subsystem-usb-mass-storage-client-device-driver

Handles:
The special files that are used by the scdisk device driver include the
following (listed by type of device):

Hard disk devices:
/dev/rhdisk0, /dev/rhdisk1,..., /dev/rhdiskn	Provides an interface to allow
SCSI device drivers character access (raw I/O access and control functions) to
SCSI hard disks. /dev/hdisk0, /dev/hdisk1,..., /dev/hdiskn	Provides an
interface to allow SCSI device drivers block I/O access to SCSI hard disks.

CD-ROM devices:
/dev/rcd0, /dev/rcd1,..., /dev/rcdn	Provides an interface to allow SCSI
device drivers character access (raw I/O access and control functions) to SCSI
CD-ROM disks. /dev/cd0, /dev/cd1,..., /dev/cdn	Provides an interface to allow
SCSI device drivers block I/O access to SCSI CD-ROM disks.

Read/write optical devices:
/dev/romd0, /dev/romd1,..., /dev/romdn	Provides an interface to allow SCSI
device drivers character access (raw I/O access and control functions) to SCSI
read/write optical devices. /dev/omd0, /dev/omd1,..., /dev/omdn	Provides an
interface to allow SCSI device drivers block I/O access to SCSI read/write
optical devices.

Tape devices:
/dev/rmt255, /dev/rmt255.1, /dev/rmt255.2, ..., /dev/rmt255.7	Provide an
interface to allow SCSI device drivers to access SCSI tape drives.

SCSI Adapters:
/dev/scsi0, /dev/scsi1, ... /dev/scsin	Provides an interface for all SCSI
device drivers to access SCSI devices or adapters. /dev/vscsi0, /dev/vscsi1,...,
/dev/vscsin	Provide an interface to allow SCSI-2 Fast/Wide Adapter/A and
SCSI-2 Differential Fast/Wide Adapter/A device drivers to access SCSI devices or
adapters.
    - seems to be SCSI adapters and use SCIO<> type IOCTLs. Not sure if this is
needed for what we are after. This has a reset option though which we may want.
-TJE
    - SCIORESET can do bus or LUN reset?

NVMe controller:
The /dev/nvmen special file provides interfaces to the NVMe controller device
driver.

NVMe disk/namespace:
The /dev/hdiskn special file provides interfaces to the NVMe storage device
driver. - flags field will set DF_SSD and DF_NVME. DF_4B_ALINGED  specifies all
host buffer addresses must be 4 byte aligned
...
// Trying to use the big passthrough as it allows much larger CDBs and even
// variable length CDBs instead. Enable using this is we ever need it for
// compatibility or it does something different than normal passthrough that we
// need-TJE
//  static eReturnValues send_AIX_SCSI_Diag_IO(ScsiIoCtx *scsiIoCtx)
//  {
//      //uses the DKIOCMD when opened with the diagnostic mode flag so that
//      this command is issued with nothing else in queue
//      //and more or less exclusive access to the device.
//      //NOTE: This does not issue request sense upon an error! This will need
//      to be done manually! int         ret          = SUCCESS; bool
//      issueRequestSense = false; if (scsiIoCtx->cdbLength <= 12)
//      {
...
//         aixIoCmd.q_tag_msg = 0;//SC_NO_Q, SC_SIMPLE_Q, SC_HEAD_OF_Q,
//         SC_ORDERED_Q, SC_ACA_Q aixIoCmd.flags = SC_QUIESCE_IO;//or SC_MIX_IO?
//         Leaving as quiesce for now -TJE aixIoCmd.q_flags = 0;//SC_Q_CLR,
//         SC_Q_RESUME, SC_CLEAR_ACA
//         //setup flags
...
//         scsiIoCtx->device->drive_info.defaultTimeoutSeconds >
//         scsiIoCtx->timeout)
//         {
//             aixIoCmd.timeout_value =
//             scsiIoCtx->device->drive_info.defaultTimeoutSeconds; if
//             (scsiIoCtx->device->drive_info.defaultTimeoutSeconds >=
//             AIX_MAX_CMD_TIMEOUT_SECONDS)
//             {
//                 aixIoCmd.timeout_value = UINT32_MAX;//no timeout or maximum
//                 timeout
//             }
//         }
//         else
...
//                 if (scsiIoCtx->device->drive_info.defaultTimeoutSeconds >=
//                 AIX_MAX_CMD_TIMEOUT_SECONDS)
//                 {
//                     aixIoCmd.timeout_value = UINT32_MAX;//no timeout or
//                     maximum timeout
//                 }
//             }
//             else
...
//         aixIoCmd.lun = 0;//if greater than 7, must be used to ignore LUN bits
//         in SCSI 1 commands
...
//                 }
//                 print_Passthrough_Bus_And_Adapter_Status(aixIoCmd.status_validity,
//                 aixIoCmd.scsi_bus_status, ioctlCode == DKIOCMD ?
//                 SC_ADAP_SC_ERR : SC_ADAP_SAM_ERR, aixIoCmd.adapter_status);
...
//                 break;
//             case SC_CHECK_CONDITION:
//                 ret = SUCCESS;//succesfully issued the IO, so pass sense data
//                 back up the stack issueRequestSense = true; break;
//             case SC_BUSY_STATUS:
//             case SC_INTMD_GOOD:
//             case SC_RESERVATION_CONFLICT:
//             case SC_COMMAND_TERMINATED:
...
//             ret = OS_PASSTHROUGH_FAILURE;
//         }
//         scsiIoCtx->device->drive_info.lastCommandTimeNanoSeconds =
//         get_Nano_Seconds(commandTimer);
//     }
...
//                 }
//                 print_Passthrough_Bus_And_Adapter_Status(aixIoCmd.status_validity,
//                 aixIoCmd.scsi_bus_status, SC_ADAP_SAM_ERR,
//                 aixIoCmd.adapter_status);
//                 print_Adapter_Queue_Status(aixIoCmd.adap_q_status);
//             }
//         }
//         if (aixIoCmd.status_validity == 0)
...
//         //      There are not a lot of commands larger than 16B being sent
//         today -TJE ret = OS_COMMAND_NOT_AVAILABLE;
//     }
...
//         SC_ORDERED_Q, SC_ACA_Q aixIoCmd.flags = SC_QUIESCE_IO;//or SC_MIX_IO?
//         Leaving as quiesce for now -TJE aixIoCmd.q_flags = 0;//SC_Q_CLR,
//         SC_Q_RESUME, SC_CLEAR_ACA aixIoCmd.flags = B_READ;
...
//         aixIoCmd.scsi_cdb[1] = 0;//TODO: Descriptor bit? Can either track
//         support early on in discovery, or infer from the command that was
//         sent what to do-TJE aixIoCmd.scsi_cdb[2] = RESERVED;
...
//                 break;
//             case SC_CHECK_CONDITION:
//                 ret = OS_PASSTHROUGH_FAILURE;//this means that something bad
//                 happened during request sense, so consider this a bigger
//                 failure-TJE break;
//             case SC_BUSY_STATUS:
//             case SC_INTMD_GOOD:
//             case SC_RESERVATION_CONFLICT:
...
static eReturnValues send_AIX_SCSI_Passthrough(ScsiIoCtx* scsiIoCtx)
{
    ...
    aixPassthrough.version = SCSI_VERSION_2;  // TODO: version 1 vs version 2? Probably only helpful on
                                              // old AIX installations
        ...
        scsiIoCtx->device->drive_info.defaultTimeoutSeconds > scsiIoCtx->timeout)
    {
        ...
        {
            aixPassthrough.timeout_value = UINT32_MAX; // no timeout or maximum
                                                       // timeout
        }
    }
    else
    ...
    if (ioctlResult < 0)
    {
        ...
        if (VERBOSITY_COMMAND_VERBOSE <= scsiIoCtx->device->deviceVerbosity)
        {
            ...
            {
                printf("SCSI ID is different than was provided and the adapter has "
                       "updated it to: %" PRIu64 "\n",
            ...
            {
                // TODO: Some of these, upon error will return an allowed value. Look at
                // showing these in this error output.-TJE
                ...
                case SC_PASSTHRU_INV_TO:
                    // Supposedly this can tell what an appropriate maximum timeout is in
                    // the timeout offset when this occurs.
...
// IOCTL IDEIOIDENT with struct ide_identify may be helpful to identify when IDE
// or SATA passthrough are available-TJE

// NOTE: This issues the IDE_ATA passthrough. There is a separate ATAPI
// passthrough if we need to handle those
//       using that IOCTL instead of the SCSI passthrough IOCTLs. Can be done
//       later as we currently do not handle CD/DVDs, etc -TJE
static eReturnValues send_AIX_IDE_ATA_Passthrough(ScsiIoCtx* scsiIoCtx)
{
    ...
    {
        // only possible to issue 28bit commands in ide passthrough. SATA
        // passthrough allows more though-TJE
        return OS_COMMAND_NOT_AVAILABLE;
    }
    ...
    // NOTE: There is a flag for ATA_BUS_RESET that may be useful for implementing
    // a reset -TJE
    ...
    case XFER_DATA_OUT:
        ...
        // if needed we can use B_WRITE from the scsi flags, but skipping for now -
        // TJE idePassthrough.flags = B_WRITE;
        break;
    case XFER_NO_DATA:
        idePassthrough.flags |= IDE_PASSTHRU_READ;
        break;
        ...
        scsiIoCtx->device->drive_info.defaultTimeoutSeconds > scsiIoCtx->pAtaCmdOpts->timeout)
    {
        ...
        if (scsiIoCtx->device->drive_info.defaultTimeoutSeconds >= AIX_MAX_CMD_TIMEOUT_SECONDS)
        {
            idePassthrough.timeout_value = UINT32_MAX; // no timeout or maximum
                                                       // timeout
        }
    }
    else
    ...
    // set returned status and error registers. It looks like that is all we will
    // get -TJE
    scsiIoCtx->pAtaCmdOpts->rtfr.status = idePassthrough.ata_status;
    scsiIoCtx->pAtaCmdOpts->rtfr.error  = idePassthrough.ata_error;

    // resid is another part of the structure. I'm guessing this is a count of
    // what data was or was not transferred? -TJE
...
static eReturnValues send_AIX_IDE_ATAPI_Passthrough(ScsiIoCtx* scsiIoCtx)
{
    ...
    idePassthrough.rsv0 = IDE_PASSTHRU_VERSION_01; // when this is set we can pass the sense data
                                                   // and sense length in. otherwise you have to
                                                   // request sense manually on error :/
    idePassthrough.sense_data_length = scsiIoCtx->senseDataSize;
    idePassthrough.sense_data        = scsiIoCtx->psense;
#endif

    // now set the cdb
    idePassthrough.atapi_cmd.length = 12; // ATAPI supports up to 12 or up to 16B commands. So this is set to 12
                                          // or 16 even if the CDB is smaller.-TJE
    if (scsiIoCtx->cdbLength > 12)
    {
        idePassthrough.atapi_cmd.length = 16;
        if (scsiIoCtx->cdbLength > 16)
        {
            return BAD_PARAMETER; // this should not happen on any atapi device since
                                  // the limit is fixed to 16B
        }
    }
    ...
    memcpy(&idePassthrough.atapi_cmd.packet.bytes[0], &scsiIoCtx->cdb[1],
           M_Min(15, scsiIoCtx->cdbLength - 1)); // this holds remaining bytes after
                                                 // opcode, hence -1 from length
...
#if !defined IDE_PASSTHRU_VERSION_01
    // TODO: check if the status/error indicated a problem, then issue request
    // sense
    //       This code is likely only needed for older versions of AIX. Fill it in
    //       only when needed. Some of it has been started -TJE
    if (idePassthrough.ata_status & BIT0)
    {
        ...
        requestSensePT.atapi_cmd.length = 12; // ATAPI supports up to 12 or up to 16B commands. So this is set to
                                              // 12 or 16 even if the CDB is smaller.-TJE
        ...
        if (ioctlResult < 0)
        {
            // Any error handling after request sense that is necessary? -TJE
        }
        else
        {
...
static eReturnValues send_AIX_SATA_Passthrough(ScsiIoCtx* scsiIoCtx)
{
    ...
    {
        // is is not possible to issue the commands using AUX or ICC registers using
        // this passthrough -TJE
        return OS_COMMAND_NOT_AVAILABLE;
    }
    ...
    sataPassthrough.version = 0; // I don't see a defined version in ide.h, so setting zero as seems to
                                 // be how most of these work in AIX-TJE
    ...
    case XFER_DATA_OUT:
        // no flags for write that I can find in ide.h
        // if needed we can use B_WRITE from the scsi flags, but skipping for now -
        // TJE sataPassthrough.flags = B_WRITE;
        break;
    case XFER_NO_DATA:
        sataPassthrough.flags |= SATA_PASSTHRU_READ;
        break;
    ...
    else
    {
        ...
        {
            sataPassthrough.timeout_value = 15; // default to 15 second timeout
        }
    }
    ...
    // NOTE: The regular structure will also be setup as it has other flags, etc
    // that can be filled in that
    //       are not part of or duplicated in the ext command structure. It is not
    //       clear if those flags must also be set or not -TJE
    ...
    else
    {
        ...
        // only setting the "head" or upper-most LBA bits since the other device
        // handles dev bit
    ...
    if (ioctlResult < 0)
    {
        ...
        // adapter_set_flags will have some output information upon completion to
        // detect errors-TJE status_validity will be set to indicate these errors:
        // #define	ATA_IDE_STATUS		1	/* ata.status is valid */
        // #define ATA_ERROR_VALID		2	/* ata.error reflects error */
        // #define ATA_DIAGNOSTICS_ERROR	4	/* adapter diagnostics reflects
        // error*/ #define ATA_SMART_VALID    ATA_DIAGNOSTICS_ERROR #define
        // ATA_CMD_TIMEOUT		0x08	/* adapter timeout of command	*/
        // #define ATA_NO_DEVICE_RESPONSE	0x10	/* device continually busy
        // */ #define ATA_IDE_BUS_RESET	0x20	/* adapter reset the bus
        // */ #define ATA_IDE_DMA_ERROR	0x40	/* DMA error occurred */ #define
        // ATA_IDE_DMA_NORES	0x80	/* DMA Resource error occured   */
    }
    ...
    if (scsiIoCtx->pAtaCmdOpts->commandType == ATA_CMD_TYPE_EXTENDED_TASKFILE)
    {
        ...
        // LBA mode, backwards compat bits, and dev bit should match what went in,
        // so just take those from the tfr that was sent -TJE NOTE: Does not appear
        // to be a way to read lower 4 device/head bits after command completion
        // -TJE
        if (scsiIoCtx->pAtaCmdOpts->tfr.DeviceHead & DEVICE_SELECT_BIT)
        {
    ...
    else
    {
        ...
        // LBA mode, backwards compat bits, and dev bit should match what went in,
        // so just take those from the tfr that was sent -TJE
        if (scsiIoCtx->pAtaCmdOpts->tfr.DeviceHead & DEVICE_SELECT_BIT)
        {
            scsiIoCtx->pAtaCmdOpts->rtfr.device |= DEVICE_SELECT_BIT;
        }
...
{
    // switch based on value stored in os_info to define which passthrough
    // interface to use to issue commands -TJE
    eReturnValues ret = SUCCESS;
    ...
    case SCSI_INTERFACE:
        ...
        case AIX_PASSTHROUGH_SCSI:
            ...
            // DIAG IO when the diagnostic flag was set on open, but that is not
            // currently done today.-TJE ret = send_AIX_SCSI_Diag_IO(scsiIoCtx);
...
// TODO: In a RAID configuration, physical disks in the RAID get a /dev/pdsk
// handle
...
eReturnValues get_Device_List(tDevice* const ptrToDeviceList, uint32_t sizeInBytes, versionBlock ver, uint64_t flags)
{
    ...
    devs[i] = M_NULLPTR; // Added this so the for loop down doesn't cause a
                         // segmentation fault.
    safe_free_dirent(namelist);
    ...
    else
    {
        ...
        d               = ptrToDeviceList;
        for (driveNumber = 0; ((driveNumber >= 0 && C_CAST(unsigned int, driveNumber) < MAX_DEVICES_TO_SCAN &&
                                driveNumber < (num_devs)) &&
             ...
             ++driveNumber)
        {
            ...
            // NOTE: When opening a handle, there may be an issue if SC_DIAGNOSTIC is
            // not specified. This can be an issue when there is somethign not quite
            // right with the drive. This cannot be used all the time though. You
            // cannot use it on the system drive. So first try without it, then try
            // again if it won't open with SC_DIAGNOSTIC.
...
eReturnValues send_NVMe_IO(nvmeCmdCtx* nvmeIoCtx)
{
    ...
    // If this is not done properly, undefined behavior will occur as it may issue
    // the wrong command on the wrong queue.
    eReturnValues        ret               = SUCCESS;
    ...
    // TODO: If cdw2 or cdw3 are set, need to return some kind of
    // OS_COMMAND_NOT_SUPPORTED error

    // Setup the command. NOTE: this must be set in host endianness, not device
    // endianness (little)
        ...
        if (VERBOSITY_COMMAND_VERBOSE <= nvmeIoCtx->device->deviceVerbosity)
        {
            ...
            if (nvmeIoCtx->device->os_info.last_error == EINVAL)
            {
                printf("Invalid field in nvme passthrough struct:\n")
                    // response status has a code set to help indicate what was not
                    // allowed
    ...
    else // ret == 0. NOTE: Positive return values will currently fall here too
         // but those are not documented as far as I can see - TJE
    {
        ret = SUCCESS;
        ...
        nvmeIoCtx->commandCompletionData.dw0      = nvmePassthrough.resp.dword_0; // TODO: Is this returned in host
                                                                             // endianness or does it need swapping?
        nvmeIoCtx->commandCompletionData.dw3 =
            M_WordsTo4ByteValue(nvmePassthrough.resp.status,
                                0); // phase is cleared, command identifier is not available
    }
...
// supposedly, when not in diagnostic mode, the read(), write(), lseek() can all
// be used. This is currently not needed though. Another thing we may want to
// implement here is the read/write ioctl codes that are available.
...
eReturnValues os_Lock_Device(tDevice* device)
{
    ...
    {
        close(device->os_info.fd); // this must be done first or the openx will
                                   // fail!
