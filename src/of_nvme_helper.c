...
// Need to setup an admin identify and try sending it. If this device doesn't
// support this IOCTL, it should fail, otherwise it will work. This is the same
// way the sample app works. Would be better if there was some other buffer to
...
bool supports_OFNVME_IO(HANDLE deviceHandle)
{
    ...
    if (passthroughBuffer)
    {
        ...
        if (ERROR_IO_PENDING == last_error) // This will only happen for overlapped commands. If the
...
{
    eReturnValues ret = OS_COMMAND_NOT_AVAILABLE; // Start with this since older drivers may or
...
#    endif // OFNVME_DEBUG
    uint32_t bufferSize =
        sizeof(NVME_PASS_THROUGH_IOCTL) + nvmeIoCtx->dataSize; // NOTE: No metadata. Don't think Windows supports a
                                                               // separate metadata buffer
    uint8_t* passthroughBuffer =
        C_CAST(uint8_t*, safe_calloc_aligned(bufferSize, sizeof(uint8_t), nvmeIoCtx->device->os_info.minimumAlignment));
    if (passthroughBuffer)
    {
        seatimer_t               commandTimer;
        BOOL                     success = TRUE;
        ...
        case NVM_ADMIN_CMD:
            ioctl->QueueId    = 0;                              // admin queue
            ioctl->NVMeCmd[0] = nvmeIoCtx->cmd.adminCmd.opcode; // This doesn't currently take into
                                                                // account fused or PRP vs SGL transfers
            ...
            // data pointers are in next DWORDs not sure if these should be set here
            // since they will be virtual addresses
            ...
            break;
        case NVM_CMD:
            ioctl->QueueId = 1;                               // should this always be set to 1? or something else
                                                              // depending on capabitlies?
            ioctl->NVMeCmd[0] = nvmeIoCtx->cmd.nvmCmd.opcode; // This doesn't currently take into
                                                              // account fused or PRP vs SGL transfers
        ...
        // Setting to zero like the sample file does. No idea what these are used
        // for.
        ioctl->VendorSpecific[0] = 0;
        ioctl->VendorSpecific[1] = 0;

        // databuffer length...set to zero for read or non-data. set to a value for
        // sending data to the device set direction along with length set return
        // buffer length...at least size of the IOCTL structure, but larger if
        // expecting data from the device...whatever that length will be
        ...
        case XFER_DATA_IN:
            ioctl->Direction   = NVME_FROM_DEV_TO_HOST; // read or non-data
            ioctl->MetaDataLen = 0;                     // no metadata or interleaved should be set to zero.
            ioctl->ReturnBufferLen =
                sizeof(NVME_PASS_THROUGH_IOCTL) + nvmeIoCtx->dataSize; // this should include metadata size when
                                                                       // supporting metadata!
            ioctl->DataBufferLen = 0; // Set to zero because we aren't sending any data, only receiving.
            break;
        case XFER_DATA_OUT:
            ...
            ioctl->MetaDataLen     = 0; // no metadata or interleaved should be set to zero.
            ioctl->ReturnBufferLen = sizeof(NVME_PASS_THROUGH_IOCTL); // only the size of the structure
                                                                      // since we aren't reading anything
                                                                      // back from the device.
            ioctl->DataBufferLen = nvmeIoCtx->dataSize;               // NOTE: This size is supposed to include
                                                                      // metadata! It also depends on if metadata is
                                                                      // interleaved or at the beginning of the buffer
        ...
        if (ERROR_IO_PENDING ==
            nvmeIoCtx->device->os_info.last_error) // This will only happen for overlapped commands. If
        ...
        if (success)
        {
            ...
            if (ioctl->SrbIoCtrl.ReturnCode == NVME_IOCTL_SUCCESS)
            {
                if (nvmeIoCtx->commandDirection == XFER_DATA_IN)
                {
                    ...
                    memcpy(nvmeIoCtx->ptrData, ioctl->DataBuffer,
