...
bool set_ATA_Passthrough_Type_By_Trial_And_Error(tDevice* device)
{
        ...
        device->drive_info.drive_type == SCSI_DRIVE)
    {
        ...
        while (device->drive_info.passThroughHacks.passthroughType != ATA_PASSTHROUGH_UNKNOWN)
        {
            ...
            {
                // command succeeded so this is most likely the correct pass-through
                // type to use for this device setting drive type while we're in here
                // since it could help with a faster scan
