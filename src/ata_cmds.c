                               ...
                               uint16_t featureRegister)
{
    ...
    else
    {
        ...
        // set the DMA protocol to use based off the supported DMA modes of the
        // drive
...
eReturnValues ata_Security_Disable_Password(tDevice* device, uint8_t* ptrData)
{
    ...
    ataCommandOptions.dataSize = LEGACY_DRIVE_SEC_SIZE; // spec defines that this command always transfers
                                                        // this much data
    ataCommandOptions.ptrData           = ptrData;
...
eReturnValues ata_Get_Native_Max_Address_Ext(tDevice* device, uint64_t* nativeMaxLBA)
{
    ...
    {
        *nativeMaxLBA = M_BytesTo8ByteValue(0, 0, rtfrs.lbaHiExt, rtfrs.lbaMidExt, rtfrs.lbaLowExt, rtfrs.lbaHi,
                                            rtfrs.lbaMid, rtfrs.lbaLow);
    }
    return ret;
}
...
{
    eReturnValues ret = UNKNOWN;
    ret = ata_Accessible_Max_Address_Feature(device, AMAC_SET_ACCESSIBLE_MAX_ADDRESS, newMaxLBA, M_NULLPTR,
                                             changeId ? 1 : 0);
    return ret;
}
...
eReturnValues ata_Read_Native_Max_Address(tDevice* device, uint64_t* nativeMaxLBA, bool ext)
{
    ...
    if (ret == SUCCESS && nativeMaxLBA)
    {
        if (ext)
        {
            *nativeMaxLBA = M_BytesTo8ByteValue(0, 0, ataCommandOptions.rtfr.lbaHiExt, ataCommandOptions.rtfr.lbaMidExt,
                                                ataCommandOptions.rtfr.lbaLowExt, ataCommandOptions.rtfr.lbaHi,
                                                ataCommandOptions.rtfr.lbaMid, ataCommandOptions.rtfr.lbaLow);
        }
        else
        {
            *nativeMaxLBA = M_BytesTo4ByteValue(M_Nibble0(ataCommandOptions.rtfr.device), ataCommandOptions.rtfr.lbaHi,
                                                ataCommandOptions.rtfr.lbaMid, ataCommandOptions.rtfr.lbaLow);
        }
    }
                          ...
                          uint32_t    dataSize)
{
    ...
    case HPA_SET_MAX_PASSWORD:
        ...
        ataCommandOptions.commadProtocol           = ATA_PROTOCOL_PIO;
        ataCommandOptions.ataTransferBlocks        = ATA_PT_512B_BLOCKS;
        ataCommandOptions.tfr.SectorCount = 1; // spec says NA, but this will help the lower layer just like the
                                               // identify command
        break;
    default:
        return BAD_PARAMETER;
                                     ...
                                     uint32_t                   timeoutSeconds)
{
    ...
    {
        // change length location to TPSIU so the SATL knows how to calculate the
        // transfer length correctly since it is no longer just the sector count
        // field.
    ...
    {
        ataCommandOptions.timeout = 30; // using 30 seconds since some firmwares can
                                        // take a little longer to activate
    }

    // if we are told to use the activate command, we need to set the protocol to
    // non-data, set pdata to M_NULLPTR, and dataSize to 0
...
eReturnValues ata_Flush_Cache(tDevice* device, bool extendedCommand)
{
    ...
    // Changed from 45 seconds to max command timeout to make sure this has enough
    // time to complete without the system sending a reset. The spec mentions this
    // can take up to 30 minutes, but that is likely a rare condition. It should
...
eReturnValues ata_Read_Buffer(tDevice* device, uint8_t* ptrData, bool useDMA)
{
    ...
    ataCommandOptions.commandType              = ATA_CMD_TYPE_TASKFILE;
    ataCommandOptions.ptrData                  = ptrData;
    ataCommandOptions.dataSize = LEGACY_DRIVE_SEC_SIZE; // defined in the spec that this will only read a
                                                        // 512byte block of data
    ataCommandOptions.tfr.SectorCount = 1; // spec says N/A, but this helps with SAT translators and should be
                                ...
                                bool     extendedCmd)
{
    ...
    // now set the multiple count setting for the SAT builder so that this command
    // can actually work...and we need to set this as a power of 2, whereas the
    // device info is a number of logical sectors
    uint16_t multipleLogicalSectors = device->drive_info.ata_Options.logicalSectorsPerDRQDataBlock;
    while (ataCommandOptions.multipleCount <= 7 &&
           multipleLogicalSectors > 1) // multipleLogicalSectors should be greater than 1 so that we
                                       // get the proper 2^X power value for the SAT command.
    {
...
// eReturnValues ata_Packet(tDevice *device, uint8_t *scsiCDB, bool dmaBit, bool
// dmaDirBit, uint16_t byteCountLimit, uint8_t *ptrData, uint32_t *dataSize)
//{
                                    ...
                                    uint32_t dataSize)
{
    ...
    {
        // number of zones was set by set features/identify device data log field
        // last set by set features
        return ata_ZAC_Management_In(device, ZM_ACTION_ZONE_ACTIVATE, otherZoneDomainID, actionSpecificBits,
                                               ...
                                               uint8_t  sectorConfigurationDescriptorIndex)
{
    ...
    // Setting a 1 hour timeout. This should be way more than enough to complete
    // while allowing a way to handle a failing command due to a timeout instead
    // of using infinite which would never return. Using 1 hour since there are a
    // few rare cases where a drive may be in a state of processing something in
    // the background which could make this take longer than expected, but should
    // still complete long before 1 hour has elapsed.
    if (device->drive_info.ata_Options.isDevice1)
    {
        ataCommandOptions.tfr.DeviceHead |= DEVICE_SELECT_BIT;
    }
                                              ...
                                              uint32_t dataSize)
{
    ...
    ataCommandOptions.tfr.SectorCount          = M_Byte0(dataSize / LEGACY_DRIVE_SEC_SIZE);
    ataCommandOptions.tfr.SectorCount48        = M_Byte1(dataSize / LEGACY_DRIVE_SEC_SIZE);
    ataCommandOptions.tfr.Feature48            = C_CAST(uint8_t,
                               ...
                               uint32_t auxilary)
{
    ...
    ataCommandOptions.tfr.ErrorFeature =
        C_CAST(uint8_t, (M_Nibble0(C_CAST(uint64_t, subCommandSpecificFeature)) << 4) | M_Nibble0(subCommand));
                                           ...
                                           uint8_t* ptrData)
{
    ...
    ataCommandOptions.tfr.SectorCount48 =
        C_CAST(uint8_t, (subCommand & 0x1F) | ((prio & 0x03) << 6));  // prio, subcommand
    ataCommandOptions.tfr.SectorCount = C_CAST(uint8_t, ncqTag << 3); // shift into bits 7:3
    ataCommandOptions.tfr.LbaLow      = M_Byte0(lba);
    ataCommandOptions.tfr.LbaMid      = M_Byte1(lba);
    ataCommandOptions.tfr.LbaHi       = M_Byte3(lba);
...
eReturnValues ata_NCQ_Data_Set_Management(tDevice* device,
                                          ...
                                          uint8_t  ncqTag)
{
    uint32_t auxreg = 0; // bits 15:0 represent feature register of the NCQ data
