...
// the define below is to switch between different levels of SAT spec support.
// It is recommended that this is set to the highest version available valid
// values are 1 - 4 1 = SAT 2 = SAT2 3 = SAT3 4 = SAT4 This flag is not fully
// implemented at this time, but should be largely functional. All current SAT
// translation is according to SAT4 (or as close as possible)
...
// TODO: SPC and SBC optional feature flag translations. A good SATL should
// enable everything to make the drive look more and act more like a SCSI device

#if defined(_MSC_VER)
// Visual studio level 4 produces lots of warnings for "assignment within
// conditional expression" which is normally a good warning, but it is used
// HEAVILY in this file by the software SAT translator to return the field
// pointer on errors. So for VS only, this warning will be disabled in this
// file.
#    pragma warning(push)
...
{
    eReturnValues ret = NOT_SUPPORTED; // Many devices don't support this log
                                       // page.
    uint8_t* sense70logBuffer = C_CAST(
        uint8_t*, safe_calloc_aligned(14 + LOG_PAGE_HEADER_LENGTH, sizeof(uint8_t),
                                      device->os_info.minimumAlignment)); // allocate a buffer to get the rtfrs in. the
                                                                          // size of 12 is ATA Passthrough Descriptor +
                                                                          // 4byte log page header
    if (!sense70logBuffer)
    {
        perror("Calloc Failure!\n");
        return MEMORY_FAILURE;
    }
    // 0x16 is the ATA-pass through results log..which should have a descriptor
    // format sense data to parse
                                      ...
                                      parameterCode & 0x0F, sense70logBuffer, 14 + LOG_PAGE_HEADER_LENGTH))
    {
        ...
        {
            // first parameter starts at byte 4...and since we only requested 1, that
            // should be it 4 bytes after the start of the parameter, we get the
            // values check the descriptor code
                                                          ...
                                                          ataReturnTFRs* rtfr)
{
    ...
    if (senseDataFormat == SCSI_SENSE_CUR_INFO_DESC || senseDataFormat == SCSI_SENSE_DEFER_ERR_DESC)
    {
        ...
        // loop through the descriptors in case we were returned multiple...we only
        // want the ATA pass through descriptor.
        for (descriptorOffset = SCSI_DESC_FORMAT_DESC_INDEX;
             descriptorOffset <
             M_Min(C_CAST(uint8_t, senseDataSize - SCSI_DESC_FORMAT_DESC_INDEX), C_CAST(uint8_t, ptrSenseData[7] + 7));
                                                     ...
                                                     ataPassthroughCommand* ataCmd)
{
    ...
    )
    {
        ...
        // first check if the returned RTFRs have nonzero ext registers and if a log
        // page is available to read to get them
        ...
        bool unknownExtRegisters = false; // to be used below when deciding to issue
                                          // additional commands to get rtfrs
        uint8_t resultsLogPageParameterPointer = ptrSenseData[8] & 0x0F;
        // first save whatever we did get back from the sense data based on these
        // flags from fixed format information field
        ...
        if (asc != 0x00 && ascq != 0x1D) // 00,1D means that this was DEFINITELY
                                         // related to ATA passthrough results
        {
            // need to do a little validation that the returned registers make some
            // amount of sense
            if (asc == 0x21 && ascq == 0x04) // unaligned write command
            {
                // for unaligned write command, this seems to happen from SATLs when the
                // alignment status bit is set.
                if (senseKey == SENSE_KEY_ILLEGAL_REQUEST)
                {
                    ...
                    ataCmd->rtfr.lbaMidExt = 0;
                    ataCmd->rtfr.lbaHiExt  = 0;
                    // In Linux's LibATA, I have observed these to be returned in these
                    // positions.
                ...
                {
                    // TODO: check if a valid LBA was returned in information rather than
                    // RTFRs
                }
            }
            // THis need to be at the end.
            // if the status is empty, this is going to be treated as though no rtfrs
            // were received from the device
        ...
        // now, on the mini D4 firmware (and possibly other satls), the descriptor
        // with the complete rtfrs may also be present in byte 18 onwards, so check
        // if it is there or not to grab the data
        if (ptrSenseData[7] == 24 && unknownExtRegisters) // this length is very specific since it is the
                                                          // normal 10 bytes returned in fixed format data +
                                                          // the 14 bytes for the ata return descriptor in
                                                          // SAT
        {
            // check bytes 18 and 19 to make sure they match the descriptor
            uint8_t descriptorOffset = 18; // using this offset variable to make it easier to read and
                                           // compare to the sat spec offsets for the descriptor
            if (ptrSenseData[descriptorOffset] == SAT_DESCRIPTOR_CODE &&
                ptrSenseData[descriptorOffset + 1] == SAT_ADDT_DESC_LEN)
            {
        ...
        {
            ret = WARN_INCOMPLETE_RFTRS; // assume that the following code to get the
                                         // ext registers fails for some reason or
                                         // another.
            // ok, so we don't know all of the ext registers, so now we need to check
            // if there is a log page available, then try a follow up command (if
            // supported), then try request sense for descriptor format data
            ...
            if (ret != SUCCESS && !device->drive_info.passThroughHacks.ataPTHacks
                                       .returnResponseInfoSupported) // rerequest only if the return
                                                                     // response info is not supported
            {
        ...
        // Some devices say passthrough info available, but populate nothing...so
        // need to set this error!
            ...
            ataCmd->rtfr.secCnt == 0 && ataCmd->rtfr.lbaHi == 0 && ataCmd->rtfr.lbaMid == 0 && ataCmd->rtfr.lbaLow == 0)
        {
                ...
                ataCmd->commadProtocol == ATA_PROTOCOL_DMA_FPDMA)
            {
                ...
                // If it succeeds and data came back, these may be empty on some
                // translators. Do not consider this a failure, but success
                                     ...
                                     eReturnValues          senseRet)
{
    ...
    if (ataCommandOptions->ptrSenseData)
    {
        ...
        // attempt to copy back RTFRs from the sense data buffer if we got sense
        // data at all parse the descriptor format sense data
        if (senseDataFormat == SCSI_SENSE_CUR_INFO_DESC || senseDataFormat == SCSI_SENSE_DEFER_ERR_DESC)
        {
        ...
        // Parse the fixed format sense data if it says there is ATA Pass through
        ...
        )
        {
            ...
            // if the RTFRs are incomplete, but the device supports the request
            // command, send the request command to get the RTFRs
        ...
        if (!gotRTFRsFromSenseData && ioRet != OS_PASSTHROUGH_FAILURE)
        {
            ...
            get_Sense_Key_ASC_ASCQ_FRU(ataCommandOptions->ptrSenseData, ataCommandOptions->senseDataSize, &senseKey,
                                       &asc, &ascq, &fru);
            switch (senseKey)
            {
            ...
            case SENSE_KEY_MEDIUM_ERROR:
                ...
                {
                    ataCommandOptions->rtfr.status = ATA_STATUS_BIT_SEEK_COMPLETE | ATA_STATUS_BIT_ERROR;
                    ataCommandOptions->rtfr.error  = ATA_ERROR_BIT_ID_NOT_FOUND; // idnf
                                                                                 // bit
                    gotRTFRsFromSenseData = true;
                }
                break;
            ...
            case SENSE_KEY_NO_ERROR:
                ...
                break;
            default:
                // Don't do anything! We don't have enough information to decide what
                // happened. The SATL be returning it's own error that is not related to
...
/*
 * NOTE: Setting the off-line field of a SAT command is not really necessary in
 * current cases (SATA). It was added as a way to work around some issues
 * between certain combinations of PATA devices and HBAs where there could be an
 * occasional error/glitch where the status register was invalid by reading it
 * too quickly. Setting this field will resolve this problem on these
 * troublesome combinations. None are currently known in this code, but if you
 * happen to read this while debugging a really strange error that might be
 * this, add a special case for setting these bits on the HBA this was detected
 * on.
 */
...
// eReturnValues set_Offline_Bits(uint8_t *satCDB, uint32_t timeout, uint8_t
// transferBitsOffset)
//{
...
eReturnValues set_Registers(uint8_t* satCDB, ataPassthroughCommand* ataCommandOptions)
{
    ...
    {
        // special case for some commands (all 28bit corrected in here. Extended
        // commands do not seem to have this issue) Some commands perform a data
        // transfer, but the sector count was N/A and may be set to zero. Example:
        // Identify, DCO, read/write buffer, security, set max address Other
        // commands may use sector count + lbalow for transfer length. Example:
        // Download microcode or trusted send/receive. These should not fall into
        // here. all read/write userspace commands use zero to do a "maximum"
        // transfer of 256 (28bit) or 65536 (48bit) sectors.

        // The next check is to detect format tracks and write same
        // legacy/retired/obsolete commands. These CANNOT be corrected, but every
        // other command I found can.-TJE Note: old write same uses same opcode as
        // read buffer DMA, so need to check feature register as well and verify the
        // protocol field
    ...
    else if (satCDB[OPERATION_CODE] == ATA_PASS_THROUGH_12)
    {
        ...
        //Currently removed since it isn't working on one device I am able to test
        and poke around with. if (ataCommandOptions->commandType ==
...
eReturnValues request_Return_TFRs_From_Device(tDevice* device, ataReturnTFRs* rtfr)
{
    ...
    eReturnValues rtfrRet = NOT_SUPPORTED; // by default, most devices don't
                                           // actually support this SAT command
    uint8_t* rtfrBuffer =
        C_CAST(uint8_t*, safe_calloc_aligned(14, sizeof(uint8_t),
                                             device->os_info.minimumAlignment)); // this size is the size of the ATA
                                                                                 // pass through descriptor which is
                                                                                 // all that should be returned from
                                                                                 // the SATL with this command
    uint8_t* rtfr_senseData =
    ...
    requestRTFRs = C_CAST(uint8_t*, safe_calloc_aligned(cdbLen, sizeof(uint8_t), device->os_info.minimumAlignment));
    ...
    // set the protocol to Fh (15) to request the return TFRs be returned in that
    // data in buffer.
    set_Protocol_Field(requestRTFRs, ATA_PROTOCOL_RET_INFO, XFER_DATA_IN, protocolOffset);
    // SAT says these bits should be ignored.
    // I'm setting them anyways because I have found a few SATLs that NEED the
    // T_DIR bit at minimum to determine that this is a read since they do not
    // validate the protocol as a read in and of itself.
    requestRTFRs[protocolOffset] |= SAT_T_DIR_DATA_IN;
    requestRTFRs[transferBitsOffset] |= ATA_PT_LEN_TPSIU; // While this may not be necessary, it at least makes
                                                          // the most sense for this protocol and should be
                                                          // ignored by a good SATL
    ...
    // any other return value doesn't matter since this will not affect pass fail
    // of our command. After this we will be dummying up a status anyways
    safe_free_aligned(&rtfrBuffer);
    safe_free_aligned(&rtfr_senseData);
    safe_free_aligned(&requestRTFRs);
    if (VERBOSITY_COMMAND_NAMES <= device->deviceVerbosity)
                            ...
                            ataPassthroughCommand* ataCommandOptions)
{
    ...
    {
        // override whatever came in here so that commands go through
        // successfully....mostly for USB
        ataCommandOptions->ataCommandLengthLocation = ATA_PT_LEN_TPSIU;
    }
    if (ataCommandOptions->forceCDBSize > 0)
    {
        // before proceeding to allocate memory, need to place some checks on some
        // commands or things won't work exactly right. This force is rarely useful
        // other than troubleshooting devices or support for certain features of a
        // given translator.
        ...
        case 12:
            ...
            {
                *satCDB = C_CAST(uint8_t*, safe_calloc_aligned(ataCommandOptions->forceCDBSize, sizeof(uint8_t),
            ...
            break;
        case 16:
            // If aux or ICC are set with this force flag, then it needs to be
            // rejected since it is not possible to issue in 16B CDBs
        ...
        case 32:
            ...
            *satCDB[OPERATION_CODE] = 0x7F; // variable length CDB
            ...
            break;
        default:
            // Do nothing as this is not a valid value and let the rest of the code
    ...
    if (!*satCDB)
    {
        ...
        case ATA_CMD_TYPE_EXTENDED_TASKFILE:
            ...
            {
                // validate that there are no ext registers to set. If there are, then
                // do nothing, otherwise allocate the memory to build the CDB
                if (!(ataCommandOptions->tfr.SectorCount48 || ataCommandOptions->tfr.LbaLow48 ||
                      ataCommandOptions->tfr.LbaMid48 || ataCommandOptions->tfr.LbaHi48 ||
                      ataCommandOptions->tfr.Feature48))
                {
                    // No ext registers are set, so we will issue the command with a 12B
                    // CDB. This is a major hack, but might help some devices get some
                    // more support. 12B CDB
                    *satCDB = C_CAST(
            ...
            if (!*satCDB) // should fall into here if the above check did not work and
                          // allocate the satCDB memory.
            {
                // 16B CDB
                *satCDB = C_CAST(uint8_t*,
                                 safe_calloc_aligned(CDB_LEN_16, sizeof(uint8_t), device->os_info.minimumAlignment));
        ...
        case ATA_CMD_TYPE_COMPLETE_TASKFILE:
            // 32B CDB
            *satCDB =
                C_CAST(uint8_t*, safe_calloc_aligned(CDB_LEN_32, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (!*satCDB)
            {
                return MEMORY_FAILURE;
            }
            *cdbLen = CDB_LEN_32;
            ...
            (*satCDB)[26]      = RESERVED;
            break;
            // TODO: handle hard/soft reset here to generate those CDBs properly as
            // well. For now, they are not supported. - TJE
        default:
            return BAD_PARAMETER;
        }
    }
    ...
    // set offline bits - This is currently disabled and not used. It's a
    // workaround for some strange PATA-HBA interactions. See full comment around
    // this function above set_Offline_Bits(*satCDB, 0, transferBitsOffset); set
    // registers
    ...
    {
        // always set the check condition bit since in this case we won't get RTFRs
        // even if there is an error...Windows low level driver workaround
        set_Check_Condition_Bit(*satCDB, transferBitsOffset);
    }
    else
    {
        // only set this bit on NON-data transfers. Many HBAs don't like this always
        // being set, but are OK with it for NON-data
        switch (ataCommandOptions->commandDirection)
        {
        case XFER_NO_DATA:
            if (!device->drive_info.passThroughHacks.ataPTHacks
                     .disableCheckCondition) // don't set check condition if it will
                                             // cause a problem with the adapter
            {
                set_Check_Condition_Bit(*satCDB, transferBitsOffset);
            }
            break;
        default:
                       ...
                       ataCommandOptions->commadProtocol == ATA_PROTOCOL_DMA_FPDMA))
            {
                ...
                // So set it for all cases EXCEPT PIO-in and FPDMA. These will not
                // return useful information when they pass. If these protocols fail,
                // they WILL return useful completion, so check condition is not
                // necessary.
                set_Check_Condition_Bit(*satCDB, transferBitsOffset);
            }
            break;
...
eReturnValues send_SAT_Passthrough_Command(tDevice* device, ataPassthroughCommand* ataCommandOptions)
{
    ...
    eReturnValues senseRet     = SUCCESS;
    uint8_t*      satCDB       = M_NULLPTR;
    eCDBLen       satCDBLength = 0;
    uint8_t*      senseData    = M_NULLPTR; // only allocate if the pointer in the
                                            // ataCommandOptions is M_NULLPTR
    ...
    if (device->drive_info.passThroughHacks.ataPTHacks
            .alwaysCheckConditionAvailable) // we want to always set this bit. May
                                            // not work on all commands or all
                                            // hbas/bridge chips. We typically
                                            // only set this for Non-Data commands
    {
    ...
    if (ret == SUCCESS)
    {
        ...
        memcpy(scsiIoCtx.cdb, satCDB, C_CAST(size_t, satCDBLength)); // should only ever be a known
                                                                     // positive integer: 12, 16, or 32
        ...
        // clear the last command sense data every single time before we issue any
        // commands
...
#if defined(_DEBUG)
        // This is different for debug because sometimes we need to see if the data
        // buffer actually changed after issuing a command. This was very important
        // for debugging windows issues, which is why I have this ifdef in place for
        // debug builds. - TJE
        if (VERBOSITY_BUFFERS <= device->deviceVerbosity && ataCommandOptions->ptrData != M_NULLPTR)
#else
        // Only print the data buffer being sent when it is a data transfer to the
        // drive (data out command)
        if (VERBOSITY_BUFFERS <= device->deviceVerbosity && ataCommandOptions->ptrData != M_NULLPTR &&
        ...
        get_Sense_Key_ASC_ASCQ_FRU(ataCommandOptions->ptrSenseData, ataCommandOptions->senseDataSize,
                                   &scsiIoCtx.returnStatus.senseKey, &scsiIoCtx.returnStatus.asc,
                                   &scsiIoCtx.returnStatus.ascq, &scsiIoCtx.returnStatus.fru);
        senseRet = check_Sense_Key_ASC_ASCQ_And_FRU(device, scsiIoCtx.returnStatus.senseKey, scsiIoCtx.returnStatus.asc,
                                                    scsiIoCtx.returnStatus.ascq, scsiIoCtx.returnStatus.fru);
        if (senseData != M_NULLPTR && ataCommandOptions->senseDataSize > 0 &&
            senseData != device->drive_info.lastCommandSenseData)
        {
            memcpy(device->drive_info.lastCommandSenseData, ataCommandOptions->ptrSenseData,
                   ataCommandOptions->senseDataSize);
        }
        ...
        if (gotRTFRs)
        {
            ...
            //       The SAT passthorugh probably needs refactoring to fix this better
            //       -TJE
            if ((ataCommandOptions->rtfr.lbaLowExt == 0xFF && ataCommandOptions->rtfr.lbaMidExt == 0xFF &&
                 ataCommandOptions->rtfr.lbaHiExt == 0xFF) ||
        ...
        if (sendIOret != OS_PASSTHROUGH_FAILURE && gotRTFRs) // If we got this error, then there's nothing we can do as
                                                             // far as judging command result since the failure was an
                                                             // OS failure..so we just want to pass this up
        {
            // Determine the command completion status to set the error code.
            // This is written after reviewing SAT, AAM, AST, APT, SATA Specs, and
            // legacy drive specs. if ((ataCommandOptions->commadProtocol ==
            // ATA_PROTOCOL_PIO && ataCommandOptions->commandDirection ==
            // XFER_DATA_IN)
            //     ||
            //     ataCommandOptions->commadProtocol == ATA_PROTOCOL_DMA_FPDMA)
            //{
            //     //If we get here, then the result register bits will be zero if the
            //     command completes successfully.
            //     //If the command failed, then we should see the error bit in the
            //     error register set to 1.
            ...
            {
                // When this bit is set, all other status register bits are
                // invalid/unused.
            ...
            else
            {
                if (ataCommandOptions->rtfr.status & ATA_STATUS_BIT_READY ||
                    ataCommandOptions->rtfr.status == 0) // if drive is ready, or we got all zeros, consider it good
                                                         // for now
                {
                ...
                {
                    // device fault bit means the drive has gone into an unrecoverable
                    // state and is basically dead. Just set failure here
                ...
                // if we have requestATASenseData set, then we will request sense, and
                // set it into the device struct and check it's meaning to set the
                // return status
            ...
            // Windows has a problem where if a command fails, the next command that
            // is sent will return the same stale status even if it was good. We need
            // to flush the command out with a check power more command
            if (device->os_info.osType == OS_WINDOWS && device->drive_info.interface_type == IDE_INTERFACE &&
                ret != SUCCESS && ataCommandOptions->tfr.CommandStatus != ATA_CHECK_POWER_MODE_CMD)
            {
                // On Windows AHCI controller (IDE Interface), for whatever reason, the
                // controller sometimes caches the bad status and will return it on the
                // very next command we issue
                //...SO send a "check Power Mode" command to force it to refresh and
                // prevent this issue from happening again..basically a test unit ready
                // command - TJE
                uint64_t commandTimeNanoseconds =
                    device->drive_info.lastCommandTimeNanoSeconds; // back this up first since they'll
                                                                   // want the time from the command
                                                                   // they issued, not the check power
                                                                   // mode.
                uint8_t powerMode = 0; // we don't actually care about this...just holding it for now
                ata_Check_Power_Mode(device, &powerMode);
                // The rtfrs from this command will get overwritten below so this will
                // appear transparent.
        ...
        {
            ret = senseRet; // We didn't get RTFRs to judge and there wasn't an issue
                            // sending the command through the OS or driver...so go
                            // with what the sense data says
        }
        else
        {
            ret = sendIOret;
    ...
    if (senseRet == NOT_SUPPORTED && ataCommandOptions->commadProtocol == ATA_PROTOCOL_UDMA)
    {
        ...
        get_Sense_Key_ASC_ASCQ_FRU(device->drive_info.lastCommandSenseData, SPC3_SENSE_LEN, &senseKey, &asc, &ascq,
                                   &fru);
        // Checking for illegal request, invalid field in CDB since this is what
        // we've seen reported when UDMA commands are not supported.
        if (senseKey == SENSE_KEY_ILLEGAL_REQUEST && asc == 0x24 && ascq == 0x00)
        {
            // if the sense data says "NOT_SUPPORTED", it's highly likely that the
            // SATL didn't like something in the command. Local testing shows that
            // sometimes a SATL likes the mode set to DMA instead of UDMA, so retry
            ...
            if (ret == SUCCESS)
            {
                // if changing back to DMA worked, then we're changing some flags in the
                // ataOptions struct to make sure we have success with future commands.
                                                            ...
                                                            uint16_t fieldPointer)
{
    if (data)
    {
        ...
        data[4] |= BIT7; // if this function is being called, then this bit is
                         // assumed to be set
        if (cd)
        {
            data[4] |= BIT6;
    ...
    if (descriptorFormat)
    {
        ...
        if (descriptor)
        {
            ...
            while (counter < descriptorCount)
            {
                ...
                {
                    descriptorOffset += 1; // adding 1 since we put the log index in one extra byte, which
                                           // is non standard and not part of the actual descriptor. This
                                           // handles going on to the next descriptor in the list
                                           // properly. We don't return this byte in the descriptor
                                           // anyways
                }
            }
        }
        ...
        if (descriptor)
        {
            ...
            while (counter < descriptorCount)
            {
                ...
                case 9:                   // ATA Status return
                    senseData[0] |= BIT7; // set the valid bit since we are filling in the
                                          // information field
                    ...
                    senseData[8] |=
                        M_Nibble0(descriptor[descriptorOffset + 14]);  // setting the log index...this offset is
                                                                       // nonstandard, so we will need to increment the
                                                                       // offset one before leaving this case
                    ...
                    descriptorOffset += 1; // setting this since we added the log index in the last byte
                                           // of this descriptor to make this easier to pass around
                    break;
...
static void set_Sense_Data_By_RTFRs(tDevice* device, ataReturnTFRs* rtfrs, uint8_t* sensePtr, uint32_t senseDataLength)
{
    ...
    // make sure these are cleared out still (compiler should optimize this away
    ...
    else if (rtfrs->status & BIT0)
    {
        ...
        // wp - write protected? also bit 6....not sure how to differentiate between
        // both cases(ATA/ATAPI7)
    ...
    // We processed the error above according to other RTFRs, but if the sense
    // data available bit is set, then we should request sense data and return
    // that info back up instead of our translated info
            ...
            device->drive_info.IdentifyData.ata.Word120 & BIT6 && rtfrs->status & BIT1))
    {
        ...
        // if everything goes according to plan, we will return success, otherwise
        // restore back up rtfrs and process based on those.
        if (SUCCESS != ata_Request_Sense_Data(device, &senseKey, &asc, &ascq))
        {
            // restore backed up rtfrs
            memcpy(rtfrs, &rtfrBackup, sizeof(ataReturnTFRs));
        }
    }
...
// not using NCQ as this software translation cannot issue NCQ commands
// reliably, so issuing what we can issue here.-TJE
                                       ...
                                       bool       fua)
{
    ...
    else // 28bit command
    {
        ...
        if (scnt == 256)
        {
            // ATA Spec says to transfer this may sectors, you must set the sector
            // count to zero (Not that any passthrough driver will actually allow
            // this)
            scnt = 0;
        }
        if (fua && scsiIoCtx->device->drive_info.IdentifyData.ata.Word085 & BIT5)
...
// To be used by the SATL when issuing a single read-verify command (or read and
// compare data bytes if bytecheck is 1)
static eReturnValues satl_Read_Verify_Command(ScsiIoCtx* scsiIoCtx, uint64_t lba, uint32_t dataSize, uint8_t byteCheck)
{
    ...
    if (scsiIoCtx->device->drive_info.IdentifyData.ata.Word083 & BIT10)
    {
        ...
        else
        {
            ...
            compareBuf = C_CAST(uint8_t*, safe_calloc_aligned(scsiIoCtx->dataLength, sizeof(uint8_t),
            ...
            if (byteCheck == 0x01)
            {
                if (memcmp(compareBuf, scsiIoCtx->pdata, scsiIoCtx->dataLength))
                {
            ...
            else if (byteCheck == 0x03)
            {
                ...
                for (; iter < scsiIoCtx->dataLength; iter += scsiIoCtx->device->drive_info.deviceBlockSize)
                {
                    if (memcmp(&compareBuf[iter], scsiIoCtx->pdata,
                               M_Min(scsiIoCtx->device->drive_info.deviceBlockSize, scsiIoCtx->dataLength)))
                    {
                        errorFound = true;
                        break;
                    }
                                                    ...
                                                    uint32_t   patternLength)
{
    ...
    // eReturnValues satl_Write_Command(ScsiIoCtx *scsiIoCtx, uint64_t lba,
    // uint8_t *ptrData, uint32_t dataSize, bool fua)
...
static bool are_RTFRs_Non_Zero_From_Identify(ataReturnTFRs* identRTFRs)
{
    ...
    if (identRTFRs->device != 0 || identRTFRs->lbaHi != 0 || identRTFRs->lbaMid != 0 || identRTFRs->lbaLow != 0 ||
        identRTFRs->secCnt != 0
        // NOTE: Not checking error or status because those could be dummied up in
        // low level passthrough stuff on some occasions - TJE
    )
    {
        nonZero = true;
    }
...
static eReturnValues translate_ATA_Information_VPD_Page_89h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (safe_strlen(openseaVersionString) < 8)
    {
        ...
        // snprintf(C_CAST(char*, &ataInformation[24]), 8, " %-s",
        // openseaVersionString);
    }
    else
    {
    ...
    ataInformation[35] = 48 + SAT_SPEC_SUPPORTED; // set's the ascii character for which sat level
                                                  // the code is enabled for
#else
    ataInformation[35] = '?';
#endif // SAT_SPEC_SUPPORTED
    // device signature (response from identify command)
    if (is_ATA_Identify_Word_Valid_SATA(device->drive_info.IdentifyData.ata.Word076)) // Only Serial ATA Devices will
                                                                                      // set the bits in words 76-79
    {
    ...
    {
        // In this case, we got the rtfrs from the drive and can use them to set the
        // signature instead of dummying it up
    ...
    else
    {
        if (peripheralDevice == 0 || peripheralDevice == 0x14) // ATA
        {
            ...
            // this is set by what I could find in the SATA spec. I'm assuming if we
            // made it to here in the code, the drive sent a good signature. - TJE
    ...
    memcpy(&ataInformation[60], &device->drive_info.IdentifyData.ata, LEGACY_DRIVE_SEC_SIZE);
    // now copy all the data we set up back to the scsi io ctx
    if (scsiIoCtx->pdata)
    {
        memcpy(scsiIoCtx->pdata, ataInformation, M_Min(572, scsiIoCtx->dataLength));
    }
    return ret;
}
...
static eReturnValues translate_Unit_Serial_Number_VPD_Page_80h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx->pdata)
    {
        memcpy(scsiIoCtx->pdata, unitSerialNumber, M_Min(24, scsiIoCtx->dataLength));
    }
    return ret;
}
...
#define SAT_SCSI_NAME_STRING_LENGTH 21
        ...
        // WWN Supported
        naaDesignator = C_CAST(uint8_t*, safe_calloc(naaDesignatorLength * sizeof(uint8_t), sizeof(uint8_t)));
        if (!naaDesignator)
        {
            return MEMORY_FAILURE;
        }
        ...
        SCSINameStringDesignator =
            C_CAST(uint8_t*, safe_calloc(SCSINameStringDesignatorLength * sizeof(uint8_t), sizeof(uint8_t)));
        ...
        memcpy(&SCSINameStringDesignator[4], scsiNameString, 20);
    }
    ...
    deviceIdentificationPage =
        C_CAST(uint8_t*, safe_calloc((UINT32_C(4) + UINT32_C(72) + C_CAST(uint32_t, naaDesignatorLength) +
    ...
    {
        memcpy(&deviceIdentificationPage[4 + naaDesignatorLength], SCSINameStringDesignator,
               SCSINameStringDesignatorLength);
    }
    // t10 vendor identification last
    memcpy(&deviceIdentificationPage[4 + naaDesignatorLength + SCSINameStringDesignatorLength], t10VendorIdDesignator,
           UINT32_C(72));
    ...
    if (scsiIoCtx->pdata)
    {
        memcpy(scsiIoCtx->pdata, deviceIdentificationPage,
               M_Min(UINT32_C(72) + naaDesignatorLength + SCSINameStringDesignatorLength, scsiIoCtx->dataLength));
    }
...
static eReturnValues translate_Block_Device_Characteristics_VPD_Page_B1h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    case 3: // reserved
        ...
        break;
    default: // not supported or host managed (host managed reported in the
...
{
    eReturnValues ret = SUCCESS;
    // assuming the drive supports the page since identify bit was checked before
    // getting here.-TJE
    DECLARE_ZERO_INIT_ARRAY(uint8_t, powerConditionsLog,
                            1024); // currently, this log is only two pages long and we want to read
                                   // both for the data we're filling in-TJE
                                    ...
                                    device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
    {
        ...
        // standby y
        descriptor = C_CAST(ataPowerConditionsDescriptor*,
                            &powerConditionsLog[384 + LEGACY_DRIVE_SEC_SIZE]); // in second sector
        ...
        // standby z
        descriptor = C_CAST(ataPowerConditionsDescriptor*,
                            &powerConditionsLog[448 + LEGACY_DRIVE_SEC_SIZE]); // in second sector
        if (descriptor->powerConditionFlags & BIT7)
        {
        ...
        {
            memcpy(scsiIoCtx->pdata, powerConditionPage, M_Min(18, scsiIoCtx->dataLength));
        }
    }
    else
    {
...
static eReturnValues translate_Logical_Block_Provisioning_VPD_Page_B2h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word169))
    {
        if (device->drive_info.IdentifyData.ata.Word169 & BIT0)
        {
            ...
            // lbpws bit (set to zero since we don't support unmap during write same
            // yet) logicalBlockProvisioning[5] |= BIT6; lbpws10 bit (set to zero
            // since we don't support unmap during write same yet)
...
static eReturnValues translate_Block_Limits_VPD_Page_B0h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    uint8_t peripheralDevice = 0;
    // maximum transfer length (unspecified....we decide) - set to 65535 or 255
    // depending on if we support 48 bit or not
    uint32_t maxTransferLength = 255;
    ...
    blockLimits[2] = 0x00;
    blockLimits[3] = 0x3C;
    // wsnz bit - leave as zero since we support a value of zero to overwrite the
    // full drive max compare and write length - set to 0
    blockLimits[5] = 0;
    // optimal transfer length granularity (unspecified....we decide) - setting to
    // number of logical per physical sectors
    ...
    // optimal transfer length (unspecified....we decide) - set to 128 for
    // hopefully highest compatibility.
    blockLimits[12] = M_Byte3(128);
    blockLimits[13] = M_Byte2(128);
    blockLimits[14] = M_Byte1(128);
    blockLimits[15] = M_Byte0(128);
    // maximum prefetch length (unspecified....we decide) - leave at zero since we
    // don't support the prefetch command
    ...
    // maximum write same length (unspecified....we decide). We will allow the
    // full drive to be write same'd
...
static eReturnValues translate_Mode_Page_Policy_VPD_Page_87h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (is_ATA_Identify_Word_Valid_SATA(
            device->drive_info.IdentifyData.ata.Word076)) // Only Serial ATA Devices will set the bits in words
                                                          // 76-79. Bit zero should always be set to zero, so the
                                                          // FFFF case won't be an issue
    {
        // pata control
        modePagePolicy[pageOffset + 0] = 0xA0;     // pageCode
        modePagePolicy[pageOffset + 1] = 0xF1;     // subpage code
    ...
    if (scsiIoCtx->pdata)
    {
        memcpy(scsiIoCtx->pdata, modePagePolicy, M_Min(pageOffset, scsiIoCtx->dataLength));
    }
    return ret;
}
...
static eReturnValues translate_Zoned_Block_Device_Characteristics_VPD_Page_B6h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (SUCCESS == ata_Read_Log_Ext(device, ATA_LOG_IDENTIFY_DEVICE_DATA, ATA_ID_DATA_LOG_ZONED_DEVICE_INFORMATION,
                                    ...
                                    device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
    {
        // validate that we got a valid page from the ID Data log (it can return all
        // zeros on this page if it isn't supported, but read successfully)
        ...
        if (scsiIoCtx->pdata)
        {
            memcpy(scsiIoCtx->pdata, zonedDeviceCharacteristics, M_Min(64, scsiIoCtx->dataLength));
        }
    }
    else
    {
        // This data only exists in this log, so if we can't read it, time to return
        // a failure.
        ret = NOT_SUPPORTED;
        set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense,
                                scsiIoCtx->senseDataSize);
...
static eReturnValues translate_Extended_Inquiry_Data_VPD_Page_86h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    // activate microcode may be 0 (not specified) or 01b (activates before
    // completion of final command in write buffer sequence)
    extendedInquiry[4] |= BIT6; // 01b
    // WU_SUP - set to value of ATA Write Uncorrectable command support from
    // identify data.
    if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word086) &&
        device->drive_info.IdentifyData.ata.Word086 & BIT15) /*words 119, 120 valid*/
    {
    ...
    // extended self test completion time minutes to value from SMART Read Data
    // command
        ...
        device->drive_info.IdentifyData.ata.Word084 & BIT1) // smart enabled and self test supported
    {
        ...
        {
            uint16_t extendedSelfTestCompletionTimeMinutes = 0;
    ...
    {
        // DO NOT Set the HRA, or VSA supported bits since the SATL has no way of
        // knowing/doing those activation events!
...
#endif // SAT_SPEC_SUPPORTED
#if defined(SAT_SPEC_SUPPORTED) && SAT_SPEC_SUPPORTED > 2
    // logical block provisioning (only show when we have a drive that supports
    // the TRIM command...otherwise this page has little to no meaning)
    if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word169) &&
...
static eReturnValues translate_SCSI_Inquiry_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (scsiIoCtx->cdb[1] & 0xFE)
    {
        ...
        bitPointer = counter - 1; // because we should always get a count of at
                                  // least 1 if here and bits are zero indexed
...
#endif // SAT_SPEC_SUPPORTED
       // Transport...skipping this one since I'm not sure of what exactly to
       // set (we could try to do parallel vs serial, but we only get to set
       // ATA8-APT or ATA8-AST, which may not be enough)-TJE ATA Version(s)
       // ATA/ATAPI-6 through ACS-4 searching for specs between ATA/ATAPI-6 and
       // ACS-3 since ACS3 is the highest thing with a value at this time.
...
static eReturnValues translate_SCSI_Read_Capacity_Command(tDevice* device, bool readCapacity16, ScsiIoCtx* scsiIoCtx)
{
        ...
        ata_Identify(device, C_CAST(uint8_t*, &device->drive_info.IdentifyData.ata.Word000), LEGACY_DRIVE_SEC_SIZE))
    {
        ...
        {
            cylinder = M_BytesTo2ByteValue(identifyData[3], identifyData[2]); // word
                                                                              // 1
        ...
        if (is_ATA_Identify_Word_Valid(ident_word[53]))
        {
            ...
            {
                // only override if these are non-zero. If all are zero, then we cannot
                // determine the current configuration and should rely on the defaults
                // read earlier. This is being checked again sincea device may set bit0
                // of word 53 meaning this is a valid field. however if the values are
                // zero, we do not want to use them.
                if (M_BytesTo2ByteValue(identifyData[109], identifyData[108]) > 0 && identifyData[110] > 0 &&
                    identifyData[112] > 0)
                {
                    cylinder = M_BytesTo2ByteValue(identifyData[109],
                                                   identifyData[108]); // word 54
        ...
        {
            lbaModeSupported = true; // workaround for some USB devices that do support lbamode as
                                     // can be seen by reading this LBA value
        ...
        // TODO: Zoned info (RC Basis)...need update to SAT 4 before I can fill this
        // in

        // Got the data we wanted, now check for the Identify Device Data Log to get
        // zoned information (add in later when translation becomes available)
    }
    else
    {
        ret = FAILURE;
    ...
    if (ret != FAILURE && scsiIoCtx->pdata)
    {
        ...
        {
            // data length is required by spec so return an error if we don't have at
            // least that length
...
static eReturnValues translate_SCSI_ATA_Passthrough_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    uint8_t               protocol;
    memset(&ataCommand, 0, sizeof(ataPassthroughCommand));
    ...
    {
        // make sure reserved bits (which are multiple count in 16&12 byte) aren't
        // set. That command was made obsolete so don't need it here...let's follow
        // the spec.
    ...
    else
    {
        ...
        if (multipleCount > 0)
        {
            // This can only be non-zero if it's a read or write multiple command
            // operation code!
    ...
    // check bytes 1 and 2 of the CDB for the different BITs to make sure
    // everything get's set up correctly for a lower layer
    ...
    case 15: // return response information
        // just get whatever is in the last command rtfrs of the device, and set
        // that into the data return buffer and return success
    ...
    else if (thirtyTwoByteCommand)
    {
        ...
        // If any of the ICC or AUX registers are set, we need to return an error
        // since we cannot issue that command since OS passthrough doesn't support
        // it
    ...
    // now we need to dummy up sense data if we are on the IDE_INTERFACE (ATA) and
    // it was unsuccessful or the check condition bit was set, otherwise the SATL
    // (USB or SAS) will do this for us
    if ((ret != SUCCESS || scsiIoCtx->cdb[2] & BIT5) && device->drive_info.interface_type == IDE_INTERFACE)
    {
        DECLARE_ZERO_INIT_ARRAY(uint8_t, ataReturnDescriptor,
                                15); // making this 1 byte larger than it needs to be. This is done so
                                     // that the log index can be set for fixed format sense data.
       ...
       // copy the result to the ata-passthrough results log
        ...
        // now copy the rtfr
        memcpy(&device->drive_info.softSATFlags.ataPassthroughResults[device->drive_info.softSATFlags.rtfrIndex - 1],
               &ataCommand.rtfr, sizeof(ataReturnTFRs));
        // set the log index
...
static eReturnValues translate_SCSI_Read_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    case 0x08: // read 6
        ...
        if (transferLength == 0) // read6 a zero is more like ATA to do a larger transfer. Other
                                 // reads, just return success without doing anything.
        {
            transferLength = 256;
        }
        break;
...
static eReturnValues translate_SCSI_Write_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (transferLength == 0)
    {
        // a transfer length of zero means do nothing but validate inputs and is not
        // an error
        return SUCCESS;
    }
    else if (transferLength > 65536)
...
static eReturnValues translate_SCSI_Write_Same_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (((fieldPointer = 1) != 0 && (bitPointer = 7) != 0 && wrprotect != 0) ||
        ...
        ((fieldPointer = 1) != 0 && (bitPointer = 2) != 0 && physicalBlockData) // not supporting physical or logical
                                                                                // block data bits at this time. Can be
                                                                                // implemented according to SAT2 though!
    ...
    {
        // invalid operation code...this should be caught below in the function that
        // calls this
        fieldPointer = 0;
        bitPointer   = 7;
        set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, true, true, bitPointer,
    ...
    else
    {
        ...
        {
            // writing zeros
            // If the zeros ext command is supported, it can be used (if number of
            // logical blocks is not zero)
            ...
            {
                // else if SCT write same, function 01 or 101 (foreground or
                // background...SATL decides)
       ...
       // If SCT - function 02 or 04 (background) 1 or more SCT commands
        ...
        if (useATAWriteCommands)
        {
            ...
            writePattern =
                C_CAST(uint8_t*, safe_calloc_aligned(patternLength, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (writePattern)
            {
                if (!ataWritePatternZeros)
                {
            ...
            {
                // the satl_sequential_write_commands function will handle setting the
                // return sense data if it fails for any reason
            }
        }
    }
    return ret;
    ...
    if (device->drive_info.IdentifyData.ata.Word083 & BIT10)
    {
        ...
        else
        {
            ...
            else if (byteCheck == 0x03)
            {
                ...
                {
                    if (memcmp(&compareBuf[iter], scsiIoCtx->pdata,
                               M_Min(device->drive_info.deviceBlockSize, scsiIoCtx->dataLength)))
                    {
                        errorFound = true;
                        break;
                    }
...
static eReturnValues translate_SCSI_Format_Unit_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    // Other bytes are newer than SBC3 or really old and obsolete (error checking
    // is done top to bottom, left bit to right bit)
    ...
    else
    {
        if (formatData)
        {
            ...
            // bool disablePrimary = scsiIoCtx->pdata[1] & BIT6; //SATL ignores this
            // bit. Commented out so we can use it if we ever need to.
            ...
            if (/*check all combinations that we don't support*/
                ...
                || (formatOptionsValid && (fieldPointer = 1) != 0 && (bitPointer = 4) != 0 &&
                    stopFormat) // Doesn't make sense to support since we cannot
                ...
                ||
                ((fieldPointer = 1) != 0 && (bitPointer = 1) != 0 && vendorSpecific) // We don't have a vendor specific
            ...
            )
            {
                ...
                {
                    // defect list issue detected. Need to set which field is actually in
                    // error
                    ...
                    {
                        // error in defect list data. Need to figure out the offset of the
                        // defect list (if any)
                        if (initializationPattern)
                        {
                ...
                if (bitPointer == 0)
                {
                    ...
                    bitPointer = counter - 1; // because we should always get a count of at least 1
                                              // if here and bits are zero indexed
                }
            ...
            else
            {
                ...
                if (formatOptionsValid)
                {
                    ...
                    if (initializationPattern)
                    {
                        // Set up the initialization pattern information since we were given
                        // one
                    ...
                    if (initializationPattern && // this must be set for us to care about
                    ...
                    else
                    {
                        ...
                        if (performWriteOperation)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    uint32_t writeSectors64K = 65535 / device->drive_info.deviceBlockSize;
                                    // ATA Write commands
                                    uint32_t ataWriteDataLength = writeSectors64K * device->drive_info.deviceBlockSize;
                                    uint8_t* ataWritePattern =
                                        C_CAST(uint8_t*, safe_calloc_aligned(writeSectors64K, sizeof(uint8_t),
                                                                             device->os_info.minimumAlignment));
                                    if (ataWritePattern)
                                    {
                                        if (initializationPatternLength > 0)
                                        {
                                            ...
                                            for (uint32_t copyIter = 0; copyIter < ataWriteDataLength;
                                                 copyIter += device->drive_info.deviceBlockSize)
                                            {
                                                memcpy(&ataWritePattern[copyIter], initializationPatternPtr,
                                                       initializationPatternLength);
                                            }
                                        }
                        ...
                        if (performCertifyOperation)
                        {
                            ...
                            // the SATL will continually try write commands to the sector
                            // until it is fixed or an error other than unrecoverable read
                            // error is returned make sure the initialization pattern is less
                            // than or equal to the logical block length
                            if (initializationPatternLength > device->drive_info.deviceBlockSize)
                            {
                                fieldPointer = initializationPatternOffset + 2;
                                bitPointer   = 7;
                            ...
                            {
                                uint32_t verifySectors64K = 65535 / device->drive_info.deviceBlockSize;
                                bool     deviceFault      = false;
                                for (uint64_t lbaIter = 0; lbaIter < device->drive_info.deviceMaxLba && !deviceFault;
                                     lbaIter += verifySectors64K)
                                {
                                    ...
                                    if (ret != SUCCESS)
                                    {
                                        ...
                                        {
                                            // TODO: add logic to check the RTFRs to see which LBA was
                                            // the first bad one during the verify so that we only
                                            // touch it... Problem with this TODO is if we are talking
                                            // to something that doesn't give back all RTFRs (SAT to
                                            // OS) or is a broken driver, this won't work
                                            uint32_t writeSectors64K = 65536 / device->drive_info.deviceBlockSize;
                                            // ATA Write commands
                                            ...
                                            if (ataWritePattern)
                                            {
                                                ...
                                                {
                                                    unrecoveredReadError = false; // set to false before writing and
                                                                                  // re-verifying. This will get changed
                                                                                  // if it's still an unrecovered error
                                                    if (lbaIter + writeSectors64K > device->drive_info.deviceMaxLba)
                                                    {
                                                        // make sure we don't try to write off the end of
                                                        // the drive!
                                                        writeSectors64K =
                                                    ...
                                                    if (!deviceFault)
                                                    {
                                                        // haven't had a device fault, so try verifying the
                                                        // sector, then continue if it worked,
                                                        ret = satl_Read_Verify_Command(scsiIoCtx, lbaIter,
                                                                                       verifySectors64K, 0);
                                                        if (ret != SUCCESS)
                                                        {
                                                            // if this was an unrecovered error, then we need
                                                            // to exit with bad sense data...otherwise
                                                            // continue onwards
                                                            if (device->drive_info.lastCommandRTFRs.status == 0x51 &&
                                                                device->drive_info.lastCommandRTFRs.error & BIT6)
                                                            {
...
static eReturnValues translate_SCSI_Test_Unit_Ready_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (SUCCESS != ata_Check_Power_Mode(device, &powerMode))
    {
        ...
        {
            // set sense data to hardware error->Logical unit failure | internal
            // target failure
        ...
        {
            // check sanitize status...check if device supports sanitize before doing
            // this?
...
static eReturnValues translate_SCSI_Reassign_Blocks_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    writeData = C_CAST(uint8_t*, safe_calloc_aligned(device->drive_info.deviceBlockSize, sizeof(uint8_t),
                                                     device->os_info.minimumAlignment));
...
static eReturnValues translate_SCSI_Security_Protocol_In_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (securityProtocol == 0xEF) // ATA Security
    {
        ...
        else
        {
            ...
            if (scsiIoCtx->pdata && scsiIoCtx->dataLength > 0)
            {
                memcpy(scsiIoCtx->pdata, ataSecurityInformation, M_Min(16, scsiIoCtx->dataLength));
            }
        }
    }
    else
    {
        if (allocationLength == 0)
        {
            ...
            {
                // allow it to pass in this case without an error so that the protocols
                // can be read and ATA security will be available - TJE
        ...
        else
        {
            if (inc512)
            {
                ...
                {
                    // command failed, but we still want to add ATA Security to the
                    // protocol list - TJE
                ...
                else
                {
                    ...
                    {
                        uint16_t listLength =
                            M_BytesTo2ByteValue(scsiIoCtx->pdata[7],
                                                scsiIoCtx->pdata[6]); // ATA reports this as little endian - TJE
                        // add security protocol EF to the list
                        scsiIoCtx->pdata[8 + listLength] = 0xEF;
                        // now reset the list length
                        listLength += 1;
                    ...
                    else if (securityProtocol == 0 && securityProtocolSpecific == 1 && scsiIoCtx->pdata)
                    {
                        ...
                        uint16_t certLength =
                            M_BytesTo2ByteValue(scsiIoCtx->pdata[3],
                                                scsiIoCtx->pdata[2]); // ATA reports this as little endian - TJE
                    ...
                    else if (securityProtocol == 0 && securityProtocolSpecific == 2 && scsiIoCtx->pdata)
                    {
                        ...
                        // need to byte swap the length of compliance descriptors, then any
                        // compliance descriptors that the drive reports.
                             ...
                             offset += descriptorLength + 8)
                        {
                            ...
                            reservedBytes =
                                M_BytesTo2ByteValue(scsiIoCtx->pdata[offset + 3],
                                                    scsiIoCtx->pdata[offset + 2]); // one table shows this as a
                                                                                   // word, another as bytes - TJE
                            ...
                            case 0x0001: // FIPS 140-2 & FIPS140-3
                                ...
                                // Bytes 16 - 143 are ATA string for hardware version...must be
                                // swapped...up to the end of the buffer! Don't go over! Bytes
                                // 144 - 271 are ATA string for version...must be swapped...up
                                // to the end of the buffer! Don't go over! Bytes 272 - 527 are
                                // ATA string for module name...must be swapped...up to the end
                                // of the buffer! Don't go over!
            ...
            else
            {
                ...
                // allocate memory and pad data....then copy back the amount that was
                // requested
                                                   ...
                                                   paddedLength))
                {
                    ...
                    {
                        uint16_t listLength =
                            M_BytesTo2ByteValue(scsiIoCtx->pdata[6],
                                                scsiIoCtx->pdata[7]); // check that these bbytes are being
                                                                      // interpretted in the correct order!
                        listLength += 1;
                        // add security protocol EF to the list
                        scsiIoCtx->pdata[8 + listLength] = 0xEF;
                        // now reset the list length
                ...
                else
                {
                    ...
                    {
                        // need to byte swap the certificates length
                        uint16_t certLength = M_BytesTo2ByteValue(tempSecurityMemory[3],
                                                                  tempSecurityMemory[2]); // ATA reports this as little
                                                                                          // endian - TJE
                    ...
                    else if (securityProtocol == 0 && securityProtocolSpecific == 2)
                    {
                        ...
                        uint32_t lengthOfComplianceDescriptors =
                            M_BytesTo4ByteValue(tempSecurityMemory[3], tempSecurityMemory[2], tempSecurityMemory[1],
                                                tempSecurityMemory[0]); // ATA reports this as little endian -
                                                                        // TJE
                        ...
                        for (uint32_t offset = 4; offset < (lengthOfComplianceDescriptors + 4) && offset < paddedLength;
                             offset += descriptorLength + 8)
                        {
                            uint16_t reservedBytes;
                            uint16_t descriptorType =
...
static eReturnValues translate_SCSI_Security_Protocol_Out_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (securityProtocol == 0xEF) // ATA Security
    {
        ...
        else // TODO: Check that ATA security is supported before issuing
             // commands???
        {
            DECLARE_ZERO_INIT_ARRAY(uint8_t, ataSecurityCommandBuffer,
                                    LEGACY_DRIVE_SEC_SIZE); // for use in ATA security commands that
                                                            // transfer data
            ...
            case 0x0004: // erase unit
                ...
                ataSecurityWordPtr[16] = M_BytesTo2ByteValue(scsiIoCtx->pdata[32], scsiIoCtx->pdata[33]);
                if (SUCCESS !=
                    ata_Security_Erase_Unit(device, ataSecurityCommandBuffer,
                                            scsiIoCtx->timeout)) // using the timeout from the SCSI Io Ctx to
                                                                 // make it match what the caller expects. This
                                                                 // may cause the command to return before
                                                                 // complete and the OS will then recover by
                                                                 // issuing a hard reset and locking the
                                                                 // drive....CHANGE THIS LATER! - TJE
                {
                    ret = FAILURE;
                    set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense,
                                            scsiIoCtx->senseDataSize);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                memcpy(tempSecurityMemory, scsiIoCtx->pdata, transferLength);
                if (SUCCESS != ata_Trusted_Send(device, device->drive_info.ata_Options.dmaSupported, securityProtocol,
...
static eReturnValues translate_SCSI_Write_Long(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
             ...
             device->drive_info.IdentifyData.ata.Word120 & BIT2)))
    {
        ...
        case WRITE_LONG_10_CMD:
            ...
            break;
        case WRITE_LONG_16_CMD: // skipping checking service action as it should
                                // already be checked by now.-TJE
            if (scsiIoCtx->cdb[1] & BIT7)
            {
                correctionDisabled = true;
            }
...
static eReturnValues translate_SCSI_Sanitize_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
        ...
        device->drive_info.IdentifyData.ata.Word059 & BIT12)
    {
        ...
        bool    immediate     = false; // this is ignored for now since there is no way to
                                       // handle this without multi-threading
        bool     znr                 = false;
        bool     ause                = false;
        ...
        case 0x01: // overwrite
            ...
            else if (!scsiIoCtx->pdata) // if this pointer is invalid set sense data
                                        // saying the cdb list is invalid...which
            ...
            else
            {
                if (device->drive_info.IdentifyData.ata.Word059 & BIT13)
                {
                    ...
                    // validate the parameter data for the number of passes and pattern
                    // length according to SAT
                    ...
                    {
                        numberOfPasses = 0; // this needs to be set to zero to specify 16
                                            // passes as this is how ATA does it.
                    }
                    if (SUCCESS !=
                    ...
                    else if (!immediate)
                    {
                        ...
                        while (device->drive_info.lastCommandRTFRs.secCntExt &
                               BIT6) // this should be bit 14 of the returned sector count
                                     // register, but since we break them up, I'm checking
                                     // bit 6 - TJE
                        {
                            delay_Seconds(15);
                            if (SUCCESS != ata_Sanitize_Status(device, false))
        ...
        case 0x1F: // exit failure mode
            ...
            else
            {
                ...
                else if (!immediate)
                {
                    ...
                    while (device->drive_info.lastCommandRTFRs.secCntExt &
                           BIT6) // this should be bit 14 of the returned sector count
                                 // register, but since we break them up, I'm checking bit
                                 // 6 - TJE
                    {
                        delay_Seconds(5); // this should be enough time in between even for
                                          // an overwrite...not optimal but it'll work.-TJE
...
static eReturnValues translate_SCSI_Read_Buffer_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
            ...
            if (bufferOffset == 0)
            {
                ...
                else
                {
                    if (allocationLength == 0)
                    {
                        ...
                        {
                            memcpy(scsiIoCtx->pdata, readBufferData, LEGACY_DRIVE_SEC_SIZE);
                        }
                    }
                    else
                    {
                        if (scsiIoCtx->pdata)
                        {
                            memcpy(scsiIoCtx->pdata, readBufferData, M_Min(LEGACY_DRIVE_SEC_SIZE, allocationLength));
                        }
                    }
                }
            }
    ...
    case 0x03: // descriptor mode
        ...
        else
        {
            if (bufferID == 0)
            {
                ...
                if (scsiIoCtx->pdata)
                {
                    memcpy(scsiIoCtx->pdata, readBufferDescriptor, M_Min(4, allocationLength));
                }
            }
            else
            {
    ...
    case 0x1C: // error history mode (optional)
            ...
            device->drive_info.softSATFlags.currentInternalStatusLogSupported)
        {
            if (bufferID <= 3)
            {
                DECLARE_ZERO_INIT_ARRAY(uint8_t, gplDirectory, LEGACY_DRIVE_SEC_SIZE);
                // establish error history I_T nexus (do nothing in this software
                // implementation)
                if (bufferID == 1 || bufferID == 3)
                {
                    ...
                    if (SUCCESS != ata_Read_Log_Ext(device, ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG, 0,
                                                    gplDirectory, LEGACY_DRIVE_SEC_SIZE,
                    ...
                    memset(gplDirectory, 0, LEGACY_DRIVE_SEC_SIZE);
                }
                // error history directory
                if (SUCCESS == ata_Read_Log_Ext(device, ATA_LOG_DIRECTORY, 0, gplDirectory, LEGACY_DRIVE_SEC_SIZE,
                                                device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
                {
                    uint32_t currentStatusLogSize =
                        M_BytesTo2ByteValue(gplDirectory[(ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG * 2) + 1],
                                            gplDirectory[(ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG * 2)]) *
                        LEGACY_DRIVE_SEC_SIZE;
                    uint32_t savedStatusLogSize =
                        M_BytesTo2ByteValue(gplDirectory[(ATA_LOG_SAVED_DEVICE_INTERNAL_STATUS_DATA_LOG * 2) + 1],
                                            gplDirectory[(ATA_LOG_SAVED_DEVICE_INTERNAL_STATUS_DATA_LOG * 2)]) *
                        LEGACY_DRIVE_SEC_SIZE;
                    if (SUCCESS != ata_Read_Log_Ext(device, ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG, 0,
                                                    gplDirectory, LEGACY_DRIVE_SEC_SIZE,
                                                    device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
                    ...
                    else
                    {
                        ...
                        bool     savedID        = false;
                        DECLARE_ZERO_INIT_ARRAY(uint8_t, errorHistoryData, LEGACY_DRIVE_SEC_SIZE);
                        uint16_t offset = 32;
                        ...
                        if (scsiIoCtx->pdata)
                        {
                            memcpy(scsiIoCtx->pdata, errorHistoryData, M_Min(offset, allocationLength));
                        }
                    }
                }
                else
            ...
            else if (bufferID >= 0x10 && bufferID <= 0xEF)
            {
                ...
                else
                {
                    ...
                    if (bufferID == 0x10) // current
                    {
                        ...
                        {
                            uint32_t IEEEOUIField;
                            uint16_t dataArea1;
                            uint16_t dataArea2;
                            uint16_t dataArea3;
                            // We need to byte swap a few fields in the very first data sector
                            // because of ATA vs SCSI endianness. Bytes 0 - 3 are reserved on
                            // SCSI, so memset them to zero
                            memset(&scsiIoCtx->pdata[0], 0, 4);
                            // Bytes 4 - 7 are the IEEE OUI, but this shows as a DWORD in ACS,
                            // so it needs swapping to SCSI endianness.
                            ...
                            // Bytes 10 - 11 are data area 2 and are a WORD in ACS, so swap
                            // it.
                            ...
                            // Bytes 12 - 13 are data area 3 and are a WORD in ACS, so swap
                            // it.
                            ...
                            // Bytes 14 - 17 are mentioned only in SPC5, so we will memset
                            // them to zero to prevent issues with future devices.
                            memset(&scsiIoCtx->pdata[14], 0,
                                   369 /*this covers bytes 14 through 382*/); // memsetting all
                                                                              // reserved fields
                                                                              // to make sure
                                                                              // this doesn't
                                                                              // cause problems
            ...
            else if (bufferID == 0xFE)
            {
                // shall clear the error history I_T nexus (Nothing to do here in this
                // software implementation)
            }
            else if (bufferID == 0xFF)
            {
                // clear the error history I_T nexus as described (Nothing to do here in
                // this software implementation) release the current device internal
                // status data using an unspecified method (uhhh......no idea)
            }
            else
            {
                ret = NOT_SUPPORTED;
...
static eReturnValues translate_SCSI_Send_Diagnostic_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (parameterListLength == 0)
    {
        ...
        // NOTE: On some old drives this is not a reliable enough check for smart
        // self test.
        //       on these drives you must also issue the SMART read data command and
        //       check if self-test is reported as supported in the SMART data as
        //       well.-TJE
            ...
            if (smartSelfTestSupported)
            {
                if (smartEnabled)
                {
                    ...
                    case 0: // default self test
                        // return good status (not required to do anything according to the
                        // spec) - TJE
                        break;
...
static eReturnValues translate_SCSI_Request_Sense_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    else
    {
        ...
        {
            bool checkDST = false;
            // check the value of the power mode (TODO: indicate by command vs by
            // timer)
            switch (powerMode)
            {
            case 0x00: // standby_z
            ...
            case 0xFF: // active
            default:
                checkDST = true; // only set this here because the drive will only be
                                 // active when DST is running. It won't be in standby
                                 // or idle conditions.
                // all is well
                ...
                if (SUCCESS == ata_SMART_Read_Data(device, smartData, 512))
                {
                    if (M_Nibble1(smartData[363]) == 0x0F)
                    {
                        ...
                        uint16_t dstProgress =
                            656 * M_Nibble0(smartData[363]); // This comes out very close to the actual
                                                             // percent...close enough anyways. There is
                                                             // probably a way to scale it to more even
                                                             // round numbers but this will do fine.
    ...
    if (checkSMARTStatus)
    {
        ...
        else
        {
            ...
            set_Sense_Data_For_Translation(&senseData[0], SPC3_SENSE_LEN, SENSE_KEY_NOT_READY, 0x08, 0x00,
                                           descriptorFormat, M_NULLPTR,
                                           0); // setting logical unit communication failure since that sounds
                                               // about like what happened-TJE
        }
    }
...
static eReturnValues translate_SCSI_Write_Buffer_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    case 0x02: // Write buffer command
        if (((fieldPointer = 1) != 0 && (bitPointer = 7) != 0 &&
             modeSpecific == 0) // mode specific field is reserved in this command,
                                // so make sure it's zero!
    ...
    case 0x05: // ATA Download Microcode
        if (downloadCommandSupported || device->drive_info.ata_Options.downloadMicrocodeDMASupported)
        {
            ...
            else // these fields are reserved or vendor specific so make sure they
                 // are zerod out
            {
                set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, true, true, bitPointer,
                                                                fieldPointer);
    ...
    case 0x07: // ATA Download Microcode Segmented
            ...
            downloadMode3Supported)
        {
            if (((fieldPointer = 1) != 0 && (bitPointer = 7) != 0 &&
                ...
                &&
                ((fieldPointer = 2) != 0 && (bitPointer = 7) != 0 && bufferID == 0) // buffer ID should be zero since we
                                                                                    // don't support other buffer IDs
            )
            {
                if ((bufferOffset & 0x1FF) == 0 && (parameterListLength & 0x1FF) == 0)
                {
                    if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word234) &&
                        device->drive_info.IdentifyData.ata.Word234 > blockCount) // check minimum transfer size
                    {
                        fieldPointer = 6;
                        bitPointer   = 7;
                        set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, true, true,
                    ...
                    else if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word235) &&
                             device->drive_info.IdentifyData.ata.Word235 < blockCount) // check maximum transfer size
                    {
                        fieldPointer = 6;
                        bitPointer   = 7;
                        set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, true, true,
        ...
        else // mode not supported by drive, so we cannot support this command
             // translation
        {
            fieldPointer = 1;
            bitPointer   = 4;
            ...
            )
            {
                if ((bufferOffset & 0x1FF) == 0 && (parameterListLength & 0x1FF) == 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // need to save that microcode is ready to be activated which can
                            // be reported somewhere else. This is also needed within Sanitize
                            // to create an error condition that the code needs to be
                            // activated first.
                        }
                    }
                }
...
static eReturnValues translate_SCSI_Start_Stop_Unit_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
            ...
            device->drive_info.IdentifyData.ata.Word119 & BIT7))
    {
        ...
        case 0x00: // start valid
            ...
            if (start)
            {
                ...
                else
                {
                    ...
                    randomLba = random_Range_64(0, device->drive_info.deviceMaxLba);
                    if (SUCCESS != ata_Read_Verify_Sectors(device, extCommand, 1, randomLba) &&
                        !immediate) // uhh....the spec  doens't mention what happens if
                                    // this command fails...I'll return an error - TJE
                    {
                        ret = FAILURE;
                        set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense,
                                                scsiIoCtx->senseDataSize);
        ...
        case 0x01: // active
            ...
            break;
        case 0x0A: // Force Idle (check power condition modifier for specific idle
                   // state)
            enable = true;
            M_FALLTHROUGH;
        case 0x02: // Idle (check power condition modifier for specific idle state)
            // first read the power conditions log....yes, do this first as that's
            // what the SAT spec says. We could probably cache this data, but we
                ...
                if (descriptor)
                {
                    if (descriptor->powerConditionFlags & BIT7)
                    {
                        ...
                        if (SUCCESS != ata_EPC_Set_Power_Condition_State(device, 0xFF, enable, false))
                        {
                            ...
                            {
                                return SUCCESS; // should save this error condition to report in
                                                // the request sense command later. - TJE
                            }
                        }
                        if (SUCCESS !=
                    ...
                    else // drive doesn't support this power condition, so modifier is
                         // invalid
                    {
                        fieldPointer = 3;
                        bitPointer   = 3;
            ...
            break;
        case 0x0B: // Force Standby (check power condition modifier for specific
                   // idle state)
            enable = true;
            M_FALLTHROUGH;
        case 0x03: // standby (check power condition modifier for specific idle
                   // state)
            // first read the power conditions log....yes, do this first as that's
            // what the SAT spec says. We could probably cache this data, but we
            // aren't doing that today
                                            ...
                                            device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
            {
                ...
                if (descriptor)
                {
                    ...
                    else // power condition not supported by the drive, so modifier is
                         // bad
                    {
                        fieldPointer = 3;
                        bitPointer   = 3;
        ...
        case 0x07: // LU Control
            if (SUCCESS == ata_Check_Power_Mode(device, &powerMode))
            {
                ...
                case 0xFF:
                    ...
                    break;
                default: // unknown power mode. Cannot say a specific thing in the CDB
                         // is bad
                    ret = NOT_SUPPORTED;
                    set_Sense_Data_For_Translation(
                        scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x24, 0,
...
static eReturnValues translate_Supported_Log_Pages(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    DECLARE_ZERO_INIT_ARRAY(uint8_t, supportedPages,
                            LEGACY_DRIVE_SEC_SIZE); // this should be plenty big for now
    uint16_t offset    = 4;
    uint8_t  increment = 1;
    ...
    if (scsiIoCtx->pdata)
    {
        memcpy(scsiIoCtx->pdata, supportedPages,
               M_Min(scsiIoCtx->dataLength, C_CAST(uint16_t, M_Min(LEGACY_DRIVE_SEC_SIZE, C_CAST(uint16_t, offset)))));
    }
    return ret;
}
    ...
    if (SUCCESS == ata_SMART_Return_Status(device))
    {
            ...
            device->drive_info.IdentifyData.ata.Word206 & BIT0)
        {
            ...
            if (SUCCESS == ata_SMART_Read_Log(device, ATA_SCT_COMMAND_STATUS, sctData, LEGACY_DRIVE_SEC_SIZE))
            {
                ...
                {
                    if (sctData[200] & 0x80) // if the sign bit is set, then the termperature is negative
                                             // since this is a 2's compliment value
                    {
...
static eReturnValues translate_Self_Test_Results_Log_0x10(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    // remaining data comes from the logs...check if GPL is supported since that
    // can get us a larger LBA to return for errors
    if (device->drive_info.ata_Options.generalPurposeLoggingSupported)
    {
        DECLARE_ZERO_INIT_ARRAY(uint8_t, extSelfTestLog,
                                LEGACY_DRIVE_SEC_SIZE); // 2 sectors in size in case we need to read 2
                                                        // pages of the log
        // read log ext (up to 3449 pages, but we only need to read 2 pages at most,
                                        ...
                                        device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
        {
            ...
            if (selfTestIndex != 0)
            {
                uint16_t lastPageRead = 0;
                uint16_t iter         = 4;
                uint32_t ataLogOffset = 4; // point to first descriptor
                // now we need to go through and save the most recent entries to the log
                // we'll return
                for (; parameterCode <= 0x0014; iter += 20, parameterCode++)
                {
                    uint8_t  selfTestCode                 = 0;
                    uint8_t  selfTestResult               = 0;
                    uint8_t  selfTestNumber               = 0;
                    uint16_t accumulatedPowerOnHours      = 0;
                    uint64_t addressOfFirstFailure        = 0;
                    uint8_t  senseKey                     = 0;
                    ...
                    // This translation is a little trickier to translate. We need to do:
                    // selfTestIndex - paramcode + 1 and ONLY if that result is greater
                    // than zero do we use that descriptor value
                    ataDescriptorNumber = C_CAST(int16_t, selfTestIndex - parameterCode + INT16_C(1));
                    if (ataDescriptorNumber > 0)
                    {
                        uint16_t pageNumber;
                        // set the buffer offset from the descriptor number we got above -
                        // we may need to read a different page of the log if it's a
                        // multipage log
                        ...
                        // set the self test code (unspecified - attempting to translate if
                        // possible)
                        ...
                        if (M_Nibble0(extSelfTestLog[ataLogOffset + 1]) !=
                            0x07) // only keep LBA field when we had a read failure.
                                  // Otherwise, it should be set to all F's
                        {
    ...
    else
    {
        ...
        if (SUCCESS == ata_SMART_Read_Log(device, ATA_LOG_SMART_SELF_TEST_LOG, selfTestLog, LEGACY_DRIVE_SEC_SIZE))
        {
            ...
            if (selfTestIndex != 0)
            {
                // valid log data
                uint16_t iter         = 4;
                uint16_t ataLogOffset = 2; // this should put us at the beginning of the descriptor we're
                                           // going to read (SAT spec says, just match the parameter number
                                           // to the self test descriptor number)
                for (; parameterCode <= 0x0014; iter += 20, parameterCode++, ataLogOffset += 24)
                {
                    ...
                    selfTestResults[iter + 5] =
                        selfTestLog[ataLogOffset + 4]; // unspecified - setting to the content of the
                                                       // self test failure check point byte
                    // power on hours
                    accumulatedPowerOnHours =
...
static eReturnValues translate_Read_Error_Counters_Log_0x03(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (reportedUncorrectablesValid || correctiveAlgValid)
    {
        ...
        if (scsiIoCtx->pdata)
        {
            memcpy(scsiIoCtx->pdata, readErrorCountersLog, M_Min(M_Min(20U, offset), scsiIoCtx->dataLength));
        }
    }
...
static eReturnValues translate_Background_Scan_Results_Log_0x15(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (pohvalid)
    {
        ...
        if (scsiIoCtx->pdata)
        {
            memcpy(scsiIoCtx->pdata, backgroundResults, M_Min(M_Min(20U, offset), scsiIoCtx->dataLength));
        }
    }
    else // cannot translate the one translatable field, so set invalid page
         // code
    {
        fieldPointer = 2;
        bitPointer   = 5;
...
static eReturnValues translate_General_Statistics_And_Performance_Log_0x19(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (numReadsValid || numWritesValid || logicalSectorsWrittenValid || logicalSectorsReadValid)
    {
        ...
        if (scsiIoCtx->pdata)
        {
            memcpy(scsiIoCtx->pdata, generalStatisticsAndPerformance,
                   M_Min(M_Min(UINT32_C(72), offset), scsiIoCtx->dataLength));
        }
    }
    else // none of the translatable fields are valid, so say this page is not
         // supported
    {
        fieldPointer = 2;
        bitPointer   = 5;
...
static eReturnValues translate_ATA_Passthrough_Results_Log_Page_16(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    DECLARE_ZERO_INIT_ARRAY(uint8_t, ataPassthroughResults,
                            274); // 15 (number of results) * (14 + 4) (size of pass-through results
                                  // descriptor and parameter header) + 4 (log header)
    uint16_t parameterCode = M_BytesTo2ByteValue(scsiIoCtx->cdb[5], scsiIoCtx->cdb[6]);
    uint16_t offset        = 4;
    ...
    // now we need to go through and save the most recent entries to the log we'll
    // return
    ...
    if (scsiIoCtx->pdata)
    {
        memcpy(scsiIoCtx->pdata, ataPassthroughResults, M_Min(M_Min(274U, offset), scsiIoCtx->dataLength));
    }
    return ret;
}
...
static eReturnValues translate_Application_Client_Log_Sense_0x0F(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    numberOfParametersToReturn =
        C_CAST(uint16_t, (allocationLength - 4) / (4 + 0xFC)); //(4 + 0xFC) is the size of a parameter for the
                                                               // application client. allocation length - 4
                                                               // takes into account the header of the log
    // set the header
    ...
    while (parameterCode <= 0x01FF && offset < allocationLength && parameterCounter < numberOfParametersToReturn)
    {
        ...
        // The parameter headers may be written to the host vendor logs if they've
        // been written...but if not, then we'll need to create the header. It's
        // much simpler to take the parameters and write directly to the logs for a
        // SATL, but this may not happen in this software implementation, so we need
        // to handle that.
        ...
        offsetOnATAPage = C_CAST(uint16_t,
        ...
        // each iteration through the loop will read a different page for the
        // request Read all 16 sectors of the log page we need to, then go through
        // and set up the data to return
        if ((is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word085) &&
        ...
        // need another for loop to go through the ATA log data we just read. Set
        // the number of parameter's we've added.
        for (uint8_t perATAPageCounter = 0; parameterCounter <= numberOfParametersToReturn && perATAPageCounter < 32 &&
             ...
             offsetOnATAPage += 256, ++parameterCounter, offset += 256, ++perATAPageCounter)
        {
            ...
            {
                // Parameter code and control and length are not saved in the log so we
                // need to set them up when returning data to the host
                ...
                memcpy(&scsiIoCtx->pdata[offset + 4], &hostLogData[offsetOnATAPage + 4], 0xFC);
            }
            else
            {
                // simple memcpy is all that's necessary
                memcpy(&scsiIoCtx->pdata[offset], &hostLogData[offsetOnATAPage], 256);
            }
        }
    }
    return ret;
...
static eReturnValues translate_SCSI_Log_Sense_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    {
        // uint16_t allocationLength = M_BytesTo2ByteValue(scsiIoCtx->cdb[7],
        // scsiIoCtx->cdb[8]);
        if (pageControl == LPC_CUMULATIVE_VALUES)
        {
...
#endif                                                                      // SAT_SPEC_SUPPORTED
                    ret = translate_Supported_Log_Pages(device, scsiIoCtx); // Update this page as additional pages of
                                                                            // support are added!
                    break;
                default:
                    fieldPointer = 3;
            ...
            case 0x0F: // application client
                ...
                {
                case 0:
                    // First, make sure GPL or SMART are supported/enabled and then that
                    // the device supports the host-vendor specific logs
                    if ((device->drive_info.ata_Options.generalPurposeLoggingSupported ||
                         (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word085) &&
                          device->drive_info.IdentifyData.ata.Word085 & BIT0)) &&
                                                                  ...
                                                                  uint16_t   totalParameterListLength)
{
    ...
    {
        // Support the "parameterCodeReset" option which will write zeros to all
        // application client parameters. We need to preserve the log parameters in
        // the data (just the parameter header...4 bytes. A simple memset to zero
        // would probably work, but this way would be better for when the data is
        // supposed to be read back
        uint8_t ataLogPageToWrite = 0x90; // SAT only uses pages 90h - 9Fh. 80h - 8Fh are left alone
        for (uint16_t parameterCode = 0; ataLogPageToWrite <= 0x9F && parameterCode <= 0x01FF;
             ++ataLogPageToWrite, ++parameterCode)
        {
            DECLARE_ZERO_INIT_ARRAY(uint8_t, hostLogData,
                                    16 * LEGACY_DRIVE_SEC_SIZE); // this memory should be all zeros, but
                                                                 // doing a memset to be certain
            ...
            {
                // error...we shouldn't be here! This condition will be caught in the
                // translate_scsi_Log_Select function
                break;
            }
        }
    }
    else if (totalParameterListLength > 0) // this must be non-zero to enter here and do anything,
                                           // otherwise it is not an error and nothing happens
    {
        // look at the parameter data. Determine the first parameter that is to be
        // written. SATL is required to preserve any data the host isn't requesting
        // to modify Also verify the page code is correct (length can vary depending
        // on how many parameters we were sent)
        ...
        else if (parameterListLength > 4 && totalParameterListLength > 4)
        {
            ...
            uint8_t  parameterLength     = 0xFC;        // ideally, this won't ever change if the
                                                        // command was formatted correctly.
            // Loop through the parameters and check lengths, parameter codes, and
            // parameter control bytes to make sure they are valid
                 ...
                 parameterDataOffset += C_CAST(uint16_t, parameterLength + UINT16_C(4)))
            {
                ...
                bool parameterDisableSave = ptrData[parameterDataOffset + 2] & BIT6; // obsolete. Likely why this is now
                                                                                     // in byte zero for the whole page
                ...
                // uint8_t thresholdCriteriaMet =
                // M_GETBITRANGE(ptrData[parameterDataOffset + 2], 3, 2);//SATL ignores
                // this field since ETC must be zero
            ...
            // we know the data we were sent is good, so time to start writing it to
            // the correct log page. NOTE: For simplicity, we'll read an ATA Host Log
            // page, modify data in it as necessary, then write it back to keep this
            // fairly simple
            ...
            while (parameterDataOffset < parameterListLength && parameterDataOffset < totalParameterListLength)
            {
                ...
                parameterLength = ptrData[parameterDataOffset + 3];
                hostLogData     = C_CAST(uint8_t*, safe_calloc_aligned(16 * LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t),
                                                                       device->os_info.minimumAlignment));
                if (hostLogData == M_NULLPTR)
                {
                    return MEMORY_FAILURE;
                }
                // The parameter headers may be written to the host vendor logs if
                // they've been written...but if not, then we'll need to create the
                // header. It's much simpler to take the parameters and write directly
                // to the logs for a SATL, but this may not happen in this software
                // implementation, so we need to handle that.
                ...
                offsetOnATAPage = C_CAST(uint16_t,
                                         (parameterCode - (UINT16_C(2) * (ataLogPageToRead & 0x0F))) *
                                             UINT16_C(256)); // this should adjust the offset based on the
                                                             // parameter code and the page we're reading.
                // each iteration through the loop will read a different page for the
                // request Read all 16 sectors of the log page we need to, then go
                // through and set up the data to return
                if (scsiIoCtx->device->drive_info.ata_Options.generalPurposeLoggingSupported) // GPL
                {
                ...
                // need another for loop to go through the ATA log data we just read so
                // that we can modify the data before we write it.
                for (uint8_t perATAPageCounter = C_CAST(uint8_t, offsetOnATAPage / UINT16_C(256));
                     ...
                     offsetOnATAPage += UINT16_C(256),
                             ...
                             ++perATAPageCounter)
                {
                    // first, we need to make sure that we write the correct parameters in
                    ...
                    {
                        // parameters aren't exactly next to each other, so we may have
                        // gotten to another page so break out. The next time through the
                        // outer loop, we'll pick it up
                        break;
                    }
                    // simple memcpy is all that's necessary. We're copying all of the
                    // parameter data (number, control, length, etc) into the ATA log
                    // buffer before we write
...
static eReturnValues translate_SCSI_Log_Select_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    else
    {
        ...
        {
            // SPC3 and earlier the pageCode and Subpage code are reserved in the
            // CDB....But that doesn't matter because SAT2 and higher conform to SPC4
            // and only SAT2+ define this command's translation
            switch (pageCode)
            {
                // TODO: If we get page 0, this means there are 1 or more pages in the
                // parameter data to go through
#if defined(SAT_SPEC_SUPPORTED) && SAT_SPEC_SUPPORTED > 2
            case 0x0F: // application client
                switch (subpageCode)
                {
                case 0:
                    ...
                    else // invalid page code because application client log is not
                         // supported
                    {
                        fieldPointer = 2;
                        bitPointer   = 5;
...
static eReturnValues translate_SCSI_Unmap_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    {
        // process the contents of the parameter data and send some commands to the
        // drive uint16_t unmapDataLength = M_BytesTo2ByteValue(scsiIoCtx->pdata[0],
        // scsiIoCtx->pdata[1]);
        uint16_t unmapBlockDescriptorLength = (M_BytesTo2ByteValue(scsiIoCtx->pdata[2], scsiIoCtx->pdata[3]) / 16) *
                                              16; // this can be set to zero, which is NOT an error. Also, I'm making
                                                  // sure this is a multiple of 16 to avoid partial block
                                                  // descriptors-TJE
        if (unmapBlockDescriptorLength > 0)
        {
            ...
            uint8_t* trimBuffer =
                C_CAST(uint8_t*, safe_calloc_aligned(trimBufferSize * LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t),
                                                     device->os_info.minimumAlignment)); // allocate the max size the
                                                                                         // device supports...we'll fill
            ...
            uint64_t maxUnmapRangePerDescriptor =
                device->drive_info.softSATFlags.dataSetManagementXLSupported ? UINT64_MAX : UINT16_MAX;
#else  // SAT_SPEC_SUPPORTED
            bool useXL = false uint8_t maxDescriptorsPerBlock     = 64;
            uint8_t                    descriptorSize             = 8;
            uint64_t                   maxUnmapRangePerDescriptor = UINT16_MAX;
#endif // SAT_SPEC_SUPPORTED
       // need to check to make sure there weren't any truncated block
       // descriptors before we begin
            ...
            uint64_t numberOfLBAsToTRIM       = 0; // this will be checked later to make sure it isn't greater than
                                                   // what we reported on the VPD pages
            uint16_t numberOfBlockDescriptors = 0; // this will be checked later to make sure it isn't greater than
                                                   // what we reported on the VPD pages
            uint16_t ataTrimOffset = 0;
            ...
            trimBuffer =
                C_CAST(uint8_t*, calloc_aligned(trimBufferSize * LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t),
                                                device->os_info.minimumAlignment)); // allocate the max size the
                                                                                    // device supports...we'll fill in
                 ...
                 unmapBlockDescriptorIter += 16, numberOfBlockDescriptors++)
            {
                ...
                // increment this so we can check if we were given more to do than we
                // support (to be checked after the loop)
                ...
                {
                    // not setting sense key specific information because it's not clear
                    // in this condition what error we should point to
                    ret = FAILURE;
                    set_Sense_Data_For_Translation(
                        scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x26, 0,
                        device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                ...
                // check that we haven't been asked to TRIM more LBAs than we care to
                // support in this code
                ...
                // now that we've done all of our checks so far, start setting up the
                // buffer break this block descriptor into multiple ATA entries if
                // necessary
                while (unmapNumberOfLogicalBlocks > 0)
                {
                    ...
                    // check if the ATA Trim buffer is full...if it is and there are more
                    // or potentially more block descriptors, send the command now
                    if ((ataTrimOffset > (trimBufferSize * LEGACY_DRIVE_SEC_SIZE)) &&
                        ((unmapBlockDescriptorIter + 16) < minBlockDescriptorLength))
                    {
            ...
            if (ret == SUCCESS)
            {
                // send the data set management command with whatever is in the trim
                // buffer at this point (all zeros is safe to send if we do get that)
                if (SUCCESS !=
...
// mode parameter header must be 4 bytes for short format and 8 bytes for long
// format (longHeader set to true) dataBlockDescriptor must be non-null when
// returnDataBlockDescriiptor is true. When non null, it must be 8 bytes for
                                                      ...
                                                      uint16_t   allocationLength)
{
    ...
    uint16_t      pageLength  = 12;        // add onto this depending on the length of the
                                           // header and block descriptors
    uint16_t offset = 0;                   // used later when we start setting data in the buffer since we need to
                                           // account for mode parameter header and DBDs
    ...
    controlPage = C_CAST(uint8_t*, safe_calloc(pageLength, sizeof(uint8_t)));
    if (!controlPage)
    {
        return MEMORY_FAILURE;
    }
    ...
    else
    {
        ...
        {
            // default to d_sense is zero for fixed format data. TODO: should this be
            // a compile time flag?
            controlPage[offset + 2] = 0; // TST = 0, TMF_only = 0, dsense = 0, gltsd = 0, rlec = 0
        }
                                                               ...
                                                               uint16_t   allocationLength)
{
    ...
    uint16_t      pageLength      = 8;         // add onto this depending on the length of the
                                               // header and block descriptors
    uint16_t offset = 0;                       // used later when we start setting data in the buffer since we need to
                                               // account for mode parameter header and DBDs
    uint8_t headerLength    = 4;
    ...
    // now that we know how many bytes we need for this, allocate memory
    pataControlPage = C_CAST(uint8_t*, safe_calloc(pageLength, sizeof(uint8_t)));
    if (!pataControlPage)
    {
        return MEMORY_FAILURE;
    }
    // copy header into place
    memcpy(&pataControlPage[0], modeParameterHeader, headerLength);
    // copy block descriptor if it is to be returned
    if (blockDescLength > 0)
    {
        memcpy(&pataControlPage[headerLength], dataBlockDescriptor, blockDescLength);
    }
    // set the remaining part of the page up
    ...
    if (pageControl == 0x01)
    {
        ...
        // Don't actually change the mode though since this can cause a loss of
        // communication with the OS/driver/or HBA!
    }
    else
    {
        // current saved and default pages will be the same since we don't allow
        // changes... read the current mode and use that as a "max" value. Then set
        // bits for everything below that.
        ...
        // now set up the current transfer mode. FYI, the code is the same for
        // "changeable" values except there are no break statements. But we don't
        // support that here...-TJE
        switch (currentMode)
        {
        case 18: // udma 6
        ...
        case 9: // mwdma 0
            ...
            break;
        default:
            // All other cases are unknown since we cannot report or read them from
            // the device...so don't do anything.
            break;
        }
    }
    // set the mode data length
                                                                    ...
                                                                    uint16_t   allocationLength)
{
    ...
    uint16_t      pageLength     = 32;        // add onto this depending on the length of the
                                              // header and block descriptors
    uint16_t offset = 0;                      // used later when we start setting data in the buffer since we need to
                                              // account for mode parameter header and DBDs
    uint8_t headerLength    = 4;
    ...
    // now that we know how many bytes we need for this, allocate memory
    controlExtPage = C_CAST(uint8_t*, safe_calloc(pageLength, sizeof(uint8_t)));
    if (!controlExtPage)
    {
        return MEMORY_FAILURE;
    }
    ...
    if (pageControl != 0x01) // default, current, & saved...nothing on this page
                             // will be changeable - TJE
    {
                                                             ...
                                                             uint16_t   allocationLength)
{
    ...
    uint16_t      pageLength         = 40;        // add onto this depending on the length of the
                                                  // header and block descriptors
    uint16_t offset = 0; // used later when we start setting data in the buffer since we need to
                         // account for mode parameter header and DBDs
    uint8_t headerLength    = 4;
    ...
    // now that we know how many bytes we need for this, allocate memory
    powerConditionPage = C_CAST(uint8_t*, safe_calloc(pageLength, sizeof(uint8_t)));
    ...
    else
    {
        ...
        {
            powerConditionPage[offset + 3] |= BIT0;
            // TODO: store when the timer is changed by mode select so we can report
            // what it was changed to...for now set all F's
            powerConditionPage[offset + 8]  = 0xFF;
            powerConditionPage[offset + 9]  = 0xFF;
            powerConditionPage[offset + 10] = 0xFF;
            powerConditionPage[offset + 11] = 0xFF;
    ...
    {
        powerConditionPage[offset + 39] = 0x54; // check condition while transitioning out of these states is
                                                // disabled.
    }
    // set the mode data length
    if (longHeader)
                                                                    ...
                                                                    uint16_t   allocationLength)
{
    eReturnValues ret                = SUCCESS;
    uint8_t*      powerConditionPage = M_NULLPTR; // will be allocated later
    uint16_t      pageLength         = 16;        // add onto this depending on the length of the
                                                  // header and block descriptors
    uint16_t offset = 0; // used later when we start setting data in the buffer since we need to
                         // account for mode parameter header and DBDs
    uint8_t headerLength    = 4;
    ...
    if (pageControl == 0x00 || pageControl == 0x3) // current and saved
    {
            ...
            device->drive_info.IdentifyData.ata.Word086 & BIT3)
        {
            ...
            powerConditionPage[offset + 6] =
                M_Byte0(device->drive_info.IdentifyData.ata.Word091); // assuming this is valid since the bit above
                                                                      // was checked
        }
    }
    else if (pageControl == 0x2) // default
    {
        // TODO: how do we handle default? we should probably store what APM was
        // when we started software SAT to know for sure. For now, match the
        // current/saved mode
        if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word086) &&
            device->drive_info.IdentifyData.ata.Word086 & BIT3)
        {
                                                                        ...
                                                                        uint16_t   allocationLength)
{
    ...
    // now that we know how many bytes we need for this, allocate memory
    readWriteErrorRecovery = C_CAST(uint8_t*, safe_calloc(pageLength, sizeof(uint8_t)));
    ...
    if (pageControl != 0x1)                    // default, saved, and current pages. Nothing on this
                                               // page is changable.
    {
                                                      ...
                                                      uint16_t   allocationLength)
{
    ...
    uint16_t      pageLength = 20;        // add onto this depending on the length of the
                                          // header and block descriptors
    uint16_t offset = 0;                  // used later when we start setting data in the buffer since we need to
                                          // account for mode parameter header and DBDs
    uint8_t headerLength    = 4;
    ...
    // now that we know how many bytes we need for this, allocate memory
    caching = C_CAST(uint8_t*, safe_calloc(pageLength, sizeof(uint8_t)));
    if (!caching)
    {
    ...
    else // saved, current, and default. TODO: Handle saving what the drive had
         // when we started talking to it.
    {
        ata_Identify(device, C_CAST(uint8_t*, &device->drive_info.IdentifyData.ata.Word000), LEGACY_DRIVE_SEC_SIZE);
        set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_NO_ERROR, 0, 0,
                                       device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
        if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word085) &&
            device->drive_info.IdentifyData.ata.Word085 & BIT5)
        {
            caching[offset + 2] = BIT2; // ic = 0, abpf = 0, cap = 0, disc = 0, size =
                                        // 0, wce = 1, mf = 0, rcd = 0
        }
        else
        {
            caching[offset + 2] = 0; // ic = 0, abpf = 0, cap = 0, disc = 0, size = 0,
                                     // wce = 0, mf = 0, rcd = 0
        }
        if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word085) &&
            device->drive_info.IdentifyData.ata.Word085 & BIT6)
        {
            caching[offset + 12] = 0; // fsw = 0, lbcss = 0, dra = 0, vendor specific
                                      // (2bits) = 0, sync_prog(2bits) = 0, nv_dis = 0
        }
        else
        {
            caching[offset + 12] = BIT5; // fsw = 0, lbcss = 0, dra = 1, vendor specific (2bits) = 0,
                                         // sync_prog(2bits) = 0, nv_dis = 0
        }
    }
                                                                               ...
                                                                               uint16_t   allocationLength)
{
    ...
    // now that we know how many bytes we need for this, allocate memory
    informationalExceptions = C_CAST(uint8_t*, safe_calloc(pageLength, sizeof(uint8_t)));
    ...
    if (pageControl != 0x1)
    {
        informationalExceptions[offset + 2] = 0; // perf = 0, reserved = 0, ebf = 0 (no background functions), EWAsc =
                                                 // 0 (doesn't report warnings), DExcpt = 0 (device does not disable
                                                 // reporting failure predictions), test = 0, ebackerr = 0, logerr = 0
    ...
    else
    {
        ...
        // MRIE modes other than 6 are allowed, but unspecified. Not going to
        // support them right now - TJE
    }
    // set the mode data length
    if (longHeader)
    {
...
{
    eReturnValues ret                       = SUCCESS;
    bool          returnDataBlockDescriptor = true; // true means return a data block descriptor, false means don't
                                                    // return one
    bool longLBABit = false;                        // true = longlba format, false = standard format for
                                                    // block descriptor
    bool    longHeader  = false;                    // false for mode sense 6, true for mode sense 10
    uint8_t pageControl = (scsiIoCtx->cdb[2] & 0xC0) >> 6; // only current values needs to be
                                                           // supported...anything else is unspecified
    uint8_t  pageCode         = scsiIoCtx->cdb[2] & 0x3F;
    uint8_t  subpageCode      = scsiIoCtx->cdb[3];
    ...
    case 0xA0: // control and control extension and PATA control
        ...
        case 0x01: // control extension
            ...
            break;
#endif             // SAT_SPEC_SUPPORTED
        case 0xF1: // PATA control. Report this information BUT DO NOT ALLOW
                   // CHANGES!
            if (!is_ATA_Identify_Word_Valid_SATA(
                    device->drive_info.IdentifyData.ata.Word076)) // Only Serial ATA Devices will set the bits in
                                                                  // words 76-79. Bit zero should always be set to
                                                       ...
                                                       uint16_t   pageLength)
{
    eReturnValues ret = SUCCESS;
    uint16_t      dataOffset =
        C_CAST(uint16_t, ptrToBeginningOfModePage - scsiIoCtx->pdata); // to be used when setting which field is
                                                                       // invalid in parameter list
    DECLARE_ZERO_INIT_ARRAY(uint8_t, senseKeySpecificDescriptor, 8);
    uint8_t       bitPointer   = 0;
    uint16_t      fieldPointer = 0;
    eReturnValues wceRet       = SUCCESS;
    eReturnValues draRet       = SUCCESS;
    // start checking everything to make sure it looks right before we issue
    // commands
    if (pageLength != 0x12)
    {
                                                       ...
                                                       uint16_t   pageLength)
{
        ...
        ((fieldPointer = 9) != 0 && ptrToBeginningOfModePage[9] != 0xFF))
    {
        ...
        if (fieldPointer == 9)
        {
            --fieldPointer; // this is the same field in this mode page, just
                            // adjusting what we're setting so that we return the
                            // correct information.
        }
        if (fieldPointer == 8)
        {
    ...
    // TODO: make sure they aren't trying to change the extended self test
    // timeout! Ignoring it for now...-TJE
    if (ptrToBeginningOfModePage[2] & BIT2)
    {
                                                               ...
                                                               uint16_t   pageLength)
{
            ...
            device->drive_info.IdentifyData.ata.Word119 & BIT7)) // EPC supported
    {
        if (((fieldPointer = 1) != 0 && (bitPointer = 7) != 0 && pageLength != 0x0026) ||
            ((fieldPointer = 2) != 0 && (bitPointer = 7) != 0 &&
             M_GETBITRANGE(ptrToBeginningOfModePage[2], 7, 6) != 0) // PM_BG_PRECEDENCE
            ...
            || ((fieldPointer = 39) != 0 && (bitPointer = 7) != 0 &&
                M_GETBITRANGE(ptrToBeginningOfModePage[39], 7, 6) != 1) // CCF IDLE
            || ((fieldPointer = 39) != 0 && (bitPointer = 5) != 0 &&
                M_GETBITRANGE(ptrToBeginningOfModePage[39], 5, 4) != 1) // CCF STANDBY
            || ((fieldPointer = 39) != 0 && (bitPointer = 3) != 0 &&
                M_GETBITRANGE(ptrToBeginningOfModePage[39], 3, 2) != 1) // CCF STOPPED
        ...
        else
        {
                                            ...
                                            device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
            {
                ...
                // Now check what modes/timers the request is for and make sure the
                // device allows changes!
                uint16_t timerOffset = 0;
                // idle_a check...changable or not, we need to check if they are trying
                // to request a change as well as if the device supports the change
...
{
    eReturnValues ret = SUCCESS;
    // There are only three pages that we care about changing...Power conditions
    // and caching and control
    ...
    if (scsiIoCtx->cdb[OPERATION_CODE] == 0x15 || scsiIoCtx->cdb[OPERATION_CODE] == 0x55)
    {
        ...
        {
            uint8_t byte1 = scsiIoCtx->cdb[1] & 0x11; // removing PF and SP bits since we can
                                                      // handle those, but not any other bits
            if (((fieldPointer = 1) != 0 && byte1 != 0) ||
                ((fieldPointer = 2) != 0 && (bitPointer = 0) == 0 && scsiIoCtx->cdb[2] != 0) ||
                ((fieldPointer = 3) != 0 && (bitPointer = 0) == 0 && scsiIoCtx->cdb[3] != 0))
            {
    ...
    if (pageFormat)
    {
        ...
        uint8_t  modePage;
        bool     subPageFormat;
        // bool parametersSaveble = scsiIoCtx->pdata[headerLength +
        // blockDescriptorLength] & BIT7;
        uint8_t  subpage;
        uint16_t pageLength;

        if (parameterListLength == 0)
        {
            // Spec says this is not to be considered an error. Since we didn't get
            // any data, there is nothing to do but return good status - TJE
            return SUCCESS;
        }
        bitPointer = 0;
        ...
        if (tenByteCommand)
        {
            // modeDataLength = M_BytesTo2ByteValue(scsiIoCtx->pdata[0],
            // scsiIoCtx->pdata[1]);
            ...
            if ((scsiIoCtx->pdata[3] & 0x7F) != 0) // device specific parameter - WP bit is ignored in SBC. dpofua is
                                                   // reserved.
            {
                fieldPointer = 3;
                if (bitPointer == 0)
        ...
        else
        {
            ...
            if ((scsiIoCtx->pdata[2] & 0x7F) != 0) // device specific parameter - WP bit is ignored in SBC. dpofua is
                                                   // reserved.
            {
                fieldPointer = 2;
                if (bitPointer == 0)
            ...
            if (tenByteCommand)
            {
                if (longLBA)
                {
                         ...
                         scsiIoCtx->pdata[MODE_PARAMETER_HEADER_10_LEN + 11] != 0))
                    {
                        ...
                        bitPointer = counter - 1; // because we should always get a count of at least
                                                  // 1 if here and bits are zero indexed
                        set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, false, true,
                                                                        bitPointer, fieldPointer);
                        set_Sense_Data_For_Translation(
                            scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x26, 0,
        ...
        // time to call the function that handles the changes for the mode page
        // requested...save all this info and pass it in for convenience in that
        // function
        ...
        case 0x1A:
            ...
            case 0: // power conditions
                ...
                break;
#else                  // SAT_SPEC_SUPPORTED
                fieldPointer = C_CAST(uint16_t,
                                      headerLength + blockDescriptorLength); // we don't support page 0 in
                                                                             // this version of SAT
                bitPointer = 5;
                set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, false, true, bitPointer,
                                                                fieldPointer);
...
static eReturnValues translate_SCSI_Zone_Management_In_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if ((allocationLength % 512) != 0)
    {
        ...
        dataBuf =
            C_CAST(uint8_t*, safe_calloc_aligned(dataBufLength, sizeof(uint8_t), device->os_info.minimumAlignment));
        localMemory = true;
    }
    ...
    case ZM_ACTION_REPORT_ZONES:
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // first two bytes are bit fields that translate exactly the
                    // same...leave them alone zone length
...
static eReturnValues translate_SCSI_Zone_Management_Out_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    // uint8_t *dataBuf = M_NULLPTR;
    // uint32_t dataBufLength = 0;
    // uint32_t allocationLength = M_BytesTo4ByteValue(scsiIoCtx->cdb[10],
    // scsiIoCtx->cdb[11], scsiIoCtx->cdb[12], scsiIoCtx->cdb[13]); if
    // (scsiIoCtx->cdb[14] & BIT0)
    //{
    //     allBit = true;
    // }
    ////allocate memory if it's not to a round 512 byte block size
    ...
    //{
    //     dataBufLength = (allocationLength + 511) / 512;
    //     dataBuf = C_CAST(uint8_t*, safe_calloc_aligned(dataBufLength,
    //     sizeof(uint8_t), device->os_info.minimumAlignment)); localMemory =
    //     true;
    // }
    // else if (allocationLength == 0)
    //{
    ...
    //{
    //     //copy the data based on allocation length
    //     memcpy(scsiIoCtx->pdata, dataBuf, M_Min(scsiIoCtx->dataLength,
    //     dataBufLength));
    // }
...
static eReturnValues translate_SCSI_Set_Timestamp_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (parameterListLength > 0 && scsiIoCtx->pdata)
    {
        ...
        memcpy(dataBuf, scsiIoCtx->pdata, M_Min(12, parameterListLength));
...
static eReturnValues translate_SCSI_Report_Timestamp_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    {
        memcpy(scsiIoCtx->pdata, dataBuf, M_Min(12, allocationLength));
    }
    return ret;
}
...
static eReturnValues translate_SCSI_Read_Media_Serial_Number_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
         ...
         device->drive_info.IdentifyData.ata.Word087 & BIT2))
    {
        ...
        memcpy(&mediaSerialNumberPage[4], ataMediaSN, 60);
    }
    if (scsiIoCtx->pdata)
    {
        memcpy(scsiIoCtx->pdata, mediaSerialNumberPage, M_Min(allocationLength, 64));
    }
    return ret;
}
...
{
    eReturnValues ret = SUCCESS;
    *dataLength       = 4;         // add onto this for each of the different commands below,
    ...
    *dataLength               = 4; // add onto this for each of the different commands below,
    ...
    case INQUIRY_CMD:
        ...
        *pdata = C_CAST(uint8_t*, safe_calloc(*dataLength, sizeof(uint8_t)));
        if (!*pdata)
        {
            return MEMORY_FAILURE;
        }
    ...
    case SCSI_FORMAT_UNIT_CMD:
        ...
        *pdata = C_CAST(uint8_t*, safe_calloc(*dataLength, sizeof(uint8_t)));
        ...
        pdata[0][offset + 0] = operationCode;
        pdata[0][offset + 1] = 0x37; // protection information is not supported, complete list is not
                                     // supported, but we are supposed to be able to take 2 different
                                     // defect list formats even though we don't use them - TJE
        pdata[0][offset + 2] = RESERVED;
        pdata[0][offset + 3] = RESERVED;
        pdata[0][offset + 4] = RESERVED;
        pdata[0][offset + 5] = controlByte; // control byte
    ...
    case 0x7F:
        ...
        case 0x1FF0: // ATA Pass-through 32
            ...
            *pdata = C_CAST(uint8_t*, safe_calloc(*dataLength, sizeof(uint8_t)));
            ...
            pdata[0][offset + 27] = 0x00; // ICC - we can't send this in this software
                                          // version. OS doesn't allow it
            pdata[0][offset + 28] = 0x00; // AUX - we can't send this in this software
                                          // version. OS doesn't allow it
            pdata[0][offset + 29] = 0x00; // AUX - we can't send this in this software
                                          // version. OS doesn't allow it
            pdata[0][offset + 30] = 0x00; // AUX - we can't send this in this software
                                          // version. OS doesn't allow it
            pdata[0][offset + 31] = 0x00; // AUX - we can't send this in this software
                                          // version. OS doesn't allow it
            break;
        default:
            commandSupported = false;
        ...
        case 0x0D: // download offsets defer
            if (downloadCommandSupported && device->drive_info.softSATFlags.deferredDownloadSupported)
            {
                ...
                *pdata = C_CAST(uint8_t*, safe_calloc(*dataLength, sizeof(uint8_t)));
                ...
                pdata[0][offset + 0] = operationCode;
                pdata[0][offset + 1] =
                    C_CAST(uint8_t, (serviceAction & UINT16_C(0x001F)) | BIT7); // service action plus mode specific set
                                                                                // for power cycle activation
                pdata[0][offset + 2] = 0;
                pdata[0][offset + 3] = 0x3F;
                pdata[0][offset + 4] = 0xFE;
                pdata[0][offset + 5] = 0x00;
                                                          ...
                                                          uint32_t* dataLength)
{
    ...
    uint32_t      offset             = 4;
    *pdata                           = C_CAST(uint8_t*, safe_calloc(reportAllMaxLength, sizeof(uint8_t)));
    if (!*pdata)
    {
        return MEMORY_FAILURE;
    }
...
static eReturnValues translate_SCSI_Report_Supported_Operation_Codes_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    {
    case 0: // return all op codes (return not supported for now until we get the
    ...
    case 1: // check operation code, service action ignored
        ...
        break;
    case 2: // check operation code and service action (error on commands that
            // don't have service actions)
    ...
    case 3: // case 1 or case 2 (SPC4+)
        ...
        {
            // free this memory since the last function allocated it, but failed, then
            // check if the op/sa combination is supported
...
eReturnValues translate_SCSI_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    /*
    static bool satConfigInitialized = false;//this is static because we only want
    to initialize the struct once! if (!satConfigInitialized)
    {
    //TODO: initialize it with empty data
    memset(&satConfig, 0, sizeof(satPassthroughConfiguration));
    satConfigInitialized = true;
    }
    */
    // if we weren't given a sense data pointer, use the sense data in the device
    // structure
    if (!scsiIoCtx->psense)
    {
    ...
    )
    {
        ...
        set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, true, true, bitPointer,
                                                        fieldPointer);
        // set up a sense key specific information descriptor to say that this bit
        // is not valid
        set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x24,
                                       0x00, device->drive_info.softSATFlags.senseDataDescriptorFormat,
                                       senseKeySpecificDescriptor, 1);
    ...
    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_NO_ERROR, 0, 0,
                                   device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
    // if the ataIdentify data is zero, send an identify at least once so we
    // aren't sending that every time we do a read or write command...inquiry,
    // read capacity will always do one though to get the most recent data
    if (!deviceInfoAvailable)
    {
        ...
        {
            // call fill ata drive info to set up vars inside the device struct which
            // the other commands will use.
            ...
            set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_NO_ERROR, 0, 0,
                                           device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
            ////read identify data
            // if (SUCCESS != ata_Identify(device, C_CAST(uint8_t*,
            ...
            //     &device->drive_info.IdentifyData.ata.Word000),
            //     LEGACY_DRIVE_SEC_SIZE))
            //     {
            //         //set that we are an ATAPI_DEVICE, then this function will just
            //         encapsulate every scsi command into an ATA_PACKET command
    ...
    {
        // TODO: set up an ata packet command and send it to the device to let it
        // handle the scsi command translation NOTE: There are a few things that
        // actually do need translation to an ATAPI:
        //       1) ATA Information VPD page
        //       2) A1h CDB. This could be a Blank command, or a SAT ATA Passthrough
        //       command. Need to do some checking of the fields to figure this out
    ...
    else
    {
        ...
        // All functions within this switch-case should dummy up their own sense
        // data specific to the translation!
        switch (scsiIoCtx->cdb[OPERATION_CODE])
        {
        case INQUIRY_CMD: // mostly identify information, but some log info for some
                          // pages.
...
#if defined(SAT_SPEC_SUPPORTED) && SAT_SPEC_SUPPORTED > 3
        case 0x7F:
            // 32byte CDB...check service action. We're looking for ATA Pass-through
            // 32
            {
                uint16_t serviceAction = M_BytesTo2ByteValue(scsiIoCtx->cdb[8], scsiIoCtx->cdb[9]);
                switch (serviceAction)
                {
                case 0x1FF0: // ATA pass-through 32
                    // we likely won't ever hit this case unless we are purposely doing
                    // software SAT, but we are going to reverse the CDB and create the
                    // ataCommandOptions structure and then issue the command (or just
            ...
            break;
#endif                            // SAT_SPEC_SUPPORTED
        case ATA_PASS_THROUGH_12: // any command (other than asynchronous or resets)
                                  // is allowed
        case ATA_PASS_THROUGH_16:
            // we likely won't ever hit this case unless we are purposely doing
            // software SAT, but we are going to reverse the CDB and create the
            // ataCommandOptions structure and then issue the command (or just issue
            // it depending on the interface)
            ret = translate_SCSI_ATA_Passthrough_Command(device, scsiIoCtx);
            break;
        case SCSI_FORMAT_UNIT_CMD:
        ...
        case LOG_SELECT_CMD:                              // only needs to support the application client page as
                                                          // of SAT4
        ...
        case 0xA3: // check the service action for this one!
            switch (scsiIoCtx->cdb[1] & 0x1F)
            {
            case 0x0C: // report supported op codes <- this is essentially returning
                       // either a massive table of supported commands, or it is
                       // sending back data or an error based off a switch statement
            ...
            break;
#endif                          // SAT_SPEC_SUPPORTED
        case REQUEST_SENSE_CMD: // bunch of different commands...or read the "last
                                // command sense data" and change it from fixed to
                                // descriptor, or the other way around
...
#endif // SAT_SPEC_SUPPORTED
#if defined(SAT_SPEC_SUPPORTED) && SAT_SPEC_SUPPORTED > 1
        case SECURITY_PROTOCOL_IN: // Trusted non-data or trusted receive or ata
                                   // security
            ret = translate_SCSI_Security_Protocol_In_Command(device, scsiIoCtx);
            break;
        case SECURITY_PROTOCOL_OUT: // Trusted non-data or trusted send or ata
                                    // security
        ...
        case UNMAP_CMD: // Data Set management-TRIM
            ...
            else // this op code is invalid when the drive doesn't support this
                 // command - TJE
            {
                invalidOperationCode = true;
            }
