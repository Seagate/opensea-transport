...
//the define below is to switch between different levels of SAT spec support. It is recommended that this is set to the highest version available
//valid values are 1 - 4
//1 = SAT
//2 = SAT2
//3 = SAT3
//4 = SAT4
//This flag is not fully implemented at this time, but should be largely functional. All current SAT translation is according to SAT4 (or as close as possible)
...
eReturnValues get_Return_TFRs_From_Passthrough_Results_Log(tDevice *device, ataReturnTFRs *ataRTFRs, uint16_t parameterCode)
{
    ...
    uint8_t *sense70logBuffer = C_CAST(uint8_t*, safe_calloc_aligned(14 + LOG_PAGE_HEADER_LENGTH, sizeof(uint8_t), device->os_info.minimumAlignment));//allocate a buffer to get the rtfrs in. the size of 12 is ATA Passthrough Descriptor + 4byte log page header
    if (!sense70logBuffer)
    {
    ...
    if (SUCCESS == scsi_Log_Sense_Cmd(device, false, LPC_CUMULATIVE_VALUES, LP_ATA_PASSTHROUGH_RESULTS, 0, parameterCode & 0x0F, sense70logBuffer, 14 + LOG_PAGE_HEADER_LENGTH))
    {
        if ((sense70logBuffer[0] & 0x3F) == LP_ATA_PASSTHROUGH_RESULTS)//check that we got the correct page back
        {
            ...
            if (sense70logBuffer[LOG_PAGE_HEADER_LENGTH] == SAT_DESCRIPTOR_CODE && sense70logBuffer[1 + LOG_PAGE_HEADER_LENGTH] == SAT_ADDT_DESC_LEN)
            {
                //CHECK THESE OFFSETS!
...
eReturnValues get_RTFRs_From_Fixed_Format_Sense_Data(tDevice *device, uint8_t *ptrSenseData, uint32_t senseDataSize, ataPassthroughCommand *ataCmd)
{
        ...
        )
    {
        ...
        {
            ret = WARN_INCOMPLETE_RFTRS;//assume that the following code to get the ext registers fails for some reason or another.
            //ok, so we don't know all of the ext registers, so now we need to check if there is a log page available, then try a follow up command (if supported), then try request sense for descriptor format data
            ...
            if (ret != SUCCESS && !device->drive_info.passThroughHacks.ataPTHacks.returnResponseInfoSupported)//rerequest only if the return response info is not supported
            {
                ...
                uint8_t *descriptorFormatSenseData = C_CAST(uint8_t*, safe_calloc_aligned(SPC3_SENSE_LEN, sizeof(uint8_t), device->os_info.minimumAlignment));
...
eReturnValues request_Return_TFRs_From_Device(tDevice *device, ataReturnTFRs *rtfr)
{
    ...
    uint8_t *rtfrBuffer = C_CAST(uint8_t*, safe_calloc_aligned(14, sizeof(uint8_t), device->os_info.minimumAlignment));//this size is the size of the ATA pass through descriptor which is all that should be returned from the SATL with this command
    uint8_t *rtfr_senseData = C_CAST(uint8_t*, safe_calloc_aligned(SPC3_SENSE_LEN, sizeof(uint8_t), device->os_info.minimumAlignment));
...
eReturnValues build_SAT_CDB(tDevice *device, uint8_t **satCDB, eCDBLen *cdbLen, ataPassthroughCommand *ataCommandOptions)
{
    ...
    if (ataCommandOptions->forceCDBSize > 0)
    {
        ...
        case 12:
            ...
            {
                *satCDB = C_CAST(uint8_t*, safe_calloc_aligned(ataCommandOptions->forceCDBSize, sizeof(uint8_t), device->os_info.minimumAlignment));
                if (!*satCDB)
                {
                    return MEMORY_FAILURE;
                }
    ...
    if (!*satCDB)
    {
        ...
        case ATA_CMD_TYPE_TASKFILE:
            ...
            {
                //12B CDB
                *satCDB = C_CAST(uint8_t*, safe_calloc_aligned(CDB_LEN_12, sizeof(uint8_t), device->os_info.minimumAlignment));
                if (!*satCDB)
                {
                    return MEMORY_FAILURE;
                }
            ...
            {
                //16B CDB
                *satCDB = C_CAST(uint8_t*, safe_calloc_aligned(CDB_LEN_16, sizeof(uint8_t), device->os_info.minimumAlignment));
                if (!*satCDB)
                {
                    return MEMORY_FAILURE;
                }
        ...
        case ATA_CMD_TYPE_COMPLETE_TASKFILE:
            //32B CDB
            *satCDB = C_CAST(uint8_t*, safe_calloc_aligned(CDB_LEN_32, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (!*satCDB)
            {
                return MEMORY_FAILURE;
            }
    ...
    {
        //only set this bit on NON-data transfers. Many HBAs don't like this always being set, but are OK with it for NON-data
...
eReturnValues send_SAT_Passthrough_Command(tDevice *device, ataPassthroughCommand  *ataCommandOptions)
{
    ...
    if (ret == SUCCESS)
    {
        ...
        safe_memcpy(scsiIoCtx.cdb, SCSI_IO_CTX_MAX_CDB_LEN, satCDB, C_CAST(size_t, satCDBLength));//should only ever be a known positive integer: 12, 16, or 32
        ...
        if (VERBOSITY_BUFFERS <= device->deviceVerbosity && ataCommandOptions->ptrData != M_NULLPTR && ataCommandOptions->commandDirection == XFER_DATA_OUT)
#endif
        {
        ...
        if (senseData != M_NULLPTR && ataCommandOptions->senseDataSize > 0 && senseData != device->drive_info.lastCommandSenseData)
        {
            safe_memcpy(device->drive_info.lastCommandSenseData, SPC3_SENSE_LEN, ataCommandOptions->ptrSenseData, ataCommandOptions->senseDataSize);
        }
...
#else
        //Only print the data buffer being sent when it is a data transfer to the drive (data out command)
        if (VERBOSITY_BUFFERS <= device->deviceVerbosity && ataCommandOptions->ptrData != M_NULLPTR && ataCommandOptions->commandDirection == XFER_DATA_IN)
#endif
        {
        ...
        if (sendIOret != OS_PASSTHROUGH_FAILURE && gotRTFRs)//If we got this error, then there's nothing we can do as far as judging command result since the failure was an OS failure..so we just want to pass this up
        {
            ...
            {
                if (ataCommandOptions->rtfr.status & ATA_STATUS_BIT_READY || ataCommandOptions->rtfr.status == 0)//if drive is ready, or we got all zeros, consider it good for now
                {
                ...
                {
                    //device fault bit means the drive has gone into an unrecoverable state and is basically dead. Just set failure here
    ...
    safe_memcpy(&device->drive_info.lastCommandRTFRs, sizeof(ataReturnTFRs), &ataCommandOptions->rtfr, sizeof(ataReturnTFRs));
        ...
        if (descriptor)
        {
            ...
            while (counter < descriptorCount)
            {
                ...
                case 0://information
                {
                    ...
                    descriptorInformation = M_BytesTo8ByteValue(descriptor[descriptorOffset + 4], descriptor[descriptorOffset + 5], descriptor[descriptorOffset + 6], descriptor[descriptorOffset + 7], descriptor[descriptorOffset + 8], descriptor[descriptorOffset + 9], descriptor[descriptorOffset + 10], descriptor[descriptorOffset + 11]);
                ...
                case 13://Direct Access Block Device
                    ...
                    safe_memcpy(&senseData[15], SPC3_SENSE_LEN - 16, &descriptor[descriptorOffset + 4], 3);
                    //fru code
                    senseData[14] = descriptor[descriptorOffset + 7];
                    {
                        ...
                        descriptorCmdInformation = M_BytesTo8ByteValue(descriptor[descriptorOffset + 16], descriptor[descriptorOffset + 17], descriptor[descriptorOffset + 18], descriptor[descriptorOffset + 19], descriptor[descriptorOffset + 20], descriptor[descriptorOffset + 21], descriptor[descriptorOffset + 22], descriptor[descriptorOffset + 23]);
                        if (descriptorCmdInformation > UINT32_MAX)
                        {
                            safe_memset(&senseData[8], SPC3_SENSE_LEN - 8, UINT8_MAX, 4);
                        }
...
static void set_Sense_Data_By_RTFRs(tDevice *device, ataReturnTFRs *rtfrs, uint8_t *sensePtr, uint32_t senseDataLength)
{
    ...
    else if (rtfrs->status & BIT0)
    {
        ...
        {
            uint64_t returnedLBA = M_BytesTo8ByteValue(0, 0, rtfrs->lbaHiExt, rtfrs->lbaMidExt, rtfrs->lbaLowExt, rtfrs->lbaHi, rtfrs->lbaMid, rtfrs->lbaLow);
        ...
        && (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word120) && device->drive_info.IdentifyData.ata.Word120 & BIT6 && rtfrs->status & BIT1))
    {
        ...
        //if everything goes according to plan, we will return success, otherwise restore back up rtfrs and process based on those.
    ...
    {
        set_Sense_Data_For_Translation(sensePtr, senseDataLength, senseKey, asc, ascq, device->drive_info.softSATFlags.senseDataDescriptorFormat, informationSenseDescriptor, 1);
    }
    else
    {
        set_Sense_Data_For_Translation(sensePtr, senseDataLength, senseKey, asc, ascq, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
    }
    return;
}
...
static eReturnValues satl_Read_Command(ScsiIoCtx *scsiIoCtx, uint64_t lba, uint8_t *ptrData, uint32_t dataSize, bool fua)
{
    eReturnValues ret = SUCCESS;
    ...
    if (scsiIoCtx->device->drive_info.IdentifyData.ata.Word083 & BIT10)
    {
        ...
        {
            //return an error
            set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x24, 0x00, scsiIoCtx->device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
            return SUCCESS;
        }
    ...
    //now set sense data
    set_Sense_Data_By_RTFRs(scsiIoCtx->device, &scsiIoCtx->device->drive_info.lastCommandRTFRs, scsiIoCtx->psense, scsiIoCtx->senseDataSize);
    return ret;
}
//To be used by the SATL when issuing a single write command
static eReturnValues satl_Write_Command(ScsiIoCtx *scsiIoCtx, uint64_t lba, uint8_t *ptrData, uint32_t dataSize, bool fua)
{
    eReturnValues ret = SUCCESS;
    bool dmaSupported = false;
    if (scsiIoCtx->device->drive_info.IdentifyData.ata.Word049 & BIT8)//lba mode
...
static eReturnValues satl_Read_Verify_Command(ScsiIoCtx *scsiIoCtx, uint64_t lba, uint32_t dataSize, uint8_t byteCheck)
{
    ...
    if (scsiIoCtx->device->drive_info.IdentifyData.ata.Word083 & BIT10)
    {
        ...
        else
        {
            ...
            compareBuf = C_CAST(uint8_t*, safe_calloc_aligned(scsiIoCtx->dataLength, sizeof(uint8_t), scsiIoCtx->device->os_info.minimumAlignment));
            if (!compareBuf)
            {
                set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x24, 0x00, scsiIoCtx->device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                return MEMORY_FAILURE;
            ...
            else if (byteCheck == 0x03)
            {
                ...
                {
                    if (memcmp(&compareBuf[iter], scsiIoCtx->pdata, M_Min(scsiIoCtx->device->drive_info.deviceBlockSize, scsiIoCtx->dataLength)))
                    {
                        errorFound = true;
                        break;
                    }
            ...
            {
                //set failure
                set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_MISCOMPARE, 0x1D, 0x00, scsiIoCtx->device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                return SUCCESS;
            }
            else
            {
                //set success
                set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_NO_ERROR, 0x00, 0x00, scsiIoCtx->device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                return SUCCESS;
            }
        }
    }
...
static eReturnValues satl_Sequential_Write_Commands(ScsiIoCtx *scsiIoCtx, uint64_t startLba, uint64_t range, uint8_t *pattern, uint32_t patternLength)
{
    eReturnValues ret = SUCCESS;
    //use this function to issue the writes.
    //eReturnValues satl_Write_Command(ScsiIoCtx *scsiIoCtx, uint64_t lba, uint8_t *ptrData, uint32_t dataSize, bool fua)
...
static eReturnValues translate_ATA_Information_VPD_Page_89h(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_NO_ERROR, 0, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
#if defined(SAT_SPEC_SUPPORTED) && SAT_SPEC_SUPPORTED > 3
    ...
    {
        ataInformation[24] = ' ';
        safe_memcpy(&ataInformation[25], SAT_ATA_INFO_VPD_PAGE_LEN_SOFTSATL - 25, openseaVersionString, safe_strlen(openseaVersionString));
    ...
    safe_memcpy(&ataInformation[60], SAT_ATA_INFO_VPD_PAGE_LEN_SOFTSATL - 60, &device->drive_info.IdentifyData.ata, LEGACY_DRIVE_SEC_SIZE);
    //now copy all the data we set up back to the scsi io ctx
    if (scsiIoCtx->pdata)
    {
        safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, ataInformation, M_Min(SAT_ATA_INFO_VPD_PAGE_LEN_SOFTSATL, scsiIoCtx->dataLength));
    }
    return ret;
}
...
static eReturnValues translate_Unit_Serial_Number_VPD_Page_80h(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    safe_memcpy(ataSerialNumber, ATA_IDENTIFY_SN_LENGTH + 1, device->drive_info.IdentifyData.ata.SerNum, SERIAL_NUM_LEN);
    ...
    if (scsiIoCtx->pdata)
    {
        safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, unitSerialNumber, M_Min(SOFT_SATL_UNIT_SN_LEN, scsiIoCtx->dataLength));
    }
    return ret;
}
...
#define SAT_SCSI_NAME_STRING_LENGTH 21
        ...
        if (naaDesignator != M_NULLPTR)
        {
            ...
            SCSINameStringDesignator = C_CAST(uint8_t*, safe_calloc(SCSINameStringDesignatorLength * sizeof(uint8_t), sizeof(uint8_t)));
    ...
    //now set MN
    safe_memcpy(ataModelNumber, ATA_IDENTIFY_MN_LENGTH + 1, device->drive_info.IdentifyData.ata.ModelNum, ATA_IDENTIFY_MN_LENGTH);
    byte_Swap_String_Len(ataModelNumber, ATA_IDENTIFY_MN_LENGTH);
    safe_memcpy(&t10VendorIdDesignator[12], SOFT_SAT_T10_VENDOR_ID_DESIGNATOR_LEN, ataModelNumber, ATA_IDENTIFY_MN_LENGTH);
    //now set SN
    safe_memcpy(ataSerialNumber, ATA_IDENTIFY_SN_LENGTH + 1, device->drive_info.IdentifyData.ata.SerNum, ATA_IDENTIFY_SN_LENGTH);
    byte_Swap_String_Len(ataSerialNumber, ATA_IDENTIFY_SN_LENGTH);
    safe_memcpy(&t10VendorIdDesignator[52], SOFT_SAT_T10_VENDOR_ID_DESIGNATOR_LEN - 52, ataSerialNumber, ATA_IDENTIFY_SN_LENGTH);

    //now setup the device identification page
    size_t devIDPageLen = UINT32_C(4) + SOFT_SAT_T10_VENDOR_ID_DESIGNATOR_LEN + C_CAST(uint32_t, naaDesignatorLength) + C_CAST(uint32_t, SCSINameStringDesignatorLength);
    deviceIdentificationPage = C_CAST(uint8_t*, safe_calloc(devIDPageLen * sizeof(uint8_t), sizeof(uint8_t)));
    ...
    deviceIdentificationPage[2] = M_Byte1(SOFT_SAT_T10_VENDOR_ID_DESIGNATOR_LEN + naaDesignatorLength + SCSINameStringDesignatorLength);
    deviceIdentificationPage[3] = M_Byte0(SOFT_SAT_T10_VENDOR_ID_DESIGNATOR_LEN + naaDesignatorLength + SCSINameStringDesignatorLength);
    //copy naa first
    ...
    {
        safe_memcpy(&deviceIdentificationPage[4 + naaDesignatorLength], devIDPageLen - 4 - naaDesignatorLength, SCSINameStringDesignator, SCSINameStringDesignatorLength);
    }
    //t10 vendor identification last
    safe_memcpy(&deviceIdentificationPage[4 + naaDesignatorLength + SCSINameStringDesignatorLength], devIDPageLen - 4 - naaDesignatorLength - SCSINameStringDesignatorLength, t10VendorIdDesignator, SOFT_SAT_T10_VENDOR_ID_DESIGNATOR_LEN);
    ...
    if (scsiIoCtx->pdata)
    {
        safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, deviceIdentificationPage, M_Min(SOFT_SAT_T10_VENDOR_ID_DESIGNATOR_LEN + naaDesignatorLength + SCSINameStringDesignatorLength, scsiIoCtx->dataLength));
    }
...
static eReturnValues translate_Block_Device_Characteristics_VPD_Page_B1h(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (scsiIoCtx->pdata)
    {
        safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, blockDeviceCharacteriticsPage, M_Min(64, scsiIoCtx->dataLength));
    }
    return ret;
}
...
static eReturnValues translate_Block_Limits_VPD_Page_B0h(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word169) && device->drive_info.IdentifyData.ata.Word169 & BIT0 && device->drive_info.IdentifyData.ata.Word069 & BIT14)
    {
        ...
        maxUnmapRangePerDescriptor = device->drive_info.softSATFlags.dataSetManagementXLSupported ? UINT64_MAX : UINT16_MAX;
        maxLBAsPerUnmap = C_CAST(uint64_t, maxDescriptorsPerBlock) * C_CAST(uint64_t, maxBlocks) * maxUnmapRangePerDescriptor;
...
static eReturnValues translate_Zoned_Block_Device_Characteristics_VPD_Page_B6h(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (SUCCESS == ata_Read_Log_Ext(device, ATA_LOG_IDENTIFY_DEVICE_DATA, ATA_ID_DATA_LOG_ZONED_DEVICE_INFORMATION, zonedDeviceInformation, LEGACY_DRIVE_SEC_SIZE, device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
    {
        ...
        if (zonedQword & ATA_ID_DATA_QWORD_VALID_BIT && pageNumber == ATA_ID_DATA_LOG_ZONED_DEVICE_INFORMATION)
        {
            ...
            zonedDeviceCharacteristics[12] = M_Byte3(optimalNumberOfNonSequentiallyWrittenSequentialWritePreferredZones);
            zonedDeviceCharacteristics[13] = M_Byte2(optimalNumberOfNonSequentiallyWrittenSequentialWritePreferredZones);
            zonedDeviceCharacteristics[14] = M_Byte1(optimalNumberOfNonSequentiallyWrittenSequentialWritePreferredZones);
            zonedDeviceCharacteristics[15] = M_Byte0(optimalNumberOfNonSequentiallyWrittenSequentialWritePreferredZones);
        ...
        if (scsiIoCtx->pdata)
        {
            safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, zonedDeviceCharacteristics, M_Min(64, scsiIoCtx->dataLength));
        }
    }
    else
    {
    ...
    else
    {
        ...
        else
        {
            ...
            if (SUCCESS != ata_Identify(device, C_CAST(uint8_t*, &device->drive_info.IdentifyData.ata.Word000), LEGACY_DRIVE_SEC_SIZE))
            {
                ...
                if (SUCCESS != ata_Identify_Packet_Device(device, C_CAST(uint8_t*, &device->drive_info.IdentifyData.ata.Word000), LEGACY_DRIVE_SEC_SIZE))
                {
                    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_NOT_READY, 0x04, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                    return FAILURE;
                }
                peripheralDevice = 0x05;
            }
            ...
            safe_memcpy(&inquiryData[36], INQ_RETURN_DATA_LENGTH - 36, ataSN, M_Min(safe_strlen(ataSN), ATA_IDENTIFY_SN_LENGTH));
            ...
            if (device->drive_info.zonedType == ZONED_TYPE_HOST_AWARE || device->drive_info.zonedType == ZONED_TYPE_DEVICE_MANAGED)
            {
                inquiryData[versionOffset] = 0x06;
                inquiryData[versionOffset + 1] = 0x20;
                versionOffset += 2;
            ...
            if (scsiIoCtx->pdata && scsiIoCtx->dataLength > 0)
            {
                safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, inquiryData, M_Min(INQ_RETURN_DATA_LENGTH, scsiIoCtx->dataLength));
            }
        }
    }
    return ret;
...
static eReturnValues translate_SCSI_Read_Capacity_Command(tDevice *device, bool readCapacity16, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (ret != FAILURE && scsiIoCtx->pdata)
    {
        ...
        if (readCapacity16)
        {
            ...
            if (scsiIoCtx->pdata)
            {
                safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, readCapacityData, M_Min(32, scsiIoCtx->dataLength));
            }
        }
        else
        {
...
static eReturnValues translate_SCSI_ATA_Passthrough_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    //check bytes 1 and 2 of the CDB for the different BITs to make sure everything get's set up correctly for a lower layer
    ...
    case 15://return response information
        ...
        if (device->drive_info.interface_type == IDE_INTERFACE)
        {
            ...
            if (device->drive_info.lastCommandRTFRs.secCntExt || device->drive_info.lastCommandRTFRs.lbaLowExt || device->drive_info.lastCommandRTFRs.lbaMidExt || device->drive_info.lastCommandRTFRs.lbaHiExt)
            {
                //set the extend bit
                response[2] |= BIT0;
            ...
            if (scsiIoCtx->pdata)
            {
                safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, response, M_Min(scsiIoCtx->dataLength, sizeof(response) / sizeof(*response)));
            }
    ...
    else if (thirtyTwoByteCommand)
    {
        ...
        //If any of the ICC or AUX registers are set, we need to return an error since we cannot issue that command since OS passthrough doesn't support it
        if ((fieldPointer = 27 && ataCommand.tfr.icc)
            || (fieldPointer = 28 && ataCommand.tfr.aux4)
            || (fieldPointer = 29 && ataCommand.tfr.aux3)
            || (fieldPointer = 30 && ataCommand.tfr.aux2)
            || (fieldPointer = 31 && ataCommand.tfr.aux1)
            || (fieldPointer = 26 && scsiIoCtx->cdb[26])
            || (fieldPointer = 12 && scsiIoCtx->cdb[12])
            || (fieldPointer = 13 && scsiIoCtx->cdb[13])
            )
        {
            bitPointer = 7;
    ...
    if ((ret != SUCCESS || scsiIoCtx->cdb[2] & BIT5) && device->drive_info.interface_type == IDE_INTERFACE)
    {
        ...
        //now copy the rtfr
        safe_memcpy(&device->drive_info.softSATFlags.ataPassthroughResults[device->drive_info.softSATFlags.rtfrIndex - 1], sizeof(ataReturnTFRs), &ataCommand.rtfr, sizeof(ataReturnTFRs));
        //set the log index
...
static eReturnValues translate_SCSI_Read_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    case 0x28://read 10
        ...
        if (((fieldPointer = 1) != 0 && (bitPointer = 0) == 0 && scsiIoCtx->cdb[1] & BIT0)//reladr bit. Obsolete.
            //|| ((fieldPointer = 1) != 0 && (bitPointer = 1) != 0 && scsiIoCtx->cdb[1] & BIT1)//FUA_NV bit. Can be ignored by SATLs or implemented
            || ((fieldPointer = 1) != 0 && (bitPointer = 2) != 0 && scsiIoCtx->cdb[1] & BIT2)//cannot support RACR bit in this translation since we cannot do fpdma
...
static eReturnValues translate_SCSI_Write_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    default:
        ...
        bitPointer = 7;
        set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, true, true, bitPointer, fieldPointer);
        set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x20, 0x00, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
        return BAD_PARAMETER;
    }
    if (invalidField)
    {
...
static eReturnValues translate_SCSI_Write_And_Verify_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (device->drive_info.IdentifyData.ata.Word083 & BIT10)
    {
        ...
        else
        {
            ...
            compareBuf = C_CAST(uint8_t*, safe_calloc_aligned(scsiIoCtx->dataLength, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (!compareBuf)
            {
                set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x24, 0x00, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                return MEMORY_FAILURE;
            }
            if (dmaSupported)
            {
            ...
            else if (byteCheck == 0x03)
            {
                ...
                {
                    if (memcmp(&compareBuf[iter], scsiIoCtx->pdata, M_Min(device->drive_info.deviceBlockSize, scsiIoCtx->dataLength)))
                    {
                        errorFound = true;
                        break;
                    }
            ...
            {
                //set failure
                set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_MISCOMPARE, 0x1D, 0x00, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                return SUCCESS;
            }
            else
            {
                //set success
                set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_NO_ERROR, 0x00, 0x00, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                return SUCCESS;
            }
        }
    }
                ...
                if (formatOptionsValid)
                {
                    ...
                    else
                    {
                        ...
                        if (performWriteOperation)
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    uint8_t *ataWritePattern = C_CAST(uint8_t*, safe_calloc_aligned(ataWriteDataLength, sizeof(uint8_t), device->os_info.minimumAlignment));
                                    if (ataWritePattern)
                                    {
                                        if (initializationPatternLength > 0)
                                        {
                                            ...
                                            {
                                                safe_memcpy(&ataWritePattern[copyIter], ataWriteDataLength - (copyIter * device->drive_info.deviceBlockSize), initializationPatternPtr, initializationPatternLength);
                                            }
                                        }

                                        ret = satl_Sequential_Write_Commands(scsiIoCtx, 0, device->drive_info.deviceMaxLba, ataWritePattern, ataWriteDataLength);
                                        if (ret != SUCCESS)
                                        {
                                            set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_MEDIUM_ERROR, 0x03, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                                        }
                        ...
                        if (performCertifyOperation)
                        {
                            ...
                            //the SATL will continually try write commands to the sector until it is fixed or an error other than unrecoverable read error is returned
                            //make sure the initialization pattern is less than or equal to the logical block length
                            if (initializationPatternLength > device->drive_info.deviceBlockSize)
                            {
                                fieldPointer = initializationPatternOffset + 2;
                            ...
                            else
                            {
                                ...
                                for (uint64_t lbaIter = 0; lbaIter < device->drive_info.deviceMaxLba && !deviceFault; lbaIter += verifySectors64K)
                                {
                                    ...
                                    if (ret != SUCCESS)
                                    {
                                        //if this was an unrecovered error, then we need to exit with bad sense data...otherwise continue onwards
                                        if (device->drive_info.lastCommandRTFRs.status == 0x51 && device->drive_info.lastCommandRTFRs.error & BIT6)
                                        {
                                            //TODO: add logic to check the RTFRs to see which LBA was the first bad one during the verify so that we only touch it...
                                            //Problem with this TODO is if we are talking to something that doesn't give back all RTFRs (SAT to OS) or is a broken driver, this won't work
                                            ...
                                            if (ataWritePattern)
                                            {
                                                ...
                                                {
                                                    unrecoveredReadError = false;//set to false before writing and re-verifying. This will get changed if it's still an unrecovered error
                                                    ...
                                                    if (!deviceFault)
                                                    {
                                                        //haven't had a device fault, so try verifying the sector, then continue if it worked, 
                                                        ret = satl_Read_Verify_Command(scsiIoCtx, lbaIter, verifySectors64K, 0);
                                                        if (ret != SUCCESS)
                                                        {
                                                            //if this was an unrecovered error, then we need to exit with bad sense data...otherwise continue onwards
                                ...
                                if (deviceFault || ret != SUCCESS)
                                {
                                    ...
                                    ret = NOT_SUPPORTED;
                                    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_MEDIUM_ERROR, 0x14, 0x01, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                                }
                            }
                        }
                    }
...
static eReturnValues translate_SCSI_Test_Unit_Ready_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (SUCCESS != ata_Check_Power_Mode(device, &powerMode))
    {
        ...
        if (device->drive_info.lastCommandRTFRs.status & ATA_STATUS_BIT_DEVICE_FAULT)
        {
            ...
            set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_HARDWARE_ERROR, 0x44, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
        }
        else
        {
            ...
            if (SUCCESS == ata_Sanitize_Status(device, false))
            {
                ...
                else if (device->drive_info.lastCommandRTFRs.secCntExt & BIT15)
                {
                    ...
                    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, 0, 0, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                }
                else
                {
                    //sanitize completed with error
                    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_MEDIUM_ERROR, 0x31, 0x03, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                }
            }
            else
            {
                set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_NOT_READY, 0x05, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
            }
        }
    }
    else
...
static eReturnValues translate_SCSI_Reassign_Blocks_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    for (iterator = 4; iterator < scsiIoCtx->dataLength && iterator < (reassignLBALength + 4); iterator += incrementAmount)
    {
        uint64_t currentLBA = 0;
        if (longLba)
        {
            currentLBA = M_BytesTo8ByteValue(scsiIoCtx->pdata[iterator + 0], scsiIoCtx->pdata[iterator + 1], scsiIoCtx->pdata[iterator + 2], scsiIoCtx->pdata[iterator + 3], scsiIoCtx->pdata[iterator + 4], scsiIoCtx->pdata[iterator + 5], scsiIoCtx->pdata[iterator + 6], scsiIoCtx->pdata[iterator + 7]);
        }
        else
        {
            currentLBA = M_BytesTo4ByteValue(scsiIoCtx->pdata[iterator + 0], scsiIoCtx->pdata[iterator + 1], scsiIoCtx->pdata[iterator + 2], scsiIoCtx->pdata[iterator + 3]);
        }
        if (currentLBA > device->drive_info.deviceMaxLba)
        {
            ret = FAILURE;
            //set LBA out of range
            set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x21, 0x00, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
            break;
        }
        if (SUCCESS != ata_Read_Verify_Sectors(device, extCommand, 1, currentLBA))
        {
            ...
            {
                ret = FAILURE;
                set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_MEDIUM_ERROR, 0x11, 0x04, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                break;
            }
            else
            {
...
static eReturnValues translate_SCSI_Security_Protocol_In_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (securityProtocol == 0xEF)//ATA Security
    {
        ...
        else
        {
            ...
            if (scsiIoCtx->pdata && scsiIoCtx->dataLength > 0)
            {
                safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, ataSecurityInformation, M_Min(16, scsiIoCtx->dataLength));
            }
        }
    }
    else
    {
        if (allocationLength == 0)
        {
            ...
            {
                //allow it to pass in this case without an error so that the protocols can be read and ATA security will be available - TJE
        ...
        else
        {
            if (inc512)
            {
                if (SUCCESS != ata_Trusted_Receive(device, device->drive_info.ata_Options.dmaSupported, securityProtocol, securityProtocolSpecific, scsiIoCtx->pdata, scsiIoCtx->dataLength))
                {
                ...
                else
                {
                    if (securityProtocol == 0 && securityProtocolSpecific == 0 && scsiIoCtx->pdata)
                    {
                        uint16_t listLength = M_BytesTo2ByteValue(scsiIoCtx->pdata[7], scsiIoCtx->pdata[6]);//ATA reports this as little endian - TJE
                        //add security protocol EF to the list
                        scsiIoCtx->pdata[8 + listLength] = 0xEF;
                        //now reset the list length
                        listLength += 1;
                    ...
                    else if (securityProtocol == 0 && securityProtocolSpecific == 1 && scsiIoCtx->pdata)
                    {
                        ...
                        uint16_t certLength = M_BytesTo2ByteValue(scsiIoCtx->pdata[3], scsiIoCtx->pdata[2]);//ATA reports this as little endian - TJE
                    ...
                    else if (securityProtocol == 0 && securityProtocolSpecific == 2 && scsiIoCtx->pdata)
                    {
                        ...
                        for (uint32_t offset = 4; offset < (lengthOfComplianceDescriptors + 4) && offset < scsiIoCtx->dataLength; offset += descriptorLength + 8)
                        {
                            ...
                            reservedBytes = M_BytesTo2ByteValue(scsiIoCtx->pdata[offset + 3], scsiIoCtx->pdata[offset + 2]);//one table shows this as a word, another as bytes - TJE
            ...
            else
            {
                ...
                uint8_t *tempSecurityMemory = C_CAST(uint8_t*, safe_calloc_aligned(paddedLength, sizeof(uint8_t), device->os_info.minimumAlignment));
                if (!tempSecurityMemory)
                {
                    return MEMORY_FAILURE;
                }
                if (SUCCESS != ata_Trusted_Receive(device, device->drive_info.ata_Options.dmaSupported, securityProtocol, securityProtocolSpecific, tempSecurityMemory, paddedLength))
                {
                    //command failed but we still want to add ata security to the protocol list! - TJE
                    if (securityProtocol == 0 && securityProtocolSpecific == 0 && scsiIoCtx->pdata)
                    {
                        uint16_t listLength = M_BytesTo2ByteValue(scsiIoCtx->pdata[6], scsiIoCtx->pdata[7]);//check that these bbytes are being interpretted in the correct order!
                        listLength += 1;
                        //add security protocol EF to the list
                        scsiIoCtx->pdata[8 + listLength] = 0xEF;
                        //now reset the list length
                ...
                else
                {
                    ...
                    {
                        //need to byte swap the certificates length
                        uint16_t certLength = M_BytesTo2ByteValue(tempSecurityMemory[3], tempSecurityMemory[2]);//ATA reports this as little endian - TJE
                    ...
                    else if (securityProtocol == 0 && securityProtocolSpecific == 2)
                    {
                        ...
                        for (uint32_t offset = 4; offset < (lengthOfComplianceDescriptors + 4) && offset < paddedLength; offset += descriptorLength + 8)
                        {
                            ...
                            reservedBytes = M_BytesTo2ByteValue(tempSecurityMemory[offset + 3], tempSecurityMemory[offset + 2]);//one table shows this as a word, another as bytes - TJE
...
static eReturnValues translate_SCSI_Security_Protocol_Out_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (securityProtocol == 0xEF)//ATA Security
    {
        if (inc512 || (transferLength < 34 && (securityProtocolSpecific != 0x0003 && securityProtocolSpecific != 0x0005)))
        {
            //error
            bitPointer = 7;
            fieldPointer = 4;
        ...
        else //TODO: Check that ATA security is supported before issuing commands???
        {
            ...
            case 0x0004://erase unit
                ...
                ataSecurityWordPtr[15] = M_BytesTo2ByteValue(scsiIoCtx->pdata[30], scsiIoCtx->pdata[31]);
                ataSecurityWordPtr[16] = M_BytesTo2ByteValue(scsiIoCtx->pdata[32], scsiIoCtx->pdata[33]);
                if (SUCCESS != ata_Security_Erase_Unit(device, ataSecurityCommandBuffer, scsiIoCtx->timeout))//using the timeout from the SCSI Io Ctx to make it match what the caller expects. This may cause the command to return before complete and the OS will then recover by issuing a hard reset and locking the drive....CHANGE THIS LATER! - TJE
                {
                    ret = FAILURE;
                    set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense, scsiIoCtx->senseDataSize);
                }
    ...
    else
    {
        ...
        else
        {
            if (inc512)
            {
                if (SUCCESS != ata_Trusted_Send(device, device->drive_info.ata_Options.dmaSupported, securityProtocol, securityProtocolSpecific, scsiIoCtx->pdata, scsiIoCtx->dataLength))
                {
                    ret = FAILURE;
                    set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense, scsiIoCtx->senseDataSize);
                }
            ...
            else
            {
                ...
                if (SUCCESS != ata_Trusted_Send(device, device->drive_info.ata_Options.dmaSupported, securityProtocol, securityProtocolSpecific, tempSecurityMemory, paddedLength))
                {
                    ret = FAILURE;
                    set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense, scsiIoCtx->senseDataSize);
                }
            ...
            else
            {
                if (device->drive_info.IdentifyData.ata.Word059 & BIT13)
                {
                    ...
                    else if (!immediate)
                    {
                        ...
                        while (device->drive_info.lastCommandRTFRs.secCntExt & BIT6)//this should be bit 14 of the returned sector count register, but since we break them up, I'm checking bit 6 - TJE
                        {
                            delay_Seconds(15);
                            if (SUCCESS != ata_Sanitize_Status(device, false))
                            {
        ...
        case 0x1F://exit failure mode
            ...
            else
            {
                ...
                else if (!immediate)
                {
                    ...
                    while (device->drive_info.lastCommandRTFRs.secCntExt & BIT6)//this should be bit 14 of the returned sector count register, but since we break them up, I'm checking bit 6 - TJE
                    {
                        delay_Seconds(5);//this should be enough time in between even for an overwrite...not optimal but it'll work.-TJE
    ...
    case 0x02://data mode
        if (bufferID == 0)
        {
            if (bufferOffset == 0)
            {
                ...
                if (SUCCESS != ata_Read_Buffer(device, readBufferData, device->drive_info.ata_Options.readBufferDMASupported))
                {
                    ret = FAILURE;
                    set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense, scsiIoCtx->senseDataSize);
                }
                else
                {
                    ...
                    else
                    {
                        if (scsiIoCtx->pdata)
                        {
                            safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, readBufferData, M_Min(LEGACY_DRIVE_SEC_SIZE, allocationLength));
                        }
                    }
                }
            }
    ...
    case 0x03://descriptor mode
        ...
        else
        {
            if (bufferID == 0)
            {
                ...
                if (scsiIoCtx->pdata)
                {
                    safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, readBufferDescriptor, M_Min(4, allocationLength));
                }
            }
            else
            {
        ...
        if (device->drive_info.ata_Options.generalPurposeLoggingSupported && device->drive_info.softSATFlags.currentInternalStatusLogSupported)
        {
            if (bufferID <= 3)
            {
                ...
                if (bufferID == 1 || bufferID == 3)
                {
                    ...
                    if (SUCCESS != ata_Read_Log_Ext(device, ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG, 0, gplDirectory, LEGACY_DRIVE_SEC_SIZE, device->drive_info.ata_Options.readLogWriteLogDMASupported, 1))
                    {
                        ret = NOT_SUPPORTED;
                        set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x2C, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                        break;
                    }
                    safe_memset(gplDirectory, LEGACY_DRIVE_SEC_SIZE, 0, LEGACY_DRIVE_SEC_SIZE);
                }
                //error history directory
                if (SUCCESS == ata_Read_Log_Ext(device, ATA_LOG_DIRECTORY, 0, gplDirectory, LEGACY_DRIVE_SEC_SIZE, device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
                {
                    uint32_t currentStatusLogSize = M_BytesTo2ByteValue(gplDirectory[(ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG * 2) + 1], gplDirectory[(ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG * 2)]) * LEGACY_DRIVE_SEC_SIZE;
                    uint32_t savedStatusLogSize = M_BytesTo2ByteValue(gplDirectory[(ATA_LOG_SAVED_DEVICE_INTERNAL_STATUS_DATA_LOG * 2) + 1], gplDirectory[(ATA_LOG_SAVED_DEVICE_INTERNAL_STATUS_DATA_LOG * 2)]) * LEGACY_DRIVE_SEC_SIZE;
                    if (SUCCESS != ata_Read_Log_Ext(device, ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG, 0, gplDirectory, LEGACY_DRIVE_SEC_SIZE, device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
                    {
                        ret = NOT_SUPPORTED;
                        set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x2C, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                    }
                    else
                    {
                        ...
                        if (scsiIoCtx->pdata)
                        {
                            safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, errorHistoryData, M_Min(offset, allocationLength));
                        }
                    }
                }
                else
            ...
            else if (bufferID >= 0x10 && bufferID <= 0xEF)
            {
                ...
                else
                {
                    ...
                    if (bufferID == 0x10)//current
                    {
                        ...
                        {
							uint32_t IEEEOUIField = 0;
							uint16_t dataArea1 = 0;
							uint16_t dataArea2 = 0;
							uint16_t dataArea3 = 0;
                            //We need to byte swap a few fields in the very first data sector because of ATA vs SCSI endianness.
                            //Bytes 0 - 3 are reserved on SCSI, so memset them to zero
                            safe_memset(&scsiIoCtx->pdata[0], scsiIoCtx->dataLength, 0, 4);
                            //Bytes 4 - 7 are the IEEE OUI, but this shows as a DWORD in ACS, so it needs swapping to SCSI endianness.
                            IEEEOUIField = M_BytesTo4ByteValue(scsiIoCtx->pdata[7], scsiIoCtx->pdata[6], scsiIoCtx->pdata[5], scsiIoCtx->pdata[4]);
                            ...
                            //Bytes 14 - 17 are mentioned only in SPC5, so we will memset them to zero to prevent issues with future devices.
                            safe_memset(&scsiIoCtx->pdata[14], scsiIoCtx->dataLength - 14, 0, 369/*this covers bytes 14 through 382*/);//memsetting all reserved fields to make sure this doesn't cause problems later. - TJE
                        }
                    }
                    else if (bufferID == 0x11)//saved
                    {
                        ...
                        else if (bufferOffset == 0)
                        {
                            ...
                            scsiIoCtx->pdata[13] = M_Byte0(dataArea3);
                            //Bytes 14 - 17 are mentioned only in SPC5, so we will memset them to zero to prevent issues with future devices.
                            safe_memset(&scsiIoCtx->pdata[14], scsiIoCtx->dataLength, 0, 369/*this covers bytes 14 through 382*/);//memsetting all reserved fields to make sure this doesn't cause problems later. - TJE
                        }
                    }
                    else
                    {
            ...
            {
                //shall clear the error history I_T nexus (Nothing to do here in this software implementation)
            }
            else if (bufferID == 0xFF)
            {
                //clear the error history I_T nexus as described (Nothing to do here in this software implementation)
                //release the current device internal status data using an unspecified method (uhhh......no idea)
            }
            else
            {
                ret = NOT_SUPPORTED;
    ...
    if (parameterListLength == 0)
    {
        ...
        if (selfTest)
        {
            if (smartEnabled && smartSelfTestSupported)
            {
                ...
                {
                    ret = FAILURE;
                    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_HARDWARE_ERROR, 0x3E, 0x03, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                }
            }
        ...
        else
        {
            if (smartSelfTestSupported)
            {
                ...
                {
                    ret = NOT_SUPPORTED;
                    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ABORTED_COMMAND, 0x67, 0x0B, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                }
            }
            else
            {
...
static eReturnValues translate_SCSI_Report_Luns_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    default:
        ret = NOT_SUPPORTED;
        set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x25, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
        break;
    }
    return ret;
}
...
static eReturnValues translate_SCSI_Request_Sense_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (SUCCESS != ret && is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word059) && device->drive_info.IdentifyData.ata.Word059 & BIT12)
    {
        ...
        if (SUCCESS == ata_Sanitize_Status(device, false))
        {
            ...
            {
                //sanitize error
                set_Sense_Data_For_Translation(&senseData[0], SPC3_SENSE_LEN, SENSE_KEY_MEDIUM_ERROR, 0x31, 0x03, descriptorFormat, M_NULLPTR, 0);
            }
            else if (device->drive_info.lastCommandRTFRs.secCntExt & BIT6)
            {
                ...
                set_Sense_Data_For_Translation(&senseData[0], SPC3_SENSE_LEN, SENSE_KEY_NOT_READY, 0x04, 0x1B, descriptorFormat, senseKeySpecificDescriptor, 1);
            }
            else
            {
    ...
    else
    {
        ...
        else
        {
            ...
            {
            case 0x00://standby_z
                set_Sense_Data_For_Translation(&senseData[0], SPC3_SENSE_LEN, SENSE_KEY_NO_ERROR, 0x5E, 0x43, descriptorFormat, M_NULLPTR, 0);
                break;
            case 0x01://standby_y
                set_Sense_Data_For_Translation(&senseData[0], SPC3_SENSE_LEN, SENSE_KEY_NO_ERROR, 0x5E, 0x43, descriptorFormat, M_NULLPTR, 0);
                break;
            case 0x80://idle
            case 0x81://idle_a
                set_Sense_Data_For_Translation(&senseData[0], SPC3_SENSE_LEN, SENSE_KEY_NO_ERROR, 0x5E, 0x42, descriptorFormat, M_NULLPTR, 0);
                break;
            case 0x82://idle_b
                set_Sense_Data_For_Translation(&senseData[0], SPC3_SENSE_LEN, SENSE_KEY_NO_ERROR, 0x5E, 0x42, descriptorFormat, M_NULLPTR, 0);
                break;
            ...
            default:
                ...
                //all is well
                set_Sense_Data_For_Translation(&senseData[0], SPC3_SENSE_LEN, SENSE_KEY_NO_ERROR, 0, 0, descriptorFormat, M_NULLPTR, 0);
                break;
            }
    ...
    if (checkSMARTStatus)
    {
            ...
            && SUCCESS == ata_SMART_Return_Status(device))
        {
            ...
            {
                set_Sense_Data_For_Translation(&senseData[0], SPC3_SENSE_LEN, SENSE_KEY_NO_ERROR, 0x5D, 0x10, descriptorFormat, M_NULLPTR, 0);
            }
            else
            {
                //call it good status
        ...
        else
        {
            ...
            set_Sense_Data_For_Translation(&senseData[0], SPC3_SENSE_LEN, SENSE_KEY_NOT_READY, 0x08, 0x00, descriptorFormat, M_NULLPTR, 0);//setting logical unit communication failure since that sounds about like what happened-TJE
        }
    }
    ...
    case 0x05://ATA Download Microcode
        if (downloadCommandSupported || device->drive_info.ata_Options.downloadMicrocodeDMASupported)
        {
                ...
                )
            {
                ...
                {
                    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_UNIT_ATTENTION, 0x3F, 0x01, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                }
            }
        ...
        case 0x00://start valid
            ...
            if (start)
            {
                ...
                else
                {
                    ...
                    seed_64(12432545);
                    randomLba = random_Range_64(0, device->drive_info.deviceMaxLba);
                    if (SUCCESS != ata_Read_Verify_Sectors(device, extCommand, 1, randomLba) && !immediate)//uhh....the spec  doens't mention what happens if this command fails...I'll return an error - TJE
                    {
                        ret = FAILURE;
                        set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense, scsiIoCtx->senseDataSize);
                    }
            ...
            else
            {
                if (loej)
                {
                    if (device->drive_info.IdentifyData.ata.Word000 & BIT7)
                    {
                        ...
                        {
                            ret = NOT_SUPPORTED;
                            set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ABORTED_COMMAND, 0x53, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                        }
                    }
                    else
                    {
                    ...
                    if (noFlush)
                    {
                        ...
                        if (SUCCESS != ata_Flush_Cache(device, flushCacheExt))
                        {
                            ...
                            if (SUCCESS != ata_Standby_Immediate(device))
                            {
                                ...
                                ret = ABORTED;
                                set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ABORTED_COMMAND, 0x2C, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                            }
                        }
                    }
                    else
                ...
                if (descriptor)
                {
                    if (descriptor->powerConditionFlags & BIT7)
                    {
                        ...
                        if (SUCCESS != ata_EPC_Set_Power_Condition_State(device, 0xFF, enable, false))
                        {
                            ...
                            {
                                return SUCCESS;//should save this error condition to report in the request sense command later. - TJE
                            }
                        }
                        if (SUCCESS != ata_EPC_Go_To_Power_Condition(device, powerConditionModifier + 0x81, false, false) && !immediate)
                        {
                            set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ABORTED_COMMAND, 0x2C, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                            ret = ABORTED;
                        }
            ...
            if (SUCCESS == ata_Read_Log_Ext(device, ATA_LOG_POWER_CONDITIONS, 1, powerConditionsLog, LEGACY_DRIVE_SEC_SIZE, device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
            {
                ...
                if (descriptor)
                {
                    if (descriptor->powerConditionFlags & BIT7)
                    {
                        ...
                        if (SUCCESS != ata_EPC_Go_To_Power_Condition(device, powerConditionModifier + 0x01, false, false) && !immediate)
                        {
                            set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ABORTED_COMMAND, 0x2C, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                            ret = ABORTED;
                        }
        ...
        case 0x07://LU Control
            if (SUCCESS == ata_Check_Power_Mode(device, &powerMode))
            {
                ...
                default: //unknown power mode. Cannot say a specific thing in the CDB is bad
                    ret = NOT_SUPPORTED;
                    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x24, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                    break;
                }
            }
            else if (!immediate)
...
static eReturnValues translate_Supported_Log_Pages(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (scsiIoCtx->pdata)
    {
        safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, supportedPages, M_Min(scsiIoCtx->dataLength, C_CAST(uint16_t, M_Min(LEGACY_DRIVE_SEC_SIZE, C_CAST(uint16_t, offset)))));
    }
    return ret;
}
...
static eReturnValues translate_Informational_Exceptions_Log_Page_2F(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (SUCCESS == ata_SMART_Return_Status(device))
    {
        ...
        if (scsiIoCtx->pdata)
        {
            safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, informationalExceptions, M_Min(11U, scsiIoCtx->dataLength));
        }
    }
                ...
                for (; parameterCode <= 0x0014; iter += 20, parameterCode++)
                {
                    ...
                    if (ataDescriptorNumber > 0)
                    {
                        ...
                        if (pageNumber > 0 && lastPageRead != pageNumber)
                        {
                            ...
                            if (SUCCESS != ata_Read_Log_Ext(device, ATA_LOG_EXTENDED_SMART_SELF_TEST_LOG, pageNumber, extSelfTestLog, LEGACY_DRIVE_SEC_SIZE, device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
                            {
                                set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense, scsiIoCtx->senseDataSize);
                                return ABORTED;
                            }
                        ...
                        selfTestNumber = extSelfTestLog[ataLogOffset + 4];//unspecified - setting to the content of the self test failure check point byte
                        //power on hours
    ...
    else
    {
        ...
        if (SUCCESS == ata_SMART_Read_Log(device, ATA_LOG_SMART_SELF_TEST_LOG, selfTestLog, LEGACY_DRIVE_SEC_SIZE))
        {
            ...
            if (selfTestIndex != 0)
            {
                //valid log data
                uint16_t iter = 4;
                uint16_t ataLogOffset = 2;//this should put us at the beginning of the descriptor we're going to read (SAT spec says, just match the parameter number to the self test descriptor number)
                for (; parameterCode <= 0x0014; iter += 20, parameterCode++, ataLogOffset += 24)
                {
                    ...
                    //set the self test number
                    selfTestResults[iter + 5] = selfTestLog[ataLogOffset + 4];//unspecified - setting to the content of the self test failure check point byte
                    //power on hours
                    accumulatedPowerOnHours = M_BytesTo2ByteValue(selfTestLog[ataLogOffset + 3], selfTestLog[ataLogOffset + 2]);
                    selfTestResults[iter + 6] = M_Byte1(accumulatedPowerOnHours);
                    selfTestResults[iter + 7] = M_Byte0(accumulatedPowerOnHours);
                    //address of failure
...
static eReturnValues translate_Read_Error_Counters_Log_0x03(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (reportedUncorrectablesValid || correctiveAlgValid)
    {
        ...
        if (scsiIoCtx->pdata)
        {
            safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, readErrorCountersLog, M_Min(M_Min(20U, offset), scsiIoCtx->dataLength));
        }
    }
...
static eReturnValues translate_Temperature_Log_0x0D(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (SUCCESS != ata_Read_Log_Ext(device, ATA_LOG_DEVICE_STATISTICS, ATA_DEVICE_STATS_LOG_TEMP, logPage, LEGACY_DRIVE_SEC_SIZE, device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
    {
        set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense, scsiIoCtx->senseDataSize);
        return FAILURE;
    }
    ...
    if (currentValid || referenceValid)
    {
        ...
        if (scsiIoCtx->pdata)
        {
            safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, temperatureLog, M_Min(M_Min(14U, offset), scsiIoCtx->dataLength));
        }
    }
...
static eReturnValues translate_Solid_State_Media_Log_0x11(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (SUCCESS != ata_Read_Log_Ext(device, ATA_LOG_DEVICE_STATISTICS, ATA_DEVICE_STATS_LOG_SSD, logPage, LEGACY_DRIVE_SEC_SIZE, device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
    {
        set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense, scsiIoCtx->senseDataSize);
        return FAILURE;
    }
    ...
    if (endurancevalid)
    {
        ...
        if (scsiIoCtx->pdata)
        {
            safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, solidStateMediaLog, M_Min(M_Min(12U, offset), scsiIoCtx->dataLength));
        }
    }
...
static eReturnValues translate_Background_Scan_Results_Log_0x15(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (SUCCESS != ata_Read_Log_Ext(device, ATA_LOG_DEVICE_STATISTICS, ATA_DEVICE_STATS_LOG_GENERAL, logPage, LEGACY_DRIVE_SEC_SIZE, device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
    {
        set_Sense_Data_By_RTFRs(device, &device->drive_info.lastCommandRTFRs, scsiIoCtx->psense, scsiIoCtx->senseDataSize);
        return FAILURE;
    }
    ...
    if (pohvalid)
    {
        ...
        if (scsiIoCtx->pdata)
        {
            safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, backgroundResults, M_Min(M_Min(20U, offset), scsiIoCtx->dataLength));
        }
    }
...
static eReturnValues translate_General_Statistics_And_Performance_Log_0x19(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    if (numReadsValid || numWritesValid || logicalSectorsWrittenValid || logicalSectorsReadValid)
    {
        ...
        if (scsiIoCtx->pdata)
        {
            safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, generalStatisticsAndPerformance, M_Min(M_Min(UINT32_C(72), offset), scsiIoCtx->dataLength));
        }
    }
...
static eReturnValues translate_ATA_Passthrough_Results_Log_Page_16(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    for (; parameterCode <= 0x000E && offset < 274; offset += 18, ++parameterCode)
    {
            ...
            device->drive_info.softSATFlags.ataPassthroughResults[parameterCode].lbaLowExt > 0)
        {
            ...
            ataPassthroughResults[offset + 8] = device->drive_info.softSATFlags.ataPassthroughResults[parameterCode].secCntExt;
            ataPassthroughResults[offset + 10] = device->drive_info.softSATFlags.ataPassthroughResults[parameterCode].lbaLowExt;
            ataPassthroughResults[offset + 12] = device->drive_info.softSATFlags.ataPassthroughResults[parameterCode].lbaMidExt;
            ataPassthroughResults[offset + 14] = device->drive_info.softSATFlags.ataPassthroughResults[parameterCode].lbaHiExt;
        }
        ...
        ataPassthroughResults[offset + 11] = device->drive_info.softSATFlags.ataPassthroughResults[parameterCode].lbaLow;
        ataPassthroughResults[offset + 13] = device->drive_info.softSATFlags.ataPassthroughResults[parameterCode].lbaMid;
        ataPassthroughResults[offset + 15] = device->drive_info.softSATFlags.ataPassthroughResults[parameterCode].lbaHi;
        ataPassthroughResults[offset + 16] = device->drive_info.softSATFlags.ataPassthroughResults[parameterCode].device;
        ataPassthroughResults[offset + 17] = device->drive_info.softSATFlags.ataPassthroughResults[parameterCode].status;
    }
    //set page length at the end
    ...
    if (scsiIoCtx->pdata)
    {
        safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, ataPassthroughResults, M_Min(M_Min(274U, offset), scsiIoCtx->dataLength));
    }
    return ret;
}
...
static eReturnValues translate_Application_Client_Log_Sense_0x0F(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    while (parameterCode <= 0x01FF && offset < allocationLength && parameterCounter < numberOfParametersToReturn)
    {
        ...
        for (uint8_t perATAPageCounter = 0; parameterCounter <= numberOfParametersToReturn && perATAPageCounter < 32 && offsetOnATAPage < (16 * LEGACY_DRIVE_SEC_SIZE) && offset < allocationLength; offsetOnATAPage += 256, ++parameterCounter, offset += 256, ++perATAPageCounter)
        {
                ...
                )
            {
                ...
                scsiIoCtx->pdata[offset + 2] = 0x83;
                scsiIoCtx->pdata[offset + 3] = 0xFC;
                safe_memcpy(&scsiIoCtx->pdata[offset + 4], scsiIoCtx->dataLength - (offset + 4) ,&hostLogData[offsetOnATAPage + 4], 0xFC);
            }
            else
            {
                //simple memcpy is all that's necessary
                safe_memcpy(&scsiIoCtx->pdata[offset], scsiIoCtx->dataLength - offset, &hostLogData[offsetOnATAPage], 256);
            }
        }
    }
    return ret;
...
static eReturnValues translate_Application_Client_Log_Select_0x0F(tDevice *device, ScsiIoCtx *scsiIoCtx, uint8_t *ptrData, bool parameterCodeReset, bool saveParameters, uint16_t totalParameterListLength)
{
    ...
    if (parameterCodeReset)
    {
        ...
        for (uint16_t parameterCode = 0; ataLogPageToWrite <= 0x9F && parameterCode <= 0x01FF; ++ataLogPageToWrite, ++parameterCode)
        {
            ...
            safe_memset(hostLogData, 16 * LEGACY_DRIVE_SEC_SIZE, 0, 16 * LEGACY_DRIVE_SEC_SIZE);
        ...
        else if (parameterListLength > 4 && totalParameterListLength > 4)
        {
            ...
            while (parameterDataOffset < parameterListLength && parameterDataOffset < totalParameterListLength)
            {
                ...
                for (uint8_t perATAPageCounter = C_CAST(uint8_t, offsetOnATAPage / UINT16_C(256)); perATAPageCounter < UINT8_C(32) && offsetOnATAPage < (UINT16_C(16) * LEGACY_DRIVE_SEC_SIZE) && parameterDataOffset < parameterListLength && parameterDataOffset < totalParameterListLength; offsetOnATAPage += UINT16_C(256), parameterDataOffset += C_CAST(uint16_t, parameterLength + UINT16_C(4)), ++perATAPageCounter)
                {
                    ...
                    safe_memcpy(&hostLogData[offsetOnATAPage], (16 * LEGACY_DRIVE_SEC_SIZE) - offsetOnATAPage, &scsiIoCtx->pdata[parameterDataOffset], 256);
                }
...
static eReturnValues translate_SCSI_Unmap_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    else if (parameterListLength != 0 && scsiIoCtx->pdata)
    {
        ...
        if (unmapBlockDescriptorLength > 0)
        {
            ...
            uint8_t *trimBuffer = C_CAST(uint8_t*, safe_calloc_aligned(trimBufferSize * LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t), device->os_info.minimumAlignment));//allocate the max size the device supports...we'll fill in as much as we need to
#if defined(SAT_SPEC_SUPPORTED) && SAT_SPEC_SUPPORTED > 3
			...
			trimBuffer = C_CAST(uint8_t*, calloc_aligned(trimBufferSize * LEGACY_DRIVE_SEC_SIZE, sizeof(uint8_t), device->os_info.minimumAlignment));//allocate the max size the device supports...we'll fill in as much as we need to
            ...
            for (; unmapBlockDescriptorIter < minBlockDescriptorLength; unmapBlockDescriptorIter += 16, numberOfBlockDescriptors++)
            {
                ...
                if (numberOfBlockDescriptors > (maxDescriptorsPerBlock * trimBufferSize))
                {
                    ...
                    ret = FAILURE;
                    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x26, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                    exitLoop = true;
                    break;
                }
                ...
                while (unmapNumberOfLogicalBlocks > 0)
                {
                    ...
                    if ((ataTrimOffset > (trimBufferSize * LEGACY_DRIVE_SEC_SIZE)) && ((unmapBlockDescriptorIter + 16) < minBlockDescriptorLength))
                    {
                        if (SUCCESS == ata_Data_Set_Management(device, true, trimBuffer, trimBufferSize * LEGACY_DRIVE_SEC_SIZE, useXL))
                        {
                            ...
                            safe_memset(trimBuffer, trimBufferSize * LEGACY_DRIVE_SEC_SIZE, 0, trimBufferSize * LEGACY_DRIVE_SEC_SIZE);
...
static eReturnValues translate_Mode_Sense_Control_0Ah(tDevice *device, ScsiIoCtx *scsiIoCtx, uint8_t pageControl, bool returnDataBlockDescriptor, bool longLBABit, uint8_t *dataBlockDescriptor, bool longHeader, uint8_t *modeParameterHeader, uint16_t allocationLength)
{
    eReturnValues ret = SUCCESS;
...
static eReturnValues translate_Mode_Sense_PATA_Control_0Ah_F1h(ScsiIoCtx *scsiIoCtx, uint8_t pageControl, bool returnDataBlockDescriptor, bool longLBABit, uint8_t *dataBlockDescriptor, bool longHeader, uint8_t *modeParameterHeader, uint16_t allocationLength)
{
    eReturnValues ret = SUCCESS;
    uint8_t *pataControlPage = M_NULLPTR;//will be allocated later
    uint16_t pageLength = 8;//add onto this depending on the length of the header and block descriptors
    uint16_t offset = 0;//used later when we start setting data in the buffer since we need to account for mode parameter header and DBDs
    uint8_t headerLength = 4;
    uint8_t blockDescLength = 0;
    if (!modeParameterHeader)
    ...
    else
    {
        ...
        //now set up the current transfer mode. FYI, the code is the same for "changeable" values except there are no break statements. But we don't support that here...-TJE
        switch (currentMode)
        {
        case 18://udma 6
...
static eReturnValues translate_Mode_Sense_Control_Extension_0Ah_01h(ScsiIoCtx *scsiIoCtx, uint8_t pageControl, bool returnDataBlockDescriptor, bool longLBABit, uint8_t *dataBlockDescriptor, bool longHeader, uint8_t *modeParameterHeader, uint16_t allocationLength)
{
    eReturnValues ret = SUCCESS;
...
//mode parameter header must be 4 bytes for short format and 8 bytes for long format (longHeader set to true)
//dataBlockDescriptor must be non-null when returnDataBlockDescriiptor is true. When non null, it must be 8 bytes for short, or 16 for long (when longLBABit is set to true)
static eReturnValues translate_Mode_Sense_Power_Condition_1A(tDevice *device, ScsiIoCtx *scsiIoCtx, uint8_t pageControl, bool returnDataBlockDescriptor, bool longLBABit, uint8_t *dataBlockDescriptor, bool longHeader, uint8_t *modeParameterHeader, uint16_t allocationLength)
{
    eReturnValues ret = SUCCESS;
    ...
    uint16_t offset = UINT16_C(0);//used later when we start setting data in the buffer since we need to account for mode parameter header and DBDs
        ...
        && (is_ATA_Identify_Word_Valid_With_Bits_14_And_15(device->drive_info.IdentifyData.ata.Word119) && device->drive_info.IdentifyData.ata.Word119 & BIT7))
    {
        ...
        case 0://current
            ...
            safe_memcpy(&powerConditionPage[offset + 4], pageLength - (offset + UINT32_C(4)), &ataPowerConditionsLog[0 + 12], 4);
            //byte swap the value
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 12], pageLength - (offset + UINT32_C(12)), &ataPowerConditionsLog[64 + 12], 4);
            //byte swap the value
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 16], pageLength - (offset + UINT32_C(16)), &ataPowerConditionsLog[128 + 12], 4);
            //byte swap the value
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 20], pageLength - (offset + UINT32_C(20)), &ataPowerConditionsLog[512 + 384 + 12], 4);
            //byte swap the value
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 8], pageLength - (offset + UINT32_C(8)), &ataPowerConditionsLog[512 + 448 + 12], 4);
            //byte swap the value
        ...
        case 2://default
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 4], pageLength - (offset + UINT32_C(4)),&ataPowerConditionsLog[0 + 4], 4);
            //byte swap the value
            byte_Swap_32(C_CAST(uint32_t*, &powerConditionPage[offset + 4]));
            //idle_b is bytes 64-127
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 12], pageLength - (offset + UINT32_C(12)), &ataPowerConditionsLog[64 + 4], 4);
            //byte swap the value
            byte_Swap_32(C_CAST(uint32_t*, &powerConditionPage[offset + 12]));
            //idle_c is bytes 128-191
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 16], pageLength - (offset + UINT32_C(16)), &ataPowerConditionsLog[128 + 4], 4);
            //byte swap the value
            byte_Swap_32(C_CAST(uint32_t*, &powerConditionPage[offset + 16]));
            //ata page 1
            //standby_y is bytes 384-447
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 20], pageLength - (offset + UINT32_C(20)), &ataPowerConditionsLog[512 + 384 + 4], 4);
            //byte swap the value
            byte_Swap_32(C_CAST(uint32_t*, &powerConditionPage[offset + 20]));
            //standby_z is  bytes 448-511
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 8], pageLength - (offset + UINT32_C(8)), &ataPowerConditionsLog[512 + 448 + 4], 4);
            //byte swap the value
            byte_Swap_32(C_CAST(uint32_t*, &powerConditionPage[offset + 8]));
            break;
        case 3://saved
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 4], pageLength - (offset + UINT32_C(4)), &ataPowerConditionsLog[0 + 8], 4);
            //byte swap the value
            byte_Swap_32(C_CAST(uint32_t*, &powerConditionPage[offset + 4]));
            //idle_b is bytes 64-127
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 12], pageLength - (offset + UINT32_C(12)), &ataPowerConditionsLog[64 + 8], 4);
            //byte swap the value
            byte_Swap_32(C_CAST(uint32_t*, &powerConditionPage[offset + 12]));
            //idle_c is bytes 128-191
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 16], pageLength - (offset + UINT32_C(16)), &ataPowerConditionsLog[128 + 8], 4);
            //byte swap the value
            byte_Swap_32(C_CAST(uint32_t*, &powerConditionPage[offset + 16]));
            //ata page 1
            //standby_y is bytes 384-447
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 20], pageLength - (offset + UINT32_C(20)), &ataPowerConditionsLog[512 + 384 + 8], 4);
            //byte swap the value
            byte_Swap_32(C_CAST(uint32_t*, &powerConditionPage[offset + 20]));
            //standby_z is  bytes 448-511
            ...
            //copy the timer value
            safe_memcpy(&powerConditionPage[offset + 8], pageLength - (offset + UINT32_C(8)), &ataPowerConditionsLog[512 + 448 + 8], 4);
            //byte swap the value
            byte_Swap_32(C_CAST(uint32_t*, &powerConditionPage[offset + 8]));
            break;
        }
...
//mode parameter header must be 4 bytes for short format and 8 bytes for long format (longHeader set to true)
//dataBlockDescriptor must be non-null when returnDataBlockDescriiptor is true. When non null, it must be 8 bytes for short, or 16 for long (when longLBABit is set to true)
static eReturnValues translate_Mode_Sense_ATA_Power_Condition_1A_F1(tDevice *device, ScsiIoCtx *scsiIoCtx, uint8_t pageControl, bool returnDataBlockDescriptor, bool longLBABit, uint8_t *dataBlockDescriptor, bool longHeader, uint8_t *modeParameterHeader, uint16_t allocationLength)
{
    eReturnValues ret = SUCCESS;
    uint8_t *powerConditionPage = M_NULLPTR;//will be allocated later
    uint16_t pageLength = 16;//add onto this depending on the length of the header and block descriptors
    uint16_t offset = 0;//used later when we start setting data in the buffer since we need to account for mode parameter header and DBDs
    uint8_t headerLength = 4;
    uint8_t blockDescLength = 0;
    if (!modeParameterHeader)
...
//mode parameter header must be 4 bytes for short format and 8 bytes for long format (longHeader set to true)
//dataBlockDescriptor must be non-null when returnDataBlockDescriiptor is true. When non null, it must be 8 bytes for short, or 16 for long (when longLBABit is set to true)
static eReturnValues translate_Mode_Sense_Read_Write_Error_Recovery_01h(ScsiIoCtx *scsiIoCtx, uint8_t pageControl, bool returnDataBlockDescriptor, bool longLBABit, uint8_t *dataBlockDescriptor, bool longHeader, uint8_t *modeParameterHeader, uint16_t allocationLength)
{
    eReturnValues ret = SUCCESS;
    ...
    if (blockDescLength > 0)
    {
        safe_memcpy(&readWriteErrorRecovery[headerLength], pageLength - headerLength, dataBlockDescriptor, blockDescLength);
    }
    //set the remaining part of the page up
    ...
    if (scsiIoCtx->pdata)
    {
        safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, readWriteErrorRecovery, M_Min(pageLength, allocationLength));
    }
...
//mode parameter header must be 4 bytes for short format and 8 bytes for long format (longHeader set to true)
//dataBlockDescriptor must be non-null when returnDataBlockDescriiptor is true. When non null, it must be 8 bytes for short, or 16 for long (when longLBABit is set to true)
static eReturnValues translate_Mode_Sense_Caching_08h(tDevice *device, ScsiIoCtx *scsiIoCtx, uint8_t pageControl, bool returnDataBlockDescriptor, bool longLBABit, uint8_t *dataBlockDescriptor, bool longHeader, uint8_t *modeParameterHeader, uint16_t allocationLength)
{
    eReturnValues ret = SUCCESS;
    ...
    else//saved, current, and default. TODO: Handle saving what the drive had when we started talking to it.
    {
        ...
        {
            caching[offset + 12] = BIT5;//fsw = 0, lbcss = 0, dra = 1, vendor specific (2bits) = 0, sync_prog(2bits) = 0, nv_dis = 0
        }
    }
...
//mode parameter header must be 4 bytes for short format and 8 bytes for long format (longHeader set to true)
//dataBlockDescriptor must be non-null when returnDataBlockDescriiptor is true. When non null, it must be 8 bytes for short, or 16 for long (when longLBABit is set to true)
static eReturnValues translate_Mode_Sense_Informational_Exceptions_Control_1Ch(ScsiIoCtx *scsiIoCtx, uint8_t pageControl, bool returnDataBlockDescriptor, bool longLBABit, uint8_t *dataBlockDescriptor, bool longHeader, uint8_t *modeParameterHeader, uint16_t allocationLength)
{
    eReturnValues ret = SUCCESS;
    ...
    if (blockDescLength > 0)
    {
        safe_memcpy(&informationalExceptions[headerLength], pageLength - headerLength, dataBlockDescriptor, blockDescLength);
    }
    //set the remaining part of the page up
    ...
    if (scsiIoCtx->pdata)
    {
        safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, informationalExceptions, M_Min(pageLength, allocationLength));
    }
...
static eReturnValues translate_SCSI_Mode_Sense_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    case 0x01://read-write error recovery
        ...
        {
        case 0:
            ret = translate_Mode_Sense_Read_Write_Error_Recovery_01h(scsiIoCtx, pageControl, returnDataBlockDescriptor, longLBABit, dataBlockDescriptor, longHeader, modeParameterHeader, allocationLength);
            break;
        default:
            ret = NOT_SUPPORTED;
            fieldPointer = 2;
    ...
    case 0x08://caching
        ...
        {
        case 0:
            ret = translate_Mode_Sense_Caching_08h(device, scsiIoCtx, pageControl, returnDataBlockDescriptor, longLBABit, dataBlockDescriptor, longHeader, modeParameterHeader, allocationLength);
            break;
        default:
            ret = NOT_SUPPORTED;
            fieldPointer = 2;
...
static eReturnValues translate_Mode_Select_Caching_08h(tDevice *device, ScsiIoCtx *scsiIoCtx, uint8_t *ptrToBeginningOfModePage, uint16_t pageLength)
{
    eReturnValues ret = SUCCESS;
    ...
    if (draRet || wceRet)
    {
        ...
        set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ABORTED_COMMAND, 0x44, 0x71, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
    }
    else
    {
        ret = SUCCESS;
...
static eReturnValues translate_Mode_Select_Control_0Ah(tDevice *device, ScsiIoCtx *scsiIoCtx, uint8_t *ptrToBeginningOfModePage, uint16_t pageLength)
{
    eReturnValues ret = SUCCESS;
    uint16_t dataOffset = C_CAST(uint16_t, ptrToBeginningOfModePage - scsiIoCtx->pdata);//to be used when setting which field is invalid in parameter list
    DECLARE_ZERO_INIT_ARRAY(uint8_t, senseKeySpecificDescriptor, 8);
        ...
        )
    {
        ...
        if (fieldPointer == 9)
        {
            --fieldPointer;//this is the same field in this mode page, just adjusting what we're setting so that we return the correct information.
        }
        if (fieldPointer == 8)
        {
            bitPointer = 7;
...
static eReturnValues translate_Mode_Select_Power_Conditions_1A(tDevice *device, ScsiIoCtx *scsiIoCtx, uint8_t *ptrToBeginningOfModePage, uint16_t pageLength)
{
    eReturnValues ret = SUCCESS;
    bool saveParameters = false;
    DECLARE_ZERO_INIT_ARRAY(uint8_t, senseKeySpecificDescriptor, 8);
        ...
        && (is_ATA_Identify_Word_Valid_With_Bits_14_And_15(device->drive_info.IdentifyData.ata.Word119) && device->drive_info.IdentifyData.ata.Word119 & BIT7))//EPC supported
    {
        ...
        else
        {
            ...
            if (SUCCESS == ata_Read_Log_Ext(device, ATA_LOG_POWER_CONDITIONS, 0, epcLog, LEGACY_DRIVE_SEC_SIZE * 2, device->drive_info.ata_Options.readLogWriteLogDMASupported, 0))
            {
                ...
                uint32_t idle_a_timer = M_BytesTo4ByteValue(ptrToBeginningOfModePage[4], ptrToBeginningOfModePage[5], ptrToBeginningOfModePage[6], ptrToBeginningOfModePage[7]);
                uint32_t idle_b_timer = M_BytesTo4ByteValue(ptrToBeginningOfModePage[12], ptrToBeginningOfModePage[13], ptrToBeginningOfModePage[14], ptrToBeginningOfModePage[15]);
                uint32_t idle_c_timer = M_BytesTo4ByteValue(ptrToBeginningOfModePage[16], ptrToBeginningOfModePage[17], ptrToBeginningOfModePage[18], ptrToBeginningOfModePage[19]);
                uint32_t standby_y_timer = M_BytesTo4ByteValue(ptrToBeginningOfModePage[20], ptrToBeginningOfModePage[21], ptrToBeginningOfModePage[22], ptrToBeginningOfModePage[23]);
                ...
                //idle_a check...changable or not, we need to check if they are trying to request a change as well as if the device supports the change
                if (!((idle_a && (epcLog[timerOffset + 1] & BIT2)) &&
                    (M_BytesTo4ByteValue(epcLog[timerOffset + 15], epcLog[timerOffset + 14], epcLog[timerOffset + 13], epcLog[timerOffset + 12])) == idle_a_timer))
                {
                    //change is requested
                    change_idle_a = true;
                    //check if changable
                ...
                if (!((idle_b && (epcLog[timerOffset + 1] & BIT2)) &&
                    (M_BytesTo4ByteValue(epcLog[timerOffset + 15], epcLog[timerOffset + 14], epcLog[timerOffset + 13], epcLog[timerOffset + 12])) == idle_b_timer))
                {
                    //change is requested
                    change_idle_b = true;
                    //check if changable
                ...
                if (!((idle_c && (epcLog[timerOffset + 1] & BIT2)) &&
                    (M_BytesTo4ByteValue(epcLog[timerOffset + 15], epcLog[timerOffset + 14], epcLog[timerOffset + 13], epcLog[timerOffset + 12])) == idle_c_timer))
                {
                    //change is requested
                    change_idle_c = true;
                    //check if changable
                ...
                if (!((standby_y && (epcLog[timerOffset + 1] & BIT2)) &&
                    (M_BytesTo4ByteValue(epcLog[timerOffset + 15], epcLog[timerOffset + 14], epcLog[timerOffset + 13], epcLog[timerOffset + 12])) == standby_y_timer))
                {
                    //change is requested
                ...
                if (!((standby_z && (epcLog[timerOffset + 1] & BIT2)) &&
                    (M_BytesTo4ByteValue(epcLog[timerOffset + 15], epcLog[timerOffset + 14], epcLog[timerOffset + 13], epcLog[timerOffset + 12])) == standby_z_timer))
                {
                    //change is requested
                ...
                {
                    set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_RECOVERED_ERROR, 0x37, 0, device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
                }
            }
            else
            {
...
static eReturnValues translate_Mode_Select_ATA_Power_Condition_1A_F1(tDevice *device, ScsiIoCtx *scsiIoCtx, uint8_t *ptrToBeginningOfModePage, uint16_t pageLength)
{
    eReturnValues ret = SUCCESS;
    uint16_t dataOffset = C_CAST(uint16_t, ptrToBeginningOfModePage - scsiIoCtx->pdata);//to be used when setting which field is invalid in parameter list
    DECLARE_ZERO_INIT_ARRAY(uint8_t, senseKeySpecificDescriptor, 8);
    ...
    if (pageFormat)
    {
        ...
        case 0x08:
            ...
            case 0://caching mode page
                ret = translate_Mode_Select_Caching_08h(device, scsiIoCtx, &scsiIoCtx->pdata[headerLength + blockDescriptorLength], pageLength);
                break;
            default:
...
static eReturnValues translate_SCSI_Zone_Management_In_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    uint64_t zoneStartLBA = M_BytesTo8ByteValue(scsiIoCtx->cdb[2], scsiIoCtx->cdb[3], scsiIoCtx->cdb[4], scsiIoCtx->cdb[5], scsiIoCtx->cdb[6], scsiIoCtx->cdb[7], scsiIoCtx->cdb[8], scsiIoCtx->cdb[9]);
    ...
    if ((allocationLength % 512) != 0)
    {
        ...
        dataBuf = C_CAST(uint8_t*, safe_calloc_aligned(dataBufLength, sizeof(uint8_t), device->os_info.minimumAlignment));
        localMemory = true;
    }
...
static eReturnValues translate_SCSI_Zone_Management_Out_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    //bool localMemory = false;
    eZMAction serviceAction = scsiIoCtx->cdb[1] & 0x1F;
    uint64_t zoneID = M_BytesTo8ByteValue(scsiIoCtx->cdb[2], scsiIoCtx->cdb[3], scsiIoCtx->cdb[4], scsiIoCtx->cdb[5], scsiIoCtx->cdb[6], scsiIoCtx->cdb[7], scsiIoCtx->cdb[8], scsiIoCtx->cdb[9]);
    //uint8_t reportingOptions = scsiIoCtx->cdb[14] & 0x3f;
    //uint8_t *dataBuf = M_NULLPTR;
    //uint32_t dataBufLength = 0;
    //uint32_t allocationLength = M_BytesTo4ByteValue(scsiIoCtx->cdb[10], scsiIoCtx->cdb[11], scsiIoCtx->cdb[12], scsiIoCtx->cdb[13]);
    //if (scsiIoCtx->cdb[14] & BIT0)
    //{
    //    allBit = true;
    //}
    ////allocate memory if it's not to a round 512 byte block size
...
static eReturnValues translate_SCSI_Set_Timestamp_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    {
        uint64_t timeStamp = M_BytesTo8ByteValue(0, 0, dataBuf[4], dataBuf[5], dataBuf[6], dataBuf[7], dataBuf[8], dataBuf[9]);
...
static eReturnValues create_All_Supported_Op_Codes_Buffer(tDevice *device, bool rctd, uint8_t **pdata, uint32_t *dataLength)
{
    eReturnValues ret = SUCCESS;
...
static eReturnValues translate_SCSI_Report_Supported_Operation_Codes_Command(tDevice *device, ScsiIoCtx *scsiIoCtx)
{
    ...
    case 2://check operation code and service action (error on commands that don't have service actions)
        ...
        ret = check_Operation_Code_and_Service_Action(device, requestedOperationCode, requestedServiceAction, rctd, &supportedOpData, &supportedOpDataLength);
        break;
    case 3://case 1 or case 2 (SPC4+)
        if (SUCCESS == check_Operation_Code(device, requestedOperationCode, rctd, &supportedOpData, &supportedOpDataLength))
        {
            ret = SUCCESS;
        }
        else
    ...
    {
        safe_memcpy(scsiIoCtx->pdata, scsiIoCtx->dataLength, supportedOpData, M_Min(supportedOpDataLength, allocationLength));
    }
    safe_free(&supportedOpData);
    return ret;
}
