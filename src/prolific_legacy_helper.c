...
// ******************************************************************************************
//
// \file prolific_legacy_helper.c   Implementation for Prolific Legacy USB
// Pass-through CDBs
                                                     ...
                                                     ataPassthroughCommand* ataCommandOptions)
{
    ...
    if (ataCommandOptions->commandType == ATA_CMD_TYPE_EXTENDED_TASKFILE)
    {
        *highCDBValid         = true;
        ...
        // these two may need to be set to zero instead...when debugging this code,
        // check this.
        hiCDB[14] = ataCommandOptions->tfr.DeviceHead;
        hiCDB[15] = ataCommandOptions->tfr.CommandStatus;
    }
    // set the low registers...if this was an ext command, then we will have
    // filled out the ext registers above.
...
eReturnValues send_Prolific_Legacy_Passthrough_Command(tDevice* device, ataPassthroughCommand* ataCommandOptions)
{
    ...
    if (ret == SUCCESS)
    {
        ...
        else if (ataCommandOptions->rtfr.status == 0 && ret == SUCCESS) // the IO was successful, however we didn't fill
                                                                        // in any tfrs...This should allow us one more
                                                                        // chance to dummy up values
        {
            ret = SUCCESS;
            ataCommandOptions->rtfr.status =
                ATA_STATUS_BIT_READY | ATA_STATUS_BIT_SEEK_COMPLETE; // just make sure we set passing status
                                                                     // for anything that cares to check
                                                                     // rtfrs
        }
