...
//
// Copyright (c) 2019-2024 Seagate Technology LLC and/or its Affiliates, All
// Rights Reserved
//
...
// All code in this file is from a ASMedia USB to NVMe product specification for
// pass-through NVMe commands. This code should only be used on products that
// are known to use this pass-through interface.
...
// This will return OS_COMMAND_NOT_AVAILABLE if it is given a command that is
// unable to be sent using this pass-through method. NVMe passthrough command.
// Examples only show identify and get log page. CDB Example (get log page): 0 -
// E6h opcode 1 - NVMe opcode - get log page 2 - reserved 3 - DW10, byte 0 (bits
// 7:0) - log identifier 4 - Reserved 5 - Reserved 6 - Num DWords (MSB) 7 - Num
// DWords (LSB) 8 - Log page offset (MSB) 9 - Log page offset 10 - Log page
// offset 11 - Log page offset 12 - Log page offset 13 - Log page offset 14 -
// Log page offset 15 - Log page offset (LSB)
...
static eReturnValues build_Basic_Passthrough_CDB(nvmeCmdCtx* nvmCmd, uint8_t* cdb)
{
    ...
    case NVME_ADMIN_CMD_IDENTIFY:
        ...
        // Need to make sure no reserved or other newer fields are set such as
        // CNTID, NVMSETID, or UUID Index since those cannot be sent.
        ...
        else
        {
            ...
            memset(cdb, 0, ASMEDIA_NVME_PASSTHROUGH_CDB_SIZE);
    ...
    case NVME_ADMIN_CMD_GET_LOG_PAGE:
        // Need to filter out fields that cannot be set in this pass-through command
        // before proceeding
                                                      ...
                                                      uint32_t                 dataSize)
{
    ...
    memset(cdb, 0, ASMEDIA_NVME_PACKET_CDB_SIZE);

    // Before validating other parameters, need to know which operation is being
    // requested.
    switch (asmOperation)
    {
    ...
    case ASMEDIA_NVMP_OP_RELINK_USB:
        ...
        // cdb[ASMEDIA_NVME_PACKET_PARAMETER_2_OFFSET] = parameter2;//parameter 2 is
    ...
    case ASMEDIA_NVMP_OP_GET_BRIDGE_INFO:
        ...
        cdb[ASMEDIA_NVME_PACKET_PARAMETER_1_OFFSET] = parameter1;
        // cdb[ASMEDIA_NVME_PACKET_PARAMETER_2_OFFSET] = parameter2;//parameter 2 is
        // unused for this operation set allocation length to 40h
        cdb[10]           = M_Byte3(0x40);
        cdb[11]           = M_Byte2(0x40);
        cdb[12]           = M_Byte1(0x40);
        cdb[13]           = M_Byte0(0x40);
    ...
    case ASMEDIA_NVMP_OP_SEND_ADMIN_IO_NVM_CMD:
        ...
        else
        {
            ...
            cdb[1]                                    = ASMEDIA_NVME_PACKET_SIGNATURE;
            cdb[ASMEDIA_NVME_PACKET_OPERATION_OFFSET] = C_CAST(uint8_t, asmOperation);
            // ignore input parameter 1 value as we can look at the nvmCmd structure
            // to set it properly for this command setup transfer length as 64B since
            // that's the size of the command in NVMe and that is what the spec shows
            // it is looking for.
            ...
            *cdbDataDirection = XFER_DATA_OUT;
            ...
            // finally, setup the data buffer with the NVM command DWORDS for the USB
            // adapter to send to the device. setup each dword in the buffer
            memset(dataPtr, 0, dataSize);
            if (nvmCmd->commandType == NVM_ADMIN_CMD)
            {
                // CDW0
                dataPtr[0] = nvmCmd->cmd.adminCmd.opcode;
                // NOTE: bytes 1, 2, 3 hold fused bits, prp vs sgl, and CID. None of
                // these are filled in for now...-TJE NSID
    ...
    case ASMEDIA_NVMP_OP_DATA_PHASE:
    {
        ...
        case XFER_DATA_IN:
            *cdbDataDirection                           = XFER_DATA_IN;
            ...
            break;
        case XFER_DATA_OUT:
            cdb[OPERATION_CODE] = ASMEDIA_NVME_PACKET_WRITE_OP; // change to a write opcode if sending
                                                                // data to the device.
            *cdbDataDirection                           = XFER_DATA_OUT;
            cdb[ASMEDIA_NVME_PACKET_PARAMETER_2_OFFSET] = ASM_NVMP_DATA_OUT;
            if (nvmCmd->dataSize == 0)
...
// NOTE: There is currently a bug in this code on the data phase command being
// rejected for invalid field in CDB.
//       This will debugged once I get a device in hand to figure out what is
//       going wrong.
eReturnValues send_ASM_NVMe_Cmd(nvmeCmdCtx* nvmCmd)
{
    ...
    // if the NVMe command is not doing a multiple of 512B data transfer, we need
    // to allocate local memory, rounded up to 512B boundaries before the command.
    ...
    if (nvmCmd->ptrData && nvmCmd->dataSize > 0 && nvmCmd->dataSize % 512)
    {
        ...
        // if a data-out command, need to copy what is intended to go to the device
        // to the new buffer
    ...
    if (localMemory)
    {
        // copy back to original smaller buffer from the oversized padded buffer if
        // read command
    ...
    if (sendRet != OS_COMMAND_TIMEOUT)
    {
        ...
        {
            // check for invalid entry by looking for 0xFF at bytes 14 and 15 of
            // returned data
    ...
    if (senseDataIsAllWeGot)
    {
        ...
        // Didn't get NVMe response, so no judgments beyond sense data translation
        // can be made.
        ret = sendRet;
    }
    return ret;
