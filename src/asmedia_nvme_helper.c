...
//This will return OS_COMMAND_NOT_AVAILABLE if it is given a command that is unable to be sent using this pass-through method.
//NVMe passthrough command. Examples only show identify and get log page.
//CDB Example (get log page):
//0 - E6h opcode
//1 - NVMe opcode - get log page
//2 - reserved
//3 - DW10, byte 0 (bits 7:0) - log identifier
//4 - Reserved
//5 - Reserved
//6 - Num DWords (MSB)
//7 - Num DWords (LSB)
//8 - Log page offset (MSB)
//9 - Log page offset
//10 - Log page offset
//11 - Log page offset
//12 - Log page offset
...
int build_Basic_Passthrough_CDB(nvmeCmdCtx *nvmCmd, uint8_t* cdb)
{
    ...
    case NVME_ADMIN_CMD_GET_LOG_PAGE:
        ...
        if (nvmCmd->cmd.adminCmd.cdw11 > 0 || nvmCmd->cmd.adminCmd.cdw14 > 0 || nvmCmd->cmd.adminCmd.cdw15 > 0
            || nvmCmd->cmd.adminCmd.cdw2 > 0 || nvmCmd->cmd.adminCmd.cdw3 > 0 || M_GETBITRANGE(nvmCmd->cmd.adminCmd.cdw10, 15, 8) > 0)
        {
            ret = OS_COMMAND_NOT_AVAILABLE;
        }
        else
    ...
    if (sendRet != COMMAND_TIMEOUT)
    {
        ...
        if (SUCCESS == scsi_Send_Cdb(nvmCmd->device, asmCDB, ASMEDIA_NVME_PACKET_CDB_SIZE, completionData, ASM_NVMP_RESPONSE_DATA_SIZE, asmCDBDir, NULL, 0, 15))
        {
            ...
            else
            {
                ...
                nvmCmd->commandCompletionData.sqIDandHeadPtr = M_BytesTo4ByteValue(completionData[11], completionData[10], completionData[9], completionData[8]);
                nvmCmd->commandCompletionData.statusAndCID = M_BytesTo4ByteValue(completionData[15], completionData[14], completionData[13], completionData[12]);
