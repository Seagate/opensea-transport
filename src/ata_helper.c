...
// This is a basic validity indicator for a given ATA identify word. Checks that
// it is non-zero and not FFFFh
...
// This will send a read log ext command, and if it's DMA and sense data tells
// us that we had an invalid field in CDB, then we retry with PIO mode
                                        ...
                                        uint16_t featureRegister)
{
    ...
    if (device->drive_info.ata_Options.generalPurposeLoggingSupported)
    {
            ...
            (logAddress == ATA_LOG_NCQ_COMMAND_ERROR_LOG || logAddress == ATA_LOG_SATA_PHY_EVENT_COUNTERS_LOG))
        {
            ...
            // This uses SATA id word 76 to identify the case and adjust which command
            ...
            device->drive_info.ata_Options.readLogWriteLogDMASupported && !sataLogRequiresPIO)
        {
            ...
            else
            {
                ...
                // we've seen reported when DMA commands are not supported.
                ...
                {
                    // we likely had a failure not related to DMA mode command, so return
                    // the error
                    return ret;
                }
            }
        ...
        {
            // this means something else is wrong, and it's not the DMA mode, so we
            // can turn it back on
                           ...
                           bool                   forceRTFRs)
{
    ...
    // This is a hack for some USB drives. While a caller somewhere above this
    // should handle this, this needs to be here to ensure we don't hang these
    // devices.
                                                  ...
                                                  uint16_t  recoveryTimeLimit)
{
    ...
    // if we are retrieving the current values, then we better have a good
    // pointer...no point in sending the command if we don't
                                      ...
                                      uint32_t dataSize)
{
    ...
    if (ret == SUCCESS)
    {
        if (functionCode == 0x0001)
        {
            ...
            memset(dataBuf, 0,
                   dataSize); // clear the buffer before we read in data since we are
                              // done with what we had to send to the drive
            ret = send_ATA_SCT_Data_Transfer(device, XFER_DATA_IN, dataBuf, dataSize);
        }
        // else we need to add functionality since something new was added to the
        // spec
    }
    return ret;
}
                                              ...
                                              uint32_t                   timeoutSeconds)
{
    ...
    if (dmaRetry && ret != SUCCESS)
    {
        // this means something else is wrong, and it's not the DMA mode, so we can
        // turn it back on
...
// This is a quick check to assist with the fill in ATA drive info function to
// be more efficient
                                         ...
                                         char     ataFW[ATA_IDENTIFY_FW_LENGTH + 1])
{
    if (ptrIdentifyData)
    {
        ...
        bool              validMN = true;
        bool              validFW = true;
        // check for valid strings (ATA-2 mentioned if these are set to zero, then
        // they are not defined in the standards
        if (idData->Word010 == 0)
        {
            validSN = false;
        }
...
eReturnValues fill_In_ATA_Drive_Info(tDevice* device)
{
    ...
    // try an identify command, then also try an identify packet device command.
    // The data we care about parsing will be in the same location so everything
    // inside this if should work as expected Note: if this is NOT an ATA HDD or
    // SSD, then this code will only try one command: 85h SAT CDB since most ATAPI
    // devices do not properly validate all fields of A1h and often receive it as
    // a packet passthrough command for "blank"
    ...
    {
        // make sure we disable sending the A1h SAT CDB or we could accidentally
        // send a "blank" command due to how most of these devices receive commands
        // under different OSs or through translators.
    ...
    else if (!device->drive_info.passThroughHacks.ataPTHacks.a1NeverSupported) // retry only if this is not already set
                                                                               // to true since the above commands would
                                                                               // have been sent with 85h already
    {
        // TODO: Check the sense data to see if it was invalid operation code. If
        // so, then the device does not support the A1h command. If this failed,
        // issue a test unit ready command, followed by switching to 16B sat
        // commands. Most devices tested will support A1h and very few will not if
        // they support SAT at all.
        if (device->drive_info.passThroughHacks.passthroughType == ATA_PASSTHROUGH_SAT)
        {
            // If IDE_INTERFACE, do not attempt the test unit ready, since this will
            // go through softwareSAT translation and fail/get hung in an infinite
            // loop. This test unit ready exists to help clear out stuck bad status
            // for other devices (such as USB) where this can help work around
            // limitations. - TJE
    ...
    if (retrievedIdentifyData)
    {
        ...
        {
            device->drive_info.scsiVersion = SCSI_VERSION_SPC_5; // SPC5. This is what software translator will set
                                                                 // at the moment. Can make this configurable
                                                                 // later, but this should be OK
        }
        ret = SUCCESS;
        if (device->drive_info.lastCommandRTFRs.device & DEVICE_SELECT_BIT) // Checking for the device select bit being
                                                                            // set to know it's device 1 (Not that we
                                                                            // really need it). This may not always be
                                                                            // reported correctly depending on the lower
                                                                            // layers of the OS and hardware. - TJE
        {
            ...
            is_ATA_Identify_Word_Valid(ident_word[6]))
        {
            ...
            head     = M_Byte0(ident_word[3]); // Word3 - adapted from ESDI so discard high byte.
                                               // High byte was number of removable drive heads
        ...
        // set some pointers to where we want to fill in information...we're doing
        // this so that on USB, we can store some info about the child drive,
        // without disrupting the standard drive_info that has already been filled
        // in by the fill_SCSI_Info function
        ...
        // IDE interface means we're connected to a native SATA/PATA interface so we
        // leave the default pointer alone and don't touch the drive info that was
        // filled in by the SCSI commands since that is how the OS talks to it for
        // read/write and we don't want to disrupt that Everything else is some sort
        // of SAT interface (UDS, SAS, IEEE1394, etc) so we want to fill in bridge
        // info here
        if ((device->drive_info.interface_type != IDE_INTERFACE) &&
        ...
        // this will catch all IDE interface devices to set a vendor identification
        ...
        device->drive_info.ata_Options.noNeedLegacyDeviceHeadCompatBits =
            false;                                       // for PATA devices, continue setting these bits for backwards
                                                         // compatibility.
        ...
        *fillPhysicalSectorSize = LEGACY_DRIVE_SEC_SIZE; // start with this and change later
        ...
        if (is_ATA_Identify_Word_Valid(ident_word[53]))
        {
                 ...
                 is_ATA_Identify_Word_Valid(ident_word[58])))
            {
                ...
                // read earlier. This is being checked again since a device may set bit0
        ...
        {
            // if we do not know the current read/write multiple setting, do not use
            // these commands on these really old drives!
        ...
        *fillMaxLba = C_CAST(uint64_t, cylinder) * C_CAST(uint64_t, head) * C_CAST(uint64_t, spt);
        ...
        // Word62 has SWDMA, but this is long obsolete. Just use PIO mode
        // instead-TJE
        ...
        {
            // bits 13:0 are valid for alignment. bit 15 will be 0 and bit 14 will
            // be 1. remove bit 14 with an xor
        ...
        // word supported (words 230 - 233) (Use this to get the max LBA since words
        // 100 - 103 may only contain a value of FFFF_FFFF)
        ...
        // Special case for SSD detection. One of these SSDs didn't set the
        // media_type to SSD but it is an SSD. So this match will catch it when this
        // happens. It should be uncommon to find though -TJE
            ...
            device->drive_info.ata_Options.dmaSupported && device->drive_info.ata_Options.isParallelTransport)
        {
            ...
            {
                // most likely some old drive with SWDMA only. Disable DMA mode commands
                // just to be sure everything works as expected.-TJE
                device->drive_info.ata_Options.dmaSupported                    = false;
                device->drive_info.passThroughHacks.ataPTHacks.dmaNotSupported = true;
            }
            else
            {
                // weird case as all SATA will support DMA...so set the lowest DMA
                // mode...should be compatible as any incompatible translators will
                // retry and turn this off
        ...
        {
            // turn off the DMA supported bits so upper layers issue PIO mode commands
            // instead.
            device->drive_info.ata_Options.dmaMode                       = ATA_DMA_MODE_NO_DMA;
            device->drive_info.ata_Options.dmaSupported                  = false;
            device->drive_info.ata_Options.readLogWriteLogDMASupported   = false;
        ...
        // This is to detect realtek USB to NVMe device since it will respond to SAT
        // ATA identify commands with valid strings and NOTHING else. If it has a
        // SATA drive, these will all report DMA mode of some kind and a maxLBA and
        // will never be ATAPI So this should be a reasonably good check to catch
        // this thing for now.
        if (device->drive_info.passThroughHacks.ataPTHacks.possilbyEmulatedNVMe &&
            (!device->drive_info.ata_Options.dmaSupported &&
             ...
             device->drive_info.drive_type != ATAPI_DRIVE))
        {
            // This means it's an emulated NVMe device where only the MN/SN/FW were
            // reported.
            device->drive_info.drive_type = SCSI_DRIVE;
                  ...
                  device->drive_info.drive_type != ATAPI_DRIVE))
        {
            // This very likely is emulated since a valid ATA device will have
            // fillMaxLba set to SOMETHING even in really old CHS drives since the LBA
            // is simulated in software.
            if (device->deviceVerbosity <= VERBOSITY_DEFAULT)
            {
                printf("WARNING: possible RTL 9210 detected and missed with all other "
                       "checks.\n");
                printf("         this may cause adverse behavior and require "
                       "--forceSCSI\n");
            }
        }
        ...
        {
            // for the SCSI interface, copy this information back to the main drive
            // info since SCSI translated info may truncate these fields and we don't
            // want that
            memcpy(device->drive_info.product_identification, device->drive_info.bridge_info.childDriveMN,
                   MODEL_NUM_LEN);
    ...
    // This may not be required...need to do some testing to see if reading the
    // logs below wakes a drive up - TJE
    ...
    if (!device->drive_info.passThroughHacks.ataPTHacks.dmaNotSupported)
    {
        ...
        {
            // forcing using DMA mode instead of UDMA since the translator doesn't
            // like UDMA mode set
            device->drive_info.ata_Options.dmaMode = ATA_DMA_MODE_DMA;
        }
    }
    ...
    // check if we're being asked to set the protocol to DMA for DMA commands
    // (default behavior depends on drive support from identify)
    if ((device->dFlags & FORCE_ATA_DMA_SAT_MODE) != 0)
    {
        device->drive_info.ata_Options.dmaMode = ATA_DMA_MODE_DMA;
    }
    // check if we're being asked to set the protocol to UDMA for DMA commands
    // (default behavior depends on drive support from identify)
    ...
    // device->drive_info.softSATFlags.senseDataDescriptorFormat = true;//by
    // default software SAT will set this to descriptor format so that ATA
    // pass-through works as expected with RTFRs. only bother reading logs if GPL
    // is supported...not going to bother with SMART even though some of the
    // things we are looking for are in SMART - TJE
    if (retrievedIdentifyData && device->drive_info.ata_Options.generalPurposeLoggingSupported)
    {
        ...
        if (SUCCESS == send_ATA_Read_Log_Ext_Cmd(device, ATA_LOG_DIRECTORY, 0, logBuffer, ATA_LOG_PAGE_LEN_BYTES, 0))
        {
            ...
            // check for support of ID Data Log, Current Device Internal Status, Saved
            // Device Internal Status, Device Statistics Log
            if (M_BytesTo2ByteValue(logBuffer[(ATA_LOG_DEVICE_STATISTICS * 2) + 1],
                                    logBuffer[(ATA_LOG_DEVICE_STATISTICS * 2)]) > 0)
            {
                readDeviceStatisticsLog = true;
            }
            if (M_BytesTo2ByteValue(logBuffer[(ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG * 2) + 1],
                                    logBuffer[(ATA_LOG_CURRENT_DEVICE_INTERNAL_STATUS_DATA_LOG * 2)]) > 0)
            {
                device->drive_info.softSATFlags.currentInternalStatusLogSupported = true;
            }
            if (M_BytesTo2ByteValue(logBuffer[(ATA_LOG_SAVED_DEVICE_INTERNAL_STATUS_DATA_LOG * 2) + 1],
                                    logBuffer[(ATA_LOG_SAVED_DEVICE_INTERNAL_STATUS_DATA_LOG * 2)]) > 0)
            {
                device->drive_info.softSATFlags.savedInternalStatusLogSupported = true;
            }
            if (M_BytesTo2ByteValue(logBuffer[(ATA_LOG_IDENTIFY_DEVICE_DATA * 2) + 1],
                                    logBuffer[(ATA_LOG_IDENTIFY_DEVICE_DATA * 2)]) > 0)
            {
                readIDDataLog = true;
            }
            // could check any log from address 80h through 9Fh...but one should be
            // enough (used for SAT application client log page translation) Using 90h
            // since that is the first page the application client translation uses.
            if (M_BytesTo2ByteValue(logBuffer[(0x90 * 2) + 1], logBuffer[(0x90 * 2)]) > 0)
            {
                device->drive_info.softSATFlags.hostLogsSupported = true;
            }
            // now read the couple pages of logs we care about to set some more flags
            // for software SAT
            if (readIDDataLog)
            {
                                                         ...
                                                         ATA_LOG_PAGE_LEN_BYTES, 0))
                {
                        ...
                        M_Word0(qword0) >= ATA_ID_DATA_VERSION_1)
                    {
                        ...
                        if (supportedZACCapabilities & ATA_ID_DATA_QWORD_VALID_BIT) // qword valid
                        {
                            ...
                            {
                                // according to what I can find in the spec, a HOST Managed
                                // drive reports a different signature, but doens't set any
                                // identify bytes like a host aware drive. because of this and
                                // not being able to get the real signature, this check is the
                                // only way to determine we are talking to an ATA host managed
                                // drive. - TJE
                                                                            ...
                                                                            logBuffer, ATA_LOG_PAGE_LEN_BYTES, 0))
                {
                    ...
                    {
                        // according to what I can find in the spec, a HOST Managed drive
                        // reports a different signature, but doens't set any identify bytes
                        // like a host aware drive. because of this and not being able to
                        // get the real signature, this check is the only way to determine
                        // we are talking to an ATA host managed drive. - TJE
                        if (device->drive_info.zonedType == ZONED_TYPE_NOT_ZONED)
                        {
                            device->drive_info.zonedType = ZONED_TYPE_HOST_MANAGED;
                        }
            ...
            if (readDeviceStatisticsLog)
            {
                                                         ...
                                                         logBuffer, ATA_LOG_PAGE_LEN_BYTES, 0))
                {
                    ...
                    {
                        // need to read this page and check if the data and time timestamp
                        // statistic is supported
                        memset(logBuffer, 0, ATA_LOG_PAGE_LEN_BYTES);
                                                                 ...
                                                                 ATA_LOG_PAGE_LEN_BYTES, 0))
                        {
                            ...
                            if (M_Byte2(qword0) == ATA_DEVICE_STATS_LOG_GENERAL &&
                                M_Word0(qword0) >= ATA_DEV_STATS_VERSION_1) // validating we got the right
                                                                            // page
                            {
...
// is this a read/write command that the rtfr output needs to know might report
// a bit specific to these commands ex: corrected data (old) or alignment error
// or address mark not found (old) NOTE: Streaming cmds not included in here!
...
static bool is_User_Data_Access_Command(ataPassthroughCommand* ataCommandOptions)
{
    ...
    if (ataCommandOptions)
    {
        ...
        case ATA_WRITE_SECTV_RETRY: // write & verify...old. ATA, ATA2, ATA3, then
                                    // obsolete ever since then
        ...
        case ATA_READ_BUF_DMA:
            ...
            // write same was in ATA and ATA2 and obsolete in ATA 3 and retired until
            // reuse for read buf DMA
            if (ataCommandOptions->commandDirection == XFER_DATA_OUT &&
                ataCommandOptions->commadProtocol == ATA_PROTOCOL_PIO) // this seems like the easiest way to figure out
                                                                       // which command it is -TJE
            {
                userDataAccess = true;
            }
            break;
        ...
        case ATA_SECURITY_FREEZE_LOCK_CMD: // also CFA wear level, but both of these
                                           // will be false
...
// added interpretation of status and error fields to print out more info and
// make it easier to interpret results This is not perfect, but a good
// enhancement to the output. Note that not all combinations of bits are
// handled. Specifically some ATAPI specific meanings are not handled right now.
// Support for old commands and old bits may not be fully functional, but it
// should cover most cases.
// TODO: Better ATAPI support
// TODO: Use supported ATA versions from identify (not just most recent, but
// anything with a bit set) to help better identify some status and error
// outputs
...
void print_Verbose_ATA_Command_Result_Information(ataPassthroughCommand* ataCommandOptions, tDevice* device)
{
    ...
    if (ataCommandOptions->rtfr.status & ATA_STATUS_BIT_ERROR) // assuming NOT a packet command
    {
        ...
        if (ataCommandOptions->rtfr.error & BIT7)
        {
            ...
            // NOTE: Since some translators only allow SAT set to DMA, need to make
            // sure drive's mode is UDMA and protocol can be DMA. Not perfect and can
            // be further improved
        ...
        // bit6 means either uncorrectable data or write protected (removable
        // medium)
        ...
        if (ataCommandOptions->rtfr.error & BIT0)
        {
            ...
            // address mark not found basically matched ID not found being set...or
            // when it should be set (LBA/CHS user data access commands, but not seek)
            ...
            {
                // if this is also set, for a user data access, possibly the old address
                // mark not found bit not: does not apply to old seek commands
    ...
    // Stream error only for streaming feature commands and read/write continuos
    // set to 1 in the command
    ...
    if (ataCommandOptions->rtfr.status & BIT2)
    {
        if (is_User_Data_Access_Command(ataCommandOptions))
        {
            ...
            {
                // todo: alignment error needs lps misalignment reporting to be
                // supported and error reporting set to 01b or 10b and only on write
                // commands
    ...
    // Bit 1 is either index (flips with rev) or sense data available. Sense data
    // reporting must at least be supported for this one to be useful
    ...
    // Bit 0 is error...or for ATAPI it will be check condition Packet commands
    // only
...
static bool is_Current_CHS_Info_Valid(tDevice* device)
{
    ...
    if (!(device->drive_info.IdentifyData.ata.Word053 & BIT0) || // if this bit is set, then the current fields are
                                                                 // valid. If not, they may or may not be valid
        device->drive_info.IdentifyData.ata.Word001 == 0 || device->drive_info.IdentifyData.ata.Word003 == 0 ||
        device->drive_info.IdentifyData.ata.Word006 == 0 || device->drive_info.IdentifyData.ata.Word054 == 0 ||
        device->drive_info.IdentifyData.ata.Word055 == 0 || device->drive_info.IdentifyData.ata.Word056 == 0 ||
...
//     for (cyl = 0; cyl <
//     deviceList[deviceIter].drive_info.IdentifyData.ata.Word054; ++cyl)
//     {
//         for (head = 0; head <
//         deviceList[deviceIter].drive_info.IdentifyData.ata.Word055; ++head)
//         {
//             for (sector = 1; sector <=
//             deviceList[deviceIter].drive_info.IdentifyData.ata.Word056;
//             ++sector)
//             {
//                 uint32_t lba = 0;
//                 convert_CHS_To_LBA(&deviceList[deviceIter], cyl, head,
//                 sector, &lba); fprintf(chsTest, "%5" PRIu32 " | %2" PRIu16 "
//                 | %2" PRIu16 " - %" PRIu32 "\n", cyl, head, sector, lba);
//             }
//         }
//     }
...
//         uint8_t head = 0;
//         uint8_t sector = 0;
//         convert_LBA_To_CHS(&deviceList[deviceIter], lba, &cylinder, &head,
//         &sector); fprintf(lbaToCHSTest, "%8" PRIu32 " - %5" PRIu16 " | %2"
...
// device parameter needed so we can see the current CHS configuration and
// translate properly...
...
eReturnValues convert_LBA_To_CHS(tDevice* device, uint32_t lba, uint16_t* cylinder, uint8_t* head, uint8_t* sector)
{
    ...
    {
        uint8_t* identifyPtr = (uint8_t*)&device->drive_info.IdentifyData.ata.Word000;
        // uint32_t lbaCapacity = M_BytesTo4ByteValue(identifyPtr[123],
        // identifyPtr[122], identifyPtr[121], identifyPtr[120]);//28bit LBA value
        ...
        if (is_CHS_Mode_Supported(device))
        {
            if (is_Current_CHS_Info_Valid(device))
            {
                ...
                *sector   = C_CAST(uint8_t, (lba % sectorsPerTrack) + UINT8_C(1));
                ...
                {
                    // change the return value, but leave the calculated values as they
                    // are
                    ret = NOT_SUPPORTED;
                }
            }
