...
static M_INLINE eReturnValues get_NOP_Result(const ataReturnTFRs* M_NONNULL result,
                                             sNOP_Test_Codes* M_NONNULL     nopResult)
{
    ...
    nopResult->lbaTestVal =
        M_BytesTo4ByteValue(M_Nibble0(result->device), result->lbaHi, result->lbaMid, result->lbaLow);
...
static void test_Passthrough_Register_Response_NonData(tDevice* M_NONNULL device)
{
    ...
    print_tDevice_Verbose_Formatted_String(
        device, VERBOSITY_COMMAND_VERBOSE, "NOP Test: SAT vendor='%s', product='%s'\n",
        device->drive_info.bridge_info.t10SATvendorID, device->drive_info.bridge_info.SATproductID);
    ...
    for (int testNum = 0; testNum < NOP_MAX_TESTS; ++testNum)
    {
        ...
        print_tDevice_Verbose_Formatted_String(device, VERBOSITY_COMMAND_VERBOSE,
                                               ...
                                               nopTests[testNum].countTestVal, nopTests[testNum].lbaTestVal);
        ...
        sNOP_Test_Codes result = {nopResults[testNum].secCnt,
                                  M_BytesTo4ByteValue(M_Nibble0(nopResults[testNum].device), nopResults[testNum].lbaHi,
        ...
        if (!standardExtractionMatches && testNum == 0 && !useAlternateOffsets)
        {
            ...
            for (size_t byteIdx = 0; byteIdx < sizeof(ataReturnTFRs); ++byteIdx)
            {
                if (resultBytes[byteIdx] == nopTests[testNum].countTestVal)
                {
                    ...
                    print_tDevice_Verbose_Formatted_String(
                        ...
                        nopTests[testNum].countTestVal, countOffsetInResult);
                    break;
                }
            }
            ...
            if (countOffsetInResult >= 0)
            {
                ...
                for (size_t byteIdx = 0; byteIdx < sizeof(ataReturnTFRs); ++byteIdx)
                {
                    if (resultBytes[byteIdx] == lbaLoExpected)
                    {
                        ...
                        if (byteIdx + 2 < sizeof(ataReturnTFRs) && resultBytes[byteIdx + 1] == lbaMidExpected &&
                            resultBytes[byteIdx + 2] == lbaHiExpected)
                        {
                            ...
                            print_tDevice_Verbose_Formatted_String(
                                ...
                                lbaLoExpected, lbaMidExpected, lbaHiExpected, lbaOffsetInResult);
                            break;
                        }
                    }
                }
            ...
            if (countOffsetInResult >= 0)
            {
                ...
                print_tDevice_Verbose_Formatted_String(
                    ...
                    countOffsetInResult, lbaOffsetInResult);
                ...
                if (lbaOffsetInResult >= 0)
                {
                    ...
                    result.lbaTestVal =
                        M_BytesTo4ByteValue(M_Nibble0(nopResults[testNum].device), lbaHi, lbaMid, lbaLo);
                }

                print_tDevice_Verbose_Formatted_String(
                    device, VERBOSITY_COMMAND_VERBOSE,
                    "Test %d: Re-extracted COUNT from offset %d = 0x%02X, LBA from offset %d = 0x%08X\n", testNum,
                    countOffsetInResult, result.countTestVal, lbaOffsetInResult, result.lbaTestVal);
            }
        }
        ...
        if (useAlternateOffsets && testNum > 0 && countOffsetInResult >= 0)
        {
            ...
            print_tDevice_Verbose_Formatted_String(device, VERBOSITY_COMMAND_VERBOSE,
                                                   "Test %d: Re-extracted COUNT from offset %d = 0x%02X (expected "
                                                   "0x%02X), LBA from offset %d = 0x%08X (expected 0x%08X)\n",
                                                   testNum, countOffsetInResult, extractedCount,
                                                   nopTests[testNum].countTestVal, lbaOffsetInResult, result.lbaTestVal,
                                                   nopTests[testNum].lbaTestVal);
        }
        ...
        else
        {
            ...
            if (nopResults[testNum].lbaLow == expectedHi && nopResults[testNum].lbaMid == expectedMid &&
                nopResults[testNum].lbaHi == expectedLo)
            {
                ++swappedLBAResponses;
                print_tDevice_Verbose_Formatted_String(device, VERBOSITY_COMMAND_VERBOSE,
                                                       "Test %d: LBA Lo/Hi bytes are reversed (Lo=0x%02X got 0x%02X, "
                                                       "Hi=0x%02X got 0x%02X) - swap pattern detected\n",
                                                       testNum, expectedLo, nopResults[testNum].lbaLow, expectedHi,
                                                       nopResults[testNum].lbaHi);
            }
            else
            {
                ++brokenLBAResponses;
        ...
        if (countTestMatch == lbaTestMatch)
        {
            ...
            if (device->drive_info.ata_Options.nopSupported == true)
            {
                ...
                print_tDevice_Verbose_Formatted_String(device, VERBOSITY_COMMAND_VERBOSE,
                                                       "NOP Test: EARLY EXIT - High confidence detected "
                                                       "(nopSupported=true, consistent results, confidence=%d)\n",
                                                       confidenceScore);
            ...
            {
                // without this set, we have lower confidence in drive behavior nad need more testing to confirm the
                // behavior. if it continues to pass, we are good to go onwards and this device responds as we expect it
                // to.
            }
        }
        else
        {
            ...
            if (senseFields.fixedFormat)
            {
                ...
                // status cannot have ATA_STATUS_BIT_BUSY, ATA_STATUS_BIT_DEVICE_FAULT, ATA_STATUS_BIT_DATA_REQUEST,
                // ATA_STATUS_BIT_CORRECTED_DATA/ATA_STATUS_BIT_ALIGNMENT_ERROR Status MAY have
                // ATA_STATUS_BIT_SENSE_DATA_AVAILABLE, ATA_STATUS_BIT_SEEK_COMPLETE Error must only set
                // ATA_ERROR_BIT_ABORT
                    ...
                    error == ATA_ERROR_BIT_ABORT)
                {
                    ...
                    uint8_t expectedHi  = M_Byte2(nopTests[testNum].lbaTestVal);
                    bool    isSwappedLBA =
                        (nopResults[testNum].lbaLow == expectedHi && nopResults[testNum].lbaMid == expectedMid &&
                         nopResults[testNum].lbaHi == expectedLo);
                    if (!isSwappedLBA)
                    {
                        // Looks like a common way we've seen this report in libATA which is not the right location, but
                        // we can work around this.
                        device->drive_info.passThroughHacks.ataPTHacks.fixedSenseHack =
                            SAT_FIXED_SENSE_HACK_UNALIGNED_WRITE_BUG;
                        // Try locating any other part of our command/pattern in the information/command specific
                        // information fields. Note that during debugging 12B and 16B provided different responses with
                        // different registers in different locations! It was observed that with the 12B device and
                        // count were next in command specific wheras with 16B it was LBA registers (not all of them
                        // though)
                        //
                        // Try finding the sent patterns in a different location, or an expected result in a
                        // different location than was expected. This happens in libata with the "unaligned write
                        // command" response.
                        ...
                        if (senseFields.senseKeySpecificInformation.senseKeySpecificValid &&
                            memchr(&senseFields.senseKeySpecificInformation.unknownDataType[0],
                                   nopTests[testNum].countTestVal,
    ...
    print_tDevice_Verbose_Formatted_String(device, VERBOSITY_COMMAND_VERBOSE,
                                           "NOP Test Loop Complete: successfulMatches=%d, brokenCount=%d, "
                                           "brokenLBA=%d, swappedLBA=%d, confidenceScore=%d\n",
                                           successfulMatches, brokenCountResponses, brokenLBAResponses,
                                           swappedLBAResponses, confidenceScore);
    print_tDevice_Verbose_Formatted_String(
        device, VERBOSITY_COMMAND_VERBOSE,
        "NOP Test Summary: Score=%d, Matches=%d, BrokenCount=%d, BrokenLBA=%d, SwappedLBA=%d\n", confidenceScore,
        successfulMatches, brokenCountResponses, brokenLBAResponses, swappedLBAResponses);
    flush_tDevice_Verbose_Stream(device);
    ...
    else if (swappedLBAResponses == NOP_MAX_TESTS && confidenceScore >= 20)
    {
        ...
        print_tDevice_Verbose_String(device, VERBOSITY_COMMAND_VERBOSE,
                                     "NOP Test Result: SWAPPED LBA BYTE ORDER - Lo and Hi bytes consistently reversed, "
                                     "setting SWAPPED_LBA_BYTE_ORDER hack\n");
    }
    ...
    else if (successfulMatches == NOP_MAX_TESTS && brokenCountResponses == NOP_MAX_TESTS &&
             brokenLBAResponses == NOP_MAX_TESTS && !useAlternateOffsets && confidenceScore >= 0)
    {
    ...
    else
    {
        ...
        print_tDevice_Verbose_Formatted_String(device, VERBOSITY_COMMAND_VERBOSE,
                                               "NOP Test Result: INCONCLUSIVE - Success=%d, BrokenCount=%d, "
                                               "BrokenLBA=%d, Score=%d, AltOffsets=%s, Offset=%d\n",
                                               successfulMatches, brokenCountResponses, brokenLBAResponses,
                                               confidenceScore, useAlternateOffsets ? "true" : "false",
                                               countOffsetInResult);
    ...
    if (device->drive_info.passThroughHacks.ataPTHacks.fixedSenseHack == SAT_FIXED_SENSE_HACK_UNALIGNED_WRITE_BUG)
    {
        ...
        eReturnValues verifyRet = ata_NOP(device, ATA_NOP_RETURN_ABORTED, verificationCountPattern,
                                          verificationLBAPattern, &verificationResult);
        if (verifyRet == SUCCESS || verifyRet == ABORTED)
        {
            ...
            if (getResult == SUCCESS)
            {
                ...
                print_tDevice_Verbose_Formatted_String(
                    device, VERBOSITY_COMMAND_VERBOSE, "NOP Verification Results: COUNT %s, LBA %s\n",
                    countFixedByWorkaround ? "FIXED" : "STILL_BROKEN", lbaFixedByWorkaround ? "FIXED" : "STILL_BROKEN");
                ...
                else if ((countFixedByWorkaround && lbaFixedByWorkaround) ||
                         verifyResult.countTestVal == verificationCountPattern)
                {
