...
// \brief Defines constants, structures, and functions necessary to communicate with physical drives behind an Adaptec RAID (aac, aacraid, arcsas)

#if defined (ENABLE_AAC) || 1
...
//  4. Internal driver code sets the address in little endian...probably need to do this too! (addr[0] = addr & UINT32_MAX, addr[1] = addr >> 32)
//  5. Can issue some commands with FIB passthrough instead of the SRB passthrough. Not sure when this would be preferred or not. SRB is converted to FIB anyways
...
// 10. aac_bus_info can tell you which bus' are valid to scan them. This is a container command of VM_Ioctl, FT_DRIVE object, method 1, CTLCmd GetBusInfo
// 11. DAC mode only if dma>32bit, more than 32bit memory, and controller supports 64bit sg list
// 12. There are some quirks in the code that may require using scsi32 when discovered. Need to find in driver identiy with cardtype to read quirks
// 13. CT_GET_PHYDEV_LIST & CT_GET_PHYDEV_INFO look like they will help get info about physical devices. Phydev info gives bus, target, slice, lun. Use VM_ContainerConfig to read this
// 14. GetDynAdapPropsFIB gives info about how the controller is configured (RAID, HBA, Simple Volume)

//Easy IOCTLs:
//  1. FSACTL_SENDFIB for more specific requests than other codes allow - struct fib_ioctl. Add additional structures to read more info as needed that are not in other simple IOCTLs
//  2. FSACTL_SEND_RAW_SRB - struct user_aac_srb. If on 32bit system use 32bit sg list, otherwise check controller support for 64bit sglist before using 64bits
...
//  5. FSACTL_GET_HBA_INFO - struct aac_hba_info 
//  6. FSACTL_MINIPORT_REV_CHECK - struct revision
//  7. FSACTL_QUERY_DISK - struct aac_query_disk controller, bus, target, lun, etc
...
// ARC-SAS
// AACAPI <- smartctl uses this in Windows for passthrough using a srb driver structure. Need to check if the structures I'm using will match or not...
// ARCIOCTL

#if defined (_WIN32)
//Store possible signatures here so we can easily access them later as needed
static const char *arcsas_ioctl_signatures[] = {
    "ARCSAS",
    "ARC-SAS",
    "AACAPI",
    "Arcsas",
...
void print_AAC_SRB_Status(uint32_t statusCode)
{
    printf("AAC SRB Status: ");
    switch (statusCode)
    {
...
static eReturnValues send_aac_raid_fib(AAC_HANDLE fd, AAC_FibCommands fibCommand, const void* dataRequestIn, uint32_t dataRequestInSize, void* dataRequestOut, uint32_t dataRequestOutSize, eVerbosityLevels verbosity)
{
    eReturnValues ret = OS_COMMAND_NOT_AVAILABLE;
    int localIoctlReturn = 0;
    struct aac_fib fib;
    uint32_t fibxferLen = sizeof(struct aac_fib);// + dataRequestInSize;
    ...
    safe_memset(&fib, sizeof(struct aac_fib), 0, sizeof(struct aac_fib));
    ...
    fib.Header.StructType = AAC_FIBTYPE_TFIB;//if flag AAC_FLAGS_NEW_COMM_TYPE2 use TFIB2, AAC_FIBTYPE_TFIB2_64 if setting SenderFibHighAddress
    fib.Header.Size = sizeof(struct aac_fib_header) + dataRequestInSize;
    ...
    if (VERBOSITY_COMMAND_NAMES <= verbosity)
    {
        ...
        print_Data_Buffer(&fib, sizeof(struct aac_fib), true);
    }
    ...
    //Issue the IO
        ...
        safe_memcpy(srbControl->Signature, 8, arcsas_ioctl_signatures[ARC_SIG_AACAPI], safe_strnlen(arcsas_ioctl_signatures[ARC_SIG_AACAPI], 8));
        ...
        localIoctlReturn = DeviceIoControl(fd, IOCTL_SCSI_MINIPORT, srbbuf, srbBufferSize, srbbuf, srbBufferSize, &returnedLength, &overlappedStruct);
        lastError = GetLastError();
        if (ERROR_IO_PENDING == lastError)//This will only happen for overlapped commands. If the drive is opened without the overlapped flag, everything will work like old synchronous code.-TJE
        {
...
static eReturnValues aac_Get_Adapter_Info(AAC_HANDLE fd, struct aac_adapter_info *info, eVerbosityLevels verbosity)
{
    eReturnValues ret = SUCCESS;
    printf("aac_Get_Adapter_Info\n");
    ret = send_aac_raid_fib(fd, RequestAdapterInfo, M_NULLPTR, 0, info, sizeof(struct aac_adapter_info), verbosity);
    print_Return_Enum("aac_Get_Adapter_Info\n",ret);
    return ret;
}

static eReturnValues aac_Get_Adapter_Supplemental_Info(AAC_HANDLE fd, struct aac_supplement_adapter_info *info, eVerbosityLevels verbosity)
{
    eReturnValues ret = SUCCESS;
    ret = send_aac_raid_fib(fd, RequestSupplementAdapterInfo, info, sizeof(struct aac_supplement_adapter_info), info, sizeof(struct aac_supplement_adapter_info), verbosity);
...
{
    eReturnValues ret = SUCCESS;
    struct aac_ctcfg containerCommand;
    struct aac_ctcfg_resp containerCommandResponse;
    safe_memset(&containerCommand, sizeof(struct aac_ctcfg), 0, sizeof(struct aac_ctcfg));
    safe_memset(&containerCommandResponse, sizeof(struct aac_ctcfg_resp), 0, sizeof(struct aac_ctcfg_resp));
    containerCommand.Command = VM_ContainerConfig;
    containerCommand.cmd = CT_GET_SCSI_METHOD;
    containerCommand.param = 0;
    ret = send_aac_raid_fib(fd, ContainerCommand, &containerCommand, sizeof(struct aac_ctcfg), &containerCommandResponse, sizeof(struct aac_ctcfg_resp), verbosity);
...
static eReturnValues aac_VM_Container_Get_Mount_Info(AAC_HANDLE fd, uint32_t containerID, bool ctrlVarBlkSzSupported, bool ctrl64bitLBASupported, struct aac_mntinforesp *mntInfo, uint32_t mntInfoLen, eVerbosityLevels verbosity)
{
    eReturnValues ret = SUCCESS;
    struct aac_mntinfo mntinfocmd;
    safe_memset(&mntinfocmd, sizeof(struct aac_mntinfo), 0, sizeof(struct aac_mntinfo));
    ...
    ret = send_aac_raid_fib(fd, ContainerCommand, &mntinfocmd, sizeof(struct aac_mntinfo), &mntInfo, mntInfoLen, verbosity);
...
static eReturnValues send_VM_Container_Get_Bus_Info(AAC_HANDLE fd, uint32_t scsiMethod, struct aac_vmi_businf_resp * businfo, eVerbosityLevels verbosity)
{
    eReturnValues ret = SUCCESS;
    struct aac_vmioctl vmio;
    safe_memset(&vmio, sizeof(struct aac_vmioctl), 0, sizeof(struct aac_vmioctl));
    vmio.Command = VM_Ioctl;
    vmio.ObjType = FT_DRIVE;
    vmio.MethId = scsiMethod;
    vmio.ObjId = UINT32_C(0);
    vmio.IoctlCmd = GetBusInfo;
    ret = send_aac_raid_fib(fd, ContainerCommand, &vmio, sizeof(struct aac_vmioctl), businfo, sizeof(struct aac_vmi_businf_resp), verbosity);
...
//This can be helpful to figure out what information is already known by the controller before we begin issuing commands
static eReturnValues send_VM_Container_Get_DevInfo(AAC_HANDLE fd, uint32_t scsiMethod, uint32_t bus, uint32_t target, uint32_t lun, struct aac_vmi_devinfo_resp *devinfo, eVerbosityLevels verbosity)
{
    eReturnValues ret = SUCCESS;
    struct aac_vmioctl vmio;
    safe_memset(&vmio, sizeof(struct aac_vmioctl), 0, sizeof(struct aac_vmioctl));
    ...
    vmio.ObjType = FT_DRIVE;
    vmio.MethId = scsiMethod;
    vmio.ObjId = AAC_BTL_TO_HANDLE(bus, target, lun);//Not sure if this goes here or somewhere else in the buffer. It's possible it's all in IOCTL_BUF but I don't know yet - TJE
    vmio.IoctlCmd = GetDeviceProbeInfo;
    ret = send_aac_raid_fib(fd, ContainerCommand, &vmio, sizeof(struct aac_vmioctl), devinfo, sizeof(struct aac_vmi_devinfo_resp), verbosity);
    return ret;
}
    ...
    if (pciInfo != M_NULLPTR)
    {
        int localIoctlReturn = 0;
            ...
            if (VERBOSITY_COMMAND_NAMES <= verbosity)
            {
                ...
                print_Data_Buffer(&srbControl, srbBufferSize, true);
            }
            SetLastError(ERROR_SUCCESS);
            localIoctlReturn = DeviceIoControl(fd, IOCTL_SCSI_MINIPORT, srbbuf, srbBufferSize, srbbuf, srbBufferSize, &returnedLength, &overlappedStruct);
            lastError = GetLastError();
            ...
            if (localIoctlReturn == TRUE)
            {
                ...
                {
                    printf("AAC PCI Info:\n");
                    print_Data_Buffer(&pciInfo, sizeof(struct aac_pci_info), true);
                }
            }
            else 
            {
...
static eReturnValues send_aac_miniport_rev_check(AAC_HANDLE fd, struct aac_rev_check *inrev, struct aac_rev_check_resp *outrev, eVerbosityLevels verbosity)
{
    eReturnValues ret = SUCCESS;
    if (inrev != M_NULLPTR && outrev != M_NULLPTR)
    {
        int localIoctlReturn = 0;
            ...
            safe_memcpy(srbbuf + sizeof(SRB_IO_CONTROL), srbBufferSize - sizeof(SRB_IO_CONTROL), inrev, sizeof(struct aac_rev_check));
            if (VERBOSITY_COMMAND_NAMES <= verbosity)
            {
            ...
            if (localIoctlReturn == TRUE)
            {
                safe_memcpy(outrev, sizeof(struct aac_rev_check_resp), srbbuf + sizeof(SRB_IO_CONTROL), sizeof(struct aac_rev_check_resp));
                if (VERBOSITY_COMMAND_NAMES <= verbosity)
                {
                    printf("AAC Miniport Rev Response:\n");
                    print_Data_Buffer(&outrev, sizeof(struct aac_rev_check_resp), true);
                }
            }
            else 
            {
            ...
            else
            {
                ...
                safe_memcpy(outrev, sizeof(struct aac_rev_check_resp), inrev, sizeof(aac_rev_check));
            }
        #endif
...
{
    size_t sgsize = sizeof(struct aac_sg_table) - sizeof(struct aac_sg_entry64);//just the "top" level structure for now
    //Need to take into account max elements and max element len to figure out how many need to be part of the allocation to pass to the driver
    ...
    for (; sgcounter < maxsgelements && sgcounter < UINT32_MAX && cmdTransferLen > UINT32_C(0); ++sgcounter)
    {
        ...
        {
            //in this case we need to set cmdTransferLen to zero since it's a final "partial" transfer based on what is allowed in the sg element len
            //This allows the counter to increment one final time then exit the loop
            cmdTransferLen = 0;
        }
    }
    sgsize += sizeof(struct aac_sg_entry64) * sgcounter;
    if (sgcounter == UINT32_C(0))
    {
        //if for some reason we got a zero then most likely someone called this with transferlen set to zero in the first place and we do not need any sglist
        sgsize = 0;
    }
    return sgsize;
}
...
{
    size_t sgsize = sizeof(struct aac_sg_table) - sizeof(struct aac_sg_entry);//just the "top" level structure for now
    //Need to take into account max elements and max element len to figure out how many need to be part of the allocation to pass to the driver
    uint32_t sgcounter = UINT32_C(0);
    for (; sgcounter < maxsgelements && sgcounter < UINT32_MAX && cmdTransferLen > UINT32_C(0); ++sgcounter)
    {
    ...
    sgsize += sizeof(struct aac_sg_entry) * sgcounter;
    if (sgcounter == UINT32_C(0))
    {
        //if for some reason we got a zero then most likely someone called this with transferlen set to zero in the first place and we do not need any sglist
        sgsize = 0;
...
static void setup_sg_list(struct aac_sg_table *sgtable, uint8_t *ptrdata, uint32_t cmdTransferLen, uint32_t maxsgelements, uint32_t maxsgelementlen)
{
    if (sgtable != M_NULLPTR && ptrdata != M_NULLPTR && cmdTransferLen > UINT32_C(0))
    {
        uintptr_t sgdataptr = M_REINTERPRET_CAST(uintptr_t, ptrdata);
        sgtable->SgCount = 0;
        for (uint32_t sgcounter = UINT32_C(0); sgcounter < maxsgelements && sgcounter < UINT32_MAX && cmdTransferLen > UINT32_C(0); ++sgcounter)
        {
            if (cmdTransferLen > maxsgelementlen)
            {
            ...
            else
            {
                ...
                //This allows the counter to increment one final time then exit the loop
                cmdTransferLen = 0;
                sgdataptr = 0;//if the loop were to run again we have setup a null pointer with zero length to be "used" next time
            }
            sgtable->SgCount += 1;
        }   
    }
...
//The caller of this function must have already allocated memory based on the length returned by get_sg_list_length
static void setup_sg_list64(struct aac_sg_table64 *sgtable, uint8_t *ptrdata, uint32_t cmdTransferLen, uint32_t maxsgelements, uint32_t maxsgelementlen)
{
    if (sgtable != M_NULLPTR && ptrdata != M_NULLPTR && cmdTransferLen > UINT32_C(0))
    {
        uintptr_t sgdataptr = M_REINTERPRET_CAST(uintptr_t, ptrdata);
...
static M_INLINE void safe_free_aac_srb(struct aac_srb **srb)
{
    safe_Free(M_REINTERPRET_CAST(void**, srb));
}

static M_INLINE void safe_free_aac_srb64(struct aac_srb64 **srb)
{
    safe_Free(M_REINTERPRET_CAST(void**, srb));
}
...
static eReturnValues issue_io_aacraid_Dev64(ScsiIoCtx * scsiIoCtx)
{
    ...
    //Note: FreeBSD is limited to 1 entry. Linux is limited to 256 entries and entry length limited by controller or 64K, Illumos has no limits in the driver code unless dma alloc fails.
    size_t srblen = sizeof(struct aac_srb) + sizeof(struct aac_srb_response);
    size_t sglistlen = 0;
    seatimer_t commandTimer;
    if (scsiIoCtx->direction != XFER_NO_DATA)
    {
        sglistlen = get_sg_list64_length(scsiIoCtx->dataLength, scsiIoCtx->device->os_info.aacDeviceData->maxSGList, scsiIoCtx->device->os_info.aacDeviceData->maxSGTransferLength);
        srblen += sglistlen;
    }
    struct aac_srb64 *aaccmd = safe_malloc(srblen);
    if (aaccmd == M_NULLPTR)
    {
        return MEMORY_FAILURE;
    }
    safe_memset(&commandTimer, sizeof(seatimer_t), 0, sizeof(seatimer_t));
    safe_memset(aaccmd, sizeof(struct aac_srb), 0, sizeof(struct aac_srb));
    ...
    //Not currently using this, but it can be used to allow us to issue the request sense which might be something we want to do in the future-TJE
    ...
    case XFER_DATA_IN:
        ...
        setup_sg_list64(&aaccmd->sg_map, scsiIoCtx->pdata, scsiIoCtx->dataLength, scsiIoCtx->device->os_info.aacDeviceData->maxSGList, scsiIoCtx->device->os_info.aacDeviceData->maxSGTransferLength);
        break;
    case XFER_DATA_OUT:
    ...
    aaccmd->data_len = srblen;//NOTE: This field is a little confusing but reading the driver code this seems to be the size of the allocated structure, not the amount of data this command will transfer.
    #if defined (_WIN32)
        PSRB_IO_CONTROL srbControl = M_NULLPTR;
        ...
        srbControl = M_REINTERPRET_CAST(PSRB_IO_CONTROL, srbbuf);
        srbControl->HeaderLength = sizeof(SRB_IO_CONTROL);
        safe_memcpy(srbControl->Signature, 8, arcsas_ioctl_signatures[ARC_SIG_AACAPI], safe_strlen(arcsas_ioctl_signatures[ARC_SIG_AACAPI]));
        ...
        localIoctlReturn = DeviceIoControl(scsiIoCtx->device->os_info.aacDeviceData->aacHandle, IOCTL_SCSI_MINIPORT, srbbuf, srbBufferSize, srbbuf, srbBufferSize, &returnedLength, &overlappedStruct);
        lastError = GetLastError();
        if (ERROR_IO_PENDING == lastError)//This will only happen for overlapped commands. If the drive is opened without the overlapped flag, everything will work like old synchronous code.-TJE
        {
            localIoctlReturn = GetOverlappedResult(scsiIoCtx->device->os_info.aacDeviceData->aacHandle, &overlappedStruct, &returnedLength, TRUE);
        }
        else if (lastError != ERROR_SUCCESS)
        {
            ret = OS_PASSTHROUGH_FAILURE;
    ...
    struct aac_srb_response *response = M_REINTERPRET_CAST(struct aac_srb_response*, M_STATIC_CAST(uintptr_t, aaccmd) + sizeof(struct aac_srb64) + sglistlen);//response is after the cmd and sglist

    if (scsiIoCtx->device->deviceVerbosity >= VERBOSITY_COMMAND_NAMES)
    {
        printf("Copied srb response\n");
        print_Data_Buffer(aaccmd, srblen, true);
    ...
    if (scsiIoCtx->psense)
    {
        safe_memcpy(scsiIoCtx->psense, scsiIoCtx->senseDataSize, response->sense, M_Min(M_Min(response->sense_len, AAC_HOST_SENSE_DATA_MAX), scsiIoCtx->senseDataSize));
    }
...
static eReturnValues issue_io_aacraid_Dev32(ScsiIoCtx * scsiIoCtx)
{
    ...
    if (scsiIoCtx->direction != XFER_NO_DATA)
    {
        sglistlen = get_sg_list_length(scsiIoCtx->dataLength, scsiIoCtx->device->os_info.aacDeviceData->maxSGList, scsiIoCtx->device->os_info.aacDeviceData->maxSGTransferLength);
        srblen += sglistlen;
    }
    struct aac_srb *aaccmd = safe_malloc(srblen);
    if (aaccmd == M_NULLPTR)
    {
        return MEMORY_FAILURE;
    }
    ...
    case XFER_DATA_IN:
        aaccmd->flags |= AAC_SRB_FLAGS_DATA_IN;
        setup_sg_list(&aaccmd->sg_map, scsiIoCtx->pdata, scsiIoCtx->dataLength, scsiIoCtx->device->os_info.aacDeviceData->maxSGList, scsiIoCtx->device->os_info.aacDeviceData->maxSGTransferLength);
        break;
    case XFER_DATA_OUT:
        aaccmd->flags |= AAC_SRB_FLAGS_DATA_OUT;
        setup_sg_list(&aaccmd->sg_map, scsiIoCtx->pdata, scsiIoCtx->dataLength, scsiIoCtx->device->os_info.aacDeviceData->maxSGList, scsiIoCtx->device->os_info.aacDeviceData->maxSGTransferLength);
    ...
    aaccmd->data_len = srblen;//NOTE: This field is a little confusing but reading the driver code this seems to be the size of the allocated structure, not the amount of data this command will transfer.
        ...
        srbControl = M_REINTERPRET_CAST(PSRB_IO_CONTROL, srbbuf);
        srbControl->HeaderLength = sizeof(SRB_IO_CONTROL);
        safe_memcpy(srbControl->Signature, 8, arcsas_ioctl_signatures[ARC_SIG_ARCSAS], safe_strlen(arcsas_ioctl_signatures[ARC_SIG_ARCSAS]));
        srbControl->Timeout = aaccmd->timeout;
        srbControl->ControlCode = FSACTL_SEND_RAW_SRB;
        srbControl->Length = srbBufferSize - sizeof(SRB_IO_CONTROL);
        safe_memcpy(&srbbuf[sizeof(SRB_IO_CONTROL)], srbBufferSize - sizeof(SRB_IO_CONTROL), aaccmd, srblen);
    ...
    struct aac_srb_response *response = M_REINTERPRET_CAST(struct aac_srb_response*, (aaccmd + 1) + sglistlen);//response is after the cmd and sglist
            ...
            //In Windows, open \\.\SCSIbus: since that is equivalent to the portnumber windows wants
            ...
            #endif
                {
                    ...
                    if (supports_aac_raid_ioctls(device->os_info.aacDeviceData->aacHandle))
                    {
                        ...
                        //NOTE: Setting 64K per sglist entry for now since that is noted in the linux driver code as a limit.
                        ...
                        device->os_info.aacDeviceData->maxSGTransferLength = UINT32_C(65536);
                            ...
                            device->os_info.aacDeviceData->maxSGList = 1;
                        #endif
                        struct aac_adapter_info ainfo;
                        safe_memset(&ainfo, sizeof(struct aac_adapter_info), 0, sizeof(struct aac_adapter_info));
                        ...
                        if (device->os_info.aacDeviceData->controllerSupportedOptions & AAC_SUPPORTED_SUPPLEMENT_ADAPTER_INFO)
                        {
                            //get supplemental info
                            struct aac_supplement_adapter_info supinfo;
                            safe_memset(&supinfo, sizeof(struct aac_supplement_adapter_info), 0, sizeof(struct aac_supplement_adapter_info));
                            if (SUCCESS == aac_Get_Adapter_Supplemental_Info(device->os_info.fd, &supinfo, device->deviceVerbosity))
                            {
...
eReturnValues close_AAC_RAID_Device(tDevice *device)
{
    ...
    if (device)
    {
            ...
            //TODO: Cloase handle
        #endif
        //Free this structure since we are done with it. The AACHande is also os_info.fd so no need to attempt closing it twice
        safe_free_aac_dev_info(&device->os_info.aacDeviceData);
    }
    return ret;
}
...
eReturnValues get_AAC_RAID_Device_Count(uint32_t * numberOfDevices, uint64_t flags, ptrRaidHandleToScan *beginningOfList)
{
    ...
    //Get Mount info for container 0 to figure out how many RAIDs the controller has enabled that we need to scan, then go through them and figure out how many disks are in each one.
    //Use get bus info to figure out which bus's are valid or not before attempting to scan through the device list and how many targets per bus are supported
        ...
        if (raidList->raidHint.adaptecRAID || raidList->raidHint.unknownRAID)
        {
...
#if (AAC_USE_FIBS)
            {
                ...
                if (SUCCESS == send_aac_get_pci_info(fd, &pciInfo, aacCountVerbosity) || true)
                {
                    uint32_t scsimode = 0;
                    struct aac_adapter_info ainfo;
                    struct aac_supplement_adapter_info asupinfo;//check ainfo for flag that this is supported, otherwise it will be empty
                    safe_memset(&ainfo, sizeof(struct aac_adapter_info), 0, sizeof(struct aac_adapter_info));
                    safe_memset(&asupinfo, sizeof(struct aac_supplement_adapter_info), 0, sizeof(struct aac_supplement_adapter_info));
                    struct aac_rev_check revcheck;
                    safe_memset(&revcheck, sizeof(struct aac_rev_check), 0, sizeof(struct aac_rev_check));
                    revcheck.callingComponent = RevApplication;
                    revcheck.callingRevision.external.comp.major = 1;
                    struct aac_rev_check_resp revresponse;
                    safe_memset(&revresponse, sizeof(struct aac_rev_check_resp), 0, sizeof(struct aac_rev_check_resp));
                    ...
                    if (SUCCESS == aac_Get_Adapter_Info(fd, &ainfo, aacCountVerbosity))
                    {
                        ...
                        struct aac_mntinforesp mountinfo;
                        safe_memset(&mountinfo, sizeof(struct aac_mntinforesp), 0, sizeof(struct aac_mntinforesp));
                        uint32_t maxContainers = AAC_MAX_CONTAINERS;
                        if (SUCCESS == aac_VM_Container_Get_Mount_Info(fd, 0, asupinfo.SupportedOptions2 & AAC_SUPPORTED_VARIABLE_BLOCK_SIZE, ainfo.SupportedOptions & AAC_SUPPORTED_64BIT_DATA, &mountinfo, sizeof(struct aac_mntinforesp), aacCountVerbosity))
                        {
                        ...
                        for (uint32_t containerIter = UINT32_C(1); containerIter <= maxContainers; ++containerIter)
                        {
                            ...
                            if (SUCCESS == aac_VM_Container_Get_Mount_Info(fd, containerIter, asupinfo.SupportedOptions2 & AAC_SUPPORTED_VARIABLE_BLOCK_SIZE, ainfo.SupportedOptions & AAC_SUPPORTED_64BIT_DATA, &mountinfo, sizeof(struct aac_mntinforesp), aacCountVerbosity))
                            {
                                printf("Container %" PRIu32 "\tCount: %" PRIu32 "\n", containerIter, mountinfo.MntRespCount);
                            }
                            uint32_t maxTargets = AAC_MAX_TARGETS;
                            uint32_t maxBuses = AAC_SCSI_MAX_PORTS;
                            struct aac_vmi_businf_resp businfo;
                            safe_memset(&businfo, sizeof(struct aac_vmi_businf_resp), 0, sizeof(struct aac_vmi_businf_resp));
                            ...
                            //now loop through the valid bus's and try getting drive info. If we get good info, then count that as a device
                            for(uint32_t busIter = 0; busIter < maxBuses && busIter < AAC_SCSI_MAX_PORTS; ++busIter)
                            {
                                if (businfo.BusInf.BusValid[busIter] == AAC_BUS_VALID)
                                {
                                    ...
                                    {
                                        //we also need to check for luns...typically only one lun (zero), but SAS may have multiple luns, so we need to check for them.
                                        //As soon as we fail from checking luns, exit the loop.
                                        for (uint32_t lunIter = 0; lunIter < UINT8_MAX; ++lunIter)
                                        {
                                            struct aac_vmi_devinfo_resp deviceInfo;
                                            if (SUCCESS == send_VM_Container_Get_DevInfo(fd, scsimode, busID, targetIter, lunIter, &deviceInfo, aacCountVerbosity))
                                            {
                                                printf("Vendor: %8s\tProduct: %16s\tRev:%4s\n", deviceInfo.VendorId, deviceInfo.ProductId, deviceInfo.ProductRev);
...
#else //AAC_USE_FIBS
            {
                //Try issuing test unit ready to each bus-target-lun to get a response and figure out what devices are available
                tDevice dummydev;
                ScsiIoCtx inquiry;
                ...
                dummydev.os_info.aacDeviceData = M_REINTERPRET_CAST(ptrAacDeviceInfo, safe_calloc(1, sizeof(aacDeviceInfo)));
                dummydev.os_info.aacDeviceData->maxSGList = 1;
                dummydev.os_info.aacDeviceData->maxSGTransferLength = 65536;
                //TODO: Figured out better maximums for bus, target, and lun. Currently limiting to 255 since that is reasonable-TJE
                //for (uint16_t busval = 0; busval < UINT8_MAX; ++busval)
                {
                    for(uint16_t targetval = 0; targetval < UINT8_MAX; ++targetval)
                    {
                        for(uint16_t lunval = 0; lunval < UINT8_MAX; ++lunval)
                        {
                            ...
                            {
                                ++found;
                                printf("Test Unit Ready Success on BTL: %" PRIu16 ":%" PRIu16 ":%" PRIu16 "!\n", dummydev.os_info.aacDeviceData->bus, dummydev.os_info.aacDeviceData->target, dummydev.os_info.aacDeviceData->lun);
                            }
                            printf("STD INQ OUT:\n");
...
eReturnValues get_AAC_RAID_Device_List(tDevice * const ptrToDeviceList, uint32_t sizeInBytes, versionBlock ver, uint64_t flags, ptrRaidHandleToScan *beginningOfList)
{
