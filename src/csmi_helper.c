...
static eReturnValues csmi_Return_To_OpenSea_Result(uint32_t returnCode)
{
    ...
    case CSMI_SAS_RAID_SET_BUFFER_TOO_SMALL:
        ret = FAILURE;
        break;
    case CSMI_SAS_RAID_SET_DATA_CHANGED: // not sure if this is just informative
                                         // or an error
        ret = SUCCESS;
        break;
    case CSMI_SAS_PHY_INFO_NOT_CHANGEABLE:
...
typedef struct _csmiIOin
{
    ...
    void*       ioctlBuffer;
    uint32_t    ioctlCode; // CSMI IOCTL code. Linux needs this, Windows doesn't
                           // since it's in the header for Windows.
    uint32_t ioctlBufferSize;
    char     ioctlSignature[8]; // Signature of the IOCTL to send
    uint32_t timeoutInSeconds;
    uint32_t dataLength;       // The length of all the data AFTER the ioctl header. This
                               // helps track how much to send/receive. The structure trying
                               // to read or write sizeof(CSMI struct) or possibly larger for
                               // those that have variable length data
    uint32_t controllerNumber; // For Linux drivers, we need to specify the
                               // controller number since the drivers may manage
                               // more than a single controller at a time. This
                               // will be ignored in Linux
    eVerbosityLevels csmiVerbosity;
    uint16_t         ioctlDirection; // Is this sending data (set) or receiving data
                                     // (get). Needed for Linux
} csmiIOin, *ptrCsmiIOin;

typedef struct _csmiIOout
{
    unsigned long bytesReturned; // Windows only and returned because it may be needed to
                                 // fully process the result. Will be 0 for other OSs
    int sysIoctlReturn;          // to save return from calling DeviceIoControl or Ioctl
                                 // functions.
    uint32_t*   lastError;       // pointer to store last error in. Optional
    seatimer_t* ioctlTimer;      // pointer to a timer to start and stop if the IOCTL
                                 // needs timing.
} csmiIOout, *ptrCsmiIOout;

// static because this should be an internal function to be reused below for
...
#    else
    int lastError = 0;
#    endif
    PIOCTL_HEADER ioctlHeader = csmiIoInParams->ioctlBuffer; // ioctl buffer should point to the beginning
...
//  in some distributions/cross compilers is defined as ioctl(int, unsigned
//  long, ...) and in others is defined as ioctl(int, int, ...)
// While debugging there does not seem to be a real conversion issue here.
// These ioctls still work in either situation, so disabling the warning seems
// best since there is not another way I have found to determine when to cast or
// not cast the sign conversion.-TJE
...
static eKnownCSMIDriver get_Known_CSMI_Driver_Type(PCSMI_SAS_DRIVER_INFO driverInfo)
{
    ...
    if (driverInfo)
    {
        // TODO: May need to track specific driver versions if capabilities vary
        // significantly at any point-TJE
        ...
        {
            // intel rapid storage technology VD driver. No idea what VD means or how
            // it differs yet.
            csmiDriverType = CSMI_DRIVER_INTEL_RAPID_STORAGE_TECHNOLOGY_VD;
        }
        // Check for a generic Intel match LAST. Check all other intel driver
        // variants above this condition to have a better intel driver
        // classification!
        else if (strstr(C_CAST(const char*, driverInfo->szName), "iaStor"))
        {
            // some kind of intel driver. Not sure all of the capabilities, but we are
            // unable to further discover anything else about it for now.
        ...
        else if (strstr(C_CAST(const char*, driverInfo->szName), "rcraid"))
        {
            ...
            csmiDriverType = CSMI_DRIVER_AMD_RCRAID;
                 ...
                 strstr(C_CAST(const char*, driverInfo->szName),
                        "HpCISSs3")) // need to check if final s can also be
                                     // lowercase
        {
...
// NOTE: This function needs the firmwareBuffer to be allocated with additional
// length for the firmware to send to the controller. In order to make this
// simple, we will assume the caller has already copied the controller firmware
// to the buffer, but we still need the total length
                                                ...
                                                uint32_t                           timeoutSeconds)
{
    ...
    memset(&ioIn, 0, sizeof(csmiIOin));
    memset(&ioOut, 0, sizeof(csmiIOout));
    memset(firmwareBuffer, 0,
           sizeof(CSMI_SAS_FIRMWARE_DOWNLOAD_BUFFER) - 1); // Memsetting ONLY the header and flags section so that the
                                                           // firmware we are sending is left untouched.
...
static void print_CSMI_RAID_Info(PCSMI_SAS_RAID_INFO raidInfo)
{
    if (raidInfo)
    {
        ...
        // Check if remaining bytes are zeros. This helps track differences between
        // original CSMI spec and some drivers that added onto it.
        if (!is_Empty(&raidInfo->uMaxRaidSets,
                      92)) // 92 is original reserved length from original
                           // documentation...can change to something else based on
                           // actual structure size if needed
        {
...
static void print_CSMI_RAID_Config(PCSMI_SAS_RAID_CONFIG config, uint32_t configLength)
{
    if (config)
    {
        ...
        if (!is_Empty(&config->bDataType,
                      20)) // this is being checked since failure code and change
                           // count were added later
        {
            printf("\tFailure Code: ");
        ...
        // at some point, need to switch to using CSMI version
        // information....somehow driverInfo.Information.usCSMIMajorRevision > 0 ||
        ...
        if (driveDataValid)
        {
            ...
            {
                uint32_t totalDrives = C_CAST(uint32_t,
                                              (configLength - UINT32_C(36)) /
                ...
                for (uint32_t iter = 0; iter < totalDrives && iter < config->bDriveCount; ++iter)
                {
                    ...
                    if (!is_Empty(&config->Drives[iter].usBlockSize,
                                  previouslyReservedBytes)) // original spec says 22 reserved
                                                            // bytes, however I count 30 more
                                                            // bytes to check...-TJE
                    {
...
// NOTE: This buffer should be allocated as sizeof(CSMI_SAS_RAID_CONFIG_BUFFER)
// + (raidInfo.uMaxDrivesPerSet * sizeof(CSMI_SAS_RAID_DRIVES)) at minimum. If
// the device identification VPD page is returned instead, it may be longer
//       RAID set index must be lower than the number of raid sets listed as
//       supported by RAID INFO
// NOTE: Dataype field may not be supported depending on which version of CSMI
// is supported. Intel RST will not support this.
                                   ...
                                   eVerbosityLevels             verbosity)
{
    ...
    raidConfigBuffer->Configuration.bDataType     = dataType; // NOTE: This may only be implemented on SOME CSMI
                                                          // implementations. Not supported by Intel RST as they only
                                                          // support up to .77 changes, but this is a newer field.
...
static void print_CSMI_SAS_Identify(PCSMI_SAS_IDENTIFY identify)
{
    if (identify)
    {
        // everything printed with 3 tabs to fit with print function below since
        // this is only used by Phy info data
        printf("\t\t\tDevice Type: ");
...
// NOTE: AMD's rcraid driver seems to treat non-raid drives slightly different
// in the phy info output. In the case I observed, the raid drives were in the
// phy info, but the
//       non-RAID drive was not shown here at all. However in the RAID IOCTLs,
//       it does show as a separate single drive raid (but no MN or SN info to
//       match to it). So it does not appear possible to issue a CSMI
//       passthrough command to non-raid drives. This is completely opposite of
//       what the Intel drivers do. Intel's drivers show every drive attached,
//       RAID or non-RAID in the phy info. This may be something we want to
//       detect in the future to reduce the number of IOCTLs sent, but for now,
//       it works ok. We can optimize this more later. -TJE
...
// ARCSAS Driver: For some unknown reason this driver needs an additional 484
// bytes after the sizeof(CSMI_SAS_PHY_INFO_BUFFER) to respond.
//                I do not know why since it's all zeroes in my case, but that
//                may also depend on RAID configuration setup. But this seems to
//                be the minimum number of bytes required for this driver to
//                respond to this IOCTL. It may be some alignment issue or
//                appending extra vendor unique data somewhere.-TJE

// Caller allocated full buffer, then we fill in the rest and send it. Data
// length not needed since this one is a fixed size
                                ...
                                eVerbosityLevels          verbosity)
{
    ...
    memset(&ioIn, 0, sizeof(csmiIOin));
    memset(&ioOut, 0, sizeof(csmiIOout));
    uint32_t phyInfoSize =
        sizeof(CSMI_SAS_PHY_INFO_BUFFER) + 484; // ARCSAS Workaround. Easier to handle by over-allocating as this
                                                // does not break compatibility with other drivers. Unknown what
    ...
    safe_Free(M_REINTERPRET_CAST(void**, &temp)); // temp holds the passed in phyInfoBuffer and that has
                                                  // already been updated when this succeeds
                                ...
                                eVerbosityLevels              verbosity)
{
    ...
    memset(&ioIn, 0, sizeof(csmiIOin));
    memset(&ioOut, 0, sizeof(csmiIOout));
    memset(phyInfoBuffer, 0,
           sizeof(IOCTL_HEADER)); // only clear out the header...caller should
                                  // setup the changes they want to make
...
// SSP Passthrough function (just for internal use, other functions should parse
// tdevice and scsiIoCtx to issue this
typedef struct _csmiSSPIn
{
    ...
    uint8_t lun[8];
    uint8_t flags; // read, write, unspecified, head of queue, simple, ordered,
                   // aca, etc. Must set read/write/unspecified at minimum. Simple
                   // attribute is recommended
    uint8_t  cdbLength;
    uint8_t* cdb;
    uint8_t* ptrData;    // pointer to buffer to use as source for writes. This will
                         // be used for reads as well.
...
{
    seatimer_t* sspTimer;     // may be null, but incredibly useful for knowing how
                              // long a command took
    uint8_t* senseDataPtr;    // Should not be null. In case of a drive error, this
                                          ...
                                          eVerbosityLevels verbosity)
{
    ...
    // ioIn.ioctlDirection = CSMI_SAS_DATA_READ;//This is set below, however it
    // may only need to be set one way....will only knwo when testing on linux
    ...
    if (sspInputs->cdbLength > 16)
    {
        ...
        sspPassthrough->Parameters.bAdditionalCDBLength =
            C_CAST(uint8_t,
                   (sspInputs->cdbLength - 16) / sizeof(uint32_t)); // this is in dwords according to the spec
    }
    else
    {
    ...
    if (ioOut.sysIoctlReturn == CSMI_SYSTEM_IOCTL_SUCCESS)
    {
        ...
        // check if response data is present in the status before trying to go any
        // further copying it back
            ...
            sspPassthrough->Status.bDataPresent & CSMI_SAS_SSP_RESPONSE_DATA_PRESENT)
        {
            ...
            {
                // copy what we got
                memcpy(sspInputs->ptrData, sspPassthrough->bDataBuffer,
        ...
        if (sspPassthrough->Status.bDataPresent & CSMI_SAS_SSP_SENSE_DATA_PRESENT)
        {
            ...
            if (sspOutputs->senseDataPtr)
            {
                ...
                memcpy(sspOutputs->senseDataPtr, sspPassthrough->Status.bResponse,
...
typedef struct _csmiSTPIn
{
    ...
    uint8_t  connectionRate; // strongly recommend leaving as negotiated
    uint8_t  destinationSASAddress[8];
    uint32_t flags;      // read, write, unspecified, must also specify pio, dma, etc
                         // for the protocol of the command being issued.
...
{
    seatimer_t* stpTimer; // may be null, but incredibly useful for knowing how
                          // long a command took
    uint8_t* statusFIS;   // Should not be null. In case of a drive error, this
                          // gives you what happened. This should point to a 20 byte
                          // array to store the result. May be D2H or PIO Setup
                          // depend on the command issued.
    uint32_t* scrPtr;     // Optional. This is the current status and control registers
                          // value. See SATA spec for more details on the registers that
                          // these map to. These are not writable through this interface.
    uint8_t connectionStatus;
    bool    retryAsSSPPassthrough; // This may be set, but will only be set, if the
                                   // driver does not support STP passthrough, but
                                   // DOES support taking a SCSI translatable CDB.
                                   // This cannot tell whether to use SAT or legacy
                                   // CSMI passthrough though...that's a trial and
                                   // error thing unless we figure out which drivers
                                   // and versions require that. -TJE
                                          ...
                                          eVerbosityLevels verbosity)
{
    ...
    if (ioOut.sysIoctlReturn == CSMI_SYSTEM_IOCTL_SUCCESS)
    {
        ...
        if (stpPassthrough->Parameters.uFlags & CSMI_SAS_STP_READ)
        {
            ...
            {
                // copy what we got
                memcpy(stpInputs->ptrData, stpPassthrough->bDataBuffer,
        ...
        {
            // if the caller allocted memory for the SCR data, then copy it back for
            // them
...
// TODO: SAS Task management function (can be used for a few things, but hard
// reset is most interesting CC_CSMI_SAS_TASK_MANAGEMENT

static void print_CSMI_Connector_Info(PCSMI_SAS_CONNECTOR_INFO_BUFFER connectorInfo)
{
    if (connectorInfo)
    {
        // need to loop through, but only print out non-zero structures since this
        // data doesn't give us a count. It is intended to be used alongside the phy
        // info data which does provide a count, but we don't want to be passing
        // that count in for this right now.
        printf("\n====CSMI Connector Info====\n");
...
// Function to check for CSMI IO support on non-RAID devices (from Windows
// mostly since this can get around Win passthrough restrictions in some cases)
// TODO: need a way to make sure we are only checking this on drives not
// configured as a raid member.
bool handle_Supports_CSMI_IO(CSMI_HANDLE deviceHandle, eVerbosityLevels verbosity)
{
    ...
    {
        // Send the following 2 IOs to check if CSMI passthrough is supported on a
        // device that is NOT a RAID device, meaning it is not configured as a
        // member of a RAID. eReturnValues csmi_Get_Driver_Info(CSMI_HANDLE
        // deviceHandle, uint32_t controllerNumber, PCSMI_SAS_DRIVER_INFO_BUFFER
        // driverInfoBuffer, eVerbosityLevels verbosity) eReturnValues
        // csmi_Get_Controller_Configuration(CSMI_HANDLE deviceHandle, uint32_t
        // controllerNumber, PCSMI_SAS_CNTLR_CONFIG_BUFFER ctrlConfigBuffer,
        // eVerbosityLevels verbosity)
...
bool device_Supports_CSMI_With_RST(tDevice* device)
{
    ...
    {
        // check for FWDL IOCTL support. If this works, then the Intel Additions are
        // supported.
...
#    endif //_WIN32
// This is really only here for Windows, but could be used under Linux if you
// wanted to use CSMI instead of SGIO, but that really is unnecessary controller
// number is to target the CSMI IOCTL inputs on non-windows. hostController is a
// SCSI address number, which may or may not be different...If these end up the
// same on Linux, this should be update to remove the duplicate parameters. If
// not, delete part of this comment. NOTE: this does not handle Intel NVMe
// devices in JBOD mode right now. These devices will be handled separately from
// this function which focuses on SATA/SAS
...
#    endif //_WIN32
        ...
        // Read controller info, driver info, get phy info for this device too...in
        // non-RAID mode, Windows scsi address should match the csmi scsi address
        CSMI_SAS_DRIVER_INFO_BUFFER  driverInfo;
        CSMI_SAS_CNTLR_CONFIG_BUFFER controllerConfig;
        CSMI_SAS_CNTLR_STATUS_BUFFER controllerStatus;
        memset(&driverInfo, 0, sizeof(CSMI_SAS_DRIVER_INFO_BUFFER));
...
#    if defined(CSMI_DEBUG)
        printf("JSCI: Getting driver Info, controller config, and controller "
               "status\n");
#    endif // CSMI_DEBUG
                                           ...
                                           &controllerConfig, &controllerStatus, device->deviceVerbosity))
        {
            ...
            // set CSMI scsi address based on what was passed in since it may be
            // needed later
            ...
            // before continuing, check if this is a known driver to work around known
            // issues.
...
#    endif // CSMI_DEBUG
            ...
            {
                // Need to figure out the device another way to get the SAS address IF
                // this is a SAS drive. If this is SATA, this is less important overall
                // unless it's on a SAS HBA, but a driver should be able to handle this
                // call already. The only other place to find a SAS Address is in the
                // RAID Config, but the drive may or may not be listed there...try it
                // anyways...if we still don't find it, we'll only get the sasAddress
                // from the phy info. RAID info/config will only be available if it's a
                // SAS or SATA RAID capable controller
#    if defined(CSMI_DEBUG)
                printf("JSCI: Using alternate method to get SAS address. Checking for "
                       "compatible controller flags first.\n");
#    endif // CSMI_DEBUG
...
#    endif // CSMI_DEBUG
                        for (uint32_t raidSet = 0; !gotSASAddress && raidSet < raidInfo.Information.uNumRaidSets;
                             ++raidSet)
                        {
                            // with the RAID info, now we can allocate and read the RAID
                            // config
                            ...
                            PCSMI_SAS_RAID_CONFIG_BUFFER raidConfig =
                                C_CAST(PCSMI_SAS_RAID_CONFIG_BUFFER, safe_calloc(raidConfigLength, sizeof(uint8_t)));
                            if (raidConfig)
                            {
#    if defined(CSMI_DEBUG)
...
#    endif // CSMI_DEBUG
                                            ...
                                            {
#    if defined(CSMI_DEBUG)
                                                printf("JSCI: CSMI Minor rev > 81, so checking "
                                                       "bDataType\n");
#    endif // CSMI_DEBUG
...
#    endif // CSMI_DEBUG
                                                ...
                                                {
                                                    // Found the match!!!
                                                    memcpy(device->os_info.csmiDeviceData->sasAddress,
                                                           raidConfig->Configuration.Drives[driveIter].bSASAddress, 8);
                                                    memcpy(device->os_info.csmiDeviceData->sasLUN,
                                                           raidConfig->Configuration.Drives[driveIter].bSASLun, 8);
                                                    // Intel drivers are known to support a SASAddress
                                                    // that is all zeroes as a valid address, so trust
                                                    // this result for these drivers -TJE
                                                    if (is_Intel_Driver(
                                                            device->os_info.csmiDeviceData->csmiKnownDriverType) ||
                                                        !is_Empty(device->os_info.csmiDeviceData->sasAddress,
                                                                  8)) // an empty (all zeros) SAS address means
                                                                      // it is not valid, such as on a SATA
                                                                      // controller. NOTE: Some SATA controllers
                                                                      // will fill this is, but this is not
                                                                      // guaranteed-TJE
                                                    {
#    if defined(CSMI_DEBUG)
                                                        printf("JSCI: Found matching drive data. Can send "
                                                               "CSMI IOs in addition to normal system IOs\n");
#    endif // CSMI_DEBUG
                                                        gotSASAddress = true;
                                                    }
                                                }
            ...
            // Attempt to read phy info to get phy identifier and port identifier
            // data...this may not work on RST NVMe if this code is hit...that's OK
            // since we are unlikely to see that. NOTE: AMD's rcraid driver seems to
            // treat non-raid drives slightly different in the phy info output. In the
            // case I observed, the raid drives were in the phy info, but the
            //       non-RAID drive was not shown here at all. However in the RAID
            //       IOCTLs, it does show as a separate single drive raid (but no MN
            //       or SN info to match to it). So it does not appear possible to
            //       issue a CSMI passthrough command to non-raid drives. This is
            //       completely opposite of what the Intel drivers do. Intel's drivers
            //       show every drive attached, RAID or non-RAID in the phy info. This
            //       may be something we want to detect in the future to reduce the
            //       number of IOCTLs sent, but for now, it works ok. We can optimize
            //       this more later. -TJE
...
#    endif // CSMI_DEBUG
            if (SUCCESS == csmi_Get_Phy_Info(device->os_info.csmiDeviceData->csmiDevHandle,
                                             ...
                                             device->deviceVerbosity))
            {
                // TODO: Is there a better way to match against the port identifier with
                // the address information provided? match to attached port or phy
                // identifier???
#    if defined(CSMI_DEBUG)
                printf("JSCI: Checking Phy info for a match. Number of phys: %" CPRIu8 "\n",
                       phyInfo.Information.bNumberOfPhys);
#    endif // CSMI_DEBUG
                for (uint8_t phyIter = 0, physFound = 0;
                     !foundPhyInfo && physFound < phyInfo.Information.bNumberOfPhys && phyIter < 32; ++phyIter)
                {
...
#    endif // CSMI_DEBUG
                        ...
                        {
                            // Found it. We can save the portID and phyID to use to issue
                            // commands :)
...
#    endif // CSMI_DEBUG
                        ...
                        {
#    if defined(CSMI_DEBUG)
                            printf("JSCI: No SAS address to match. Attempting passthrough "
                                   "commands to match\n");
#    endif // CSMI_DEBUG
                            ScsiIoCtx csmiPTCmd;
                            ...
                            // Don't have a SAS Address to match to, so we need to send an
                            // identify or inquiry to the device to see if it is the same MN,
                            // then check the SN. NOTE: This will not work if we don't already
                            // know the sasLUN for SAS drives. SATA will be ok though.
                            device->os_info.csmiDeviceData->portIdentifier =
                                phyInfo.Information.Phy[phyIter].bPortIdentifier;
                            device->os_info.csmiDeviceData->phyIdentifier =
                                phyInfo.Information.Phy[phyIter].Attached.bPhyIdentifier;
                            device->os_info.csmiDeviceData->portProtocol =
                                phyInfo.Information.Phy[phyIter].Attached.bTargetPortProtocol;
                            memcpy(&device->os_info.csmiDeviceData->sasAddress[0],
                                   phyInfo.Information.Phy[phyIter].Attached.bSASAddress, 8);
                            // Attempt passthrough command and compare identifying data.
                            // for this to work, SCSIIoCTX structure must be manually defined
                            // for what we want to do right now and call the CSMI IO
                            // directly...not great, but don't want to have other force flags
                            // elsewhere at the moment- TJE
...
#    endif // CSMI_DEBUG
                                if (SUCCESS == send_CSMI_IO(&csmiPTCmd))
                                {
                                    // TODO: If this is a multi-LUN device, this won't currently
                                    // work and it may not be possible to make this work if we got
                                    // to this case in the first place. HOPEFULLY the other CSMI
                                    // translation IOCTLs just work and this is unnecessary. - TJE
                                    // If MN matches, send inquiry to unit SN vpd page to confirm
                                    // we have a matching SN
...
#    endif // CSMI_DEBUG
           // now read the unit SN VPD page since this matches so far that way we
           // can compare the serial number. Not checking SCSI 2 since every SAS
           // drive *SHOULD* support this.
                                        memset(inqData, 0, 96);
...
#    endif // CSMI_DEBUG
                                        if (SUCCESS == send_CSMI_IO(&csmiPTCmd))
                                        {
                                            ...
                                            char* serialNumber =
                                                C_CAST(char*, safe_calloc(serialNumberLength, sizeof(char)));
                                            if (serialNumber)
                                            {
                                                memcpy(serialNumber, &inqData[4],
                                                       serialNumberLength - 1); // minus 1 to leave null terminator in
                                                                                // tact at the end
...
#    endif // CSMI_DEBUG
           // found a match!
                                                    foundPhyInfo = true;
                                                    // TODO: To help prevent multiport or multi-lun
                                                    // issues, we should REALLY check the device
                                                    // identification VPD page, but that can be a future
                                                    // enhancement
                                                }
                                                safe_free(&serialNumber);
                                            }
                                        }
                                        // else...catastrophic failure? Not sure what to do here
                                        // since this should be really rare to begin with.
                                    }
                                }
                            }
                        }
...
#    endif // CSMI_DEBUG
            ...
            {
#    if defined(CSMI_DEBUG)
                printf("JSCI: No phy info. Not enough information to use CSMI "
                       "passthrough\n");
#    endif // CSMI_DEBUG
           // We don't have enough information to use CSMI passthrough on this
           // device. Free memory and return NOT_SUPPORTED
                safe_free_csmi_dev_info(&device->os_info.csmiDeviceData);
                ret = NOT_SUPPORTED;
            }

#    if defined(_WIN32) && defined(ENABLE_INTEL_RST)
            // Check if Intel Driver and if FWDL IOs are supported or not.
            // version 14.8+
...
#        endif // CSMI_DEBUG
               // Intel driver, check for Additional IOCTLs by trying to read FWDL info
                if (supports_Intel_Firmware_Download(device))
                {
                    // No need to do anything here right now since the function above will
                    // fill in parameters as necessary.
#        if defined(CSMI_DEBUG)
                                              ...
                                              char**      nixbasehandle)
{
    ...
    {
        char* end = M_NULLPTR;
        char* str = M_CONST_CAST(char*, filename); // need to update str pointer as we scan the string,
                                                   // but not actually modifying data
...
eReturnValues get_CSMI_RAID_Device(const char* filename, tDevice* device)
{
    ...
    uint32_t      phyID         = 0;
    uint32_t      lun           = 0;
    // Need to open this handle and setup some information then fill in the device
    // information.
...
#    endif // CSMI_DEBUG
    ...
    {
#    if defined(CSMI_DEBUG)
        printf("GRD: Handle doesn't match std csmi format or Intel NVMe csmi "
               "format!\n");
#    endif // CSMI_DEBUG
...
#    if defined(_WIN32)
    ...
    else
    {
        ...
        if (intelNVMe)
        {
            snprintfres = snprintf(device->os_info.friendlyName, OS_HANDLE_FRIENDLY_NAME_MAX_LENGTH,
        ...
        {
            snprintfres = snprintf(device->os_info.friendlyName, OS_HANDLE_FRIENDLY_NAME_MAX_LENGTH,
                                   CSMI_HANDLE_BASE_NAME ":%" PRIu32 ":%" PRIu32 ":%" PRIu32 ":%" PRIu32, controllerNum,
                                   portID, phyID, lun);
        }
...
#    else  //_WIN32
    ...
    {
        int snprintfres = snprintf(device->os_info.friendlyName, OS_HANDLE_FRIENDLY_NAME_MAX_LENGTH,
...
#    endif                                                // CSMI_DEBUG
        device->os_info.minimumAlignment = sizeof(void*); // setting alignment this way to be compatible across
                                                          // OSs since CSMI doesn't really dictate an alignment,
                                                          // but we should set something. - TJE
...
#    endif // CSMI_DEBUG
        ...
        device->os_info.csmiDeviceData->csmiDeviceInfoValid = true;
        // we were able to open the requested handle...now it's time to collect some
        // information we'll need to save for this device so we can talk to it
        // later. get some controller/driver into then start checking for connected
...
#    endif // CSMI_DEBUG
                                            ...
                                            device->deviceVerbosity))
        {
            device->os_info.csmiDeviceData->csmiMajorVersion = driverInfo.Information.usCSMIMajorRevision;
            device->os_info.csmiDeviceData->csmiMinorVersion = driverInfo.Information.usCSMIMinorRevision;
            // TODO: If this is an Intel RST driver, check the name and additionally
            // check to see if it supports the Intel IOCTLs NOTE: If it's an Intel
            // NVMe, then we need to special case some of the below IOCTLs since it
            // won't respond the same...
            device->os_info.csmiDeviceData->securityAccess = get_CSMI_Security_Access(
                C_CAST(char*,
                       driverInfo.Information.szName)); // With this, we could add some intelligence to when
                                                        // commands are supported or not, at least under
                                                        // Windows, but mostly just a placeholder today. - TJE
        }
        else
        {
#    if defined(CSMI_DEBUG)
...
#    endif //_WIN32 && ENABLE_INTEL_RST
        {
            device->os_info.csmiDeviceData->portIdentifier = C_CAST(uint8_t, portID);
            device->os_info.csmiDeviceData->phyIdentifier  = C_CAST(uint8_t, phyID);
            // read phy info and match the provided port and phy identifier values
            // with the phy data to store sasAddress since it may be needed later.
            CSMI_SAS_PHY_INFO_BUFFER phyInfo;
#    if defined(CSMI_DEBUG)
            printf("GRD: Getting phy info\n");
#    endif // CSMI_DEBUG
           // NOTE: AMD's rcraid driver seems to treat non-raid drives slightly
           // different in the phy info output. In the case I observed, the raid
           // drives were in the phy info, but the
           //       non-RAID drive was not shown here at all. However in the RAID
           //       IOCTLs, it does show as a separate single drive raid (but no MN
           //       or SN info to match to it). So it does not appear possible to
           //       issue a CSMI passthrough command to non-raid drives. This is
           //       completely opposite of what the Intel drivers do. Intel's
           //       drivers show every drive attached, RAID or non-RAID in the phy
           //       info. This may be something we want to detect in the future to
           //       reduce the number of IOCTLs sent, but for now, it works ok. We
           //       can optimize this more later. -TJE
            if (SUCCESS == csmi_Get_Phy_Info(device->os_info.csmiDeviceData->csmiDevHandle,
                                             device->os_info.csmiDeviceData->controllerNumber, &phyInfo,
                                             device->deviceVerbosity))
            {
                // Using the data we've already gotten, we need to save phy identifier,
                // port identifier, port protocol, and SAS address.
                // TODO: Check if we should be using the Identify or Attached structure
                // information to populate the support fields. Identify appears to
                // contain initiator data, and attached seems to include target data...
...
#    endif // CSMI_DEBUG
            ...
            // Need to get SASLun from RAID config IF SSP is supported since we need
            // the SAS LUN value for issuing commands This is not needed for other
            // protocols.
            if (device->os_info.csmiDeviceData->portProtocol & CSMI_SAS_PROTOCOL_SSP)
            {
                // Read the RAID config and find the matching device from SAS address.
                // This is needed because SSP passthrough will need the SAS LUN
#    if defined(CSMI_DEBUG)
...
#    if defined(CSMI_DEBUG)
                    printf("GRD: Got RAID info, checking RAID sets. Number of RAID sets: "
                           "%" CPRIu32 "\n",
                           raidInfo.Information.uNumRaidSets);
#    endif // CSMI_DEBUG
                    for (uint32_t raidSet = 0; raidSet < raidInfo.Information.uNumRaidSets && !foundDrive; ++raidSet)
                    {
                        // need to parse the RAID info to figure out how much memory to
                        // allocate and read the
                        uint32_t raidConfigLength = C_CAST(
                            uint32_t, sizeof(CSMI_SAS_RAID_CONFIG_BUFFER) +
                                          (raidInfo.Information.uMaxDrivesPerSet * sizeof(CSMI_SAS_RAID_DRIVES)));
                        PCSMI_SAS_RAID_CONFIG_BUFFER raidConfig =
...
#    endif // CSMI_DEBUG
                                                            ...
                                                            CSMI_SAS_RAID_DATA_DRIVES, device->deviceVerbosity))
                        {
                            ...
                            // If we find a matching SAS address, we need to check the
                            // LUN....since we are only doing this for SSP, we should be able
                            // to use the get SCSI address function and validate that we have
                            // the correct lun.
#    if defined(CSMI_DEBUG)
...
#    endif // CSMI_DEBUG
           // take the SAS Address and SAS Lun and convert to SCSI Address...this
           // should be supported IF we find a SAS drive.
...
#    endif // CSMI_DEBUG
                    memcpy(&device->os_info.csmiDeviceData->signatureFIS, &signature.Signature.bSignatureFIS,
                           sizeof(sataD2HFis));
                    ...
                    device->os_info.csmiDeviceData->sataPMPort =
                        M_Nibble0(device->os_info.csmiDeviceData->signatureFIS
                                      .byte1); // lower nibble of this byte holds the port
                                               // multiplier port that the device is attached
                                               // to...this can help route the FIS properly
                }
            }

            // Need to check for Intel IOCTL support on SATA drives so we can send the
            // Intel FWDL ioctls instead of passthrough.
...
#    endif // CSMI_DEBUG
           // There is a way to get path-target-lun data from the SAS address if the
           // other IOCTLs didn't work (which they don't seem to support this
           // translation anyways)
...
#    endif // CSMI_DEBUG
           // convert SAS address to SCSI address using proprietary intel formatting
           // since IOCTLs above didn't work or weren't used. NOTE: This is only
           // valid for the noted driver. Previous versions used different formats
           // for sasAddress that don't support firmware update IOCTLs, and are not
           // supported - TJE
...
#    if defined(CSMI_DEBUG)
        printf("GRD: Initialization of structures completed. Calling fill drive "
               "info\n");
#    endif // CSMI_DEBUG
...
{
    bool isCSMI = false;
    // TODO: Expand this check to make sure all necessary parts of handle are
    // present???
...
// Storport will use a DWORD named CSMI set to a value between 0 and 3 (0=no
// access, 3 = full access) The following link describes where to find the
// DriverParameter key:
// https://learn.microsoft.com/en-us/windows-hardware/drivers/storage/registry-entries-for-storport-miniport-drivers
// Miniport scope: HKLM\System\CurrentControlSet\Services<miniport
// name>\Parameters\Device Adapter scope:
// HKLM\System\CurrentControlSet\Services<miniport
// name>\Parameters\Device<adapter#> Storport key location
// HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\<PortDriverName>\Parameters\CSMI
// = dword value
eCSMISecurityAccess get_CSMI_Security_Access(char* driverName)
{
    eCSMISecurityAccess access = CSMI_SECURITY_ACCESS_NONE;
#    if defined(_WIN32)
    if (strstr(driverName, "iaStor")) // Expand this list as other drives not
                                      // using these registry keys are found-TJE
    {
    ...
    else
    {
        ...
        if (tdriverName && registryKey && _tcslen(tdriverName) > 0 && _tcslen(registryKey) > 0)
        {
            ...
            {
                // Found the driver's parameters. Now search for CSMI DWORD for port
                // drivers
                ...
                else if (ERROR_FILE_NOT_FOUND == regQueryStatus)
                {
                    ...
                    // No port driver key found. Check for the miniport driver key in
                    // DriverParameter for Device or Device#
                    const TCHAR* paramDeviceKeyPath       = TEXT("\\Device");
                    size_t       paramDeviceKeyPathLength = _tcsclen(paramDeviceKeyPath);
                    registryKeyStringLength += paramDeviceKeyPathLength + 3; // Adding 3 for if we need to check for
                                                                             // a device number (adapter number)
                    ...
                    if (temp)
                    {
                        ...
                        if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, registryKey, 0, KEY_READ, &keyHandle))
                        {
                            ...
                            if (regQueryStatus == ERROR_SUCCESS) // since we had no memory allocated, this
                                                                 // returned success rather than ERROR_MORE_DATA
                                                                 // so we can go and allocate then read it.-TJE
                            {
                                ...
                                if (regData)
                                {
                                    ...
                                    if (regQueryStatus == ERROR_SUCCESS)
                                    {
                                        ...
                                        if (csmiConfig)
                                        {
                                            ...
                                            // Note: Case sensitive match for now. Switch to _tcsnicmp
                                            // if needing to eliminate case sensitivity-TJE
            ...
            {
                // This shouldn't happen, but it could happen...setting Limited for now
                // - TJE
                access = CSMI_SECURITY_ACCESS_LIMITED;
            }
        }
...
#    else // not windows, need root, otherwise not available at all. Return FULL if
          // running as root
...
//!   \param[in] beginningOfList = list of handles to use to check the count.
//!   This can prevent duplicate devices if we know some handles should not be
//!   looked at.
//!
//  Exit:
//!   \return SUCCESS - pass, !SUCCESS fail or something went wrong
...
#    endif // CSMI_DEBUG
    if (!beginningOfList || !*beginningOfList)
    {
        ...
        // Each OS that want's to do this should generate a list of handles to look
        // for.
#    if defined(CSMI_DEBUG)
...
#    endif // CSMI_DEBUG
    ...
    // On non-Windows systems, we also have to check controller numbers...so there
    // is one extra top-level loop for this on these systems.
#    if defined(CSMI_DEBUG)
...
#    if !defined(_WIN32)
                ...
                {
#    endif
                    // first, check if this handle supports CSMI before we try anything
                    // else
                    CSMI_SAS_DRIVER_INFO_BUFFER  driverInfo;
                    CSMI_SAS_CNTLR_CONFIG_BUFFER controllerConfig;
                    CSMI_SAS_CNTLR_STATUS_BUFFER controllerStatus;
                    ...
                    memset(&controllerStatus, 0, sizeof(CSMI_SAS_CNTLR_STATUS_BUFFER));
#    if defined(CSMI_DEBUG)
                    printf("GDC: Getting controller config, controller status, and "
                           "driver info\n");
#    endif // CSMI_DEBUG
...
#    endif // CSMI_DEBUG
                        ...
                        // Check if it's a RAID capable controller. We only want to
                        // enumerate devices on those in this function
                        if ((controllerConfig.Configuration.uControllerFlags & CSMI_SAS_CNTLR_SAS_RAID ||
                             controllerConfig.Configuration.uControllerFlags & CSMI_SAS_CNTLR_SATA_RAID ||
                             controllerConfig.Configuration.uControllerFlags & CSMI_SAS_CNTLR_SMART_ARRAY)
...
#    endif // CSMI_DEBUG
           // Get RAID info
           // NOTE Adaptec's API doesn't seem to like this. May need to pull phy
           // info instead if this fails. -TJE
...
#    endif                                                     // CSMI_DEBUG
                            bool raidConfigIncomplete = false; // if a driver has not filled in SASAddress or MN/SN
                                                               // info, then set this to true to use the PhyInfo
                                                               // instead. This may also happen for specific drivers
                                                               // in the future - TJE
                            // note: not checking raidConfigIncomplete in this loop to assist
                            // with some debug output later. This allows for warning about
                            // possible duplicates here. ARCSAS note: Raidsets start at 1
                            // instead of zero!
                            ...
                            {
                                // start with a length that adds no padding for extra drives,
                                // then reallocate to a new size when we know the new size
                                uint32_t raidConfigLength =
                                    C_CAST(uint32_t,
                                           sizeof(CSMI_SAS_RAID_CONFIG_BUFFER) +
                                               csmiRAIDInfo.Information.uMaxDrivesPerSet *
                                                   sizeof(CSMI_SAS_RAID_DRIVES)); // Intel driver
                                                                                  // recommends allocating
                                                                                  // for 8 drives to make
                                                                                  // sure nothing is
                                                                                  // missed. Maybe check
                                                                                  // if maxdriverperset
                                                                                  // less than this to
...
#    endif // CSMI_DEBUG
                                    ...
                                    {
                                        // make sure we got all the drive information...if now, we
                                        // need to reallocate with some more memory
#    if defined(CSMI_DEBUG)
...
#    endif // CSMI_DEBUG
                                        for (uint32_t iter = 0; iter < csmiRAIDConfig->Configuration.bDriveCount &&
...
#    endif // CSMI_DEBUG
                                            ...
                                            {
#    if defined(CSMI_DEBUG)
                                                printf("GDC: CSMI Minor rev > 81, so checking "
                                                       "bDataType\n");
#    endif // CSMI_DEBUG
                                            ...
                                            if (driveInfoValid)
                                            {
                                                ...
                                                {
                                                case CSMI_SAS_DRIVE_CONFIG_NOT_USED:
                                                    // Don't count drives with this flag, because they are
                                                    // not configured in a RAID at this time. We only want
                                                    // those configured in a RAID/RAID-like scenario.
#    if defined(CSMI_DEBUG)
...
#    endif // CSMI_DEBUG
                                                ...
                                                case CSMI_SAS_DRIVE_CONFIG_SRT_DATA:
                                                    ...
                                                    // check if SAS address is non-Zero or if MN/SN are
                                                    // available. removed special intel case since it
                                                    // appears to only work in some configurations for
...
#    if defined(CSMI_DEBUG)
                                                        printf("GDC: Detected incomplete raid config data. "
                                                               "Will need phy info to complete count\n");
#    endif // CSMI_DEBUG
                                                    }
...
#    endif // CSMI_DEBUG
                                    ...
                                    else // didn't successfully read a RAID config, which is
                                         // most likely a driver problem. This code requests
                            ...
                            if (raidConfigIncomplete)
                            {
                                // Need to check the phy info as a fall back since there is not
                                // enough information to uniquely identify all drives attached
                                // to this CSMI driver.
                                CSMI_SAS_PHY_INFO_BUFFER phyInfo;
                                memset(&phyInfo, 0, sizeof(CSMI_SAS_PHY_INFO_BUFFER));
#    if defined(CSMI_DEBUG)
                                printf("GDC: Getting phy info due to incomplete RAID "
                                       "configuration info.\n");
#    endif // CSMI_DEBUG
           // NOTE: AMD's rcraid driver seems to treat non-raid drives slightly
           // different in the phy info output. In the case I observed, the raid
...
#    if defined(CSMI_DEBUG)
                                        ...
                                        print_CSMI_Port_Protocol(
                                            phyInfo.Information.Phy[phyIter].Attached.bTargetPortProtocol);
#    endif // CSMI_DEBUG
           // Creating a temporary tDevice structure to use for the passthrough
           // commands.-TJE
                                        tDevice tempDevice;
                                        memset(&tempDevice, 0, sizeof(tDevice));
                                        tempDevice.os_info.minimumAlignment =
                                            sizeof(void*); // setting alignment this way to be
                                                           // compatible across OSs since CSMI
                                                           // doesn't really dictate an alignment,
                                                           // but we should set something. - TJE
                                        ...
                                        tempDevice.os_info.csmiDeviceData =
                                            C_CAST(ptrCsmiDeviceInfo, safe_calloc(1, sizeof(csmiDeviceInfo)));
...
#    endif // CSMI_DEBUG
                                        ...
                                        csmiPTCmd.senseDataSize = SPC3_SENSE_LEN;
                                        // Don't have a SAS Address to match to, so we need to send
                                        // an identify or inquiry to the device to see if it is the
                                        // same MN, then check the SN. NOTE: This will not work if
                                        // we don't already know the sasLUN for SAS drives. SATA
                                        // will be ok though.
                                        ...
                                        tempDevice.os_info.csmiDeviceData->portProtocol =
                                            phyInfo.Information.Phy[phyIter].Attached.bTargetPortProtocol;
                                        memcpy(&tempDevice.os_info.csmiDeviceData->sasAddress[0],
                                               phyInfo.Information.Phy[phyIter].Attached.bSASAddress, 8);
                                        // Attempt passthrough command and compare identifying data.
                                        // for this to work, SCSIIoCTX structure must be manually
                                        // defined for what we want to do right now and call the
                                        // CSMI IO directly...not great, but don't want to have
                                        // other force flags elsewhere at the moment- TJE
                                        if (phyInfo.Information.Phy[phyIter].Attached.bTargetPortProtocol &
                                                CSMI_SAS_PROTOCOL_SATA ||
                                            phyInfo.Information.Phy[phyIter].Attached.bTargetPortProtocol &
                                                CSMI_SAS_PROTOCOL_STP)
                                        {
                                            ...
                                            identify.tfr.DeviceHead = DEVICE_REG_BACKWARDS_COMPATIBLE_BITS;
#    if defined(CSMI_DEBUG)
                                            printf("GDC: Detected SATA protocol. Attempting Identify "
                                                   "CMD\n");
#    endif // CSMI_DEBUG
                                            if (SUCCESS == send_CSMI_IO(&csmiPTCmd))
                                            {
...
#    if defined(CSMI_DEBUG)
                                                    ...
                                                    printf("GDC: Not adding to the count since ATAPI "
                                                           "should use system handle instead.\n");
#    endif // CSMI_DEBUG
                                                }
                                            }
                                        }
...
#    endif // CSMI_DEBUG
                        ...
                        // raidList->handle); This was a CSMI handle, remove it from the
                        // list! This will also increment us to the next handle
#    if defined(CSMI_DEBUG)
...
#    endif // CSMI_DEBUG
                        ...
                        {
                            // if the first entry in the list was removed, we need up update
                            // the pointer before we exit so that the code that called here
                            // won't have an invalid pointer
...
#    endif
        ...
        if (!handleRemoved)
        {
            previousRaidListEntry = raidList; // store handle we just looked at in case we need to remove
                                              // one from the list
    ...
    {
        printf("WARNING: Possible duplicate devices found due to incomplete RAID "
               "config response from CSMI driver\n");
...
#    else
    DECLARE_ZERO_INIT_ARRAY(char, deviceName, CSMI_NIX_MAX_DEVICE_NAME_LENGTH);
#    endif
    eVerbosityLevels csmiListVerbosity = VERBOSITY_DEFAULT; // If debugging, change this and down below where this
                                                            // is set per device will also need changing
...
#    endif // CSMI_DEBUG
    ...
    else
    {
        ...
        // On non-Windows systems, we also have to check controller numbers...so
        // there is one extra top-level loop for this on these systems.
#    if defined(CSMI_DEBUG)
...
#    if defined(_WIN32)
                // Get the controller number from the scsi handle since we need it
                // later!
...
#    endif //_WIN32
           // first, check if this handle supports CSMI before we try anything else
                        ...
                        memset(&controllerConfig, 0, sizeof(CSMI_SAS_CNTLR_CONFIG_BUFFER));
                        memset(&controllerStatus, 0, sizeof(CSMI_SAS_CNTLR_STATUS_BUFFER));
                        csmiListVerbosity = d->deviceVerbosity; // this is to preserve any verbosity set
                                                                // when coming into this function
#    if defined(CSMI_DEBUG)
                        printf("GDL: Getting controller config, controller status, and "
                               "driver info\n");
#    endif // CSMI_DEBUG
                        if (SUCCESS == csmi_Get_Basic_Info(fd, controllerNumber, &driverInfo, &controllerConfig,
...
#    endif // CSMI_DEBUG
                            ...
                            case CSMI_SECURITY_ACCESS_NONE:
                                printf("CSMI Security access set to none! Won't be able to "
                                       "properly communicate with the device(s)!\n");
                                break;
                            case CSMI_SECURITY_ACCESS_RESTRICTED:
                                printf("CSMI Security access set to restricted! Won't be able "
                                       "to properly communicate with the device(s)!\n");
                                break;
                            case CSMI_SECURITY_ACCESS_LIMITED:
                                printf("CSMI Security access set to limited! Won't be able to "
                                       "properly communicate with the device(s)!\n");
                                break;
...
#    endif // CSMI_DEBUG
           // Check if it's a RAID capable controller. We only want to enumerate
           // devices on those in this function
                            if ((controllerConfig.Configuration.uControllerFlags & CSMI_SAS_CNTLR_SAS_RAID ||
                                 controllerConfig.Configuration.uControllerFlags & CSMI_SAS_CNTLR_SATA_RAID ||
                                 controllerConfig.Configuration.uControllerFlags & CSMI_SAS_CNTLR_SMART_ARRAY) &&
                                knownCSMIDriver != CSMI_DRIVER_ARCSAS)
                            {
                                // Get RAID info & Phy info. Need to match the RAID config
                                // (below) to some of the phy info as best we can...-TJE
#    if defined(_WIN32)
...
#    endif // CSMI_DEBUG
                                ...
                                // array is used to track which phys have already been scanned
...
#    endif // CSMI_DEBUG
                                        ...
                                        {
#    if defined(CSMI_DEBUG)
                                            printf("GDL: Checking drive usage in each RAID config. "
                                                   "Max Drives per set: %" CPRIu32 "\tDrive Count: %" CPRIu8 "\n",
                                                   csmiRAIDInfo.Information.uMaxDrivesPerSet,
                                                   csmiRAIDConfig->Configuration.bDriveCount);
#    endif // CSMI_DEBUG
           // make sure we got all the drive information...if now, we need to
           // reallocate with some more memory
                                            for (uint32_t iter = 0; iter < csmiRAIDConfig->Configuration.bDriveCount &&
                                                                    iter < csmiRAIDInfo.Information.uMaxDrivesPerSet &&
                                                 ...
                                                 ++iter)
                                            {
                                                ...
                                                bool driveInfoValid = true; // for version 81 and earlier, assume this
                                                                            // is true.
#    if defined(CSMI_DEBUG)
...
#    endif // CSMI_DEBUG
                                                ...
                                                {
#    if defined(CSMI_DEBUG)
                                                    printf("GDL: CSMI Minor rev > 81, so checking "
                                                           "bDataType\n");
#    endif // CSMI_DEBUG
                                                    switch (csmiRAIDConfig->Configuration.bDataType)
                                                    {
                                                ...
                                                if (driveInfoValid)
                                                {
                                                    ...
                                                    {
                                                    case CSMI_SAS_DRIVE_CONFIG_NOT_USED:
                                                        // Don't count drives with this flag, because they
                                                        // are not configured in a RAID at this time. We
                                                        // only want those configured in a RAID/RAID-like
                                                        // scenario.
#    if defined(CSMI_DEBUG)
...
#    endif // CSMI_DEBUG
                                                    ...
                                                    case CSMI_SAS_DRIVE_CONFIG_SRT_CACHE:
                                                    case CSMI_SAS_DRIVE_CONFIG_SRT_DATA:
                                                        // Need to setup a handle and try get_Device to see
                                                        // if it works. NOTE: Need to know if on intel AND
                                                        // if model contains "NVMe" because we need to setup
                                                        // that differently to discover it properly
#    if defined(CSMI_DEBUG)
...
#    if defined(_WIN32)
                                                        if (isIntelDriver &&
                                                            strncmp(
                                                                ...
                                                                "NVMe", 4) == 0)
                                                        {
                                                            // This should only happen on Intel Drivers using
                                                            // SRT The SAS Address holds port-target-lun data
                                                            // in it. NOTE: This is correct for this version
                                                            // of the driver, but this is not necessarily true
                                                            // for previous RST drivers according to
                                                            // documentation received from Intel. -TJE
                                                            uint8_t path   = 0;
                                                            uint8_t target = 0;
                                                            uint8_t lun    = 0;
                                                            ...
                                                            path = csmiRAIDConfig->Configuration.Drives[iter]
                                                                       .bSASAddress[2];
                                                            // don't know which bytes hold target and
                                                            // lun...leaving as zero since they are
                                                            // TECHNICALLY reserved in the documentation
                                                            //\\.\SCSI?: number is needed in windows, this is
                                                            // the controllerNumber in Windows.
                                                            snprintf(handle, RAID_HANDLE_STRING_MAX_LEN,
                                                                     "csmi:%" CPRIu8 ":N:%" CPRIu8 ":%" CPRIu8
                                                                     ":%" CPRIu8,
                                                                     controllerNumber, path, target, lun);
                                                            foundDevice = true;
#        if defined(CSMI_DEBUG)
                                                            printf("GDL: Intel NVMe detected, setting up "
                                                                   "handle as %s\n",
                                                                   handle);
#        endif // CSMI_DEBUG
                                                        }
...
#    endif                                                   //_WIN32
                                                        {
#    if defined(CSMI_DEBUG)
                                                            printf("GDL: Standard CSMI detected. Checking "
                                                                   "phy info. Number of Phys: %" CPRIu8 "\n",
                                                                   phyInfo.Information.bNumberOfPhys);
#    endif // CSMI_DEBUG
           // Compare this drive info to phy info as best we can using SASAddress
           // field. NOTE: If this doesn't work on some controllers, then this will
           // get even more complicated as we will need to try other CSMI commands
           // and attempt reading drive identify or inquiry data to make the match
           // correctly!!! Loop through phy info and find matching SAS
           // address...should only occur ONCE even with multiple Luns since they
           // attach to the same Phy
                                                            for (uint8_t phyIter = 0, physFound = 0;
                                                                 !foundDevice && phyIter < 32 &&
                                                                 physFound < phyInfo.Information.bNumberOfPhys;
                                                                 ++phyIter)
                                                            {
#    if defined(CSMI_DEBUG)
                                                                printf("GDL: Comparing SAS address to RAID "
                                                                       "config SAS address\n");
#    endif // CSMI_DEBUG
...
#    endif // CSMI_DEBUG
                                                                ...
                                                                ++physFound; // increment since we have found a
                                                                             // valid phy to check information
                                                                             // on.
                                                                if (matchedPhys[phyIter] == false)
                                                                {
                                                                    // NOTE: SATA controllers will set SASAddress
                                                                    // to zero (unless It's Intel, they fill this
                                                                    // in anyways), so this is not enough of a
                                                                    // check.
                                                                    //       If there is a non-zero SASAddress,
                                                                    //       use it. Otherwise, we need to roll
                                                                    //       back to matching MN, SN, with an
                                                                    //       Identify command -TJE
                                                                    // Removed special case for Intel since that
                                                                    // seems to only work for specific versions of
                                                                    // the drive and specific configurations.
                                                                    if ((!is_Empty(
                                                                    ...
                                                                    {
#    if defined(CSMI_DEBUG)
                                                                        printf("GDL: Matching SAS address in Phy "
                                                                               "info found\n");
#    endif // CSMI_DEBUG
                                                                        uint8_t lun = 0;
...
#    endif // CSMI_DEBUG
           // This would be a multi-lun SAS drive. This device and the driver should
           // actually be able to translate SASAddress and SASLun to a SCSI address
           // for us.
...
#    endif // CSMI_DEBUG
                                                                    ...
                                                                    {
                                                                        // This is most likely a SATA drive on a
                                                                        // SATA controller. Since we do not have a
                                                                        // SAS address to use for matching, we need
                                                                        // to issue an identify command and match
                                                                        // the MN and SN.
#    if defined(CSMI_DEBUG)
                                                                        printf("GDL: No SASAddress, so matching "
...
#    endif // CSMI_DEBUG
                                                                        ...
                                                                        remove_Leading_And_Trailing_Whitespace(
                                                                            csmiRaidDevSerial);
                                                                        // Creating a temporary tDevice structure to
                                                                        // use for the passthrough commands.-TJE
                                                                        tDevice tempDevice;
                                                                        tempDevice.os_info.minimumAlignment = sizeof(
                                                                            void*); // setting alignment this way
                                                                                    // to be compatible across OSs
                                                                                    // since CSMI doesn't really
                                                                                    // dictate an alignment, but we
                                                                                    // should set something. - TJE
                                                                        tempDevice.issue_io =
                                                                            C_CAST(issue_io_func, send_CSMI_IO);
                                                                        tempDevice.drive_info.drive_type =
                                                                            SCSI_DRIVE; // assume SCSI for now. Can
                                                                                        // be changed later
...
#    endif // CSMI_DEBUG
                                                                        ...
                                                                        csmiPTCmd.senseDataSize = SPC3_SENSE_LEN;
                                                                        // Don't have a SAS Address to match to, so
                                                                        // we need to send an identify or inquiry to
                                                                        // the device to see if it is the same MN,
                                                                        // then check the SN. NOTE: This will not
                                                                        // work if we don't already know the sasLUN
                                                                        // for SAS drives. SATA will be ok though.
                                                                        tempDevice.os_info.csmiDeviceData
                                                                            ->portIdentifier =
                                                                            phyInfo.Information.Phy[phyIter]
                                                                                .bPortIdentifier;
                                                                        ...
                                                                        memcpy(&tempDevice.os_info.csmiDeviceData
                                                                                    ->sasAddress[0],
                                                                               phyInfo.Information.Phy[phyIter]
                                                                                   .Attached.bSASAddress,
                                                                               8);
                                                                        // Attempt passthrough command and compare
                                                                        // identifying data. for this to work,
                                                                        // SCSIIoCTX structure must be manually
                                                                        // defined for what we want to do right now
                                                                        // and call the CSMI IO directly...not
                                                                        // great, but don't want to have other force
...
#    if defined(CSMI_DEBUG)
                                                                                    ...
                                                                                    printf(
                                                                                        ...
                                                                                        "since ATAPI should use "
                                                                                        "system handle instead.\n");
#    endif // CSMI_DEBUG
                                                                                    if (strstr(ataMN,
                                                                                               csmiRaidDevModel) &&
                                                                                        strstr(ataSN,
...
#    endif // CSMI_DEBUG
                                                                                 ...
                                                                                 CSMI_SAS_PROTOCOL_SSP)
                                                                        {
                                                                            // SCSI Inquiry and read unit serial
                                                                            // number VPD page
...
#    endif // CSMI_DEBUG
                                                                            if (SUCCESS == send_CSMI_IO(&csmiPTCmd))
                                                                            {
                                                                                ...
                                                                                memcpy(inqVendor, &inqData[8], 8);
                                                                                memcpy(inqProductID, &inqData[16], 16);
                                                                                // memcpy(inqProductRev, &inqData[32],
                                                                                // 4);
                                                                                snprintf(
...
#    endif // CSMI_DEBUG
           ...
           // identification and product identification fields from a standard
           // INQUIRY
#    if defined(CSMI_DEBUG)
                                                                        ...
                                                                        safe_free_csmi_dev_info(
                                                                            &tempDevice.os_info.csmiDeviceData);
                                                                             ...
                                                                             !is_Empty(csmiRAIDConfig->Configuration
                                                                                           .Drives[iter]
                                                                                           .bSerialNumber,
                                                                                       40)) // MN is empty, but SN is
                                                                                            // not. Missing drive from
                                                                                            // the set.
                                                                    {
                                                                        // in this case the MN is blank, but there
                                                                        // is a SN. So this is likely a case of a
                                                                        // missing drive, so do NOT scan the phyinfo
#    if defined(CSMI_DEBUG)
...
#    if defined(CSMI_DEBUG)
                                                                        printf("GDL: Logging error for iter = %" PRIu8
                                                                               "\n",
                                                                               phyIter);
                                                                        printf("GDL: Cannot use SASAddress or "
                                                                               "MN+SN to match drives. Trying "
                                                                               "final possibility: PhyInfo\n");
#    endif // CSMI_DEBUG
                                                                    }
                                                                }
                                                            }
...
#    endif // CSMI_DEBUG
                                                            ...
                                                            // get_CSMI_Device, then  we need to increment the
                                                            // failure counter. - TJE
                                                            ++found;
                                                        }
                                                        break;
                                ...
                                {
                                    // So there is not any other method that will work to match
                                    // devices. So this will rely strictly on the PhyInfo. This
                                    // has a lot of draw backs. There is nothing that can be done
                                    // to ensure we find all drives, we also may end up finding
                                    // duplicates, or even extras. Solving these problems is
                                    // probably possible by removing duplicates at the end, but
                                    // that is far from optimal -TJE
                                    for (uint8_t phyIter = 0, physFound = 0;
                                         phyIter < 32 && physFound < phyInfo.Information.bNumberOfPhys &&
                                         found < numberOfDevices;
                                         ++phyIter)
                                    {
                                        if (phyInfo.Information.Phy[phyIter].Attached.bDeviceType ==
...
#    endif // CSMI_DEBUG
                                        ...
                                        ++physFound;               // increment since we have found a valid phy to
                                                                   // check information on.
                                        if (!matchedPhys[phyIter]) // only do this for phys we did
                                                                   // not already scan successfully
                                                                   // above-TJE
                                        {
#    if defined(CSMI_DEBUG)
...
#    endif // CSMI_DEBUG
           // Each attached device will be considered a "found device" in this case.
                                            ...
                                            snprintf(handle, RAID_HANDLE_STRING_MAX_LEN,
                                                     ...
                                                     controllerNumber, phyInfo.Information.Phy[phyIter].bPortIdentifier,
                                                     phyInfo.Information.Phy[phyIter].Attached.bPhyIdentifier, 0);
#    if defined(CSMI_DEBUG)
                                            printf("GDL: Phy Info last resort device handle found "
                                                   "and set as %s\n",
                                                   handle);
#    endif // CSMI_DEBUG
                                            memset(d, 0, sizeof(tDevice));
...
#    endif // CSMI_DEBUG
                                            ...
                                            {
                                                // ATAPI drives can show up, but we do not need to scan
                                                // them with CSMI, so check if it is ATAPI or not! ATAPI
                                                // devices will not be part of a RAID, so we do not need
                                                // to add them in CSMI code. If we want to show them in
                                                // the software, windows can scan \\.\CDROM<id> handles
#    if defined(CSMI_DEBUG)
                                                printf("GDL: Found ATAPI drive. Skipping.\n");
#    endif // CSMI_DEBUG
           // memset the device because it can still show up and we do not want it
           // to leave anything behind if we are reusing the device structure for a
           // different device.
                                                memset(d, 0, sizeof(tDevice));
                                                continue;
                                            }
...
#    endif //_WIN32
                if (!handleRemoved)
                {
                    previousRaidListEntry = raidList; // store handle we just looked at in case we need to
...
static eReturnValues send_SSP_Passthrough_Command(ScsiIoCtx* scsiIoCtx)
{
    ...
    sspInputs.flags = CSMI_SAS_SSP_TASK_ATTRIBUTE_SIMPLE; // start with this. don't really care
...
static eReturnValues send_STP_Passthrough_Command(ScsiIoCtx* scsiIoCtx)
{
    ...
    case ATA_PROTOCOL_DEV_DIAG:
        stpInputs.flags |= CSMI_SAS_STP_EXECUTE_DIAG; // note: cast is to remove a warning that
                                                      // only shows up on this flag due to its
                                                      // value.
        break;
    ...
    if (stpOutputs.retryAsSSPPassthrough)
    {
        ...
        // First, try SAT (no changes), if that fails for invalid operation code,
        // then try legacy CSMI...after that, we are done retrying.
        ret = send_SSP_Passthrough_Command(scsiIoCtx); // This is all we should have to do since SAT style CDBs are
    ...
    else if (ret == SUCCESS)
    {
        ...
        ataReturnTFRs      rtfrs; // create this temporarily to save fis output results,
                                  // then we'll pack it into sense data
        ...
        // dummy up sense data since that is what the above layers look for. Use
        // descriptor format.
...
void print_CSMI_Device_Info(tDevice* device)
{
    ...
    {
        // print the things we stored since those are what we currently care about.
        // Can add printing other things out later if they are determined to be of
        // use. - TJE
        printf("\n=====CSMI Info=====\n");
