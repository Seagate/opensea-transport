...
// This could be useful to put into a function for all nix systems to use since
// it could be useful for them too.
...
eReturnValues get_Device(const char* filename, tDevice* device)
{
    ...
    if (isScsi)
    {
        ...
        if ((device->os_info.fd >= 0) && (ret == SUCCESS))
        {
            ...
            // this code to set up passthrough commands for USB and IEEE1394 has been
            // removed for now to match Windows functionality. Need better
    ...
    else
    {
        ...
        // Set NSID from the incoming handle. It's not clear if this is correct, but
        // based on the vmware structures, the "cookie" in the adapter info says it
        // points to the controller, so currently assuming this is a reasonable way
        // to read the nsid.
...
void print_usr_io_struct(struct usr_io uio)
{
    ...
    printf("\tStatus: %" PRIX16 "\n",
           uio.status); // If this starts with 0x0BADxxxx then it is indicating an
                        // error. Use vmware API to translate it.
...
#    endif
    ...
    case NVM_ADMIN_CMD:
        ...
        {
            uio.namespaceID = C_CAST(vmk_uint8, -1); // this is what the header files say to do for
                                                     // non-specific namespace -TJE
        }
        else
        {
        ...
        // Get error?
        if (ioctlret < 0 ||
            (uio.status & 0x0FFF0000) == 0x0BAD0000) // If this starts with 0x0BADxxxx then it is indicating
                                                     // an error. Use vmware API to translate it.
        {
...
{
    // This is a stub. If this is possible, this should perform an nvme subsystem
    // reset;
    return OS_COMMAND_NOT_AVAILABLE;
}
