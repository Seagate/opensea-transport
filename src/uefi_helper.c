...
//
// Copyright (c) 2017-2024 Seagate Technology LLC and/or its Affiliates, All
// Rights Reserved
//
...
// Define this to turn on extra prints to the screen for debugging UEFI
// passthrough issues. #define UEFI_PASSTHRU_DEBUG_MESSAGES

#if defined(UEFI_PASSTHRU_DEBUG_MESSAGES)
// This color will be used for normal debug messages that are infomative.
// Critical memory allocation errors among other things are going to be
// CONSOLE_COLOR_RED.
...
eReturnValues get_Passthru_Protocol_Ptr(EFI_GUID ptGuid, void** pPassthru, uint32_t controllerID)
{
    ...
    // NOTE: This code below assumes that the caller knows the controller they
    // intend to open. Meaning they've already done some sort of system scan.
...
static bool get_ATA_Device_Handle(const char* filename, uint16_t* controllerID, uint16_t* port, uint16_t* pmport)
{
    ...
    {
        char* dup = strdup(filename); // before tokenizing, always duplicate the original
                                      // string since tokenizing will modify it.
        if (dup)
        {
#define MAX_ATA_HANDLE_FIELDS UINT8_C(4) // ata:<controllerID>:<port>:<portMultiplierPort>
            ...
            success               = true; // set to true so we can exit the loop quickly if an error
                                          // is detected during parsing
...
#define MAX_SCSI_HANDLE_FIELDS UINT8_C(4) // scsiEx:<controllerID>:<target>:<lun> //16, 128, 64
                ...
                ;
            {
                ...
                case 2: // target //FIXME: Does not handle full 128bit targetID
                    // first try seeing if the target is less than 128bits and a 64bit
                    // conversion will be enough
                    endptr = M_NULLPTR;
                    errno  = 0; // clear to zero as stated in ISO C secure coding
                    btemp  = strtoull(token, &endptr, 16);
                    if ((btemp == ULLONG_MAX && errno == ERANGE) || (btemp == 0 && token == endptr))
                    {
                        // Either the target is a much larget value than 64bits can hold, or
                        // there was a parsing error. First see if we can seperate it into
                        // two pieces to parse before deciding this was a failure
                        ...
                        if (targetstr)
                        {
                            ...
                            if (delimiter)
                            {
                                // change the next : to a null so it's easier to break the
                                // string into two parts
                                *delimiter           = '\0';
                                size_t halftargetlen = safe_strlen(targetstr) / 2;
                                if (safe_strlen(targetstr) % 2)
                                {
                                    // this is supposed to be evenly divisible by 2 as we require
                                    // the user to type the full string!
                                    success = false;
                                }
                                else
                                {
                            ...
                            {
                                // missing the next delimiter, which is not supposed to happen,
    ...
    if (strstr(filename, "ata") == filename) // this can only be at the beginning of the string
    {
                                  ...
                                  &device->os_info.address.ata.portMultiplierPort))
        {
            ...
            if (SUCCESS == get_ATA_Passthru_Protocol_Ptr(&pPassthru, device->os_info.controllerNum))
            {
                ...
                *devicePath; // will be allocated in the call to the uefi systen
                ...
                {
                    memcpy(&device->os_info.devicePath, devicePath,
                    ...
                    {
                        // If the driver is running in IDE mode, it will set the pmport
                        // value to non-zero for device 1. Because of this, and some sample
                        // EDK2 code, we need to make sure we set the device 1 bit when
                        // issuing commands!
                    ...
                    printf("Protocol Mode = %d\n",
                           instance->Mode); // 0 means IDE, 1 means AHCI, 2 means RAID,
                                            // but we shouldn't see RAID here ever.
...
eReturnValues send_UEFI_SCSI_Passthrough(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (SUCCESS == get_SCSI_Passthru_Protocol_Ptr(&pPassthru, scsiIoCtx->device->os_info.controllerNum))
    {
        ...
        else
        {
            ...
            {
                srp->Timeout = 15 * 1e7; // 15 seconds. value is in 100ns units. zero
        ...
        if (pPassthru->Mode->IoAlign > 1 && !IS_ALIGNED(scsiIoCtx->pdata, pPassthru->Mode->IoAlign))
        {
            ...
            localBuffer =
                C_CAST(uint8_t*, safe_calloc_aligned(scsiIoCtx->dataLength, sizeof(uint8_t),
                                                     pPassthru->Mode->IoAlign > 0 ? pPassthru->Mode->IoAlign : 1));
            if (!localBuffer)
            {
#if defined(UEFI_PASSTHRU_DEBUG_MESSAGES)
        ...
        if (pPassthru->Mode->IoAlign > 1 && !IS_ALIGNED(scsiIoCtx->cdb, pPassthru->Mode->IoAlign))
        {
            // allocate an aligned buffer here!
            localCDB =
                C_CAST(uint8_t*, safe_calloc_aligned(scsiIoCtx->cdbLength, sizeof(uint8_t),
                                                     pPassthru->Mode->IoAlign > 0 ? pPassthru->Mode->IoAlign : 1));
            if (!localCDB)
            {
#if defined(UEFI_PASSTHRU_DEBUG_MESSAGES)
        ...
        if (pPassthru->Mode->IoAlign > 1 && !IS_ALIGNED(scsiIoCtx->psense, pPassthru->Mode->IoAlign))
        {
            ...
            localSensePtr =
                C_CAST(uint8_t*, safe_calloc_aligned(scsiIoCtx->senseDataSize, sizeof(uint8_t),
                                                     pPassthru->Mode->IoAlign > 0 ? pPassthru->Mode->IoAlign : 1));
            if (!localSensePtr)
            {
#if defined(UEFI_PASSTHRU_DEBUG_MESSAGES)
        ...
        case XFER_NO_DATA:
            srp->DataDirection = 0;
            break;
        case XFER_DATA_OUT_IN: // bidirectional command support not allowed with
                               // this type of passthru
#if defined(UEFI_PASSTHRU_DEBUG_MESSAGES)
            set_Console_Colors(true, uefiDebugMessageColor);
...
eReturnValues send_UEFI_SCSI_Passthrough_Ext(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (SUCCESS == get_Ext_SCSI_Passthru_Protocol_Ptr(&pPassthru, scsiIoCtx->device->os_info.controllerNum))
    {
        ...
        *srp; // Extended scsi request packet

        srp = C_CAST(EFI_EXT_SCSI_PASS_THRU_SCSI_REQUEST_PACKET*,
...
eReturnValues send_UEFI_ATA_Passthrough(ScsiIoCtx* scsiIoCtx)
{
    ...
    if (SUCCESS == get_ATA_Passthru_Protocol_Ptr(&pPassthru, scsiIoCtx->device->os_info.controllerNum))
    {
        ...
        else
        {
            ...
            {
                ataPacket->Timeout =
                    scsiIoCtx->pAtaCmdOpts->timeout * 1e7; // value is in 100ns units. zero means wait indefinitely
            }
            else
            {
                ataPacket->Timeout = 15 * 1e7; // 15 seconds. value is in 100ns units.
        ...
        if (pPassthru->Mode->IoAlign > 1 && !IS_ALIGNED(scsiIoCtx->pAtaCmdOpts->ptrData, pPassthru->Mode->IoAlign))
        {
#if defined(UEFI_PASSTHRU_DEBUG_MESSAGES)
            set_Console_Colors(true, MAGENTA);
            printf("scsiIoCtx->pAtaCmdOpts->ptrData is not aligned! Creating local "
                   "aligned buffer\n");
            set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
#endif
            // allocate an aligned buffer here!
            localAlignedBuffer = true;
            localBuffer =
                C_CAST(uint8_t*, safe_calloc_aligned(scsiIoCtx->pAtaCmdOpts->dataSize, sizeof(uint8_t),
                                                     pPassthru->Mode->IoAlign > 0 ? pPassthru->Mode->IoAlign : 1));
            if (!localBuffer)
            {
#if defined(UEFI_PASSTHRU_DEBUG_MESSAGES)
        ...
        // Set the passthrough length data (where it is, bytes, etc) (essentially
        // building an SAT ATA pass-through command)
        ataPacket->Length |= EFI_ATA_PASS_THRU_LENGTH_BYTES; // ALWAYS set this. We will always set
                                                             // the transfer length as a number of
                                                             // bytes to transfer.
        // Setting 512B vs 4096 vs anything else doesn't matter in UEFI since we can
        // set number of bytes for our transferlength anytime.
        ...
        stop_Timer(&commandTimer);
        // convert return status from sending the command into a return value for
        // opensea-transport
#if defined(UEFI_PASSTHRU_DEBUG_MESSAGES)
        set_Console_Colors(true, uefiDebugMessageColor);
        ...
        if (Status == EFI_SUCCESS)
        {
            ...
            ret = SUCCESS;
            // convert RTFRs to sense data since the above layer is using SAT for
            // everthing to make it easy to port across systems
        ...
        else // error, set appropriate return code
        {
            ...
            {
                // command failed. Not sure if this should be dummied up as 51h - 04h or
                // not.
                ret = OS_PASSTHROUGH_FAILURE;
            }
...
#    endif
        ...
        else
        {
            ...
            {
                nrp->CommandTimeout = 15 * 1e7; // 15 seconds. value is in 100ns units.
                                                // zero means wait indefinitely
            }
        }

        // This is a hack for now. We should be enforcing pointers and data transfer
        // size on in, our, or bidirectional commands up above even if nothing is
        // expected in the return data buffer - TJE
...
#    endif
            localAlignedBuffer = true;
            localBuffer =
                C_CAST(uint8_t*, safe_calloc_aligned(M_Max(512, nvmeIoCtx->dataSize), sizeof(uint8_t),
                                                     pPassthru->Mode->IoAlign > 0 ? pPassthru->Mode->IoAlign : 1));
            if (!localBuffer)
            {
#    if defined(UEFI_PASSTHRU_DEBUG_MESSAGES)
                set_Console_Colors(true, CONSOLE_COLOR_RED);
                printf("Failed to allocate memory for an aligned data pointer - "
                       "missing buffer on data xfer command!\n");
                set_Console_Colors(true, CONSOLE_COLOR_DEFAULT);
#    endif
                return MEMORY_FAILURE;
            }
...
#    endif
            ...
            localAlignedBuffer = true;
            localBuffer =
                C_CAST(uint8_t*, safe_calloc_aligned(nvmeIoCtx->dataSize, sizeof(uint8_t),
                                                     pPassthru->Mode->IoAlign > 0 ? pPassthru->Mode->IoAlign : 1));
            if (!localBuffer)
            {
#    if defined(UEFI_PASSTHRU_DEBUG_MESSAGES)
...
#    endif
        ...
        // TODO: Handle metadata pointer...right now attempting to allocate
        // something locally here just to get things working.
        nrp->MetadataBuffer =
...
uint32_t get_ATA_Device_Count()
{
    ...
    while (counter < nodeCount)
    {
        ...
        uint16_t port = UINT16_MAX; // start here since this will make the api find
                                    // the first available ata port
        while (uefiStatus == EFI_SUCCESS)
        {
            ...
            if (uefiStatus == EFI_SUCCESS && port != UINT16_MAX)
            {
                ...
                uint16_t pmport = UINT16_MAX; // start here so we can find the first
                ...
                while (getNextDevice == EFI_SUCCESS)
                {
                    ...
                    {
                        // we have a valid port - port multiplier port combination. Try
                        // "probing" it to make sure there is a device by using build device
                        // path
                        EFI_DEVICE_PATH_PROTOCOL
                        *devicePath; // will be allocated in the call to the uefi systen
                        ...
                        // EFI_INVALID_PARAMETER means DevicePath is null (this function
                        // should allocate the path for us according to the API)
...
eReturnValues get_ATA_Devices(tDevice* const ptrToDeviceList, uint32_t sizeInBytes, versionBlock ver, uint32_t* index)
{
    ...
    if (uefiStatus == EFI_NOT_FOUND)
    {
        ...
        if (deviceCount > 0)
        {
            // assuming that since we enumerated something, that everything worked and
            // we are able to talk to something
            ret = SUCCESS;
        }
    }
    return ret;
...
uint32_t get_SCSI_Device_Count()
{
    ...
    while (counter < nodeCount)
    {
        ...
        uint32_t target = UINT32_MAX; // start here since this will make the api
                                      // find the first available scsi target
        ...
        while (uefiStatus == EFI_SUCCESS)
        {
            ...
            if (uefiStatus == EFI_SUCCESS && target != UINT16_MAX && lun != UINT64_MAX)
            {
                ...
                *devicePath; // will be allocated in the call to the uefi systen
                ...
                // EFI_NOT_FOUND means no device at this place.
                // EFI_INVALID_PARAMETER means DevicePath is null (this function should
                // allocate the path for us according to the API) EFI_OUT_OF_RESOURCES
...
uint32_t get_NVMe_Device_Count()
{
    ...
    while (counter < nodeCount)
    {
        ...
        uint32_t namespaceID = UINT32_MAX; // start here since this will make the api find the first
...
//!                        memory
//!						   allocated must be the
//! multiple of device structure. 						   The application can pass in less memory than
//! needed 						   for all devices in the system, in which case the library
//!                        will fill the provided memory with how ever many
//!                        device
...
{
    // edk2 might have some kind of partition function we can call for the device
    // to erase it
    return NOT_SUPPORTED;
}
