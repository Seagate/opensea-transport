...
{
    // first make sure the current pointer is valid, if not it is most likely the
    // beginning of the list, so it needs to be allocated
    if (currentPtr)
    {
...
// entry after "toRemove", which can be M_NULLPTR If previous is M_NULLPTR, then
// this is the beginning of the list. This is allowed. Previous is used to
// update the previous entry's next pointer to make sure the list is still
// functional
