                                  ...
                                  uint8_t                modeValue)
{
    ...
    {
        // this is preferred...set the "fastest" bit to let the bridge determine the
    ...
    // all remaining bytes are marked reserved in the spec. Comments are thoughts
    // of things to "try" if we ever care enough and need to debug one of these
    // super old products
...
eReturnValues send_TI_Legacy_Passthrough_Command(tDevice* device, ataPassthroughCommand* ataCommandOptions)
{
    ...
    if (ret == SUCCESS)
    {
        ...
        // dummy up RTFRs since the TI documentation doesn't have a way to show
        // them. This is all based on how we interpret the scsi sense data in the
        // scsi level.
        switch (ret)
        {
