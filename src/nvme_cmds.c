...
eReturnValues nvme_Cmd(tDevice* device, nvmeCmdCtx* cmdCtx)
{
    ...
    // check the opcode bits for data direction and the data direction and make
    // sure they match! If they don't match, return an error for BAD_PARAMETER.
    // Most OS passthroughs parse the op code to figure this out The enum helps us
    // confirm we know what the command sender is intending to do and it will get
    // done correctly!
    ...
    {
        // didn't get a status for one reason or another, so clear out anything that
        // may have been left behind from a previous command.
...
eReturnValues nvme_Abort_Command(tDevice* device, uint16_t commandIdentifier, uint16_t submissionQueueIdentifier)
{
    ...
    //  3h = The number of concurrently outstanding Abort commands has exceeded
                                              ...
                                              uint8_t* asynchronousEventType)
{
    ...
    ret = nvme_Cmd(device, &adminCommand);
    // Command specific return codes:
    //  5h = The number of concurrently outstanding Asynchronous Event Request
...
eReturnValues nvme_Device_Self_Test(tDevice* device, uint32_t nsid, uint8_t selfTestCode)
{
    ...
    ret = nvme_Cmd(device, &adminCommand);
    // Command specific return codes:
    //  1Dh = The controller or NVM subsystem already has a device self-test
                                   ...
                                   uint32_t           timeoutSeconds)
{
    ...
    {
        FirmwareCommit.timeout = 30; // default to 30 seconds since some images may
    ...
    // removed all the code checking for specific status code since it was not
