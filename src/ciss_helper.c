...
// ******************************************************************************************
// \file ciss_helper.c
// \brief Defines the constants structures to help with CISS implementation.
// This attempts to be generic for any unix-like OS. Windows support is through
// CSMI.
...
#    if defined(__unix__) // this is only done in case someone sets weird defines
                          // for Windows even though this isn't supported
#        include <ctype.h>
#        include <dirent.h>
#        include <fcntl.h>
...
// define this macro before these includes. Doesn't need to match exactly, just
// make the compiler happy.-TJE
#        ifndef __user
#            define __user
#        endif //__user
...
// All it does is attempts an easy ioctl to get some basic info from the
// controller, which may vary between OSs
...
// If Linux SMARTPQI does not respond to this, maybe try some of the other
// non-passthrough IOCTLs to see if we get a valid response-TJE
#        if defined(__FreeBSD__)
    else
    {
    ...
    if (input)
    {
        ...
        else
        {
            ...
            if (input[0] == 'c')
            {
                ...
                char* str    = M_CONST_CAST(char*, input) + 1; // need to update str pointer as we parse the handle, but
...
static uint8_t parse_CISS_Handle(const char* devName, char* osHandle, uint16_t* physicalDriveNumber)
{
    ...
    if (devName)
    {
        // TODO: Check the format of the handle to see if it is ciss format that can
        // be supported
        char* dup = strdup(devName);
        if (strstr(dup, CISS_HANDLE_BASE_NAME) == dup)
        {
            // starts with ciss, so now we should check to make sure we found
            // everything else
            uint8_t counter = 0;
            ...
            while (token && counter < 3)
            {
                ...
                case 0: // ciss - already been validated above
                    ++parseCount;
                    break;
                case 1: // partial system handle...need to append /dev/ to the front at
                        // least. ciss will need /dev/cciss/<> varies a little bit based
                        // on the format here
...
// NOTE: "Other physical device info" does not indicate a length that I can see
// different from the 16B node data, but may be different. -TJE

/*
static eReturnValues ciss_Scsi_Report_Logical_LUNs(tDevice *device, uint8_t
extendedDataType, uint8_t* ptrData, uint32_t dataLength)
{
    eReturnValues ret = SUCCESS;
    DECLARE_ZERO_INIT_ARRAY(uint8_t, cdb, CDB_LEN_12);
    cdb[OPERATION_CODE] = CISS_REPORT_LOGICAL_LUNS_OP;
    cdb[1] = extendedDataType;//can set to receive extended information, but we
don't care about this right now... cdb[2] = RESERVED; cdb[3] = RESERVED; cdb[4]
= RESERVED; cdb[5] = RESERVED; cdb[6] = M_Byte3(dataLength); cdb[7] =
M_Byte2(dataLength); cdb[8] = M_Byte1(dataLength); cdb[9] = M_Byte0(dataLength);
    cdb[10] = RESERVED;
    ...
    {
        ret = scsi_Send_Cdb(device, cdb, CDB_LEN_12, ptrData, dataLength,
XFER_DATA_IN, device->drive_info.lastCommandSenseData, SPC3_SENSE_LEN, 15);
    }
    else
    {
...
// This is a reworked old function from first internal code to support CISS...is
// this still needed like this?-TJE
static eReturnValues get_Physical_Device_Location_Data(tDevice* device, uint8_t* physicalLocationData)
{
    ...
    if (physicalDrives)
    {
        ...
        if (ret == SUCCESS)
        {
            ...
            if (physicalDrives[4] == 0) // regular report. All other values are
                                        // extended reports that we don't need
            {
                if (device->os_info.cissDeviceData->driveNumber <= (lunListLength / PHYSICAL_LUN_DESCRIPTOR_LENGTH))
                {
                    ...
                    memcpy(physicalLocationData,
                           &physicalDrives[(device->os_info.cissDeviceData->driveNumber + 1) *
...
{
    CISS_CMD_CONTROLLER,
    CISS_CMD_LOGICAL_LUN, // not currently used or supported in this code, but may
                          // be added in the future if needed. -TJE
    CISS_CMD_PHYSICAL_LUN
} eCISSptCmdType;

// Standard passthrough
// physicalDriveCmd - true for most things. Only set to false when trying to
// issue a command directly to the controller.
...
#    if defined(__linux__) || defined(__FreeBSD__) // interface is the same on Linux and FreeBSD
            ...
            {
#        if defined(__FreeBSD__)
                // if the smartpqi bool is set, use the structures from that driver to
                // issue the command to ensure that the packing matches exactly to
                ...
                case CISS_CMD_CONTROLLER:
                    break;
                default: // this will work OK for other cases. May need modifications
                         // for logical volume commands. -TJE
                    // set path to device
                    memcpy(&pqiCmd.LUN_info, scsiIoCtx->device->os_info.cissDeviceData->physicalLocation,
                           LUN_ADDR_LEN); // this is 8 bytes in size maximum
                    break;
                }
                // now setup to send a CDB
                pqiCmd.Request.CDBLen    = scsiIoCtx->cdbLength;
                pqiCmd.Request.Type.Type = TYPE_CMD;         // TYPE_MSG also available for BMIC commands, which can be
                                                             // things like resets
                ...
                ret = OS_PASSTHROUGH_FAILURE; // OS_COMMAND_NOT_AVAILABLE,
                                              // OS_COMMAND_BLOCKED
...
#        endif
            ...
            else
            {
                ...
                default: // this will work OK for other cases. May need modifications
                         // for logical volume commands. -TJE
                    // set path to device
                    memcpy(&cissCmd.LUN_info, scsiIoCtx->device->os_info.cissDeviceData->physicalLocation,
                           LUN_ADDR_LEN); // this is 8 bytes in size maximum
                    break;
                }
                // now setup to send a CDB
                cissCmd.Request.CDBLen    = scsiIoCtx->cdbLength;
                cissCmd.Request.Type.Type = TYPE_CMD;         // TYPE_MSG also available for BMIC commands, which can be
                                                              // things like resets
...
#    elif defined(__sun)
            ...
            case CISS_CMD_CONTROLLER:
                break;
            default: // this will work OK for other cases. May need modifications for
                     // logical volume commands. -TJE
                // set path to device
                memcpy(&cissCmd.lun_addr, scsiIoCtx->device->os_info.cissDeviceData->physicalLocation,
                       LUN_ADDR_LEN); // this is 8 bytes in size maximum
                break;
            }
            ...
            // NOTE: The .buf is a uint64. Not sure if this should be a pointer value,
            // or overallocated for buffer space
...
eReturnValues issue_io_ciss_Dev(ScsiIoCtx* scsiIoCtx)
{
    ...
    {
#    if defined(CCISS_BIG_PASSTHRU)
        // use big passthrough only when making a large enough data transfer to
        // matter - TJE
...
eReturnValues get_CISS_RAID_Device(const char* filename, tDevice* device)
{
    ...
    if (PARSE_COUNT_SUCCESS == parse_CISS_Handle(filename, handlePtr, &driveNumber))
    {
        ...
        if (device->os_info.cissDeviceData)
        {
            ...
            {
                // check that CISS IOCTLs are available. If not, then we don't want to
                // proceed. There is danger in attempting a vendor unique op code as we
                // don't know how the target will respond to it unless we know it really
                // is a CISS device
                if (supports_CISS_IOCTLs(device->os_info.cissDeviceData->cissHandle))
                {
                    // setup all the things we need to be able to issue commands in the
                    // code
                    ...
                    device->os_info.fd =
                        device->os_info.cissDeviceData->cissHandle; // set this just to make the upper layers that
                                                                    // validate this happy-TJE
                    device->drive_info.passThroughHacks.ataPTHacks.alwaysUseDMAInsteadOfUDMA =
                        true; // this may not be true for all CISS controllers, but will
                              // likely work with any of them -TJE
                    // handle opened, now get the physical device location from the C2h
                    // command This is done here to reuse lots of other code to issue
                    // commands.
                    if (SUCCESS == (ret = get_Physical_Device_Location_Data(
                                        device, device->os_info.cissDeviceData->physicalLocation)))
                    {
                        // finally call fill_Drive_Info to let opensea-transport set it's
                        // support bits and other device information
...
{
    // This is a special function only used by get device count and get device
    // list. It creates a psuedo tDevice to issue a scsiIoCtx structure to
    // passthrough and read this. This keeps the rest of the code simpler for now
    eReturnValues ret = SUCCESS;
    if (count)
    {
        tDevice   pseudoDev;
        ...
        if (data)
        {
            ...
            // setup the cdb
            cdb[OPERATION_CODE] = CISS_REPORT_PHYSICAL_LUNS_OP;
            cdb[1]              = 0; // no extended data as it is not needed and this will maximize
                                     // compatibility with controllers and firmwares.
            cdb[2]  = RESERVED;
            cdb[3]  = RESERVED;
            cdb[4]  = RESERVED;
            cdb[5]  = RESERVED;
            ...
            if (ret == SUCCESS)
            {
                ...
                if (data[4] == 0) // regular report. All other values are extended
                                  // reports that we don't need
                {
                                         ...
                                         ptrRaidHandleToScan*   beginningOfList)
{
    ...
    while (raidList)
    {
        ...
        if (raidList->raidHint.cissRAID || raidList->raidHint.unknownRAID)
        {
            ...
            // check if the passed in handle contains /dev/ or not so we can open it
            // correctly
            ...
            if ((fd = open(deviceName, O_RDWR | O_NONBLOCK)) >= 0) // TODO: Verify O_NONBLOCK on FreeBSD and/or Solaris
            {
                if (supports_CISS_IOCTLs(fd))
                {
                    ...
                    // Need to call get_Device() and issue this SCSI CDB to read the
                    // physical location data, which will tell us how many IDs are
                    // available and we will then have a count of devices on this handle.
                    ...
                    {
                        // if the first entry in the list was removed, we need up update the
                        // pointer before we exit so that the code that called here won't
            ...
            if (raidList->raidHint.cissRAID || raidList->raidHint.unknownRAID)
            {
                    ...
                    0) // TODO: Verify O_NONBLOCK on FreeBSD and/or Solaris
                {
                    if (supports_CISS_IOCTLs(fd))
                    {
                        ...
                        // Need to call get_Device() and issue this SCSI CDB to read the
                        // physical location data, which will tell us how many IDs are
                        // available and we will then have a count of devices on this
                        // handle.
                        uint32_t countDevsOnThisHandle = 0;
                        // read the physical location data and count how many IDs are
                        // reported
                        if (SUCCESS == get_CISS_Physical_LUN_Count(fd, &countDevsOnThisHandle))
                        {
                            // from here we need to do get_CISS_Device on each of these
                            // available physical LUNs
                            for (uint32_t currentDev = 0; currentDev < countDevsOnThisHandle; ++currentDev)
                            {
                                ...
                                // handle is formatted as "ciss:os_handle:driveNumber" NOTE:
                                // osHandle is everything after /dev/
                                ...
                                ++d;
                                // If we were unable to open the device using get_CSMI_Device,
                                // then  we need to increment the failure counter. - TJE
                                ++found;
                            }
                        }
                        // else something went wrong, and cannot enumerate additional
                        // devices.
