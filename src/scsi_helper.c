...
static ascAscqRetDesc ascAscqLookUp[] = {
    ...
    // {0x2F, 0x07, C_CAST(int, FAILURE),       "Space Allocation Failed Write
    // Protect"},
    //
...
{
    uint16_t crc = 0; // Can also be all F's to invert it. TODO: should invert be
                      // a boolean option to this function? - TJE
...
{
    // we'll only print out a translatable string for seagate drives since fru is
    // vendor specific
...
eReturnValues check_Sense_Key_ASC_ASCQ_And_FRU(tDevice* device,
                                               ...
                                               uint8_t  fru)
{
    eReturnValues ret = UNKNOWN; // if this gets returned from this function, then
    ...
    // now check the asc and ascq combination...this is going to be very large set
    // of switch cases to do this... FYI there is no rhyme or reason to the
    // order...I just went through the massive table in SPC4...and only things for
    // direct access block devices were implemented - TJE
    switch (asc)
    {
    case 0x40:
        switch (ascq)
    ...
    default:
        ...
        if (asc_ascq_result)
        {
            ...
            // Return code of -1 means follow return code determined by sense key, do
            // not change
                                ...
                                uint8_t* fru)
{
    ...
    *fru      = 0;
    ...
    case SCSI_SENSE_DEFER_ERR_DESC:
        ...
        *asc      = pbuf[2];
        *ascq     = pbuf[3];
        // for descriptor format we have to loop through the buffer until we find
        // the FRU descriptor (if available)
        while (iter < SPC3_SENSE_LEN && iter < pbufSize &&
               iter < (C_CAST(uint32_t, additionalSenseLength) + UINT32_C(8)))
        {
            bool    gotFRU           = false;
    ...
    if (ptrSenseData && valid && senseDataLength > 0 && information)
    {
        ...
        *information              = 0;
        ...
        case SCSI_SENSE_DEFER_ERR_DESC:
            ...
            // loop through the descriptors to see if a sense key specific descriptor
            // was provided
                 ...
                 offset += descriptorLength + 2)
            {
                ...
                case SENSE_DESCRIPTOR_INFORMATION:
                    ...
                    *information   = M_BytesTo8ByteValue(ptrSenseData[offset + 4], ptrSenseData[offset + 5],
                                                         ptrSenseData[offset + 6], ptrSenseData[offset + 7],
                                                         ptrSenseData[offset + 8], ptrSenseData[offset + 9],
                                                         ptrSenseData[offset + 10], ptrSenseData[offset + 11]);
                    gotInformation = true;
                    break;
                case SENSE_DESCRIPTOR_DIRECT_ACCESS_BLOCK_DEVICE:
                    *valid         = ptrSenseData[offset + 2] & BIT7;
                    *information   = M_BytesTo8ByteValue(ptrSenseData[offset + 8], ptrSenseData[offset + 9],
                                                         ptrSenseData[offset + 10], ptrSenseData[offset + 11],
                                                         ptrSenseData[offset + 12], ptrSenseData[offset + 13],
        ...
        case SCSI_SENSE_DEFER_ERR_DESC:
                 ...
                 offset += descriptorLength + 2)
            {
                ...
                case SENSE_DESCRIPTOR_COMMAND_SPECIFIC_INFORMATION:
                    *commandSpecificInformation = M_BytesTo8ByteValue(
...
void get_Sense_Key_Specific_Information(uint8_t* ptrSenseData, uint32_t senseDataLength, ptrSenseKeySpecific sksp)
{
    if (ptrSenseData && sksp && senseDataLength > 0)
    {
        ...
        case SCSI_SENSE_DEFER_ERR_DESC:
                 ...
                 offset += descriptorLength + 2)
            {
                ...
                case SENSE_DESCRIPTOR_SENSE_KEY_SPECIFIC:
                    senseKeySpecificOffset = C_CAST(uint8_t,
                                                    offset + 4); // This shouldn't be a problem as offset should never
                                                                 // be larger than 252 in the first place.
        ...
        if (senseKeySpecificOffset > 0U && sksv && returnedLength >= (senseKeySpecificOffset + 2U) &&
            senseDataLength >= (senseKeySpecificOffset + 2U))
        {
            ...
            case SENSE_KEY_NOT_READY:
                ...
                sksp->progress.progressIndication = M_BytesTo2ByteValue(ptrSenseData[senseKeySpecificOffset + 1],
            ...
            case SENSE_KEY_MEDIUM_ERROR:
                ...
                sksp->retryCount.actualRetryCount = M_BytesTo2ByteValue(ptrSenseData[senseKeySpecificOffset + 1],
                                                                        ptrSenseData[senseKeySpecificOffset + 2]);
                break;
...
void get_Sense_Data_Fields(uint8_t* ptrSenseData, uint32_t senseDataLength, ptrSenseDataFields senseFields)
{
    ...
    {
        uint8_t  format         = ptrSenseData[0] & 0x7F; // Stripping the last bit so we just get the format
        uint16_t returnedLength = ptrSenseData[7] + 8;    // offset 7 has additional length. +8 is number of
                                                          // bytes to get to a total length
        uint8_t descriptorLength         = 0;             // for descriptor format sense data
                 ...
                 offset += descriptorLength + 2)
            {
                ...
                case SENSE_DESCRIPTOR_OSD_OBJECT_IDENTIFICATION:
                    senseFields->osdObjectIdentificationDescriptorOffset =
                        C_CAST(uint8_t, offset); // This shouldn't be a problem as offset should
                                                 // never be larger than 252 in the first place.
                    break;
                ...
                case SENSE_DESCRIPTOR_ANOTHER_PROGRESS_INDICATION:
                    if (numOfProgressIndications < MAX_PROGRESS_INDICATION_DESCRIPTORS)
                    {
                        senseFields->anotherProgressIndicationDescriptorOffset[numOfProgressIndications] =
                            C_CAST(uint8_t,
                                   offset); // This shouldn't be a problem as offset should
                                            // never be larger than 252 in the first place.;
                ...
                default: // not a known descriptor
                    ...
                    {
                        senseFields->additionalDataOffset =
                            C_CAST(uint8_t,
                                   offset); // This shouldn't be a problem as offset should
                                            // never be larger than 252 in the first place.
                    }
        ...
        default:
            ...
            // for this, there is no real translation. For these formats everything in
            // scsi says "Vendor unique" however one really old copy of SASI does
            //  define some error codes....so maybe set "sense key" to error class,
            //  asq to "error code" and a bool to say "possibly SASI" somewhere. if
            //  the valid bit is set, then an LBA is reported that we can put into the
            //  information field like fixed format sense data typically does.
            // for SASI and SCSI, the definition is simple. in byte 0, bit 7 is
            // "valid" for LBA address, bits 6-4 is error class, bits 3-0 are error
            // code. In byte 1, bits 7:4 are vendor unique, then all remaiing bits
            // (20) are the LBA
            break;
        }
    }
...
void print_Sense_Fields(ptrSenseDataFields senseFields)
{
    ...
    {
        // This function assumes that the "check_Sense_Key_ASC_ASCQ_FRU" function
        // was called before hand to print out its fields
        ...
        {
            printf("Sense Data Overflow detected! Request sense command is "
                   "recommended to retrieve full sense data!\n");
        }
        ...
        if (!senseFields->fixedFormat)
        {
            // look for other descriptor format data that we saved and can easily
            // parse here
...
eReturnValues check_SAT_Compliance_And_Set_Drive_Type(tDevice* device)
{
    ...
    bool          issueSATIdentify = true; // default to ALWAYS reading this unless
                                           // something else says not to. - TJE
    ...
    {
        // always do this on IDE_INTERFACE since we know it will work here. Doesn't
        // matter if the VPD page read fails or not
        issueSATIdentify = true;
    }
    if (device->drive_info.drive_type == ATAPI_DRIVE || device->drive_info.drive_type == LEGACY_TAPE_DRIVE)
    {
        // DO NOT try a SAT identify on these devices if we already know what they
        // are. These should be treated as SCSI since they are either SCSI or ATA
        // packet devices
        return NOT_SUPPORTED;
    }
    if (!device->drive_info.passThroughHacks.scsiHacks.noVPDPages &&
        !device->drive_info.passThroughHacks.scsiHacks
             .noSATVPDPage) // if this is set, then the device is known to not
                            // support VPD pages, so just skip to the SAT identify
    {
        ...
        if (SUCCESS == scsi_Inquiry(device, ataInformation, VPD_ATA_INFORMATION_LEN, ATA_INFORMATION, true, false))
        {
            if (ataInformation[1] == ATA_INFORMATION)
            {
                ...
                memcpy(&device->drive_info.bridge_info.t10SATvendorID[0],
                       &ataInformation[SAT_ATA_VPD_T10_VENDOR_OFFSET], SAT_ATA_VPD_T10_VENDOR_LENGTH);
                memcpy(&device->drive_info.bridge_info.SATproductID[0],
                       &ataInformation[SAT_ATA_VPD_T10_PRODUCT_ID_OFFSET], SAT_ATA_VPD_T10_PRODUCT_ID_LENGTH);
                memcpy(&device->drive_info.bridge_info.SATfwRev[0], &ataInformation[SAT_ATA_VPD_T10_PRODUCT_REV_OFFSET],
                       SAT_ATA_VPD_T10_PRODUCT_REV_LENGTH);

                // Setup flags for ATA passthrough if we know the SAT vendor/product/rev
                // info
                if (strcmp(device->drive_info.bridge_info.t10SATvendorID, "PMCS    ") == 0)
                {
                    // printf("Found PMCS SATL\n");
                    // PMCS is a PMC translator. Sometimes these also show up on HPE
                    // controllers. Tested: SAT Vendor ID: PMCS SAT Product ID:
                    // SRC73208_01 SAT Product Rev: 0106 NOTE: There is a weird case where
                    // a 1TB drive cannot report native maxLBA, but a 10TB can on this
                    // controller.
                    //       I'm guessing there is some logic to how sense data is handled
                    //       based on capacity or some other factor affecting this. The
                    //       control mode page's d_sense bit changes between the 1TB (0)
                    //       and 10TB (1), so this seems to be related to the behavior
                    //       differences -TJE Need to try changing the mode page value for
                    //       d-sense to see if it gets around this issue.
                    // This translator properly handles 0 length transfers for RW10, 12, &
                    // 16 Tested this same translator on an HPE controller with HPSA
                    // driver and non-HPE with smartpqi driver The SAT level info did not
                    // change and most behavior was the same, but there were a few odd
                    // results between them both. Leaving the below rules since they seem
                    // to work in both despite slightly different behavior. NOTE: Seems
                    // that the translators both report SAT-3 and ACS-2 compliance in
                    // inquiry, but there are slight variations in support.
                    //       One ran self-test and discovered the feature properly,
                    //       whereas the other did not. Possibly different minor versions
                    //       of these standards compliance or possibly some other
                    //       limitation in the translator that cannot be revealed by
                    //       looking at the revision on this page alone -TJE
                    ...
                    // debugging in a RAID environment is giving a few odd results with
                    // max transfer length, so not setting that for now-TJE
                }
                else if (strcmp(device->drive_info.bridge_info.t10SATvendorID, "LSI     ") == 0)
                {
                    ...
                    // NOTE: Handles zero length transfers for rw10,12,16 properly it
                    // seems Should be retested in Linux. Results are from Windows, which
                    // may be more limited, but cannot confirm
                    device->drive_info.passThroughHacks.passthroughType                       = ATA_PASSTHROUGH_SAT;
                    device->drive_info.passThroughHacks.hacksSetByReportedID                  = true;
                    device->drive_info.passThroughHacks.scsiHacks.readWrite.rw6               = true;
                    device->drive_info.passThroughHacks.scsiHacks.readWrite.rw10              = true;
                    ...
                    // = true;//while supported, not necessary to use
                }
                else if (strcmp(device->drive_info.bridge_info.t10SATvendorID, "linux   ") == 0)
                {
                    // libATA provides SAT translation.
                    // There are changes between kernel versions, but this should be
                    // pretty accurate This code may not currently be used due to how the
                    // low-level passthrough detects capabilities, but this will not hurt
                    // to setup
                    //  Tested:
                    //  SAT Vendor ID: linux
                    ...
                    device->drive_info.passThroughHacks.scsiHacks.maxTransferLength =
                        2097152; // may vary depending on OS/kernel config among other
                                 // things
                    device->drive_info.passThroughHacks.ataPTHacks.alwaysCheckConditionAvailable = true;
                ...
                {
                    if (ataInformation[43] & DEVICE_SELECT_BIT) // ATA signature device register is here.
                                                                // Checking for the device select bit being set
                                                                // to know it's device 1 (Not that we really
                                                                // need it)
                    {
                ...
                if (ataInformation[SAT_ATA_VPD_COMMAND_CODE_OFFSET] == ATA_IDENTIFY ||
                    ...
                    ataInformation[SAT_ATA_VPD_COMMAND_CODE_OFFSET] ==
                        ATA_READ_LOG_EXT_DMA) // Added read log commands here since they
                                              // are in SAT4. Only HDD/SSD should use
                                              // these.
                {
                    issueSATIdentify              = true;
                ...
                {
                    issueSATIdentify = false; // Do not read it since we want to treat ATAPI as SCSI/with
                                              // SCSI commands (at least for now)-TJE
...
static bool set_Passthrough_Hacks_By_Inquiry_Data(tDevice* device)
{
    ...
    if (responseFormat >= INQ_RESPONSE_FMT_CURRENT)
    {
        ...
        else if (strcmp(vendorID, "Realtek") == 0 && strstr(productID, "9210"))
        {
            ...
            // setup the following hacks to make this thing work as best we can for
            // now
            device->drive_info.passThroughHacks.passthroughType                     = ATA_PASSTHROUGH_SAT;
            ...
            device->drive_info.passThroughHacks.scsiHacks.noModeSubPages =
                true; // this supports some mode pages, but unable to test for
                      // subpages, so considering them not supported at this time -TJE
            device->drive_info.passThroughHacks.scsiHacks.noReportSupportedOperations = true;
            // NOTE: Security protocol is supported according to online web page. I do
            // not have a device supporting security to test against at this time to
            // see if INC512 is needed/required -TJE
            ...
            // device->drive_info.passThroughHacks.ataPTHacks.useA1SATPassthroughWheneverPossible
            // = true; Check condition will always return RTFRs, HOWEVER on data
            // transfers it returns empty data. Seems like a device bug. Only use
            // check condition for non-data commands-TJE NOTE: It may be interesting
            // to try an SCT command (write log) to see how check condition works, but
            // at least with reads, this is a no-go
            device->drive_info.passThroughHacks.scsiHacks.noSATVPDPage = true;
            device->drive_info.passThroughHacks.ataPTHacks.alwaysUseTPSIUForSATPassthrough =
                true; // seems to make no difference whether this is used or not. Can
                      // switch this to "limited use" if we need to
            ...
            device->drive_info.passThroughHacks.ataPTHacks.possilbyEmulatedNVMe =
                true; // no way to tell at this point. Will need to make full
                      // determination in the fill_ATA_Info function
            device->drive_info.passThroughHacks.ataPTHacks.noMultipleModeCommands =
                true; // probably not needed, but after what I saw testing this, it
                      // can't hurt to set this
        }
        ...
        else if (strcmp(vendorID, "SEAGATE") == 0) // Newer Seagate USB's will set "Seagate" so this can help
                                                   // filter based on case-sensitive comparison
        {
        ...
        {
            // Current Seagate USBs will report the vendor ID like this, so this will
            // match ALL of them. If we are in this function, then the low-level was
            // unable to get PID/VID, so we need to set some generic hacks to make
            // sure things work, then do device specific things.
            device->drive_info.passThroughHacks.testUnitReadyAfterAnyCommandFailure = true;
            ...
            {
                // setup some defaults that will most likely work for most current
                // products
                device->drive_info.passThroughHacks.scsiHacks.readWrite.available         = true;
                device->drive_info.passThroughHacks.scsiHacks.readWrite.rw10              = true;
                device->drive_info.passThroughHacks.scsiHacks.readWrite.rw16              = true;
    ...
    // else response format of 1 or 0 means we have to check all vendor unique
    // fields on case by case basis.
    else
    {
        // This is code that works on one old drive I have. Probably needs
        // adjustment to work on everything! Returned inq example:
        /*
        ...
        */
        ...
        if (strcmp(vendorID, "Seagate") == 0)
        {
            ...
            if (strcmp(productID, "External Drive") == 0 &&
                safe_strnlen(internalModel,
                             MODEL_NUM_LEN + 1)) // doing safe_strlen of internal model number to catch
...
void seagate_Serial_Number_Cleanup(const char* t10VendorIdent, char** unitSerialNumber, size_t unitSNSize)
{
    if (t10VendorIdent && unitSerialNumber && *unitSerialNumber)
    {
        ...
        {
            // sometimes these report with padded zeroes at beginning or end. Detect
            // this and remove the extra zeroes All of these SNs should be only 8
            // characters long.
            DECLARE_ZERO_INIT_ARRAY(char, zeroes,
                                    SERIAL_NUM_LEN + 1); // making bigger than needed for now.
            ...
            if (strncmp(zeroes, *unitSerialNumber, SEAGATE_SERIAL_NUMBER_LEN) == 0)
            {
                ...
                memmove(&(*unitSerialNumber)[0], &(*unitSerialNumber)[SEAGATE_SERIAL_NUMBER_LEN],
                        safe_strlen(*unitSerialNumber) - SEAGATE_SERIAL_NUMBER_LEN);
                memset(&(*unitSerialNumber)[SEAGATE_SERIAL_NUMBER_LEN], 0,
                       safe_strlen(*unitSerialNumber) - SEAGATE_SERIAL_NUMBER_LEN);
            }
                             ...
                             safe_strlen(*unitSerialNumber) - SEAGATE_SERIAL_NUMBER_LEN) == 0)
            {
                ...
                {
                    // after string reverse, the SN still wasn't right, so go back to
                    // stripping off the zeroes from the end.
            ...
            // NOTE: For LaCie, it is unknown what format their SNs were before
            // Seagate acquired them, so may need to add different cases for these
            // older LaCie products.
        }
...
eReturnValues fill_In_Device_Info(tDevice* device)
{
    ...
    if (turStatus.senseKey != SENSE_KEY_NO_ERROR)
    {
        ...
        {
            if (turStatus.asc == 0x3A) // NOTE: 3A seems to be all the "medium not present" status's,
                                       // so not currently checking for ascq - TJE
            {
    ...
    if (SUCCESS == scsi_Inquiry(device, inq_buf, INQ_RETURN_DATA_LENGTH, 0, false, false))
    {
        ...
        ret                                = SUCCESS;
        memcpy(device->drive_info.scsiVpdData.inquiryData, inq_buf,
               96); // store this in the device structure to make sure it is
                    // available elsewhere in the library as well.
        copy_Inquiry_Data(inq_buf, &device->drive_info);

        if (!device->drive_info.passThroughHacks.hacksSetByReportedID)
        {
            // This function will check known inquiry data to set passthrough hacks
            // for devices that are known to report a certain way.
            // TODO: can running this be used to prevent checking for SAT if this is
            // already set? This could help with certain scenarios
            set_Passthrough_Hacks_By_Inquiry_Data(device);
        }

        responseFormat = M_GETBITRANGE(inq_buf[3], 3, 0);
        version        = inq_buf[2];
        switch (version) // convert some versions since old standards broke the
                         // version number into ANSI vs ECMA vs ISO standard numbers
        {
        case 0:
            ...
            {
                checkForSAT = false; // NOTE: some cheap USB to SATA/PATA adapters will
                                     // set this version or no version. The only way to
                                     // work around this, is to make sure the low level
                                     // for the OS detects it on USB interface and it
                                     // can be run through the usb_hacks file instead.
            }
            break;
        case 0x81:
        ...
        case 0x84:
            ...
            break;
        default:
            // convert some versions since old standards broke the version number into
            // ANSI vs ECMA vs ISO standard numbers
        ...
        device->drive_info.scsiVersion = version; // changing this to one of these version numbers to keep the
                                                  // rest of the library code that would use this simple. - TJE
        ...
        case PERIPHERAL_DIRECT_ACCESS_BLOCK_DEVICE:
            device->drive_info.media_type = MEDIA_HDD; // this may not be correct because it may be SSD or USB
                                                       // Flash drive which use this same code
            break;
        ...
        case PERIPHERAL_STORAGE_ARRAY_CONTROLLER_DEVICE:
            ...
            checkForSAT                   = false;
            break;
        case PERIPHERAL_SIMPLIFIED_DIRECT_ACCESS_DEVICE: // some USB flash drives
                                                         // show up as this
                                                         // according to the USB
                                                         // mass storage
                                                         // specification...but
                                                         // unfortunately all the
                                                         // ones I've tested show up
                                                         // as Direct Access Block
                                                         // Device just like an HDD
                                                         // :(
            device->drive_info.media_type = MEDIA_SSM_FLASH;
            if (!device->drive_info.passThroughHacks.hacksSetByReportedID)
            {
        ...
        // check version descriptors first (If returned sense data is long enough
        // and reports all of this) This can help improve SAT detection and other
        // passthrough quirks
        for (uint16_t versionIter = 0, offset = 58; versionIter < 7 && offset < (inq_buf[4] + 4);
             ++versionIter, offset += 2)
        {
        ...
        // special USB detection case. If not already USB interface, do a few more
        // checks to get the interface correct
        if (device->drive_info.interface_type == SCSI_INTERFACE)
        {
            ...
            // Only rely on this as a last resort. Try using version descriptors when
            // possible NOTE: This is different from SAS where the ID is in all CAPS,
            // which makes this identification possible.
            // TODO: LaCie? Need to make sure this only catches USB and not something
            // else like thunderbolt
            if (device->drive_info.interface_type == SCSI_INTERFACE)
            {
        ...
        bool hisup = M_ToBool(inq_buf[3] & BIT4);    // historical support...this is set to 1 for nvme (SNTL)
                                                     // and not specified in SAT...may be useful later - TJE
        bool rmb = M_ToBool(inq_buf[1] & BIT1);      // removable medium. This should be zero for all
                                                     // modern HDDs and any SSD, even over USB, but
                                                     // USB sometimes plays by it's own rules. - TJE
        ...
        {
            // check that response format is 2 (or higher). SAT spec says the response
            // format should be set to 2 Not checking this on USB since some adapters
            // set this purposely to avoid certain commands, BUT DO support SAT
            if (M_Nibble0(inq_buf[3]) < 2 && device->drive_info.interface_type != USB_INTERFACE)
            {
                checkForSAT = false;
            }
            // normaca is specified as not compatible, so if it's set, we can
            // definitely skip the SAT check
            if (inq_buf[3] & BIT5)
            {
                checkForSAT = false;
            }
            // sat r09 says mchangr will be set to zero, so we will use this to filter
            // out this device
            if (inq_buf[6] & BIT3)
            {
                checkForSAT = false;
            }
            // Checking to see if any old parallel scsi bits are set. Doing this
            // because there are no known SCSI to PATA adapters that would be SAT
            // compliant and it is unlikely these will be set otherwise if less than
            // version 6 (SPC4) some bits are marked obsolete: addr32, wbus32,
            // ackreqq, trandis
            if (version < 6)
            {
            ...
            // other bits we may or may not want to check for are multip, aerc,
            // trmtsk, any vendor specific bits, sccs, protect, 3pc each of these are
            // technically not specified in SAT, but are not likely to be suppored
            // anyways. We can add these in overtime if we find them useful for the
            // filter. Most likely, protect and 3pc will be most useful. Not sure
            // about the others, but I doubt many controllers will set
            // them...certainly no USB device will.
        ...
        // do we want to check the version descriptors here too? There are a lot of
        // those...I have a table that parses them to human readable, but not
        // setting anything yet...may need to use that later

        // As per NVM Express SCSI Translation Reference.
        // NOTE: Setting this type here allows us to skip sending some extra
        // commands. (e.g. SAT compliant)
        if (memcmp(device->drive_info.T10_vendor_ident, "NVMe", 4) == 0)
        {
            // DO NOT set the drive type to NVMe here. We need to treat it as a SCSI
            // device since we can only issue SCSI translatable commands!!!
        ...
        {
            // Disable checking for SAT when the low-level device information says it
            // is not available. This prevents unnecessary discovery and slow-down on
            // devices that are already confirmed to not support SAT or other ATA
            // passthrough
            checkForSAT = false;
        }

        bool knownMemoryStickID = false;
        // Checking the product identification for "Generic-" device to see if they
        // are MMC, SD, etc type devices
        if (strcmp(device->drive_info.T10_vendor_ident, "Generic-") == 0)
        {
            ...
            {
                // TODO: We have "FLASH_DRIVE" as a type, but it won't ba handled well
                // in the rest of the library.
                //       Either need to start using it, or make more changes to handle
                //       it better -TJE
                ...
                {
                    // Only check for SAT on compact flash since it uses ATA commands. May
                    // need another case for CFast as well.
                    checkForSAT = false;
                }
        ...
        // If this is a suspected NVMe device, specifically ASMedia 236X chip, need
        // to do an inquiry with EXACTLY 38bytes to check for a specific signature
        // This will check for some known outputs to know when to do the additional
        // inquiry command for ASMedia detection. This may not catch everything. -
        // TJE
        if (!knownMemoryStickID && !device->drive_info.passThroughHacks.hacksSetByReportedID &&
            ...
            (strncmp(device->drive_info.T10_vendor_ident, "ASMT", 4) == 0 ||
             ...
             strcmp(device->drive_info.T10_vendor_ident, "LaCie") ==
                 0) // This is a special case to run on Seagate and LaCie USB
                    // adapters as they may use the ASmedia NVMe chips
            // TODO: Check when FWRev is set to 2364? At least one device I have
            // does this, but not sure this is a good thing to add in here or not
            // -TJE
            && !hisup && !rmb                             // hisup shoiuld be 1 and rmb should be zero...on the
                                                          // asmedia chips I have tested, hisup is zero
            && responseFormat >= INQ_RESPONSE_FMT_CURRENT // filter out any weird old drives with
                                                          // bizarre responses
            && inq_buf[4] == 0x47                         // SNTL says 1F, but a couple of adapter I have sets
                                                          // 47h...using this for now to help filter the list
            && cmdQueue                                   // should be set for all NVMe SNTL translators
        )
        {
            // This is likely a ASMedia 236X device. Need to do another inquiry
            ...
            if (SUCCESS == scsi_Inquiry(device, asmtInq, 38, 0, false, false))
            {
                ...
                {
                    // This is an ASMedia device with the 236X chip which supports USB to
                    // NVMe passthrough will attempt to check for full passthrough support
                    // first
                    uint8_t* nvmeIdentify =
                        C_CAST(uint8_t*, safe_calloc_aligned(NVME_IDENTIFY_DATA_LEN, sizeof(uint8_t),
                    ...
                    device->drive_info.passThroughHacks.ataPTHacks.a1NeverSupported =
                        true; // set this so in the case an ATA passthrough command is
                              // attempted, it won't try this opcode since it can cause
                              // performance problems or crash the bridge
                    device->drive_info.passThroughHacks.scsiHacks.readWrite.available         = true;
                    device->drive_info.passThroughHacks.scsiHacks.readWrite.rw10              = true;
                    device->drive_info.passThroughHacks.scsiHacks.readWrite.rw16              = true;
                    device->drive_info.passThroughHacks.scsiHacks.noLogPages                  = true;
                    ...
                    // This code will setup known hacks for these devices since it wasn't
                    // already detected by lower layers based on VID/PID reported over the
                    // USB interface
                    checkForSAT = false;
                    if (!fullCmdSupport)
                    {
                        // if the full cmd above failed, but we are here, that means it
                        // supports the basic passthrough change to this passthrough and set
                        // the limited capabilities flags
        ...
        // Need to check version descriptors here since they may be useful below,
        // but also because it can be used to help rule-out some USB to NVMe
        // devices.
        ...
        if (strncmp(device->drive_info.T10_vendor_ident, "NVMe", 4) == 0 ||
            ...
            strstr(device->drive_info.product_identification, "NVMe"))
        {
            // This means we most likely have some sort of NVMe device, so SAT (ATA
            // passthrough) makes no sense to check for.
            checkForSAT = false;
        }
        else
        {
            if (foundSATStandardDescriptor || foundATAStandardDescriptor)
            {
                    ...
                    strstr(device->drive_info.product_identification, "NVMe") == M_NULLPTR)
                {
                    ...
                    checkForSAT               = true; // Setting it as true, since some Marvell controller set
                                                      // SYNC bit, which is preventing our auto-discovery as ATA
                                                      // drive and sending SAT commands.
                }
                else
                {
                    checkForSAT = false;
        ...
        if (!device->drive_info.passThroughHacks.hacksSetByReportedID && checkForSAT &&
            ...
            (strncmp(device->drive_info.T10_vendor_ident, "JMicron", 4) == 0 || // check anything coming up as Jmicron
             ...
             strcmp(device->drive_info.T10_vendor_ident, "LaCie") ==
                 0) // This is a special case to run on Seagate and LaCie USB
                    // adapters as they may use the Jmicron NVMe chips
            && foundSATStandardDescriptor && !foundATAStandardDescriptor // these chips report SAT, but not an ATA
                                                                         // standard...might reduce how often this
                                                                         // check is performed - TJE
            && hisup && !rmb                              // hisup shoiuld be 1 and rmb should be zero...this
                                                          // should filter SOME, but not all USB adapters that
                                                          // are actually SATA drives - TJE
            && responseFormat >= INQ_RESPONSE_FMT_CURRENT // filter out any weird old drives with
                                                          // bizarre responses
            //&& inq_buf[4] == 0x5b //SNTL says 1F, but one adapter I have sets
            // 5B...need to make sure other adapters do the same before we enforce
            // this check - TJE
            && cmdQueue // should be set for all NVMe SNTL translators
        )
        {
            ...
            // Do not turn off SAT checks because there is not enough information to
            // filter down into a known NVMe vs known SATA device in this
            // "auto-detect" case checking for this will come, most likely, after SAT
            // check.
            // TODO: Further improvement: Detect A1h command supported, but 85h is
            // NOT. A1 is the opcode supported by this device's passthrough and
            // anything supporting 85h will have to be SAT, not this unique NVMe
            // passthrough - TJE
            //       note: there is not currently a good way to track the results from
            //       these commands in fill_ATA_Drive_Info...that is something we can
            //       work on going forward. A1 SAT identify should return "Invalid
            //       field in CDB" and 85h should return "Invalid operation code".
            //       While SOME SAT device may do this too, this will reduce commanmds
            //       sent to genuine SAT devices.
        }

        if (device->drive_info.interface_type == SCSI_INTERFACE &&
            is_Seagate_SAS_Vendor_ID(device->drive_info.T10_vendor_ident))
        {
            // do NOT do a SAT check. For some reason sometimes a combo of HBA and
            // some SAS drives will respond with no sense errors on A1h CDB This seems
            // to happen on LSI 9300-8i and some combinations of SAS drives....but not
            // all of them. It is oddly specific. Same HBA and drive on Windows? Works
            // fine. I want a better check than this using other fields, but this
            // should be reasonably safe for now since vendor id of "SEAGATE " is only
            // used on SAS drives
            checkForSAT = false;
        }
        ...
        {
            // This is a microsoft virtual drive setup for one of many reasons (XBox
            // game pass does this when installing games for example) It does respond
            // to limited SCSI commands, but no ATA passthrough so just disable it.
            checkForSAT = false;
        }
        ...
        if (M_Word0(device->dFlags) == DO_NOT_WAKE_DRIVE)
        {
...
#endif
            // We actually need to try issuing an ATA/ATAPI identify to the drive to
            // set the drive type...but I'm going to try and ONLY do it for ATA drives
            // with the if statement below...it should catch almost all cases (which
                ...
                (device->drive_info.drive_type != ATAPI_DRIVE && device->drive_info.drive_type != LEGACY_TAPE_DRIVE))
            {
                ...
                if (ret != SUCCESS && checkJMicronNVMe)
                {
                    ...
                    {
                        device->drive_info.passThroughHacks.passthroughType = PASSTHROUGH_NONE;
                        ret = SUCCESS; // do not fail here since this should otherwise be
                                       // treated as a SCSI drive
                    }
                }
            }
        ...
        if (M_Word0(device->dFlags) == FAST_SCAN)
        {
            ...
            {
                // I'm reading only the unit serial number page here for a quick scan
                // and the device information page for WWN - TJE
                ...
                {
                    if (unitSerialNumber[1] == UNIT_SERIAL_NUMBER) // make sure we actually got the right page
                                                                   // and not bogus data.
                    {
                        ...
                        if (serialNumberLength > 0)
                        {
                            ...
                            memcpy(&device->drive_info.serialNumber[0], &unitSerialNumber[4],
            ...
            {
                // SN may not be available...just going to read where it may otherwise
                // show up in inquiry data like some vendors like to put it
                memcpy(&device->drive_info.serialNumber[0], &inq_buf[36], SERIAL_NUM_LEN);
                device->drive_info.serialNumber[SERIAL_NUM_LEN] = '\0';
                // make sure the SN is printable if it's coming from here since it's
                // non-standardized
                for (uint8_t iter = 0; iter < SERIAL_NUM_LEN; ++iter)
                {
                    if (!safe_isascii(device->drive_info.serialNumber[iter]) ||
                ...
                !device->drive_info.passThroughHacks.scsiHacks.noVPDPages) // device identification added in SPC
            {
                                            ...
                                            true, false))
                {
                    ...
                    {
                        // this SHOULD work for getting a WWN 90% of the time, but if it
                        // doesn't, then we will need to go through the descriptors from the
                        // device and set it from the correct one. See the SATChecker util
            ...
            device->drive_info.interface_type != IEEE_1394_INTERFACE)
        {
            ...
            DECLARE_ZERO_INIT_ARRAY(uint8_t, reportLuns,
                                    REPORT_LUNS_MIN_LENGTH); // only really need first 4 bytes, but this
                                                             // will make sure we get the length,
            ...
            (version >= 2 || device->drive_info.passThroughHacks.scsiHacks.unitSNAvailable)) // SCSI 2 added VPD pages
        {
            ...
            // from here on we need to check if a VPD page is supported and read it if
            // there is anything in it that we care about to store info in the device
            // struct
            ...
            {
                // did not get the list of supported pages! Checking this since
                // occasionally we get back garbage
                memset(inq_buf, 0, INQ_RETURN_DATA_LENGTH);
                dummyUpVPDSupport = true;
            }
            ...
            if (dummyUpVPDSupport)
            {
                ...
                if (device->drive_info.passThroughHacks.scsiHacks.unitSNAvailable)
                {
                    // If this is set, then this means that the device ONLY supports the
                    // unit SN page, but not other. Only add unit serial number to this
                    // dummied data. This is a workaround for some USB devices.
                    // TODO: if these devices support a limited number of other pages, we
                    // will need to change this hack a little bit to work with them
                    // better.
                    //       Some of the devices only support the unit serial number page
                    //       and the device identification page.
            ...
            // that we care about uint16_t vpdIter = 0;
            for (uint16_t vpdIter = 0; vpdIter < supportedVPDPagesLength && vpdIter < INQ_RETURN_DATA_LENGTH &&
                                       !device->drive_info.passThroughHacks.scsiHacks.noVPDPages;
                 vpdIter++)
            {
                switch (supportedVPDPages[vpdIter])
                {
                case UNIT_SERIAL_NUMBER: // Device serial number (only grab 20
                                         // characters worth since that's what we need
                                         // for the device struct)
                {
                    ...
                    {
                        if (unitSerialNumber[1] == UNIT_SERIAL_NUMBER) // check the page code to make sure we got
                                                                       // the right thing
                        {
                            uint16_t serialNumberLength = M_BytesTo2ByteValue(unitSerialNumber[2], unitSerialNumber[3]);
                            if (serialNumberLength > 0)
                ...
                case DEVICE_IDENTIFICATION: // World wide name
                {
                                                ...
                                                DEVICE_IDENTIFICATION, true, false))
                    {
                        ...
                        {
                            // this SHOULD work for getting a WWN 90% of the time, but if it
                            // doesn't, then we will need to go through the descriptors from
                            // the device and set it from the correct one. See the SATChecker
                            // util code for how to do this
                            memcpy(&device->drive_info.worldWideName, &deviceIdentification[8], 8);
                            byte_Swap_64(&device->drive_info.worldWideName);
                        }
                    }
                ...
                case ATA_INFORMATION: // use this to determine if it's SAT compliant
                {
                    if (device->drive_info.passThroughHacks.passthroughType < NVME_PASSTHROUGH_JMICRON)
                    {
                        ...
                        // do not check the checkForSAT bool here. If we get here, then the
                        // device most likely reported support for it so it should be
                        // readable.
                        ...
                        {
                            // send test unit ready to get the device responding again (For
                            // better performance on some USB devices that don't support this
                            // page)
                            ...
                            if (checkJMicronNVMe)
                            {
                                ...
                                {
                                    device->drive_info.passThroughHacks.passthroughType = PASSTHROUGH_NONE;
                                    ret = SUCCESS; // do not fail here since this should otherwise
                                                   // be treated as a SCSI drive
                                }
                            }
                        }
                ...
                case BLOCK_DEVICE_CHARACTERISTICS: // use this to determine if it's SSD
                                                   // or HDD and whether it's a HDD or
                                                   // not
                {
                                                ...
                                                true, false))
                    {
                        if (blockDeviceCharacteristics[1] == BLOCK_DEVICE_CHARACTERISTICS)
                        {
                            ...
                            if (device->drive_info.media_type !=
                                MEDIA_SSM_FLASH) // if this is already set, we don't want to
                                                 // change it because this is a helpful filter
                                                 // for some card-reader type devices.
                            {
                                ...
                                else if (mediumRotationRate >= 0x401 && mediumRotationRate <= 0xFFFE)
                                {
                                    ...
                                    if (checkJMicronNVMe)
                                    {
                                        // The logic here is that there are no NVMe HDDs that will
                                        // use this bridge, so do not do a SAT check, and instead
                                        // check only for JMicron NVMe adapter - TJE
                                        checkForSAT = false;
                                    }
                                }
                            }
                            ...
                            case 0x07: // Universal Flash Storage
                                ...
                                break;
                            default: // not indicated or reserved or vendor unique so do
                                     // nothing
                                break;
                            }
                            // get zoned information (as long as it isn't already set from SAT
                            // passthrough)
                ...
                default:
                    // do nothing, we don't care about reading this page (at least not
                    // right now)
                    break;
                }
            }
            ...
            {
                // This device returned a list of pages already, so we know what it
                // supports. Since we did not find it in it's list of supported pages,
                // set this to skip trying to read SAT VPD since it is definitely not
                // supported.
                device->drive_info.passThroughHacks.scsiHacks.noSATVPDPage = true;
            }
        }
        ...
        if (readCapacity && !mediumNotPresent)
        {
            ...
            // send a read capacity command to get the device's logical block
            // size...read capacity 10 should be enough for this
            ...
            if (SUCCESS == scsi_Read_Capacity_10(device, readCapBuf, READ_CAPACITY_10_LEN))
            {
                copy_Read_Capacity_Info(&device->drive_info.deviceBlockSize, &device->drive_info.devicePhyBlockSize,
                                        ...
                                        readCapBuf, false);
                if (version > 3) // SPC2 and higher can reference SBC2 and higher which
                                 // introduced read capacity 16
                {
            ...
            {
                // If we did not get a physical blocksize, we need to set it to the
                // blocksize (logical). This will help with old devices or those that
                // don't support the read capacity 16 command or return other weird
                // invalid data.
                device->drive_info.devicePhyBlockSize = device->drive_info.deviceBlockSize;
            }
        }

        // NOTE: You would think that checking if physical and logical block sizes
        // don't match you can filter NVMe (they are supposed to be the same in
        // translation),
        //       but this DOES NOT WORK. For whatever reason, some report 512B
        //       logical, 4k physical....for no apparent reason. - TJE

        // printf("passthrough type set to %d\n",
        // device->drive_info.passThroughHacks.passthroughType);
        eReturnValues satCheck = FAILURE;
        // if we haven't already, check the device for SAT support. Allow this to
        // run on IDE interface since we'll just issue a SAT identify in here to set
        // things up...might reduce multiple commands later
        ...
        // Because we may find an NVMe over USB device, if we find one of these,
        // perform a little more discovery...
        if ((device->drive_info.passThroughHacks.passthroughType >= NVME_PASSTHROUGH_JMICRON &&
             device->drive_info.passThroughHacks.passthroughType < NVME_PASSTHROUGH_UNKNOWN) ||
            (satCheck != SUCCESS && checkJMicronNVMe))
        {
            ...
            // NOTE: It is OK if this fails since it will fall back to treating as
            // SCSI
            ret = fill_In_NVMe_Device_Info(device);
            if (ret == SUCCESS && checkJMicronNVMe)
            {
            ...
            {
                device->drive_info.passThroughHacks.passthroughType = PASSTHROUGH_NONE;
                ret = scsiRet; // do not fail here since this should otherwise be
                               // treated as a SCSI drive
            }
            else
            {
...
// this is a look up that makes use of the formula in SPC spec for how versions
// are set. Formula is ((standard x 32) + revision) The simplest thing to do is
// take the version descriptor and divide it by 32. Using iteger division we can
...
bool is_Standard_Supported(uint16_t versionDescriptor, eStandardCode standardCode)
{
    ...
    // To convert it to a standard, divide it by 32 as indicated in the formula in
    // the annex-TJE
...
typedef struct _scsiVersionDescriptor
{
    uint16_t    versionCode;       // combined version descriptor code as reported by the drive
    const char* stringDescription; // string for the meaning of this specific
                                   // revision code
...
void decypher_SCSI_Version_Descriptors(uint16_t versionDescriptor, char* versionString)
{
    ...
    else
    {
        ...
        case STANDARD_CODE_ACSx:
            // special case, look at the version descriptor to set an exact version
            // number
            switch (versionDescriptor)
            {
            case 0x1761: // ACS-2
