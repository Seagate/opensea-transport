        ...
        if (senseFields->senseKeySpecificInformation.senseKeySpecificValid)
        {
            ...
            case SENSE_KEY_SPECIFIC_FIELD_POINTER:
                if (senseFields->senseKeySpecificInformation.field.cdbOrData)
                {
                    ...
                    {
                        print_tDevice_Verbose_Formatted_String(
                            device, verbosity, "Invalid field in CDB byte %" PRIu16 " bit %" PRIu8 "\n",
                            senseFields->senseKeySpecificInformation.field.fieldPointer,
                            senseFields->senseKeySpecificInformation.field.bitPointer);
                    }
                    else
                    {
                        print_tDevice_Verbose_Formatted_String(
                            device, verbosity, "Invalid field in CDB byte %" PRIu16 "\n",
                            senseFields->senseKeySpecificInformation.field.fieldPointer);
                    }
                }
                else
                {
                    ...
                    {
                        print_tDevice_Verbose_Formatted_String(
                            device, verbosity, "Invalid field in Parameter byte %" PRIu16 " bit %" PRIu8 "\n",
                            senseFields->senseKeySpecificInformation.field.fieldPointer,
                            senseFields->senseKeySpecificInformation.field.bitPointer);
                    }
                    else
                    {
                        print_tDevice_Verbose_Formatted_String(
                            device, verbosity, "Invalid field in Parameter byte %" PRIu16 "\n",
                            senseFields->senseKeySpecificInformation.field.fieldPointer);
                    }
                }
                break;
            case SENSE_KEY_SPECIFIC_ACTUAL_RETRY_COUNT:
                print_tDevice_Verbose_Formatted_String(
                    device, verbosity, "Actual Retry Count: %" PRIu16 "\n",
                    senseFields->senseKeySpecificInformation.retryCount.actualRetryCount);
                break;
            case SENSE_KEY_SPECIFIC_PROGRESS_INDICATION:
                print_tDevice_Verbose_Formatted_String(
                    device, verbosity, "Progress: %0.02f%%\n",
            ...
            case SENSE_KEY_SPECIFIC_SEGMENT_POINTER:
                if (senseFields->senseKeySpecificInformation.segment.segmentDescriptor)
                {
                    ...
                    {
                        print_tDevice_Verbose_Formatted_String(
                            device, verbosity, "Invalid field in Segment Descriptor byte %" PRIu16 " bit %" PRIu8 "\n",
                            senseFields->senseKeySpecificInformation.field.fieldPointer,
                            senseFields->senseKeySpecificInformation.field.bitPointer);
                    }
                    else
                    {
                        print_tDevice_Verbose_Formatted_String(
                            device, verbosity, "Invalid field in Segment Descriptor byte %" PRIu16 "\n",
                            senseFields->senseKeySpecificInformation.field.fieldPointer);
                    }
                }
                else
            ...
            case SENSE_KEY_SPECIFIC_UNIT_ATTENTION_CONDITION_QUEUE_OVERFLOW:
                if (senseFields->senseKeySpecificInformation.unitAttention.overflow)
                {
                    print_tDevice_Verbose_String(device, verbosity,
                                                 "Unit attention condition is due to Queue Overflow\n");
                }
                else
                {
                    print_tDevice_Verbose_String(device, verbosity,
                                                 "Unit attention condition is not due to a queue overflow\n");
                }
                break;
            case SENSE_KEY_SPECIFIC_UNKNOWN:
            default:
                print_tDevice_Verbose_Formatted_String(
                    device, verbosity, "Unknown sense key specific data: %" PRIX8 "h %" PRIX8 "h %" PRIX8 "h\n",
                                        ...
                                        M_ATTR_UNUSED size_t     ataInfoLen)
{
    ...
    if (strcmp(device->drive_info.bridge_info.t10SATvendorID, "PMCS    ") == 0)
    {
        ...
        // SAT_FIXED_SENSE_HACK_FIXED_FORMAT_SWAPPED_LBA_BYTE_ORDER; if (device->deviceVerbosity >=
        // VERBOSITY_COMMAND_VERBOSE)
        // {
        //     printf(
...
{
    decypher_SCSI_Version_Descriptors_Len(versionDescriptor, versionString, MAX_VERSION_DESCRIPTOR_STRING_LENGTH);
}
    ...
    else
    {
        ...
        shortVerDescRes =
            C_CAST(const shortSCSIVersionDescriptor*,
                   safe_bsearch(&shortVerDesKey, M_REINTERPRET_CAST(const void*, shortScsiVersionDescriptorTable),
                                SIZE_OF_STACK_ARRAY(shortScsiVersionDescriptorTable),
