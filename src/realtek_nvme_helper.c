...
eReturnValues build_Realtek_NVMe_CDB_And_Payload(uint8_t* cdb, eDataTransferDirection* cdbDataDirection, uint8_t* dataPtr, uint32_t dataSize, eRealtekNVMCMDPhase phase, nvmeCmdCtx* nvmCmd)
{
    if (!cdb)
    {
...
eReturnValues send_Realtek_NVMe_Cmd(nvmeCmdCtx* nvmCmd)
{
    ...
    if (nvmCmd->dataSize > REALTEK_NVME_MAX_TRANSFER_SIZE_BYTES
        || nvmCmd->commandDirection == XFER_DATA_IN_OUT
    ...
    ret = build_Realtek_NVMe_CDB_And_Payload(realtekCDB, &realtekCDBDir, realtekPayload, REALTEK_NVME_CMD_PAYLOAD_LEN, REALTEK_PHASE_COMMAND, nvmCmd);
    if (SUCCESS != ret)
    {
        return ret;
    }
    ret = scsi_Send_Cdb(nvmCmd->device, realtekCDB, REALTEK_NVME_CDB_SIZE, realtekPayload, REALTEK_NVME_CMD_PAYLOAD_LEN, realtekCDBDir, NULL, 0, 15);
    if (SUCCESS != ret)
    {
        return ret;
    }
    ...
    eReturnValues sendRet = scsi_Send_Cdb(nvmCmd->device, realtekCDB, REALTEK_NVME_CDB_SIZE, nvmCmd->ptrData, nvmCmd->dataSize, realtekCDBDir, NULL, 0, nvmCmd->timeout);
    ...
    if (sendRet != OS_COMMAND_TIMEOUT)
    {
        ...
        if (SUCCESS == scsi_Send_Cdb(nvmCmd->device, realtekCDB, REALTEK_NVME_CDB_SIZE, realtekPayload, REALTEK_NVME_COMPLETION_PAYLOAD_LEN, realtekCDBDir, NULL, 0, 15))
        {
            ...
            //This is the correct endianness as it has been validated by trying commands with invalid fields to make sure it is correctly interpretted.
            nvmCmd->commandCompletionData.dw0 = M_BytesTo4ByteValue(realtekPayload[3], realtekPayload[2], realtekPayload[1], realtekPayload[0]);
            nvmCmd->commandCompletionData.dw1 = M_BytesTo4ByteValue(realtekPayload[7], realtekPayload[6], realtekPayload[5], realtekPayload[4]);
            nvmCmd->commandCompletionData.dw2 = M_BytesTo4ByteValue(realtekPayload[11], realtekPayload[10], realtekPayload[9], realtekPayload[8]);
            nvmCmd->commandCompletionData.dw3 = M_BytesTo4ByteValue(realtekPayload[15], realtekPayload[14], realtekPayload[13], realtekPayload[12]);
        }
    }
    return ret;
}
