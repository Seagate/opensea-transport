...
// This file is written based on what is described in the SCSI to NVMe
// translation white paper (SNTL). Some things that are clearly wrong (incorrect
// bit or offset, etc) are fixed as this was written. Most of these are noted in
// comments. The plan is to expand this beyond what is provided in the white
// paper for other translations. These will be wrapped in a #define for SNTL_EXT
// The whitepaper was originally created to help cover transitions in some
// environments from SCSI to native NVMe at a driver level. Most operating
// systems quickly implemented a driver and this is not necessary. This was put
// into this code base to help adapt for places that are treating NVMe as SCSI.
// It will probably not be used much, but it is educational to see how the
// translation works. The biggest thing missing from the SNTL whitepaper is a
// way to issue pass-through commands. Any implementation of this should use a
// vendor unique opertion code
//   so as to not confuse other OS's or software into thinking it's some other
//   type of device (such as an ATA device behind a SATL)
...
//  -compare and write. This translation is not possible without the ability to
//  issue fused commands, which is not currently possible. -format unit
//  translation. Needs the ability to save changes in a block descriptor.
//  Recommend implementing a mode page 0 that is empty for this (use page
//  format, set first 4 bytes to "SNTL" as a signature. -need clarification on
//  translation of SCSI verify with bytechk set to zero

#define SNTL_EXT
// SNTL_EXT is used to enable extensions beyond the SNTL spec...which we want
// since it's pretty out of date and we might as well add everything we can
// Extention translations not yet complete:
//  - mode page policy VPD page
//  - nvme passthrough command (needs to handle admin vs nvm, nondata, data-in,
//  data-out, bidirectional transfers and vendor unique commands
//  - read buffer command to return the NVMe telemetry log (similar to SAT
//  translation to return current internal status log)
...
// the completion queue will tell us if the error is specific to a command
                                                           ...
                                                           uint32_t senseDataLength)
{
    ...
    case 0x80: // conflicting attributes (dataset management, read, write)
        senseKey = SENSE_KEY_ILLEGAL_REQUEST;
        // invalid field in CDB (should be able to set param pointer for this some
        // day)
        asc  = 0x24;
        ascq = 0x00;
        break;
    case 0x81: // invalid protection information (compare, read, write, write
               // zeros)
        genericFailureSenseData = true;
        break;
    case 0x82: // attempted write to read only range (dataset management, write,
               // write uncorrectable, write zeros)
...
{
    eReturnValues ret = SUCCESS;
    DECLARE_ZERO_INIT_ARRAY(uint8_t, unitSerialNumber,
                            SNTL_UNIT_SERIAL_NUMBER_VPD_MAX_LENGTH); // 44 is the max size of this
                                                                     // page with the translation spec
    ...
    else // If both of these fields aren't set, this is an NVMe 1.0 device that
         // needs a different thing to be returned here.
    {
#define NSID_STRING_LENGTH 10
    ...
    if (scsiIoCtx->pdata)
    {
        memcpy(scsiIoCtx->pdata, unitSerialNumber,
...
static eReturnValues sntl_Translate_Device_Identification_VPD_Page_83h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (eui64nonZero) // this must be non-zero to be supported.
    {
        ...
        naaDesignator       = C_CAST(uint8_t*, safe_calloc(naaDesignatorLength, sizeof(uint8_t)));
        if (naaDesignator)
        {
    ...
    else if (!eui64nonZero && !nguidnonZero) // NVMe 1.0 devices won't support EUI or NGUID, so
                                             // we should be able to detect them like this
    {
        naaDesignatorLength = 20 /*ext*/ + 12 /*locally assigned*/;
        naaDesignator       = C_CAST(uint8_t*, safe_calloc(naaDesignatorLength, sizeof(uint8_t)));
        if (naaDesignator)
    ...
    // T10 Vendor ID descriptor (VendorID + productIdentification + (EUI64 ||
    // NGUID))
    if (eui64nonZero || nguidnonZero)
    {
        uint8_t offset              = 12;
        ...
        t10VendorIdDesignator = C_CAST(uint8_t*, safe_calloc(t10VendorIdDesignatorLength, sizeof(uint8_t)));
    ...
    else // nvme 1.0 devices: (VendorID + productIdentification + PCI VendorID +
         // lower 52Bits of SN + NSID)
    {
        uint8_t offset              = 12;
    ...
    if (eui64nonZero && nguidnonZero)
    {
        ...
        SCSINameStringDesignator       = C_CAST(uint8_t*, safe_calloc(SCSINameStringDesignatorLength, sizeof(uint8_t)));
    ...
    else // nvme 1.0 - //2bytes of PCI Vendor ID (utf8) + 40 bytes of MN + 4
         // bytes of NSID (utf8) + 20 bytes of SN
    {
        uint8_t offset                 = 8;
    ...
    // EUI64 designator (depends on NGUID and EUI64 support...can return one for
    // each of these)
    if (eui64nonZero && nguidnonZero)
    {
        // 1 descriptor for eui64 and 1 for nguid
        ...
        eui64Designator       = C_CAST(uint8_t*, safe_calloc(eui64DesignatorLength, sizeof(uint8_t)));
        if (eui64Designator)
        {
            // NGUID first
            ...
            eui64Designator[21] = 2; // designator type 2, associated with logical
                                     // unit
    ...
    deviceIdentificationPage = C_CAST(uint8_t*, safe_calloc(4U + eui64DesignatorLength + t10VendorIdDesignatorLength +
                                                                naaDesignatorLength + SCSINameStringDesignatorLength,
                                                            sizeof(uint8_t)));
    ...
    {
        memcpy(&deviceIdentificationPage[4 + naaDesignatorLength + t10VendorIdDesignatorLength],
...
static eReturnValues sntl_Translate_Extended_Inquiry_Data_VPD_Page_86h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    // Extended self test completion time set to zero since not supported (our
    // extension will set this if the drive supports the DST commands from
    // NVMe 1.3)
#if defined(SNTL_EXT)
    // if (device->drive_info.IdentifyData.nvme.ctrl.oacs & BIT4)
    //{
    //     //DST command is supported! So get the time long dst will take to run
    //     and put it in here extendedInquiry[10] =
    //     M_Byte1(device->drive_info.IdentifyData.nvme.ctrl.edstt);
    //     extendedInquiry[11] =
    //     M_Byte0(device->drive_info.IdentifyData.nvme.ctrl.edstt);
    // }
    // else
#endif
    {
    ...
    extendedInquiry[13] = 0; // can also set 252 since that is the spec maximum.
                             // This also means there is not a max
...
static eReturnValues sntl_Translate_Block_Limits_VPD_Page_B0h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    // max compare and write length - 0 if fused operation not supported.
    // Otherwise less than or equal to max transfer length field
    blockLimits[5] = 0;

    // optimal transfer length granularity (unspecified) TODO: set this to
    // something
    blockLimits[6] = 0;
    blockLimits[7] = 0;
    ...
    // set a maximum for write same length. Currently zero since we aren't
    // supporting the command right now maximum write same length
    // (unspecified....we decide). We will allow the full drive to be write same'd
...
static eReturnValues sntl_Translate_Block_Device_Characteristics_VPD_Page_B1h(ScsiIoCtx* scsiIoCtx)
{
        ...
        scsiIoCtx->device->drive_info.IdentifyData.nvme.ns.endgid > 0)
    {
        ...
        // First read the supported logs log page, then if the rotating media log is
        // there, read it.
        uint8_t* supportedLogs =
            C_CAST(uint8_t*, safe_calloc_aligned(1024, sizeof(uint8_t), scsiIoCtx->device->os_info.minimumAlignment));
        if (supportedLogs)
        {
...
static eReturnValues sntl_Translate_Logical_Block_Provisioning_VPD_Page_B2h(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    */
    // lbpws10 bit (set to zero since we don't support unmap during write same
    // yet)
    /*
    if (device->drive_info.IdentifyData.nvme.ctrl.oncs & BIT2)
    {
...
static eReturnValues sntl_Translate_SCSI_Inquiry_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    else
    {
       ...
       // check if protect bit needs to be set from namespace data
            ...
            // currently this is where the translation spec ends. Anything below here
            // is above and beyond the spec
#if defined SNTL_EXT
        ...
        if (readCapacity16)
        {
            ...
            if (device->drive_info.IdentifyData.nvme.ctrl.oncs & BIT2 &&
                device->drive_info.IdentifyData.nvme.ns.nsfeat &
                    BIT0) // supports provisioning...did this like how we get
                          // provisioning type in logical block provisioining page
            {
                readCapacityData[14] |= BIT7;
            }
            if (M_GETBITRANGE(device->drive_info.IdentifyData.nvme.ns.dlfeat, 2, 0) ==
...
static eReturnValues sntl_Translate_Supported_Log_Pages(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    offset += increment;
#if defined(SNTL_EXT)
    // if rotating media log is supportd on NVMe, then we can also support the
...
static eReturnValues sntl_Translate_Informational_Exceptions_Log_Page_2F(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    // if a critical warning bit is set, should we return failure??? The old
    // translation whitepaper does not say to do this... if ()
    //{
...
static eReturnValues sntl_Translate_Start_Stop_Cycle_Log_0x0E(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
            ...
            if (SUCCESS == nvme_Get_Log_Page(scsiIoCtx->device, &supLogs))
            {
                ...
                if (rotMediaSup & BIT0)
                {
                    ...
                    if (SUCCESS == nvme_Get_Log_Page(scsiIoCtx->device, &rotationMediaLog))
                    {
                        ...
                        // TODO: Vendor unique parameters 8004 and 8006 for the failed
                        // counts reported in NVMe
                    }
                    else
                    {
                ...
                {
                    // rotating media log is not supported...so call this invalid field in
                    // CDB
                    fieldPointer = 3;
                    bitPointer   = 7;
    ...
    else
    {
        ...
        // Set up sense data for an error for invalid field in CDB, specifying the
        // log page
        fieldPointer = 3;
        bitPointer   = 7;
        sntl_Set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, true, true, bitPointer,
...
static eReturnValues sntl_Translate_Self_Test_Results_Log_0x10(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    uint16_t nvmDSTOffset =
        C_CAST(uint16_t,
               UINT16_C(4) + (parameterCode * UINT16_C(28))); // each NVM DST entry is 28B long and the first
                                                              // starts at byte 4. Set this to match the parameter
                                                              // code since that can select an "offset" in the log
         ...
         ++parameterCode, nvmDSTOffset += UINT16_C(28), selfTestOffset += UINT16_C(20))
    {
        ...
        // If we have a valid entry in the NVMe dst log, then set up remaining
        // bytes, otherwise leave set to zero
        if (M_Nibble0(nvmDSTLog[nvmDSTOffset]) != 0x0F)
        {
            ...
            // if (nvmDSTLog[nvmDSTOffset + 2] & BIT2 && nvmDSTLog[nvmDSTOffset + 2] &
            // BIT3)
            //{
            ...
            // else
            {
                ...
                case 14:
                default:
                    senseKey = SENSE_KEY_NO_ERROR; // don't set an error, but setup
                                                   // remaining fields
                    additionalSenseCode          = 0x40;
                    additionalSenseCodeQualifier = 0x80 + M_Nibble0(nvmDSTLog[nvmDSTOffset]);
                    break;
        ...
        if (pageControl == LPC_CUMULATIVE_VALUES)
        {
            ...
            case 0x00: // supported pages
                ...
                case 0xFF:                                                       // supported pages and subpages
                    ret = sntl_Translate_Supported_Log_Pages(device, scsiIoCtx); // Update this page as additional pages
                                                                                 // of support are added!
                    break;
                default:
                    fieldPointer = 3;
                    bitPointer   = 7;
            ...
            case LP_START_STOP_CYCLE_COUNTER: // start-stop cycle counter log page
                ...
                {
                case 0:
                    // This will only be supported on rotating media for start-stop cycle
                    // counter and load-unload counts
                    if (device->drive_info.media_type == MEDIA_HDD) // this check is good enough for now for how SNTL
                                                                    // gets used today - TJE
                    {
                                                                             ...
                                                                             uint16_t   allocationLength)
{
    ...
    {
        readWriteErrorRecovery[offset + 2] = BIT7 | BIT6; // awre = 1, arre = 1, tb = 0, rc = 0, eer = 0, per = 0,
                                                          // dte = 0, dcr = 0
                                                           ...
                                                           uint16_t   allocationLength)
{
    ...
    if (pageControl == 0x1)     // changeable
    {
        ...
        if (device->drive_info.IdentifyData.nvme.ctrl.vwc & BIT0)
        {
            // TODO: if sel field of get features is supported, send that command to
            // query if VWC is changeable?
    ...
    else // saved, current, and default.
    {
        if (device->drive_info.IdentifyData.nvme.ctrl.vwc & BIT0)
        {
            ...
            if (SUCCESS == nvme_Get_Features(device, &getVWC))
            {
                ...
                {
                    caching[offset + 2] = BIT2; // ic = 0, abpf = 0, cap = 0, disc = 0,
                                                // size = 0, wce = 1, mf = 0, rcd = 0
                }
            }
            else
            {
                                                           ...
                                                           uint16_t   allocationLength)
{
    ...
    {
        controlPage[offset + 2] |= BIT2 | BIT1; // TST = 0, TMF_only = 0, dpicz = 0,
                                                // dsense = 1, gltsd = 1, rlec = 0
                                                                  ...
                                                                  uint16_t   allocationLength)
{
    ...
    M_USE_UNUSED(pageControl); // This is not being used as all bytes are the same
                               // for this mode page today...
    // set the mode data length
    if (longHeader)
    {
                                                            ...
                                                            uint16_t   pageLength)
{
    ...
    {
        // drive doesn't support Volatile write cache, so we need to set an error
        // for invalid field in CDB
        bitPointer   = 2;
        fieldPointer = 2;
        sntl_Set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, false, true, bitPointer,
                                                             fieldPointer);
...
// TODO: a way to cache changes from the incoming block descriptor to change
// sector size in a format command.
static eReturnValues sntl_Translate_SCSI_Mode_Select_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (pageFormat)
    {
        ...
        case 0x08:
            ...
            break;
            // while we do use these pages, nothing is changable, so just return an
            // error for now...
        // case 0x0A:
        //     switch (subpage)
        //     {
        //     case 0://control mode page
        //         ret = sntl_Translate_Mode_Select_Control_0Ah(device, scsiIoCtx,
        //         &scsiIoCtx->pdata[headerLength + blockDescriptorLength],
        //         pageLength); break;
        //     default:
        //         fieldPointer = C_CAST(uint16_t, headerLength +
        //         blockDescriptorLength + UINT16_C(1));//plus one for subpage
        //         bitPointer = 7;
        //         sntl_Set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor,
        //         false, true, bitPointer, fieldPointer); ret = NOT_SUPPORTED;
        //         sntl_Set_Sense_Data_For_Translation(scsiIoCtx->psense,
        //         scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST, 0x26, 0,
        //         device->drive_info.softSATFlags.senseDataDescriptorFormat,
        ...
        //         ret = sntl_Translate_Mode_Select_Power_Conditions_1A(device,
        //         scsiIoCtx, &scsiIoCtx->pdata[headerLength +
...
static eReturnValues sntl_Translate_SCSI_Verify_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    case 2: // not defined
        ...
        sntl_Set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, SENSE_KEY_ILLEGAL_REQUEST,
                                            ...
                                            senseKeySpecificDescriptor, 1);
        return NOT_SUPPORTED;
    case 3: // compare a single logical block of data to each LBA in the
            // range...SNTL does not specify this mode...but we can probably
            // implement it ourselves as an extension
        fieldPointer = 1;
        bitPointer   = 2;
        sntl_Set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, true, true, bitPointer,
                                                             fieldPointer);
...
static eReturnValues sntl_Translate_SCSI_Report_Luns_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    case 0x02:
        // read the identify active namespace list
        {
            ...
            uint8_t* activeNamespaces =
                C_CAST(uint8_t*, safe_calloc_aligned(4096, sizeof(uint8_t), device->os_info.minimumAlignment));
            if (activeNamespaces)
            {
                if (SUCCESS == nvme_Identify(device, activeNamespaces, 0, 2))
                {
                    ...
                    reportLunsData = C_CAST(uint8_t*, safe_calloc(reportLunsDataLength, sizeof(uint8_t)));
                    if (reportLunsData)
                    {
...
static eReturnValues sntl_Translate_SCSI_Test_Unit_Ready_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
...
#if defined(SNTL_EXT)
    // If the device supports sanitize or DST, check if either of these is in
    // progress to report that before returing the default "ready"
    if (scsiIoCtx->device->drive_info.IdentifyData.nvme.ctrl.sanicap != 0)
    {
        ...
        } // no need for an else. We shouldn't fail just because this log read
          // failed.
    }
#endif
    // SNTL only specifies saying if the drive is ready for commands or not...just
    // going to say ready.
    sntl_Set_Sense_Data_For_Translation(scsiIoCtx->psense, scsiIoCtx->senseDataSize, 0, 0, 0,
                                        device->drive_info.softSATFlags.senseDataDescriptorFormat, M_NULLPTR, 0);
    return ret;
...
static eReturnValues sntl_Translate_SCSI_Write_Long(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (device->drive_info.IdentifyData.nvme.ctrl.oncs & BIT1) // check that write uncorrectable command is supported
    {
        ...
        if (byteTransferLength == 0)
        {
            ...
            {
                if (writeUncorrectableError) // NOTE! SNTL version 1.1 has this marked
                                             // as must be zero, but that doesn't match
                                             // later versions or what is in SBC for a
                                             // non-data version of this command!
                {
                    if (physicalBlock)
                    {
...
static eReturnValues sntl_Translate_SCSI_Send_Diagnostic_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (parameterListLength == 0)
    {
        ...
        if (selfTest)
        {
            ret = SUCCESS; // nothing to do! We may be able to take hints from SAT to
                           // implement some kind of similar extension in the future.
        }
        else
        {
#if defined(SNTL_EXT)
            ...
            {
                // NOTE: doing all namespaces for now...not sure if this should be
            ...
            else
            {
                ...
                if (selfTestCode == 0)
                {
                    ret = SUCCESS; // nothing to do! Take hints from SAT to translate the
                                   // self test code to a DST if DST is supported.
                }
                else
                {
                    ret          = NOT_SUPPORTED;
...
static eReturnValues sntl_Translate_SCSI_Write_Buffer_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    case 0x05: // Full buffer given.
        ...
        {
            // send in 1 command, followed by activate. TODO: ifdef for sending in
            // chunks? (might be needed in case system doesn't allow a transfer of
            // that size)
        ...
        else // these fields are reserved or vendor specific so make sure they are
             // zeroed out
        {
            sntl_Set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, true, true, bitPointer,
                                                                 fieldPointer);
    ...
    case 0x0E: // Firmware image download
        ...
        {
            // Check the granularity requirements from fwug in controller identify
            // data so we can check the command properly before issuing it.
            ...
            if ((bufferOffset % granularity) == 0 &&
                (parameterListLength % granularity) == 0) // check length and offset for the same granularity
                                                          // requirements!
            {
                ret =
    ...
    case 0x0F: // Firmware image commit
        if (((fieldPointer = 1) != 0 && (bitPointer = 7) != 0 && modeSpecific == 0) &&
            ...
            ((fieldPointer = 6) != 0 && (bitPointer = 7) != 0 && parameterListLength == 0))
        {
            // TODO: Store a way of to switch to existing firmware images? This would
            // be better to handle when we're switching between existing
            // images...unlikley with SCSI translation though
            nvmeFWCommitAction commitAction = NVME_CA_REPLACE_ACTIVITE_ON_RST;
            if (device->drive_info.IdentifyData.nvme.ctrl.frmw & BIT4)
            {
                commitAction = NVME_CA_ACTIVITE_IMMEDIATE;
        ...
        break;
    case 0x07: // Download code and activate on final segment...we cannot support
               // this without some other way of notifying us that it's the final
               // segment. Can reinvestigate it later!
...
static eReturnValues sntl_Translate_SCSI_Start_Stop_Unit_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    // filter out invalid fields
    if (((fieldPointer = 1) != 0 &&
         M_GETBITRANGE(scsiIoCtx->cdb[1], 7, 0) != 0) // bit zero is immediate which we don't support
    ...
    case 0x00: // start valid
        if (powerConditionModifier == 0)
        {
            ...
            else
            {
                ...
                memset(&features, 0, sizeof(nvmeFeaturesCmdOpt));
                features.fid = NVME_FEAT_POWER_MGMT_;
                // send lowest state, which is a higher number value for lowest power
                // consumption (zero means max).
    ...
    case 0x02: // Idle (check power condition modifier for specific idle state)
        ...
        break;
    // case 0x0B://Force Standby (check power condition modifier for specific idle
    // state)
    //     enable = true;
    //     //fall through
...
static eReturnValues sntl_Translate_SCSI_Unmap_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    else if (parameterListLength != 0 && scsiIoCtx->pdata)
    {
        ...
        if (unmapBlockDescriptorLength > 0)
        {
            ...
            uint64_t numberOfLBAsToDeallocate = 0; // this will be checked later to make sure it isn't greater than
                 ...
                 unmapBlockDescriptorIter += 16, numberOfBlockDescriptors++)
            {
                ...
                if (numberOfLBAsToDeallocate > (UINT64_C(255) * UINT32_MAX)) // max of 255 ranges * UINT32_MAX for
                                                                             // number LBAs in each range
                {
                    // not setting sense key specific information because it's not clear
                    // in this condition what error we should point to
                    ret = FAILURE;
                ...
                while (unmapNumberOfLogicalBlocks > 0)
                {
                    ...
                    nvmeDSMOffset += 16;
                    // check if the NVMe DSM buffer is full...if it is and there are more
...
static eReturnValues sntl_Translate_SCSI_Request_Sense_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    // NOTE: DST progress should only report like this under request sense. In
    // test unit ready, DST in progress should only happen for foreground mode
    // (i.e. captive) which isn't supported on NVMe
        ...
        if (SUCCESS == nvme_Get_Log_Page(device, &dstLog))
        {
            ...
            if (currentSelfTest == 0x1 || currentSelfTest == 0x2) // DST in progress
            {
                ...
                uint16_t dstProgress =
                    656 * M_GETBITRANGE(logPage[1], 6,
                                        0); // This comes out very close to the actual percent...close
...
static eReturnValues sntl_Translate_Persistent_Reserve_In(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    uint32_t      persistentReserveDataLength = 8; // start with this...it could
                                                   // change.
    DECLARE_ZERO_INIT_ARRAY(uint8_t, senseKeySpecificDescriptor, SNTL_SENSE_KEY_SPECIFIC_DESCRIPTOR_LENGTH);
    uint8_t  bitPointer       = 0;
    uint16_t fieldPointer     = 0;
    ...
    case 0: // read keys
    {
        DECLARE_ZERO_INIT_ARRAY(uint8_t, nvmeReportKeys,
                                4096); // I hope this is big enough...may need to redo this!
        ...
        persistentReserveData = C_CAST(uint8_t*, safe_calloc(persistentReserveDataLength, sizeof(uint8_t)));
    ...
    {
        DECLARE_ZERO_INIT_ARRAY(uint8_t, nvmeReport,
                                4096); // I hope this is big enough...may need to redo this!
        ...
        // uint16_t numberOfRegisteredControllers =
        // M_BytesTo2ByteValue(nvmeReport[5], nvmeReport[6]); figure out what, if
        // any controller is holding a reservation
        ...
        if (persistentReserveData)
        {
            ...
            if (foundReservationActive)
            {
                ...
                case 6:
                    ...
                    break;
                default:
                    persistentReserveData[21] = 0x0F; // set to something invalid..we
    ...
    {
        // send NVMe identify command, CNS set to 0, current namespace being
        // queried.
    ...
    case 3: // read full status
    {
        ...
        if (persistentReserveData)
        {
                 ...
                 persistentReseverOffset += 32, nvmeReportOffset += 24)
            {
                ...
                // set r_holder
                if (nvmeReport[nvmeReportOffset + 2] & BIT0) // SNTL says bit 1, but that is not correct as that bit is
                                                             // still reserved...
                {
                ...
                case 6:
                    ...
                    break;
                default:
                    persistentReserveData[persistentReseverOffset + 13] = 0x0F; // set to something invalid..we should
                                                                                // be able to get this right
                    break;
                }
                // set relative target port identifier to nvme host identifier (swap
                // endianness)
                ...
                // set transport ID to nvme host idnetifier (remember diffferent
                // endianness...)
...
static eReturnValues sntl_Translate_Persistent_Reserve_Out(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    if (serviceAction == 7)
    {
        // different parameter data format...handle it separate of other
        // translations. check if any reserved fields are set.
        if (((fieldPointer = 16) != 0 && (bitPointer = 0) == 0 && scsiIoCtx->pdata[16] != 0) // reserved
            || ((fieldPointer = 17) != 0 && (bitPointer = 0) == 0 &&
                M_GETBITRANGE(scsiIoCtx->pdata[17], 7, 1) != 0) // reserved and unreg bit...not supporting unreg since
                                                                // it isn't mentioned in SNTL
        )
        {
            if (bitPointer == 0)
            {
                ...
                {
                    reservedByteVal =
                        scsiIoCtx->pdata[fieldPointer] & 0xFE; // remove lower bits since they may be valid
                }
                while (reservedByteVal > 0 && counter < 8)
                {
                    reservedByteVal >>= 1;
        ...
        // TODO: the SNTL doesn't mention what, if anything, to do with the
        // transport ID that can be provided here...ignoring it for now since I
        // don't see a way to send that to the nvme drive. NOTE: NVMe spec doesn't
        // mention the unreg bit or the relative target port identifier...should it
        // be an error? now translate to the register command with the correct
        // inputs for the NVMe data iekey = 0, rrega = 010 (replace)
    ...
    {
        // check if bytes 16 though 19 are set...they are obsolete and not
        // supported.
        ...
        )
        {
            if (bitPointer == 0)
            {
                ...
                {
                    reservedByteVal =
                        scsiIoCtx->pdata[fieldPointer] & 0xF0; // remove lower bits since they may be valid
                }
                while (reservedByteVal > 0 && counter < 8)
                {
                    reservedByteVal >>= 1;
        ...
        bool aptpl = scsiIoCtx->pdata[20] & BIT0; // used on register and register and ignore
                                                  // existing key actions. ignored otherwise
        // NOTE: parameter data is DIFFERENT for register and move. Everything else
        ...
        case 6: // register and ignore existing key
        {
            ...
            // if service action key is zero, set rrega to 001 (unregister) and set
            // CRKEY to the reservation key. else reservation key is ignored. CRKEY is
            // as though it is reserved. RREGA set to zero
            if (is_Empty(&scsiIoCtx->pdata[8], 8))
            {
                ...
                buffer[7] = scsiIoCtx->pdata[0];
                // set the service action reservation key to new reservation key field
                // (can safely do this as it's just setting zeros)
                buffer[8]  = scsiIoCtx->pdata[15];
                buffer[9]  = scsiIoCtx->pdata[14];
                buffer[10] = scsiIoCtx->pdata[13];
                buffer[11] = scsiIoCtx->pdata[12];
            ...
            else
            {
                ...
                // crkey is reserved (leave set to zero, ignoring any provided
                // reservation key)
                rrega = 0; // register
            ...
            if (aptpl) // aptpl
            {
                ...
                if (device->drive_info.IdentifyData.nvme.ns.rescap & BIT0)
                {
                    // send a set features for reservation persistence with PTPL set to 1
                    // before sending the reservation register command
                ...
                {
                    // ERROR! Invalid field in parameter list since the drive doesn't
                    // support this mode!
                    bitPointer   = 0;
                    fieldPointer = 17;
                    sntl_Set_Sense_Key_Specific_Descriptor_Invalid_Field(senseKeySpecificDescriptor, false, true,
        ...
        break;
        // case 7://register and move <- handled above in if since parameter data is
        // different
                                               ...
                                               uint32_t* dataLength)
{
    ...
    *dataLength       = 4;         // add onto this for each of the different commands below,
    ...
    //     }
    //     pdata[0][offset + 0] = operationCode;
    //     pdata[0][offset + 1] = 0x37;//protection information is not supported,
    //     complete list is not supported, but we are supposed to be able to take
    //     2 different defect list formats even though we don't use them - TJE
    //     pdata[0][offset + 2] = RESERVED;
    //     pdata[0][offset + 3] = RESERVED;
    //     pdata[0][offset + 4] = RESERVED;
    //     pdata[0][offset + 5] = controlByte;//control byte
                                                                  ...
                                                                  uint32_t* dataLength)
{
    ...
    *dataLength       = 4;         // add onto this for each of the different commands below,
        ...
        if (device->drive_info.IdentifyData.nvme.ctrl.oacs & BIT2)
        {
            ...
            case 0x05: // download
                ...
                *pdata = C_CAST(uint8_t*, safe_calloc(*dataLength, sizeof(uint8_t)));
                ...
                pdata[0][offset + 9] = controlByte; // control byte
                break;
                // Mode 7 not supported since we don't always know when we are at the
                // final download segment. case 0x07://download offsets
                //         cdbLength = 10;
                //         *dataLength += cdbLength;
                //         *pdata = C_CAST(uint8_t*, safe_calloc(*dataLength,
                //         sizeof(uint8_t))); if (!*pdata)
                //         {
                //             return MEMORY_FAILURE;
                //         }
                //         pdata[0][offset + 0] = operationCode;
            ...
            case 0x0D: // download offsets defer
                ...
                *pdata = C_CAST(uint8_t*, safe_calloc(*dataLength, sizeof(uint8_t)));
                ...
                pdata[0][offset + 0] = operationCode;
                pdata[0][offset + 1] = (serviceAction & 0x001F) | BIT7; // service action plus mode specific set
    ...
    //         pdata[0][offset + 9] = controlByte;//control byte
    //         break;
    // case 0x1C://TODO: only show this when ISL log is supported...for now this
    // should be ok
                                                               ...
                                                               uint32_t* dataLength)
{
    ...
    *pdata                           = C_CAST(uint8_t*, safe_calloc(reportAllMaxLength, sizeof(uint8_t)));
    ...
    // pdata[0][offset + 1] = RESERVED;
    // pdata[0][offset + 2] = M_Byte1(0);//service action msb
    // pdata[0][offset + 3] = M_Byte0(0);//service action lsb if non zero set byte
    // 5, bit0 pdata[0][offset + 4] = RESERVED;
    ////skipping offset 5 for this
    // pdata[0][offset + 6] = M_Byte1(CDB_LEN_6);
    // pdata[0][offset + 7] = M_Byte0(CDB_LEN_6);
    // offset += 8;
    ...
    if (device->drive_info.IdentifyData.nvme.ctrl.oacs & BIT2)
    {
        ...
        //     pdata[0][offset + 3] = M_Byte0(0x07);//service action lsb if non zero
        //     set byte 5, bit0 pdata[0][offset + 4] = RESERVED; pdata[0][offset +
        //     5] = BIT0; pdata[0][offset + 6] = M_Byte1(CDB_LEN_12);
        //     pdata[0][offset + 7] = M_Byte0(CDB_LEN_12);
        //     offset += 8;
        //     if (rctd)
        //     {
    ...
    // not able to support normal read buffer since there is no translation...but
    // maybe the descriptor since it could tell us offset requirements for
    // downloads??? pdata[0][offset + 0] = READ_BUFFER_CMD; pdata[0][offset + 1] =
    // RESERVED; pdata[0][offset + 2] = M_Byte1(0x02);//service action msb
    // pdata[0][offset + 3] = M_Byte0(0x02);//service action lsb if non zero set
    // byte 5, bit0 pdata[0][offset + 4] = RESERVED; pdata[0][offset + 5] = BIT0;
    // pdata[0][offset + 6] = M_Byte1(CDB_LEN_12);
    // pdata[0][offset + 7] = M_Byte0(CDB_LEN_12);
    // offset += 8;
    // if (rctd)
    ...
    // pdata[0][offset + 3] = M_Byte0(0x03);//service action lsb if non zero set
    // byte 5, bit0 pdata[0][offset + 4] = RESERVED; pdata[0][offset + 5] = BIT0;
    // pdata[0][offset + 6] = M_Byte1(CDB_LEN_12);
    // pdata[0][offset + 7] = M_Byte0(CDB_LEN_12);
    // offset += 8;
...
//         pdata[0][offset + 3] = M_Byte0(0x1C);//service action lsb if non zero
//         set byte 5, bit0 pdata[0][offset + 4] = RESERVED; pdata[0][offset +
//         5] = BIT0; pdata[0][offset + 6] = M_Byte1(CDB_LEN_12);
//         pdata[0][offset + 7] = M_Byte0(CDB_LEN_12);
//         offset += 8;
...
// #endif
    ...
    // pdata[0][offset + 2] = M_Byte1(0);//service action msb
    // pdata[0][offset + 3] = M_Byte0(0);//service action lsb if non zero set byte
    // 5, bit0 pdata[0][offset + 4] = RESERVED; pdata[0][offset + 5] = 0;
    // pdata[0][offset + 6] = M_Byte1(CDB_LEN_12);
    // pdata[0][offset + 7] = M_Byte0(CDB_LEN_12);
    // offset += 8;
    // if (rctd)
...
eReturnValues sntl_Translate_SCSI_Command(tDevice* device, ScsiIoCtx* scsiIoCtx)
{
    ...
    // All functions within this switch-case should dummy up their own sense data
    ...
    case 0x9E:
        ...
        break;
    // To Support format, we need to store the last block descriptor so we format
    // with the correct block size when running format. We also need to send back
    // "format corrupt" until the format has been done. case SCSI_FORMAT_UNIT_CMD:
    ...
    case READ16: // read commands
        ...
        break;
        // SNTL spec doesn't define it, but we should add a way to read the
        // telemetry log through here similar to SAT's translation for SATA Internal
        // Status log
#if defined(SNTL_EXT)
        // case READ_BUFFER_CMD:
    ...
    case 0xA3: // check the service action for this one!
        ...
        case 0x0C: // report supported op codes <- this is essentially returning
                   // either a massive table of supported commands, or it is sending
                   // back data or an error based off a switch statement
    ...
    case SECURITY_PROTOCOL_OUT:
        ...
        break;
    case SEND_DIAGNOSTIC_CMD: // SNTL's translation is lacking. Should add DST
                              // support similar to SAT if the drive supports DST
    ...
    case VERIFY12:
    case VERIFY16:
        ret = sntl_Translate_SCSI_Verify_Command(device,
                                                 scsiIoCtx); // compare command
        break;
    case WRITE6:
    case WRITE10:
    case WRITE12:
    case WRITE16:
        ...
        break;
#if defined(SNTL_EXT)
        // These are not part of SNTL. We could add support IF the bytecheck field
        // is specified the same as verify translation requires. The similar command
        // (but reverse order of operations) is below (compare and write) case
        // WRITE_AND_VERIFY_10: case WRITE_AND_VERIFY_12: case WRITE_AND_VERIFY_16:
        //   ret = sntl_Translate_SCSI_Write_And_Verify_Command(device,
        //   scsiIoCtx);//ATA Write, then read-verify commands break;
#endif
    ...
    case 0x9F: // write uncorrectable ext-check service action for 11h
        ...
        break;
#if defined(SNTL_EXT)
        // SNTL doesn't describe these, but they could be added similar to SAT's
        // specification case WRITE_SAME_10_CMD://Sequential write commands case
        // WRITE_SAME_16_CMD://Sequential write commands
