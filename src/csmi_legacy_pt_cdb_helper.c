...
// \brief Defines the constants, structures, & functions to help with legacy
// CSMI ATA passthrough CDB implementation
...
// This is an E0 command, much like SAT to issue a generic command through STP
// by making the drive look like SCSI...This is likely not even used since SAT
// does this... From section 7.1 of CSMI spec
...
eReturnValues build_CSMI_Passthrough_CDB(uint8_t cdb[CSMI_PASSTHROUGH_CDB_LENGTH], ataPassthroughCommand* ataPtCmd)
{
    ...
    if (cdb && ataPtCmd)
    {
        ...
        // check if the command is doing a transfer number in words...otherwise set
        // blocks
        ...
        // TODO: If a rare case occurs and this is used + the rare case of a non
        // 512B logical sector size, we may need to change from the blocks bit
        // below...this combination is highly unlikely to be found though and SAT
        // can conver this as necessary.
        else
        {
            cdb[1] |= BIT7; // set the blocks bit
            // set block count (OLD spec says this is in terms of 512...might be
            // different for new 4k drives though...not like this is even used though)
...
{
    // TODO: Whenever a driver is found using this legacy CDB, we need to figure
    // out how RTFRs are returned, IF there is a way that they are returned.
    M_USE_UNUSED(device);
...
eReturnValues send_CSMI_Legacy_ATA_Passthrough(tDevice* device, ataPassthroughCommand* ataCommandOptions)
{
    ...
    if (ret == SUCCESS)
    {
        ...
        //     ret = IN_PROGRESS;
        // }
        // else if (ataCommandOptions->rtfr.status == 0 && ret == SUCCESS)//the IO
        // was successful, however we didn't fill in any tfrs...This should allow us
        // one more chance to dummy up values
        //{
        //     ret = SUCCESS;
        //     ataCommandOptions->rtfr.status = ATA_STATUS_BIT_READY |
        //     ATA_STATUS_BIT_SEEK_COMPLETE;//just make sure we set passing status
        //     for anything that cares to check rtfrs
        // }
    ...
    // before we get rid of the sense data, copy it back to the last command sense
    // data
    memset(device->drive_info.lastCommandSenseData, 0,
           SPC3_SENSE_LEN); // clear before copying over data
    memcpy(&device->drive_info.lastCommandSenseData[0], &ataCommandOptions->ptrSenseData,
           M_Min(SPC3_SENSE_LEN, ataCommandOptions->senseDataSize));
    // memcpy(&device->drive_info.lastCommandRTFRs, &ataCommandOptions->rtfr,
