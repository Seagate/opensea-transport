...
// ******************************************************************************************
//
// \file ata_legacy_cmds.c   Implementation for ATA Spec command functions that
// are old/obsolete from ATA specs prior to ATA-ATAPI7. Also contains CHS
// versions of some other commands.
...
// This file only contains commands that include retries today, but many old
// commands have a no-retries version. These are not implemented as we want the
// retries. Can add these definitions if needed in the future.
                                           ...
                                           bool     extendedCmd)
{
    ...
    ataCommandOptions.tfr.CylinderHigh         = M_Byte1(cylinder);
    ataCommandOptions.tfr.SectorCount          = M_Byte0(dataSize / device->drive_info.deviceBlockSize);
    if (extendedCmd) // This is only here to use a different command op code if
                     // someone using a newer 48bit LBA drive wants to (for some
                     // reason) talk to it with CHS, but the 28bit version of the
                     // command isn't supported. -TJE
    {
        ataCommandOptions.commandType         = ATA_CMD_TYPE_EXTENDED_TASKFILE;
                                       ...
                                       uint32_t dataSize)
{
    ...
    ataCommandOptions.commandType              = ATA_CMD_TYPE_TASKFILE;
    ataCommandOptions.commandDirection         = XFER_DATA_IN;
    ataCommandOptions.ataCommandLengthLocation = ATA_PT_LEN_TPSIU; // this must be used since the transfer is some
                                                                   // number of bytes
                                        ...
                                        uint32_t dataSize)
{
    ...
    ataCommandOptions.commandType              = ATA_CMD_TYPE_TASKFILE;
    ataCommandOptions.commandDirection         = XFER_DATA_OUT;
    ataCommandOptions.ataCommandLengthLocation = ATA_PT_LEN_TPSIU; // this must be used since the transfer is always 1
                                                                   // sector to the drive. The sector count says how
                                                                   // many sectors to write the data sent to
    ataCommandOptions.ataTransferBlocks = ATA_PT_NUMBER_OF_BYTES;
    ataCommandOptions.ptrData           = ptrData;
    ataCommandOptions.dataSize          = dataSize;
    ataCommandOptions.commadProtocol    = ATA_PROTOCOL_PIO;
    ...
    else if (subcommand == LEGACY_WRITE_SAME_INITIALIZE_ALL_SECTORS)
    {
        ...
        ataCommandOptions.ataCommandLengthLocation = ATA_PT_LEN_SECTOR_COUNT; // this must be used since the transfer is
                                                                              // always 1 sector to the drive. The
                                                                              // sector count says how many sectors to
                                                                              // write the data sent to
