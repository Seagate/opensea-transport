                                            ...
                                            bool     znr)
{
    ...
    case ATA_DRIVE:
    {
        ...
        //       In order to be consistent with SCSI and NVMe specifications, this
        //       should be set when the Device supports it. Basically it means that
        //       the provided pattern WILL DEFINITELY be the pattern on the drive no
        //       matter how many passes or inversions happen. When this is not
        //       supported/set then the device may or may not be consistent with
        //       this behavior...it's up to the firmware to decide. Because of this,
        //       this bit will be set when it is discovered as supported whenever
        //       possible -TJE
    ...
    break;
    case SCSI_DRIVE:
        // overwrite passes set to 0 on scsi is reserved. This is being changed to
        // the maximum for SCSI to mean 16 passes
        if ((overwritePasses & 0x0F) == 0)
        {
            overwritePasses = 0x1F;
        }
        // we need to allocate a zero pattern if none was provided. 4 bytes is
        // enough for this (and could handle SAT transaltion if necessary).
        if (!pattern)
        {
            localPattern = true;
...
eReturnValues fill_Drive_Info_Data(tDevice* device)
{
    ...
    if (device)
    {
        ...
        {
        case IDE_INTERFACE:
            // We know this is an ATA interface and we SHOULD be able to send either
            // an ATA or ATAPI identify...but that doesn't work right, so if the OS
            // layer told us it is ATAPI, do SCSI device discovery
            ...
            else
            {
                ...
                if (status == FAILURE || status == UNKNOWN)
                {
                    ...
                    // could not enumerate as ATA, try SCSI in case it's taking CDBs at
                    // the low layer to communicate and not translating more than the A1
                    // op-code to check it if's a SAT command.
                    status = fill_In_Device_Info(device);
                }
            }
            break;
        ...
        case USB_INTERFACE:
            // Previously there was separate function to fill in drive info for USB,
            // but has now been combined with the SCSI fill device info. Low-level
            // code capable of figuring out hacks for working with these devices is
            // now able to preconfigure most flags
            status = fill_In_Device_Info(device);
            break;
        case NVME_INTERFACE:
            ...
            break;
        case RAID_INTERFACE:
            // if it's RAID interface, the low-level RAID code may already have set
            // the drive type, so treat it based off of what drive type is set to
            switch (device->drive_info.drive_type)
            {
            case ATA_DRIVE:
                status = fill_In_ATA_Drive_Info(device);
        ...
        case SCSI_INTERFACE:
        default:
            // call this instead. It will handle issuing scsi commands and at the end
            // will attempt an ATA Identify if needed
            status = fill_In_Device_Info(device);
            break;
        }
    }
                                                   ...
                                                   uint32_t      timeoutSeconds)
{
    ...
    // ret = ata_Download_Microcode(device, ataDLMode, xferLen /
    // LEGACY_DRIVE_SEC_SIZE, offset / LEGACY_DRIVE_SEC_SIZE, useDMA, ptrData,
    // xferLen); Switching to this new function since it will automatically try
    // DMA mode if supported by the drive. If the controller or driver don't like
    // issuing DMA mode, this will detect it and retry the command with PIO mode.
                                                    ...
                                                    bool          forceDisableReset)
{
    ...
    case DL_FW_ACTIVATE:
    {
        ...
        if (!nvmeForceCA)
        {
            ...
            // This is stored in tDevice so we can use it, plus the drive supported
...
#endif // _WIN32
            {
                ...
                {
                    // this activate action can be used for replacing or activating
                    // existing images if the controller supports it.
                    nvmeCommitAction = NVME_CA_ACTIVITE_IMMEDIATE;
                }
                else
                {
        ...
        if (statusCodeType == NVME_SCT_COMMAND_SPECIFIC_STATUS)
        {
            ...
            case NVME_CMD_SP_SC_FW_ACT_REQ_MAX_TIME_VIOALTION:
                ...
                {
                    // needs to be reissued for an activate on reset instead due to
        ...
        if (issueReset && !forceDisableReset) // if the reset is being forced to not run, there is
                                              // a good reason for it! Listen to this flag ALWAYS
        {
            // send an appropriate reset to the device to activate the firmware.
            // NOTE: On Windows, this is a stub since their API call will do this for
            // us.
            if (subsystem)
            {
                // subsystem reset
                            ...
                            uint32_t dataSize)
{
        ...
        if (dataSize != 0)
        {
            ...
            if (dataSize < LEGACY_DRIVE_SEC_SIZE || dataSize % LEGACY_DRIVE_SEC_SIZE != 0)
            {
                ...
                size_t newBufferSize =
                    (((dataSize + LEGACY_DRIVE_SEC_SIZE) - 1) / LEGACY_DRIVE_SEC_SIZE) * LEGACY_DRIVE_SEC_SIZE;
                tcgBufPtr = C_CAST(
                    uint8_t*, safe_calloc_aligned(newBufferSize, sizeof(uint8_t), device->os_info.minimumAlignment));
            ...
            // ret = ata_Trusted_Send(device, useDMA, securityProtocol,
            // securityProtocolSpecific, ptrData, dataSize); Switching to this new
            // function since it will automatically try DMA mode if supported by the
            // drive. If the controller or driver don't like issuing DMA mode, this
            // will detect it and retry the command with PIO mode.
    ...
    case SCSI_DRIVE:
    {
        // The inc512 bit is not allowed on NVMe drives when sent this command....we
        // may want to remove setting it, but for now we'll leave it here.
        bool inc512 = false;
        if ((dataSize >= LEGACY_DRIVE_SEC_SIZE && (dataSize % LEGACY_DRIVE_SEC_SIZE) == 0) &&
    ...
    case ATA_DRIVE:
    {
        if (dataSize != 0)
        {
            ...
            if (dataSize < LEGACY_DRIVE_SEC_SIZE || dataSize % LEGACY_DRIVE_SEC_SIZE != 0)
            {
                ...
                tcgDataSize =
                    (((dataSize + LEGACY_DRIVE_SEC_SIZE) - 1) / LEGACY_DRIVE_SEC_SIZE) * LEGACY_DRIVE_SEC_SIZE;
                tcgBufPtr = C_CAST(uint8_t*,
                                   safe_calloc_aligned(tcgDataSize, sizeof(uint8_t), device->os_info.minimumAlignment));
                if (!tcgBufPtr)
                {
                    return MEMORY_FAILURE;
                }
            ...
            // ret = ata_Trusted_Receive(device, useDMA, securityProtocol,
            // securityProtocolSpecific, tcgBufPtr, tcgDataSize); Switching to this
            // new function since it will automatically try DMA mode if supported by
            // the drive. If the controller or driver don't like issuing DMA mode,
    ...
    case ATA_DRIVE:
            ...
            device->drive_info.IdentifyData.ata.Word206 & BIT2)
        {
            if (noDataTransfer)
            {
                ...
                // ret = ata_SCT_Write_Same(device, useGPL, useDMA,
                // WRITE_SAME_BACKGROUND_USE_PATTERN_FIELD, startingLba,
                // numberOfLogicalBlocks, zeroPattern, sizeof(zeroPattern) /
                // sizeof(*zeroPattern));
            ...
            {
                // ret = ata_SCT_Write_Same(device, useGPL, useDMA,
                // WRITE_SAME_BACKGROUND_USE_SINGLE_LOGICAL_SECTOR, startingLba,
                 ...
                 (!(is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word053) &&
                  ...
                  && (is_ATA_Identify_Word_Valid(device->drive_info.IdentifyData.ata.Word069) &&
                      (device->drive_info.IdentifyData.ata.Word069 &
                       BIT11)))) // Legacy Write same uses same op-code as read
                                 // buffer DMA, so that command cannot be supported
                                 // or the drive won't do the right thing
        {
            bool    localPattern     = false;
            ...
            if (noDataTransfer)
            {
                pattern      = C_CAST(uint8_t*, safe_calloc_aligned(device->drive_info.deviceBlockSize, sizeof(uint8_t),
        ...
        break;
    case SCSI_DRIVE:
        // todo: if there is no data transfer and the drive doesn't support that
        // feature, we need to allocate local zeroed memory to send as the pattern
        ...
        {
            // write same 16 was made in SBC2 so need to report conformance to version
            // greater than SPC (3) to do this.
...
bool is_Write_Psuedo_Uncorrectable_Supported(tDevice* device)
{
    ...
    case SCSI_DRIVE:
    {
        // check for wu_supp in extended inquiry vpd page (SPC4+) since this matches
        // when it was added to SBC3
...
eReturnValues write_Psuedo_Uncorrectable_Error(tDevice* device, uint64_t corruptLBA)
{
    ...
    case ATA_DRIVE:
        ...
        else // write psuedo uncorrectable command is not supported by this drive.
...
eReturnValues ata_Read(tDevice* device, uint64_t lba, bool forceUnitAccess, uint8_t* ptrData, uint32_t dataSize)
{
    ...
    if (forceUnitAccess)
    {
        // The synchronous commands in here are not able to set a bit for this, so
        // the closest thing is to issue a read-verify to force cached data to the
        // media ahead of the read
    ...
    if (SUCCESS == ret) // don't try the read if the read verify fails
    {
        if (device->drive_info.ata_Options.fourtyEightBitAddressFeatureSetSupported)
        {
            ...
            {
                if (sectors == 65536) // this is represented in the command with sector
                                      // count set to 0
                {
                    sectors = 0;
                }
                ...
                else
                {
                    ...
                    {
                        // use PIO commands
                        // check if read multiple is supported (current # logical sectors
                        // per DRQ data block) Also, only bother with read multiple if it's
                        // a PATA drive. There isn't really an advantage to this on SATA
                        ...
                        {
                            // read multiple supported and drive is currently configured in a
                            // mode that will work.
                            if (device->drive_info.ata_Options.chsModeOnly)
                            {
                                uint16_t cylinder = 0;
                                ...
                                else // Couldn't convert or the LBA is greater than the
                                     // current CHS mode
                                {
                                    ret = NOT_SUPPORTED;
                                }
                    ...
                    else
                    {
                        if (device->drive_info.ata_Options.chsModeOnly)
                        {
                            ...
                            else // Couldn't convert or the LBA is greater than the current
                                 // CHS mode
                            {
                                ret = NOT_SUPPORTED;
                            }
                        ...
                        if (ret != SUCCESS)
                        {
                            // check the sense data. Make sure we didn't get told we have an
                            // invalid field in the CDB. If we do, try turning off DMA mode
                            // and retrying with PIO mode commands.
                            uint8_t senseKey = 0;
                            uint8_t asc      = 0;
                            uint8_t ascq     = 0;
                            uint8_t fru      = 0;
                            ...
                            // Checking for illegal request, invalid field in CDB since this
                            // is what we've seen reported when DMA commands are not
                            // supported.
                            if (senseKey == SENSE_KEY_ILLEGAL_REQUEST && asc == 0x24 && ascq == 0x00)
                            {
                                ...
                                if (ret != SUCCESS)
                                {
                                    // this means that the error is not related to DMA mode
                                    // command, so we can turn that back on and pass up the return
                                    // status.
        ...
        {
            // use the 28bit commands...first check that they aren't requesting more
            // data than can be transferred in a 28bit command, exception being 256
            // since that can be represented by a 0
            if (sectors > 256)
            {
                ret = BAD_PARAMETER;
            }
    ...
    if (device->drive_info.ata_Options.fourtyEightBitAddressFeatureSetSupported)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // use PIO commands
                    // check if read multiple is supported (current # logical sectors per
                    // DRQ data block) Also, only bother with write multiple if it's a
                    // PATA drive. There isn't really an advantage to this on SATA other
                ...
                else
                {
                    ...
                    {
                        // check the sense data. Make sure we didn't get told we have an
                        // invalid field in the CDB. If we do, try turning off DMA mode and
                        // retrying with PIO mode commands.
                        uint8_t senseKey = 0;
                        uint8_t asc      = 0;
                        uint8_t ascq     = 0;
                        uint8_t fru      = 0;
                        get_Sense_Key_ASC_ASCQ_FRU(device->drive_info.lastCommandSenseData, SPC3_SENSE_LEN, &senseKey,
                                                   &asc, &ascq, &fru);
                        // Checking for illegal request, invalid field in CDB since this is
                        // what we've seen reported when DMA commands are not supported.
                        if (senseKey == SENSE_KEY_ILLEGAL_REQUEST && asc == 0x24 && ascq == 0x00)
                        {
                            // turn off DMA mode
                            eATASynchronousDMAMode currentDMAMode = device->drive_info.ata_Options.dmaMode;
                            ...
                            if (ret != SUCCESS)
                            {
                                // this means that the error is not related to DMA mode command,
                                // so we can turn that back on and pass up the return status.
                                device->drive_info.ata_Options.dmaMode = currentDMAMode;
                            }
                        }
                    }
...
eReturnValues scsi_Read(tDevice* device, uint64_t lba, bool forceUnitAccess, uint8_t* ptrData, uint32_t dataSize)
{
    ...
    if (SUCCESS == ret)
    {
        ...
        {
            // This device is in the database or the command support has been
            // determined some other way to allow us to issue a correct command
        ...
        else // Use the generic rules below to issue what will most likely be the
             // correct commands...-TJE
        {
            if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC_3) // SBC2 introduced read 16 command, so checking
                                                                      // for SPC3
            {
                // there's no real way to tell when scsi drive supports read 10 vs read
                // 16 (which are all we will care about in here), so just based on
                // transfer length and the maxLBA
...
eReturnValues scsi_Write(tDevice* device, uint64_t lba, bool forceUnitAccess, uint8_t* ptrData, uint32_t dataSize)
{
    ...
    if (device->drive_info.passThroughHacks.scsiHacks.readWrite.available)
    {
        // This device is in the database or the command support has been determined
        // some other way to allow us to issue a correct command without any other
        // issues.
        if (device->drive_info.passThroughHacks.scsiHacks.readWrite.rw16)
        {
        ...
        else
        {
         ...
         // correct commands...-TJE
    {
        if (device->drive_info.scsiVersion >= SCSI_VERSION_SPC_3) // SBC2 introduced write 16 command, so checking for
                                                                  // SPC3
        {
            // there's no real way to tell when scsi drive supports write 10 vs write
            // 16 (which are all we will care about in here), so just based on
            // transfer length and the maxLBA
...
eReturnValues io_Read(tDevice* device, uint64_t lba, bool forceUnitAccess, uint8_t* ptrData, uint32_t dataSize)
{
    ...
    case RAID_INTERFACE:
        // perform SCSI reads for now. We may need to add unique functions for NVMe
        // and RAID reads later
        return scsi_Read(device, lba, forceUnitAccess, ptrData, dataSize);
    default:
        return NOT_SUPPORTED;
    }
...
eReturnValues io_Write(tDevice* device, uint64_t lba, bool forceUnitAccess, uint8_t* ptrData, uint32_t dataSize)
{
    ...
    case RAID_INTERFACE:
        // perform SCSI writes for now. We may need to add unique functions for NVMe
        // and RAID writes later
        return scsi_Write(device, lba, forceUnitAccess, ptrData, dataSize);
    default:
        return NOT_SUPPORTED;
    }
...
eReturnValues read_LBA(tDevice* device, uint64_t lba, bool forceUnitAccess, uint8_t* ptrData, uint32_t dataSize)
{
    ...
    {
        // Old comment says this function does not always work reliably in
        // Windows...This is NOT functional in other OS's.
...
eReturnValues ata_Read_Verify(tDevice* device, uint64_t lba, uint32_t range)
{
    ...
    if (device->drive_info.ata_Options.fourtyEightBitAddressFeatureSetSupported)
    {
        ...
        {
            if (range == 65536) // this is represented in the command with sector
                                // count set to 0
            {
                range = 0;
            }
...
{
    eReturnValues ret = SUCCESS; // assume success
    // there's no real way to tell when scsi drive supports verify 10 vs verify 16
    // (which are all we will care about in here), so just based on transfer
    // length and the maxLBA
...
eReturnValues nvme_Verify_LBA(tDevice* device, uint64_t lba, uint32_t range)
{
    ...
    {
        // NVME doesn't have a verify command like ATA or SCSI, so we're going to
        // substitute by doing a read with FUA set....should be the same minus doing
        // a data transfer.
...
eReturnValues verify_LBA(tDevice* device, uint64_t lba, uint32_t range)
{
    ...
    else
    {
        ...
        case RAID_INTERFACE:
            // perform SCSI verifies for now. We may need to add unique functions for
            // NVMe and RAID writes later
            return scsi_Verify(device, lba, range);
        default:
            return NOT_SUPPORTED;
        }
...
{
    // synch/flush cache introduced in SCSI2. Not going to check for it though
    // since some USB drives do support this command and report SCSI or no
    // version. - TJE there's no real way to tell when SCSI drive supports
    // synchronize cache 10 vs synchronize cache 16 (which are all we will care
    // about in here), so just based on the maxLBA
    eReturnValues ret = SUCCESS;
...
eReturnValues flush_Cache(tDevice* device)
{
    ...
    else
    {
        ...
        case RAID_INTERFACE:
            // perform SCSI writes for now. We may need to add unique functions for
            // NVMe and RAID writes later
            return scsi_Synchronize_Cache_Command(device);
        default:
            return NOT_SUPPORTED;
