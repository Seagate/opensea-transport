...
extern "C"
{
    ...
    // scsi/ata devices using FreeBSD Common Access Method (CAM) library.
...
// This is the maximum timeout a command can use in CAM passthrough with
// FreeBSD...1193 hours NOTE: CAM also supports an infinite timeout, but that is
...
#define CAM_MAX_CMD_TIMEOUT_SECONDS 4294967
    ...
    //
    //  Entry:
    //!   \param scsiIoCtx = pointer to the ScsiIoCtx struct describing the command
    //!   to send to a device
    //
    //  Exit:
    ...
    //
    //! \brief   Description:  Dummy function since there do not appear to be
    //! similar shared vs exclusive access flags to cam device.
    //
    //  Entry:
    //!   \param[in]  device = pointer to device context!
    //!
    ...
    //
    //! \brief   Description:  Dummy function since there do not appear to be
    //! similar shared vs exclusive access flags to a cam device.
    //
    //  Entry:
    //!   \param[in]  device = pointer to device context!
    //!
