...
// \brief Defines the structures, ioctls, etc for Intel's RST driver specific
// IOCTLs for additional support beyond CSMI spec
//        This is only for Windows. All things in this file is defined by Intel
//        documentation
...
#    endif
    ...
    // Intel RST NVMe passthrough definitions. RST
    // Version 14.6, 14.8, 15.x, 16.x, 17.x 18.x
...
#    define NVME_PASS_THROUGH_VERSION 1
    ...
    typedef struct _GENERIC_COMMAND
    {
        ULONG DWord0;
        ULONG DWord1;
        ULONG DWord2;
    ...
    typedef struct _COMPLETION_QUEUE_ENTRY
    {
        ULONG completion0;
        ULONG completion1;
        ULONG completion2;
    ...
    typedef struct _NVME_PASS_THROUGH_PARAMETERS
    {
        GENERIC_COMMAND        Command;
        BOOLEAN                IsIOCommandSet; // False for Admin queue
        COMPLETION_QUEUE_ENTRY Completion;
        ULONG                  DataBufferOffset; // Must be DWORD aligned offset from beginning of
                                                 // SRB_IO_CONTROL
        ULONG DataBufferLength;
        ULONG Reserved[10];
    } NVME_PASS_THROUGH_PARAMETERS;

    typedef struct _NVME_IOCTL_PASS_THROUGH
    {
        SRB_IO_CONTROL Header;
        UCHAR          Version;                // Set to NVME_PASS_THROUGH_VERSION
        UCHAR          PathId;                 // Port number from Windows (non-RAID) or CSMI port number
                                               // (GET_RAID_INFO and GET_RAID_CONFIG)
        UCHAR                        TargetId; // Reserved for now
        UCHAR                        Lun;      // Reserved for now
        NVME_PASS_THROUGH_PARAMETERS Parameters;
        UCHAR                        data[1]; // So that is is easy to select the data buffer offset when
                                              // building the command.
    } NVME_IOCTL_PASS_THROUGH;

    // AER API Support starting RST versino 17.5

    /////////////////////////////////////End of NVME Passthrough
    /// API//////////////////////////////////////////

    // Firmware Download API (Starting RST version 14.8)
    // For NVMe and SATA drives. SATA Drives must meet Win10 requirements (deferred
    // download support) Lots redefined from WinAPI to make sure this is more
    // portable - TJE Process should follow this, but using the below Intel & RAID
    // FW update functions&structs:
...
// Redefined from FIRMWARE_REQUEST_BLOCK so that up to date Windows API is not
// necessary for compilation
#    define INTEL_FIRMWARE_REQUEST_BLOCK_STRUCTURE_VERSION 0x1

// firmware functions (matches windows API, but added separate name in case this
// is used in a Windows API that doesn't define these
...
//
// The request is for Controller if this flag is set. Otherwise, it's for
// Device/Unit.
//
...
// This flag is only valid for fimrware_activate request. It's ignored for other
// requests.
//
#    define INTEL_FIRMWARE_REQUEST_FLAG_SWITCH_TO_EXISTING_FIRMWARE 0x80000000

    typedef struct _INTEL_FIRMWARE_REQUEST_BLOCK
    {
        ...
        ULONG DataBufferOffset; // the offset is from the beginning of buffer. e.g.
                                // from beginning of SRB_IO_CONTROL. The value should
                                // be multiple of sizeof(PVOID); Value 0 means that
    ...
    typedef struct _RAID_FIRMWARE_REQUEST_BLOCK
    {
        UCHAR                        Version;
        UCHAR                        PathId;
        UCHAR                        TargetId;
...
#    endif //__clang__
    ...
    // The RAID FW ioctl can send only STORAGE_FIRMWARE_INFO_V2,
    // STORAGE_FIRMWARE_DOWNLOAD_V2, STORAGE_FIRMWARE_ACTIVATE These are redefined
    // here so that an up to date Windows API is not required to compile this code.
...
#    define INTEL_STORAGE_FIRMWARE_SLOT_INFO_V2_REVISION_LENGTH 16
    ...
    typedef struct _INTEL_STORAGE_FIRMWARE_SLOT_INFO_V2
    {
...
#    endif //__clang__
    typedef struct _INTEL_STORAGE_FIRMWARE_INFO_V2
    {
        ...
        BOOLEAN FirmwareShared; // The firmware applies to both device and adapter.
                                // For example: PCIe SSD.
        UCHAR Reserved[3];

        ULONG ImagePayloadAlignment; // Number of bytes. Max: PAGE_SIZE. The transfer
                                     // size should be multiple of this unit size.
                                     // Some protocol requires at least sector size. 0
                                     // means the value is not valid.
...
#    define INTEL_STORAGE_FIRMWARE_DOWNLOAD_STRUCTURE_VERSION_V2 0x2
    ...
    typedef struct _INTEL_STORAGE_FIRMWARE_DOWNLOAD_V2
    {

        ULONG Version; // INTEL_STORAGE_FIRMWARE_DOWNLOAD_STRUCTURE_VERSION_V2
        ULONG Size;    // sizeof(INTEL_STORAGE_FIRMWARE_DOWNLOAD_V2)

        ULONGLONG Offset;     // image file offset, should be aligned to value of
                              // "ImagePayloadAlignment" from STORAGE_FIRMWARE_INFO.
        ULONGLONG BufferSize; // should be multiple of value of
                              // "ImagePayloadAlignment" from STORAGE_FIRMWARE_INFO

        UCHAR Slot; // Must be set to INVALID SLOT definition above according to Intel
                    // documentation
        UCHAR Reserved[3];
        ULONG ImageSize;
...
#    define INTEL_STORAGE_FIRMWARE_ACTIVATE_STRUCTURE_VERSION 0x1
    ...
    typedef struct _INTEL_STORAGE_FIRMWARE_ACTIVATE
    {
...
#    define INTEL_FIRMWARE_STATUS_ILLEGAL_LENGTH           0x87
    ...
    // AER API is for setting/receiving asynchronous notifications on NVMe devices.
    // Available starting in RST 17.5 RAID configured devices only. Will not work
    // with devices setup as separate non-RAID passthrough devices according to
    // Intel doc. Use the event mask to set and clear notifications based on bits
    // being set to 1 or 0. This IOCTL should go to a SCSI Port, not a specific
    // device. Use CSMI CC_CSMI_SAS_GET_DRIVER_INFO to check if a port is controlled
    // by the Intel RST driver before using this IOCTL Use the NVMe passthrough
    // IOCTL to receive Admin Identify data to check if the device on the port is an
    // NVMe device, and then this AER IOCTL can also be used. Event names should be
    // unique (Use something like a GUID). This is used to help the applicatio track
    // events it requests and separate them from other events Keep the event handle
    // after creating the event handle with CreateEvent(). This can be used for
    // notifications when an even completes on a given SCSI port, which can then be
...
#    define NVME_GET_AEN_DATA_MAX_COMPLETIONS                                                                          \
        10 // Up to 10 events can be stored, and must call GET DATA method multiple
           // times to read each one.

// Defined events (Only 2 so far according to Intel documentation). These can be
// OR'd together since the event mask is a bit field
...
#    define AEN_EVENT_HEALTH_LOG_WARNING  2
    ...
    typedef struct _NVME_IOCTL_REGISTER_AER
    {
        SRB_IO_CONTROL                 Header;
        RAIDPORT_REGISTER_SHARED_EVENT EventData;
    } NVME_IOCTL_REGISTER_AER;

    typedef struct _ADMIN_ASYNCHRONOUS_EVENT_COMPLETION_DW0
    {
        union
        {
            struct
    ...
    typedef struct _NVME_AER_DATA
    {
        UCHAR EventName[AEN_MAX_EVENT_NAME_LENGTH];
        UCHAR Reserved;
        ADMIN_ASYNCHRONOUS_EVENT_COMPLETION_DW0
        Completions[NVME_GET_AEN_DATA_MAX_COMPLETIONS];
        ULONG CompletionsCount;
    } NVME_AER_DATA;

    typedef struct _NVME_IOCTL_GET_AER_DATA
    {
        SRB_IO_CONTROL Header;
        NVME_AER_DATA  Data;
    ...
    // This should be used alongside CSMI CC_CSMI_SAS_GET_DRIVER_INFO,
    // CC_CSMI_SAS_GET_RAID_INFO, CC_CSMI_SAS_GET_RAID_CONFIG
...
#    pragma pack(push, remapport_ioctl, 1)
    typedef struct _REMAPPORT_LOCATION_PCI_BDF
    {
        UCHAR Bus;
        UCHAR Device : 5;
        UCHAR Function : 3;
    ...
    typedef struct _REMAPPORT_LOCATION_SATA
    {
        UCHAR PortNumber;
    } REMAPPORT_LOCATION_SATA;

    typedef struct _REMAPPORT_LOCATION_CR
    {
        UCHAR CycleRouterNumber;
    } REMAPPORT_LOCATION_CR;

    typedef struct _REMAPPORT_LOCATION_SW_REMAP
    {
        REMAPPORT_LOCATION_PCI_BDF PciAddress;
    } REMAPPORT_LOCATION_SW_REMAP;

    typedef struct _REMAPPORT_LOCATION_VMD
    {
        USHORT SocketNumber;
...
#    define REMAPPORT_IOCTL_GET_DEVICE_LOCATION_VERSION 1
    ...
    typedef struct _REMAPPORT_IOCTL_GET_DEVICE_LOCATION
    {
        SRB_IO_CONTROL          Header;
        UCHAR                   Version;
        UCHAR                   Size;
...
////////////////////////////////////////////////////End of Remap Port IOCTL
/// stuff (device
/// location)///////////////////////////////////////////////////////////

// SRB Status matches Windows, redefined here to reduce includes, like the FWDL
// structures and flags were redefined (see srb.h)
...
// TODO: Are these the SRB status's for firmware, similar to the miniport in
// Windows to use when doing firmware updates instead???-TJE
#    define INTEL_FIRMWARE_STATUS_SUCCESS                  0x0
#    define INTEL_FIRMWARE_STATUS_ERROR                    0x1
#    define INTEL_FIRMWARE_STATUS_ILLEGAL_REQUEST          0x2
#    define INTEL_FIRMWARE_STATUS_INVALID_PARAMETER        0x3
