...
// \brief Defines the structures, ioctls, etc for Intel's RST driver specific IOCTLs for additional support beyond CSMI spec
//        This is only for Windows. All things in this file is defined by Intel documentation
...
extern "C"
{
    ...
    //Process should follow this, but using the below Intel & RAID FW update functions&structs: https://docs.microsoft.com/en-us/windows-hardware/drivers/storage/upgrading-firmware-for-an-nvme-device?redirectedfrom=MSDN

#define INTEL_RAID_FW_SIGNATURE "IntelFw "

#define IOCTL_RAID_FIRMWARE \
CTL_CODE(0xF000, 0x010, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#define INTEL_FIRMWARE_REQUEST_BLOCK_STRUCTURE_VERSION            0x1
    ...
    //firmware functions (matches windows API, but added separate name in case this is used in a Windows API that doesn't define these
...
#define INTEL_FIRMWARE_STATUS_ILLEGAL_LENGTH                      0x87
    ...
    //RAID configured devices only. Will not work with devices setup as separate non-RAID passthrough devices according to Intel doc.
    //Use the event mask to set and clear notifications based on bits being set to 1 or 0.
    //This IOCTL should go to a SCSI Port, not a specific device. Use CSMI CC_CSMI_SAS_GET_DRIVER_INFO to check if a port is controlled by the Intel RST driver before using this IOCTL
    //Use the NVMe passthrough IOCTL to receive Admin Identify data to check if the device on the port is an NVMe device, and then this AER IOCTL can also be used.
    //Event names should be unique (Use something like a GUID). This is used to help the applicatio track events it requests and separate them from other events
    //Keep the event handle after creating the event handle with CreateEvent(). This can be used for notifications when an even completes on a given SCSI port, which can then be read for the data when the event occurs.

#define INTELNVM_AER_SIGNATURE "IntelNvm "
#define INTEL_NVME_REGISTER_AER \
CTL_CODE(0xF000, 0xC00, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define INTEL_NVME_GET_AER_DATA \
CTL_CODE(0xF000, 0xE00, METHOD_BUFFERED, FILE_ANY_ACCESS)

#define AEN_MAX_EVENT_NAME_LENGTH 32
#define NVME_GET_AEN_DATA_MAX_COMPLETIONS 10 //Up to 10 events can be stored, and must call GET DATA method multiple times to read each one.

    //Defined events (Only 2 so far according to Intel documentation). These can be OR'd together since the event mask is a bit field
...
#define AEN_EVENT_HEALTH_LOG_WARNING 2
    ...
    //This should be used alongside CSMI CC_CSMI_SAS_GET_DRIVER_INFO, CC_CSMI_SAS_GET_RAID_INFO, CC_CSMI_SAS_GET_RAID_CONFIG

#define INTEL_RMP_SIGNATURE "IntelRmp"
#define INTEL_REMAPPORT_GET_DEVICE_LOCATION 0x80000D05 //CTL_CODE(0x8000, 0x341, METHOD_IN_DIRECT, FILE_ANY_ACCESS);
