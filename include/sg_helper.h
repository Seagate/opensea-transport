...
extern "C"
{
    ...
    // \file sg_helper.h
    // \brief Defines the constants structures and function headers to help parse
    // scsi drives.
...
// \todo Figure out which scsi.h & sg.h should we be including kernel specific
// or in /usr/..../include
#include "nvme_helper.h"

#define SG_PHYSICAL_DRIVE "/dev/sg" // followed by a number
#define SD_PHYSICAL_DRIVE "/dev/sd" // followed by a letter
#define BSG_PHYSICAL_DRIVE                                                                                             \
    "/dev/bsg/" // remaining part of the handle is
                // h:c:t:l
#define OPENSEA_PATH_MAX PATH_MAX

// This is the maximum timeout a command can use in SG passthrough with
// linux...1193 hours NOTE: SG also supports an infinite timeout, but that is
...
#define SG_MAX_CMD_TIMEOUT_SECONDS 4294967
    ...
    // SG Driver status's since they are not available through standard includes
    // we're using
...
#    define OPENSEA_SG_ERR_DID_SOFT_ERROR 0x000B
    ...
    // eReturnValues nvme_Namespace_Rescan(int fd);//rescans a controller for
    // namespaces. This must be a file descriptor without a namespace. EX:
    // /dev/nvme0 and NOT /dev/nvme0n1
    ...
    //
    //! \brief   Description:  removes the O_NONBLOCK flag from the handle to get
    ...
    //
    //! \brief   Description:  adds the O_NONBLOCK flag to the handle to restore
