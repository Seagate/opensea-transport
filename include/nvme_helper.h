...
//
// \file nvme_helper.h
// \brief Defines the constants structures to help with NVM Express
// Specification
...
//! \note While a simple + 1 is easy to do, this helps make it readable and
//! clear what the value being accessed is.
#define NVME_0_BASED(value) ((value) + 1)

//! \def NVME_0_BASED_ADJUST
//! \brief adjust a value from 1's based to 0 based.
//! \details This is most often used when taking something written in code to
//! convert to a value the drive is expecting. \note While a simple - 1 is easy
//! to do, this helps make it readable and clear what the value being converted
//! is.
#define NVME_0_BASED_ADJUST(value) ((value) - 1)
    ...
    typedef enum _eNvmeCmdType
    {
        NVM_ADMIN_CMD,
        NVM_CMD,
        NVM_UNKNOWN_CMD_SET // NVMe allows for additional command sets to be defined,
                            // but only 2 are currently, so make sure we have an
                            // "unknown" option
    } eNvmeCmdType;
...
#    pragma pack(pop)
    ...
    */
    ...
    typedef struct _nvmeFormatCmdOpts
    {
        ...
        uint8_t  pil;  // Protection Information Location. 1=PI xfered as first 8 bytes
                       // of metadata
        uint8_t ses;   // Secure Erase Settings. 0=none requested, 1=requested, 2=use
                       // Crtypto Erase
    } nvmeFormatCmdOpts;
...
#    pragma pack(push, 1)
    ...
    typedef struct _nvmeAdminCommand
    {
        ...
        uint64_t addr;     // PRP Entry 1
        uint32_t metadataLen;
        uint32_t empty; // Was set to datalen, but that is redundant where this is
                        // otherwise used. Need this here to keep this the correct
                        // size for use elsewhere.
...
#    pragma pack(pop)
    ...
    typedef struct _nvmeCommands
    {
        union
        {
            ...
            nvmCommandDWORDS dwords;   // So that we can see the whole thing as separate
                                       // DWORDs...mostly used for unknown case to print
                                       // out. This is the 64 bytes of command structure.
        };
    } nvmeCommands;

    typedef struct _completionQueueEntry
    {
        bool dw0Valid; // OS doing passthrough may or may not get this field back...so
                       // this is set to indicate it was retrieved by the OS
        bool dw1Valid;
        bool dw2Valid;
        bool dw3Valid; // AKA status and CID. Don't expect a valid CID though! Not
                       // every OS will give us that.
        union
        {
        ...
        union
        {
            uint32_t sqIDandHeadPtr; // This likely won't contain anything valid even if
                                     // the OS passthrough gave us this DWORD
            uint32_t dw2;
        };
        union
        {
    ...
    {
        tDevice*     device;
        eNvmeCmdType commandType;                // admin vs nvm command (needed for some OSs to send
                                                 // to the correct queue)
        eDataTransferDirection commandDirection; // this should match the NVMe definition in opcode bits
                                                 // 1:0. 00 - no data, 01 - host to controller (out), 10
                                                 // - controller to host (in), 11 - bidirectional
        nvmeCommands cmd;                        // cmd definition. This will be accessed depending on what
                                                 // is set to comandType field
