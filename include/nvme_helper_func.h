...
extern "C"
{
    ...
    eReturnValues check_NVMe_Status(uint32_t nvmeStatusDWord); // converts NVMe status to a return
                                                               // status used by open-sea libs

    // These reset functions will be defined in the os_Helper file since this is OS
    // specific. Not all OS's will support this function either.
    ...
    //  nvme_Get_Features
    //
    //! \brief   Description:  Function to send Get Features NVMe command to a
    //! device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    ...
    //  nvme_Get_Log_Page
    //
    //! \brief   Description:  Function to send Get Log Page NVMe command to a
    //! device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    ...
    //
    //! \brief   Description:  Function to send Get SMART/Health Information Log
    ...
    //!   \param[in] dataLen = Data buffer Length (multiple of 64 bytes for each
    //!   entry)
    //!                        [NVMe Identify data shows how many entries are
    //!                        present]
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //
    //! \brief   Description:  Function to send Get Firmware Slots Log Page NVMe
    //! command to a device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    ...
    //
    //! \brief   Description:  Function to send Get Commands Supported and Effects
    //! Log Page NVMe command to a device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    //!   \param[out] pData = Data buffer
    ...
    //
    //! \brief   Description:  Function to send Get Device Self-test Log Page NVMe
    //! command to a device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    ...
    //!   identify data \param[in] nvmeNamespace = \param[in] cns = 0~NS Data
    //!   Structre 1~Controller Data Structure 2~list of up to 1024 NSIDs,
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //! \brief   Description:  Function to send a NVMe Firmware Image Download
    //! command to a device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    ...
    //!   \param[in] ptrData = pointer to the data buffer to be filled in with
    //!   identify data \param[in] firstSegment = Flag to help some low-level OSs
    ...
    //
    //! \brief   Description:  Function to send a NVMe Firmware Commit (Activate)
    //! command to a device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    ...
    //!   \param[in] timeoutSeconds = timeout for the activate command. Selectable
    //!   since some commands take more time than others
    //!
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //  nvme_Set_Features
    //
    //! \brief   Description:  Function to send Set Features NVMe command to a
    //! device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
    ...
    //  nvme_Read_Ctrl_Reg
    //
    //! \brief   Description:  Function to read NVMe controller registers from the
    //! device
    //
    //  Entry:
    //!   \param[in] device = pointer to tDevice structure
...
#define FORMAT_NVME_XFER_METADATA   (64)
    ...
    // Do not use the asynchronous event request at this time. More work is required
    // at low levels to properly support this. This definition is here for
    ...
    nvme_Verify(tDevice* device, uint64_t startingLBA, bool limitedRetry, bool fua, uint16_t numberOfLogicalBlocks);
