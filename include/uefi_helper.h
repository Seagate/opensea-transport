...
// This is the maximum timeout a command can use in UEFI...which is nearly
// infinite to begin with NOTE: UEFI also supports an infinite timeout, but that
// is checked in a separate function
#define UEFI_MAX_CMD_TIMEOUT_SECONDS                                                                                   \
    UINT32_MAX // Technically, max seconds is 18446744074, but I don't want to
               // switch to a 64bit for the timeout. Anything with this value will
               // round up to infinite in UEFI...where a timeout this long may as
               // well be infinite
