...
//All code in this file is from an OLD Prolific USB product specification for pass-through commands.
...
extern 'C'
{
    ...
    //
    //! \brief   Description:  Function to construct a Prolific Legacy USB Pass-through CDBs based on the ATA Command Options
    //
    //  Entry:
    ...
    //!   \param[out] highCDBValid = output parameter to let you know when the data in the hiCDB is valid and should be send to the device. Only issue the HighCDB to the device when this bool returns true.
    ...
    //
    //! \brief   Description:  Function to send a Prolific Legacy Pass-through command. This will automatically call the function to build the command, then send it to the drive.
    //
    //  Entry:
    //!   \param[in] device = pointer to the device structure for the device to issue the command to.
    //!   \param[in] ataCommandOptions = ATA command options
