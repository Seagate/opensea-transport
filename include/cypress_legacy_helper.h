...
// All code in this file is from an OLD Cypress USB product specification for
// pass-through commands. This code should only be used on products that are
// known to use this pass-through interface. Some of this code may also be from
// something found in legacy source. This will be commented if it is.
...
#define CYPRESS_IDENTIFY_DATA_BIT                                                                                      \
    BIT7                              // set when the command is an identify device or identify packet device,
...
#define CYPRESS_DEVICE_OVERRIDE_BIT                                                                                    \
    BIT5                                       // using this forces the controller to use the ATACB field 0x0B bit 4
                                               // instead of the assignedLUN (don't recommend setting this - TJE)
...
#define CYPRESS_POLL_ALTERNATE_STATUS_OVERRIDE_BIT                                                                     \
    BIT2 // set in order to execute the command without polling the AltStat
         // register for a value of 0 (not setting will wait until the busy bit is
         // no longer set)
...
#define CYPRESS_TASK_FILE_READ_BIT                                                                                     \
    BIT0 // Use this bit to request the RTFRs from the device (8 bytes of data
         // will be returned)
    ...
    //
    //! \brief   Description:  Function to construct a Cypress Legacy USB
    //! Pass-through CDB based on the ATA Command Options
    //
    //  Entry:
    ...
    //
    //! \brief   Description:  This will build and send the command to get the
    //! RTFR results of the last pass-through command. The RTFRs in the
    //! ataCommandOptions will be filled in when this is successful.
    //
    //  Entry:
    ...
    //!   \param[in] commandRet = return value from sending the ATA Pass-through
    ...
    //
    //! \brief   Description:  Function to send a Cypress Legacy Pass-through
    //! command. This will automatically call the function to build the command,
    //! then send it to the drive.
    //
    //  Entry:
    //!   \param[in] device = pointer to the device structure for the device to
    //!   issue the command to. \param[in] ataCommandOptions = ATA command options
