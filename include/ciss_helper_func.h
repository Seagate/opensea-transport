...
#    if defined(__unix__) // this is only done in case someone sets weird defines
                          // for Windows even though this isn't supported
    // These filter functions help with scandir on /dev to find ciss compatible
    // devices. NOTE: On Linux, new devices are given /dev/sg, and those need to be
    // tested for support in addition to these filters. NOTE: smartpqi filter is
    // only available on freeBSD. It will return 0 on all other OS's.
