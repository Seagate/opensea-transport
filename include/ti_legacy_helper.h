...
//All code in this file is from an OLD TI USB product specification for pass-through commands.
...
extern 'C'
{
    ...
    #define TI_LEGACY_OPCODE_OLD 0xF0 //There is no VID/PID combination that references this type. By default this code is not using this value since we don't have a drive that responds to this.
    ...
    //!   \param[in] forceMode = set to true to force a PIO or UDMA mode (1-4). By default, we tell the USB bridge to use the highest available mode the bridge and drive support.
    //!   \param[in] modeValue = set to 1 - 4 for the mode value. This is only used when forceMode is set to TRUE.
    //!
    //  Exit:
    ...
    //
    //! \brief   Description:  Function to send a TI Legacy Pass-through command. This will automatically call the function to build the command, then send it to the drive.
    //
    //  Entry:
