...
// are known to use this pass-through interface. NOTE: There are 2 possible
// pass-through types for ASMedia. 1. Limited/basic capability to get identify
// and log page data (most common) and 2. a full pass-through capability (not
// supported on every device)
//       This file contains both. It is not clear how to determine if the full
//       packet passthrough is supported other than trying it.
...
// Documentation shows that the inquiry data sets an additional product
// identifier in bytes 36 & 37. These defines are below. These values will only
// show when reading EXACTLY 38 bytes of std inquiry data.
...
#define ASMEDIA_NVME_PACKET_CDB_SIZE           UINT8_C(16)
    ...
    typedef enum _eASM_NVMPacket_Operation
    {
        ASMEDIA_NVMP_OP_POWER_DOWN_NVME = 0x00, // ND (w) - param 1 used to control shutdown process. Only useful if
                                                // power control circuit is present
        ASMEDIA_NVMP_OP_RESET_BRIDGE    = 0x01, // ND (w) - no params
        ASMEDIA_NVMP_OP_GET_BRIDGE_INFO = 0x02, // R - no params, returns 64B of data. USB SN, VID, PID, PCIe
                                                // detection, PCIe speed, PCIe lane width, power control of PCIe
        ...
        // Below operations are for NVM packet passthrough. Max transfer is 128KB in
        // provided documentation, but may have previously been different based on
        // revision history.
...
#define ASM_NVMP_RELINK_NORMAL_SHUTDOWN UINT8_C(1)
    ...
    // Other notes: metadata is reserved, data pointer and CID will be modified by
    // the bridge. Must be 512B aligned transfers according to documentation. If
    // command completion data bytes 14 & 15 are FFh, then entry is invalid and no
    // NVMe command was sent or executed. Data phase/direction parameter should
    // match for all 3 steps of packet passthrough

#define ASM_NVMP_DWORDS_DATA_PACKET_SIZE UINT8_C(64) // Size of packet for NVMe DWORDS
#define ASM_NVMP_RESPONSE_DATA_SIZE                                                                                    \
    UINT8_C(16) // size of NVM completion
                // results
