...
//
// Indicate that the existing firmware in slot should be activated immediately
// without controller reset. Only valid for IOCTL_STORAGE_FIRMWARE_ACTIVATE.
//
...
//
// Indicate that any existing firmware in slot should be replaced with the
// downloaded image, and activated with controller reset. Only valid for
...
//
// Indicate that any existing firmware in slot should be replaced with the
// downloaded image. Only valid for IOCTL_STORAGE_FIRMWARE_ACTIVATE.
//
...
//
// Indicate that the existing firmware in slot should be activated with a
...
#    define STORAGE_HW_FIRMWARE_REQUEST_FLAG_SWITCH_TO_EXISTING_FIRMWARE 0x80000000
    ...
    // NOTE: This versions checks win10/11 to determine when the requested mode is
    // supporetd. If not supported, it returns OS_COMMAND_NOT_AVAILABLE
    eReturnValues set_NVMe_Firmware_Activate_Flags(nvmeCmdCtx* nvmeIoCtx, uint32_t* currentFlags);

#define WIN_SCSI_SRB                                                                                                   \
    "\\\\.\\SCSI" // can be used to issue mini port ioctls. Not really supported
                  // right now...
#define WIN_PHYSICAL_DRIVE "\\\\.\\PhysicalDrive"
#define WIN_TAPE_DRIVE     "\\\\.\\Tape"
#define WIN_CDROM_DRIVE                                                                                                \
    "\\\\.\\CDROM"                                  // Most likely an ATAPI device, but it could be a really old
...
#define DOUBLE_BUFFERED_MAX_TRANSFER_SIZE                                                                              \
    16384 // Bytes....16KiB to be exact since that is what MS documentation says.
          // - TJE
...
// didn't exist when I went looking for it - TJE NOTE: ARM requires 10.0.16299.0
// API to get this library!
#if !defined(__MINGW32__) && !defined(__MINGW64__)
#    pragma comment(lib, "Cfgmgr32.lib") // make sure this get's linked in
#endif
    // \fn send_IO(scsiIoCtx * scsiIoCtx)
    // \brief Function to send a ioctl after converting it from the ScsiIoCtx to OS
    // tSPTIoContext \param ScsiIoCtx \return SUCCESS - pass, !SUCCESS fail or
    ...
    //
    //! \brief   Description:  Attempts a device reset through OS functions
    //! available. NOTE: This won't work on every device
    //
    //  Entry:
    ...
    //!   or driver of the device, OS_COMMAND_BLOCKED = failed to perform the reset
    //
    //-----------------------------------------------------------------------------
    ...
    //
    //! \brief   Description:  Attempts a bus reset through OS functions available.
    //! NOTE: This won't work on every device
    //
    //  Entry:
    //!   \param[in]  device = pointer to device context!
    //!
    ...
    //
    //! \brief   Description:  Attempts a controller reset through OS functions
    //! available. NOTE: This won't work on every device
    //
    //  Entry:
    //!   \param[in]  device = pointer to device context!
    ...
    //
    //! \brief   Description:  Issues the FSCTL_LOCK_VOLUME ioctl on the open handle
    //! to prevent any interuptions during a command or sequence of commands.
    //!                        It is strongly recommended that the unlock is called
    //!                        after this is done to return the device to a
    //!                        "sharing" mode again.
    //
    //  Entry:
    //!   \param[in]  device = pointer to device context!
    ...
    //
    //! \brief   Description:  Issues the FSCTL_UNLOCK_VOLUME ioctl on the open
    //! handle to restore shared functionality on the device.
    //
    //  Entry:
    //!   \param[in]  device = pointer to device context!
    //!
    ...
    //  os_Unlock_Device(tDevice *device)
    //
    //! \brief   Description:  Issues IOCTL_DISK_UPDATE_PROPERTIES to force an
    //! update of the known filesystem...or attempts to.
    //
    //  Entry:
    //!   \param[in]  device = pointer to device context!
    //!
    //  Exit:
    //!   \return SUCCESS = pass, NOT_SUPPORTED = IOCTL not available, or did not
    //!   work. - TJE
    //
    //-----------------------------------------------------------------------------
