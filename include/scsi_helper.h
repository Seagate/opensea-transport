...
#define INQ_RESPONSE_FMT_CCS                                                                                           \
    (1)                                 // SCSI common command set definition. More or less meets modern
                                        // requirements
...
#define INQ_MAX_VERSION_DESCRIPTORS (8)
    ...
    typedef enum _eSenseKeyValues
    {
        ...
        SENSE_KEY_EQUAL           = 0x0C, // scsi and scsi 2 defined this as "EQUAL" to say a
                                          // "search data" command found an equal comparison.
    ...
    typedef enum _eSenseDescriptorType
    {
...
#define SCSI_MAX_64_LBA                  UINT64_MAX
    ...
    {
        bool senseKeySpecificValid; // Will be set when the sense data contains sense
...
#define MAX_FORWARDED_SENSE_DATA_DESCRIPTORS UINT8_C(2)
    ...
    {
        bool validStructure;          // Set to true if the rest of this structure was able to
                                      // be parsed/filled in. This will only be false if we do
                                      // not get 70h - 73h response codes
        bool fixedFormat;             // This will tell you if some fields, like information and
                                      // command-specific information, are limited to 32bits or not
        bool       deferredError;     // Set to true for response codes 71h & 73h
        scsiStatus scsiStatusCodes;   // sense key, asc, ascq, fru
        bool       senseDataOverflow; // gets set if the sense buffer is not big enough to
                                      // return all necessary fields of the sense data.
                                      // Request sense command is needed to get all the data.
        bool valid;                   // valid bit. Used to know when the information field contains
                                      // valid information
        bool filemark;                // filemark bit is set (stream commands)
        bool endOfMedia;              // end of media bit is set (stream commands)
        bool illegalLengthIndication; // illegal length indicator bit is set (stream
                                      // commands or read/write long SBC commands)
        union
        {
        ...
        // bools below can be used to know if other fields that are only available in
        // some cases/commands are found. If so, the caller can interpret these
        // themselves. We'll supply the offset in the sense data for them. The offset
        // myst be > 7 to be valid.
        ...
        {
            bool valid; // must be set for this data to be valid. Means we found this in
                        // the sense data.
            bool    extend;
            uint8_t error;
        ...
        // This will be set to true for any descriptors that could not be parsed
        // (vendor unique or not part of the above output) or if the additional sense
        // bytes field of fixed format is non-zero If this happens, the caller should
        // check the sense data buffer themselves for the additional data that they
        // could find useful
        bool    additionalDataAvailable;
        uint8_t additionalDataOffset; // if bool above is set, then this will be set
                                      // to the offset of the additional data that
                                      // couldn't be parsed
    ...
    // \param direction is it XFER_DATA_IN (from the drive) XFER_DATA_OUT (to the
    // device) \param pdata pointer to the user data to be read/written \param
    // dataLength length of the data to be read/written \param psense \param
    // senseDataSize \param timeout \param verbose will print some more debug info.
    ...
    typedef struct _ScsiIoCtx
    {
        ...
        uint32_t               dataLength;
        uint8_t*               psense;
        uint32_t               senseDataSize; // should be reduced to uint8 in the future as sense
                                              // data maxes at 252Bytes
...
#define SCSI_CTDP_BIT_SET               (0x02)
    ...
    typedef enum _eScsiSanitizeOverwriteTest
    {
    ...
    // some of these commands have something like _CMD in the name or a missing
    // underscore in order to avoid conflict with a system header somewhere in linux
    // or windows. - TJE
    typedef enum _ScsiCmds
    {
        ...
        REASSIGN_BLOCKS_6            = 0x07, // I added the _6 to the name because it conflicts
                                             // with a windows structure definition - TJE
    ...
    typedef enum _eScsiVpdPages
    {
        SUPPORTED_VPD_PAGES = 0x00,
        STANDARD_INQUIRY    = 0x00,
        // 01h - 7Fh - ASCII Information
        UNIT_SERIAL_NUMBER                = 0x80,
        IMPLEMENTED_OPERATING_DEFINITIONS = 0x81,            // Obsolete. Used to tell what can be sent with the change
                                                             // definition command (scsi 2 vs scsi 3, etc)
    ...
    // these enums are only for VPD pages with fixed lengths..add onto this as we
    // need more things in here
    typedef enum _eScsiVPDPageLengths
    {
        ...
        VPD_LOGICAL_BLOCK_PROVISIONING_LEN   = 8,  // This is only a correct length if there are no provisioning group
                                                   // descriptors
    ...
    typedef enum _eScsiLogPageControl
    {
    ...
    typedef enum _eScsiModeParameters // does not do subpage codes...only page
                                      // codes. Add more as needed
    {
...
#define MP_SP_ALL_SUBPAGES 0xFF
    ...
    // put defined lengths in here....not all mode pages/parameters have defined
    // lengths so only the ones that do are in here This is TOTAL length, not
    // necessarily the length field in the page
    ...
    typedef enum _eScsiLogPages // these are all the pages. Some may need a specific
                                // subpage to be used to access them
    {
    ...
    typedef enum _eScsiLogPageLengths // only lengths that are specified are in
                                      // here. Some pages may not have a set length
    {
        LP_SELF_TEST_RESULTS_LEN = 404,
        LP_TEMPERATURE_LEN       = 16,      // this is only set to this size because there are
                                            // currently only 2 temperature parameters defined in
                                            // the spec and this size takes that into account
        LP_SOLID_STATE_MEDIA_LEN = 12,      // this is only set to this size since only 1 parameter code is
                                            // currently defined in the SBC spec
        LP_INFORMATION_EXCEPTIONS_LEN = 12, // setting to 12 since a SAT device will only likely return 12 bytes
                                            // of data....this shouldn't be used when reading this page from a SAS
                                            // device.
    } eScsiLogPageLengths;
...
#define READ_CAPACITY_16_LEN           UINT8_C(32)
    ...
    typedef enum _eSCSIAddressDescriptors
    {
...
#define SCSI_VPD_PAGE_HEADER_LENGTH      UINT8_C(4) // bytes
    ...
    // The values in here may be incomplete. They are from the SPC5 spec, Table E.19
    // - Standard Code value guidelines
...
#define SAT_SECURITY_INFO_LEN                                                                                          \
    UINT8_C(16) // security protocol in to receive ATA security information is
                // 16bytes in length from SAT
#define SAT_SECURITY_PASS_LEN                                                                                          \
    UINT8_C(36) // all security protocol out commands that send the password must
                // be 36 bytes in length according to SAT
    // SECURITY_PROTOCOL_ATA_DEVICE_SERVER_PASSWORD = 0xEF (defined in cmds.h for
    // common use)
    typedef enum _eSATSecurityDevicePassword
    {
        SAT_SECURITY_PROTOCOL_SPECIFIC_READ_INFO = 0x0000, // should only be used to read security protocol information
                                                           // (ATA security info)
...
#define REPORT_LUNS_MIN_LENGTH                                                                                         \
    UINT16_C(16) // this is the minimum length from SPC, but this requirement was
                 // removed later -TJE
