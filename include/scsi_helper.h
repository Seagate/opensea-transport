...
// \param ascq
	...
	typedef struct _senseDataFields
	{
		...
		uint8_t microcodeActivationDescriptorOffset;
...
//to avoid conflict with a system header somewhere in linux or windows. - TJE
    typedef enum _ScsiCmds
    {
        ...
        REASSIGN_BLOCKS_6                           = 0x07, //I added the _6 to the name because it conflicts with a windows structure definition - TJE
    ...
    typedef enum _eScsiLogPages //these are all the pages. Some may need a specific subpage to be used to access them
    {
    ...
    typedef enum _eScsiLogPageLengths //only lengths that are specified are in here. Some pages may not have a set length
    {
        LP_SELF_TEST_RESULTS_LEN = 404,
        LP_TEMPERATURE_LEN = 16,//this is only set to this size because there are currently only 2 temperature parameters defined in the spec and this size takes that into account
        LP_SOLID_STATE_MEDIA_LEN = 12,//this is only set to this size since only 1 parameter code is currently defined in the SBC spec
