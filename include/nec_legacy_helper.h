...
//All code in this file is from an OLD NEC USB product specification for pass-through commands.
...
extern 'C'
{
    ...
    {
        SECTOR_COUNT_FROM_19H_1AH_SECTOR_COUNT_REGISTERS = 0, //00b = Sector Count Register Sector Count (15:8), Byte 10, Sector Count Low = Sector Count Register Sector Count (7:0), Byte 11
        SECTOR_COUNT_LOW_FROM_1EH = 1, //01b - sector count high = 0, sector count low = Control (0), 15h
        SECTOR_COUNT_LOW_FROM_OFFSET_1AH_SECTOR_COUNT_HIGH_FROM_OFFSET_18H = 2, //10b - sector count high = sector number register LBA (7:0), Byte 9, sector count Low = Sector Count Register Sector Count (7:0), Byte 11
    ...
    //
    //! \brief   Description:  Function to send a NEC Legacy Pass-through command. This will automatically call the function to build the command, then send it to the drive.
    //
    //  Entry:
    //!   \param[in] device = pointer to the device structure for the device to issue the command to.
    //!   \param[in] ataCommandOptions = ATA command options
