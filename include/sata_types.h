...
//
// \file sata_types.h
// \brief Defines types that are specific to SATA interface. Mostly FIS
// structures that may be used by some passthrough interfaces. This file should
// only have things unique to SATA transport.
...
// Unknown or vendor unique FIS's may have various lengths, but we will print
// out at most 20 bytes of the FIS
#define UNKNOWN_FIS_TYPE_LENGTH UINT8_C(20)

// Bit masks for H2D Fis. use these instead of the bit fields for portability.
// Bitfields are more informational and useful in debugging environments that
// pack them in the order they are specified below (mostly checked in MSFT
// compilers)
#define H2D_COMMAND_BIT_MASK (0x80)

    typedef struct _sataH2DFis
    {
        // DWORD 0
        ...
        union
        {
            uint8_t byte1; // Bit field below for CRRR_PM_Port bits...this SHOULD be ok,
                           // but do not rely on the compiler to handle your bit field
                           // correctly! they can order them differently! - TJE
            struct
            {
...
#define PIO_SETUP_INTERUPT_BIT_MASK (0x40)
    ...
    typedef struct _sataPIOSetupFis
    {
        // DWORD 0
...
#define SET_DEVICE_BITS_NOTIFICATION_BIT_MASK (0x80)
    ...
    typedef struct _sataSetDeviceBitsFis
    {
        // DWORD 0
        uint8_t fisType; // A1h
        ...
        union
        {
            uint8_t status; // status register. There are some reserved bits in here,
                            // hence bitfields
            struct
            {
    ...
    typedef struct _sataDMAActivateFis
    {
        // DWORD 0
        uint8_t fisType; // 39h
        union
...
#define DMA_SETUP_AUTO_ACTIVATE_BIT_MASK (0x10)
    ...
    typedef struct _sataDMASetupFis
    {
        // DWORD 0
        uint8_t fisType; // 41h
        union
...
#define BIST_ACTIVATE_PATTERN_V_BITMASK (0x01)
    typedef struct _sataBISTActivateFis
    {
        // DWORD 0
        uint8_t fisType; // 58h
        union
    ...
    typedef struct _sataDataFis
    {
        ...
        // DWORD 1
        uint32_t nDWordsData[1]; // This is variable length. up to 2048 DWords, so this
                                 // struct should be variable size depending on the
                                 // transfer. Because of this, we represent it like this.
