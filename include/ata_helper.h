...
#define DEVICE_REG_BACKWARDS_COMPATIBLE_BITS          0xA0
    ...
    // Device/Head pre-ATA standardization called this the Sector Size, Device, Head
    // register bit7 was defined as the ECC bit. With this set to zero it used CRC
    // bits 6:5 were the sector size. 11b=128B, 00b=256B, 01b=512B, 10b=1024B.
    // With standardization forcing these bits to A0 sets ECC and 512B sector size,
    // which is all that was likely used in the real-world For backwards
    // compatibility with these really old devices, it is recommended to set this
    // register to these values. This is not necessary for SATA drives unless they
    // are aborting commands for no other reason. Setting these bits may not be
    // necessary for most PATA devices that conform to ATA standards
    ...
    // This one is a little more advanced as some words specify bit 15 is zero and
    // bit 14 is 1 as a key, so it checks for this in addition to checking non-zero
    // and non FFFFh example: Word 83, 84, 87, 93 (pata), 106, 119, 120, 209
    OPENSEA_TRANSPORT_API bool is_ATA_Identify_Word_Valid_With_Bits_14_And_15(uint16_t word);
    // checks same as is_ATA_Identify_Word_Valid and that bit 0 is cleared to zero
    // in the SATA words (76 - 79)
...
#define LEGACY_WRITE_SAME_INITIALIZE_ALL_SECTORS              0xDD
    ...
    typedef enum _eSMART_features
    {
        ...
        ATA_SMART_AUTO_OFFLINE     = 0xDB, // NOTE: never officially adopted by ATA spec, but was part of
                                           // original SMART spec by SFF committee (SFF-8035i).
        ATA_SMART_UNKNOWN
    } eSMART_features;
...
#define ATA_SMART_ATTRIBUTE_NOMINAL_COMMON_START                                                                       \
    0x64 // most attributes start at 100, but there are occasionally some that
         // don't
#define ATA_SMART_ATTRIBUTE_WORST_COMMON_START                                                                         \
    0xFD // It's fairly common for a worst-ever to start at this highest possible
         // value then move down as data is collected.
...
#define ATA_SMART_THRESHOLD_ALWAYS_FAILING                                                                             \
    0xFF // Test code purposes only to test system or software's ability to detect
         // SMART trips
...
#define ATA_SMART_AUTO_OFFLINE_DISABLE_SIG       0x00
    ...
    typedef enum _eAtaCmdType
    {
        ...
        ATA_CMD_TYPE_EXTENDED_TASKFILE, // 48bit command, minus AUX and ICC registers
                                        // (which usually cannot be passed through
                                        // anyways)
        ATA_CMD_TYPE_COMPLETE_TASKFILE, // Used for commands trying to set ICC or AUX
                                        // registers as this will create a 32B CDB.
                                        // This is LIKELY not supported as it has yet
                                        // to be seen "in the wild". Avoid trying
                                        // these commands since 90% of HBAs tested
                                        // won't allow FPDMA protocol to passthrough
                                        // anyways - TJE
    ...
    typedef enum _eAtaProtocol
    {
        ...
        ATA_PROTOCOL_DMA_QUE,    // Read/Write DMA Queued (tagged command queuing
                                 // non-SATA)
    ...
    typedef struct _ataPassthroughCommand
    {
        ...
        uint32_t               timeout;                  // timeout override for the lower layers. This is a time in
                                                         // seconds.
        eATAPassthroughTransferBlocks ataTransferBlocks; // this is in here for building the SAT command to set
                                                         // the T_Type bits properly depending on the command we
                                                         // are issuing.
        eATAPassthroughLength ataCommandLengthLocation;  // this is in here for building the SAT command
                                                         // to set the T_Length field properly. This is
                                                         // important for asynchronous commands
        uint8_t multipleCount;                           // This is the exponent value specifying the number of
                                                         // sectors used in a read/write multiple command transfer.
                                                         // All other commands should leave this at zero. This ONLY
                                                         // matters on read/write multiple commands, if this is
                                                         // nonzero on any other command, it will fail. Only bits
                                                         // 0:2 are valid (SAT limitation)
        bool forceCheckConditionBit;                     // Set this to force setting the check condition
                                                         // bit on a command. This is here because by
                                                         // default,only non-data gets this bit due to
                                                         // some weird chipsets. This is an override that
                                                         // can be used in certain commands.
        uint8_t forceCDBSize;                            // only set this if you want to force a specific SAT
                                                         // passthrough CDB size (12B, 16B, or 32B). Bad
                                                         // parameter may be returned if setting registers in a
                                                         // command that cannot be set in the specified SAT CDB
        bool fwdlFirstSegment;                           // firmware download unique flag to help low-level OSs
                                                         // (Windows)
        bool fwdlLastSegment;                            // firmware download unique flag to help low-level OSs
                                                         // (Windows)
...
#    pragma pack(push, 1)
    ...
    typedef struct _ataSMARTAttribute
    {
        ...
        uint16_t status; // bit 0 = prefail warranty bit, bit 1 = online collection,
                         // bit 2 = performance, bit 3 = error rate, bit 4 = even
                         // counter, bit 5 = self preserving
        uint8_t nominal;
        uint8_t worstEver;
        uint8_t rawData[SMART_ATTRIBUTE_RAW_DATA_BYTE_COUNT]; // attribute and vendor
                                                              // specific
#if !defined(__GNUC__) || defined(__MINGW32__) || defined(__MINGW64__)
    } ataSMARTAttribute;
#    pragma pack(pop)
...
SL  - SMART Logging
(a) - The device shall return command aborted if a GPL feature set (see 4.11)
command accesses a log that is marked only with SL. (b) - The device shall
return command aborted if a SMART feature set (see 4.19) command accesses a log
...
#    pragma pack(push, 1)
    ...
    {
        //  Log Address | Log Name                                  | Feature Set   |
        //  R/W   | Access
        uint16_t LogDir;                     //  00          | Log directory                             |
                                             //  none          | RO    | GPL,SL
        uint16_t SummarySMARTErrLog;         //  01          | Summary SMART Error Log | SMART
                                             //  | RO    | SL (a)
        uint16_t CompSMARTErrLog;            //  02          | Comprehensive SMART Error Log |
                                             //  SMART         | RO    | SL (a)
        uint16_t ExtCompSMARTErrLog;         //  03          | Ext. Comprehensive SMART Error
                                             //  Log        | SMART         | RO    | GPL (b)
        uint16_t DeviceStatistics;           //  04          | Device Statistics | none | RO |
                                             //  GPL, SL
        uint16_t ReservedCFA1;               //  05          | |               |       |
        uint16_t SMARTSelfTestLog;           //  06          | SMART Self-Test Log | SMART | RO
                                             //  | SL (a)
        uint16_t ExtSMARTSelfTestLog;        //  07          | Ext. SMART Self-Test Log |
                                             //  SMART         | RO    | GLB (b)
        uint16_t PowerConditions;            //  08          | Power Conditions | EPC           |
                                             //  RO    | GPL (b)
        uint16_t SelectiveSelfTestLog;       //  09          | Selective Self-Test Log |
                                             //  SMART         | R/W   | SL (a)
        uint16_t DeviceStatNotification;     //  0A          | Device Statistics Notification
                                             //  | DSN           | R/W   | GPL (b)
        uint16_t ReservedCFA2;               //  0B          | |               |       |
        uint16_t Reserved1;                  //  0C          | |               |       |
        uint16_t LPSMisAlignLog;             //  0D          | LPS Mis-alignment Log | LPS | RO |
                                             //  GPL,SL
        uint16_t Reserved2[2];               //  OE..0F      | |               |       |
        uint16_t NCQCmdErrLog;               //  10          | NCQ Command Error Log | NCQ | RO    |
                                             //  GPL (b)
        uint16_t SATAPhyEventCountLog;       //  11          | SATA Phy Event Counters Log
                                             //  | none          | RO    | GPL (b)
        uint16_t SATANCQQueueManageLog;      //  12          | SATA NCQ Queue Management Log |
                                             //  NCQ           | RO    | GPL (b)
        uint16_t SATANCQSendRecvLog;         //  13          | SATA NCQ Send & Receive Log |
                                             //  NCQ           | RO    | GPL (b)
        uint16_t ReservedSATA[4];            //  14..17      | Reserved for Serial ATA | | |
        uint16_t LBAStatus;                  //  18          | LBA Status | none          | RO    | GPL
                                             //  (b)
        uint16_t Reserved3[7];               //  19..20      | Reserved, 20h is Obsolete | |       |
        uint16_t WriteStreamErrLog;          //  21          | Write Stream Error Log |
                                             //  Streaming     | RO    | GPL (b)
        uint16_t ReadStreamErrLog;           //  22          | Read Stream Error Log | Streaming
                                             //  | RO    | GPL (b)
        uint16_t Obsolete1;                  //  23          | |               |       |
        uint16_t CurrDevInternalStsDataLog;  //  24          | Current Device Internal
                                             //  Status Data Log   | none          | RO
                                             //  | GPL (b)
        uint16_t SavedDevInternalStsDataLog; //  25          | Saved Device Internal
                                             //  Status Data Log     | none          |
                                             //  RO    | GPL (b)
        uint16_t Reserved4[10];              //  26..2F      | |               |       |
        uint16_t IdentifyDeviceData;         //  30          | IDENTIFY DEVICE data | none |
                                             //  RO    | GPL, SL
        uint16_t Reserved5[79];              //  31..7F      | |               |       |
        uint16_t HostSpecific[32];           //  80..9F      | Host Specific | SMART         |
                                             //  R/W   | GPL, SL
        uint16_t DeviceVendorSpecific[64];   //  A0..DF      | Device Vendor Specific |
                                             //  SMART         | VS    | GPL, SL
        uint16_t SCTCmdSts;                  //  E0          | SCT Command / Status | SCT           |
                                             //  R/W   | GPL, SL
        uint16_t SCTDataXfer;                //  E1          | SCT Data Transfer | SCT           |
                                             //  R/W   | GPL, SL
...
#    pragma pack(pop)
    ...
    typedef struct _ataSMARTValue
    {
        ...
        bool              thresholdDataValid; // new ATA specs no longer support the threshold
                                              // sector so some drives may not report thresholds
        bool isWarrantied;
    } ataSMARTValue;
...
#define ATA_SMART_LOG_MAX_ATTRIBUTES (256)
    ...
    {
        ataSMARTValue attributes[ATA_SMART_LOG_MAX_ATTRIBUTES]; // attribute numbers 1 - 255 are
                                                                // valid (check valid bit to
                                                                // make sure it's a used
                                                                // attribute)
    } ataSMARTLog;
...
// If bit 12 of word 59 is set to one the device supports the Sanitize Device
// feature set.
...
#define ATA_SANITIZE_INVERT_PAT_BIT_SET    (0x80)
    ...
    typedef enum _eATASetFeaturesSubcommands
    {
        ...
        SF_TLC_SET_CCTL = 0x20,           // set command completion time limit for devices supporting the old
                                          // time limited commands feature set
        SF_TCL_SET_ERROR_HANDLING = 0x21, // Sets error handling for devices supporting TLC and read/write
                                          // continuous mode
        ...
        SF_CFA_NOP_ACCEPTED_FOR_BACKWARDS_COMPATIBILITY = 0x69, // Likely defined in old manual if you can find it:
                                                                // SanDisk SDP Series OEM Manual, 1993
        ...
        SF_CFA_NOP_ACCEPTED_FOR_BACKWARDS_COMPATIBILITY_1 = 0x96, // Likely defined in old manual if you can find it:
                                                                  // SanDisk SDP Series OEM Manual, 1993
        SF_CFA_ACCEPTED_FOR_BACKWARDS_COMPATIBILITY =
            0x97, // In one old vendor unique CFA spec, this is set clock speed per
                  // sector cnt reg. Valid codes are 0 - divide by 4, A&B - divide by
                  // 2, E&F - divide by 1. I have never found a description of this
                  // any where else and I found this on accident looking for something
                  // completely different. - TJE
...
#define WRV_USER_MULTIPLIER                                                                                            \
    UINT16_C(1024) // sector count * this = number of sectors being verified in
                   // this mode.

    // this is the 7:3 bits of the count register for the SF_SET_TRANSFER_MODE
    // option Bits 2:0 can be used to specify the mode.
...
#define ATA_LOG_PAGE_LEN_BYTES                                                                                         \
    UINT16_C(512) // each page of a log is 512 bytes. A given log may be multiple
                  // pages long, or multiples of this value.
...
#define ATA_ID_DATA_SUP_PG_LIST_LEN_OFFSET                                                                             \
    UINT16_C(8) // this is the offset in the data where the list length is
                // specified to be read from. The next value is where the list of
                // supported pages begins.
#define ATA_ID_DATA_SUP_PG_LIST_OFFSET                                                                                 \
    UINT16_C(9)                              // when reading the ID Data log's list of supported pages, this is the
                                             // offset to start at to find the page numbers that are supported.
...
#define ATA_DEV_STATS_SUP_PG_LIST_OFFSET                                                                               \
    UINT16_C(9)                                      // when reading the device statistics log's list of supported
                                                     // pages, this is the offset to start at to find the page numbers
                                                     // that are supported.
...
#define DEVICE_CONTROL_nIEN       BIT3
    ...
    // Blank lines indicate where undefined/reserved values are located so it could
    // be easy to put those definitions in place as new specs are released
