...
extern "C"
{
    ...
    // This is a private function that is used by the send CDB function below in
    // order to contain printing debug output in one location instead of multiple so
    // that the ATA layer can also call this. Do not use this directly to send a
    ...
    //  scsi_Send_Cdb()
    //
    //! \brief   Description:  This is a function to allow a caller to send whatever
    //! CDB they want. It is also used by the other functions in this file to send
    //! commands
    //
    //  Entry:
    //!   \param device - pointer to the device structure containing a valid device
    //!   handle \param cdb - pointer to the array holding a CDB. MUST BE
    //!   NON-M_NULLPTR \param cdbLen - value indicating a CDB len. Must be of
    //!   eCDBLen type to be valid \param pdata - pointer to the data buffer. Can be
    //!   M_NULLPTR. If set to M_NULLPTR, dataLen must be set to 0 \param dataLen -
    //!   length of the data buffer to use with the specified command \param
    //!   dataDirection - the data transfer direction. Must be of type
    //!   eDataTransferDirection to be valid \param senseData - pointer to the sense
    //!   data buffer to be used. This can be M_NULLPTR. If set to M_NULLPTR, the
    //!   last command sense data in the device structure will be used. \param
    //!   senseDataLen - length of the sense data buffer. If set to 0, the last
    //!   command sense data in the device structure will be used. \param
    //!   timeoutSeconds - number of seconds to set for the command timeout to the
    //!   OS. If this is 0, 15 seconds will be set.
    //!
    //  Exit:
    ...
    //-----------------------------------------------------------------------------
    //
    //  uint16_t calculate_Logical_Block_Guard(uint8_t *buffer, uint32_t
    //  userDataLength, uint32_t totalDataLength)
    //
    //! \brief   Description:  This function will perform the SBC CRC16 calculation
    //! for a logical block guard used when a drive is formatted with PI. This
    //! should be used one sector at a time.
    //
    //  Entry:
    //!   \param buffer - pointer a buffer with user data
    //!   \param userDataLength - how many bytes of user data were returned (sector
    //!   size, IE 512 or 4096) \param totalDataLength - total length of the data
    //!   buffer being pointed to so we don't run over memory. Should be sector + 8
    //!   bytes.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    //-----------------------------------------------------------------------------
    OPENSEA_TRANSPORT_API uint16_t calculate_Logical_Block_Guard(uint8_t* buffer,
    ...
    //
    //! \brief   Description:  Check the Sense Key, ACQ, and ACSQ. Based on these
    //! values, return value. return value of SUCCESS, FAILURE, UNSUPPORTED, etc.
    //!                        This should be used to help judge pass/fail based off
    //!                        of these variables. If verbosity is set to
    //!                        VERBOSITY_COMMAND_NAMES, this function will print out
    //!                        what these inputs mean.
    //
    //  Entry:
    //!   \param device - pointer to the device structure. Used when deciding what
    //!   to show for the FRU code (more of a future expansion option if we start
    //!   parsing to human readable form) \param senseKey - senseKey \param asc -
    //!   additional code qualifier \param ascq - additional code sense qualifier
    //!   \param fru - field replaceable unit code (if available, just used for
    //!   verbose output, can always insert a 0 here)
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    OPENSEA_TRANSPORT_API void print_Field_Replacable_Unit_Code(tDevice*    device,
                                                                const char* fruMessage,
                                                                uint8_t     fruCode);
    ...
    //
    //! \brief   Description:  get the sense key, additional sense code, qualifier,
    //! and field replaceable unit code from the sense buffer
    //
    //  Entry:
    ...
    //!   \param ascq - pointer to the variable to hold the additional code sense
    //!   qualifier \param fru - pointer tot he variable to hold the field
    ...
    //-----------------------------------------------------------------------------
    //
    //  get_Sense_Key_Specific_Information(uint8_t *ptrSenseData, uint32_t
    //  senseDataLength, ptrSenseKeySpecific sksp)
    //
    //! \brief   Description:  Will get the sense key specific information from a
    //! sense data buffer if one is available.
    //
    //  Entry:
    //!   \param ptrSenseData - pointer to the sense buffer to analyze
    //!   \param senseDataLength - size of the sense buffer pointed to by
    //!   ptrSenseData \param sksp - pointer to the structure that will hold the
    //!   returned data. check the valid bit to make sure something was filled in,
    //!   use the type to parse the info out correctly
    //!
    //  Exit:
    //
    //-----------------------------------------------------------------------------
    ...
    //
    //! \brief   Description:  get the number of returned sense bytes from the sense
    //! data. For no sense data, 8 bytes will be returned, for vendor specific or
    //! unknown formats, 252 bytes will be returned, otherwise, the return value is
    //! 8 + additional sense length (byte 7 in fixed and descriptor format)
    //
    //  Entry:
    //!   \param pbuf - pointer to the sense buffer to analyze
    //!
    ...
    //!   \param[in] dataLength - length of data to request and length of data
    //!   buffer \param[in] pageCode - which vital product data page to request
    //!   (leave at 0 for standard inquiry data) \param[in] evpd - set to 1 to read
    //!   vital product data pages, set to 0 for standard inquiry data \param[in]
    //!   cmdDt - set to 1 to get command support information. NOTE: This was
    //!   declared obsolete in SPC3
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //  fill_In_Device_Info()
    //
    //! \brief   Description:  Sends a SCSI Inquiry and fills in the device
    //! information
    //
    //  Entry:
    ...
    //
    //-----------------------------------------------------------------------------
    OPENSEA_TRANSPORT_API eReturnValues fill_In_Device_Info(tDevice* device);
    ...
    //
    //! \brief   Description:  copy the maxLBA and logical blocksize from read
    //! capacity 10 or read capacity 16 information
    //
    //  Entry:
    //!   \param logicalBlockSize - pointer to the variable to set with the logical
    //!   block size \param physicalBlockSize - pointer to the variable to set with
    //!   the physical block size \param maxLBA - pointer to the variable to get set
    //!   \param sectorAlignment - pointer to the variable to set the sector
    //!   alignment \param ptrBuf - pointer to the data buffer containing VPD page
    //!   0x80 data \param readCap16 - set to true is the data buffer is from a read
    //!   capacity 16 command, otherwise set to false for a buffer that is from read
    //!   capacity 10
    //!
    //  Exit:
    //
    ...
    //  scsi_sanitize_cmd()
    //
    //! \brief   Description: Function to send a SCSI Sanitize command. Use the
    //! functions below this one for the features defined in SBC4 spec. Only call
    //! this if you know what you are doing.
    //
    //  Entry:
    //!   \param[in] device - pointer to the device structure
    //!   \param[in] sanitizeFeature - enum value specifying the sanitize service
    //!   action to perform \param[in] immediate - set to true to set the immediate
    //!   bit \param[in] znr - zone no reset bit. This is used on host managed and
    //!   host aware drives to not reset the zone pointers during a sanitize.
    //!   \param[in] ause - set to true to set the allow unrestricted sanitize exit
    //!   bit \param[in] parameterListLength - this should be 0 for all features
    //!   that are not an overwrite \param[in] ptrData - should be M_NULLPTR for all
    //!   features that are not an overwrite
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //! \brief   Description: Sends a SCSI sanitize overwrite erase command using
    //! scsi_Sanitize_Cmd. This command will allocate the data buffer for you with
    //! all the bits and patterns set according to the inputs
    //
    //  Entry:
    //!   \param[in] device - pointer to the device structure
    //!   \param[in] allowUnrestrictedSanitizeExit = set to true to set the allow
    //!   unrestricted sanitize exit bit \param[in] znr - zone no reset bit. This is
    //!   used on host managed and host aware drives to not reset the zone pointers
    //!   during a sanitize. \param[in] immediate = set to true to set the immediate
    //!   bit \param[in] invertBetweenPasses = set to true to set the invert between
    //!   passes in the transferred buffer \param[in] test = enum value specifying
    //!   the test bits. Should be SANITIZE_OVERWRITE_NO_CHANGES unless you know
    //!   what a vendor is expecting \param[in] overwritePasses = set to the number
    //!   of overwrite passes to perform. A value of zero is reserved \param[in]
    //!   pattern = pointer to an array containing your pattern seperated in bytes.
    //!   This will be copied for you to the transferred data buffer. may be
    //!   M_NULLPTR if patternLengthBytes is set to 0 \param[in] patternLengthBytes
    //!   = set to the length of your pattern in bytes. This length must be less
    //!   than the logical block length of the device as required by SBC4
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //! \brief   Description: Sends a SCSI sanitize exit failure mode command using
    //! scsi_Sanitize_Cmd
    //
    //  Entry:
    //!   \param[in] device - pointer to the device structure
    ...
    //
    //-----------------------------------------------------------------------------
    OPENSEA_TRANSPORT_API eReturnValues scsi_Sanitize_Exit_Failure_Mode(tDevice* device);

    //-----------------------------------------------------------------------------
    //
    //  scsi_Sanitize_Cryptographic_Erase()
    //
    //! \brief   Description: Sends a SCSI sanitize cryptographic erase command
    //! using scsi_Sanitize_Cmd
    //
    //  Entry:
    //!   \param[in] device - pointer to the device structure
    //!   \param[in] allowUnrestrictedSanitizeExit = set to true to set the allow
    //!   unrestricted sanitize exit bit \param[in] immediate = set to true to set
    //!   the immediate bit \param[in] znr - zone no reset bit. This is used on host
    //!   managed and host aware drives to not reset the zone pointers during a
    //!   sanitize.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //
    //! \brief   Description: Sends a SCSI sanitize block erase command using
    //! scsi_Sanitize_Cmd
    //
    //  Entry:
    //!   \param[in] device - pointer to the device structure
    ...
    //  Entry:
    //!   \param[in] device - pointer to the device structure
    //!   \param[in] descriptorBit - set to 1 to request descriptor format sense
    //!   data, 0 for fixed format sense data \param[out] pdata - pointer to the
    //!   data buffer to fill in with sense data \param[in] dataSize - size of the
    //!   data requested. Default should be 252
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param dataSize - size of the data buffer and amount of data to be
    ...
    //!   \param pageControl - the value of the page control field. Only bits 1:0
    //!   are valid \param pageCode - the logpage you wish to read \param
    //!   subpageCode - the subpage you wish to read \param parameterListLength -
    //!   the parameter list length \param ptrData - pointer to the data buffer to
    //!   transfer. Must be Non-M_NULLPTR \param dataSize - size of the data buffer
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param timeoutSeconds - timeout for the command. Useful for foreground
    //!   tests.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //!   \param device - pointer to the device structure
    //!   \param parameterListLength -
    //!   \param pageFormat - set to false when reading page 0 if it is not
    //!   formatted in page zero format. \param savePages - set to true to save the
    //!   mode page(s) to non-volatile memory. This may or may not be supported on
    //!   some drives. Some may only support saved pages \param resetToDefaults -
    //!   set to true when resetting all pages to defaults. No data shall be
    //!   transferred with this bit set, or an error will occur \param ptrData -
    //!   pointer to the data buffer to send \param dataSize - value describing
    //!   length of the data buffer being passed in
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param modeSpecific - See SPC spec for details since this depends on the
    //!   contents of the mode field \param bufferID - \param bufferOffset - \param
    //!   parameterListLength - \param ptrData - pointer to the data buffer to send
    //!   to the tDevice \param[in] firstSegment = Flag to help some low-level OSs
    //!   know when the first segment of a firmware download is
    //!   happening...specifically Windows \param[in] lastSegment = Flag to help
    //!   some low-level OSs know when the last segment of a firmware download is
    //!   happening...specifrically Windows \param[in] timeoutSeconds = set a
    //!   timeout in seconds for the command. This can be useful if some FWDL
    //!   commands take longer (code activation for example)
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //! \brief   Description:  Function to Send a SCSI Read Media Serial Number
    //! command
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param ptrData - pointer to the data buffer to fill upon command
    //!   completion \param allocationLength - length of the data buffer being sent
    //!   to the device and length being requested from the device
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param allocationLength - length of the data buffer being sent to the
    //!   device and length being requested from the device \param ptrData - pointer
    //!   to the data buffer to fill upon command completion
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //-----------------------------------------------------------------------------
    //
    //  scsi_Read_Buffer_16(tDevice *device, uint8_t mode, uint8_t modeSpecific,
    //  uint8_t bufferID, uint64_t bufferOffset, uint32_t allocationLength, uint8_t
    //  *ptrData)
    //
    ...
    //
    //! \brief   Description:  Function to Send a SCSI Receive Diagnostic Result
    //! command
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    ...
    //!   \param allocationLength - length of the data buffer being sent to the
    //!   device and length being requested from the device \param ptrData - pointer
    //!   to the data buffer to fill upon command completion \param timeoutSeconds -
    //!   number of seconds to wait for this command (at most) to complete. If not
    //!   sure, 0 will set the default of 15 seconds
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param parameterListLength - length of the data buffer being sent to the
    //!   device and length being requested from the device \param ptrData - pointer
    //!   to the data buffer to fill upon command completion \param dataSize - size
    //!   of the data buffer to use with the command
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //! \brief   Description:  Function to Send a SCSI Report Identifying
    //! Information command
    //
    //  Entry:
    //!   \param device - pointer to the device structure
    ...
    //!   \param allocationLength - length of the data buffer being sent to the
    //!   device and length being requested from the device \param
    //!   identifyingInformationType - See SPC4. Only bits 6:0 are valid (this
    //!   function will automatically shift the bits into the proper position for
    //!   the CDB) \param ptrData - pointer to the data buffer to fill upon command
    //!   completion
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //
    //! \brief   Description:  Function to Send a SCSI Report Supported Task
    ...
    //!   \param securityProtocolSpecific - any specific information to the security
    //!   protocol being used \param inc512 - set the inc512 bit. This means the
    //!   value in allocationLength field specifies the amount of data to transfer
    //!   is 512byte sizes \param ptrData - pointer to the data buffer that will do
    //!   the transfer \param allocationLength - size of the data to transfer
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //! \brief   Description:  Sends a SCSI Security Protocol Out command to a
    //! device
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param securityProtocol - security protocol being used
    //!   \param securityProtocolSpecific - any specific information to the security
    //!   protocol being used \param inc512 - set the inc512 bit. This means the
    //!   value in transferLength field specifies the amount of data to transfer is
    //!   512byte sizes \param ptrData - pointer to the data buffer that will do the
    //!   transfer \param transferLength - size of the data to transfer \param
    //!   timeout - time in seconds to wait for the command. (Added this for passing
    //!   through ATA Security commands)
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param identifyingInformationType - see SPC4. Only bits 6:0 are valid
    //!   since this function shifts this information into the appropriate place in
    //!   the CDB \param ptrData - pointer to the data buffer to send to the device
    ...
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param I_T_L_NexusToSet - only bits 1:0 are valid since this data gets
    //!   shifted when set in the CDB \param parameterListLength - length of the
    //!   data buffer \param ptrData - pointer to the data buffer to send to the
    ...
    //
    //-----------------------------------------------------------------------------
    OPENSEA_TRANSPORT_API eReturnValues scsi_Test_Unit_Ready(tDevice* device, scsiStatus* pReturnStatus);
    ...
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param wrprotect - bits 2:0 are valid since this data gets shifted into
    //!   position in this function \param dpo - set to true to set the dpo bit
    ...
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param fmtpInfo - bits 2:0 are valid since this data gets shifted into
    //!   position in this function \param longList - set to true to set the
    //!   longList bit \param fmtData - set to true to set the fmtData bit \param
    //!   cmplst - set to true to set the cmplst bit \param defectListFormat - set
    //!   to a value for the defect list format \param vendorSpecific - set to 0
    //!   unless you know what you need here \param ptrData - pointer to the data
    //!   buffer to send to the device \param dataSize - size of the data buffer to
    //!   transfer \param ffmt - set to the ffmt bit value you want. 0 - 3 are valid
    //!   values. See SBC4. For default format unit, set to 0 \param timeoutSeconds
    //!   - set to how long to wait for a format in seconds. If the immediate bit is
    //!   not set in the paramter data, then this should be a long time...otherwise
    ...
    //-----------------------------------------------------------------------------
    //
    //  scsi_Read_6(tDevice *device, uint32_t logicalBlockAddress, uint8_t
    //  transferLengthBlocks, uint8_t* ptrData, uint32_t transferLengthBytes);
    //
    ...
    //!   \param ptrData - pointer to the data buffer to fill. This MUST be at least
    //!   (transferLength * device.driveInfo.deviceBlockSize) in size or you will
    //!   have memory errors \param transferLengthBytes - number of bytes to
    //!   transfer. This should be transferLengthBlocks * sector size. If PI, add 8.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param transferLengthBlocks - number of logical blocks to transfer
    //!   \param ptrData - pointer to the data buffer to fill. This MUST be at least
    //!   (transferLengthBlocks * device.driveInfo.deviceBlockSize) in size or you
    //!   will have memory errors \param expectedInitialLogicalBlockRefTag - \param
    //!   expectedLogicalBlockAppTag - \param logicalBlockAppTagMask - \param
    //!   transferLengthBytes - number of bytes to transfer. This should be
    //!   transferLengthBlocks * sector size. If PI, add 8.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param defectListFormat - format of the defect list. Only bits 2:0 are
    //!   valid \param allocationLength - size of the data buffer and the amount of
    //!   data to be returned from the device \param ptrData - pointer to the data
    //!   buffer to fill. This MUST be at least (transferLength *
    ...
    //!   \param requestGList - set to true to set the request GList Bit
    //!   \param defectListFormat - format of the defect list. Only bits 2:0 are
    //!   valid \param addressDescriptorIndex - specifies the index of the first
    //!   address descriptor for the device to return \param allocationLength - size
    //!   of the data buffer and the amount of data to be returned from the device
    //!   \param ptrData - pointer to the data buffer to fill.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //  Entry:
    //!   \param device - pointer to the device structure
    //!   \param physicalBlock - set the physical block bit (only valid if device
    //!   supports multiple logical sectors per physical sector) \param correctBit -
    //!   set to correct bit (device will do error correction to retrieve the data,
    //!   but if that fails this command will abort) \param logicalBlockAddress -
    //!   The logical block address to read \param byteTransferLength - this MUST be
    //!   set to the logical block size. If the physicalBlock bit is set, this must
    //!   be set to the size of the physical block \param ptrData - pointer to the
    //!   data buffer to fill.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //!   \param longList - set the long list bit (short header or long header used
    //!   in the buffer) \param dataSize - size of the data buffer to transfer
    //!   \param ptrData - pointer to the data buffer to send to the device. This
    //!   buffer must be properly filled and formatted before issuing this command
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param numberOfLogicalBlocks - number of logical blocks to flush (set to 0
    //!   to flush all logical blocks starting with the LBA specified in the
    //!   logicalBlockAddress parameter)
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param groupNumber - the group number. Only bits 4:0 are valid
    //!   \param parameterListLength - length of the parameter list (and the data
    //!   buffer being sent to the device) \param ptrData - pointer to the data
    //!   buffer to transfer to the device
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param ptrData - pointer to the data buffer to transfer to the device. may
    //!   be M_NULLPTR when byteCheck is set to 0 \param dataSize - size of the data
    //!   Buffer. May be zero when byteCheck is set to 0
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param ptrData - pointer to the data buffer to transfer to the device. may
    //!   be M_NULLPTR when byteCheck is set to 0 \param dataSize - size of the data
    //!   Buffer. May be zero when byteCheck is set to 0 \param
    //!   expectedInitialLogicalBlockRefTag - \param expectedLogicalBlockAppTag -
    ...
    //-----------------------------------------------------------------------------
    //
    //  scsi_Write_6(tDevice *device, uint32_t logicalBlockAddress, uint8_t
    //  transferLengthBlocks, uint8_t* ptrData, uint32_t transferLengthBytes)
    //
    ...
    //!   \param ptrData - pointer to the data buffer to transfer to the device. may
    //!   be M_NULLPTR when byteCheck is set to 0 \param transferLengthBytes - size
    ...
    //!   \param logicalBlockAddress - LBA
    //!   \param groupNumber - the group number. Only bits 4:0 are valid
    //!   \param transferLengthBlocks - length of the data to write to the device in
    //!   logical blocks \param ptrData - pointer to the data buffer to transfer to
    //!   the device. \param transferLengthBytes - length of the data to write to
    //!   the device in bytes. May be larger than sectors for PI.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param logicalBlockAddress - LBA
    //!   \param groupNumber - the group number. Only bits 4:0 are valid
    //!   \param transferLengthBlocks - length of the data to write in logical
    //!   blocks \param ptrData - pointer to the data buffer to transfer to the
    //!   device. \param expectedInitialLogicalBlockRefTag - \param
    //!   expectedLogicalBlockAppTag - \param logicalBlockAppTagMask - \param
    //!   transferLengthBytes - length of the data to write to the device in bytes.
    //!   May be larger than sectors for PI.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param transferLengthBlocks - length of the data to write to the device in
    //!   logical blocks \param ptrData - pointer to the data buffer to transfer to
    //!   the device. \param transferLengthBytes - number of bytes to transfer with
    //!   the command
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //!   device. \param expectedInitialLogicalBlockRefTag - \param
    //!   expectedLogicalBlockAppTag - \param logicalBlockAppTagMask - \param
    //!   transferLengthBytes - number of bytes to transfer with the command
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   \param logicalBlockAddress - LBA
    //!   \param byteTransferLength - length of the data to write to the device.
    //!   Also the length of the ptrData \param ptrData - pointer to the data buffer
    ...
    //!   \param logicalBlockAddress - LBA
    //!   \param groupNumber -
    //!   \param numberOfLogicalBlocks - the number of logical blocks to write data
    //!   to \param ptrData - pointer to the data buffer to transfer to the device.
    //!   This buffer must be at least the logical block size of the device! \param
    //!   transferLengthBytes - number of bytes to transfer to the drive that were
    //!   allocated by ptrData
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   to \param ptrData - pointer to the data buffer to transfer to the device.
    //!   This buffer must be at least the logical block size of the device! \param
    //!   transferLengthBytes - number of bytes to transfer pointed to by ptrData
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   expectedInitialLogicalBlockRefTag - \param expectedLogicalBlockAppTag -
    //!   \param logicalBlockAppTagMask -
    //!   \param transferLengthBytes - number of bytes to transfer pointed to by
    //!   ptrData
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //!   \param transferLength - the length of the data to read/write/transfer.
    //!   Buffers must be this big \param ptrData - pointer to the data out buffer.
    //!   Must be non-M_NULLPTR
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //-----------------------------------------------------------------------------
    //
    //  eReturnValues scsi_Zone_Management_In_Report(tDevice* device, eZMAction
    //  action, uint8_t actionSpecific1, uint64_t location, bool partial, uint8_t
    //  reportingOptions, uint32_t allocationLength, uint8_t* ptrData)//95h
    //
    //! \brief   Description:  Sends a zone management in command to a device for
    //! report zones, report realms, and report zone domains since they share a
    //! mostly common format. Recommend using help functions below
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] action = set this to the zone management action to perform.
    //!   (enum is in common_public.h) \param[in] actionSpecific1 = set the action
    //!   specific bits in byte 1 \param[in] location = LBA, zone locator, realm
    //!   locator, etc \param[in] partial = partial bit for report zones. Reserved
    //!   otherwise \param[in] reportingOptions = reporting options. Specific to the
    //!   type of report being requested. \param[in] allocationLength = used on data
    //!   transfer commands. This is how many bytes to transfer. Should be 0 for
    //!   non-data commands \param[out] ptrData = pointer to the data buffer to use.
    //!   Can be M_NULLPTR for non-data actions
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    // Command to help with zone activate and zone query since they have mostly
    // common formatting. Recommend using helper function below
    ...
    //-----------------------------------------------------------------------------
    //
    //  scsi_Zone_Management_Out_Std_Format_CDB(tDevice *device, eZMAction action,
    //  uint8_t actionSpecific, uint32_t allocationLength, uint64_t
    //  actionSpecificLBA, uint8_t *ptrData)
    //
    //! \brief   Description:  Sends a zone management out command to a device that
    //! uses the common format for certain commands (recommend using helper
    //! functions below)
    //
    //  Entry:
    //!   \param[in] device = file descriptor
    //!   \param[in] action = set this to the zone management action to perform.
    //!   (enum is in common_public.h) \param[in] zoneID = zoneID field \param[in]
    //!   zoneCount = zone count to apply the action to. for backwards compatibiity
    //!   with ZBC, use zero. On ZBC2 and later values of 0 and 1 mean one zone.
    //!   \param[in] all = action affects all zones (zone ID and count should be
    //!   zero) \param[in] commandSPecific_10_11 = command specific bytes 10 and 11
    //!   \param[in] cmdSpecificBits1 = bits 7:5 in byte 1 of the CDB. All other
    //!   bits will be stripped off. \param[in] actionSpecific14 = set the action
    //!   specific bits. (cdb byte 14) \param[in] allocationLength = used on data
    //!   transfer commands. This is how many bytes to transfer. Should be 0 for
    //!   non-data commands

    //!   \param[in] ptrData = pointer to the data buffer to use. Can be M_NULLPTR
    //!   for non-data actions
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //!   backwards compatibiity with ZBC, use zero. On ZBC2 and later values of 0
    //!   and 1 mean one zone.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //!   compatibiity with ZBC, use zero. On ZBC2 and later values of 0 and 1 mean
    //!   one zone.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    ...
    //-----------------------------------------------------------------------------
    //
    //  scsi_Report_Zones_Ext(tDevice *device, eZoneReportingOptions
    //  reportingOptions, uint32_t allocationLength, uint64_t zoneLocator, uint8_t
    //  *ptrData)
    //
    ...
    //!   partial bit to 1 \param[in] allocationLength = This is a number of bytes
    //!   to transfer \param[in] zoneStartLBA = zone locator field. Set the an LBA
    //!   value for the lowest reported zone (0 for all zones) \param[out] ptrData =
    //!   pointer to the data buffer to use. Must be non-M_NULLPTR
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //-----------------------------------------------------------------------------
    //
    //  scsi_Reset_Write_Pointers_Ext(tDevice *device, bool resetAll, uint64_t
    //  zoneID)
    //
    ...
    //!   zoneID will be ignored by the device. \param[in] zoneID = the zoneID to
    //!   open \param[in] zoneCount = zone count to apply the action to. for
    //!   backwards compatibiity with ZBC, use zero. On ZBC2 and later values of 0
    //!   and 1 mean one zone.
    //!
    //  Exit:
...
#define MAX_VERSION_DESCRIPTOR_STRING_LENGTH 65
    //-----------------------------------------------------------------------------
    //
    //  decypher_SCSI_Version_Descriptors(uint16_t versionDescriptor, char*
    //  versionString)
    //
    //! \brief   Description:  Translates a version descriptor code into a string
    //! describing the specification it refers to. This uses versionDescriptor / 32
    //! to find the specification support. This does not take into account
    //! revisions.
    //
    //  Entry:
    //!   \param[in] versionDescriptor = word value that is a version descriptor
    //!   from the standard inquiry data. \param[out] versionString = pointer to a
    //!   char array that will hold a string describing the version. This should be
    //!   MAX_VERSION_DESCRIPTOR_STRING_LENGTH in size or larger
    //!
    //  Exit:
    //
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    //
    //  is_Standard_Supported(uint16_t versionDescriptor, eStandardCode
    //  standardCode)
    //
    //! \brief   Description:  Checks if the provided version descriptor is for the
    //! standardCode provided. This is checked by doing versionDescriptor / 32.
    //! Standard codes are from SPC specification.
    //
    //  Entry:
    //!   \param[in] versionDescriptor = word value that is a version descriptor
    //!   from the standard inquiry data. \param[in] standardCode = enum type or int
    //!   type that matches a standard code from SPC spec.
    //!
    //  Exit:
    //!   \return true = version descriptor is for standard code provided, false =
    //!   version descriptor does not match standard code.
    //
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    //
    //  scsi_Get_Physical_Element_Status(tDevice *device, uint32_t startingElement,
    //  uint32_t allocationLength, uint8_t filter, uint8_t reportType, uint8_t
    //  *ptrData)
    //
    ...
    //-----------------------------------------------------------------------------
    //
    //  scsi_Remove_And_Truncate(tDevice *device, uint64_t requestedCapacity,
    ...
    //!   capacity. Can be left as zero for drive to make this call \param[in]
    //!   elementIdentifier = identifier of the element to truncate
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //-----------------------------------------------------------------------------
    OPENSEA_TRANSPORT_API eReturnValues scsi_Restore_Elements_And_Rebuild(tDevice* device);

    //-----------------------------------------------------------------------------
    //
    //  scsi_Persistent_Reserve_In(tDevice *device, uint8_t serviceAction, uint16_t
    //  allocationLength, uint8_t *ptrData)
    //
    ...
    //!   \param[in] allocationLength = length of any data associated with the
    //!   service action to receive from the device \param[in] ptrData = pointer to
    //!   data buffer to issue to the device, if any.
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    //-----------------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------------
    //
    //  scsi_Persistent_Reserve_Out(tDevice *device, uint8_t serviceAction, uint8_t
    //  scope, uint8_t type,  uint32_t parameterListLength, uint8_t *ptrData)
    //
    ...
    //!   \param[in] type =
    //!   \param[in] parameterListLength = length of any data associated with the
    //!   service action to send to the device \param[in] ptrData = pointer to data
    //!   buffer to issue to the device, if any.
    //!
    //  Exit:
    //!   \return SUCCESS = pass, !SUCCESS = something when wrong
    //
    ...
    //
    //! \brief   Description:  Sends the SCSI Rezero Unit command. SCSI 2 mentions
    //! that this returns the unit to a good state.
    //
    //  Entry:
    //!   \param[in] device = pointer to device structure
    //!
    ...
    //
    //-----------------------------------------------------------------------------
    OPENSEA_TRANSPORT_API eReturnValues scsi_Rezero_Unit(tDevice* device);
